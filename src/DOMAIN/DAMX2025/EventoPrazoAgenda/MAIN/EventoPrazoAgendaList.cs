...
#region ListarDados_EventoPrazoAgenda
    public static async IAsyncEnumerable<DBEventoPrazoAgenda> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEventoPrazoAgenda(dbRec);
        }
    }

    public static IEnumerable<DBEventoPrazoAgenda> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
