...
public partial class PaisesService(IOptions<AppSettings> appSettings, IFPaisesFactory paisesFactory, IPaisesReader reader, IPaisesValidation validation, IPaisesWriter writer, IUFService ufService, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IPaisesService, IDisposable
{
    ...
    private async Task<PaisesResponse?> GetDataByIdAsync(int id, MsiSqlConnection? oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    ...
    public async Task<PaisesResponse?> Validation([FromBody] Models.Paises? regPaises, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regPaises.Id, oCnn);
    }
    ...
    public async Task<PaisesResponse?> Delete([FromQuery] int? id, [FromRoute, Required] string uri)
    {
        ...
        var paises = await reader.Read(id ?? default, oCnn);
        try
        {
            if (paises != null)
            {
                await writer.Delete(paises, nOperador, oCnn);
