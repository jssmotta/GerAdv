...
describe('CidadeApi', () => {
  ...
  beforeEach(() => {
    ...
    cidadeApi = new CidadeApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(cidadeApi).toBeInstanceOf(CidadeApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new CidadeApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(CidadeApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all cidade items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cidadeApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(cidadeApi.getAll()).rejects.toThrow(CidadeApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await cidadeApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Cidade_last_getAll_${200}`)));
    });
    ...
    it('should throw CidadeApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockCidade = CidadeTestEmpty();
    ...
    it('should fetch cidade by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cidadeApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockCidade));
      ...
      expect(result.data).toEqual(mockCidade);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cidade',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter cidade items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await cidadeApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new cidade successfully', async () => {
      const responseCidade = { ...mockCidade, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseCidade,
        status: 200,
        statusText: 'OK',
      ...
      const result = await cidadeApi.addAndUpdate(mockCidade);
    ...
    it('should update existing cidade successfully', async () => {
      const existingCidade = { ...mockCidade, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingCidade,
        status: 200,
        statusText: 'OK',
      ...
      const result = await cidadeApi.addAndUpdate(existingCidade);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(cidadeApi.addAndUpdate(mockCidade)).rejects.toThrow(CidadeApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cidade',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete cidade successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await cidadeApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(cidadeApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cidade',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(cidadeApi.delete(1)).rejects.toThrow(CidadeApiError);
    });
  });
  ...
  describe('CidadeApiError', () => {
    it('should create error with correct properties', () => {
      const error = new CidadeApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('CidadeApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new CidadeApiError('Test message', 500);
