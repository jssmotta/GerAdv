...
export const useProValoresList = (dataService: IProValoresService, currentFilter?: FilterProValores | undefined | null) => {
  const [data, setData] = useState<IProValores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterProValores | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterProValores | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterProValores);
      setData(result);
    } catch (err) {
