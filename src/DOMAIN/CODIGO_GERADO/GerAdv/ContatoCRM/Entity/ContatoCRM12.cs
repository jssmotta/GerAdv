...
public partial class FContatoCRM : MenphisSI.SG.GerAdv.DBContatoCRM, IDBContatoCRM
{
    // DI-compatible default constructor
    public FContatoCRM() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FContatoCRM CreateFromReader(SqlDataReader? dbRec)
    {
        return new FContatoCRM(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FContatoCRM CreateFromDataRow(DataRow? dbRec)
    {
        return new FContatoCRM(dbRec);
    }

    private FContatoCRM(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FContatoCRM(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FContatoCRM> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fContatoCRM = new FContatoCRM();
        await fContatoCRM.CarregarAsync(id, oCnn);
        return fContatoCRM;
    }

    private FContatoCRM(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FContatoCRM CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FContatoCRM(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
