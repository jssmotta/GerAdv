...
const BensClassificacaoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IBensClassificacao | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IBensClassificacao | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IBensClassificacao);
    ...
    const handleAddClick = () => {
        ...
        const add = BensClassificacaoEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <BensClassificacaoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedBensClassificacao={editar ?? addValue ?? BensClassificacaoEmpty()} /> 
            )}
