...
export class TipoStatusBiuApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'TipoStatusBiuApiError';
    this.status = status;
    this.code = code;
...
export class TipoStatusBiuApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoStatusBiu_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoStatusBiu');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoStatusBiu_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Staus  Usuários com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoStatusBiu_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoStatusBiu');
        }
    }

            public async filter(filtro: FilterTipoStatusBiu): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoStatusBiu_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoStatusBiu');
                }
            }


    public async validation(regTipoStatusBiu: ITipoStatusBiu): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoStatusBiu as TipoStatusBiu, this.getHeaders());
    ...
    public async addAndUpdate(regTipoStatusBiu: ITipoStatusBiu): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Staus  Usuários');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Staus  Usuários com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoStatusBiu) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<TipoStatusBiu[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de tipostatusbiu:', error);
            this.notificationService.notify({
              entity: 'TipoStatusBiu',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tipostatusbiu'
            });
          }
        });
    }
