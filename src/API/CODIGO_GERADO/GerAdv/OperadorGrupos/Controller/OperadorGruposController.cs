...
public partial class OperadorGruposController(IOperadorGruposService operadorgruposService) : ControllerBase
{
    ...
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] MenphisSI.GerAdv.Filters.FilterOperadorGrupos filter, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("OperadorGrupos: Filter called max {0} with filtro = {1}, {2}", max, filter, uri);
        var result = await _operadorgruposService.Filter(max, filter, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> FilterVoice([FromBody] MenphisSI.GerAdv.Filters.FilterOperadorGruposWithVoiceRequest request, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("OperadorGrupos: Filter called with {0} filtro = {1}", request, uri);
        var result = await _operadorgruposService.FilterVoice(request.Filter, request.VoiceCommand, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetAuditor(int id, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        try
        {
            //_logger.Info("OperadorGrupos: GetAuditor called with id = {0}, {1}", id, uri);
            var result = await _operadorgruposService.GetAuditor(id, uri, token);
            if (result == null)
            {
