...
public static class FilterEscritoriosTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Escritorios em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterEscritorios:

- O filter irá filtrar os Escritorios conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterEscritorios:(Escritorios)
...
- Se ouvir "todos os Escritorios" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterEscritorios? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Escritorios length: {result.Length}");
#endif
