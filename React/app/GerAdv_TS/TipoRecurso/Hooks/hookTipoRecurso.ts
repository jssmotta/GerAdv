...
export const useTipoRecursoForm = (
  ...
  const loadTipoRecurso = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoRecurso);
      return;
    }
  ...
  }, [dataService, initialTipoRecurso]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoRecurso, resetForm]);
  return returnValue;
};


export const useTipoRecursoNotifications = (
...
export const useTipoRecursoList = (dataService: ITipoRecursoService) => {
  const [data, setData] = useState<ITipoRecurso[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
