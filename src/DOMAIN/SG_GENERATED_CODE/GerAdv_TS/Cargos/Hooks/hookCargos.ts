...
export const useCargosList = (dataService: ICargosService, currentFilter?: FilterCargos | undefined | null) => {
  const [data, setData] = useState<ICargos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterCargos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterCargos | undefined | null) => {
