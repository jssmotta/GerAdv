...
public partial class AgendaRecordsController(IAgendaRecordsService agendarecordsService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.AgendaRecords regAgendaRecords, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaRecords: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaRecords: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.AgendaRecords regAgendaRecords, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _agendarecordsService.Validation(regAgendaRecords, uri);
            if (result == null)
            {
                _logger.Warn("AgendaRecords: Validation failed to add or update AgendaRecords, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "AgendaRecords: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}