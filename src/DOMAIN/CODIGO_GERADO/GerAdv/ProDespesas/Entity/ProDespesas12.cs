...
public partial class FProDespesas : MenphisSI.SG.GerAdv.DBProDespesas, IDBProDespesas
{
    // DI-compatible default constructor
    public FProDespesas() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FProDespesas CreateFromReader(SqlDataReader? dbRec)
    {
        return new FProDespesas(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FProDespesas CreateFromDataRow(DataRow? dbRec)
    {
        return new FProDespesas(dbRec);
    }

    private FProDespesas(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FProDespesas(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FProDespesas> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fProDespesas = new FProDespesas();
        await fProDespesas.CarregarAsync(id, oCnn);
        return fProDespesas;
    }

    private FProDespesas(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FProDespesas CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FProDespesas(parameters, oCnn, fullSql, sqlWhere, join);
    }
