...
/// <summary>
/// Comprehensive unit tests for ConfigureUpdateFields and ConfigureAuditorFields methods in DBProcessOutputSources class
...
/// </summary>
public class DBProcessOutputSourcesConfigureMethodsTests : IDisposable
{
    private readonly DBProcessOutputSources _processoutputsources;
    private readonly Mock<MsiSqlConnection> _mockConnection;
    public DBProcessOutputSourcesConfigureMethodsTests()
    {
        _processoutputsources = new DBProcessOutputSources();
    ...
    public void ConfigureUpdateFields_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        // Arrange
        ResetAllFieldFlags();
        _processoutputsources.ID = 1; // Update scenario
        // Act
        var result = _processoutputsources.Update(_mockConnection.Object);
    ...
    public async Task ConfigureUpdateFieldsAsync_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        ...
        try
        {
            var result = await _processoutputsources.UpdateAsync(_mockConnection.Object);
...
#region Helper Methods
    ...
    {
        var field = typeof(DBProcessOutputSources).GetField(fieldName, BindingFlags.NonPublic | BindingFlags.Instance);
        field?.SetValue(_processoutputsources, value);
    }
    ...
    private void SetFieldValue(string fieldName, object? value)
    {
        ...
        {
            field?.SetValue(_processoutputsources, Convert.ToDateTime(value));
        }
    }
...
#region Fields_All
    ...
    public void ConfigureUpdateFields_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _processoutputsources.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
    ...
    public async Task ConfigureUpdateFieldsAsync_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _processoutputsources.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _processoutputsources.UpdateAsync(_mockConnection.Object));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithCancellationToken_ShouldRespectCancellation()
    {
        ...
        var exception = await Record.ExceptionAsync(async () => await _processoutputsources.UpdateAsync(_mockConnection.Object, cancellationToken: cts.Token));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithTimeout_ShouldRespectTimeout()
    {
        ...
        _processoutputsources.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _processoutputsources.UpdateAsync(_mockConnection.Object, timeoutSeconds: 1));
        exception.Should().NotBeNull();
    }
    ...
    {
        // Arrange
        _processoutputsources.ID = 123; // Update scenario
        _processoutputsources.AuditorQuem = 7;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection but audit fields should be set
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        _processoutputsources.AuditorQuem.Should().Be(7);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemIsZero_ShouldSetToDefaultValue()
    {
        // Arrange
        _processoutputsources.ID = 0; // Insert scenario
        _processoutputsources.AuditorQuem = 0; // Should be set to 1
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        ...
        _processoutputsources.AuditorQuem.Should().Be(1);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemHasValue_ShouldPreserveValue(int auditorValue)
    {
        // Arrange
        _processoutputsources.ID = 0; // Insert scenario
        _processoutputsources.AuditorQuem = auditorValue;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        ...
        _processoutputsources.AuditorQuem.Should().Be(auditorValue);
    }
    ...
    {
        // Arrange
        _processoutputsources.ID = 0;
        _processoutputsources.AuditorQuem = 10;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        ...
        _processoutputsources.AuditorQuem.Should().Be(10);
    }
    ...
    {
        // Arrange
        _processoutputsources.ID = id;
        _processoutputsources.AuditorQuem = 15;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        ...
        _processoutputsources.AuditorQuem.Should().Be(15);
    }
    ...
    public void ConfigureMethodsNome_Integration_BothMethodsWorkTogether_ShouldNotInterfere()
    {
        ...
        SetAllFieldValues();
        _processoutputsources.ID = 123;
        _processoutputsources.AuditorQuem = 99;
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _processoutputsources.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        // Assert - Both update and audit configuration should work
        _processoutputsources.FNome.Should().Be("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        _processoutputsources.AuditorQuem.Should().Be(99);
    }
    ...
    public void ConfigureFieldsNome_Integration_FieldChangeTracking_ShouldWorkCorrectly()
    {
        ...
        var result1 = _processoutputsources.Update(_mockConnection.Object);
    ...
    public void ConfigureMethodsGUID_Integration_BothMethodsWorkTogether_ShouldNotInterfere()
    {
        ...
        exception.Should().NotBeNull();
        // Assert - Both update and audit configuration should work
        _processoutputsources.FGUID.Should().Be("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        _processoutputsources.AuditorQuem.Should().Be(99);
    }
