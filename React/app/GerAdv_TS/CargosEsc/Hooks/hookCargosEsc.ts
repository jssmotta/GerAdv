...
export const useCargosEscForm = (
  ...
  const loadCargosEsc = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Cargos Esc";
...
export const useCargosEscList = (
  dataService: ICargosEscService,
  currentFilter?: FilterCargosEsc | undefined | null,
) => {
  const [data, setData] = useState<ICargosEsc[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterCargosEsc | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterCargosEsc | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar cargosesc";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
