...
public partial class UltimosProcessosReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBUltimosProcessosDicInfo.CampoCodigo}";
        ...
        var query = $@"SELECT TOP ({max})
                   ...
                   FROM {DBUltimosProcessos.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBProcessosDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBProcessosDicInfo.PTabelaNome}].[{DBProcessos.CampoCodigo}]=[{DBUltimosProcessosDicInfo.PTabelaNome}].[{DBUltimosProcessosDicInfo.Processo}]
