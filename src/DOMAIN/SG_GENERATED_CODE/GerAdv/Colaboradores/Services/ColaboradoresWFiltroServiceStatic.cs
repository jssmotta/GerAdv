...
public partial class ColaboradoresService
{
    ...
    public async Task<ResultApi<IEnumerable<NomeID>>> GetListN(int max, Filters.FilterColaboradores? filtro, string uri, CancellationToken token = default)
    {
        // Tracking: 20250606-0
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-Colaboradores-{max}-{filterHash}-GetListN-{keyCache}";
    ...
    public async Task<ResultApi<IEnumerable<ColaboradoresResponseAll>>> GetAll(int max, string uri, CancellationToken token = default)
    {
        ThrowIfDisposed();
        ...
        {
            _logger.Error(ex, "Colaboradores: GetAll failed for uri = {0}", uri);
