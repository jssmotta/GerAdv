...
public partial class RecadosController(IRecadosService recadosService) : ControllerBase
{
    ...
    {
        _logger.Info("Recados: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _recadosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Recados: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _recadosService.UpdateColumns(parameters, uri);
        if (!result)
        {
