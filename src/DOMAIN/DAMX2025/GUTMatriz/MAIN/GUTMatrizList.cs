...
#region ListarDados_GUTMatriz
    public static async IAsyncEnumerable<DBGUTMatriz> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTMatriz(dbRec);
        }
    }

    public static IEnumerable<DBGUTMatriz> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
