...
import { EnderecoSistemaApiError } from '../GerAdv_TS/EnderecoSistema/Apis/ApiEnderecoSistema';
import { EnderecoSistemaApi } from '@/app/GerAdv_TS/EnderecoSistema/Apis/ApiEnderecoSistema';
import { FilterEnderecoSistema } from '../GerAdv_TS/EnderecoSistema/Filters/EnderecoSistema';
...
describe('EnderecoSistemaApi', () => {
  let enderecosistemaApi: EnderecoSistemaApi;
  ...
  beforeEach(() => {
    ...
    enderecosistemaApi = new EnderecoSistemaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(enderecosistemaApi).toBeInstanceOf(EnderecoSistemaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EnderecoSistemaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EnderecoSistemaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all enderecosistema items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enderecosistemaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(enderecosistemaApi.getAll()).rejects.toThrow(EnderecoSistemaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await enderecosistemaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-EnderecoSistema_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnderecoSistema',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw EnderecoSistemaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEnderecoSistema = EnderecoSistemaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockEnderecoSistema,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch enderecosistema by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enderecosistemaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEnderecoSistema));
      ...
      expect(result.data).toEqual(mockEnderecoSistema);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnderecoSistema',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterEnderecoSistema = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter enderecosistema items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await enderecosistemaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockEnderecoSistema: IEnderecoSistema = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new enderecosistema successfully', async () => {
      const responseEnderecoSistema = { ...mockEnderecoSistema, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEnderecoSistema,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enderecosistemaApi.addAndUpdate(mockEnderecoSistema);
    ...
    it('should update existing enderecosistema successfully', async () => {
      const existingEnderecoSistema = { ...mockEnderecoSistema, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEnderecoSistema,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enderecosistemaApi.addAndUpdate(existingEnderecoSistema);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(enderecosistemaApi.addAndUpdate(mockEnderecoSistema)).rejects.toThrow(EnderecoSistemaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnderecoSistema',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete enderecosistema successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await enderecosistemaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(enderecosistemaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnderecoSistema',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(enderecosistemaApi.delete(1)).rejects.toThrow(EnderecoSistemaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterEnderecoSistema = { funcionario: 1 };
      
      const result = enderecosistemaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await enderecosistemaApi.delete(1);
  ...
  describe('EnderecoSistemaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EnderecoSistemaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EnderecoSistemaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EnderecoSistemaApiError('Test message', 500);
