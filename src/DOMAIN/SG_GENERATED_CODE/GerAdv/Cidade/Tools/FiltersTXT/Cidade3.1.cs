...
public static class FilterCidadeTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Cidade em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterCidade:

- O filter irá filtrar os Cidade conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterUF: filtro específico para UF
...
- Se ouvir "todos os Cidade" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterCidade? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Cidade length: {result.Length}");
#endif
