...
public static class FilterStatusInstanciaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar StatusInstancia em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterStatusInstancia:

- O filter irá filtrar os StatusInstancia conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterStatusInstancia:(Status Instancia)
- nome: _nome ou descrição para Status Instancia;
- bold: Bold;
...
- Se ouvir "todos os StatusInstancia" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterStatusInstancia? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt StatusInstancia length: {result.Length}");
#endif
