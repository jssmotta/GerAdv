...
export const useOperadorEMailPopupForm = (
  ...
  const loadOperadorEMailPopup = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Operador E Mail Popup";
...
export const useOperadorEMailPopupList = (
  dataService: IOperadorEMailPopupService,
  currentFilter?: FilterOperadorEMailPopup | undefined | null,
) => {
  const [data, setData] = useState<IOperadorEMailPopup[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOperadorEMailPopup | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterOperadorEMailPopup | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar operadoremailpopup";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsOperadorEMailPopup() {
  async function runValidation(
    ...
    const operadoremailpopupApi = new OperadorEMailPopupApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IOperadorEMailPopup): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.smtp.length > 255) {
        return {
          isValid: false,
          message: "O campo SMTP não pode ter mais de 255 caracteres.",
        };
      }
      if (data.pop3.length > 255) {
        return {
          isValid: false,
          message: "O campo POP3 não pode ter mais de 255 caracteres.",
        };
      }
      if (data.descricao.length > 100) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 100 caracteres.",
        };
      }
      if (data.usuario.length > 50) {
        return {
          isValid: false,
          message: "O campo Usuario não pode ter mais de 50 caracteres.",
        };
      }
