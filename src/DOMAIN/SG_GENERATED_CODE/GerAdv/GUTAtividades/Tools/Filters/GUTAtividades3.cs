// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Tabela:GUTAtividades
// Fonte:FiltersApiGenerator2
namespace MenphisSI.GerAdv.Filters;
[Serializable]
[ExcludeFromCodeCoverage]
public class FilterGUTAtividadesWithVoiceRequest
{
    /// <summary>
    /// Filtro de GUTAtividades padrão
    /// </summary>
    [JsonPropertyName("filter")]
    public Filters.FilterGUTAtividades Filter { get; set; } = new();

    /// <summary>
    /// Comando de voz opcional para processamento via AI
    /// </summary>
    [JsonPropertyName("voiceCommand")]
    public CommandSpeakerRequest? VoiceCommand { get; set; }
}

[Serializable]
public partial record FilterGUTAtividades
{
    [JsonPropertyName("filterGUTPeriodicidade")]
    [Description("Filtro para a _tabela GUTPeriodicidade")]
    public FilterGUTPeriodicidade? FilterGUTPeriodicidade { get; set; } = new();

    [JsonPropertyName("filterOperador")]
    [Description("Filtro para a _tabela Operador")]
    public FilterOperador? FilterOperador { get; set; } = new();

    [JsonPropertyName("logicalOperator")]
    [Description(" ' AND ' OU ' OR ' ")]
    public string? LogicalOperator { get; set; } = TSql.And;

    [JsonPropertyName("wildcardChar")]
    [Description("Vazio ou '%'")]
    public char? WildcardChar { get; set; } = '%';

    [JsonPropertyName("nome")]
    [Description("Nome")]
    public string? Nome { get; set; } = string.Empty;

    [JsonPropertyName("observacao")]
    [Description("Observacao")]
    public string? Observacao { get; set; } = string.Empty;

    [JsonPropertyName("gutgrupo")]
    [Description("GUTGrupo -2147483648 para nada ou id")]
    public int GUTGrupo { get; set; } = int.MinValue;

    [JsonPropertyName("gutgrupo_end")]
    [Description("GUTGrupo -2147483648 para nada ou id para final do intervalo")]
    public int GUTGrupo_end { get; set; } = int.MinValue;

    [JsonPropertyName("gutperiodicidade")]
    [Description("GUTPeriodicidade -2147483648 para nada ou id")]
    public int GUTPeriodicidade { get; set; } = int.MinValue;

    [JsonPropertyName("gutperiodicidade_end")]
    [Description("GUTPeriodicidade -2147483648 para nada ou id para final do intervalo")]
    public int GUTPeriodicidade_end { get; set; } = int.MinValue;

    [JsonPropertyName("operador")]
    [Description("Operador -2147483648 para nada ou id")]
    public int Operador { get; set; } = int.MinValue;

    [JsonPropertyName("operador_end")]
    [Description("Operador -2147483648 para nada ou id para final do intervalo")]
    public int Operador_end { get; set; } = int.MinValue;

    [JsonPropertyName("concluido")]
    [Description("Concluido -2147483648 para nada; 0 para não, 1 para sim")]
    public int Concluido { get; set; } = int.MinValue;

    [JsonPropertyName("dataconcluido")]
    [Description("DataConcluido")]
    public string? DataConcluido { get; set; } = string.Empty;

    [JsonPropertyName("dataconcluido_end")]
    [Description("DataConcluido final para intervalo")]
    public string? DataConcluido_end { get; set; } = string.Empty;

    [JsonPropertyName("diasparainiciar")]
    [Description("DiasParaIniciar -2147483648 para nada ou id")]
    public int DiasParaIniciar { get; set; } = int.MinValue;

    [JsonPropertyName("diasparainiciar_end")]
    [Description("DiasParaIniciar -2147483648 para nada ou id para final do intervalo")]
    public int DiasParaIniciar_end { get; set; } = int.MinValue;

    [JsonPropertyName("minutospararealizar")]
    [Description("MinutosParaRealizar -2147483648 para nada ou id")]
    public int MinutosParaRealizar { get; set; } = int.MinValue;

    [JsonPropertyName("minutospararealizar_end")]
    [Description("MinutosParaRealizar -2147483648 para nada ou id para final do intervalo")]
    public int MinutosParaRealizar_end { get; set; } = int.MinValue;

    [JsonPropertyName("guid")]
    [Description("GUID")]
    public string? Guid { get; set; } = string.Empty;

    [JsonPropertyName("quemcad")]
    [Description("Usuário de Cadastro -2147483648 para nada ou id")]
    public int QuemCad { get; set; } = int.MinValue;

    [JsonPropertyName("quemcad_end")]
    [Description("Usuário de Cadastro -2147483648 para nada ou id para final do intervalo")]
    public int QuemCad_end { get; set; } = int.MinValue;

    [JsonPropertyName("dtcad")]
    [Description("Data de Cadastro")]
    public string? DtCad { get; set; } = string.Empty;

    [JsonPropertyName("dtcad_end")]
    [Description("Data de Cadastro final para intervalo")]
    public string? DtCad_end { get; set; } = string.Empty;

    [JsonPropertyName("quematu")]
    [Description("Usuário de Atualização -2147483648 para nada ou id")]
    public int QuemAtu { get; set; } = int.MinValue;

    [JsonPropertyName("quematu_end")]
    [Description("Usuário de Atualização -2147483648 para nada ou id para final do intervalo")]
    public int QuemAtu_end { get; set; } = int.MinValue;

    [JsonPropertyName("dtatu")]
    [Description("Data de Atualização")]
    public string? DtAtu { get; set; } = string.Empty;

    [JsonPropertyName("dtatu_end")]
    [Description("Data de Atualização final para intervalo")]
    public string? DtAtu_end { get; set; } = string.Empty;

    [JsonPropertyName("visto")]
    [Description("Visto -2147483648 para nada; 0 para não, 1 para sim")]
    public int Visto { get; set; } = int.MinValue;

    [JsonPropertyName("codigo_filtro")]
    [Description("Código inicial")]
    public int Codigo_filtro { get; set; } = int.MinValue;

    [JsonPropertyName("codigo_filtro_end")]
    [Description("Código final do intervalo")]
    public int Codigo_filtro_end { get; set; } = int.MinValue;
}