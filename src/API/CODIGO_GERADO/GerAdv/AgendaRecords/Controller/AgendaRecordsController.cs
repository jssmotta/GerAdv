...
public partial class AgendaRecordsController(IAgendaRecordsService agendarecordsService) : ControllerBase
{
    ...
    {
        _logger.Info("AgendaRecords: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _agendarecordsService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AgendaRecords: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _agendarecordsService.UpdateColumns(parameters, uri);
        if (!result)
        {
