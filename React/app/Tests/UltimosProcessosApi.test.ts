...
import { UltimosProcessosApiError } from '../GerAdv_TS/UltimosProcessos/Apis/ApiUltimosProcessos';
import { UltimosProcessosApi } from '@/app/GerAdv_TS/UltimosProcessos/Apis/ApiUltimosProcessos';
import { FilterUltimosProcessos } from '../GerAdv_TS/UltimosProcessos/Filters/UltimosProcessos';
...
describe('UltimosProcessosApi', () => {
  let ultimosprocessosApi: UltimosProcessosApi;
  ...
  beforeEach(() => {
    ...
    ultimosprocessosApi = new UltimosProcessosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(ultimosprocessosApi).toBeInstanceOf(UltimosProcessosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new UltimosProcessosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(UltimosProcessosApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [UltimosProcessosTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all ultimosprocessos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ultimosprocessosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(ultimosprocessosApi.getAll()).rejects.toThrow(UltimosProcessosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await ultimosprocessosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-UltimosProcessos_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UltimosProcessos',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw UltimosProcessosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockUltimosProcessos = UltimosProcessosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockUltimosProcessos,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch ultimosprocessos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ultimosprocessosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockUltimosProcessos));
      ...
      expect(result.data).toEqual(mockUltimosProcessos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UltimosProcessos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterUltimosProcessos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter ultimosprocessos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await ultimosprocessosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockUltimosProcessos: IUltimosProcessos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new ultimosprocessos successfully', async () => {
      const responseUltimosProcessos = { ...mockUltimosProcessos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseUltimosProcessos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await ultimosprocessosApi.addAndUpdate(mockUltimosProcessos);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockUltimosProcessos,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing ultimosprocessos successfully', async () => {
      const existingUltimosProcessos = { ...mockUltimosProcessos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingUltimosProcessos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await ultimosprocessosApi.addAndUpdate(existingUltimosProcessos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(ultimosprocessosApi.addAndUpdate(mockUltimosProcessos)).rejects.toThrow(UltimosProcessosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UltimosProcessos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete ultimosprocessos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await ultimosprocessosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(ultimosprocessosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UltimosProcessos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(ultimosprocessosApi.delete(1)).rejects.toThrow(UltimosProcessosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterUltimosProcessos = { funcionario: 1 };
      
      const result = ultimosprocessosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await ultimosprocessosApi.delete(1);
  ...
  describe('UltimosProcessosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new UltimosProcessosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('UltimosProcessosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new UltimosProcessosApiError('Test message', 500);
