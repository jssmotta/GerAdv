...
public static class FilterProSucumbenciaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProSucumbencia em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProSucumbencia:

- O filter irá filtrar os ProSucumbencia conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterInstancia: filtro específico para Instancia
- filterTipoOrigemSucumbencia: filtro específico para Tipo Origem Sucumbencia
...
CAMPOS filterInstancia (Instancia):
- liminarpedida: LiminarPedida;
- objeto: Objeto;
- statusresultado: número ou IDs;
...
- subdivisao: número ou IDs;
- principal: Principal;
- acao: número ou IDs;
- foro: número ou IDs;
- tiporecurso: número ou IDs;
- zkey: ZKey;
- zkeyquem: número ou IDs;
- zkeyquando: ZKeyQuando;
- nroantigo: NroAntigo;
- accesscode: AccessCode;
- julgador: número ou IDs;
- zkeyia: ZKeyIA;
...
CAMPOS filterTipoOrigemSucumbencia (Tipo Origem Sucumbencia):
- nome: _nome ou descrição para Tipo Origem Sucumbencia;

CAMPOS filterProSucumbencia:(Pro Sucumbencia)
- processo: número ou IDs;
- instancia: número ou IDs;
- data: não é a data de cadastramento é a data do ProSucumbencia;
- nome: Nome;
- tipoorigemsucumbencia: número ou IDs;
...
- Se ouvir "todos os ProSucumbencia" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProSucumbencia? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProSucumbencia length: {result.Length}");
#endif
