...
export const useProResumosForm = (
  ...
  const loadProResumos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Pro Resumos";
...
export const useProResumosList = (
  dataService: IProResumosService,
  currentFilter?: FilterProResumos | undefined | null,
) => {
  const [data, setData] = useState<IProResumos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProResumos | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterProResumos | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar proresumos";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IProResumos): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.resumo.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo Resumo não pode ter mais de 2147483647 caracteres.",
        };
      }
