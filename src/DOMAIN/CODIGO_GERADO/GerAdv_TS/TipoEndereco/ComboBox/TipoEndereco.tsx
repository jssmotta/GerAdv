...
const TipoEnderecoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, descricao: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as TipoEndereco;
        const novoDado = await dadoApi.addAndUpdate(nova as ITipoEndereco);
