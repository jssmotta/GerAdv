...
public partial class TipoCompromissoController(ITipoCompromissoService tipocompromissoService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterTipoCompromisso filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("TipoCompromisso: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _tipocompromissoService.Filter(max, filtro, uri);
        return Ok(result);
    }
