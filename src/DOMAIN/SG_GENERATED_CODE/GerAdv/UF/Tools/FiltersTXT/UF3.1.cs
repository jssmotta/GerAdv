...
public static class FilterUFTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar UF em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterUF:

- O filter irá filtrar os UF conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterPaises: filtro específico para Paises
...
CAMPOS filterPaises (Paises):
- nome: _nome ou descrição para Paises;
...
CAMPOS filterUF:(UF)
- ddd: DDD;
- iduf: _nome ou descrição para UF;
- pais: número ou IDs;
- top: Top;
...
- Se ouvir "todos os UF" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterUF? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt UF length: {result.Length}");
#endif
