...
public class FTipoContatoCRMFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FTipoContatoCRMFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FTipoContatoCRMFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FTipoContatoCRMFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "tccCodigo")
        var dt = new DataTable();
        dt.Columns.Add("tccCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["tccCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FTipoContatoCRM>();
    }
    ...
    {
        // Arrange
        var factory = new FTipoContatoCRMFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("tccCodigo", typeof(int));
        mockDataTable.Columns.Add("tccNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["tccCodigo"] = testId;
        row["tccNome"] = "Test TipoContatoCRM";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["tccCodigo"] = 123;
        row["tccNome"] = "Test TipoContatoCRM Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FTipoContatoCRM properties
        }
    }
