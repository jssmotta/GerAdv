...
import { ClientesSociosApiError } from '../GerAdv_TS/ClientesSocios/Apis/ApiClientesSocios';
import { ClientesSociosApi } from '@/app/GerAdv_TS/ClientesSocios/Apis/ApiClientesSocios';
import { FilterClientesSocios } from '../GerAdv_TS/ClientesSocios/Filters/ClientesSocios';
...
describe('ClientesSociosApi', () => {
  let clientessociosApi: ClientesSociosApi;
  ...
  beforeEach(() => {
    ...
    clientessociosApi = new ClientesSociosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(clientessociosApi).toBeInstanceOf(ClientesSociosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ClientesSociosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ClientesSociosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all clientessocios items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await clientessociosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(clientessociosApi.getAll()).rejects.toThrow(ClientesSociosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await clientessociosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ClientesSocios_last_getAll_${200}`)));
    });
    ...
    it('should throw ClientesSociosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockClientesSocios = ClientesSociosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockClientesSocios,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch clientessocios by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await clientessociosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockClientesSocios));
      ...
      expect(result.data).toEqual(mockClientesSocios);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ClientesSocios',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterClientesSocios = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter clientessocios items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await clientessociosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockClientesSocios: IClientesSocios = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new clientessocios successfully', async () => {
      const responseClientesSocios = { ...mockClientesSocios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseClientesSocios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await clientessociosApi.addAndUpdate(mockClientesSocios);
    ...
    it('should update existing clientessocios successfully', async () => {
      const existingClientesSocios = { ...mockClientesSocios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingClientesSocios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await clientessociosApi.addAndUpdate(existingClientesSocios);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(clientessociosApi.addAndUpdate(mockClientesSocios)).rejects.toThrow(ClientesSociosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ClientesSocios',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete clientessocios successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await clientessociosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(clientessociosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ClientesSocios',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(clientessociosApi.delete(1)).rejects.toThrow(ClientesSociosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterClientesSocios = { funcionario: 1 };
      
      const result = clientessociosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await clientessociosApi.delete(1);
  ...
  describe('ClientesSociosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ClientesSociosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ClientesSociosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ClientesSociosApiError('Test message', 500);
