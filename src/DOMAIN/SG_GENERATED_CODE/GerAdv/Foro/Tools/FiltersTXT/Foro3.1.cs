...
public static class FilterForoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Foro em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterForo:

- O filter irá filtrar os Foro conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Foro" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterForo? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Foro length: {result.Length}");
#endif
