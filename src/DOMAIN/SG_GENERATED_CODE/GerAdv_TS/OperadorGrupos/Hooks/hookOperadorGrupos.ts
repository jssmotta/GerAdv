...
export const useOperadorGruposList = (dataService: IOperadorGruposService, currentFilter?: FilterOperadorGrupos | undefined | null) => {
  const [data, setData] = useState<IOperadorGrupos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterOperadorGrupos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterOperadorGrupos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterOperadorGrupos);
      setData(result);
    } catch (err) {
