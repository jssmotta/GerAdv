...
public partial class DBLivroCaixa
{
    ...
    /// </summary>
    public static async Task<DBLivroCaixa> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBLivroCaixa();
...
#if (!NOTSTORED_LivroCaixa)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFIDDes || pFldFPessoal || pFldFAjuste || pFldFIDHon || pFldFIDHonParc || pFldFIDHonSuc || pFldFData || pFldFProcesso || pFldFValor || pFldFTipo || pFldFHistorico || pFldFPrevisto || pFldFGrupo;
