...
public partial class ClientesSocios
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(cscDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.ClientesSocios WITH (NOLOCK, INDEX = idx_ClientesSocios_AuditorDtAtu)
WHERE cscCodigo = @id
...
FROM {oCnn.UseDbo}.ClientesSocios WITH (NOLOCK)
