...
export const useServicosList = (
  dataService: IServicosService,
  currentFilter?: FilterServicos | undefined | null,
) => {
  const [data, setData] = useState<IServicos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterServicos | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IServicos): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.descricao.length > 200) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 200 caracteres.",
        };
      }
