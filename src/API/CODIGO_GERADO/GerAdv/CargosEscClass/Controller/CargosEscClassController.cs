...
public partial class CargosEscClassController(ICargosEscClassService cargosescclassService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterCargosEscClass filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("CargosEscClass: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _cargosescclassService.Filter(max, filtro, uri);
        return Ok(result);
    }
