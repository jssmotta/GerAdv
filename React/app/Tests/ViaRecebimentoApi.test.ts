...
import { ViaRecebimentoApiError } from '../GerAdv_TS/ViaRecebimento/Apis/ApiViaRecebimento';
import { ViaRecebimentoApi } from '@/app/GerAdv_TS/ViaRecebimento/Apis/ApiViaRecebimento';
import { FilterViaRecebimento } from '../GerAdv_TS/ViaRecebimento/Filters/ViaRecebimento';
...
describe('ViaRecebimentoApi', () => {
  let viarecebimentoApi: ViaRecebimentoApi;
  ...
  beforeEach(() => {
    ...
    viarecebimentoApi = new ViaRecebimentoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(viarecebimentoApi).toBeInstanceOf(ViaRecebimentoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ViaRecebimentoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ViaRecebimentoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all viarecebimento items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await viarecebimentoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(viarecebimentoApi.getAll()).rejects.toThrow(ViaRecebimentoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await viarecebimentoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ViaRecebimento_last_getAll_${200}`)));
    });
    ...
    it('should throw ViaRecebimentoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockViaRecebimento = ViaRecebimentoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockViaRecebimento,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch viarecebimento by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await viarecebimentoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockViaRecebimento));
      ...
      expect(result.data).toEqual(mockViaRecebimento);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ViaRecebimento',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterViaRecebimento = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter viarecebimento items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await viarecebimentoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockViaRecebimento: IViaRecebimento = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new viarecebimento successfully', async () => {
      const responseViaRecebimento = { ...mockViaRecebimento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseViaRecebimento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await viarecebimentoApi.addAndUpdate(mockViaRecebimento);
    ...
    it('should update existing viarecebimento successfully', async () => {
      const existingViaRecebimento = { ...mockViaRecebimento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingViaRecebimento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await viarecebimentoApi.addAndUpdate(existingViaRecebimento);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(viarecebimentoApi.addAndUpdate(mockViaRecebimento)).rejects.toThrow(ViaRecebimentoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ViaRecebimento',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete viarecebimento successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await viarecebimentoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(viarecebimentoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ViaRecebimento',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(viarecebimentoApi.delete(1)).rejects.toThrow(ViaRecebimentoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterViaRecebimento = { funcionario: 1 };
      
      const result = viarecebimentoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await viarecebimentoApi.delete(1);
  ...
  describe('ViaRecebimentoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ViaRecebimentoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ViaRecebimentoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ViaRecebimentoApiError('Test message', 500);
