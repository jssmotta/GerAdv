...
export class OponentesRepLegalApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterOponentesRepLegal): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OponentesRepLegal_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterOponentesRepLegal) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<OponentesRepLegal[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de oponentesreplegal:', error);
            this.notificationService.notify({
              entity: 'OponentesRepLegal',
              id: 0,
              action: NotifySystemActions.ERROR,
