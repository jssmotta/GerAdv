...
{
    public (string where, List<SqlParameter> parametros)? WFiltroEnderecos(Filters.FilterEnderecos? filtro)
    {
        if (filtro == null)
            return null;
        ...
        {
            parameters.Add(new($"@{(DBEnderecosDicInfo.QuemCad)}", filtro.QuemCad));
            if (filtro.QuemCad_end != int.MinValue)
            {
                parameters.Add(new($"@{(DBEnderecosDicInfo.QuemCad)}_end", filtro.QuemCad_end));
            }
        }
        ...
        if (!filtro.DtCad.IsEmptyDX())
        {
            if (DateTime.TryParse(filtro.DtCad, out var dataParam1))
                parameters.Add(new($"@{(DBEnderecosDicInfo.DtCad)}", dataParam1));
            if (!filtro.DtCad_end.IsEmptyDX())
            {
                if (DateTime.TryParse(filtro.DtCad_end, out var dataParam2))
                    parameters.Add(new($"@{(DBEnderecosDicInfo.DtCad)}_end", dataParam2));
            }
        }

        if (filtro.QuemAtu != int.MinValue)
        {
            parameters.Add(new($"@{(DBEnderecosDicInfo.QuemAtu)}", filtro.QuemAtu));
            if (filtro.QuemAtu_end != int.MinValue)
            {
                parameters.Add(new($"@{(DBEnderecosDicInfo.QuemAtu)}_end", filtro.QuemAtu_end));
            }
        }
        ...
        if (!filtro.DtAtu.IsEmptyDX())
        {
            if (DateTime.TryParse(filtro.DtAtu, out var dataParam3))
                parameters.Add(new($"@{(DBEnderecosDicInfo.DtAtu)}", dataParam3));
            if (!filtro.DtAtu_end.IsEmptyDX())
            {
                if (DateTime.TryParse(filtro.DtAtu_end, out var dataParam4))
                    parameters.Add(new($"@{(DBEnderecosDicInfo.DtAtu)}_end", dataParam4));
            }
        }

        if (filtro.Visto != int.MinValue)
        {
            parameters.Add(new($"@{(DBEnderecosDicInfo.Visto)}", filtro.Visto));
        }
        ...
        if (!filtro.DtNasc.IsEmptyDX())
        {
            if (DateTime.TryParse(filtro.DtNasc, out var dataParam5))
                parameters.Add(new($"@{(DBEnderecosDicInfo.DtNasc)}", dataParam5));
            if (!filtro.DtNasc_end.IsEmptyDX())
            {
                if (DateTime.TryParse(filtro.DtNasc_end, out var dataParam6))
                    parameters.Add(new($"@{(DBEnderecosDicInfo.DtNasc)}_end", dataParam6));
            }
        }
        ...
        {
            cWhere.Append(filtro.QuemCad.IsEmptyX() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].[{DBEnderecosDicInfo.QuemCad}] = @{(DBEnderecosDicInfo.QuemCad)}");
        }
        else if (!(filtro.QuemCad.IsEmptyX()) && !(filtro.QuemCad_end.IsEmptyX()))
        {
            cWhere.Append((cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].{DBEnderecosDicInfo.QuemCad} BETWEEN @{(DBEnderecosDicInfo.QuemCad)} AND @{(DBEnderecosDicInfo.QuemCad)}_end");
        }

        if (!(filtro.DtCad.IsEmptyDX()) && filtro.DtCad_end.IsEmptyDX())
        {
            cWhere.Append(filtro.DtCad.IsEmptyDX() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"CONVERT(DATE,[{DBEnderecosDicInfo.PTabelaNome}].[{DBEnderecosDicInfo.DtCad}], 103) = CONVERT(DATE, @{(DBEnderecosDicInfo.DtCad)}, 103)");
        }
        else if (!(filtro.DtCad.IsEmptyDX()) && !(filtro.DtCad_end.IsEmptyDX()))
        {
            cWhere.Append((cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].{DBEnderecosDicInfo.DtCad} BETWEEN @{(DBEnderecosDicInfo.DtCad)} AND @{(DBEnderecosDicInfo.DtCad)}_end");
        }

        if (!(filtro.QuemAtu.IsEmptyX()) && filtro.QuemAtu_end.IsEmptyX())
        {
            cWhere.Append(filtro.QuemAtu.IsEmptyX() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].[{DBEnderecosDicInfo.QuemAtu}] = @{(DBEnderecosDicInfo.QuemAtu)}");
        }
        else if (!(filtro.QuemAtu.IsEmptyX()) && !(filtro.QuemAtu_end.IsEmptyX()))
        {
            cWhere.Append((cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].{DBEnderecosDicInfo.QuemAtu} BETWEEN @{(DBEnderecosDicInfo.QuemAtu)} AND @{(DBEnderecosDicInfo.QuemAtu)}_end");
        }

        if (!(filtro.DtAtu.IsEmptyDX()) && filtro.DtAtu_end.IsEmptyDX())
        {
            cWhere.Append(filtro.DtAtu.IsEmptyDX() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"CONVERT(DATE,[{DBEnderecosDicInfo.PTabelaNome}].[{DBEnderecosDicInfo.DtAtu}], 103) = CONVERT(DATE, @{(DBEnderecosDicInfo.DtAtu)}, 103)");
        }
        else if (!(filtro.DtAtu.IsEmptyDX()) && !(filtro.DtAtu_end.IsEmptyDX()))
        {
            cWhere.Append((cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].{DBEnderecosDicInfo.DtAtu} BETWEEN @{(DBEnderecosDicInfo.DtAtu)} AND @{(DBEnderecosDicInfo.DtAtu)}_end");
        }

        cWhere.Append(filtro.Visto == int.MinValue ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBEnderecosDicInfo.PTabelaNome}].[{DBEnderecosDicInfo.Visto}] = @{(DBEnderecosDicInfo.Visto)}");
...
public partial class EnderecosService
{
    ...
    /// <returns>FilterEnderecos processado</returns>
    private async Task<Filters.FilterEnderecos?> ProcessFilterVoiceCommandAsync(CommandSpeakerRequest message, Filters.FilterEnderecos? existingFilter, string uri)
    {
        try
        {
            // Usar o contexto do FilterEnderecosTXT
            var context = FilterEnderecosTXT.GetPrompt(existingFilter, FilterEnderecosTXT.GetOptions());
            // Instruções específicas para Enderecos
        ...
        catch (Exception)
        {
            ...
            return existingFilter ?? new Filters.FilterEnderecos();
        }
    }

    private async Task<IEnumerable<EnderecosResponseAll>> GetDataAllAsync(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken cancellationToken)
    {
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter>? parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarNAsync(max, uri, where, parameters, DBEnderecosDicInfo.CampoNome);
    ...
    public async Task<IEnumerable<EnderecosResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        using var oCnn = scope.Connection;
        var keyCache = await reader.ReadStringAuditorAsync(max, uri, "", [], oCnn);
        var cacheKey = $"{uri}-Enderecos-Filter-{max}-{keyCache}";
    ...
    private async Task<IEnumerable<EnderecosResponseAll>> GetDataAllAsync2(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken token)
    {
        using var scope = _connectionService.CreateConnectionScope(uri);
        using var oCnn = scope.Connection;
        if (oCnn == null)
