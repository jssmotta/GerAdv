...
#region ListarDados_PosicaoOutrasPartes
    public static async IAsyncEnumerable<DBPosicaoOutrasPartes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPosicaoOutrasPartes(dbRec);
        }
    }

    public static IEnumerable<DBPosicaoOutrasPartes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
