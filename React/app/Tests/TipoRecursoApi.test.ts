...
describe('TipoRecursoApi', () => {
  let tiporecursoApi: TipoRecursoApi;
  ...
  beforeEach(() => {
    ...
    tiporecursoApi = new TipoRecursoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tiporecursoApi).toBeInstanceOf(TipoRecursoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoRecursoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoRecursoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tiporecurso items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tiporecursoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tiporecursoApi.getAll()).rejects.toThrow(TipoRecursoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tiporecursoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoRecurso_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoRecursoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoRecurso = TipoRecursoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoRecurso,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tiporecurso by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tiporecursoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoRecurso));
      ...
      expect(result.data).toEqual(mockTipoRecurso);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoRecurso',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter tiporecurso items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tiporecursoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new tiporecurso successfully', async () => {
      const responseTipoRecurso = { ...mockTipoRecurso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoRecurso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tiporecursoApi.addAndUpdate(mockTipoRecurso);
    ...
    it('should update existing tiporecurso successfully', async () => {
      const existingTipoRecurso = { ...mockTipoRecurso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoRecurso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tiporecursoApi.addAndUpdate(existingTipoRecurso);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tiporecursoApi.addAndUpdate(mockTipoRecurso)).rejects.toThrow(TipoRecursoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoRecurso',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tiporecurso successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tiporecursoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tiporecursoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoRecurso',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tiporecursoApi.delete(1)).rejects.toThrow(TipoRecursoApiError);
    });
  });
  ...
  describe('TipoRecursoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoRecursoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoRecursoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoRecursoApiError('Test message', 500);
