...
public partial class ProPartesController(IProPartesService propartesService) : ControllerBase
{
    ...
    {
        _logger.Info("ProPartes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _propartesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProPartes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _propartesService.UpdateColumns(parameters, uri);
        if (!result)
        {
