...
public partial class AgendaRepetirService(IOptions<AppSettings> appSettings, IFAgendaRepetirFactory agendarepetirFactory, IAgendaRepetirReader reader, IAgendaRepetirValidation validation, IAgendaRepetirWriter writer, IAdvogadosReader advogadosReader, IClientesReader clientesReader, IFuncionariosReader funcionariosReader, IProcessosReader processosReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IAgendaRepetirService, IDisposable
{
    ...
    private readonly IFAgendaRepetirFactory agendarepetirFactory = agendarepetirFactory;
    private readonly IAgendaRepetirReader reader = reader;
    private readonly IAgendaRepetirValidation validation = validation;
    private readonly IAgendaRepetirWriter writer = writer;
    ...
    public async Task<IEnumerable<AgendaRepetirResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-AgendaRepetir-Filter-{keyCache}";
    ...
    private async Task<AgendaRepetirResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<AgendaRepetirResponse?> AddAndUpdate([FromBody] Models.AgendaRepetir regAgendaRepetir, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regAgendaRepetir, operadorId, oCnn);
    ...
    public async Task<AgendaRepetirResponse?> Validation([FromBody] Models.AgendaRepetir regAgendaRepetir, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regAgendaRepetir.Id, oCnn);
    }
    ...
    public async Task<AgendaRepetirResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var agendarepetir = await reader.Read(id, oCnn);
        try
        {
            if (agendarepetir != null)
            {
                await writer.Delete(agendarepetir, nOperador, oCnn);
