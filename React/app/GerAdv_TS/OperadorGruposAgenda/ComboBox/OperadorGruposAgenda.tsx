...
const OperadorGruposAgendaComboBox: React.FC<DadosSelectProps> = ({
  ...
  const [editRecord, setEditRecord] = useState<IOperadorGruposAgenda | null>(
    null,
  );
  const [addRecord, setAddRecord] = useState<IOperadorGruposAgenda | null>(
    null,
  );
  ...
  const loadRecordForEdit = async (id: number) => {
    try {
      const record =
        await operadorgruposagendaService.fetchOperadorGruposAgendaById(id);
  ...
  const handleSuccess = (newOperadorGruposAgenda?: IOperadorGruposAgenda) => {
    if (newOperadorGruposAgenda) {
      setValue({
        id: newOperadorGruposAgenda.id,
        nome: newOperadorGruposAgenda.nome,
      });
      ...
      {(editRecord || addRecord) && isOpen && (
        <OperadorGruposAgendaWindow
          ...
          selectedOperadorGruposAgenda={
            editRecord || addRecord || OperadorGruposAgendaEmpty()
          }
        />
      )}
