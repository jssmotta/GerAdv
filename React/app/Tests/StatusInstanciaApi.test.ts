...
import { StatusInstanciaApiError } from '../GerAdv_TS/StatusInstancia/Apis/ApiStatusInstancia';
import { StatusInstanciaApi } from '@/app/GerAdv_TS/StatusInstancia/Apis/ApiStatusInstancia';
import { FilterStatusInstancia } from '../GerAdv_TS/StatusInstancia/Filters/StatusInstancia';
...
describe('StatusInstanciaApi', () => {
  let statusinstanciaApi: StatusInstanciaApi;
  ...
  beforeEach(() => {
    ...
    statusinstanciaApi = new StatusInstanciaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(statusinstanciaApi).toBeInstanceOf(StatusInstanciaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new StatusInstanciaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(StatusInstanciaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all statusinstancia items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusinstanciaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(statusinstanciaApi.getAll()).rejects.toThrow(StatusInstanciaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await statusinstanciaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-StatusInstancia_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusInstancia',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw StatusInstanciaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockStatusInstancia = StatusInstanciaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockStatusInstancia,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch statusinstancia by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusinstanciaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockStatusInstancia));
      ...
      expect(result.data).toEqual(mockStatusInstancia);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusInstancia',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterStatusInstancia = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter statusinstancia items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await statusinstanciaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockStatusInstancia: IStatusInstancia = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new statusinstancia successfully', async () => {
      const responseStatusInstancia = { ...mockStatusInstancia, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseStatusInstancia,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusinstanciaApi.addAndUpdate(mockStatusInstancia);
    ...
    it('should update existing statusinstancia successfully', async () => {
      const existingStatusInstancia = { ...mockStatusInstancia, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingStatusInstancia,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusinstanciaApi.addAndUpdate(existingStatusInstancia);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(statusinstanciaApi.addAndUpdate(mockStatusInstancia)).rejects.toThrow(StatusInstanciaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusInstancia',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete statusinstancia successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await statusinstanciaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(statusinstanciaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusInstancia',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(statusinstanciaApi.delete(1)).rejects.toThrow(StatusInstanciaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterStatusInstancia = { funcionario: 1 };
      
      const result = statusinstanciaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await statusinstanciaApi.delete(1);
  ...
  describe('StatusInstanciaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new StatusInstanciaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('StatusInstanciaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new StatusInstanciaApiError('Test message', 500);
