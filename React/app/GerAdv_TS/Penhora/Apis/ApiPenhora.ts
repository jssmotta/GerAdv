...
export class PenhoraApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class PenhoraApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Penhora`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Penhora_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Penhora');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Penhora_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Penhora com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Penhora_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Penhora');
        }
    }

            public async filter(filtro: FilterPenhora): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Penhora_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Penhora');
                }
            }


    public async validation(regPenhora: IPenhora): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regPenhora as Penhora, this.getHeaders());
    ...
    public async addAndUpdate(regPenhora: IPenhora): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Penhora');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Penhora. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Penhora',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Penhora com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterPenhora) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Penhora[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de penhora:', error);
            this.notificationService.notify({
              entity: 'Penhora',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos penhora'
            });
          }
        });
    }
