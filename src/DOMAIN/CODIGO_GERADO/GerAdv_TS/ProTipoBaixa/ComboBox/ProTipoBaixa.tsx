...
const ProTipoBaixaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProTipoBaixa | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProTipoBaixa | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProTipoBaixa);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProTipoBaixaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProTipoBaixa={editar ?? addValue ?? ProTipoBaixaEmpty()} /> 
            )}
