...
export const useProcessOutputEngineForm = (
  ...
  const loadProcessOutputEngine = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Process Output Engine";
...
export const useProcessOutputEngineList = (
  dataService: IProcessOutputEngineService,
  currentFilter?: FilterProcessOutputEngine | undefined | null,
) => {
  const [data, setData] = useState<IProcessOutputEngine[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProcessOutputEngine | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterProcessOutputEngine | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar processoutputengine";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProcessOutputEngine() {
  async function runValidation(
    ...
    const processoutputengineApi = new ProcessOutputEngineApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IProcessOutputEngine): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.database.length > 255) {
        return {
          isValid: false,
          message: "O campo Database não pode ter mais de 255 caracteres.",
        };
      }
      if (data.tabela.length > 255) {
        return {
          isValid: false,
          message: "O campo Tabela não pode ter mais de 255 caracteres.",
        };
      }
      if (data.campo.length > 255) {
        return {
          isValid: false,
          message: "O campo Campo não pode ter mais de 255 caracteres.",
        };
      }
      if (data.valor.length > 255) {
        return {
          isValid: false,
          message: "O campo Valor não pode ter mais de 255 caracteres.",
        };
      }
      if (data.output.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo Output não pode ter mais de 2147483647 caracteres.",
        };
      }
