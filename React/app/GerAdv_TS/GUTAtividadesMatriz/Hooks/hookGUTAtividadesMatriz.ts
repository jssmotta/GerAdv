...
export const useGUTAtividadesMatrizForm = (
  ...
  const loadGUTAtividadesMatriz = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar G U T Atividades Matriz";
...
export const useGUTAtividadesMatrizList = (
  dataService: IGUTAtividadesMatrizService,
  currentFilter?: FilterGUTAtividadesMatriz | undefined | null,
) => {
  const [data, setData] = useState<IGUTAtividadesMatriz[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGUTAtividadesMatriz | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterGUTAtividadesMatriz | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar gutatividadesmatriz";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsGUTAtividadesMatriz() {
  async function runValidation(
    ...
    const gutatividadesmatrizApi = new GUTAtividadesMatrizApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IGUTAtividadesMatriz): {
    isValid: boolean;
    message: string;
  } {
