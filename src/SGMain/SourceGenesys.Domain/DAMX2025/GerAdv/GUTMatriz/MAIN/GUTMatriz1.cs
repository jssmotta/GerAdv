...
public partial class DBGUTMatriz : VNoAuditor, ICrud
{
...
#region GravarDados_GUTMatriz
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (pFldFDescricao)
            clsW.Fields(DBGUTMatrizDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFGUTTipo)
            clsW.Fields(DBGUTMatrizDicInfo.GUTTipo, FGUTTipo, EGenericTypeFields.FNumberNull);
        if (pFldFValor)
            clsW.Fields(DBGUTMatrizDicInfo.Valor, FValor, EGenericTypeFields.FNumberNull);
        if (insertId != 0)
            return GravaNewId();
