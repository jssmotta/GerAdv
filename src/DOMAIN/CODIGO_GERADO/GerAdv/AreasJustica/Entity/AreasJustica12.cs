...
public partial class FAreasJustica : MenphisSI.SG.GerAdv.DBAreasJustica, IDBAreasJustica
{
    // DI-compatible default constructor
    public FAreasJustica() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FAreasJustica CreateFromReader(SqlDataReader? dbRec)
    {
        return new FAreasJustica(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FAreasJustica CreateFromDataRow(DataRow? dbRec)
    {
        return new FAreasJustica(dbRec);
    }

    private FAreasJustica(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FAreasJustica(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FAreasJustica> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fAreasJustica = new FAreasJustica();
        await fAreasJustica.CarregarAsync(id, oCnn);
        return fAreasJustica;
    }

    private FAreasJustica(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FAreasJustica CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FAreasJustica(parameters, oCnn, fullSql, sqlWhere, join);
    }
