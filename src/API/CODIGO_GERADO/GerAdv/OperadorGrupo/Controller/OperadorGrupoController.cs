...
public partial class OperadorGrupoController(IOperadorGrupoService operadorgrupoService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterOperadorGrupo filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("OperadorGrupo: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _operadorgrupoService.Filter(max, filtro, uri);
        return Ok(result);
    }
