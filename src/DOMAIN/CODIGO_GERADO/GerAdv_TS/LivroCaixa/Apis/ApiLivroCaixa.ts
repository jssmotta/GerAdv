...
export class LivroCaixaApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterLivroCaixa): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterLivroCaixa) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<LivroCaixa[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de livrocaixa:', error);
            this.notificationService.notify({
              entity: 'LivroCaixa',
              id: 0,
              action: NotifySystemActions.ERROR,
