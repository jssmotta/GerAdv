// app/TestsForms/FornecedoresComboBox.test.tsx - Vers�o Corrigida
...
import { FornecedoresEmpty, FornecedoresTestEmpty } from '@/app/GerAdv_TS/Models/Fornecedores';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Fornecedores/Hooks/hookFornecedores', () => ({
  useFornecedoresComboBox: jest.fn(() => ({
    options: [
    { ...FornecedoresEmpty(), id: 1, nome: 'Fornecedor 1' },
    { ...FornecedoresEmpty(), id: 2, nome: 'Fornecedor 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Fornecedores/Apis/ApiFornecedores', () => ({
  FornecedoresApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Fornecedores' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Fornecedores' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Fornecedores/Services/Fornecedores.service', () => ({
  FornecedoresService: jest.fn().mockImplementation(() => ({
    fetchFornecedoresById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Fornecedores' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Fornecedores/Crud/Grids/FornecedoresWindow', () => {
  return function MockFornecedoresWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='fornecedores-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Fornecedor' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...FornecedoresTestEmpty(), id: 1, nome: 'Fornecedor 1' },
  { ...FornecedoresTestEmpty(), id: 2, nome: 'Fornecedor 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: FornecedoresEmpty(), 
};
describe('FornecedoresComboBox', () => {
  ...
  const { useFornecedoresComboBox } = require('@/app/GerAdv_TS/Fornecedores/Hooks/hookFornecedores');
  (useFornecedoresComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<FornecedoresComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Fornecedor')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...FornecedoresTestEmpty(), id: 1, nome: 'Fornecedor 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useFornecedoresComboBox } = require('@/app/GerAdv_TS/Fornecedores/Hooks/hookFornecedores');
    useFornecedoresComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens FornecedoresWindow on action button click (add)', async () => {
    render(<FornecedoresComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('fornecedores-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('fornecedores-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<FornecedoresComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles FornecedoresWindow onClose and onSuccess', () => {
    render(<FornecedoresComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<FornecedoresComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
