...
import { FilterParceriaProc, FilterParceriaProcDefaults } from '../Filters/ParceriaProc'
import { IFilterParceriaProcWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for ParceriaProc API */
const PARCERIAPROC_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class ParceriaProcApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `ParceriaProc-${uri}`,
          {
            ...PARCERIAPROC_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${PARCERIAPROC_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParceriaProc-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os ParceriaProc'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParceriaProc-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar Parceria Proc com ID ${id}`,
          id
        );
    }
 public async filterPreload(max: number, filtro?: FilterParceriaProc): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterParceriaProcDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParceriaProc-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterParceriaProc): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar ParceriaProc'
        );
    }


    public async filterVoice(filtro?: FilterParceriaProc, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterParceriaProcDefaults();

        // Create request object according to new backend structure
        const request: IFilterParceriaProcWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParceriaProc-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter ParceriaProc'
        );
    }
    ...
    public async validation(regParceriaProc: IParceriaProc): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regParceriaProc as ParceriaProc, this.getHeaders());
            });
    ...
    public async addAndUpdate(regParceriaProc: IParceriaProc): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regParceriaProc as ParceriaProc, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ParceriaProcApiError(
                'Erro ao salvar Parceria Proc: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ParceriaProcApiError(
                `Erro ao excluir Parceria Proc com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterParceriaProc, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterParceriaProcWithVoiceRequest = {filter: filtro || new FilterParceriaProcDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<ParceriaProc[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de ParceriaProc');
            this.notificationService.notify({
              entity: 'ParceriaProc',
              id: 0,
              action: NotifySystemActions.ERROR,
