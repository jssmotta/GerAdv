...
const EnquadramentoEmpresaComboBox: React.FC<DadosSelectProps> = ({
  ...
  const [editRecord, setEditRecord] = useState<IEnquadramentoEmpresa | null>(
    null,
  );
  const [addRecord, setAddRecord] = useState<IEnquadramentoEmpresa | null>(
    null,
  );
  ...
  const loadRecordForEdit = async (id: number) => {
    try {
      const record =
        await enquadramentoempresaService.fetchEnquadramentoEmpresaById(id);
  ...
  const handleSuccess = (newEnquadramentoEmpresa?: IEnquadramentoEmpresa) => {
    if (newEnquadramentoEmpresa) {
      setValue({
        id: newEnquadramentoEmpresa.id,
        nome: newEnquadramentoEmpresa.nome,
      });
      ...
      {(editRecord || addRecord) && isOpen && (
        <EnquadramentoEmpresaWindow
          ...
          selectedEnquadramentoEmpresa={
            editRecord || addRecord || EnquadramentoEmpresaEmpty()
          }
        />
      )}
