...
#region TableDefinition_AgendaRepetirDias
    ...
    // REF. 250325
    public DBAgendaRepetirDias(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-3
        if (oCnn is null)
            return;
...
#region GravarDados_AgendaRepetirDias
    public int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        {
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_AgendaRepetirDias)
#endif
