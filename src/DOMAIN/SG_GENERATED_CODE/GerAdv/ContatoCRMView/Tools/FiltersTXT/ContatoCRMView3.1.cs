...
public static class FilterContatoCRMViewTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ContatoCRMView em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterContatoCRMView:

- O filter irá filtrar os ContatoCRMView conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterContatoCRMView:(Contato C R M View)
- cguid: CGUID;
- data: não é a data de cadastramento é a data do ContatoCRMView;
- ip: IP;
...
- Se ouvir "todos os ContatoCRMView" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterContatoCRMView? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ContatoCRMView length: {result.Length}");
#endif
