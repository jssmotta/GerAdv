...
import { FilterProTipoBaixa, FilterProTipoBaixaDefaults } from '../Filters/ProTipoBaixa'
import { IFilterProTipoBaixaWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for ProTipoBaixa API */
const PROTIPOBAIXA_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class ProTipoBaixaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `ProTipoBaixa-${uri}`,
          {
            ...PROTIPOBAIXA_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${PROTIPOBAIXA_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProTipoBaixa-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os ProTipoBaixa'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProTipoBaixa-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar Pro Tipo Baixa com ID ${id}`,
          id
        );
    }
    ...
    public async getListN(max?: number, filtro?: FilterProTipoBaixa): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProTipoBaixa-${uniqueKeyDay()}_lst_listN_data`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar lista de ProTipoBaixa'
        );
    }
 public async filterPreload(max: number, filtro?: FilterProTipoBaixa): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterProTipoBaixaDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProTipoBaixa-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterProTipoBaixa): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar ProTipoBaixa'
        );
    }


    public async filterVoice(filtro?: FilterProTipoBaixa, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterProTipoBaixaDefaults();

        // Create request object according to new backend structure
        const request: IFilterProTipoBaixaWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProTipoBaixa-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter ProTipoBaixa'
        );
    }
    ...
    public async validation(regProTipoBaixa: IProTipoBaixa): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regProTipoBaixa as ProTipoBaixa, this.getHeaders());
            });
    ...
    public async addAndUpdate(regProTipoBaixa: IProTipoBaixa): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regProTipoBaixa as ProTipoBaixa, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ProTipoBaixaApiError(
                'Erro ao salvar Pro Tipo Baixa: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ProTipoBaixaApiError(
                `Erro ao excluir Pro Tipo Baixa com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterProTipoBaixa, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterProTipoBaixaWithVoiceRequest = {filter: filtro || new FilterProTipoBaixaDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<ProTipoBaixa[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de ProTipoBaixa');
            this.notificationService.notify({
              entity: 'ProTipoBaixa',
              id: 0,
              action: NotifySystemActions.ERROR,
