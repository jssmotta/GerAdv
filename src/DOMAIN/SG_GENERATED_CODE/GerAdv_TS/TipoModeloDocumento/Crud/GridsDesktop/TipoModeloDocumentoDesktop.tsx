...
export const TipoModeloDocumentoGridDesktopComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('tipomodelodocumento-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('tipomodelodocumento-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
