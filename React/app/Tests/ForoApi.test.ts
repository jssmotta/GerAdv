...
describe('ForoApi', () => {
  ...
  beforeEach(() => {
    ...
    foroApi = new ForoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(foroApi).toBeInstanceOf(ForoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ForoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ForoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all foro items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await foroApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(foroApi.getAll()).rejects.toThrow(ForoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await foroApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Foro_last_getAll_${200}`)));
    });
    ...
    it('should throw ForoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    ...
    it('should fetch foro by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await foroApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockForo));
      ...
      expect(result.data).toEqual(mockForo);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Foro',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter foro items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await foroApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new foro successfully', async () => {
      const responseForo = { ...mockForo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseForo,
        status: 200,
        statusText: 'OK',
      ...
      const result = await foroApi.addAndUpdate(mockForo);
    ...
    it('should update existing foro successfully', async () => {
      const existingForo = { ...mockForo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingForo,
        status: 200,
        statusText: 'OK',
      ...
      const result = await foroApi.addAndUpdate(existingForo);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(foroApi.addAndUpdate(mockForo)).rejects.toThrow(ForoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Foro',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete foro successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await foroApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(foroApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Foro',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(foroApi.delete(1)).rejects.toThrow(ForoApiError);
    });
  });
  ...
  describe('ForoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ForoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ForoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ForoApiError('Test message', 500);
