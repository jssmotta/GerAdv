...
import { FilterLivroCaixa, FilterLivroCaixaDefaults } from '../Filters/LivroCaixa'
import { IFilterLivroCaixaWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for LivroCaixa API */
const LIVROCAIXA_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class LivroCaixaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `LivroCaixa-${uri}`,
          {
            ...LIVROCAIXA_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${LIVROCAIXA_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os LivroCaixa'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar Livro Caixa com ID ${id}`,
          id
        );
    }
    ...
    public async getListN(max?: number, filtro?: FilterLivroCaixa): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa-${uniqueKeyDay()}_lst_listN_data`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar lista de LivroCaixa'
        );
    }
 public async filterPreload(max: number, filtro?: FilterLivroCaixa): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterLivroCaixaDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterLivroCaixa): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar LivroCaixa'
        );
    }


    public async filterVoice(filtro?: FilterLivroCaixa, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterLivroCaixaDefaults();

        // Create request object according to new backend structure
        const request: IFilterLivroCaixaWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixa-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter LivroCaixa'
        );
    }
    ...
    public async validation(regLivroCaixa: ILivroCaixa): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regLivroCaixa as LivroCaixa, this.getHeaders());
            });
    ...
    public async addAndUpdate(regLivroCaixa: ILivroCaixa): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regLivroCaixa as LivroCaixa, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new LivroCaixaApiError(
                'Erro ao salvar Livro Caixa: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new LivroCaixaApiError(
                `Erro ao excluir Livro Caixa com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterLivroCaixa, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterLivroCaixaWithVoiceRequest = {filter: filtro || new FilterLivroCaixaDefaults(),
