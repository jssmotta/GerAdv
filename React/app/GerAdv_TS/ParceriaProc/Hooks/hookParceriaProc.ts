...
export const useParceriaProcForm = (
  ...
  const loadParceriaProc = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Parceria Proc";
...
export const useParceriaProcList = (
  dataService: IParceriaProcService,
  currentFilter?: FilterParceriaProc | undefined | null,
) => {
  const [data, setData] = useState<IParceriaProc[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterParceriaProc | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterParceriaProc | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar parceriaproc";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsParceriaProc() {
  ...
  function validate(data: IParceriaProc): {
    isValid: boolean;
    message: string;
  } {
