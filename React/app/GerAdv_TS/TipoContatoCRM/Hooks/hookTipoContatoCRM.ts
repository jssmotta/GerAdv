...
export const useTipoContatoCRMForm = (
  ...
  const loadTipoContatoCRM = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Contato C R M";
...
export const useTipoContatoCRMList = (
  dataService: ITipoContatoCRMService,
  currentFilter?: FilterTipoContatoCRM | undefined | null,
) => {
  const [data, setData] = useState<ITipoContatoCRM[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoContatoCRM | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterTipoContatoCRM | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipocontatocrm";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoContatoCRM() {
  ...
  function validate(data: ITipoContatoCRM): {
    isValid: boolean;
    message: string;
  } {
