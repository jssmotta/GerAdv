...
import { StatusTarefas } from '../../Models/StatusTarefas';
...
export class StatusTarefasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'StatusTarefasApiError';
    this.status = status;
    this.code = code;
...
export class StatusTarefasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/StatusTarefas`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusTarefas_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os StatusTarefas');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusTarefas_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Status Tarefas com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterStatusTarefas): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusTarefas_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de StatusTarefas');
        }
    }

            public async filter(filtro: FilterStatusTarefas): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusTarefas_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar StatusTarefas');
                }
            }


    public async validation(regStatusTarefas: IStatusTarefas): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regStatusTarefas as StatusTarefas, this.getHeaders());
    ...
    public async addAndUpdate(regStatusTarefas: IStatusTarefas): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regStatusTarefas as StatusTarefas, this.getHeaders());
            var register = result.data as IStatusTarefas;        
            const action = regStatusTarefas.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Status Tarefas');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o StatusTarefas. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'StatusTarefas',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Status Tarefas com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterStatusTarefas) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<StatusTarefas[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de statustarefas:', error);
            this.notificationService.notify({
              entity: 'StatusTarefas',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos statustarefas'
            });
          }
        });
    }
