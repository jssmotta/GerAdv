...
import ClientesWindow from '../Crud/Grids/ClientesWindow';
...
import { useClientesComboBox } from '../Hooks/hookClientes';
const ClientesComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useClientesComboBox(clientesService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IClientes | null>(null);
const [addRecord, setAddRecord] = useState<IClientes | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await clientesService.fetchClientesById(id);
...
const handleSuccess = (newClientes?: IClientes) => {
  if (newClientes) {
    setValue({ id: newClientes.id, nome: newClientes.nome });
    loadRecordForEdit(newClientes.id);
    handleValueChange(newClientes);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .clientes-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .clientes-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .clientes-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedClientes={editRecord || addRecord || ClientesEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'clientes-dropdown-popup-msi'
          }}
          />
...
export default ClientesComboBox;