...
const OponentesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOponentes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOponentes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOponentes);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OponentesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOponentes={editar ?? addValue ?? OponentesEmpty()} /> 
            )}
