...
public partial class PrecatoriaService(IOptions<AppSettings> appSettings, IFPrecatoriaFactory precatoriaFactory, IPrecatoriaReader reader, IPrecatoriaValidation validation, IPrecatoriaWriter writer, IProcessosReader processosReader, IHistoricoService historicoService, INENotasService nenotasService, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IPrecatoriaService, IDisposable
{
    ...
    private readonly IFPrecatoriaFactory precatoriaFactory = precatoriaFactory;
    private readonly IPrecatoriaReader reader = reader;
    private readonly IPrecatoriaValidation validation = validation;
    private readonly IPrecatoriaWriter writer = writer;
    ...
    public async Task<IEnumerable<PrecatoriaResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-Precatoria-Filter-{keyCache}";
    ...
    private async Task<PrecatoriaResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<PrecatoriaResponse?> AddAndUpdate([FromBody] Models.Precatoria regPrecatoria, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regPrecatoria, operadorId, oCnn);
    ...
    public async Task<PrecatoriaResponse?> Validation([FromBody] Models.Precatoria regPrecatoria, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regPrecatoria.Id, oCnn);
    }
    ...
    public async Task<PrecatoriaResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var precatoria = await reader.Read(id, oCnn);
        try
        {
            if (precatoria != null)
            {
                await writer.Delete(precatoria, nOperador, oCnn);
