...
public partial class AgendaQuemController(IAgendaQuemService agendaquemService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.AgendaQuem regAgendaQuem, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaQuem: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaQuem: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.AgendaQuem regAgendaQuem, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _agendaquemService.Validation(regAgendaQuem, uri);
            if (result == null)
            {
                _logger.Warn("AgendaQuem: Validation failed to add or update AgendaQuem, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "AgendaQuem: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}