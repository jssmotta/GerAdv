...
import { FilterCargosEscClass } from '@/app/GerAdv_TS/CargosEscClass/Filters/CargosEscClass';
...
interface UseCargosEscClassFilterProps {
    handleFetchWithFilter: (filtro?: FilterCargosEscClass | undefined | null) => Promise<void>;
}

interface UseCargosEscClassFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterCargosEscClass;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterCargosEscClass>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterCargosEscClass) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterCargosEscClass>) => React.ReactNode;
...
export const useCargosEscClassFilter = ({ handleFetchWithFilter }: UseCargosEscClassFilterProps): UseCargosEscClassFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterCargosEscClass>({} as FilterCargosEscClass);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterCargosEscClass;
    ...
    const handleConfirmSearch = async (filter: FilterCargosEscClass) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterCargosEscClass);
        sessionStorage.removeItem(btoa('CargosEscClassFilter'));
        handleFetchWithFilter({} as FilterCargosEscClass);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterCargosEscClass>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
