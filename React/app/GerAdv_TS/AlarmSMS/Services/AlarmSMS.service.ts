...
export class AlarmSMSService implements IAlarmSMSService {
  ...
  async saveAlarmSMS(alarmsms: IAlarmSMS): Promise<IAlarmSMS> {    
    ...
    try {
      ...
      console.log('Error fetching AlarmSMS list');
      return [];
    }
  }
   ...
   async getAll(
    filtro?: FilterAlarmSMS,
    onOnlineData?: (data: IAlarmSMS[]) => void
  ): Promise<IAlarmSMS[]> {
    try {
      ...
      if (onOnlineData) {
        ...
        this.api.filter(filtro ?? {})
          ...
          .catch(error => {
            console.log('Error fetching online AlarmSMS');
          });
      ...
      console.log('Error fetching all AlarmSMS:');
      return [];
    }
  }
