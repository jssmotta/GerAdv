...
{
    Task<TipoEMailResponse?> ReadAsync(int id, MsiSqlConnection? oCnn);
    Task<Models.TipoEMail?> ReadMAsync(int id, MsiSqlConnection? oCnn);
    ...
    Task<IEnumerable<TipoEMailResponseAll>> ListarAsync(MsiSqlConnection oCnn, int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken);
}