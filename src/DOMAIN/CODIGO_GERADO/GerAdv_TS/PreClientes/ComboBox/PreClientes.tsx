...
const PreClientesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPreClientes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPreClientes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPreClientes);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PreClientesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPreClientes={editar ?? addValue ?? PreClientesEmpty()} /> 
            )}
