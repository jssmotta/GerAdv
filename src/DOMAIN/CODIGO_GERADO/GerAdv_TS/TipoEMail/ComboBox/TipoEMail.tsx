...
import TipoEMailWindow from '../Crud/Grids/TipoEMailWindow';
...
import { useTipoEMailComboBox } from '../Hooks/hookTipoEMail';
const TipoEMailComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useTipoEMailComboBox(tipoemailService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<ITipoEMail | null>(null);
const [addRecord, setAddRecord] = useState<ITipoEMail | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await tipoemailService.fetchTipoEMailById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nome });
  } catch (error) {
  console.log('Erro ao carregar Tipo E Mail:');
}
};
...
const handleActionClick = () => {
  ...
  const newRecord = TipoEMailEmpty();
...
const handleSuccess = (newTipoEMail?: ITipoEMail) => {
  if (newTipoEMail) {
    setValue({ id: newTipoEMail.id, nome: newTipoEMail.nome });
    loadRecordForEdit(newTipoEMail.id);
    handleValueChange(newTipoEMail);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .tipoemail-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .tipoemail-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .tipoemail-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedTipoEMail={editRecord || addRecord || TipoEMailEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'tipoemail-dropdown-popup-msi'
          }}
          />
...
export default TipoEMailComboBox;