...
/// <summary>
/// Comprehensive unit tests for ConfigureUpdateFields and ConfigureAuditorFields methods in DBStatusInstancia class
...
/// </summary>
public class DBStatusInstanciaConfigureMethodsTests : IDisposable
{
    private readonly DBStatusInstancia _statusinstancia;
    private readonly Mock<MsiSqlConnection> _mockConnection;
    public DBStatusInstanciaConfigureMethodsTests()
    {
        _statusinstancia = new DBStatusInstancia();
    ...
    public void ConfigureUpdateFields_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        // Arrange
        ResetAllFieldFlags();
        _statusinstancia.ID = 1; // Update scenario
        // Act
        var result = _statusinstancia.Update(_mockConnection.Object);
    ...
    public async Task ConfigureUpdateFieldsAsync_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        ...
        try
        {
            var result = await _statusinstancia.UpdateAsync(_mockConnection.Object);
...
#region Helper Methods
    ...
    {
        var field = typeof(DBStatusInstancia).GetField(fieldName, BindingFlags.NonPublic | BindingFlags.Instance);
        field?.SetValue(_statusinstancia, value);
    }
    ...
    private void SetFieldValue(string fieldName, object? value)
    {
        ...
        {
            field?.SetValue(_statusinstancia, Convert.ToDateTime(value));
        }
    }
...
#region Fields_All
    ...
    public void ConfigureUpdateFields_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _statusinstancia.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
    ...
    public async Task ConfigureUpdateFieldsAsync_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _statusinstancia.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _statusinstancia.UpdateAsync(_mockConnection.Object));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithCancellationToken_ShouldRespectCancellation()
    {
        ...
        var exception = await Record.ExceptionAsync(async () => await _statusinstancia.UpdateAsync(_mockConnection.Object, cancellationToken: cts.Token));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithTimeout_ShouldRespectTimeout()
    {
        ...
        _statusinstancia.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _statusinstancia.UpdateAsync(_mockConnection.Object, timeoutSeconds: 1));
        exception.Should().NotBeNull();
    }
    ...
    {
        // Arrange
        _statusinstancia.ID = 123; // Update scenario
        _statusinstancia.AuditorQuem = 7;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection but audit fields should be set
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        _statusinstancia.AuditorQuem.Should().Be(7);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemIsZero_ShouldSetToDefaultValue()
    {
        // Arrange
        _statusinstancia.ID = 0; // Insert scenario
        _statusinstancia.AuditorQuem = 0; // Should be set to 1
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        ...
        _statusinstancia.AuditorQuem.Should().Be(1);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemHasValue_ShouldPreserveValue(int auditorValue)
    {
        // Arrange
        _statusinstancia.ID = 0; // Insert scenario
        _statusinstancia.AuditorQuem = auditorValue;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        ...
        _statusinstancia.AuditorQuem.Should().Be(auditorValue);
    }
    ...
    {
        // Arrange
        _statusinstancia.ID = 0;
        _statusinstancia.AuditorQuem = 10;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        ...
        _statusinstancia.AuditorQuem.Should().Be(10);
    }
    ...
    {
        // Arrange
        _statusinstancia.ID = id;
        _statusinstancia.AuditorQuem = 15;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        ...
        _statusinstancia.AuditorQuem.Should().Be(15);
    }
    ...
    public void ConfigureMethodsNome_Integration_BothMethodsWorkTogether_ShouldNotInterfere()
    {
        ...
        SetAllFieldValues();
        _statusinstancia.ID = 123;
        _statusinstancia.AuditorQuem = 99;
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _statusinstancia.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        // Assert - Both update and audit configuration should work
        _statusinstancia.FNome.Should().Be("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        _statusinstancia.AuditorQuem.Should().Be(99);
    }
    ...
    public void ConfigureFieldsNome_Integration_FieldChangeTracking_ShouldWorkCorrectly()
    {
        ...
        var result1 = _statusinstancia.Update(_mockConnection.Object);
    ...
    public void ConfigureMethodsGUID_Integration_BothMethodsWorkTogether_ShouldNotInterfere()
    {
        ...
        exception.Should().NotBeNull();
        // Assert - Both update and audit configuration should work
        _statusinstancia.FGUID.Should().Be("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        _statusinstancia.AuditorQuem.Should().Be(99);
    }
