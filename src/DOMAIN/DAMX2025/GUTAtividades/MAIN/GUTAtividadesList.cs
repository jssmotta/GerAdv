...
#region ListarDados_GUTAtividades
    public static async IAsyncEnumerable<DBGUTAtividades> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTAtividades(dbRec);
        }
    }

    public static IEnumerable<DBGUTAtividades> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
