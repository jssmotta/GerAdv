...
public static class FilterGUTTipoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar GUTTipo em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGUTTipo:

- O filter irá filtrar os GUTTipo conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os GUTTipo" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterGUTTipo? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt GUTTipo length: {result.Length}");
#endif
