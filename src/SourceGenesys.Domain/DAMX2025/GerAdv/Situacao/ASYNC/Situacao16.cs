...
public partial class DBSituacao
{
...
#if (!NOTSTORED_Situacao)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFParte_Int || pFldFParte_Opo || pFldFTop || pFldFGUID;
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBSituacaoDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBSituacaoDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBSituacaoDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBSituacaoDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBSituacaoDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
