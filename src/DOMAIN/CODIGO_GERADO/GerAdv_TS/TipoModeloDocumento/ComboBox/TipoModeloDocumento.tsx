...
const TipoModeloDocumentoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, nome: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as TipoModeloDocumento;
        const novoDado = await dadoApi.addAndUpdate(nova as ITipoModeloDocumento);
