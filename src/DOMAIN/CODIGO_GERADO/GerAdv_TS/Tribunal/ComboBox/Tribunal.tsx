...
const TribunalComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITribunal | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITribunal | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITribunal);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TribunalWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTribunal={editar ?? addValue ?? TribunalEmpty()} /> 
            )}
