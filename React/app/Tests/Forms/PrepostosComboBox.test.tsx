// app/TestsForms/PrepostosComboBox.test.tsx - Vers�o Corrigida
...
import { PrepostosEmpty, PrepostosTestEmpty } from '@/app/GerAdv_TS/Models/Prepostos';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Prepostos/Hooks/hookPrepostos', () => ({
  usePrepostosComboBox: jest.fn(() => ({
    options: [
    { ...PrepostosEmpty(), id: 1, nome: 'Prepostos 1' },
    { ...PrepostosEmpty(), id: 2, nome: 'Prepostos 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Prepostos/Apis/ApiPrepostos', () => ({
  PrepostosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Prepostos' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Prepostos' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Prepostos/Services/Prepostos.service', () => ({
  PrepostosService: jest.fn().mockImplementation(() => ({
    fetchPrepostosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Prepostos' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Prepostos/Crud/Grids/PrepostosWindow', () => {
  return function MockPrepostosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='prepostos-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Prepostos' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...PrepostosTestEmpty(), id: 1, nome: 'Prepostos 1' },
  { ...PrepostosTestEmpty(), id: 2, nome: 'Prepostos 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: PrepostosEmpty(), 
};
describe('PrepostosComboBox', () => {
  ...
  const { usePrepostosComboBox } = require('@/app/GerAdv_TS/Prepostos/Hooks/hookPrepostos');
  (usePrepostosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<PrepostosComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Prepostos')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Prepostos')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...PrepostosTestEmpty(), id: 1, nome: 'Prepostos 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { usePrepostosComboBox } = require('@/app/GerAdv_TS/Prepostos/Hooks/hookPrepostos');
    usePrepostosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens PrepostosWindow on action button click (add)', async () => {
    render(<PrepostosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('prepostos-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('prepostos-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<PrepostosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles PrepostosWindow onClose and onSuccess', () => {
    render(<PrepostosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<PrepostosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
