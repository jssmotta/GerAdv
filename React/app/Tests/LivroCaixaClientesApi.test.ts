...
import { LivroCaixaClientesApiError } from '../GerAdv_TS/LivroCaixaClientes/Apis/ApiLivroCaixaClientes';
import { LivroCaixaClientesApi } from '@/app/GerAdv_TS/LivroCaixaClientes/Apis/ApiLivroCaixaClientes';
import { FilterLivroCaixaClientes } from '../GerAdv_TS/LivroCaixaClientes/Filters/LivroCaixaClientes';
...
describe('LivroCaixaClientesApi', () => {
  let livrocaixaclientesApi: LivroCaixaClientesApi;
  ...
  beforeEach(() => {
    ...
    livrocaixaclientesApi = new LivroCaixaClientesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(livrocaixaclientesApi).toBeInstanceOf(LivroCaixaClientesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new LivroCaixaClientesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(LivroCaixaClientesApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [LivroCaixaClientesTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all livrocaixaclientes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await livrocaixaclientesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(livrocaixaclientesApi.getAll()).rejects.toThrow(LivroCaixaClientesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await livrocaixaclientesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-LivroCaixaClientes_last_getAll_${200}`)));
    });
    ...
    it('should throw LivroCaixaClientesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockLivroCaixaClientes = LivroCaixaClientesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockLivroCaixaClientes,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch livrocaixaclientes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await livrocaixaclientesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockLivroCaixaClientes));
      ...
      expect(result.data).toEqual(mockLivroCaixaClientes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixaClientes',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterLivroCaixaClientes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter livrocaixaclientes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await livrocaixaclientesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockLivroCaixaClientes: ILivroCaixaClientes = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new livrocaixaclientes successfully', async () => {
      const responseLivroCaixaClientes = { ...mockLivroCaixaClientes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseLivroCaixaClientes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await livrocaixaclientesApi.addAndUpdate(mockLivroCaixaClientes);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockLivroCaixaClientes,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing livrocaixaclientes successfully', async () => {
      const existingLivroCaixaClientes = { ...mockLivroCaixaClientes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingLivroCaixaClientes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await livrocaixaclientesApi.addAndUpdate(existingLivroCaixaClientes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(livrocaixaclientesApi.addAndUpdate(mockLivroCaixaClientes)).rejects.toThrow(LivroCaixaClientesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixaClientes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete livrocaixaclientes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await livrocaixaclientesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(livrocaixaclientesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixaClientes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(livrocaixaclientesApi.delete(1)).rejects.toThrow(LivroCaixaClientesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterLivroCaixaClientes = { funcionario: 1 };
      
      const result = livrocaixaclientesApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await livrocaixaclientesApi.delete(1);
  ...
  describe('LivroCaixaClientesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new LivroCaixaClientesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('LivroCaixaClientesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new LivroCaixaClientesApiError('Test message', 500);
