...
export const useHonorariosDadosContratoForm = (
  ...
  const loadHonorariosDadosContrato = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Honorarios Dados Contrato";
...
export const useHonorariosDadosContratoList = (
  dataService: IHonorariosDadosContratoService,
  currentFilter?: FilterHonorariosDadosContrato | undefined | null,
) => {
  const [data, setData] = useState<IHonorariosDadosContrato[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    async (filtro?: FilterHonorariosDadosContrato | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar honorariosdadoscontrato";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsHonorariosDadosContrato() {
  async function runValidation(
    ...
    const honorariosdadoscontratoApi = new HonorariosDadosContratoApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IHonorariosDadosContrato): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      if (data.arquivocontrato.length > 2048) {
        return {
          isValid: false,
          message:
            "O campo ArquivoContrato não pode ter mais de 2048 caracteres.",
        };
      }
      if (data.textocontrato.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo TextoContrato não pode ter mais de 2147483647 caracteres.",
        };
      }
