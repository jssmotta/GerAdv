...
import { FilterTiposAcao } from "@/app/GerAdv_TS/TiposAcao/Filters/TiposAcao";
...
interface UseTiposAcaoFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterTiposAcao | undefined | null,
  ) => Promise<void>;
}

interface UseTiposAcaoFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterTiposAcao;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterTiposAcao>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterTiposAcao) => Promise<void>;

  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterTiposAcao>,
  ) => React.ReactNode;
...
export const useTiposAcaoFilter = ({
  handleFetchWithFilter,
}: UseTiposAcaoFilterProps): UseTiposAcaoFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterTiposAcao>(
    {} as FilterTiposAcao,
  );
  ...
  const handleConfirmSearch = async (filter: FilterTiposAcao) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterTiposAcao);
    sessionStorage.removeItem(btoa("TiposAcaoFilter"));
    handleFetchWithFilter({} as FilterTiposAcao);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterTiposAcao>) => (
    <>
      <InputInput
        tabIndex={0}
        type="text"
