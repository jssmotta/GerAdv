// app/TestsForms/ProSucumbenciaComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import ProSucumbenciaComboBox from '@/app/GerAdv_TS/ProSucumbencia/ComboBox/ProSucumbencia';
...
import { ProSucumbenciaEmpty, ProSucumbenciaTestEmpty } from '@/app/GerAdv_TS/Models/ProSucumbencia';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/ProSucumbencia/Hooks/hookProSucumbencia', () => ({
  useProSucumbenciaComboBox: jest.fn(() => ({
    options: [
    { ...ProSucumbenciaEmpty(), id: 1, nome: 'Pro Sucumbencia 1' },
    { ...ProSucumbenciaEmpty(), id: 2, nome: 'Pro Sucumbencia 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/ProSucumbencia/Apis/ApiProSucumbencia', () => ({
  ProSucumbenciaApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProSucumbencia' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProSucumbencia' }),
...
jest.mock('@/app/GerAdv_TS/ProSucumbencia/Services/ProSucumbencia.service', () => ({
  ProSucumbenciaService: jest.fn().mockImplementation(() => ({
    fetchProSucumbenciaById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProSucumbencia' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/ProSucumbencia/Crud/Grids/ProSucumbenciaWindow', () => {
  return function MockProSucumbenciaWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='prosucumbencia-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Pro Sucumbencia' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ProSucumbenciaTestEmpty(), id: 1, nome: 'Pro Sucumbencia 1' },
  { ...ProSucumbenciaTestEmpty(), id: 2, nome: 'Pro Sucumbencia 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Pro Sucumbencia',
  name: 'prosucumbencia',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: ProSucumbenciaEmpty(), 
};
describe('ProSucumbenciaComboBox', () => {
  ...
  const { useProSucumbenciaComboBox } = require('@/app/GerAdv_TS/ProSucumbencia/Hooks/hookProSucumbencia');
  (useProSucumbenciaComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ProSucumbenciaComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Pro Sucumbencia')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Pro Sucumbencia')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ProSucumbenciaTestEmpty(), id: 1, nome: 'Pro Sucumbencia 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useProSucumbenciaComboBox } = require('@/app/GerAdv_TS/ProSucumbencia/Hooks/hookProSucumbencia');
    useProSucumbenciaComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ProSucumbenciaWindow on action button click (add)', async () => {
    render(<ProSucumbenciaComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('prosucumbencia-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('prosucumbencia-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ProSucumbenciaComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ProSucumbenciaWindow onClose and onSuccess', () => {
    render(<ProSucumbenciaComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ProSucumbenciaComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
