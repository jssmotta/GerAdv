...
public class Auditor4KHealthCheck(IOptions<AppSettings> appSettings, Auditor4KService auditor4kService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly Auditor4KService _auditor4kService = auditor4kService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT Auditor4K successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT Auditor4K failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _auditor4kService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-Auditor4K";
                    ...
                    static async Task<IEnumerable<Models.Auditor4K>> GetData(string uri, CancellationToken token)
                    {
                        await Task.Delay(1, token);
                        return[new Models.Auditor4K
                        {
                        }
            ...
            return isHealthy ? HealthCheckResult.Healthy("Auditor4K is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
