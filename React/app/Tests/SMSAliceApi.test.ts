...
import { SMSAliceApiError } from '../GerAdv_TS/SMSAlice/Apis/ApiSMSAlice';
import { SMSAliceApi } from '@/app/GerAdv_TS/SMSAlice/Apis/ApiSMSAlice';
import { FilterSMSAlice } from '../GerAdv_TS/SMSAlice/Filters/SMSAlice';
...
describe('SMSAliceApi', () => {
  let smsaliceApi: SMSAliceApi;
  ...
  beforeEach(() => {
    ...
    smsaliceApi = new SMSAliceApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(smsaliceApi).toBeInstanceOf(SMSAliceApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new SMSAliceApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(SMSAliceApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all smsalice items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await smsaliceApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(smsaliceApi.getAll()).rejects.toThrow(SMSAliceApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await smsaliceApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-SMSAlice_last_getAll_${200}`)));
    });
    ...
    it('should throw SMSAliceApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockSMSAlice = SMSAliceTestEmpty();
    ...
    it('should fetch smsalice by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await smsaliceApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockSMSAlice));
      ...
      expect(result.data).toEqual(mockSMSAlice);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'SMSAlice',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterSMSAlice = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter smsalice items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await smsaliceApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new smsalice successfully', async () => {
      const responseSMSAlice = { ...mockSMSAlice, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseSMSAlice,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await smsaliceApi.addAndUpdate(mockSMSAlice);
    ...
    it('should update existing smsalice successfully', async () => {
      const existingSMSAlice = { ...mockSMSAlice, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingSMSAlice,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await smsaliceApi.addAndUpdate(existingSMSAlice);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(smsaliceApi.addAndUpdate(mockSMSAlice)).rejects.toThrow(SMSAliceApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'SMSAlice',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete smsalice successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await smsaliceApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(smsaliceApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'SMSAlice',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(smsaliceApi.delete(1)).rejects.toThrow(SMSAliceApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterSMSAlice = { funcionario: 1 };
      
      const result = smsaliceApi.useFilter(mockFilter);
  ...
  describe('SMSAliceApiError', () => {
    it('should create error with correct properties', () => {
      const error = new SMSAliceApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('SMSAliceApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new SMSAliceApiError('Test message', 500);
