...
public static class FilterProcessosObsReportTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterProcessosObsReport? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar ProcessosObsReport em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProcessosObsReport:

- O filter irá filtrar os ProcessosObsReport conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProcessosObsReport:(Processos Obs Report)
- data: não é a data de cadastramento é a data do ProcessosObsReport;
- processo: número ou IDs;
- observacao: Observacao;
- historico: número ou IDs;
...
- Se ouvir "todos os ProcessosObsReport" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProcessosObsReport length: {result.Length}");
#endif
