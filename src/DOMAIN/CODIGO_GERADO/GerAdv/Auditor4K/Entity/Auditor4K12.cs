...
public partial class FAuditor4K : MenphisSI.SG.GerAdv.DBAuditor4K, IDBAuditor4K
{
    // DI-compatible default constructor
    public FAuditor4K() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FAuditor4K CreateFromReader(SqlDataReader? dbRec)
    {
        return new FAuditor4K(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FAuditor4K CreateFromDataRow(DataRow? dbRec)
    {
        return new FAuditor4K(dbRec);
    }

    private FAuditor4K(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FAuditor4K(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FAuditor4K> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fAuditor4K = new FAuditor4K();
        await fAuditor4K.CarregarAsync(id, oCnn);
        return fAuditor4K;
    }

    private FAuditor4K(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FAuditor4K CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FAuditor4K(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
