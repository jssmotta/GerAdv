...
const TipoStatusBiuComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoStatusBiu | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoStatusBiu | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoStatusBiu);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoStatusBiuWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoStatusBiu={editar ?? addValue ?? TipoStatusBiuEmpty()} /> 
            )}
