...
export const useProcessOutPutIDsForm = (
  ...
  const loadProcessOutPutIDs = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Process Out Put I Ds";
...
export const useProcessOutPutIDsList = (
  dataService: IProcessOutPutIDsService,
  currentFilter?: FilterProcessOutPutIDs | undefined | null,
) => {
  const [data, setData] = useState<IProcessOutPutIDs[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProcessOutPutIDs | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterProcessOutPutIDs | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar processoutputids";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProcessOutPutIDs() {
  ...
  function validate(data: IProcessOutPutIDs): {
    isValid: boolean;
    message: string;
  } {
