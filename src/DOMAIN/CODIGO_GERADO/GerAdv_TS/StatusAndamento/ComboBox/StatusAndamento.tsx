...
import { useStatusAndamentoComboBox } from '../Hooks/hookStatusAndamento';
const StatusAndamentoComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useStatusAndamentoComboBox(statusandamentoService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IStatusAndamento | null>(null);
const [addRecord, setAddRecord] = useState<IStatusAndamento | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await statusandamentoService.fetchStatusAndamentoById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = StatusAndamentoEmpty();
...
const handleSuccess = (newStatusAndamento?: IStatusAndamento) => {
  if (newStatusAndamento) {
    setValue({ id: newStatusAndamento.id, nome: newStatusAndamento.nome });
    loadRecordForEdit(newStatusAndamento.id);
    handleValueChange(newStatusAndamento);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .statusandamento-dropdown-popup-msi {
  ...
  .statusandamento-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .statusandamento-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .statusandamento-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedStatusAndamento={editRecord || addRecord || StatusAndamentoEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'statusandamento-dropdown-popup-msi'
          }}
          />
