...
public static class FilterEnquadramentoEmpresaTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterEnquadramentoEmpresa? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar EnquadramentoEmpresa em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterEnquadramentoEmpresa:

- O filter irá filtrar os EnquadramentoEmpresa conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- nome: nome ou descrição para Enquadramento Empresa;
...
- Se ouvir "todos os EnquadramentoEmpresa" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt EnquadramentoEmpresa length: {result.Length}");
#endif
