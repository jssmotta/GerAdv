...
#region ListarDados_Oponentes
    public static async IAsyncEnumerable<DBOponentes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOponentes(dbRec);
        }
    }

    public static IEnumerable<DBOponentes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
