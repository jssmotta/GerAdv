...
export const useStatusHTrabForm = (
  ...
  const loadStatusHTrab = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialStatusHTrab);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Status H Trab';
      setError(errorMessage);
      console.log('Erro ao carregar Status H Trab');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialStatusHTrab]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadStatusHTrab, resetForm]);
  return returnValue;
};


export const useStatusHTrabNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('StatusHTrab', (entity) => {
      try {
...
export const useStatusHTrabList = (dataService: IStatusHTrabService) => {
  const [data, setData] = useState<IStatusHTrab[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar statushtrab';
      setError(errorMessage);
      console.log('Erro ao carregar statushtrab');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
