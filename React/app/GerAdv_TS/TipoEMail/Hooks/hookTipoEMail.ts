...
export const useTipoEMailForm = (
  ...
  const loadTipoEMail = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoEMail);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Tipo E Mail';
      setError(errorMessage);
      console.log('Erro ao carregar Tipo E Mail');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialTipoEMail]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoEMail, resetForm]);
  return returnValue;
};


export const useTipoEMailNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('TipoEMail', (entity) => {
      try {
...
export const useTipoEMailList = (dataService: ITipoEMailService) => {
  const [data, setData] = useState<ITipoEMail[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar tipoemail';
      setError(errorMessage);
      console.log('Erro ao carregar tipoemail');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
