...
public partial class DBAnexamentoRegistros
{
    ...
    /// </summary>
    public static async Task<DBAnexamentoRegistros> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAnexamentoRegistros();
...
#if (!NOTSTORED_AnexamentoRegistros)
    /// <summary>
...
#if (!NOTSTORED_AnexamentoRegistros)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFCliente || pFldFGUIDReg || pFldFCodigoReg || pFldFIDReg || pFldFData || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFCliente)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.Cliente, m_FCliente, ETiposCampos.FNumber);
        if (pFldFGUIDReg)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.GUIDReg, m_FGUIDReg, ETiposCampos.FString);
        if (pFldFCodigoReg)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.CodigoReg, m_FCodigoReg, ETiposCampos.FNumber);
        if (pFldFIDReg)
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBAnexamentoRegistrosDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBAnexamentoRegistrosDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
