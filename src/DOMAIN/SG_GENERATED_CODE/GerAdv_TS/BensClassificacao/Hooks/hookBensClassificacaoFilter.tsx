...
import { FilterBensClassificacao } from '@/app/GerAdv_TS/BensClassificacao/Filters/BensClassificacao';
...
interface UseBensClassificacaoFilterProps {
    handleFetchWithFilter: (filtro?: FilterBensClassificacao | undefined | null) => Promise<void>;
}

interface UseBensClassificacaoFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterBensClassificacao;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterBensClassificacao>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterBensClassificacao) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterBensClassificacao>) => React.ReactNode;
...
export const useBensClassificacaoFilter = ({ handleFetchWithFilter }: UseBensClassificacaoFilterProps): UseBensClassificacaoFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterBensClassificacao>({} as FilterBensClassificacao);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterBensClassificacao;
    ...
    const handleConfirmSearch = async (filter: FilterBensClassificacao) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterBensClassificacao);
        sessionStorage.removeItem(btoa('BensClassificacaoFilter'));
        handleFetchWithFilter({} as FilterBensClassificacao);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterBensClassificacao>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
