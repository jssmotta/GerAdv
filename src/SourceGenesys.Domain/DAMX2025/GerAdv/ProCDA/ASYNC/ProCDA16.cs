...
public partial class DBProCDA
{
...
#if (!NOTSTORED_ProCDA)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFNome || pFldFNroInterno || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBProCDADicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBProCDADicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFNroInterno)
            updateTool.Fields(DBProCDADicInfo.NroInterno, m_FNroInterno, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBProCDADicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBProCDADicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBProCDADicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBProCDADicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBProCDADicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBProCDADicInfo.Visto, false, ETiposCampos.FBoolean);
    }
