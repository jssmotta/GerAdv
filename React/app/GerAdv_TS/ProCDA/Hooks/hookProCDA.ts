  ...
  const loadProCDA = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Pro C D A";
...
export const useProCDAList = (
  dataService: IProCDAService,
  currentFilter?: FilterProCDA | undefined | null,
) => {
  const [data, setData] = useState<IProCDA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProCDA | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterProCDA | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar procda";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IProCDA): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.nrointerno.length > 255) {
        return {
          isValid: false,
          message: "O campo NroInterno não pode ter mais de 255 caracteres.",
        };
      }
