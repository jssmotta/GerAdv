...
import { ContatoCRMViewApiError } from '../GerAdv_TS/ContatoCRMView/Apis/ApiContatoCRMView';
import { ContatoCRMViewApi } from '@/app/GerAdv_TS/ContatoCRMView/Apis/ApiContatoCRMView';
import { FilterContatoCRMView } from '../GerAdv_TS/ContatoCRMView/Filters/ContatoCRMView';
...
describe('ContatoCRMViewApi', () => {
  let contatocrmviewApi: ContatoCRMViewApi;
  ...
  beforeEach(() => {
    ...
    contatocrmviewApi = new ContatoCRMViewApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(contatocrmviewApi).toBeInstanceOf(ContatoCRMViewApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ContatoCRMViewApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ContatoCRMViewApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all contatocrmview items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmviewApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(contatocrmviewApi.getAll()).rejects.toThrow(ContatoCRMViewApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await contatocrmviewApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ContatoCRMView_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMView',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw ContatoCRMViewApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockContatoCRMView = ContatoCRMViewTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockContatoCRMView,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch contatocrmview by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmviewApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockContatoCRMView));
      ...
      expect(result.data).toEqual(mockContatoCRMView);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMView',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterContatoCRMView = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter contatocrmview items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await contatocrmviewApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockContatoCRMView: IContatoCRMView = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new contatocrmview successfully', async () => {
      const responseContatoCRMView = { ...mockContatoCRMView, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseContatoCRMView,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmviewApi.addAndUpdate(mockContatoCRMView);
    ...
    it('should update existing contatocrmview successfully', async () => {
      const existingContatoCRMView = { ...mockContatoCRMView, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingContatoCRMView,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmviewApi.addAndUpdate(existingContatoCRMView);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(contatocrmviewApi.addAndUpdate(mockContatoCRMView)).rejects.toThrow(ContatoCRMViewApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMView',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete contatocrmview successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await contatocrmviewApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(contatocrmviewApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMView',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(contatocrmviewApi.delete(1)).rejects.toThrow(ContatoCRMViewApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterContatoCRMView = { funcionario: 1 };
      
      const result = contatocrmviewApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await contatocrmviewApi.delete(1);
  ...
  describe('ContatoCRMViewApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ContatoCRMViewApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ContatoCRMViewApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ContatoCRMViewApiError('Test message', 500);
