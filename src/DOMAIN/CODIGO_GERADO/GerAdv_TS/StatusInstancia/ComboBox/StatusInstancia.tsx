...
const StatusInstanciaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IStatusInstancia | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IStatusInstancia | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IStatusInstancia);
    ...
    const handleAddClick = () => {
        ...
        const add = StatusInstanciaEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <StatusInstanciaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedStatusInstancia={editar ?? addValue ?? StatusInstanciaEmpty()} /> 
            )}
