// app/TestsForms/TipoModeloDocumentoComboBox.test.tsx - Vers�o Corrigida
...
import { TipoModeloDocumentoEmpty, TipoModeloDocumentoTestEmpty } from '@/app/GerAdv_TS/Models/TipoModeloDocumento';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TipoModeloDocumento/Hooks/hookTipoModeloDocumento', () => ({
  useTipoModeloDocumentoComboBox: jest.fn(() => ({
    options: [
    { ...TipoModeloDocumentoEmpty(), id: 1, nome: 'Tipo Modelo Documento 1' },
    { ...TipoModeloDocumentoEmpty(), id: 2, nome: 'Tipo Modelo Documento 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TipoModeloDocumento/Apis/ApiTipoModeloDocumento', () => ({
  TipoModeloDocumentoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoModeloDocumento' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoModeloDocumento' }),
...
jest.mock('@/app/GerAdv_TS/TipoModeloDocumento/Services/TipoModeloDocumento.service', () => ({
  TipoModeloDocumentoService: jest.fn().mockImplementation(() => ({
    fetchTipoModeloDocumentoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoModeloDocumento' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TipoModeloDocumento/Crud/Grids/TipoModeloDocumentoWindow', () => {
  return function MockTipoModeloDocumentoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tipomodelodocumento-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Tipo Modelo Documento' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TipoModeloDocumentoTestEmpty(), id: 1, nome: 'Tipo Modelo Documento 1' },
  { ...TipoModeloDocumentoTestEmpty(), id: 2, nome: 'Tipo Modelo Documento 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TipoModeloDocumentoEmpty(), 
};
describe('TipoModeloDocumentoComboBox', () => {
  ...
  const { useTipoModeloDocumentoComboBox } = require('@/app/GerAdv_TS/TipoModeloDocumento/Hooks/hookTipoModeloDocumento');
  (useTipoModeloDocumentoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TipoModeloDocumentoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Tipo Modelo Documento')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Tipo Modelo Documento')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TipoModeloDocumentoTestEmpty(), id: 1, nome: 'Tipo Modelo Documento 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTipoModeloDocumentoComboBox } = require('@/app/GerAdv_TS/TipoModeloDocumento/Hooks/hookTipoModeloDocumento');
    useTipoModeloDocumentoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TipoModeloDocumentoWindow on action button click (add)', async () => {
    render(<TipoModeloDocumentoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tipomodelodocumento-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tipomodelodocumento-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TipoModeloDocumentoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TipoModeloDocumentoWindow onClose and onSuccess', () => {
    render(<TipoModeloDocumentoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TipoModeloDocumentoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
