...
public static class FilterAgendaRelatorioTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar AgendaRelatorio em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterAgendaRelatorio:

- O filter irá filtrar os AgendaRelatorio conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterAgendaRelatorio:(Agenda Relatorio)
- vqadata: Data;
- vqaprocesso: número ou IDs;
- xxxparanome: ParaNome;
- xxxparapessoas: ParaPessoas;
- xxxboxaudiencia: BoxAudiencia;
- xxxboxaudienciamobile: BoxAudienciaMobile;
- xxxnomeadvogado: NomeAdvogado;
- xxxnomeforo: NomeForo;
- xxxnomejustica: NomeJustica;
- xxxnomearea: NomeArea;
...
- Se ouvir "todos os AgendaRelatorio" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterAgendaRelatorio? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt AgendaRelatorio length: {result.Length}");
#endif
