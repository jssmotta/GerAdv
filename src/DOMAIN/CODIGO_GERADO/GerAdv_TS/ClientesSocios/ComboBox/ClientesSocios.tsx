...
const ClientesSociosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IClientesSocios | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IClientesSocios | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IClientesSocios);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ClientesSociosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedClientesSocios={editar ?? addValue ?? ClientesSociosEmpty()} /> 
            )}
