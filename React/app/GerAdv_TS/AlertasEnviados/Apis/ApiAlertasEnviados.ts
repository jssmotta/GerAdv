...
import { AlertasEnviados } from '../../Models/AlertasEnviados';
...
export class AlertasEnviadosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AlertasEnviadosApiError';
    this.status = status;
    this.code = code;
...
export class AlertasEnviadosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AlertasEnviados`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlertasEnviados_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AlertasEnviados');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlertasEnviados_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Alertas Enviados com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAlertasEnviados): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlertasEnviados_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AlertasEnviados');
                }
            }


    public async validation(regAlertasEnviados: IAlertasEnviados): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAlertasEnviados as AlertasEnviados, this.getHeaders());
    ...
    public async addAndUpdate(regAlertasEnviados: IAlertasEnviados): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAlertasEnviados as AlertasEnviados, this.getHeaders());
            var register = result.data as IAlertasEnviados;        
            const action = regAlertasEnviados.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Alertas Enviados');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AlertasEnviados. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AlertasEnviados',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Alertas Enviados com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAlertasEnviados) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AlertasEnviados[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de alertasenviados:', error);
            this.notificationService.notify({
              entity: 'AlertasEnviados',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos alertasenviados'
            });
          }
        });
    }
