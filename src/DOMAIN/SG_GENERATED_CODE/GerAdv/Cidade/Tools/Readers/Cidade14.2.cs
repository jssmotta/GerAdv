...
#pragma warning restore CA1822 // Mark members as static

    {
        if (oCnn is null)
            return;
        Console.WriteLine($"WarmUp Cidade: {tenantKey}");
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_Cidade_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[Cidade]')";
