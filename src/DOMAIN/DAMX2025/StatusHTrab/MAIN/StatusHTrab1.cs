...
public partial class DBStatusHTrab : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_StatusHTrab)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBStatusHTrab();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_StatusHTrab
    [XmlIgnore]
    public string TabelaNome => "StatusHTrab";

    public DBStatusHTrab()
    {
    }

#endregion
    public DBStatusHTrab(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBStatusHTrab(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBStatusHTrab(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#endif
#region GravarDados_StatusHTrab
#if (forWeb)
...
#endif
    {
        ...
        {
            throw new Exception("Campo 'shtDescricao' está vazio!");
        }
        ...
        if (pFldFDescricao)
            clsW.Fields(DBStatusHTrabDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFResID)
            clsW.Fields(DBStatusHTrabDicInfo.ResID, m_FResID, ETiposCampos.FNumberNull);
        if (insertId != 0)
            return GravaNewId();
