...
import { useProcessosParadosGrid } from '../../Hooks/hookProcessosParadosGrid';
...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('processosparados-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('processosparados-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('processosparados-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-processosparados',
  columns: exportColumns, 
  sheetName: 'ProcessosParados'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-ProcessosParados-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-processosparados',
  columns: exportColumnsPdf, 
  title: 'ProcessosParados'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-ProcessosParados-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`processosparados-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-processosparados'
