...
public partial class PreClientesController(IPreClientesService preclientesService) : ControllerBase
{
    ...
    {
        _logger.Info("PreClientes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _preclientesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("PreClientes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _preclientesService.UpdateColumns(parameters, uri);
        if (!result)
        {
