...
import CargosEscWindow from '../Crud/Grids/CargosEscWindow';
...
import { useCargosEscComboBox } from '../Hooks/hookCargosEsc';
const CargosEscComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useCargosEscComboBox(cargosescService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<ICargosEsc | null>(null);
const [addRecord, setAddRecord] = useState<ICargosEsc | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await cargosescService.fetchCargosEscById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = CargosEscEmpty();
...
const handleSuccess = (newCargosEsc?: ICargosEsc) => {
  if (newCargosEsc) {
    setValue({ id: newCargosEsc.id, nome: newCargosEsc.nome });
    loadRecordForEdit(newCargosEsc.id);
    handleValueChange(newCargosEsc);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .cargosesc-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .cargosesc-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .cargosesc-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedCargosEsc={editRecord || addRecord || CargosEscEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'cargosesc-dropdown-popup-msi'
          }}
          />
...
export default CargosEscComboBox;