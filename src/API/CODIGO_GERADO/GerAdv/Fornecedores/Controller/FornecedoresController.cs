...
public partial class FornecedoresController(IFornecedoresService fornecedoresService) : ControllerBase
{
    ...
    {
        _logger.Info("Fornecedores: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _fornecedoresService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Fornecedores: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _fornecedoresService.UpdateColumns(parameters, uri);
        if (!result)
        {
