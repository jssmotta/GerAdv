...
import OponentesWindow from '../Crud/Grids/OponentesWindow';
...
import { useOponentesComboBox } from '../Hooks/hookOponentes';
const OponentesComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useOponentesComboBox(oponentesService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IOponentes | null>(null);
const [addRecord, setAddRecord] = useState<IOponentes | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await oponentesService.fetchOponentesById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = OponentesEmpty();
...
const handleSuccess = (newOponentes?: IOponentes) => {
  if (newOponentes) {
    setValue({ id: newOponentes.id, nome: newOponentes.nome });
    loadRecordForEdit(newOponentes.id);
    handleValueChange(newOponentes);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .oponentes-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .oponentes-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .oponentes-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedOponentes={editRecord || addRecord || OponentesEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'oponentes-dropdown-popup-msi'
          }}
          />
...
export default OponentesComboBox;