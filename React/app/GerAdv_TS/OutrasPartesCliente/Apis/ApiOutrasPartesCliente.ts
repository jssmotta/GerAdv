...
import { OutrasPartesCliente } from '../../Models/OutrasPartesCliente';
...
export class OutrasPartesClienteApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'OutrasPartesClienteApiError';
    this.status = status;
    this.code = code;
...
export class OutrasPartesClienteApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/OutrasPartesCliente`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OutrasPartesCliente_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os OutrasPartesCliente');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OutrasPartesCliente_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Outras Partes Cliente com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterOutrasPartesCliente): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OutrasPartesCliente_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de OutrasPartesCliente');
        }
    }

            public async filter(filtro: FilterOutrasPartesCliente): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OutrasPartesCliente_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar OutrasPartesCliente');
                }
            }


    public async validation(regOutrasPartesCliente: IOutrasPartesCliente): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regOutrasPartesCliente as OutrasPartesCliente, this.getHeaders());
    ...
    public async addAndUpdate(regOutrasPartesCliente: IOutrasPartesCliente): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regOutrasPartesCliente as OutrasPartesCliente, this.getHeaders());
            var register = result.data as IOutrasPartesCliente;        
            const action = regOutrasPartesCliente.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Outras Partes Cliente');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o OutrasPartesCliente. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'OutrasPartesCliente',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Outras Partes Cliente com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterOutrasPartesCliente) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<OutrasPartesCliente[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de outraspartescliente:', error);
            this.notificationService.notify({
              entity: 'OutrasPartesCliente',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos outraspartescliente'
            });
          }
        });
    }
