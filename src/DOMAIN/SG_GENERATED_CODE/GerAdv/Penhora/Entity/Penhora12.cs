...
public partial class FPenhora : MenphisSI.SG.GerAdv.DBPenhora, IDBPenhora
{
    // DI-compatible default constructor
    public FPenhora() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FPenhora CreateFromReader(SqlDataReader? dbRec)
    {
        return new FPenhora(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FPenhora CreateFromDataRow(DataRow? dbRec)
    {
        return new FPenhora(dbRec);
    }

    private FPenhora(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FPenhora(DataRow? dbRec) : base(dbRec)
    {
    }

    private FPenhora(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FPenhora CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FPenhora(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FPenhora> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fPenhora = new FPenhora();
        await fPenhora.CarregarAsync(id, oCnn);
        return fPenhora;
    }
