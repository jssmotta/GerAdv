...
const CargosEscComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ICargosEsc | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ICargosEsc | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ICargosEsc);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <CargosEscWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedCargosEsc={editar ?? addValue ?? CargosEscEmpty()} /> 
            )}
