...
public partial class DBApenso2
{
    ...
    /// </summary>
    public static async Task<DBApenso2> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBApenso2();
...
#if (!NOTSTORED_Apenso2)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFApensado;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBApenso2DicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFApensado)
            updateTool.Fields(DBApenso2DicInfo.Apensado, m_FApensado, ETiposCampos.FNumber);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBApenso2DicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBApenso2DicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBApenso2DicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBApenso2DicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBApenso2DicInfo.Visto, false, ETiposCampos.FBoolean);
    }
