...
import { ApensoApiError } from '../GerAdv_TS/Apenso/Apis/ApiApenso';
import { ApensoApi } from '@/app/GerAdv_TS/Apenso/Apis/ApiApenso';
import { FilterApenso } from '../GerAdv_TS/Apenso/Filters/Apenso';
...
describe('ApensoApi', () => {
  ...
  beforeEach(() => {
    ...
    apensoApi = new ApensoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(apensoApi).toBeInstanceOf(ApensoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ApensoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ApensoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all apenso items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await apensoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(apensoApi.getAll()).rejects.toThrow(ApensoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await apensoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Apenso_last_getAll_${200}`)));
    });
    ...
    it('should throw ApensoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockApenso = ApensoTestEmpty();
    ...
    it('should fetch apenso by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await apensoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockApenso));
      ...
      expect(result.data).toEqual(mockApenso);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Apenso',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter apenso items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await apensoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new apenso successfully', async () => {
      const responseApenso = { ...mockApenso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseApenso,
        status: 200,
        statusText: 'OK',
      ...
      const result = await apensoApi.addAndUpdate(mockApenso);
    ...
    it('should update existing apenso successfully', async () => {
      const existingApenso = { ...mockApenso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingApenso,
        status: 200,
        statusText: 'OK',
      ...
      const result = await apensoApi.addAndUpdate(existingApenso);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(apensoApi.addAndUpdate(mockApenso)).rejects.toThrow(ApensoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Apenso',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete apenso successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await apensoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(apensoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Apenso',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(apensoApi.delete(1)).rejects.toThrow(ApensoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterApenso = { funcionario: 1 };
      
      const result = apensoApi.useFilter(mockFilter);
  ...
  describe('ApensoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ApensoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ApensoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ApensoApiError('Test message', 500);
