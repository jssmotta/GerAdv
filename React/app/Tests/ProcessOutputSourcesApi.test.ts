...
import { ProcessOutputSourcesApiError } from '../GerAdv_TS/ProcessOutputSources/Apis/ApiProcessOutputSources';
import { ProcessOutputSourcesApi } from '@/app/GerAdv_TS/ProcessOutputSources/Apis/ApiProcessOutputSources';
import { FilterProcessOutputSources } from '../GerAdv_TS/ProcessOutputSources/Filters/ProcessOutputSources';
...
describe('ProcessOutputSourcesApi', () => {
  let processoutputsourcesApi: ProcessOutputSourcesApi;
  ...
  beforeEach(() => {
    ...
    processoutputsourcesApi = new ProcessOutputSourcesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processoutputsourcesApi).toBeInstanceOf(ProcessOutputSourcesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessOutputSourcesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessOutputSourcesApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ProcessOutputSourcesTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all processoutputsources items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputsourcesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processoutputsourcesApi.getAll()).rejects.toThrow(ProcessOutputSourcesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processoutputsourcesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProcessOutputSources_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputSources',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw ProcessOutputSourcesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessOutputSources = ProcessOutputSourcesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProcessOutputSources,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch processoutputsources by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputsourcesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessOutputSources));
      ...
      expect(result.data).toEqual(mockProcessOutputSources);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputSources',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProcessOutputSources = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter processoutputsources items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processoutputsourcesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProcessOutputSources: IProcessOutputSources = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new processoutputsources successfully', async () => {
      const responseProcessOutputSources = { ...mockProcessOutputSources, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessOutputSources,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputsourcesApi.addAndUpdate(mockProcessOutputSources);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockProcessOutputSources,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing processoutputsources successfully', async () => {
      const existingProcessOutputSources = { ...mockProcessOutputSources, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessOutputSources,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputsourcesApi.addAndUpdate(existingProcessOutputSources);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processoutputsourcesApi.addAndUpdate(mockProcessOutputSources)).rejects.toThrow(ProcessOutputSourcesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputSources',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processoutputsources successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processoutputsourcesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processoutputsourcesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputSources',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processoutputsourcesApi.delete(1)).rejects.toThrow(ProcessOutputSourcesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProcessOutputSources = { funcionario: 1 };
      
      const result = processoutputsourcesApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await processoutputsourcesApi.delete(1);
  ...
  describe('ProcessOutputSourcesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessOutputSourcesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessOutputSourcesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessOutputSourcesApiError('Test message', 500);
