...
public partial class ColaboradoresController(IColaboradoresService colaboradoresService) : ControllerBase
{
    ...
    {
        _logger.Info("Colaboradores: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _colaboradoresService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Colaboradores: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _colaboradoresService.UpdateColumns(parameters, uri);
        if (!result)
        {
