...
SELECT TOP (1) 
    ...
    forVisto as Visto
FROM {oCnn.UseDbo}.Foro dbq WITH (INDEX = idx_Foro_AuditorDtAtu)
...
FROM {oCnn.UseDbo}.Foro WITH (INDEX = idx_{DBForoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [forCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBForoDicInfo.PTabelaNome}-RRSAA";
...
FROM [{oCnn.UseDbo}].[{DBForoDicInfo.PTabelaNome}] (NOLOCK)
