// app/TestsForms/LigacoesComboBox.test.tsx - Vers�o Corrigida
...
import { LigacoesEmpty, LigacoesTestEmpty } from '@/app/GerAdv_TS/Models/Ligacoes';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Ligacoes/Hooks/hookLigacoes', () => ({
  useLigacoesComboBox: jest.fn(() => ({
    options: [
    { ...LigacoesEmpty(), id: 1, nome: 'Ligacoes 1' },
    { ...LigacoesEmpty(), id: 2, nome: 'Ligacoes 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Ligacoes/Apis/ApiLigacoes', () => ({
  LigacoesApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ligacoes' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ligacoes' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Ligacoes/Services/Ligacoes.service', () => ({
  LigacoesService: jest.fn().mockImplementation(() => ({
    fetchLigacoesById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ligacoes' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Ligacoes/Crud/Grids/LigacoesWindow', () => {
  return function MockLigacoesWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='ligacoes-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Ligacoes' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...LigacoesTestEmpty(), id: 1, nome: 'Ligacoes 1' },
  { ...LigacoesTestEmpty(), id: 2, nome: 'Ligacoes 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: LigacoesEmpty(), 
};
describe('LigacoesComboBox', () => {
  ...
  const { useLigacoesComboBox } = require('@/app/GerAdv_TS/Ligacoes/Hooks/hookLigacoes');
  (useLigacoesComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<LigacoesComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Ligacoes')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Ligacoes')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...LigacoesTestEmpty(), id: 1, nome: 'Ligacoes 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useLigacoesComboBox } = require('@/app/GerAdv_TS/Ligacoes/Hooks/hookLigacoes');
    useLigacoesComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens LigacoesWindow on action button click (add)', async () => {
    render(<LigacoesComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ligacoes-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('ligacoes-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<LigacoesComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles LigacoesWindow onClose and onSuccess', () => {
    render(<LigacoesComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<LigacoesComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
