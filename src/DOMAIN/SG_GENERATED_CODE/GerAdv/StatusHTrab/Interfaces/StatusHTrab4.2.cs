...
{
    Task<StatusHTrabResponse?> ReadAsync(int id, MsiSqlConnection? oCnn);
    Task<Models.StatusHTrab?> ReadMAsync(int id, MsiSqlConnection? oCnn);
    ...
    Task<IEnumerable<StatusHTrabResponseAll>> ListarAsync(MsiSqlConnection oCnn, int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken);
}