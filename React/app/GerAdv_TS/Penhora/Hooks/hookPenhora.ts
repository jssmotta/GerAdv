  ...
  const loadPenhora = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Penhora";
...
export const usePenhoraList = (
  dataService: IPenhoraService,
  currentFilter?: FilterPenhora | undefined | null,
) => {
  const [data, setData] = useState<IPenhora[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPenhora | undefined | null>(
    currentFilter,
  );
    ...
    try {
      ...
      if (data.descricao.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Descricao não pode ter mais de 2147483647 caracteres.",
        };
      }
