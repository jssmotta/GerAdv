...
# Attach políticas necessárias para execution role
    ...
    Statement = [
      {
        ...
        Resource = [
          aws_secretsmanager_secret.sql_connection.arn
        ]
      }
    ]
  })
...
# IAM Role para ECS Task (runtime da aplicação)
...
# Política para CloudWatch Logs (para aplicação enviar logs)
...
# Política para S3 (se a aplicação precisar acessar S3)
...
# Política para Secrets Manager (se a aplicação precisar acessar secrets em runtime)
