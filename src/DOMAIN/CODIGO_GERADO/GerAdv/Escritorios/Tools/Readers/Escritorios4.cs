...
public partial interface IEscritoriosReader
{
    ...
    EscritoriosResponse? Read(Entity.DBEscritorios dbRec, MsiSqlConnection oCnn);
    ...
    EscritoriosResponseAll? ReadAll(DBEscritorios dbRec, SqlDataReader dr);
    IEnumerable<DBNomeID> ListarN(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
    IEnumerable<EscritoriosResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
}

public partial class Escritorios : IEscritoriosReader
{
    public IEnumerable<DBNomeID> ListarN(int max, string uri, string cWhere, List<SqlParameter> parameters, string order) => DevourerSqlData.ListarNomeID(BuildSqlQuery("{escCodigo, escNome}", cWhere, order, max), parameters, uri, caching: DevourerOne.PCachingDefault, max: max);
    public IEnumerable<EscritoriosResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order) => ListarTabela(BuildSqlQuery("*", cWhere, order, max), parameters, uri, caching: DevourerOne.PCachingDefault, max: max);
    private IEnumerable<EscritoriosResponseAll> ListarTabela(string sql, List<SqlParameter> parameters, string uri, bool caching = DevourerOne.PCachingDefault, int max = 200)
    {
        using var oCnn = new MsiSqlConnection(uri);
        return ReadLocalAsync().Result;
        async Task<List<EscritoriosResponseAll>> ReadLocalAsync()
        {
            ...
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                if (await reader.IsDBNullAsync(1).ConfigureAwait(false))
                    continue;
                result.Add(ReadAll(new Entity.DBEscritorios(reader), reader)!);
            }
    ...
    static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var query = new StringBuilder($"SELECT TOP ({max}) {campos}  FROM {"Escritorios".dbo(oCnn)} (NOLOCK) ");
    ...
    public EscritoriosResponse? Read(Entity.DBEscritorios dbRec, MsiSqlConnection oCnn)
    {
        return Read(dbRec);
    }
    ...
    public EscritoriosResponseAll? ReadAll(DBEscritorios dbRec, SqlDataReader dr)
    {
        if (dbRec == null)
        {
            return null;
