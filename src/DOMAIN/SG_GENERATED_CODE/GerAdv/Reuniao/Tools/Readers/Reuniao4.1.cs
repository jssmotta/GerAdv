...
[{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBReuniaoDicInfo.PTabelaNome}].[{DBReuniaoDicInfo.Cliente}]
...
SELECT TOP (1) 
	FORMAT(renDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(renDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    renVisto as Visto
FROM {oCnn.UseDbo}.Reuniao dbq WITH (INDEX = idx_Reuniao_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.renQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.renQuemAtu
WHERE [renCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBReuniao.PTabelaNome,
...
FROM {oCnn.UseDbo}.Reuniao WITH (INDEX = idx_{DBReuniaoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [renCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBReuniaoDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(renDtAtu, renDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBReuniaoDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(renDtAtu, renDtCad) DESC;", oCnn.InnerConnection);
