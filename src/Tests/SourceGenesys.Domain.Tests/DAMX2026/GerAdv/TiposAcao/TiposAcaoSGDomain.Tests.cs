// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Entity:TiposAcao
// Source:SGDomainTests
/// <summary>
/// Template de testes unitários para classes geradas pelo Source Genesys
/// Variáveis de substituição:
/// MenphisSI.GerAdv - Namespace da classe
/// DBTiposAcao - Nome da classe (ex: DBClientes)
/// TiposAcao - Nome da tabela (ex: Clientes)
/// tac - Prefixo dos campos (ex: cli)
/// tacCodigo - Nome do campo código (ex: cliCodigo)
/// tacNome - Nome do campo nome (ex: cliNome)
/// DBTiposAcao.CadastroGuid - GUID do cadastro
/// </summary>
namespace MenphisSI.GerAdv.Tests;
public class DBTiposAcaoTests : IDisposable
{
    private readonly DBTiposAcao _instance;
    private readonly Mock<MsiSqlConnection> _mockConnection;
    private readonly DataTable _testDataTable;
    public DBTiposAcaoTests()
    {
        _instance = new DBTiposAcao();
        _mockConnection = new Mock<MsiSqlConnection>();
        _testDataTable = CreateTestDataTable();
    }

    private DataTable CreateTestDataTable()
    {
        var dt = new DataTable();
        // Campos obrigatórios Source Genesys
        dt.Columns.Add("tacCodigo", typeof(int));
        dt.Columns.Add("tacQuemCad", typeof(int));
        dt.Columns.Add("tacDtCad", typeof(DateTime));
        dt.Columns.Add("tacQuemAtu", typeof(int));
        dt.Columns.Add("tacDtAtu", typeof(DateTime));
        dt.Columns.Add("tacVisto", typeof(bool));
        dt.Columns.Add("tacNome", typeof(string));
        dt.Columns.Add("tacInativo", typeof(string));
        dt.Columns.Add("tacBold", typeof(string));
        dt.Columns.Add("tacGuid", typeof(string));
        return dt;
    }

    [Fact]
    public void Constructor_WithValidDataRow_ShouldLoadData()
    {
        // Arrange
        var row = _testDataTable.NewRow();
        row["tacCodigo"] = 123;
        _testDataTable.Rows.Add(row);
        // Act
        var instance = new DBTiposAcao(_testDataTable.Rows[0]);
        // Assert
        Assert.Equal(123, instance.ID);
    }

#region Testes de Constantes e Propriedades Estáticas
    [Fact]
    public void Constants_ShouldHaveCorrectValues()
    {
        Assert.Equal("TiposAcao", DBTiposAcao.PTabelaNome);
        Assert.Equal(" TiposAcao.* ", DBTiposAcao.CamposSqlX);
        Assert.Equal("tacCodigo", DBTiposAcao.CampoCodigo);
        Assert.Equal("tacNome", DBTiposAcao.CampoNome);
        Assert.Equal("tac", DBTiposAcao.PTabelaPrefixo);
    }

#endregion
#region Testes de Construtores
    [Fact]
    public void Constructor_Default_ShouldInitializeCorrectly()
    {
        var instance = new DBTiposAcao();
        Assert.Equal(0, instance.ID);
        Assert.Equal("TiposAcao", instance.ITableName());
        Assert.Equal("tac", instance.Prefixo);
    }

    [Fact]
    public void Constructor_WithNullDataRow_ShouldNotThrow()
    {
        var instance = new DBTiposAcao((DataRow)null !);
        Assert.Equal(0, instance.ID);
    }

    [Fact]
    public void Constructor_WithNullSqlDataReader_ShouldNotThrow()
    {
        var instance = new DBTiposAcao((SqlDataReader)null !);
        Assert.Equal(0, instance.ID);
    }

#endregion
#region Testes de Interfaces
    [Fact]
    public void ICrud_Implementation_ShouldWork()
    {
        ICrud cadastro = (ICrud)_instance;
        Assert.Equal("TiposAcao", cadastro.ITableName());
        Assert.Equal("tacCodigo", cadastro.IFieldId());
        Assert.Equal("tacNome", cadastro.IFieldNameDescription());
        Assert.Equal("tac", cadastro.IPrefix());
    }

#endregion
#region Testes de Métodos Assíncronos
    [Fact]
    public async Task CreateAsync_WithInvalidId_ShouldReturnEmptyInstance()
    {
        var instance = await DBTiposAcao.CreateAsync(0, null);
        Assert.Equal(0, instance.ID);
    }

    [Fact]
    public async Task ExisteAsync_WithNullConnection_ShouldReturnFalse()
    {
        var exists = await DBTiposAcao.ExisteAsync(1, null);
        Assert.False(exists);
    }

    [Fact]
    public async Task DeleteRecordAsync_WithInvalidId_ShouldReturnFalse()
    {
        var result = await DBTiposAcao.DeleteRecordAsync(1, 0, null);
        Assert.False(result);
    }

#endregion
#region Testes de Dispose
    [Fact]
    public void Dispose_ShouldNotThrow()
    {
        var instance = new DBTiposAcao();
        var exception = Record.Exception(() => instance.Dispose());
        Assert.Null(exception);
    }

    [Fact]
    public void Dispose_MultipleCalls_ShouldBeIdempotent()
    {
        var instance = new DBTiposAcao();
        instance.Dispose();
        var exception = Record.Exception(() => instance.Dispose());
        Assert.Null(exception);
    }

#endregion
#region Testes de Propriedades Computadas
    [Fact]
    public void HasAuditor_ShouldReturnTrue()
    {
        Assert.True(_instance.HasAuditor());
    }

    [Fact]
    public void HasNameId_ShouldReturnTrue()
    {
        Assert.True(_instance.HasNameId());
    }

    [Fact]
    public void IsStoredProcedureOrView_ShouldReturnFalse()
    {
        Assert.False(_instance.IsStoredProcedureOrView());
    }

#endregion
    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Nome_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FNome = input;
        Assert.Equal(expected, _instance.FNome);
    }

    [Fact]
    public void Nome_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 80 + 10);
        _instance.FNome = longString;
        Assert.True(_instance.FNome.Length <= 80);
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void Inativo_ShouldAcceptBooleanValues(bool value)
    {
        _instance.FInativo = value;
        Assert.Equal(value, _instance.FInativo);
    }

    [Fact]
    public void Inativo_DefaultValue_ShouldBeFalse()
    {
        var instance = new DBTiposAcao();
        Assert.False(instance.FInativo);
    }

    public virtual void Dispose()
    {
        _instance?.Dispose();
    }
}