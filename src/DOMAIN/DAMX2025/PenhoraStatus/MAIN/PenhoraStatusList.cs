...
#region ListarDados_PenhoraStatus
    public static async IAsyncEnumerable<DBPenhoraStatus> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPenhoraStatus(dbRec);
        }
    }

    public static IEnumerable<DBPenhoraStatus> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
