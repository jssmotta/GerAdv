...
import { TiposAcao } from '../../Models/TiposAcao';
...
export class TiposAcaoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class TiposAcaoApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TiposAcao_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TiposAcao');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TiposAcao_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipos Acao com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterTiposAcao): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TiposAcao_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TiposAcao');
        }
    }

            public async filter(filtro: FilterTiposAcao): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TiposAcao_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TiposAcao');
                }
            }


    public async validation(regTiposAcao: ITiposAcao): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTiposAcao as TiposAcao, this.getHeaders());
    ...
    public async addAndUpdate(regTiposAcao: ITiposAcao): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regTiposAcao as TiposAcao, this.getHeaders());
            var register = result.data as ITiposAcao;        
            const action = regTiposAcao.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Tipos Acao');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o TiposAcao. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'TiposAcao',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Tipos Acao com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTiposAcao) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<TiposAcao[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de tiposacao:', error);
            this.notificationService.notify({
              entity: 'TiposAcao',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tiposacao'
            });
          }
        });
    }
