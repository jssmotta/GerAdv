...
public static class FilterAcaoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Acao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterAcao:

- O filter irá filtrar os Acao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Acao" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterAcao? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Acao length: {result.Length}");
#endif
