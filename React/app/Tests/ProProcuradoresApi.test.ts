...
import { ProProcuradoresApiError } from '../GerAdv_TS/ProProcuradores/Apis/ApiProProcuradores';
import { ProProcuradoresApi } from '@/app/GerAdv_TS/ProProcuradores/Apis/ApiProProcuradores';
import { FilterProProcuradores } from '../GerAdv_TS/ProProcuradores/Filters/ProProcuradores';
...
describe('ProProcuradoresApi', () => {
  let proprocuradoresApi: ProProcuradoresApi;
  ...
  beforeEach(() => {
    ...
    proprocuradoresApi = new ProProcuradoresApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(proprocuradoresApi).toBeInstanceOf(ProProcuradoresApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProProcuradoresApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProProcuradoresApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all proprocuradores items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proprocuradoresApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(proprocuradoresApi.getAll()).rejects.toThrow(ProProcuradoresApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await proprocuradoresApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProProcuradores_last_getAll_${200}`)));
    });
    ...
    it('should throw ProProcuradoresApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProProcuradores = ProProcuradoresTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProProcuradores,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch proprocuradores by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proprocuradoresApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProProcuradores));
      ...
      expect(result.data).toEqual(mockProProcuradores);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProProcuradores',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProProcuradores = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter proprocuradores items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await proprocuradoresApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProProcuradores: IProProcuradores = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new proprocuradores successfully', async () => {
      const responseProProcuradores = { ...mockProProcuradores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProProcuradores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proprocuradoresApi.addAndUpdate(mockProProcuradores);
    ...
    it('should update existing proprocuradores successfully', async () => {
      const existingProProcuradores = { ...mockProProcuradores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProProcuradores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proprocuradoresApi.addAndUpdate(existingProProcuradores);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(proprocuradoresApi.addAndUpdate(mockProProcuradores)).rejects.toThrow(ProProcuradoresApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProProcuradores',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete proprocuradores successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await proprocuradoresApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(proprocuradoresApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProProcuradores',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(proprocuradoresApi.delete(1)).rejects.toThrow(ProProcuradoresApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProProcuradores = { funcionario: 1 };
      
      const result = proprocuradoresApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await proprocuradoresApi.delete(1);
  ...
  describe('ProProcuradoresApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProProcuradoresApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProProcuradoresApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProProcuradoresApiError('Test message', 500);
