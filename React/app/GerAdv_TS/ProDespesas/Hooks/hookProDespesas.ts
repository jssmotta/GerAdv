...
export const useProDespesasForm = (
  ...
  const loadProDespesas = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Pro Despesas";
...
export const useProDespesasList = (
  dataService: IProDespesasService,
  currentFilter?: FilterProDespesas | undefined | null,
) => {
  const [data, setData] = useState<IProDespesas[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProDespesas | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterProDespesas | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar prodespesas";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IProDespesas): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.historico.length > 100) {
        return {
          isValid: false,
          message: "O campo Historico não pode ter mais de 100 caracteres.",
        };
      }
