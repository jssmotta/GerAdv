...
public static class FilterLigacoesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Ligacoes em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterLigacoes:

- O filter irá filtrar os Ligacoes conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterRamal: filtro específico para Ramal
...
CAMPOS filterRamal (Ramal):
- nome: _nome ou descrição para Ramal;
- obs: Obs;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
...
CAMPOS filterLigacoes:(Ligacoes)
- assunto: Assunto;
- ageclienteavisado: número ou IDs;
- celular: Celular;
- cliente: número ou IDs;
- contato: Contato;
- datarealizada: DataRealizada;
- quemid: número ou IDs;
- telefonista: número ou IDs;
- ultimoaviso: UltimoAviso;
- horafinal: HoraFinal;
- nome: _nome ou descrição para Ligacoes;
- quemcodigo: número ou IDs;
- solicitante: número ou IDs;
- para: Para;
- fone: Fone;
- ramal: número ou IDs;
...
- realizada: Realizada;
- status: Status;
- data: não é a data de cadastramento é a data do Ligacoes;
- hora: Hora;
- urgente: Urgente;
- ligarpara: LigarPara;
- processo: número ou IDs;
- startscreen: StartScreen;
- emotion: número ou IDs;
- bold: Bold;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
...
- Se ouvir "todos os Ligacoes" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterLigacoes? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Ligacoes length: {result.Length}");
#endif
