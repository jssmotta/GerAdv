...
public static class FilterStatusTarefasTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar StatusTarefas em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterStatusTarefas:

- O filter irá filtrar os StatusTarefas conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterStatusTarefas:(Status Tarefas)
- nome: _nome ou descrição para Status Tarefas;
...
- Se ouvir "todos os StatusTarefas" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterStatusTarefas? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt StatusTarefas length: {result.Length}");
#endif
