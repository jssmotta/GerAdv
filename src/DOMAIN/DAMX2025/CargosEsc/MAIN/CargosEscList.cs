...
#region ListarDados_CargosEsc
    public static async IAsyncEnumerable<DBCargosEsc> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBCargosEsc(dbRec);
        }
    }

    public static IEnumerable<DBCargosEsc> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
