...
import ProCDAWindow from '../Crud/Grids/ProCDAWindow';
...
import { useProCDAComboBox } from '../Hooks/hookProCDA';
const ProCDAComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useProCDAComboBox(procdaService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IProCDA | null>(null);
const [addRecord, setAddRecord] = useState<IProCDA | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await procdaService.fetchProCDAById(id);
...
const handleSuccess = (newProCDA?: IProCDA) => {
  if (newProCDA) {
    setValue({ id: newProCDA.id, nome: newProCDA.nome });
    loadRecordForEdit(newProCDA.id);
    handleValueChange(newProCDA);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .procda-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .procda-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .procda-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedProCDA={editRecord || addRecord || ProCDAEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'procda-dropdown-popup-msi'
          }}
          />
