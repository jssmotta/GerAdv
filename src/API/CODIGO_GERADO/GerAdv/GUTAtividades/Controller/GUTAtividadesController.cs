...
public partial class GUTAtividadesController(IGUTAtividadesService gutatividadesService) : ControllerBase
{
    ...
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] MenphisSI.GerAdv.Filters.FilterGUTAtividades filter, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("GUTAtividades: Filter called max {0} with filtro = {1}, {2}", max, filter, uri);
        var result = await _gutatividadesService.Filter(max, filter, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> FilterVoice([FromBody] MenphisSI.GerAdv.Filters.FilterGUTAtividadesWithVoiceRequest request, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("GUTAtividades: Filter called with {0} filtro = {1}", request, uri);
        var result = await _gutatividadesService.FilterVoice(request.Filter, request.VoiceCommand, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetAuditor(int id, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        try
        {
            //_logger.Info("GUTAtividades: GetAuditor called with id = {0}, {1}", id, uri);
            var result = await _gutatividadesService.GetAuditor(id, uri, token);
            if (result == null)
            {
