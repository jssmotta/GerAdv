...
export const useContatoCRMViewForm = (
  ...
  const loadContatoCRMView = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Contato C R M View";
...
export const useContatoCRMViewList = (
  dataService: IContatoCRMViewService,
  currentFilter?: FilterContatoCRMView | undefined | null,
) => {
  const [data, setData] = useState<IContatoCRMView[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterContatoCRMView | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterContatoCRMView | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar contatocrmview";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsContatoCRMView() {
  ...
  function validate(data: IContatoCRMView): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.cguid.length > 100) {
        return {
          isValid: false,
          message: "O campo CGUID não pode ter mais de 100 caracteres.",
        };
      }
      if (data.ip.length > 50) {
        return {
          isValid: false,
          message: "O campo IP não pode ter mais de 50 caracteres.",
        };
      }
