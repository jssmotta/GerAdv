...
import ForoWindow from '../Crud/Grids/ForoWindow';
...
import { useForoComboBox } from '../Hooks/hookForo';
const ForoComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useForoComboBox(foroService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IForo | null>(null);
const [addRecord, setAddRecord] = useState<IForo | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await foroService.fetchForoById(id);
...
const handleSuccess = (newForo?: IForo) => {
  if (newForo) {
    setValue({ id: newForo.id, nome: newForo.nome });
    loadRecordForEdit(newForo.id);
    handleValueChange(newForo);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .foro-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .foro-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .foro-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedForo={editRecord || addRecord || ForoEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'foro-dropdown-popup-msi'
          }}
          />
