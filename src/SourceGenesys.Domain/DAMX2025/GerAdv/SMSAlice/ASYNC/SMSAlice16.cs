...
public partial class DBSMSAlice
{
    ...
    {
        if (pFldFOperador)
            updateTool.Fields(DBSMSAliceDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBSMSAliceDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFTipoEMail)
            updateTool.Fields(DBSMSAliceDicInfo.TipoEMail, m_FTipoEMail, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBSMSAliceDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBSMSAliceDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
