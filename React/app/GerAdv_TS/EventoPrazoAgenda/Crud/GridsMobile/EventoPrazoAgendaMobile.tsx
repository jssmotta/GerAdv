...
import { useEventoPrazoAgendaGrid } from "../../Hooks/hookEventoPrazoAgendaGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("eventoprazoagenda-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-eventoprazoagenda .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("eventoprazoagenda-columns-order-mobile"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-eventoprazoagenda",
      columns: exportColumns,
      sheetName: "EventoPrazoAgenda",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-EventoPrazoAgenda-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-eventoprazoagenda",
      columns: exportColumnsPdf,
      title: "EventoPrazoAgenda",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-EventoPrazoAgenda-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-eventoprazoagenda"
              key={`eventoprazoagenda-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
