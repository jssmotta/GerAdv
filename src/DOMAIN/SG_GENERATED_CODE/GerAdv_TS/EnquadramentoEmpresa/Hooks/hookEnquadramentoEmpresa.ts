...
export const useEnquadramentoEmpresaList = (dataService: IEnquadramentoEmpresaService, currentFilter?: FilterEnquadramentoEmpresa | undefined | null) => {
  const [data, setData] = useState<IEnquadramentoEmpresa[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterEnquadramentoEmpresa | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterEnquadramentoEmpresa | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterEnquadramentoEmpresa);
      setData(result);
    } catch (err) {
