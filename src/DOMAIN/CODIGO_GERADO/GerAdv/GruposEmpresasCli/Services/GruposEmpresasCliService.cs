...
public partial class GruposEmpresasCliService(IOptions<AppSettings> appSettings, IFGruposEmpresasCliFactory gruposempresascliFactory, IGruposEmpresasCliReader reader, IGruposEmpresasCliValidation validation, IGruposEmpresasCliWriter writer, IGruposEmpresasReader gruposempresasReader, IClientesReader clientesReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IGruposEmpresasCliService, IDisposable
{
    ...
    private readonly IFGruposEmpresasCliFactory gruposempresascliFactory = gruposempresascliFactory;
    private readonly IGruposEmpresasCliReader reader = reader;
    private readonly IGruposEmpresasCliValidation validation = validation;
    private readonly IGruposEmpresasCliWriter writer = writer;
    private readonly IGruposEmpresasReader gruposempresasReader = gruposempresasReader;
    ...
    public async Task<IEnumerable<GruposEmpresasCliResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-GruposEmpresasCli-Filter-{max}-{keyCache}";
    ...
    public async Task<IEnumerable<GruposEmpresasCliResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterGruposEmpresasCli filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        using var oCnn = Configuracoes.GetConnectionByUri(uri);
        if (oCnn == null)
        ...
        var cacheKey = $"{uri}-{max}GruposEmpresasCli-Filter-{where.GetHashCode2()}{filterHash}{keyCache}";
        var entryOptions = new HybridCacheEntryOptions
        {
    ...
    private async Task<GruposEmpresasCliResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<GruposEmpresasCliResponse?> AddAndUpdate([FromBody] Models.GruposEmpresasCli regGruposEmpresasCli, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regGruposEmpresasCli, operadorId, oCnn);
    ...
    public async Task<GruposEmpresasCliResponse?> Validation([FromBody] Models.GruposEmpresasCli regGruposEmpresasCli, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regGruposEmpresasCli.Id, oCnn);
    }
    ...
    public async Task<GruposEmpresasCliResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var gruposempresascli = await reader.Read(id, oCnn);
        try
        {
            if (gruposempresascli != null)
            {
                await writer.Delete(gruposempresascli, nOperador, oCnn);
