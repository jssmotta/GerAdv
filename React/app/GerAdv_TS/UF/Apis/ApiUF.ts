...
export class UFApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UF_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os UF');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UF_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar UF com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterUF): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UF_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de UF');
        }
    }

            public async filter(filtro: FilterUF): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UF_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar UF');
                }
            }


    public async validation(regUF: IUF): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regUF as UF, this.getHeaders());
    ...
    public async addAndUpdate(regUF: IUF): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regUF as UF, this.getHeaders());
            var register = result.data as IUF;        
            const action = regUF.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar UF');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o UF. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'UF',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir UF com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterUF) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<UF[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de uf:', error);
            this.notificationService.notify({
              entity: 'UF',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos uf'
            });
          }
        });
    }
