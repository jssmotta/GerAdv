...
const TipoProDespositoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoProDesposito | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoProDesposito | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoProDesposito);
    ...
    const handleAddClick = () => {
        ...
        const add = TipoProDespositoEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoProDespositoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoProDesposito={editar ?? addValue ?? TipoProDespositoEmpty()} /> 
            )}
