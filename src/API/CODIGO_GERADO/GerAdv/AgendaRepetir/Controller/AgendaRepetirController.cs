...
public partial class AgendaRepetirController(IAgendaRepetirService agendarepetirService) : ControllerBase
{
    ...
    {
        _logger.Info("AgendaRepetir: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _agendarepetirService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AgendaRepetir: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _agendarepetirService.UpdateColumns(parameters, uri);
        if (!result)
        {
