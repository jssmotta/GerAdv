 ...
 const addValorRecado = (e: any) => {
    ...
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao submeter formulário de AlarmSMS:", error);
  ...
  const handleDirectSave = () => {
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao salvar AlarmSMS diretamente:", error);
        setIsSubmitting(false);
        if (onError) onError();
      }
    }
  ...
  <>
        ...
        <div className={isMobile ? 'form-container-AlarmSMS' : 'form-container form-container-AlarmSMS'}>
       
            <form className='formInputCadInc' id={`AlarmSMSForm-${alarmsmsData.id}`} onSubmit={onConfirm}>

                <ButtonsCrud data={alarmsmsData} isSubmitting={isSubmitting} onClose={onClose} formId={`AlarmSMSForm-${alarmsmsData.id}`} preventPropagation={true} onSave={handleDirectSave} />
