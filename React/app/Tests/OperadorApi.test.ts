...
import { OperadorApiError } from '../GerAdv_TS/Operador/Apis/ApiOperador';
import { OperadorApi } from '@/app/GerAdv_TS/Operador/Apis/ApiOperador';
import { FilterOperador } from '../GerAdv_TS/Operador/Filters/Operador';
...
describe('OperadorApi', () => {
  ...
  beforeEach(() => {
    ...
    operadorApi = new OperadorApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(operadorApi).toBeInstanceOf(OperadorApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OperadorApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OperadorApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all operador items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(operadorApi.getAll()).rejects.toThrow(OperadorApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await operadorApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Operador_last_getAll_${200}`)));
    });
    ...
    it('should throw OperadorApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOperador = OperadorTestEmpty();
    ...
    it('should fetch operador by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOperador));
      ...
      expect(result.data).toEqual(mockOperador);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operador',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterOperador = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter operador items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await operadorApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new operador successfully', async () => {
      const responseOperador = { ...mockOperador, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOperador,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorApi.addAndUpdate(mockOperador);
    ...
    it('should update existing operador successfully', async () => {
      const existingOperador = { ...mockOperador, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOperador,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorApi.addAndUpdate(existingOperador);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(operadorApi.addAndUpdate(mockOperador)).rejects.toThrow(OperadorApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operador',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete operador successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await operadorApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(operadorApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operador',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(operadorApi.delete(1)).rejects.toThrow(OperadorApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterOperador = { funcionario: 1 };
      
      const result = operadorApi.useFilter(mockFilter);
  ...
  describe('OperadorApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OperadorApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OperadorApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OperadorApiError('Test message', 500);
