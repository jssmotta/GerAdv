...
export class Apenso2Api {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Apenso2`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Apenso2_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Apenso2');
        }
    }
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Apenso2 com ID ${id}`);
        }
    }
    ...
    public async addAndUpdate(regApenso2: IApenso2): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Apenso2');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Apenso2. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Apenso2',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Apenso2 com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterApenso2) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Apenso2[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de apenso2:', error);
            this.notificationService.notify({
              entity: 'Apenso2',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos apenso2'
            });
          }
        });
    }
