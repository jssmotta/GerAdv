...
describe('BensMateriaisApi', () => {
  let bensmateriaisApi: BensMateriaisApi;
  ...
  beforeEach(() => {
    ...
    bensmateriaisApi = new BensMateriaisApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(bensmateriaisApi).toBeInstanceOf(BensMateriaisApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new BensMateriaisApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(BensMateriaisApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all bensmateriais items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await bensmateriaisApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(bensmateriaisApi.getAll()).rejects.toThrow(BensMateriaisApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await bensmateriaisApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-BensMateriais_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensMateriais',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw BensMateriaisApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockBensMateriais = BensMateriaisTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockBensMateriais,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch bensmateriais by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await bensmateriaisApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockBensMateriais));
      ...
      expect(result.data).toEqual(mockBensMateriais);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensMateriais',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter bensmateriais items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await bensmateriaisApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new bensmateriais successfully', async () => {
      const responseBensMateriais = { ...mockBensMateriais, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseBensMateriais,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await bensmateriaisApi.addAndUpdate(mockBensMateriais);
    ...
    it('should update existing bensmateriais successfully', async () => {
      const existingBensMateriais = { ...mockBensMateriais, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingBensMateriais,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await bensmateriaisApi.addAndUpdate(existingBensMateriais);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(bensmateriaisApi.addAndUpdate(mockBensMateriais)).rejects.toThrow(BensMateriaisApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensMateriais',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete bensmateriais successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await bensmateriaisApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(bensmateriaisApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensMateriais',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(bensmateriaisApi.delete(1)).rejects.toThrow(BensMateriaisApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await bensmateriaisApi.delete(1);
  ...
  describe('BensMateriaisApiError', () => {
    it('should create error with correct properties', () => {
      const error = new BensMateriaisApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('BensMateriaisApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new BensMateriaisApiError('Test message', 500);
