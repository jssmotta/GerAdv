...
const ProCDAComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProCDA | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProCDA | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProCDA);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProCDAWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProCDA={editar ?? addValue ?? ProCDAEmpty()} /> 
            )}
