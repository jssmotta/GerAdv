...
{
    Task<GUTAtividadesMatrizResponse?> ReadAsync(int id, MsiSqlConnection? oCnn);
    Task<Models.GUTAtividadesMatriz?> ReadMAsync(int id, MsiSqlConnection? oCnn);
    ...
    Task<IEnumerable<GUTAtividadesMatrizResponseAll>> ListarAsync(MsiSqlConnection oCnn, int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken);
}