...
public partial class AreaController(IAreaService areaService) : ControllerBase
{
    ...
    {
        _logger.Info("Area: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _areaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Area: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _areaService.UpdateColumns(parameters, uri);
        if (!result)
        {
