...
export class LivroCaixaClientesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'LivroCaixaClientesApiError';
    this.status = status;
    this.code = code;
...
export class LivroCaixaClientesApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixaClientes_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os LivroCaixaClientes');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixaClientes_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Livro Caixa Clientes com ID ${id}`);
        }
    }

            public async filter(filtro: FilterLivroCaixaClientes): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-LivroCaixaClientes_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar LivroCaixaClientes');
                }
            }


    public async validation(regLivroCaixaClientes: ILivroCaixaClientes): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regLivroCaixaClientes as LivroCaixaClientes, this.getHeaders());
    ...
    public async addAndUpdate(regLivroCaixaClientes: ILivroCaixaClientes): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Livro Caixa Clientes');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Livro Caixa Clientes com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterLivroCaixaClientes) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<LivroCaixaClientes[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de livrocaixaclientes:', error);
            this.notificationService.notify({
              entity: 'LivroCaixaClientes',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos livrocaixaclientes'
            });
          }
        });
    }
