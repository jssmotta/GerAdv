...
public partial class DBTipoEndereco
{
    ...
    {
        if (pFldFDescricao)
            updateTool.Fields(DBTipoEnderecoDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBTipoEnderecoDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBTipoEnderecoDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
