...
const PaisesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPaises | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPaises | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPaises);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PaisesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPaises={editar ?? addValue ?? PaisesEmpty()} /> 
            )}
