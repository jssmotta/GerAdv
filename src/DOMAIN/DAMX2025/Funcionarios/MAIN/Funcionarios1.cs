...
public partial class DBFuncionarios : VSexo, ICadastrosAuditor, IAuditor, IListagemCidade
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Funcionarios)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBFuncionarios();
...
#region TableDefinition_Funcionarios
...
#endregion
    public DBFuncionarios(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBFuncionarios(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBFuncionarios(in string? sqlWhere, SqlConnection? oCnn)
    {
...
#region GravarDados_Funcionarios
...
#endif
    {
        ...
        {
            throw new Exception("Campo 'funNome' está vazio!");
        }
        ...
        if (pFldFNome)
            clsW.Fields(DBFuncionariosDicInfo.Nome, sex.m_FNome, ETiposCampos.FString);
        if (pFldFFuncao)
        ...
        if (pFldFCPF)
            clsW.Fields(DBFuncionariosDicInfo.CPF, sex.m_FCPF, ETiposCampos.FString);
        if (pFldFRG)
        ...
        if (pFldFEndereco)
            clsW.Fields(DBFuncionariosDicInfo.Endereco, sex.m_FEndereco, ETiposCampos.FString);
        if (pFldFBairro)
            clsW.Fields(DBFuncionariosDicInfo.Bairro, sex.m_FBairro, ETiposCampos.FString);
        if (pFldFCidade)
            clsW.Fields(DBFuncionariosDicInfo.Cidade, m_FCidade, ETiposCampos.FNumberNull);
        if (pFldFCEP)
            clsW.Fields(DBFuncionariosDicInfo.CEP, sex.m_FCEP, ETiposCampos.FString);
        if (pFldFContato)
        ...
        if (pFldFClass)
            clsW.Fields(DBFuncionariosDicInfo.Class, sex.m_FClass, ETiposCampos.FString);
