...
public class ProSucumbenciaHealthCheck(IOptions<AppSettings> appSettings, ProSucumbenciaService prosucumbenciaService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly ProSucumbenciaService _prosucumbenciaService = prosucumbenciaService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        if (DBProSucumbenciaDicInfo.CampoCodigo.NotIsEmpty())
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) MAX(scbCodigo) FROM {"ProSucumbencia".dbo(connection)} (NOLOCK);";
                        ...
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) scbProcesso,scbInstancia,scbData,scbNome,scbTipoOrigemSucumbencia,scbValor,scbPercentual,scbGUID FROM {"ProSucumbencia".dbo(connection)} (NOLOCK);";
                            tableCheck.CommandTimeout = 5;
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT ProSucumbencia successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT ProSucumbencia failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _prosucumbenciaService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-ProSucumbencia";
                    ...
                    static async Task<IEnumerable<Models.ProSucumbencia>> GetData(string uri, CancellationToken token)
                    {
            ...
            return isHealthy ? HealthCheckResult.Healthy("ProSucumbencia is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
