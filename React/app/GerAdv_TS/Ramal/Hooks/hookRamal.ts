...
export const useRamalForm = (
  ...
  const loadRamal = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialRamal);
      return;
    }
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadRamal, resetForm]);
  return returnValue;
};


export const useRamalNotifications = (
...
export const useRamalList = (dataService: IRamalService) => {
  const [data, setData] = useState<IRamal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
