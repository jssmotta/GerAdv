...
import { OperadorEMailPopup } from '../../Models/OperadorEMailPopup';
...
export class OperadorEMailPopupApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'OperadorEMailPopupApiError';
    this.status = status;
    this.code = code;
...
export class OperadorEMailPopupApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/OperadorEMailPopup`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorEMailPopup_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os OperadorEMailPopup');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorEMailPopup_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Operador E Mail Popup com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterOperadorEMailPopup): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorEMailPopup_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de OperadorEMailPopup');
        }
    }

            public async filter(filtro: FilterOperadorEMailPopup): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorEMailPopup_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar OperadorEMailPopup');
                }
            }


    public async validation(regOperadorEMailPopup: IOperadorEMailPopup): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regOperadorEMailPopup as OperadorEMailPopup, this.getHeaders());
    ...
    public async addAndUpdate(regOperadorEMailPopup: IOperadorEMailPopup): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regOperadorEMailPopup as OperadorEMailPopup, this.getHeaders());
            var register = result.data as IOperadorEMailPopup;        
            const action = regOperadorEMailPopup.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Operador E Mail Popup');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o OperadorEMailPopup. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'OperadorEMailPopup',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Operador E Mail Popup com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterOperadorEMailPopup) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<OperadorEMailPopup[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de operadoremailpopup:', error);
            this.notificationService.notify({
              entity: 'OperadorEMailPopup',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos operadoremailpopup'
            });
          }
        });
    }
