...
const ClientesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IClientes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IClientes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IClientes);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ClientesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedClientes={editar ?? addValue ?? ClientesEmpty()} /> 
            )}
