...
public partial class ContatoCRMOperador
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(ccoDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.ContatoCRMOperador WITH (NOLOCK, INDEX = idx_ContatoCRMOperador_AuditorDtAtu)
WHERE ccoCodigo = @id
...
FROM {oCnn.UseDbo}.ContatoCRMOperador WITH (NOLOCK)
