...
export const useStatusHTrabList = (dataService: IStatusHTrabService, currentFilter?: FilterStatusHTrab | undefined | null) => {
  const [data, setData] = useState<IStatusHTrab[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterStatusHTrab | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterStatusHTrab | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterStatusHTrab);
      setData(result);
    } catch (err) {
