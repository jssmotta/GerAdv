...
public static class FilterTiposAcaoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TiposAcao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTiposAcao:

- O filter irá filtrar os TiposAcao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterTiposAcao:(Tipos Acao)
- nome: _nome ou descrição para Tipos Acao;
- inativo: Inativo;
- bold: Bold;
...
- Se ouvir "todos os TiposAcao" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTiposAcao? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TiposAcao length: {result.Length}");
#endif
