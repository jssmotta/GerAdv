...
const OperadorEMailPopupComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOperadorEMailPopup | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOperadorEMailPopup | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOperadorEMailPopup);
    ...
    const handleAddClick = () => {
        ...
        const add = OperadorEMailPopupEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OperadorEMailPopupWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOperadorEMailPopup={editar ?? addValue ?? OperadorEMailPopupEmpty()} /> 
            )}
