// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Tabela:LivroCaixa
// Fonte:InterfaceGenerator
namespace MenphisSI.GerAdv.Interface;
public partial interface ILivroCaixaService
{
    Task<IEnumerable<LivroCaixaResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterLivroCaixa filter, [FromRoute, Required] string uri = "");
    Task<Filters.FilterLivroCaixa> FilterVoice([FromBody] Filters.FilterLivroCaixa filter, [FromBody] CommandSpeakerRequest? message, [FromRoute, Required] string uri = "");
    Task<LivroCaixaResponse?> AddAndUpdate(Models.LivroCaixa? regLivroCaixa, [FromRoute, Required] string uri = "");
    Task<LivroCaixaResponse?> Validation(Models.LivroCaixa? regLivroCaixa, [FromRoute, Required] string uri = "");
    Task<LivroCaixaResponse?> Delete(int? id, [FromRoute, Required] string uri = "");
    Task<LivroCaixaResponse?> GetById(int id, [FromRoute, Required] string uri = "", CancellationToken token = default);
    Task<AuditorResponse?> GetAuditor(int id, [FromRoute, Required] string uri = "", CancellationToken token = default);
    Task<IEnumerable<LivroCaixaResponseAll>> GetAll(int max, [FromRoute, Required] string uri = "", CancellationToken token = default);
    Task<IEnumerable<NomeID>> GetListN([FromQuery] int max, [FromBody] Filters.FilterLivroCaixa? filter, [FromRoute, Required] string uri = "", CancellationToken token = default);
}