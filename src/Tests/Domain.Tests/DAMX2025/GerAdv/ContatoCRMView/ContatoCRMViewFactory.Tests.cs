...
public class FContatoCRMViewFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FContatoCRMViewFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FContatoCRMViewFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FContatoCRMViewFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "ccwCodigo")
        var dt = new DataTable();
        dt.Columns.Add("ccwCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["ccwCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FContatoCRMView>();
    }
    ...
    {
        // Arrange
        var factory = new FContatoCRMViewFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("ccwCodigo", typeof(int));
        mockDataTable.Columns.Add("ccwCGUID", typeof(string));
        var row = mockDataTable.NewRow();
        row["ccwCodigo"] = testId;
        row["ccwCGUID"] = "Test ContatoCRMView";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["ccwCodigo"] = 123;
        row["ccwCGUID"] = "Test ContatoCRMView Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FContatoCRMView properties
        }
    }
