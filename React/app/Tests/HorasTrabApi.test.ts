...
describe('HorasTrabApi', () => {
  let horastrabApi: HorasTrabApi;
  ...
  beforeEach(() => {
    ...
    horastrabApi = new HorasTrabApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(horastrabApi).toBeInstanceOf(HorasTrabApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new HorasTrabApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(HorasTrabApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all horastrab items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await horastrabApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(horastrabApi.getAll()).rejects.toThrow(HorasTrabApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await horastrabApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-HorasTrab_last_getAll_${200}`)));
    });
    ...
    it('should throw HorasTrabApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockHorasTrab = HorasTrabTestEmpty();
    ...
    it('should fetch horastrab by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await horastrabApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockHorasTrab));
      ...
      expect(result.data).toEqual(mockHorasTrab);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'HorasTrab',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter horastrab items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await horastrabApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new horastrab successfully', async () => {
      const responseHorasTrab = { ...mockHorasTrab, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseHorasTrab,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await horastrabApi.addAndUpdate(mockHorasTrab);
    ...
    it('should update existing horastrab successfully', async () => {
      const existingHorasTrab = { ...mockHorasTrab, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingHorasTrab,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await horastrabApi.addAndUpdate(existingHorasTrab);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(horastrabApi.addAndUpdate(mockHorasTrab)).rejects.toThrow(HorasTrabApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'HorasTrab',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete horastrab successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await horastrabApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(horastrabApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'HorasTrab',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(horastrabApi.delete(1)).rejects.toThrow(HorasTrabApiError);
    });
  });
  ...
  describe('HorasTrabApiError', () => {
    it('should create error with correct properties', () => {
      const error = new HorasTrabApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('HorasTrabApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new HorasTrabApiError('Test message', 500);
