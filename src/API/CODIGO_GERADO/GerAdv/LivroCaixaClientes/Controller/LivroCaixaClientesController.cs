...
public partial class LivroCaixaClientesController(ILivroCaixaClientesService livrocaixaclientesService) : ControllerBase
{
    ...
    {
        _logger.Info("LivroCaixaClientes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _livrocaixaclientesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("LivroCaixaClientes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _livrocaixaclientesService.UpdateColumns(parameters, uri);
        if (!result)
        {
