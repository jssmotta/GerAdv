...
#region ListarDados_EMPClassRiscos
    public static async IAsyncEnumerable<DBEMPClassRiscos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEMPClassRiscos(dbRec);
        }
    }

    public static IEnumerable<DBEMPClassRiscos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
