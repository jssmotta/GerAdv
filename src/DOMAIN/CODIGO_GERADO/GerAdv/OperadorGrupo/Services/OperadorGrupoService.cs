...
public partial class OperadorGrupoService(IOptions<AppSettings> appSettings, IFOperadorGrupoFactory operadorgrupoFactory, IOperadorGrupoReader reader, IOperadorGrupoValidation validation, IOperadorGrupoWriter writer, IOperadorReader operadorReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IOperadorGrupoService, IDisposable
{
    ...
    public async Task<IEnumerable<OperadorGrupoResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-OperadorGrupo-Filter-{max}-{keyCache}";
    ...
    public async Task<IEnumerable<OperadorGrupoResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterOperadorGrupo filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        using var oCnn = Configuracoes.GetConnectionByUri(uri);
        if (oCnn == null)
        ...
        var cacheKey = $"{uri}-{max}OperadorGrupo-Filter-{where.GetHashCode2()}{filterHash}{keyCache}";
        var entryOptions = new HybridCacheEntryOptions
        {
