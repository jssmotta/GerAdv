...
import GUTPeriodicidadeWindow from '../Crud/Grids/GUTPeriodicidadeWindow';
...
import { useGUTPeriodicidadeComboBox } from '../Hooks/hookGUTPeriodicidade';
const GUTPeriodicidadeComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'gutperiodicidadeInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useGUTPeriodicidadeComboBox(gutperiodicidadeService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IGUTPeriodicidade | null>(null);
const [addRecord, setAddRecord] = useState<IGUTPeriodicidade | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await gutperiodicidadeService.fetchGUTPeriodicidadeById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = GUTPeriodicidadeEmpty();
...
const handleSuccess = (newGUTPeriodicidade?: IGUTPeriodicidade) => {
  if (newGUTPeriodicidade) {
    setValue({ id: newGUTPeriodicidade.id, nome: newGUTPeriodicidade.nome });
    loadRecordForEdit(newGUTPeriodicidade.id);
    handleValueChange(newGUTPeriodicidade);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .gutperiodicidade-dropdown-popup-msi {
  ...
  .gutperiodicidade-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .gutperiodicidade-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .gutperiodicidade-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedGUTPeriodicidade={editRecord || addRecord || GUTPeriodicidadeEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'gutperiodicidade-dropdown-popup-msi'
          }}
          />
...
export default GUTPeriodicidadeComboBox;