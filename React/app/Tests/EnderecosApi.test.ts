...
describe('EnderecosApi', () => {
  let enderecosApi: EnderecosApi;
  ...
  beforeEach(() => {
    ...
    enderecosApi = new EnderecosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(enderecosApi).toBeInstanceOf(EnderecosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EnderecosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EnderecosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all enderecos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enderecosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(enderecosApi.getAll()).rejects.toThrow(EnderecosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await enderecosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Enderecos_last_getAll_${200}`)));
    });
    ...
    it('should throw EnderecosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEnderecos = EnderecosTestEmpty();
    ...
    it('should fetch enderecos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enderecosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEnderecos));
      ...
      expect(result.data).toEqual(mockEnderecos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Enderecos',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter enderecos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await enderecosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new enderecos successfully', async () => {
      const responseEnderecos = { ...mockEnderecos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEnderecos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enderecosApi.addAndUpdate(mockEnderecos);
    ...
    it('should update existing enderecos successfully', async () => {
      const existingEnderecos = { ...mockEnderecos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEnderecos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enderecosApi.addAndUpdate(existingEnderecos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(enderecosApi.addAndUpdate(mockEnderecos)).rejects.toThrow(EnderecosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Enderecos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete enderecos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await enderecosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(enderecosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Enderecos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(enderecosApi.delete(1)).rejects.toThrow(EnderecosApiError);
    });
  });
  ...
  describe('EnderecosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EnderecosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EnderecosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EnderecosApiError('Test message', 500);
