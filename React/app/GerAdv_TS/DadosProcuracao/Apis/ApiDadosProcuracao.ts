...
import { DadosProcuracao } from '../../Models/DadosProcuracao';
...
export class DadosProcuracaoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'DadosProcuracaoApiError';
    this.status = status;
    this.code = code;
...
export class DadosProcuracaoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/DadosProcuracao`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DadosProcuracao_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os DadosProcuracao');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DadosProcuracao_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Dados Procuracao com ID ${id}`);
        }
    }

            public async filter(filtro: FilterDadosProcuracao): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DadosProcuracao_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar DadosProcuracao');
                }
            }


    public async validation(regDadosProcuracao: IDadosProcuracao): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regDadosProcuracao as DadosProcuracao, this.getHeaders());
    ...
    public async addAndUpdate(regDadosProcuracao: IDadosProcuracao): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regDadosProcuracao as DadosProcuracao, this.getHeaders());
            var register = result.data as IDadosProcuracao;        
            const action = regDadosProcuracao.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Dados Procuracao');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o DadosProcuracao. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'DadosProcuracao',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Dados Procuracao com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterDadosProcuracao) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<DadosProcuracao[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de dadosprocuracao:', error);
            this.notificationService.notify({
              entity: 'DadosProcuracao',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos dadosprocuracao'
            });
          }
        });
    }
