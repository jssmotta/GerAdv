...
#region ListarDados_TiposAcao
    public static async IAsyncEnumerable<DBTiposAcao> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTiposAcao(dbRec);
        }
    }

    public static IEnumerable<DBTiposAcao> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
