...
import { ParteClienteOutrasApiError } from '../GerAdv_TS/ParteClienteOutras/Apis/ApiParteClienteOutras';
import { ParteClienteOutrasApi } from '@/app/GerAdv_TS/ParteClienteOutras/Apis/ApiParteClienteOutras';
import { FilterParteClienteOutras } from '../GerAdv_TS/ParteClienteOutras/Filters/ParteClienteOutras';
...
describe('ParteClienteOutrasApi', () => {
  let parteclienteoutrasApi: ParteClienteOutrasApi;
  ...
  beforeEach(() => {
    ...
    parteclienteoutrasApi = new ParteClienteOutrasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(parteclienteoutrasApi).toBeInstanceOf(ParteClienteOutrasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ParteClienteOutrasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ParteClienteOutrasApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ParteClienteOutrasTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all parteclienteoutras items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteclienteoutrasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(parteclienteoutrasApi.getAll()).rejects.toThrow(ParteClienteOutrasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await parteclienteoutrasApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ParteClienteOutras_last_getAll_${200}`)));
    });
    ...
    it('should throw ParteClienteOutrasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockParteClienteOutras = ParteClienteOutrasTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockParteClienteOutras,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch parteclienteoutras by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteclienteoutrasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockParteClienteOutras));
      ...
      expect(result.data).toEqual(mockParteClienteOutras);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteClienteOutras',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterParteClienteOutras = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter parteclienteoutras items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await parteclienteoutrasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockParteClienteOutras: IParteClienteOutras = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new parteclienteoutras successfully', async () => {
      const responseParteClienteOutras = { ...mockParteClienteOutras, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseParteClienteOutras,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteclienteoutrasApi.addAndUpdate(mockParteClienteOutras);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockParteClienteOutras,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing parteclienteoutras successfully', async () => {
      const existingParteClienteOutras = { ...mockParteClienteOutras, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingParteClienteOutras,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteclienteoutrasApi.addAndUpdate(existingParteClienteOutras);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(parteclienteoutrasApi.addAndUpdate(mockParteClienteOutras)).rejects.toThrow(ParteClienteOutrasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteClienteOutras',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete parteclienteoutras successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await parteclienteoutrasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(parteclienteoutrasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteClienteOutras',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(parteclienteoutrasApi.delete(1)).rejects.toThrow(ParteClienteOutrasApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterParteClienteOutras = { funcionario: 1 };
      
      const result = parteclienteoutrasApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await parteclienteoutrasApi.delete(1);
  ...
  describe('ParteClienteOutrasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ParteClienteOutrasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ParteClienteOutrasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ParteClienteOutrasApiError('Test message', 500);
