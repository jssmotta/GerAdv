// app/TestsForms/GUTAtividadesComboBox.test.tsx - Vers�o Corrigida
...
import { GUTAtividadesEmpty, GUTAtividadesTestEmpty } from '@/app/GerAdv_TS/Models/GUTAtividades';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/GUTAtividades/Hooks/hookGUTAtividades', () => ({
  useGUTAtividadesComboBox: jest.fn(() => ({
    options: [
    { ...GUTAtividadesEmpty(), id: 1, nome: 'G U T Atividades 1' },
    { ...GUTAtividadesEmpty(), id: 2, nome: 'G U T Atividades 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/GUTAtividades/Apis/ApiGUTAtividades', () => ({
  GUTAtividadesApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTAtividades' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTAtividades' }),
...
jest.mock('@/app/GerAdv_TS/GUTAtividades/Services/GUTAtividades.service', () => ({
  GUTAtividadesService: jest.fn().mockImplementation(() => ({
    fetchGUTAtividadesById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTAtividades' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/GUTAtividades/Crud/Grids/GUTAtividadesWindow', () => {
  return function MockGUTAtividadesWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='gutatividades-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'G U T Atividades' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...GUTAtividadesTestEmpty(), id: 1, nome: 'G U T Atividades 1' },
  { ...GUTAtividadesTestEmpty(), id: 2, nome: 'G U T Atividades 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: GUTAtividadesEmpty(), 
};
describe('GUTAtividadesComboBox', () => {
  ...
  const { useGUTAtividadesComboBox } = require('@/app/GerAdv_TS/GUTAtividades/Hooks/hookGUTAtividades');
  (useGUTAtividadesComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<GUTAtividadesComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('G U T Atividades')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select G U T Atividades')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...GUTAtividadesTestEmpty(), id: 1, nome: 'G U T Atividades 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useGUTAtividadesComboBox } = require('@/app/GerAdv_TS/GUTAtividades/Hooks/hookGUTAtividades');
    useGUTAtividadesComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens GUTAtividadesWindow on action button click (add)', async () => {
    render(<GUTAtividadesComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('gutatividades-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('gutatividades-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<GUTAtividadesComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles GUTAtividadesWindow onClose and onSuccess', () => {
    render(<GUTAtividadesComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<GUTAtividadesComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
