...
import { TipoProDesposito } from '../../Models/TipoProDesposito';
...
export class TipoProDespositoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'TipoProDespositoApiError';
    this.status = status;
    this.code = code;
...
export class TipoProDespositoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/TipoProDesposito`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoProDesposito_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoProDesposito');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoProDesposito_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipo Pro Desposito com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterTipoProDesposito): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoProDesposito_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoProDesposito');
        }
    }

            public async filter(filtro: FilterTipoProDesposito): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoProDesposito_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoProDesposito');
                }
            }


    public async validation(regTipoProDesposito: ITipoProDesposito): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoProDesposito as TipoProDesposito, this.getHeaders());
    ...
    public async addAndUpdate(regTipoProDesposito: ITipoProDesposito): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regTipoProDesposito as TipoProDesposito, this.getHeaders());
            var register = result.data as ITipoProDesposito;        
            const action = regTipoProDesposito.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Tipo Pro Desposito');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o TipoProDesposito. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'TipoProDesposito',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Tipo Pro Desposito com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoProDesposito) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<TipoProDesposito[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de tipoprodesposito:', error);
            this.notificationService.notify({
              entity: 'TipoProDesposito',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tipoprodesposito'
            });
          }
        });
    }
