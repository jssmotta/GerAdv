...
public partial class AgendaRelatorioReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBAgendaRelatorioDicInfo.CampoNome}";
        ...
        var query = $@"SELECT TOP ({max})
                   ...
                   FROM {DBAgendaRelatorio.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBProcessosDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBProcessosDicInfo.PTabelaNome}].[{DBProcessos.CampoCodigo}]=[{DBAgendaRelatorioDicInfo.PTabelaNome}].[{DBAgendaRelatorioDicInfo.vqaProcesso}]
