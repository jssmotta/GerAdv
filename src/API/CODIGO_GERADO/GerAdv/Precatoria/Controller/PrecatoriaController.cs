...
public partial class PrecatoriaController(IPrecatoriaService precatoriaService) : ControllerBase
{
    ...
    {
        _logger.Info("Precatoria: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _precatoriaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Precatoria: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _precatoriaService.UpdateColumns(parameters, uri);
        if (!result)
        {
