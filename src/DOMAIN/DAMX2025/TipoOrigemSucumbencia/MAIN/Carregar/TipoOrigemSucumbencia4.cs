...
{
    public DBTipoOrigemSucumbencia(DataRow? dbRec)
    {
        if (dbRec is null)
            return;
...
#region CarregarDados_TipoOrigemSucumbencia
    protected void Carregar(int id, SqlConnection? oCnn)
    {
        if (id.IsEmptyIDNumber())
            return;
        using var cmd = new SqlCommand($"SET NOCOUNT ON; SELECT TOP (1) {CamposSqlX} FROM dbo.[TipoOrigemSucumbencia] (NOLOCK) WHERE [tosCodigo] = @ThisIDToLoad", oCnn);
