...
public partial class AndamentosMDController(IAndamentosMDService andamentosmdService) : ControllerBase
{
    ...
    {
        _logger.Info("AndamentosMD: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _andamentosmdService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AndamentosMD: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _andamentosmdService.UpdateColumns(parameters, uri);
        if (!result)
        {
