...
public static class AgendaRelatorioEndpoints
{
    ...
    private static void MapAgendaRelatorioRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapGet("/GetById/{id}", async (int id, string uri, IAgendaRelatorioValidation validation, IProcessosReader processosReader, IAgendaRelatorioService service, CancellationToken token) =>
        {
            logger.Info("AgendaRelatorio: GetById called with id = {0}, {1}", id, uri);
