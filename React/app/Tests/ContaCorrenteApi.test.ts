...
import { ContaCorrenteApiError } from '../GerAdv_TS/ContaCorrente/Apis/ApiContaCorrente';
import { ContaCorrenteApi } from '@/app/GerAdv_TS/ContaCorrente/Apis/ApiContaCorrente';
import { FilterContaCorrente } from '../GerAdv_TS/ContaCorrente/Filters/ContaCorrente';
...
describe('ContaCorrenteApi', () => {
  let contacorrenteApi: ContaCorrenteApi;
  ...
  beforeEach(() => {
    ...
    contacorrenteApi = new ContaCorrenteApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(contacorrenteApi).toBeInstanceOf(ContaCorrenteApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ContaCorrenteApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ContaCorrenteApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all contacorrente items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contacorrenteApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(contacorrenteApi.getAll()).rejects.toThrow(ContaCorrenteApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await contacorrenteApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ContaCorrente_last_getAll_${200}`)));
    });
    ...
    it('should throw ContaCorrenteApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockContaCorrente = ContaCorrenteTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockContaCorrente,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch contacorrente by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contacorrenteApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockContaCorrente));
      ...
      expect(result.data).toEqual(mockContaCorrente);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContaCorrente',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterContaCorrente = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter contacorrente items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await contacorrenteApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockContaCorrente: IContaCorrente = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new contacorrente successfully', async () => {
      const responseContaCorrente = { ...mockContaCorrente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseContaCorrente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contacorrenteApi.addAndUpdate(mockContaCorrente);
    ...
    it('should update existing contacorrente successfully', async () => {
      const existingContaCorrente = { ...mockContaCorrente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingContaCorrente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contacorrenteApi.addAndUpdate(existingContaCorrente);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(contacorrenteApi.addAndUpdate(mockContaCorrente)).rejects.toThrow(ContaCorrenteApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContaCorrente',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete contacorrente successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await contacorrenteApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(contacorrenteApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContaCorrente',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(contacorrenteApi.delete(1)).rejects.toThrow(ContaCorrenteApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterContaCorrente = { funcionario: 1 };
      
      const result = contacorrenteApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await contacorrenteApi.delete(1);
  ...
  describe('ContaCorrenteApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ContaCorrenteApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ContaCorrenteApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ContaCorrenteApiError('Test message', 500);
