...
#region ListarDados_Operador
    public static async IAsyncEnumerable<DBOperador> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperador(dbRec);
        }
    }

    public static IEnumerable<DBOperador> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
