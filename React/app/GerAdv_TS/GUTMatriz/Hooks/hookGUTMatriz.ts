...
export const useGUTMatrizForm = (
  ...
  const loadGUTMatriz = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar G U T Matriz";
...
export const useGUTMatrizList = (
  dataService: IGUTMatrizService,
  currentFilter?: FilterGUTMatriz | undefined | null,
) => {
  const [data, setData] = useState<IGUTMatriz[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGUTMatriz | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterGUTMatriz | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar gutmatriz";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IGUTMatriz): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.descricao.length > 150) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 150 caracteres.",
        };
      }
