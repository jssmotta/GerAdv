...
#region CPF Validation Tests
    ...
    public async Task ValidateReg_WithDuplicateCPF_ShouldThrowSGValidationException()
    {
        ...
        _mockFuncionariosService.Setup(x => x.Filter(It.IsAny<int>(), It.IsAny<FilterFuncionarios>(), It.IsAny<string>())).ReturnsAsync([existingFuncionarios]);
        // Setup other mocks but don't override the Funcionarioss service mock
        _ = _mockCargosReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<CargosResponse?>(new CargosResponse { Id = id }));
        _ = _mockFuncaoReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<FuncaoResponse?>(new FuncaoResponse { Id = id }));
        _ = _mockCidadeReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<CidadeResponse?>(new CidadeResponse { Id = id }));
...
#region Data Validation Tests
    ...
    public async Task ValidateReg_WithInvalidDateDataFormat_ShouldPass()
    {
        // Arrange
        var funcionarios = CreateValidFuncionarios();
        funcionarios.Data = "invalid-date";
        SetupValidMocks();
        // Act
        var result = await _validation.ValidateReg(funcionarios, _mockFuncionariosService.Object, _mockCargosReader.Object, _mockFuncaoReader.Object, _mockCidadeReader.Object, _validUri, _mockConnection.Object);
        // Assert
