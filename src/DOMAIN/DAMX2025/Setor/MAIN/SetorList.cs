...
#region ListarDados_Setor
    public static async IAsyncEnumerable<DBSetor> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBSetor(dbRec);
        }
    }

    public static IEnumerable<DBSetor> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
