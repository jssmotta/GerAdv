...
public class FEnquadramentoEmpresaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FEnquadramentoEmpresaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FEnquadramentoEmpresaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FEnquadramentoEmpresaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "eqeCodigo")
        var dt = new DataTable();
        dt.Columns.Add("eqeCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["eqeCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FEnquadramentoEmpresa>();
    }
    ...
    {
        // Arrange
        var factory = new FEnquadramentoEmpresaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("eqeCodigo", typeof(int));
        mockDataTable.Columns.Add("eqeNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["eqeCodigo"] = testId;
        row["eqeNome"] = "Test EnquadramentoEmpresa";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["eqeCodigo"] = 123;
        row["eqeNome"] = "Test EnquadramentoEmpresa Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FEnquadramentoEmpresa properties
        }
    }
