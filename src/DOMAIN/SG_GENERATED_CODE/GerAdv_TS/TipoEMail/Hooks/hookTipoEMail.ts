...
export const useTipoEMailList = (dataService: ITipoEMailService, currentFilter?: FilterTipoEMail | undefined | null) => {
  const [data, setData] = useState<ITipoEMail[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoEMail | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoEMail | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoEMail);
      setData(result);
    } catch (err) {
