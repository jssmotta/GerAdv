...
public class FAgendaRelatorioFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FAgendaRelatorioFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FAgendaRelatorioFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FAgendaRelatorioFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "vqaCodigo")
        var dt = new DataTable();
        dt.Columns.Add("vqaCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["vqaCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FAgendaRelatorio>();
    }
    ...
    {
        // Arrange
        var factory = new FAgendaRelatorioFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
   ...
   [Fact]
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("vqaCodigo", typeof(int));
        mockDataTable.Columns.Add("xxxParaNome", typeof(string)); 
        
        var row = mockDataTable.NewRow();
        row["vqaCodigo"] = testId;
        row["xxxParaNome"] = "Test AgendaRelatorio";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["vqaCodigo"] = 123;
        row["xxxParaNome"] = "Test AgendaRelatorio Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FAgendaRelatorio properties
        }
    }
