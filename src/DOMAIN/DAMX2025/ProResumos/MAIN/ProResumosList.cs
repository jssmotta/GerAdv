...
#region ListarDados_ProResumos
    public static async IAsyncEnumerable<DBProResumos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProResumos(dbRec);
        }
    }

    public static IEnumerable<DBProResumos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
