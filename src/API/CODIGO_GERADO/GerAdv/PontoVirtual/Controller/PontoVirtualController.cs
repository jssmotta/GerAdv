...
public partial class PontoVirtualController(IPontoVirtualService pontovirtualService) : ControllerBase
{
    ...
    {
        _logger.Info("PontoVirtual: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _pontovirtualService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("PontoVirtual: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _pontovirtualService.UpdateColumns(parameters, uri);
        if (!result)
        {
