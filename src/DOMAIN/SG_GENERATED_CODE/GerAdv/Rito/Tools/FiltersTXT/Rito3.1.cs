...
public static class FilterRitoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Rito em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterRito:

- O filter irá filtrar os Rito conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterRito:(Rito)
- descricao: _nome ou descrição para Rito;
- top: Top;
- bold: Bold;
...
- Se ouvir "todos os Rito" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterRito? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Rito length: {result.Length}");
#endif
