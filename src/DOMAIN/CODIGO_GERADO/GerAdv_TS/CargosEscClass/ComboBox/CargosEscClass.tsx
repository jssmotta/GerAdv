...
import CargosEscClassWindow from '../Crud/Grids/CargosEscClassWindow';
...
import { useCargosEscClassComboBox } from '../Hooks/hookCargosEscClass';
const CargosEscClassComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'cargosescclassInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useCargosEscClassComboBox(cargosescclassService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<ICargosEscClass | null>(null);
const [addRecord, setAddRecord] = useState<ICargosEscClass | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await cargosescclassService.fetchCargosEscClassById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nome });
  } catch (error) {
  console.log('Erro ao carregar Cargos Esc Class:');
}
};
...
const handleActionClick = () => {
  ...
  const newRecord = CargosEscClassEmpty();
...
const handleSuccess = (newCargosEscClass?: ICargosEscClass) => {
  if (newCargosEscClass) {
    setValue({ id: newCargosEscClass.id, nome: newCargosEscClass.nome });
    loadRecordForEdit(newCargosEscClass.id);
    handleValueChange(newCargosEscClass);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .cargosescclass-dropdown-popup-msi {
  ...
  .cargosescclass-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .cargosescclass-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .cargosescclass-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedCargosEscClass={editRecord || addRecord || CargosEscClassEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'cargosescclass-dropdown-popup-msi'
          }}
          />
...
export default CargosEscClassComboBox;