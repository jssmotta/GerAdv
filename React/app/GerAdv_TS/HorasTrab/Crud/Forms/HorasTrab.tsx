...
export const HorasTrabForm: React.FC<HorasTrabFormProps> = ({
  ...
  const dadoApi = new HorasTrabApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const [_nomeServicos, setNomeServicos] = useState("");
  const servicosApi = new ServicosApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const addValorServico = (e: any) => {
    if (e?.id > 0) onChange({ target: { name: "servico", value: e.id } });
  };
  ...
  useEffect(() => {
    ...
    if (el) {
      el.textContent =
        horastrabData?.id == 0 ? "Editar HorasTrab" : "Adicionar Horas Trab";
    }
  }, [horastrabData.id]);
  ...
  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-HorasTrab"
            : "form-container form-container-HorasTrab"
        }
      >
        <form
          className="formInputCadInc"
          id={`HorasTrabForm-${horastrabData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
              ...
              onClose={onClose}
              formId={`HorasTrabForm-${horastrabData.id}`}
          ...
          <div className="grid-container">
            ...
            <InputCheckbox
              ...
              label="Honorario"
              name="honorario"
              checked={horastrabData.honorario}
              onChange={onChange}
            />
        ...
        <FooterIncDocker>
          <CopyButton formId={`HorasTrabForm-${horastrabData.id}`} />

          <PrintButton formId={`HorasTrabForm-${horastrabData.id}`} />
