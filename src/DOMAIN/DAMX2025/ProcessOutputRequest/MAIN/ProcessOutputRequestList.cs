...
#region ListarDados_ProcessOutputRequest
    public static async IAsyncEnumerable<DBProcessOutputRequest> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProcessOutputRequest(dbRec);
        }
    }

    public static IEnumerable<DBProcessOutputRequest> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
