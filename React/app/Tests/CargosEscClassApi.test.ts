...
import { CargosEscClassApiError } from '../GerAdv_TS/CargosEscClass/Apis/ApiCargosEscClass';
import { CargosEscClassApi } from '@/app/GerAdv_TS/CargosEscClass/Apis/ApiCargosEscClass';
import { FilterCargosEscClass } from '../GerAdv_TS/CargosEscClass/Filters/CargosEscClass';
...
describe('CargosEscClassApi', () => {
  let cargosescclassApi: CargosEscClassApi;
  ...
  beforeEach(() => {
    ...
    cargosescclassApi = new CargosEscClassApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(cargosescclassApi).toBeInstanceOf(CargosEscClassApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new CargosEscClassApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(CargosEscClassApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all cargosescclass items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosescclassApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(cargosescclassApi.getAll()).rejects.toThrow(CargosEscClassApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await cargosescclassApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-CargosEscClass_last_getAll_${200}`)));
    });
    ...
    it('should throw CargosEscClassApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockCargosEscClass = CargosEscClassTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockCargosEscClass,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch cargosescclass by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosescclassApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockCargosEscClass));
      ...
      expect(result.data).toEqual(mockCargosEscClass);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEscClass',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterCargosEscClass = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter cargosescclass items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await cargosescclassApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockCargosEscClass: ICargosEscClass = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new cargosescclass successfully', async () => {
      const responseCargosEscClass = { ...mockCargosEscClass, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseCargosEscClass,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await cargosescclassApi.addAndUpdate(mockCargosEscClass);
    ...
    it('should update existing cargosescclass successfully', async () => {
      const existingCargosEscClass = { ...mockCargosEscClass, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingCargosEscClass,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await cargosescclassApi.addAndUpdate(existingCargosEscClass);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(cargosescclassApi.addAndUpdate(mockCargosEscClass)).rejects.toThrow(CargosEscClassApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEscClass',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete cargosescclass successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await cargosescclassApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(cargosescclassApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEscClass',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(cargosescclassApi.delete(1)).rejects.toThrow(CargosEscClassApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterCargosEscClass = { funcionario: 1 };
      
      const result = cargosescclassApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await cargosescclassApi.delete(1);
  ...
  describe('CargosEscClassApiError', () => {
    it('should create error with correct properties', () => {
      const error = new CargosEscClassApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('CargosEscClassApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new CargosEscClassApiError('Test message', 500);
