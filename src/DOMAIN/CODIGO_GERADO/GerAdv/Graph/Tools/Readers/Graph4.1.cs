...
public partial class GraphReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var query = $@"SELECT TOP ({max})
                   {campos}
                   FROM {DBGraph.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(gphDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(gphDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    gphVisto as Visto
FROM {oCnn.UseDbo}.Graph dbq WITH (INDEX = idx_Graph_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.gphQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.gphQuemAtu
WHERE [gphCodigo] = @id
...
FROM {oCnn.UseDbo}.Graph WITH (INDEX = idx_{DBGraphDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [gphCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBGraphDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(gphDtAtu, gphDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBGraphDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(gphDtAtu, gphDtCad) DESC;", oCnn.InnerConnection);
