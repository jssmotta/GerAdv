...
public partial class HistoricoController(IHistoricoService historicoService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.Historico regHistorico, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _historicoService.Validation(regHistorico, uri);
            if (result == null)
            {
                _logger.Warn("Historico: Validation failed to add or update Historico, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "Historico: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}