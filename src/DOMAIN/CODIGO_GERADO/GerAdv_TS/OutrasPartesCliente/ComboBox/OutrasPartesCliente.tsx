...
const OutrasPartesClienteComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOutrasPartesCliente | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOutrasPartesCliente | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOutrasPartesCliente);
    ...
    const handleAddClick = () => {
        ...
        const add = OutrasPartesClienteEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OutrasPartesClienteWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOutrasPartesCliente={editar ?? addValue ?? OutrasPartesClienteEmpty()} /> 
            )}
