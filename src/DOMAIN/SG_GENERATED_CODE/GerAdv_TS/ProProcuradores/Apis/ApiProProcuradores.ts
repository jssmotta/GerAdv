...
import { FilterProProcuradores, FilterProProcuradoresDefaults } from '../Filters/ProProcuradores'
import { IFilterProProcuradoresWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for ProProcuradores API */
const PROPROCURADORES_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class ProProcuradoresApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `ProProcuradores-${uri}`,
          {
            ...PROPROCURADORES_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${PROPROCURADORES_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProProcuradores-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os ProProcuradores'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProProcuradores-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar Pro Procuradores com ID ${id}`,
          id
        );
    }
    ...
    public async getListN(max?: number, filtro?: FilterProProcuradores): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProProcuradores-${uniqueKeyDay()}_lst_listN_data`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar lista de ProProcuradores'
        );
    }
 public async filterPreload(max: number, filtro?: FilterProProcuradores): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterProProcuradoresDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProProcuradores-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterProProcuradores): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar ProProcuradores'
        );
    }


    public async filterVoice(filtro?: FilterProProcuradores, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterProProcuradoresDefaults();

        // Create request object according to new backend structure
        const request: IFilterProProcuradoresWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProProcuradores-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter ProProcuradores'
        );
    }
    ...
    public async validation(regProProcuradores: IProProcuradores): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regProProcuradores as ProProcuradores, this.getHeaders());
            });
    ...
    public async addAndUpdate(regProProcuradores: IProProcuradores): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regProProcuradores as ProProcuradores, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ProProcuradoresApiError(
                'Erro ao salvar Pro Procuradores: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new ProProcuradoresApiError(
                `Erro ao excluir Pro Procuradores com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterProProcuradores, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterProProcuradoresWithVoiceRequest = {filter: filtro || new FilterProProcuradoresDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<ProProcuradores[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de ProProcuradores');
            this.notificationService.notify({
              entity: 'ProProcuradores',
              id: 0,
              action: NotifySystemActions.ERROR,
