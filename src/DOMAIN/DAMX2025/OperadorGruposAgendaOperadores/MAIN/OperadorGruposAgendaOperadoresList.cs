...
#region ListarDados_OperadorGruposAgendaOperadores
    public static async IAsyncEnumerable<DBOperadorGruposAgendaOperadores> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperadorGruposAgendaOperadores(dbRec);
        }
    }

    public static IEnumerable<DBOperadorGruposAgendaOperadores> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
