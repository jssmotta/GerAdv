...
const filterLogic = useCallback((data: IUF, filters: Record<string, any>) => {
  const idMatches = applyFilter(data, 'id', filters.id);
  return idMatches
  ;
}, []);
// Hook para filtros
...
const openConsultaCidade = (id: number) => {
  router.push(`/pages/cidade/?uf=${id}`);
};
const EditarCellCidade = (props: any) => {
  return (
  <>
  <td>
    <div onClick={() => openConsultaCidade(props.dataItem.id)}><span title='Editar Cidade'><SvgIcon icon={pencilIcon} /></span></div>
  </td>
</>
);
};
...
const gridColumns = useMemo(() => [
  ...
  <GridColumn field='id' title='ID' sortable={true} filterable={true} />, /* Track G.02 */
  <GridColumn
  field='id'
  filterable={false}
  sortable={false}
  width={'65px'}
  title='Cidade'
  cells={{ data: EditarCellCidade }}
  />, 
  <GridColumn field='inomepaises' title='Paises' sortable={false} filterable={false} />, /* Track G.01 */
  <GridColumn
  field='id'
  width={'55px'}
  title='Excluir'
