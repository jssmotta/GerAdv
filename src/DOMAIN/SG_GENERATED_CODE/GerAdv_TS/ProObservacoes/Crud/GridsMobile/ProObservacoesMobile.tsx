...
export const ProObservacoesGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('proobservacoes-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('proobservacoes-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-proobservacoes .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
