...
public partial class StatusAndamentoController(IStatusAndamentoService statusandamentoService) : ControllerBase
{
    ...
    {
        _logger.Info("StatusAndamento: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _statusandamentoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("StatusAndamento: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _statusandamentoService.UpdateColumns(parameters, uri);
        if (!result)
        {
