...
import { CargosApiError } from '../GerAdv_TS/Cargos/Apis/ApiCargos';
import { CargosApi } from '@/app/GerAdv_TS/Cargos/Apis/ApiCargos';
import { FilterCargos } from '../GerAdv_TS/Cargos/Filters/Cargos';
...
describe('CargosApi', () => {
  ...
  beforeEach(() => {
    ...
    cargosApi = new CargosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(cargosApi).toBeInstanceOf(CargosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new CargosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(CargosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all cargos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(cargosApi.getAll()).rejects.toThrow(CargosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await cargosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Cargos_last_getAll_${200}`)));
    });
    ...
    it('should throw CargosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockCargos = CargosTestEmpty();
    ...
    it('should fetch cargos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockCargos));
      ...
      expect(result.data).toEqual(mockCargos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cargos',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter cargos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await cargosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new cargos successfully', async () => {
      const responseCargos = { ...mockCargos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseCargos,
        status: 200,
        statusText: 'OK',
      ...
      const result = await cargosApi.addAndUpdate(mockCargos);
    ...
    it('should update existing cargos successfully', async () => {
      const existingCargos = { ...mockCargos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingCargos,
        status: 200,
        statusText: 'OK',
      ...
      const result = await cargosApi.addAndUpdate(existingCargos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(cargosApi.addAndUpdate(mockCargos)).rejects.toThrow(CargosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cargos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete cargos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await cargosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(cargosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Cargos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(cargosApi.delete(1)).rejects.toThrow(CargosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterCargos = { funcionario: 1 };
      
      const result = cargosApi.useFilter(mockFilter);
  ...
  describe('CargosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new CargosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('CargosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new CargosApiError('Test message', 500);
