...
import { AgendaRepetirDias } from '../../Models/AgendaRepetirDias';
...
export class AgendaRepetirDiasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AgendaRepetirDiasApiError';
    this.status = status;
    this.code = code;
...
export class AgendaRepetirDiasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AgendaRepetirDias`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRepetirDias_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AgendaRepetirDias');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRepetirDias_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Agenda Repetir Dias com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAgendaRepetirDias): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRepetirDias_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AgendaRepetirDias');
                }
            }


    public async validation(regAgendaRepetirDias: IAgendaRepetirDias): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAgendaRepetirDias as AgendaRepetirDias, this.getHeaders());
    ...
    public async addAndUpdate(regAgendaRepetirDias: IAgendaRepetirDias): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAgendaRepetirDias as AgendaRepetirDias, this.getHeaders());
            var register = result.data as IAgendaRepetirDias;        
            const action = regAgendaRepetirDias.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Agenda Repetir Dias');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AgendaRepetirDias. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AgendaRepetirDias',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Agenda Repetir Dias com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAgendaRepetirDias) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AgendaRepetirDias[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de agendarepetirdias:', error);
            this.notificationService.notify({
              entity: 'AgendaRepetirDias',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos agendarepetirdias'
            });
          }
        });
    }
