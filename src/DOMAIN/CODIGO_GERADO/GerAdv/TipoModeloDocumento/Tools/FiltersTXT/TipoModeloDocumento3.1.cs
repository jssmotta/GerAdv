...
public static class FilterTipoModeloDocumentoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterTipoModeloDocumento? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar TipoModeloDocumento em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoModeloDocumento:

- O filter irá filtrar os TipoModeloDocumento conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- nome: nome ou descrição para Tipo Modelo Documento;
...
- Se ouvir "todos os TipoModeloDocumento" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoModeloDocumento length: {result.Length}");
#endif
