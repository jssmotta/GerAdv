...
import { ParteCliente } from "../../Models/ParteCliente";
...
export class ParteClienteApi {
    ...
    constructor(uri: string, authorization: string) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API}/${uri}/ParteCliente`;
    ...
    public async addAndUpdate(regParteCliente: IParteCliente): Promise<AxiosResponse> {
        var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regParteCliente as ParteCliente, this.getHeaders());
        var register = result.data as IParteCliente;        
        const action = regParteCliente.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
    ...
    public useGetAll(max: number = 1000) {
        ...
        return useSWR<ParteCliente[]>(key, fetcher, {
