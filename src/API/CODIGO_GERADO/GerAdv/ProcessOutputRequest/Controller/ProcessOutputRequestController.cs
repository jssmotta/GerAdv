...
public partial class ProcessOutputRequestController(IProcessOutputRequestService processoutputrequestService) : ControllerBase
{
    ...
    {
        _logger.Info("ProcessOutputRequest: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _processoutputrequestService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProcessOutputRequest: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _processoutputrequestService.UpdateColumns(parameters, uri);
        if (!result)
        {
