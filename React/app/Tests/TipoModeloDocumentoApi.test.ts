...
import { TipoModeloDocumentoApiError } from '../GerAdv_TS/TipoModeloDocumento/Apis/ApiTipoModeloDocumento';
import { TipoModeloDocumentoApi } from '@/app/GerAdv_TS/TipoModeloDocumento/Apis/ApiTipoModeloDocumento';
import { FilterTipoModeloDocumento } from '../GerAdv_TS/TipoModeloDocumento/Filters/TipoModeloDocumento';
...
describe('TipoModeloDocumentoApi', () => {
  let tipomodelodocumentoApi: TipoModeloDocumentoApi;
  ...
  beforeEach(() => {
    ...
    tipomodelodocumentoApi = new TipoModeloDocumentoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipomodelodocumentoApi).toBeInstanceOf(TipoModeloDocumentoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoModeloDocumentoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoModeloDocumentoApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [TipoModeloDocumentoTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all tipomodelodocumento items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipomodelodocumentoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipomodelodocumentoApi.getAll()).rejects.toThrow(TipoModeloDocumentoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipomodelodocumentoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoModeloDocumento_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoModeloDocumentoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoModeloDocumento = TipoModeloDocumentoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoModeloDocumento,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipomodelodocumento by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipomodelodocumentoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoModeloDocumento));
      ...
      expect(result.data).toEqual(mockTipoModeloDocumento);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoModeloDocumento',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoModeloDocumento = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipomodelodocumento items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipomodelodocumentoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoModeloDocumento: ITipoModeloDocumento = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipomodelodocumento successfully', async () => {
      const responseTipoModeloDocumento = { ...mockTipoModeloDocumento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoModeloDocumento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipomodelodocumentoApi.addAndUpdate(mockTipoModeloDocumento);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockTipoModeloDocumento,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing tipomodelodocumento successfully', async () => {
      const existingTipoModeloDocumento = { ...mockTipoModeloDocumento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoModeloDocumento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipomodelodocumentoApi.addAndUpdate(existingTipoModeloDocumento);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipomodelodocumentoApi.addAndUpdate(mockTipoModeloDocumento)).rejects.toThrow(TipoModeloDocumentoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoModeloDocumento',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipomodelodocumento successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipomodelodocumentoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipomodelodocumentoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoModeloDocumento',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipomodelodocumentoApi.delete(1)).rejects.toThrow(TipoModeloDocumentoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoModeloDocumento = { funcionario: 1 };
      
      const result = tipomodelodocumentoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipomodelodocumentoApi.delete(1);
  ...
  describe('TipoModeloDocumentoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoModeloDocumentoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoModeloDocumentoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoModeloDocumentoApiError('Test message', 500);
