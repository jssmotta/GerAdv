...
const OponentesRepLegalComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOponentesRepLegal | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOponentesRepLegal | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOponentesRepLegal);
    ...
    const handleAddClick = () => {
        ...
        const add = OponentesRepLegalEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OponentesRepLegalWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOponentesRepLegal={editar ?? addValue ?? OponentesRepLegalEmpty()} /> 
            )}
