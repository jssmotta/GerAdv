...
export const useDocsRecebidosItensForm = (
  ...
  const loadDocsRecebidosItens = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialDocsRecebidosItens);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Docs Recebidos Itens';
      setError(errorMessage);
      console.log('Erro ao carregar Docs Recebidos Itens');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialDocsRecebidosItens]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadDocsRecebidosItens, resetForm]);
  return returnValue;
};


export const useDocsRecebidosItensNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('DocsRecebidosItens', (entity) => {
      try {
...
export const useDocsRecebidosItensList = (dataService: IDocsRecebidosItensService) => {
  const [data, setData] = useState<IDocsRecebidosItens[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar docsrecebidositens';
      setError(errorMessage);
      console.log('Erro ao carregar docsrecebidositens');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useDocsRecebidosItensNotifications(
