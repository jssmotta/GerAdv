...
import { StatusInstancia } from '../../Models/StatusInstancia';
...
export class StatusInstanciaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'StatusInstanciaApiError';
    this.status = status;
    this.code = code;
...
export class StatusInstanciaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/StatusInstancia`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusInstancia_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os StatusInstancia');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusInstancia_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Status Instancia com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterStatusInstancia): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusInstancia_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de StatusInstancia');
        }
    }

            public async filter(filtro: FilterStatusInstancia): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusInstancia_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar StatusInstancia');
                }
            }


    public async validation(regStatusInstancia: IStatusInstancia): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regStatusInstancia as StatusInstancia, this.getHeaders());
    ...
    public async addAndUpdate(regStatusInstancia: IStatusInstancia): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regStatusInstancia as StatusInstancia, this.getHeaders());
            var register = result.data as IStatusInstancia;        
            const action = regStatusInstancia.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Status Instancia');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o StatusInstancia. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'StatusInstancia',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Status Instancia com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterStatusInstancia) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<StatusInstancia[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de statusinstancia:', error);
            this.notificationService.notify({
              entity: 'StatusInstancia',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos statusinstancia'
            });
          }
        });
    }
