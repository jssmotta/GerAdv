...
public partial class Fase
{
    ...
    private async Task CreateIdx(string uri, MsiSqlConnection oCnn)
    {
        ...
        if (oCnnRw is null)
            return;
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Fase_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Fase]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}]idx_Fase_Auditor;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Fase_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Fase]'))
                BEGIN                    
                    CREATE INDEX idx_Fase_AuditorDtAtu ON [{oCnnRw.UseDbo}].[Fase](fasCodigo, fasDtAtu);
                END
                ", oCnnRw);
    }
}