...
public partial class FDadosProcuracao : MenphisSI.SG.GerAdv.DBDadosProcuracao, IDBDadosProcuracao
{
    // DI-compatible default constructor
    public FDadosProcuracao() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FDadosProcuracao CreateFromReader(SqlDataReader? dbRec)
    {
        return new FDadosProcuracao(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FDadosProcuracao CreateFromDataRow(DataRow? dbRec)
    {
        return new FDadosProcuracao(dbRec);
    }

    private FDadosProcuracao(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FDadosProcuracao(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FDadosProcuracao> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fDadosProcuracao = new FDadosProcuracao();
        await fDadosProcuracao.CarregarAsync(id, oCnn);
        return fDadosProcuracao;
    }

    private FDadosProcuracao(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FDadosProcuracao CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FDadosProcuracao(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
