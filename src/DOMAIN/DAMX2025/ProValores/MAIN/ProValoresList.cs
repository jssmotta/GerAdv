...
#region ListarDados_ProValores
    public static async IAsyncEnumerable<DBProValores> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProValores(dbRec);
        }
    }

    public static IEnumerable<DBProValores> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
