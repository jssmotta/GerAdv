...
import { Apenso } from '../../Models/Apenso';
...
export class ApensoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class ApensoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Apenso`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Apenso_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Apenso');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Apenso_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Apenso com ID ${id}`);
        }
    }

            public async filter(filtro: FilterApenso): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Apenso_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Apenso');
                }
            }


    public async validation(regApenso: IApenso): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regApenso as Apenso, this.getHeaders());
    ...
    public async addAndUpdate(regApenso: IApenso): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regApenso as Apenso, this.getHeaders());
            var register = result.data as IApenso;        
            const action = regApenso.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Apenso');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Apenso. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Apenso',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Apenso com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterApenso) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Apenso[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de apenso:', error);
            this.notificationService.notify({
              entity: 'Apenso',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos apenso'
            });
          }
        });
    }
