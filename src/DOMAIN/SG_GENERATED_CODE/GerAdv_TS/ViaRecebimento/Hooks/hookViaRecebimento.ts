...
export const useViaRecebimentoList = (dataService: IViaRecebimentoService, currentFilter?: FilterViaRecebimento | undefined | null) => {
  const [data, setData] = useState<IViaRecebimento[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterViaRecebimento | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterViaRecebimento | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterViaRecebimento);
      setData(result);
    } catch (err) {
