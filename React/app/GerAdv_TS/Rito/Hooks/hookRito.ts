...
export const useRitoForm = (initialRito: IRito, dataService: IRitoService) => {
  ...
  const loadRito = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Rito";
...
export const useRitoList = (
  dataService: IRitoService,
  currentFilter?: FilterRito | undefined | null,
) => {
  const [data, setData] = useState<IRito[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterRito | undefined | null>(currentFilter);
  ...
  const fetchData = useCallback(
    async (filtro?: FilterRito | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterRito));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar rito";
  ...
  function validate(data: IRito): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.descricao.length > 20) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 20 caracteres.",
        };
      }
