...
const ServicosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IServicos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IServicos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IServicos);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ServicosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedServicos={editar ?? addValue ?? ServicosEmpty()} /> 
            )}
