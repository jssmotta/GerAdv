// app/TestsForms/PenhoraComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import PenhoraComboBox from '@/app/GerAdv_TS/Penhora/ComboBox/Penhora';
...
import { PenhoraEmpty, PenhoraTestEmpty } from '@/app/GerAdv_TS/Models/Penhora';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Penhora/Hooks/hookPenhora', () => ({
  usePenhoraComboBox: jest.fn(() => ({
    options: [
    { ...PenhoraEmpty(), id: 1, nome: 'Penhora 1' },
    { ...PenhoraEmpty(), id: 2, nome: 'Penhora 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Penhora/Apis/ApiPenhora', () => ({
  PenhoraApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Penhora' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Penhora' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Penhora/Services/Penhora.service', () => ({
  PenhoraService: jest.fn().mockImplementation(() => ({
    fetchPenhoraById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Penhora' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Penhora/Crud/Grids/PenhoraWindow', () => {
  return function MockPenhoraWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='penhora-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Penhora' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...PenhoraTestEmpty(), id: 1, nome: 'Penhora 1' },
  { ...PenhoraTestEmpty(), id: 2, nome: 'Penhora 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: PenhoraEmpty(), 
};
describe('PenhoraComboBox', () => {
  ...
  const { usePenhoraComboBox } = require('@/app/GerAdv_TS/Penhora/Hooks/hookPenhora');
  (usePenhoraComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<PenhoraComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Penhora')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...PenhoraTestEmpty(), id: 1, nome: 'Penhora 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { usePenhoraComboBox } = require('@/app/GerAdv_TS/Penhora/Hooks/hookPenhora');
    usePenhoraComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens PenhoraWindow on action button click (add)', async () => {
    render(<PenhoraComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('penhora-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('penhora-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<PenhoraComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles PenhoraWindow onClose and onSuccess', () => {
    render(<PenhoraComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<PenhoraComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
