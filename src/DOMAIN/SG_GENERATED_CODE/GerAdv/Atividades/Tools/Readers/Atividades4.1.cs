...
SELECT TOP (1) 
	FORMAT(atvDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(atvDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    atvVisto as Visto
FROM {oCnn.UseDbo}.Atividades dbq WITH (INDEX = idx_Atividades_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.atvQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.atvQuemAtu
WHERE [atvCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBAtividades.PTabelaNome,
...
FROM {oCnn.UseDbo}.Atividades WITH (INDEX = idx_{DBAtividadesDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [atvCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBAtividadesDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(atvDtAtu, atvDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBAtividadesDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(atvDtAtu, atvDtCad) DESC;", oCnn.InnerConnection);
