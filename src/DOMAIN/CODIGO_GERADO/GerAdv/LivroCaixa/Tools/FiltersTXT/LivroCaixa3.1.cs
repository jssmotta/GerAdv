...
public static class FilterLivroCaixaTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterLivroCaixa? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar LivroCaixa em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterLivroCaixa:

- O filter irá filtrar os LivroCaixa conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterLivroCaixa:(Livro Caixa)
- iddes: número ou IDs;
- pessoal: número ou IDs;
- ajuste: Ajuste;
- idhon: número ou IDs;
- idhonparc: número ou IDs;
- idhonsuc: IDHonSuc;
- data: não é a data de cadastramento é a data do LivroCaixa;
...
- Se ouvir "todos os LivroCaixa" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt LivroCaixa length: {result.Length}");
#endif
