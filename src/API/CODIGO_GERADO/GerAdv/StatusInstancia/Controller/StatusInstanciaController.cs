...
public partial class StatusInstanciaController(IStatusInstanciaService statusinstanciaService) : ControllerBase
{
    ...
    {
        _logger.Info("StatusInstancia: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _statusinstanciaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("StatusInstancia: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _statusinstanciaService.UpdateColumns(parameters, uri);
        if (!result)
        {
