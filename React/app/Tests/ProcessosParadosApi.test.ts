...
import { ProcessosParadosApiError } from '../GerAdv_TS/ProcessosParados/Apis/ApiProcessosParados';
import { ProcessosParadosApi } from '@/app/GerAdv_TS/ProcessosParados/Apis/ApiProcessosParados';
import { FilterProcessosParados } from '../GerAdv_TS/ProcessosParados/Filters/ProcessosParados';
...
describe('ProcessosParadosApi', () => {
  let processosparadosApi: ProcessosParadosApi;
  ...
  beforeEach(() => {
    ...
    processosparadosApi = new ProcessosParadosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processosparadosApi).toBeInstanceOf(ProcessosParadosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessosParadosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessosParadosApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ProcessosParadosTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all processosparados items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosparadosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processosparadosApi.getAll()).rejects.toThrow(ProcessosParadosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processosparadosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProcessosParados_last_getAll_${200}`)));
    });
    ...
    it('should throw ProcessosParadosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessosParados = ProcessosParadosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProcessosParados,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch processosparados by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosparadosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessosParados));
      ...
      expect(result.data).toEqual(mockProcessosParados);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosParados',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProcessosParados = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter processosparados items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processosparadosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProcessosParados: IProcessosParados = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new processosparados successfully', async () => {
      const responseProcessosParados = { ...mockProcessosParados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessosParados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosparadosApi.addAndUpdate(mockProcessosParados);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockProcessosParados,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing processosparados successfully', async () => {
      const existingProcessosParados = { ...mockProcessosParados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessosParados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosparadosApi.addAndUpdate(existingProcessosParados);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processosparadosApi.addAndUpdate(mockProcessosParados)).rejects.toThrow(ProcessosParadosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosParados',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processosparados successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processosparadosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processosparadosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosParados',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processosparadosApi.delete(1)).rejects.toThrow(ProcessosParadosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProcessosParados = { funcionario: 1 };
      
      const result = processosparadosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await processosparadosApi.delete(1);
  ...
  describe('ProcessosParadosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessosParadosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessosParadosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessosParadosApiError('Test message', 500);
