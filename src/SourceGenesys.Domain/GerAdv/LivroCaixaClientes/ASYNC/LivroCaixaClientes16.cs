...
public partial class DBLivroCaixaClientes
{
    ...
    /// </summary>
    public static async Task<DBLivroCaixaClientes> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBLivroCaixaClientes();
...
#if (!NOTSTORED_LivroCaixaClientes)
    /// <summary>
...
#if (!NOTSTORED_LivroCaixaClientes)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFLivroCaixa || pFldFCliente || pFldFLancado;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFLivroCaixa)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.LivroCaixa, m_FLivroCaixa, ETiposCampos.FNumber);
        if (pFldFCliente)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.Cliente, m_FCliente, ETiposCampos.FNumber);
        if (pFldFLancado)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.Lancado, m_FLancado, ETiposCampos.FBoolean);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBLivroCaixaClientesDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBLivroCaixaClientesDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
