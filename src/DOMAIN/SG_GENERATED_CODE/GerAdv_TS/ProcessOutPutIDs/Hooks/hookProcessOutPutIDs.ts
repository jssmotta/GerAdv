...
export const useProcessOutPutIDsList = (dataService: IProcessOutPutIDsService, currentFilter?: FilterProcessOutPutIDs | undefined | null) => {
  const [data, setData] = useState<IProcessOutPutIDs[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterProcessOutPutIDs | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterProcessOutPutIDs | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterProcessOutPutIDs);
      setData(result);
    } catch (err) {
