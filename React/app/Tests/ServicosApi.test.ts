...
import { ServicosApiError } from '../GerAdv_TS/Servicos/Apis/ApiServicos';
import { ServicosApi } from '@/app/GerAdv_TS/Servicos/Apis/ApiServicos';
import { FilterServicos } from '../GerAdv_TS/Servicos/Filters/Servicos';
...
describe('ServicosApi', () => {
  ...
  beforeEach(() => {
    ...
    servicosApi = new ServicosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(servicosApi).toBeInstanceOf(ServicosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ServicosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ServicosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all servicos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await servicosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(servicosApi.getAll()).rejects.toThrow(ServicosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await servicosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Servicos_last_getAll_${200}`)));
    });
    ...
    it('should throw ServicosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockServicos = ServicosTestEmpty();
    ...
    it('should fetch servicos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await servicosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockServicos));
      ...
      expect(result.data).toEqual(mockServicos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Servicos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterServicos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter servicos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await servicosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new servicos successfully', async () => {
      const responseServicos = { ...mockServicos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseServicos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await servicosApi.addAndUpdate(mockServicos);
    ...
    it('should update existing servicos successfully', async () => {
      const existingServicos = { ...mockServicos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingServicos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await servicosApi.addAndUpdate(existingServicos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(servicosApi.addAndUpdate(mockServicos)).rejects.toThrow(ServicosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Servicos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete servicos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await servicosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(servicosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Servicos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(servicosApi.delete(1)).rejects.toThrow(ServicosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterServicos = { funcionario: 1 };
      
      const result = servicosApi.useFilter(mockFilter);
  ...
  describe('ServicosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ServicosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ServicosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ServicosApiError('Test message', 500);
