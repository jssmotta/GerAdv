...
public partial class InstanciaService
{
    ...
    /// <returns>FilterInstancia processado</returns>
    private async Task<Filters.FilterInstancia?> ProcessFilterVoiceCommandAsync(CommandSpeakerRequest message, Filters.FilterInstancia? existingFilter, string uri)
    {
        try
        {
            // Usar o contexto do FilterInstanciaTXT
            var context = FilterInstanciaTXT.GetPrompt(existingFilter, FilterInstanciaTXT.GetOptions());
            // Instruções específicas para Instancia
        ...
        catch (Exception)
        {
            ...
            return existingFilter ?? new Filters.FilterInstancia();
        }
    }

    private async Task<IEnumerable<InstanciaResponseAll>> GetDataAllAsync(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken cancellationToken)
    {
    ...
    private async Task<IEnumerable<InstanciaResponseAll>> GetDataAllAsync2(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken token)
    {
        using var scope = _connectionService.CreateConnectionScope(uri);
        using var oCnn = scope.Connection;
        if (oCnn == null)
