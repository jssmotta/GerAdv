...
#region ListarDados_Servicos
    public static async IAsyncEnumerable<DBServicos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBServicos(dbRec);
        }
    }

    public static IEnumerable<DBServicos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
