...
public partial class RamalService(IOptions<AppSettings> appSettings, IFRamalFactory ramalFactory, IRamalReader reader, IRamalValidation validation, IRamalWriter writer, ILigacoesService ligacoesService, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IRamalService, IDisposable
{
    ...
    private async Task<RamalResponse?> GetDataByIdAsync(int id, MsiSqlConnection? oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    ...
    public async Task<RamalResponse?> Validation([FromBody] Models.Ramal? regRamal, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regRamal.Id, oCnn);
    }
    ...
    public async Task<RamalResponse?> Delete([FromQuery] int? id, [FromRoute, Required] string uri)
    {
        ...
        var ramal = await reader.Read(id ?? default, oCnn);
        try
        {
            if (ramal != null)
            {
                await writer.Delete(ramal, nOperador, oCnn);
