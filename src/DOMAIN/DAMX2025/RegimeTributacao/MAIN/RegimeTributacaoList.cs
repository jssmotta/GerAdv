...
#region ListarDados_RegimeTributacao
    public static async IAsyncEnumerable<DBRegimeTributacao> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBRegimeTributacao(dbRec);
        }
    }

    public static IEnumerable<DBRegimeTributacao> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
