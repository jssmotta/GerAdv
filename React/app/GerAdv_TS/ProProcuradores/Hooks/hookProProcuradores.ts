...
export const useProProcuradoresForm = (
  ...
  const loadProProcuradores = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Pro Procuradores";
...
export const useProProcuradoresList = (
  dataService: IProProcuradoresService,
  currentFilter?: FilterProProcuradores | undefined | null,
) => {
  const [data, setData] = useState<IProProcuradores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProProcuradores | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterProProcuradores | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar proprocuradores";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProProcuradores() {
  ...
  function validate(data: IProProcuradores): {
    isValid: boolean;
    message: string;
  } {
