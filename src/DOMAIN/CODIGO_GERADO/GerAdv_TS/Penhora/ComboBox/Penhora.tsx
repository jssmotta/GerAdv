...
const PenhoraComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPenhora | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPenhora | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPenhora);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PenhoraWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPenhora={editar ?? addValue ?? PenhoraEmpty()} /> 
            )}
