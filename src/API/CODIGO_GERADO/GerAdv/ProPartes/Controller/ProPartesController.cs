...
public partial class ProPartesController(IProPartesService propartesService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterProPartes filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("ProPartes: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _propartesService.Filter(max, filtro, uri);
        return Ok(result);
    }
