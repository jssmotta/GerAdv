...
#region ListarDados_AgendaFinanceiro
    public static async IAsyncEnumerable<DBAgendaFinanceiro> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAgendaFinanceiro(dbRec);
        }
    }

    public static IEnumerable<DBAgendaFinanceiro> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
