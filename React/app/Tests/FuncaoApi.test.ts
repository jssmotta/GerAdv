...
import { FuncaoApiError } from '../GerAdv_TS/Funcao/Apis/ApiFuncao';
import { FuncaoApi } from '@/app/GerAdv_TS/Funcao/Apis/ApiFuncao';
import { FilterFuncao } from '../GerAdv_TS/Funcao/Filters/Funcao';
...
describe('FuncaoApi', () => {
  ...
  beforeEach(() => {
    ...
    funcaoApi = new FuncaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(funcaoApi).toBeInstanceOf(FuncaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new FuncaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(FuncaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all funcao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await funcaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(funcaoApi.getAll()).rejects.toThrow(FuncaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await funcaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Funcao_last_getAll_${200}`)));
    });
    ...
    it('should throw FuncaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockFuncao = FuncaoTestEmpty();
    ...
    it('should fetch funcao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await funcaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockFuncao));
      ...
      expect(result.data).toEqual(mockFuncao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcao',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter funcao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await funcaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new funcao successfully', async () => {
      const responseFuncao = { ...mockFuncao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseFuncao,
        status: 200,
        statusText: 'OK',
      ...
      const result = await funcaoApi.addAndUpdate(mockFuncao);
    ...
    it('should update existing funcao successfully', async () => {
      const existingFuncao = { ...mockFuncao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingFuncao,
        status: 200,
        statusText: 'OK',
      ...
      const result = await funcaoApi.addAndUpdate(existingFuncao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(funcaoApi.addAndUpdate(mockFuncao)).rejects.toThrow(FuncaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete funcao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await funcaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(funcaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(funcaoApi.delete(1)).rejects.toThrow(FuncaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterFuncao = { funcionario: 1 };
      
      const result = funcaoApi.useFilter(mockFilter);
  ...
  describe('FuncaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new FuncaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('FuncaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new FuncaoApiError('Test message', 500);
