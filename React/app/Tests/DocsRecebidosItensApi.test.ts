...
import { DocsRecebidosItensApiError } from '../GerAdv_TS/DocsRecebidosItens/Apis/ApiDocsRecebidosItens';
import { DocsRecebidosItensApi } from '@/app/GerAdv_TS/DocsRecebidosItens/Apis/ApiDocsRecebidosItens';
import { FilterDocsRecebidosItens } from '../GerAdv_TS/DocsRecebidosItens/Filters/DocsRecebidosItens';
...
describe('DocsRecebidosItensApi', () => {
  let docsrecebidositensApi: DocsRecebidosItensApi;
  ...
  beforeEach(() => {
    ...
    docsrecebidositensApi = new DocsRecebidosItensApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(docsrecebidositensApi).toBeInstanceOf(DocsRecebidosItensApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new DocsRecebidosItensApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(DocsRecebidosItensApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [DocsRecebidosItensTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all docsrecebidositens items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await docsrecebidositensApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(docsrecebidositensApi.getAll()).rejects.toThrow(DocsRecebidosItensApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await docsrecebidositensApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-DocsRecebidosItens_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DocsRecebidosItens',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw DocsRecebidosItensApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockDocsRecebidosItens = DocsRecebidosItensTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockDocsRecebidosItens,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch docsrecebidositens by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await docsrecebidositensApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockDocsRecebidosItens));
      ...
      expect(result.data).toEqual(mockDocsRecebidosItens);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DocsRecebidosItens',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterDocsRecebidosItens = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter docsrecebidositens items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await docsrecebidositensApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockDocsRecebidosItens: IDocsRecebidosItens = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new docsrecebidositens successfully', async () => {
      const responseDocsRecebidosItens = { ...mockDocsRecebidosItens, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseDocsRecebidosItens,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await docsrecebidositensApi.addAndUpdate(mockDocsRecebidosItens);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockDocsRecebidosItens,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing docsrecebidositens successfully', async () => {
      const existingDocsRecebidosItens = { ...mockDocsRecebidosItens, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingDocsRecebidosItens,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await docsrecebidositensApi.addAndUpdate(existingDocsRecebidosItens);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(docsrecebidositensApi.addAndUpdate(mockDocsRecebidosItens)).rejects.toThrow(DocsRecebidosItensApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DocsRecebidosItens',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete docsrecebidositens successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await docsrecebidositensApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(docsrecebidositensApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DocsRecebidosItens',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(docsrecebidositensApi.delete(1)).rejects.toThrow(DocsRecebidosItensApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterDocsRecebidosItens = { funcionario: 1 };
      
      const result = docsrecebidositensApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await docsrecebidositensApi.delete(1);
  ...
  describe('DocsRecebidosItensApiError', () => {
    it('should create error with correct properties', () => {
      const error = new DocsRecebidosItensApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('DocsRecebidosItensApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new DocsRecebidosItensApiError('Test message', 500);
