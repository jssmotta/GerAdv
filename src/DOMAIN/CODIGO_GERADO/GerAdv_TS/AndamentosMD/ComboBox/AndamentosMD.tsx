...
const AndamentosMDComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IAndamentosMD | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IAndamentosMD | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IAndamentosMD);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <AndamentosMDWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedAndamentosMD={editar ?? addValue ?? AndamentosMDEmpty()} /> 
            )}
