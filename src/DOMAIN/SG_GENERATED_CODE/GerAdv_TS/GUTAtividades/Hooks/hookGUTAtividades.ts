...
export const useGUTAtividadesList = (dataService: IGUTAtividadesService, currentFilter?: FilterGUTAtividades | undefined | null) => {
  const [data, setData] = useState<IGUTAtividades[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterGUTAtividades | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterGUTAtividades | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterGUTAtividades);
      setData(result);
    } catch (err) {
