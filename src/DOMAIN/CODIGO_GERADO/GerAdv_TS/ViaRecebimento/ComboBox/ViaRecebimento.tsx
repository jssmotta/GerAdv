...
const ViaRecebimentoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IViaRecebimento | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IViaRecebimento | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IViaRecebimento);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ViaRecebimentoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedViaRecebimento={editar ?? addValue ?? ViaRecebimentoEmpty()} /> 
            )}
