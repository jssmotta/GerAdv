...
import { StatusBiuApiError } from '../GerAdv_TS/StatusBiu/Apis/ApiStatusBiu';
import { StatusBiuApi } from '@/app/GerAdv_TS/StatusBiu/Apis/ApiStatusBiu';
import { FilterStatusBiu } from '../GerAdv_TS/StatusBiu/Filters/StatusBiu';
...
describe('StatusBiuApi', () => {
  let statusbiuApi: StatusBiuApi;
  ...
  beforeEach(() => {
    ...
    statusbiuApi = new StatusBiuApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(statusbiuApi).toBeInstanceOf(StatusBiuApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new StatusBiuApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(StatusBiuApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all statusbiu items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusbiuApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(statusbiuApi.getAll()).rejects.toThrow(StatusBiuApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await statusbiuApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-StatusBiu_last_getAll_${200}`)));
    });
    ...
    it('should throw StatusBiuApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockStatusBiu = StatusBiuTestEmpty();
    ...
    it('should fetch statusbiu by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusbiuApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockStatusBiu));
      ...
      expect(result.data).toEqual(mockStatusBiu);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusBiu',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterStatusBiu = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter statusbiu items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await statusbiuApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockStatusBiu: IStatusBiu = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new statusbiu successfully', async () => {
      const responseStatusBiu = { ...mockStatusBiu, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseStatusBiu,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusbiuApi.addAndUpdate(mockStatusBiu);
    ...
    it('should update existing statusbiu successfully', async () => {
      const existingStatusBiu = { ...mockStatusBiu, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingStatusBiu,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusbiuApi.addAndUpdate(existingStatusBiu);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(statusbiuApi.addAndUpdate(mockStatusBiu)).rejects.toThrow(StatusBiuApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusBiu',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete statusbiu successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await statusbiuApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(statusbiuApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusBiu',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(statusbiuApi.delete(1)).rejects.toThrow(StatusBiuApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterStatusBiu = { funcionario: 1 };
      
      const result = statusbiuApi.useFilter(mockFilter);
  ...
  describe('StatusBiuApiError', () => {
    it('should create error with correct properties', () => {
      const error = new StatusBiuApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('StatusBiuApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new StatusBiuApiError('Test message', 500);
