...
public partial class NENotasController(INENotasService nenotasService) : ControllerBase
{
    ...
    {
        _logger.Info("NENotas: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _nenotasService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("NENotas: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _nenotasService.UpdateColumns(parameters, uri);
        if (!result)
        {
