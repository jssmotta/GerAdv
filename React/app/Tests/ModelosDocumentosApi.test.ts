...
import { ModelosDocumentosApiError } from '../GerAdv_TS/ModelosDocumentos/Apis/ApiModelosDocumentos';
import { ModelosDocumentosApi } from '@/app/GerAdv_TS/ModelosDocumentos/Apis/ApiModelosDocumentos';
import { FilterModelosDocumentos } from '../GerAdv_TS/ModelosDocumentos/Filters/ModelosDocumentos';
...
describe('ModelosDocumentosApi', () => {
  let modelosdocumentosApi: ModelosDocumentosApi;
  ...
  beforeEach(() => {
    ...
    modelosdocumentosApi = new ModelosDocumentosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(modelosdocumentosApi).toBeInstanceOf(ModelosDocumentosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ModelosDocumentosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ModelosDocumentosApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ModelosDocumentosTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all modelosdocumentos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await modelosdocumentosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(modelosdocumentosApi.getAll()).rejects.toThrow(ModelosDocumentosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await modelosdocumentosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ModelosDocumentos_last_getAll_${200}`)));
    });
    ...
    it('should throw ModelosDocumentosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockModelosDocumentos = ModelosDocumentosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockModelosDocumentos,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch modelosdocumentos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await modelosdocumentosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockModelosDocumentos));
      ...
      expect(result.data).toEqual(mockModelosDocumentos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ModelosDocumentos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterModelosDocumentos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter modelosdocumentos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await modelosdocumentosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockModelosDocumentos: IModelosDocumentos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new modelosdocumentos successfully', async () => {
      const responseModelosDocumentos = { ...mockModelosDocumentos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseModelosDocumentos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await modelosdocumentosApi.addAndUpdate(mockModelosDocumentos);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockModelosDocumentos,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing modelosdocumentos successfully', async () => {
      const existingModelosDocumentos = { ...mockModelosDocumentos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingModelosDocumentos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await modelosdocumentosApi.addAndUpdate(existingModelosDocumentos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(modelosdocumentosApi.addAndUpdate(mockModelosDocumentos)).rejects.toThrow(ModelosDocumentosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ModelosDocumentos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete modelosdocumentos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await modelosdocumentosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(modelosdocumentosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ModelosDocumentos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(modelosdocumentosApi.delete(1)).rejects.toThrow(ModelosDocumentosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterModelosDocumentos = { funcionario: 1 };
      
      const result = modelosdocumentosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await modelosdocumentosApi.delete(1);
  ...
  describe('ModelosDocumentosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ModelosDocumentosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ModelosDocumentosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ModelosDocumentosApiError('Test message', 500);
