...
public partial class ViaRecebimentoController(IViaRecebimentoService viarecebimentoService) : ControllerBase
{
    ...
    {
        _logger.Info("ViaRecebimento: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _viarecebimentoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ViaRecebimento: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _viarecebimentoService.UpdateColumns(parameters, uri);
        if (!result)
        {
