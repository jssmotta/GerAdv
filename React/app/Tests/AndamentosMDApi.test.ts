...
import { AndamentosMDApiError } from '../GerAdv_TS/AndamentosMD/Apis/ApiAndamentosMD';
import { AndamentosMDApi } from '@/app/GerAdv_TS/AndamentosMD/Apis/ApiAndamentosMD';
import { FilterAndamentosMD } from '../GerAdv_TS/AndamentosMD/Filters/AndamentosMD';
...
describe('AndamentosMDApi', () => {
  let andamentosmdApi: AndamentosMDApi;
  ...
  beforeEach(() => {
    ...
    andamentosmdApi = new AndamentosMDApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(andamentosmdApi).toBeInstanceOf(AndamentosMDApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AndamentosMDApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AndamentosMDApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all andamentosmd items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await andamentosmdApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(andamentosmdApi.getAll()).rejects.toThrow(AndamentosMDApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await andamentosmdApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AndamentosMD_last_getAll_${200}`)));
    });
    ...
    it('should throw AndamentosMDApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAndamentosMD = AndamentosMDTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAndamentosMD,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch andamentosmd by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await andamentosmdApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAndamentosMD));
      ...
      expect(result.data).toEqual(mockAndamentosMD);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndamentosMD',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAndamentosMD = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter andamentosmd items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await andamentosmdApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAndamentosMD: IAndamentosMD = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new andamentosmd successfully', async () => {
      const responseAndamentosMD = { ...mockAndamentosMD, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAndamentosMD,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await andamentosmdApi.addAndUpdate(mockAndamentosMD);
    ...
    it('should update existing andamentosmd successfully', async () => {
      const existingAndamentosMD = { ...mockAndamentosMD, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAndamentosMD,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await andamentosmdApi.addAndUpdate(existingAndamentosMD);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(andamentosmdApi.addAndUpdate(mockAndamentosMD)).rejects.toThrow(AndamentosMDApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndamentosMD',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete andamentosmd successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await andamentosmdApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(andamentosmdApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndamentosMD',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(andamentosmdApi.delete(1)).rejects.toThrow(AndamentosMDApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAndamentosMD = { funcionario: 1 };
      
      const result = andamentosmdApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await andamentosmdApi.delete(1);
  ...
  describe('AndamentosMDApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AndamentosMDApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AndamentosMDApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AndamentosMDApiError('Test message', 500);
