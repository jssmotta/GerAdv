...
public partial class AdvogadosService
{
    ...
    public async Task<ResultApi<IEnumerable<NomeID>>> GetListN(int max, Filters.FilterAdvogados? filtro, string uri, CancellationToken token = default)
    {
        // Tracking: 20250606-0
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-Advogados-{max}-{filterHash}-GetListN-{keyCache}";
    ...
    public async Task<ResultApi<IEnumerable<AdvogadosResponseAll>>> GetAll(int max, string uri, CancellationToken token = default)
    {
        ThrowIfDisposed();
        ...
        {
            _logger.Error(ex, "Advogados: GetAll failed for uri = {0}", uri);
