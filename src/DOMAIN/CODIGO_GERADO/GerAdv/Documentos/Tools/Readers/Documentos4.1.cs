...
FROM {oCnn.UseDbo}.Documentos WITH (INDEX = idx_{DBDocumentosDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [docCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBDocumentosDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(docDtAtu, docDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBDocumentosDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(docDtAtu, docDtCad) DESC;", oCnn.InnerConnection);
