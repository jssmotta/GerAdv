...
export const useTiposAcaoForm = (
  ...
  const loadTiposAcao = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Tipos Acao";
...
export const useTiposAcaoList = (
  dataService: ITiposAcaoService,
  currentFilter?: FilterTiposAcao | undefined | null,
) => {
  const [data, setData] = useState<ITiposAcao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTiposAcao | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterTiposAcao | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar tiposacao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
