...
public partial class FProValores : MenphisSI.SG.GerAdv.DBProValores, IDBProValores
{
    // DI-compatible default constructor
    public FProValores() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FProValores CreateFromReader(SqlDataReader? dbRec)
    {
        return new FProValores(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FProValores CreateFromDataRow(DataRow? dbRec)
    {
        return new FProValores(dbRec);
    }

    private FProValores(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FProValores(DataRow? dbRec) : base(dbRec)
    {
    }

    private FProValores(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FProValores CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FProValores(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FProValores> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fProValores = new FProValores();
        await fProValores.CarregarAsync(id, oCnn);
        return fProValores;
    }
