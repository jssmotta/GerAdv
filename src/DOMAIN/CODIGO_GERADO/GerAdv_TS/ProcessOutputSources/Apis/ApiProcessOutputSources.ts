...
import { ProcessOutputSources } from '../../Models/ProcessOutputSources';
...
export class ProcessOutputSourcesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ProcessOutputSourcesApiError';
    this.status = status;
    this.code = code;
...
export class ProcessOutputSourcesApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProcessOutputSources`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputSources_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProcessOutputSources');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputSources_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Process Output Sources com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterProcessOutputSources): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputSources_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ProcessOutputSources');
        }
    }

            public async filter(filtro: FilterProcessOutputSources): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputSources_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProcessOutputSources');
                }
            }


    public async validation(regProcessOutputSources: IProcessOutputSources): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProcessOutputSources as ProcessOutputSources, this.getHeaders());
    ...
    public async addAndUpdate(regProcessOutputSources: IProcessOutputSources): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProcessOutputSources as ProcessOutputSources, this.getHeaders());
            var register = result.data as IProcessOutputSources;        
            const action = regProcessOutputSources.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Process Output Sources');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProcessOutputSources. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProcessOutputSources',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Process Output Sources com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProcessOutputSources) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProcessOutputSources[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de processoutputsources:', error);
            this.notificationService.notify({
              entity: 'ProcessOutputSources',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos processoutputsources'
            });
          }
        });
    }
