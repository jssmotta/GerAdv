...
import { PrecatoriaApiError } from '../GerAdv_TS/Precatoria/Apis/ApiPrecatoria';
import { PrecatoriaApi } from '@/app/GerAdv_TS/Precatoria/Apis/ApiPrecatoria';
import { FilterPrecatoria } from '../GerAdv_TS/Precatoria/Filters/Precatoria';
...
describe('PrecatoriaApi', () => {
  let precatoriaApi: PrecatoriaApi;
  ...
  beforeEach(() => {
    ...
    precatoriaApi = new PrecatoriaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(precatoriaApi).toBeInstanceOf(PrecatoriaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PrecatoriaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PrecatoriaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all precatoria items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await precatoriaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(precatoriaApi.getAll()).rejects.toThrow(PrecatoriaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await precatoriaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Precatoria_last_getAll_${200}`)));
    });
    ...
    it('should throw PrecatoriaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPrecatoria = PrecatoriaTestEmpty();
    ...
    it('should fetch precatoria by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await precatoriaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPrecatoria));
      ...
      expect(result.data).toEqual(mockPrecatoria);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Precatoria',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterPrecatoria = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter precatoria items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await precatoriaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockPrecatoria: IPrecatoria = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new precatoria successfully', async () => {
      const responsePrecatoria = { ...mockPrecatoria, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePrecatoria,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await precatoriaApi.addAndUpdate(mockPrecatoria);
    ...
    it('should update existing precatoria successfully', async () => {
      const existingPrecatoria = { ...mockPrecatoria, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPrecatoria,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await precatoriaApi.addAndUpdate(existingPrecatoria);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(precatoriaApi.addAndUpdate(mockPrecatoria)).rejects.toThrow(PrecatoriaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Precatoria',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete precatoria successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await precatoriaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(precatoriaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Precatoria',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(precatoriaApi.delete(1)).rejects.toThrow(PrecatoriaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterPrecatoria = { funcionario: 1 };
      
      const result = precatoriaApi.useFilter(mockFilter);
  ...
  describe('PrecatoriaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PrecatoriaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PrecatoriaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PrecatoriaApiError('Test message', 500);
