...
#region ListarDados_HonorariosDadosContrato
    public static async IAsyncEnumerable<DBHonorariosDadosContrato> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBHonorariosDadosContrato(dbRec);
        }
    }

    public static IEnumerable<DBHonorariosDadosContrato> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
