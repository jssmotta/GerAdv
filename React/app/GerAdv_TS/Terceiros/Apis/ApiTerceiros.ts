...
export class TerceirosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class TerceirosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Terceiros`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Terceiros_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Terceiros');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Terceiros_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Terceiros com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Terceiros_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Terceiros');
        }
    }

            public async filter(filtro: FilterTerceiros): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Terceiros_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Terceiros');
                }
            }


    public async validation(regTerceiros: ITerceiros): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTerceiros as Terceiros, this.getHeaders());
    ...
    public async addAndUpdate(regTerceiros: ITerceiros): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Terceiros');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Terceiros. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Terceiros',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Terceiros com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTerceiros) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Terceiros[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de terceiros:', error);
            this.notificationService.notify({
              entity: 'Terceiros',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos terceiros'
            });
          }
        });
    }
