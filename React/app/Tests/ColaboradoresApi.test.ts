...
import { ColaboradoresApiError } from '../GerAdv_TS/Colaboradores/Apis/ApiColaboradores';
import { ColaboradoresApi } from '@/app/GerAdv_TS/Colaboradores/Apis/ApiColaboradores';
import { FilterColaboradores } from '../GerAdv_TS/Colaboradores/Filters/Colaboradores';
...
describe('ColaboradoresApi', () => {
  let colaboradoresApi: ColaboradoresApi;
  ...
  beforeEach(() => {
    ...
    colaboradoresApi = new ColaboradoresApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(colaboradoresApi).toBeInstanceOf(ColaboradoresApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ColaboradoresApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ColaboradoresApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all colaboradores items successfully', async () => {
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(colaboradoresApi.getAll()).rejects.toThrow(ColaboradoresApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await colaboradoresApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Colaboradores_last_getAll_${200}`)));
    });
    ...
    it('should throw ColaboradoresApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockColaboradores = ColaboradoresTestEmpty();
    ...
    it('should fetch colaboradores by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await colaboradoresApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockColaboradores));
      ...
      expect(result.data).toEqual(mockColaboradores);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Colaboradores',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterColaboradores = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter colaboradores items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await colaboradoresApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockColaboradores: IColaboradores = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new colaboradores successfully', async () => {
      const responseColaboradores = { ...mockColaboradores, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await colaboradoresApi.addAndUpdate(mockColaboradores);
    ...
    it('should update existing colaboradores successfully', async () => {
      const existingColaboradores = { ...mockColaboradores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingColaboradores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await colaboradoresApi.addAndUpdate(existingColaboradores);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(colaboradoresApi.addAndUpdate(mockColaboradores)).rejects.toThrow(ColaboradoresApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Colaboradores',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete colaboradores successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await colaboradoresApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(colaboradoresApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Colaboradores',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(colaboradoresApi.delete(1)).rejects.toThrow(ColaboradoresApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterColaboradores = { funcionario: 1 };
      
      const result = colaboradoresApi.useFilter(mockFilter);
  ...
  describe('ColaboradoresApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ColaboradoresApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ColaboradoresApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ColaboradoresApiError('Test message', 500);
