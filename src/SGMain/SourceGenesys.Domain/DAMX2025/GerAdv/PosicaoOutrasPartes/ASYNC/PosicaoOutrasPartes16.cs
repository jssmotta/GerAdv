...
public partial class DBPosicaoOutrasPartes
{
    ...
    /// </summary>
    public static async Task<DBPosicaoOutrasPartes> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBPosicaoOutrasPartes();
...
#if (!NOTSTORED_PosicaoOutrasPartes)
    /// <summary>
...
#if (!NOTSTORED_PosicaoOutrasPartes)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFDescricao || pFldFBold || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFDescricao)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFBold || updateTool.Insert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
