...
export const useProProcuradoresForm = (
  ...
  const loadProProcuradores = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialProProcuradores);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Pro Procuradores';
      setError(errorMessage);
      console.log('Erro ao carregar Pro Procuradores');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialProProcuradores]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadProProcuradores, resetForm]);
  return returnValue;
};


export const useProProcuradoresNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('ProProcuradores', (entity) => {
      try {
...
export const useProProcuradoresList = (dataService: IProProcuradoresService) => {
  const [data, setData] = useState<IProProcuradores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar proprocuradores';
      setError(errorMessage);
      console.log('Erro ao carregar proprocuradores');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useProProcuradoresNotifications(
