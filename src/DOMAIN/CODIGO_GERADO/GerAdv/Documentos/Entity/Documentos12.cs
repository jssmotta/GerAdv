...
public partial class FDocumentos : MenphisSI.SG.GerAdv.DBDocumentos, IDBDocumentos
{
    // DI-compatible default constructor
    public FDocumentos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FDocumentos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FDocumentos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FDocumentos CreateFromDataRow(DataRow? dbRec)
    {
        return new FDocumentos(dbRec);
    }

    private FDocumentos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FDocumentos(DataRow? dbRec) : base(dbRec)
    {
    }

    private FDocumentos(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FDocumentos CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FDocumentos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FDocumentos> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fDocumentos = new FDocumentos();
        await fDocumentos.CarregarAsync(id, oCnn);
        return fDocumentos;
    }
