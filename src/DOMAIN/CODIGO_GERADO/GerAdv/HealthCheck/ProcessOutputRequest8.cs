...
public class ProcessOutputRequestHealthCheck(IOptions<AppSettings> appSettings, ProcessOutputRequestService processoutputrequestService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly ProcessOutputRequestService _processoutputrequestService = processoutputrequestService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = "SELECT TOP (1) porProcessOutputEngine,porOperador,porProcesso,porUltimoIdTabelaExo FROM dbo.ProcessOutputRequest (NOLOCK);";
                            tableCheck.CommandTimeout = 5;
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT ProcessOutputRequest successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT ProcessOutputRequest failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _processoutputrequestService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-ProcessOutputRequest";
                    ...
                    static async Task<IEnumerable<Models.ProcessOutputRequest>> GetData(string uri, CancellationToken token)
                    {
            ...
            return isHealthy ? HealthCheckResult.Healthy("ProcessOutputRequest is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
