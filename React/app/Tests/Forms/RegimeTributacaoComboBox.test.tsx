// app/TestsForms/RegimeTributacaoComboBox.test.tsx - Vers�o Corrigida
...
import { RegimeTributacaoEmpty, RegimeTributacaoTestEmpty } from '@/app/GerAdv_TS/Models/RegimeTributacao';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/RegimeTributacao/Hooks/hookRegimeTributacao', () => ({
  useRegimeTributacaoComboBox: jest.fn(() => ({
    options: [
    { ...RegimeTributacaoEmpty(), id: 1, nome: 'Regime Tributacao 1' },
    { ...RegimeTributacaoEmpty(), id: 2, nome: 'Regime Tributacao 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/RegimeTributacao/Apis/ApiRegimeTributacao', () => ({
  RegimeTributacaoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock RegimeTributacao' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock RegimeTributacao' }),
...
jest.mock('@/app/GerAdv_TS/RegimeTributacao/Services/RegimeTributacao.service', () => ({
  RegimeTributacaoService: jest.fn().mockImplementation(() => ({
    fetchRegimeTributacaoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock RegimeTributacao' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/RegimeTributacao/Crud/Grids/RegimeTributacaoWindow', () => {
  return function MockRegimeTributacaoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='regimetributacao-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Regime Tributacao' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...RegimeTributacaoTestEmpty(), id: 1, nome: 'Regime Tributacao 1' },
  { ...RegimeTributacaoTestEmpty(), id: 2, nome: 'Regime Tributacao 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: RegimeTributacaoEmpty(), 
};
describe('RegimeTributacaoComboBox', () => {
  ...
  const { useRegimeTributacaoComboBox } = require('@/app/GerAdv_TS/RegimeTributacao/Hooks/hookRegimeTributacao');
  (useRegimeTributacaoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<RegimeTributacaoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Regime Tributacao')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Regime Tributacao')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...RegimeTributacaoTestEmpty(), id: 1, nome: 'Regime Tributacao 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useRegimeTributacaoComboBox } = require('@/app/GerAdv_TS/RegimeTributacao/Hooks/hookRegimeTributacao');
    useRegimeTributacaoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens RegimeTributacaoWindow on action button click (add)', async () => {
    render(<RegimeTributacaoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('regimetributacao-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('regimetributacao-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<RegimeTributacaoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles RegimeTributacaoWindow onClose and onSuccess', () => {
    render(<RegimeTributacaoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<RegimeTributacaoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
