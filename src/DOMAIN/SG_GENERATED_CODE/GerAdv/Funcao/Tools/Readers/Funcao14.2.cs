...
public partial class FuncaoWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        if (oCnn is null)
            return;
        Console.WriteLine($"WarmUp Funcao: {tenantKey}");
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_Funcao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[Funcao]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Funcao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Funcao]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_Funcao_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Funcao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Funcao]'))
                BEGIN                    
