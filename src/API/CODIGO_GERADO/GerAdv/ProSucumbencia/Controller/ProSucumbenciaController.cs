...
public partial class ProSucumbenciaController(IProSucumbenciaService prosucumbenciaService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.ProSucumbencia regProSucumbencia, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "ProSucumbencia: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "ProSucumbencia: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.ProSucumbencia regProSucumbencia, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _prosucumbenciaService.Validation(regProSucumbencia, uri);
            if (result == null)
            {
                _logger.Warn("ProSucumbencia: Validation failed to add or update ProSucumbencia, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "ProSucumbencia: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}