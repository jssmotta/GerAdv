...
import { usePenhoraGrid } from "../../Hooks/hookPenhoraGrid";
...
export const PenhoraGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(btoa("penhora-columns-order"));
        if (savedOrder) {
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        nomepenhorastatus: (
          <GridColumn
            key="nomepenhorastatus"
            field="nomepenhorastatus"
            title="Penhora Status"
            sortable={false}
            filterable={false}
          />
        ) /* Track G.04 */,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("penhora-columns-order"));
    }, []);
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Penhora-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Penhora-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`penhora-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-penhora"
