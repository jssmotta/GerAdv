...
#region ListarDados_GruposEmpresas
    public static async IAsyncEnumerable<DBGruposEmpresas> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGruposEmpresas(dbRec);
        }
    }

    public static IEnumerable<DBGruposEmpresas> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
