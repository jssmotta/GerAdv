...
export const useCargosEscClassList = (dataService: ICargosEscClassService, currentFilter?: FilterCargosEscClass | undefined | null) => {
  const [data, setData] = useState<ICargosEscClass[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterCargosEscClass | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterCargosEscClass | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterCargosEscClass);
      setData(result);
    } catch (err) {
