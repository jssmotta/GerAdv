...
const ProcessOutPutIDsComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProcessOutPutIDs | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProcessOutPutIDs | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProcessOutPutIDs);
    ...
    const handleAddClick = () => {
        ...
        const add = ProcessOutPutIDsEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProcessOutPutIDsWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProcessOutPutIDs={editar ?? addValue ?? ProcessOutPutIDsEmpty()} /> 
            )}
