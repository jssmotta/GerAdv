...
export const usePreClientesList = (dataService: IPreClientesService, currentFilter?: FilterPreClientes | undefined | null) => {
  const [data, setData] = useState<IPreClientes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterPreClientes | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterPreClientes | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterPreClientes);
      setData(result);
    } catch (err) {
