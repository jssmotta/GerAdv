...
public partial class DBOperadorGrupo
{
    ...
    /// </summary>
    public static async Task<DBOperadorGrupo> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBOperadorGrupo();
...
#if (!NOTSTORED_OperadorGrupo)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFOperador || pFldFGrupo || pFldFInativo;
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBOperadorGrupoDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBOperadorGrupoDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBOperadorGrupoDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBOperadorGrupoDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBOperadorGrupoDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
