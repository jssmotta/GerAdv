...
export const useOponentesForm = (
  ...
  const loadOponentes = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Oponentes";
...
export const useOponentesList = (
  dataService: IOponentesService,
  currentFilter?: FilterOponentes | undefined | null,
) => {
  const [data, setData] = useState<IOponentes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOponentes | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IOponentes): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.ctpsnumero.length > 15) {
        return {
          isValid: false,
          message: "O campo CTPSNumero não pode ter mais de 15 caracteres.",
        };
      }
      ...
      if (data.ctpsserie.length > 10) {
        return {
          isValid: false,
          message: "O campo CTPSSerie não pode ter mais de 10 caracteres.",
        };
      }
      ...
      if (data.pis.length > 20) {
        return {
          isValid: false,
          message: "O campo PIS não pode ter mais de 20 caracteres.",
        };
      }
      if (data.contato.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Contato não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.rg.length > 12) {
        return {
          isValid: false,
          message: "O campo RG não pode ter mais de 12 caracteres.",
        };
      }
      ...
      if (data.fax.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo Fax não pode ter mais de 2147483647 caracteres.",
        };
      }
      ...
      if (data.observacao.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Observacao não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.email.length > 100) {
        return {
          isValid: false,
          message: "O campo EMail não pode ter mais de 100 caracteres.",
        };
      }
