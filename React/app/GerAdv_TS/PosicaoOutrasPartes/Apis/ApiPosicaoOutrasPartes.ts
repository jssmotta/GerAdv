...
import { PosicaoOutrasPartes } from '../../Models/PosicaoOutrasPartes';
...
export class PosicaoOutrasPartesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'PosicaoOutrasPartesApiError';
    this.status = status;
    this.code = code;
...
export class PosicaoOutrasPartesApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/PosicaoOutrasPartes`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PosicaoOutrasPartes_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os PosicaoOutrasPartes');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PosicaoOutrasPartes_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Posicao Outras Partes com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterPosicaoOutrasPartes): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PosicaoOutrasPartes_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de PosicaoOutrasPartes');
        }
    }

            public async filter(filtro: FilterPosicaoOutrasPartes): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PosicaoOutrasPartes_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar PosicaoOutrasPartes');
                }
            }


    public async validation(regPosicaoOutrasPartes: IPosicaoOutrasPartes): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regPosicaoOutrasPartes as PosicaoOutrasPartes, this.getHeaders());
    ...
    public async addAndUpdate(regPosicaoOutrasPartes: IPosicaoOutrasPartes): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regPosicaoOutrasPartes as PosicaoOutrasPartes, this.getHeaders());
            var register = result.data as IPosicaoOutrasPartes;        
            const action = regPosicaoOutrasPartes.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Posicao Outras Partes');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o PosicaoOutrasPartes. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'PosicaoOutrasPartes',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Posicao Outras Partes com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterPosicaoOutrasPartes) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<PosicaoOutrasPartes[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de posicaooutraspartes:', error);
            this.notificationService.notify({
              entity: 'PosicaoOutrasPartes',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos posicaooutraspartes'
            });
          }
        });
    }
