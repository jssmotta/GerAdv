...
public partial class PrecatoriaReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBPrecatoriaDicInfo.CampoCodigo}";
        ...
        var query = $@"SELECT TOP ({max})
                   ...
                   FROM {DBPrecatoria.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBProcessosDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBProcessosDicInfo.PTabelaNome}].[{DBProcessos.CampoCodigo}]=[{DBPrecatoriaDicInfo.PTabelaNome}].[{DBPrecatoriaDicInfo.Processo}]
