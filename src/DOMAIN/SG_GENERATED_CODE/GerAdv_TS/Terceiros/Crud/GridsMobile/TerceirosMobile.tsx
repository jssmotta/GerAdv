...
export const TerceirosGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('terceiros-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('terceiros-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-terceiros .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
