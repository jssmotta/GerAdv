...
export const useDiario2List = (
  dataService: IDiario2Service,
  currentFilter?: FilterDiario2 | undefined | null,
) => {
  const [data, setData] = useState<IDiario2[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterDiario2 | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterDiario2 | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar diario2";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IDiario2): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.ocorrencia.length > 2048) {
        return {
          isValid: false,
          message: "O campo Ocorrencia não pode ter mais de 2048 caracteres.",
        };
      }
