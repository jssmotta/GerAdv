...
export const useTipoEnderecoForm = (
  ...
  const loadTipoEndereco = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Tipo Endereco";
...
export const useTipoEnderecoList = (
  dataService: ITipoEnderecoService,
  currentFilter?: FilterTipoEndereco | undefined | null,
) => {
  const [data, setData] = useState<ITipoEndereco[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoEndereco | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterTipoEndereco | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar tipoendereco";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoEndereco() {
  ...
  function validate(data: ITipoEndereco): {
    isValid: boolean;
    message: string;
  } {
