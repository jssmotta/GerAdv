...
public partial class DBEnderecoSistema
{
    ...
    /// </summary>
    public static async Task<DBEnderecoSistema> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBEnderecoSistema();
...
#if (!NOTSTORED_EnderecoSistema)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFCadastro || pFldFCadastroExCod || pFldFTipoEnderecoSistema || pFldFProcesso || pFldFMotivo || pFldFContatoNoLocal || pFldFCidade || pFldFEndereco || pFldFBairro || pFldFCEP || pFldFFone || pFldFFax || pFldFObservacao || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFCadastro)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Cadastro, m_FCadastro, ETiposCampos.FNumber);
        if (pFldFCadastroExCod)
            updateTool.Fields(DBEnderecoSistemaDicInfo.CadastroExCod, m_FCadastroExCod, ETiposCampos.FNumber);
        if (pFldFTipoEnderecoSistema)
            updateTool.Fields(DBEnderecoSistemaDicInfo.TipoEnderecoSistema, m_FTipoEnderecoSistema, ETiposCampos.FNumber);
        if (pFldFProcesso)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFMotivo)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Motivo, m_FMotivo, ETiposCampos.FString);
        if (pFldFContatoNoLocal)
            updateTool.Fields(DBEnderecoSistemaDicInfo.ContatoNoLocal, m_FContatoNoLocal, ETiposCampos.FString);
        if (pFldFCidade)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Cidade, m_FCidade, ETiposCampos.FNumber);
        if (pFldFEndereco)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Endereco, m_FEndereco, ETiposCampos.FString);
        if (pFldFBairro)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Bairro, m_FBairro, ETiposCampos.FString);
        if (pFldFCEP)
            updateTool.Fields(DBEnderecoSistemaDicInfo.CEP, m_FCEP, ETiposCampos.FString);
        if (pFldFFone)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Fone, m_FFone, ETiposCampos.FString);
        if (pFldFFax)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Fax, m_FFax, ETiposCampos.FString);
        if (pFldFObservacao)
            updateTool.Fields(DBEnderecoSistemaDicInfo.Observacao, m_FObservacao, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBEnderecoSistemaDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBEnderecoSistemaDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBEnderecoSistemaDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBEnderecoSistemaDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBEnderecoSistemaDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBEnderecoSistemaDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
