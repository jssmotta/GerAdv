...
export class ProcessosParadosApi {
 ...
 public async filterPreload(max: number, filtro: FilterProcessosParados): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosParados-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterProcessosParados): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosParados-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterProcessosParados) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ProcessosParados[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de ProcessosParados');
            this.notificationService.notify({
              entity: 'ProcessosParados',
              id: 0,
              action: NotifySystemActions.ERROR,
