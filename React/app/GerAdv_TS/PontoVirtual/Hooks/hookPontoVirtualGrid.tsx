...
// usePontoVirtualGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de PontoVirtual
'use client';
...
interface UsePontoVirtualGridProps {
  data: IPontoVirtual[];
...
export const usePontoVirtualGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('pontovirtual-columns-order'), JSON.stringify(reorderableFields));
