...
import { useClientesSociosGrid } from "../../Hooks/hookClientesSociosGrid";
...
import ClientesSociosHelpCrudButton from "../../Help/ClientesSociosHelpCrudButton";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("clientessocios-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-clientessocios .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-clientessocios",
      columns: exportColumns,
      sheetName: "ClientesSocios",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-ClientesSocios-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-clientessocios",
      columns: exportColumnsPdf,
      title: "ClientesSocios",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-ClientesSocios-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            <PageTitle
              title="Sócios"
              helpButton={<ClientesSociosHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-clientessocios"
                      key={`clientessocios-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IClientesSocios);
