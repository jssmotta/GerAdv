...
public static class PenhoraStatusEndpoints
{
    ...
    private static void MapPenhoraStatusRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IPenhoraStatusValidation validation, IPenhoraStatusWriter writer, IPenhoraStatusService service) =>
        {
            logger.LogInfo("PenhoraStatus", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("PenhoraStatus", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("PenhoraStatus_GetColumns").WithDisplayName("Get PenhoraStatus Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IPenhoraStatusValidation validation, IPenhoraStatusWriter writer, IPenhoraStatusService service) =>
        {
            logger.LogInfo("PenhoraStatus", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("PenhoraStatus", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("PenhoraStatus_UpdateColumns").WithDisplayName("Update PenhoraStatus Columns");
