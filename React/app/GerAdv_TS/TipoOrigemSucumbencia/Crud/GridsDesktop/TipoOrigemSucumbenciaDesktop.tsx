...
export const TipoOrigemSucumbenciaGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("tipoorigemsucumbencia-columns-order"),
        );
        if (savedOrder) {
    ...
    const SearchFromCellProSucumbencia = (props: any) => {
      return (
        <>
          <td>
            <div
              onClick={() => openSearchCellProSucumbencia(props.dataItem.id)}
            >
              <span title="Pesquisar relacionados em Pro Sucumbencia">
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("tipoorigemsucumbencia-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-tipoorigemsucumbencia",
      columns: exportColumns,
      sheetName: "TipoOrigemSucumbencia",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-TipoOrigemSucumbencia-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-tipoorigemsucumbencia",
      columns: exportColumnsPdf,
      title: "TipoOrigemSucumbencia",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-TipoOrigemSucumbencia-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`tipoorigemsucumbencia-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-tipoorigemsucumbencia"
