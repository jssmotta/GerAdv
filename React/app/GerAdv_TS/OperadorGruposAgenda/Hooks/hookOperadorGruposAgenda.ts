...
export const useOperadorGruposAgendaForm = (
  ...
  const loadOperadorGruposAgenda = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialOperadorGruposAgenda);
      return;
    }
  ...
  }, [dataService, initialOperadorGruposAgenda]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadOperadorGruposAgenda, resetForm]);
  return returnValue;
};


export const useOperadorGruposAgendaNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('OperadorGruposAgenda', (entity) => {
      try {
...
export const useOperadorGruposAgendaList = (dataService: IOperadorGruposAgendaService) => {
  const [data, setData] = useState<IOperadorGruposAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  useOperadorGruposAgendaNotifications(
...
if (data.sqlwhere.length > 2147483647) { 
                                             return { isValid: false, message: 'O campo SQLWhere não pode ter mais de 2147483647 caracteres.' };
                                         } 
