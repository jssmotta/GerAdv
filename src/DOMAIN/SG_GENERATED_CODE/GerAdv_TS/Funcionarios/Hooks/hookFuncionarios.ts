...
export const useFuncionariosList = (dataService: IFuncionariosService, currentFilter?: FilterFuncionarios | undefined | null) => {
  const [data, setData] = useState<IFuncionarios[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterFuncionarios | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterFuncionarios | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterFuncionarios);
      setData(result);
    } catch (err) {
