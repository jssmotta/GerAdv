...
public class FOperadorGruposAgendaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FOperadorGruposAgendaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FOperadorGruposAgendaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FOperadorGruposAgendaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "groCodigo")
        var dt = new DataTable();
        dt.Columns.Add("groCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["groCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FOperadorGruposAgenda>();
    }
    ...
    {
        // Arrange
        var factory = new FOperadorGruposAgendaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("groCodigo", typeof(int));
        mockDataTable.Columns.Add("groNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["groCodigo"] = testId;
        row["groNome"] = "Test OperadorGruposAgenda";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["groCodigo"] = 123;
        row["groNome"] = "Test OperadorGruposAgenda Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FOperadorGruposAgenda properties
        }
    }
