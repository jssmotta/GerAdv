...
#region ListarDados_Enderecos
    public static async IAsyncEnumerable<DBEnderecos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEnderecos(dbRec);
        }
    }

    public static IEnumerable<DBEnderecos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
