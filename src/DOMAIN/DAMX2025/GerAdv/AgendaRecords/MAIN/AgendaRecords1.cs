...
#endregion
    public DBAgendaRecords(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    // REF. 250325
    public DBAgendaRecords(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-3
        if (oCnn is null)
            return;
...
#region GravarDados_AgendaRecords
    public int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        {
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_AgendaRecords)
#endif
