...
import { FilterTipoEndereco } from '@/app/GerAdv_TS/TipoEndereco/Filters/TipoEndereco';
...
interface UseTipoEnderecoFilterProps {
    handleFetchWithFilter: (filtro?: FilterTipoEndereco | undefined | null) => Promise<void>;
}

interface UseTipoEnderecoFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterTipoEndereco;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterTipoEndereco>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterTipoEndereco) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterTipoEndereco>) => React.ReactNode;
...
export const useTipoEnderecoFilter = ({ handleFetchWithFilter }: UseTipoEnderecoFilterProps): UseTipoEnderecoFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterTipoEndereco>({} as FilterTipoEndereco);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterTipoEndereco;
    ...
    const handleConfirmSearch = async (filter: FilterTipoEndereco) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterTipoEndereco);
        sessionStorage.removeItem(btoa('TipoEnderecoFilter'));
        handleFetchWithFilter({} as FilterTipoEndereco);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterTipoEndereco>) => (        
        <>
        <InputInput
                        type='text'
                        id='descricao'
