...
public partial class DBPaises
{
    ...
    /// </summary>
    public static async Task<DBPaises> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBPaises();
    ...
    {
        if (pFldFNome)
            updateTool.Fields(DBPaisesDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFGuid)
            updateTool.Fields(DBPaisesDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBPaisesDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBPaisesDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBPaisesDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBPaisesDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBPaisesDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBPaisesDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
