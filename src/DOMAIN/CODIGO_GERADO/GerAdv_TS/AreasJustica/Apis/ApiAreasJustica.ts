...
import { AreasJustica } from '../../Models/AreasJustica';
...
export class AreasJusticaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AreasJusticaApiError';
    this.status = status;
    this.code = code;
...
export class AreasJusticaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AreasJustica`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AreasJustica_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AreasJustica');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AreasJustica_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Areas Justica com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAreasJustica): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AreasJustica_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AreasJustica');
                }
            }


    public async validation(regAreasJustica: IAreasJustica): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAreasJustica as AreasJustica, this.getHeaders());
    ...
    public async addAndUpdate(regAreasJustica: IAreasJustica): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAreasJustica as AreasJustica, this.getHeaders());
            var register = result.data as IAreasJustica;        
            const action = regAreasJustica.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Areas Justica');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AreasJustica. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AreasJustica',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Areas Justica com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAreasJustica) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AreasJustica[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de areasjustica:', error);
            this.notificationService.notify({
              entity: 'AreasJustica',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos areasjustica'
            });
          }
        });
    }
