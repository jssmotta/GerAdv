...
#pragma warning restore CA1822 // Mark members as static
    ...
    public static ImmutableArray<DBInfoSystem> List => [DBAreasJusticaDicInfo.ArjArea, DBAreasJusticaDicInfo.ArjJustica];
    public static ImmutableArray<DBInfoSystem> ListWithoutAuditor => [DBAreasJusticaDicInfo.ArjArea, DBAreasJusticaDicInfo.ArjJustica];

    public static ImmutableArray<DBInfoSystem> ListPk()
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["arjCodigo"]);
    ...
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["arjArea", "arjCodigo", "arjJustica"]);
        var result = campos.Where(campo => !campo.Equals(DBAreasJusticaDicInfo.CampoCodigo)).Select(campo => List.FirstOrDefault(t => t.FNome == campo)).Where(item => item != null).Cast<DBInfoSystem>().Distinct().ToList();
        return result.Count > 0 ? [..result] : ImmutableArray<DBInfoSystem>.Empty;
    }
}
