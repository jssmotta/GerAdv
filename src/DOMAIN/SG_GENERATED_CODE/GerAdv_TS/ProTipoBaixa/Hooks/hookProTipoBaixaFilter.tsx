...
import { FilterProTipoBaixa } from '@/app/GerAdv_TS/ProTipoBaixa/Filters/ProTipoBaixa';
...
interface UseProTipoBaixaFilterProps {
    handleFetchWithFilter: (filtro?: FilterProTipoBaixa | undefined | null) => Promise<void>;
}

interface UseProTipoBaixaFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterProTipoBaixa;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterProTipoBaixa>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterProTipoBaixa) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterProTipoBaixa>) => React.ReactNode;
...
export const useProTipoBaixaFilter = ({ handleFetchWithFilter }: UseProTipoBaixaFilterProps): UseProTipoBaixaFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterProTipoBaixa>({} as FilterProTipoBaixa);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterProTipoBaixa;
    ...
    const handleConfirmSearch = async (filter: FilterProTipoBaixa) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterProTipoBaixa);
        sessionStorage.removeItem(btoa('ProTipoBaixaFilter'));
        handleFetchWithFilter({} as FilterProTipoBaixa);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterProTipoBaixa>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
