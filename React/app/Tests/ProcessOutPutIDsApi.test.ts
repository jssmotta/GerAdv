...
import { ProcessOutPutIDsApiError } from '../GerAdv_TS/ProcessOutPutIDs/Apis/ApiProcessOutPutIDs';
import { ProcessOutPutIDsApi } from '@/app/GerAdv_TS/ProcessOutPutIDs/Apis/ApiProcessOutPutIDs';
import { FilterProcessOutPutIDs } from '../GerAdv_TS/ProcessOutPutIDs/Filters/ProcessOutPutIDs';
...
describe('ProcessOutPutIDsApi', () => {
  let processoutputidsApi: ProcessOutPutIDsApi;
  ...
  beforeEach(() => {
    ...
    processoutputidsApi = new ProcessOutPutIDsApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processoutputidsApi).toBeInstanceOf(ProcessOutPutIDsApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessOutPutIDsApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessOutPutIDsApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ProcessOutPutIDsTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all processoutputids items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputidsApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processoutputidsApi.getAll()).rejects.toThrow(ProcessOutPutIDsApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processoutputidsApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProcessOutPutIDs_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutPutIDs',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw ProcessOutPutIDsApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessOutPutIDs = ProcessOutPutIDsTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProcessOutPutIDs,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch processoutputids by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputidsApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessOutPutIDs));
      ...
      expect(result.data).toEqual(mockProcessOutPutIDs);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutPutIDs',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProcessOutPutIDs = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter processoutputids items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processoutputidsApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProcessOutPutIDs: IProcessOutPutIDs = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new processoutputids successfully', async () => {
      const responseProcessOutPutIDs = { ...mockProcessOutPutIDs, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessOutPutIDs,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputidsApi.addAndUpdate(mockProcessOutPutIDs);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockProcessOutPutIDs,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing processoutputids successfully', async () => {
      const existingProcessOutPutIDs = { ...mockProcessOutPutIDs, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessOutPutIDs,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputidsApi.addAndUpdate(existingProcessOutPutIDs);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processoutputidsApi.addAndUpdate(mockProcessOutPutIDs)).rejects.toThrow(ProcessOutPutIDsApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutPutIDs',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processoutputids successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processoutputidsApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processoutputidsApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutPutIDs',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processoutputidsApi.delete(1)).rejects.toThrow(ProcessOutPutIDsApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProcessOutPutIDs = { funcionario: 1 };
      
      const result = processoutputidsApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await processoutputidsApi.delete(1);
  ...
  describe('ProcessOutPutIDsApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessOutPutIDsApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessOutPutIDsApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessOutPutIDsApiError('Test message', 500);
