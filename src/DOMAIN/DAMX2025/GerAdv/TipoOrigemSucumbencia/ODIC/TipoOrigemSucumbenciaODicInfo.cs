...
#pragma warning restore CA1822 // Mark members as static
    ...
    public static ImmutableArray<DBInfoSystem> List => [DBTipoOrigemSucumbenciaDicInfo.TosNome];
    public static ImmutableArray<DBInfoSystem> ListWithoutAuditor => [DBTipoOrigemSucumbenciaDicInfo.TosNome];

    public static ImmutableArray<DBInfoSystem> ListPk()
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["tosNome"]);
    ...
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["tosCodigo", "tosNome"]);
        var result = campos.Where(campo => !campo.Equals(DBTipoOrigemSucumbenciaDicInfo.CampoCodigo)).Select(campo => List.FirstOrDefault(t => t.FNome == campo)).Where(item => item != null).Cast<DBInfoSystem>().Distinct().ToList();
        return result.Count > 0 ? [..result] : ImmutableArray<DBInfoSystem>.Empty;
    }
}
