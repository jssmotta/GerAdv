...
export const useGUTTipoList = (dataService: IGUTTipoService, currentFilter?: FilterGUTTipo | undefined | null) => {
  const [data, setData] = useState<IGUTTipo[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterGUTTipo | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterGUTTipo | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterGUTTipo);
      setData(result);
    } catch (err) {
