...
public partial class ContratosController(IContratosService contratosService) : ControllerBase
{
    ...
    {
        _logger.Info("Contratos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _contratosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Contratos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _contratosService.UpdateColumns(parameters, uri);
        if (!result)
        {
