...
export class NECompromissosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'NECompromissosApiError';
    this.status = status;
    this.code = code;
...
export class NECompromissosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/NECompromissos`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NECompromissos_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os NECompromissos');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NECompromissos_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar N E Compromissos com ID ${id}`);
        }
    }

            public async filter(filtro: FilterNECompromissos): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NECompromissos_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar NECompromissos');
                }
            }


    public async validation(regNECompromissos: INECompromissos): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regNECompromissos as NECompromissos, this.getHeaders());
    ...
    public async addAndUpdate(regNECompromissos: INECompromissos): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar N E Compromissos');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o NECompromissos. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'NECompromissos',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir N E Compromissos com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterNECompromissos) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<NECompromissos[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de necompromissos:', error);
            this.notificationService.notify({
              entity: 'NECompromissos',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos necompromissos'
            });
          }
        });
    }
