...
const EventoPrazoAgendaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IEventoPrazoAgenda | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IEventoPrazoAgenda | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IEventoPrazoAgenda);
    ...
    const handleAddClick = () => {
        ...
        const add = EventoPrazoAgendaEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <EventoPrazoAgendaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedEventoPrazoAgenda={editar ?? addValue ?? EventoPrazoAgendaEmpty()} /> 
            )}
