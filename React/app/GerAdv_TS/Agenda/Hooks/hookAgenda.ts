  ...
  const loadAgenda = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Compromisso";
...
export const useAgendaList = (
  dataService: IAgendaService,
  currentFilter?: FilterAgenda | undefined | null,
) => {
  const [data, setData] = useState<IAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterAgenda | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterAgenda | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterAgenda));
        setData(result);
      } catch (err) {
        const errorMessage =
  ...
  function validate(data: IAgenda): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.decisao.length > 2048) {
        return {
          isValid: false,
          message: "O campo Decisao não pode ter mais de 2048 caracteres.",
        };
      }
