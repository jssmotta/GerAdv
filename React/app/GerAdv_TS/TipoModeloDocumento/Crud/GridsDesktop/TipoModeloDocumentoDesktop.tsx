  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("tipomodelodocumento-columns-order"),
        );
        if (savedOrder) {
    ...
    const SearchFromCellModelosDocumentos = (props: any) => {
      return (
        <>
          <td>
            <div
              onClick={() => openSearchCellModelosDocumentos(props.dataItem.id)}
            >
              <span title="Pesquisar relacionados em Modelos Documentos">
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        id_edit_ModelosDocumentos: (
          <GridColumn
            key="ModelosDocumentos"
            field="ModelosDocumentos"
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("tipomodelodocumento-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-tipomodelodocumento",
      columns: exportColumns,
      sheetName: "TipoModeloDocumento",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-TipoModeloDocumento-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-tipomodelodocumento",
      columns: exportColumnsPdf,
      title: "TipoModeloDocumento",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-TipoModeloDocumento-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`tipomodelodocumento-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-tipomodelodocumento"
