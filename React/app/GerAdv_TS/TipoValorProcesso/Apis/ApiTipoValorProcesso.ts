...
export class TipoValorProcessoApi {
 ...
 public async filterPreload(max: number, filtro: FilterTipoValorProcesso): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterTipoValorProcesso): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterTipoValorProcesso) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<TipoValorProcesso[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de TipoValorProcesso');
            this.notificationService.notify({
              entity: 'TipoValorProcesso',
              id: 0,
              action: NotifySystemActions.ERROR,
