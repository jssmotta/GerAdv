...
import { TiposAcaoApiError } from '../GerAdv_TS/TiposAcao/Apis/ApiTiposAcao';
import { TiposAcaoApi } from '@/app/GerAdv_TS/TiposAcao/Apis/ApiTiposAcao';
import { FilterTiposAcao } from '../GerAdv_TS/TiposAcao/Filters/TiposAcao';
...
describe('TiposAcaoApi', () => {
  let tiposacaoApi: TiposAcaoApi;
  ...
  beforeEach(() => {
    ...
    tiposacaoApi = new TiposAcaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tiposacaoApi).toBeInstanceOf(TiposAcaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TiposAcaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TiposAcaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tiposacao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tiposacaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tiposacaoApi.getAll()).rejects.toThrow(TiposAcaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tiposacaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TiposAcao_last_getAll_${200}`)));
    });
    ...
    it('should throw TiposAcaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTiposAcao = TiposAcaoTestEmpty();
    ...
    it('should fetch tiposacao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tiposacaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTiposAcao));
      ...
      expect(result.data).toEqual(mockTiposAcao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TiposAcao',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTiposAcao = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tiposacao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tiposacaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTiposAcao: ITiposAcao = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tiposacao successfully', async () => {
      const responseTiposAcao = { ...mockTiposAcao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTiposAcao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tiposacaoApi.addAndUpdate(mockTiposAcao);
    ...
    it('should update existing tiposacao successfully', async () => {
      const existingTiposAcao = { ...mockTiposAcao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTiposAcao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tiposacaoApi.addAndUpdate(existingTiposAcao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tiposacaoApi.addAndUpdate(mockTiposAcao)).rejects.toThrow(TiposAcaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TiposAcao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tiposacao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tiposacaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tiposacaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TiposAcao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tiposacaoApi.delete(1)).rejects.toThrow(TiposAcaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTiposAcao = { funcionario: 1 };
      
      const result = tiposacaoApi.useFilter(mockFilter);
  ...
  describe('TiposAcaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TiposAcaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TiposAcaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TiposAcaoApiError('Test message', 500);
