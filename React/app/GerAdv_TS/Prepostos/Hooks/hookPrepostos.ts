...
export const usePrepostosForm = (
  ...
  const loadPrepostos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Prepostos";
...
export const usePrepostosList = (
  dataService: IPrepostosService,
  currentFilter?: FilterPrepostos | undefined | null,
) => {
  const [data, setData] = useState<IPrepostos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPrepostos | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IPrepostos): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.registro.length > 30) {
        return {
          isValid: false,
          message: "O campo Registro não pode ter mais de 30 caracteres.",
        };
      }
      ...
      if (data.pai.length > 50) {
        return {
          isValid: false,
          message: "O campo Pai não pode ter mais de 50 caracteres.",
        };
      }
      if (data.mae.length > 50) {
        return {
          isValid: false,
          message: "O campo Mae não pode ter mais de 50 caracteres.",
        };
      }
