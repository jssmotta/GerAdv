...
export class ClientesSociosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ClientesSociosApiError';
    this.status = status;
    this.code = code;
...
export class ClientesSociosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ClientesSocios`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ClientesSocios_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ClientesSocios');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ClientesSocios_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Clientes Socios com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ClientesSocios_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ClientesSocios');
        }
    }

            public async filter(filtro: FilterClientesSocios): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ClientesSocios_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ClientesSocios');
                }
            }


    public async validation(regClientesSocios: IClientesSocios): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regClientesSocios as ClientesSocios, this.getHeaders());
    ...
    public async addAndUpdate(regClientesSocios: IClientesSocios): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Clientes Socios');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ClientesSocios. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ClientesSocios',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Clientes Socios com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterClientesSocios) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ClientesSocios[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de clientessocios:', error);
            this.notificationService.notify({
              entity: 'ClientesSocios',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos clientessocios'
            });
          }
        });
    }
