...
#region ListarDados_ViaRecebimento
    public static async IAsyncEnumerable<DBViaRecebimento> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBViaRecebimento(dbRec);
        }
    }

    public static IEnumerable<DBViaRecebimento> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
