// app/TestsForms/CargosComboBox.test.tsx - Vers�o Corrigida
...
import { CargosEmpty, CargosTestEmpty } from '@/app/GerAdv_TS/Models/Cargos';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Cargos/Hooks/hookCargos', () => ({
  useCargosComboBox: jest.fn(() => ({
    options: [
    { ...CargosEmpty(), id: 1, nome: 'Cargo 1' },
    { ...CargosEmpty(), id: 2, nome: 'Cargo 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Cargos/Apis/ApiCargos', () => ({
  CargosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cargos' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cargos' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Cargos/Services/Cargos.service', () => ({
  CargosService: jest.fn().mockImplementation(() => ({
    fetchCargosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cargos' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Cargos/Crud/Grids/CargosWindow', () => {
  return function MockCargosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='cargos-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Cargo' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...CargosTestEmpty(), id: 1, nome: 'Cargo 1' },
  { ...CargosTestEmpty(), id: 2, nome: 'Cargo 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: CargosEmpty(), 
};
describe('CargosComboBox', () => {
  ...
  const { useCargosComboBox } = require('@/app/GerAdv_TS/Cargos/Hooks/hookCargos');
  (useCargosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<CargosComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Cargo')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...CargosTestEmpty(), id: 1, nome: 'Cargo 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useCargosComboBox } = require('@/app/GerAdv_TS/Cargos/Hooks/hookCargos');
    useCargosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens CargosWindow on action button click (add)', async () => {
    render(<CargosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('cargos-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('cargos-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<CargosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles CargosWindow onClose and onSuccess', () => {
    render(<CargosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<CargosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
