...
import { ContatoCRMApiError } from '../GerAdv_TS/ContatoCRM/Apis/ApiContatoCRM';
import { ContatoCRMApi } from '@/app/GerAdv_TS/ContatoCRM/Apis/ApiContatoCRM';
import { FilterContatoCRM } from '../GerAdv_TS/ContatoCRM/Filters/ContatoCRM';
...
describe('ContatoCRMApi', () => {
  let contatocrmApi: ContatoCRMApi;
  ...
  beforeEach(() => {
    ...
    contatocrmApi = new ContatoCRMApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(contatocrmApi).toBeInstanceOf(ContatoCRMApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ContatoCRMApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ContatoCRMApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all contatocrm items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(contatocrmApi.getAll()).rejects.toThrow(ContatoCRMApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await contatocrmApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ContatoCRM_last_getAll_${200}`)));
    });
    ...
    it('should throw ContatoCRMApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockContatoCRM = ContatoCRMTestEmpty();
    ...
    it('should fetch contatocrm by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockContatoCRM));
      ...
      expect(result.data).toEqual(mockContatoCRM);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRM',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterContatoCRM = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter contatocrm items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await contatocrmApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockContatoCRM: IContatoCRM = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new contatocrm successfully', async () => {
      const responseContatoCRM = { ...mockContatoCRM, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseContatoCRM,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmApi.addAndUpdate(mockContatoCRM);
    ...
    it('should update existing contatocrm successfully', async () => {
      const existingContatoCRM = { ...mockContatoCRM, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingContatoCRM,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmApi.addAndUpdate(existingContatoCRM);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(contatocrmApi.addAndUpdate(mockContatoCRM)).rejects.toThrow(ContatoCRMApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRM',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete contatocrm successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await contatocrmApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(contatocrmApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRM',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(contatocrmApi.delete(1)).rejects.toThrow(ContatoCRMApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterContatoCRM = { funcionario: 1 };
      
      const result = contatocrmApi.useFilter(mockFilter);
  ...
  describe('ContatoCRMApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ContatoCRMApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ContatoCRMApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ContatoCRMApiError('Test message', 500);
