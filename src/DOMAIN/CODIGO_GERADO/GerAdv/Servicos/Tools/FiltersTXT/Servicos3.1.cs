...
public static class FilterServicosTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterServicos? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Servicos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterServicos:

- O filter irá filtrar os Servicos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Servicos" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Servicos length: {result.Length}");
#endif
