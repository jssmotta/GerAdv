...
public partial class FAtividades : MenphisSI.SG.GerAdv.DBAtividades, IDBAtividades
{
    // DI-compatible default constructor
    public FAtividades() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FAtividades CreateFromReader(SqlDataReader? dbRec)
    {
        return new FAtividades(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FAtividades CreateFromDataRow(DataRow? dbRec)
    {
        return new FAtividades(dbRec);
    }

    private FAtividades(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FAtividades(DataRow? dbRec) : base(dbRec)
    {
    }

    private FAtividades(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FAtividades CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FAtividades(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FAtividades> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fAtividades = new FAtividades();
        await fAtividades.CarregarAsync(id, oCnn);
        return fAtividades;
    }
