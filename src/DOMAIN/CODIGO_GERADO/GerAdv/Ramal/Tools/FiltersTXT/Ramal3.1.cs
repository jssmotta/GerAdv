...
public static class FilterRamalTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Ramal em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterRamal:

- O filter irá filtrar os Ramal conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Ramal" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterRamal? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Ramal length: {result.Length}");
#endif
