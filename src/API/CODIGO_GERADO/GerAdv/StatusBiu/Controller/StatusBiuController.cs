...
public partial class StatusBiuController(IStatusBiuService statusbiuService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.StatusBiu regStatusBiu, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "StatusBiu: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "StatusBiu: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.StatusBiu regStatusBiu, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _statusbiuService.Validation(regStatusBiu, uri);
            if (result == null)
            {
                _logger.Warn("StatusBiu: Validation failed to add or update StatusBiu, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "StatusBiu: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}