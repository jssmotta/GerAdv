...
public partial class DBTipoModeloDocumento
{
    ...
    /// </summary>
    public static async Task<DBTipoModeloDocumento> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBTipoModeloDocumento();
...
#if (!NOTSTORED_TipoModeloDocumento)
    /// <summary>
    ...
    {
        if (pFldFNome)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFGuid)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBTipoModeloDocumentoDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBTipoModeloDocumentoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
