...
public partial class TribunalController(ITribunalService tribunalService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterTribunal filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Tribunal: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _tribunalService.Filter(max, filtro, uri);
        return Ok(result);
    }
