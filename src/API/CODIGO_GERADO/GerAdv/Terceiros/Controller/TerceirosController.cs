...
public partial class TerceirosController(ITerceirosService terceirosService) : ControllerBase
{
    ...
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] MenphisSI.GerAdv.Filters.FilterTerceiros filter, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Terceiros: Filter called max {0} with filtro = {1}, {2}", max, filter, uri);
        var result = await _terceirosService.Filter(max, filter, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> FilterVoice([FromBody] MenphisSI.GerAdv.Filters.FilterTerceirosWithVoiceRequest request, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Terceiros: Filter called with {0} filtro = {1}", request, uri);
        var result = await _terceirosService.FilterVoice(request.Filter, request.VoiceCommand, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetAuditor(int id, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        try
        {
            //_logger.Info("Terceiros: GetAuditor called with id = {0}, {1}", id, uri);
            var result = await _terceirosService.GetAuditor(id, uri, token);
            if (result == null)
            {
