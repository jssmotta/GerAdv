...
import { useDadosProcuracaoGrid } from '../../Hooks/hookDadosProcuracaoGrid';
...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('dadosprocuracao-columns-order-mobile'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('dadosprocuracao-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('dadosprocuracao-columns-order-mobile'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-dadosprocuracao',
  columns: exportColumns, 
  sheetName: 'DadosProcuracao'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-DadosProcuracao-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-dadosprocuracao',
  columns: exportColumnsPdf, 
  title: 'DadosProcuracao'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-DadosProcuracao-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  className='grid-mobile-crud grid-mobile-dadosprocuracao'
  key={`dadosprocuracao-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
