...
const OperadorEMailPopupComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, nome: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as OperadorEMailPopup;
        const novoDado = await dadoApi.addAndUpdate(nova as IOperadorEMailPopup);
