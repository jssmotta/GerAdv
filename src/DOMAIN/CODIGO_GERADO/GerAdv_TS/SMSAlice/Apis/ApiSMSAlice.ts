...
import { SMSAlice } from '../../Models/SMSAlice';
...
export class SMSAliceApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class SMSAliceApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/SMSAlice`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-SMSAlice_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os SMSAlice');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-SMSAlice_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar S M S Alice com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterSMSAlice): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-SMSAlice_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de SMSAlice');
        }
    }

            public async filter(filtro: FilterSMSAlice): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-SMSAlice_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar SMSAlice');
                }
            }


    public async validation(regSMSAlice: ISMSAlice): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regSMSAlice as SMSAlice, this.getHeaders());
    ...
    public async addAndUpdate(regSMSAlice: ISMSAlice): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regSMSAlice as SMSAlice, this.getHeaders());
            var register = result.data as ISMSAlice;        
            const action = regSMSAlice.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar S M S Alice');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o SMSAlice. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'SMSAlice',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir S M S Alice com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterSMSAlice) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<SMSAlice[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de smsalice:', error);
            this.notificationService.notify({
              entity: 'SMSAlice',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos smsalice'
            });
          }
        });
    }
