...
export const useTipoOrigemSucumbenciaList = (dataService: ITipoOrigemSucumbenciaService, currentFilter?: FilterTipoOrigemSucumbencia | undefined | null) => {
  const [data, setData] = useState<ITipoOrigemSucumbencia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoOrigemSucumbencia | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoOrigemSucumbencia | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoOrigemSucumbencia);
      setData(result);
    } catch (err) {
