...
const PosicaoOutrasPartesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, descricao: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as PosicaoOutrasPartes;
        const novoDado = await dadoApi.addAndUpdate(nova as IPosicaoOutrasPartes);
