...
export class HonorariosDadosContratoApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HonorariosDadosContrato_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os HonorariosDadosContrato');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HonorariosDadosContrato_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Honorarios Dados Contrato com ID ${id}`);
        }
    }

            public async filter(filtro: FilterHonorariosDadosContrato): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HonorariosDadosContrato_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar HonorariosDadosContrato');
                }
            }
    ...
    public async validation(regHonorariosDadosContrato: IHonorariosDadosContrato): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regHonorariosDadosContrato as HonorariosDadosContrato, this.getHeaders());
    ...
    public useFilter(filtro: FilterHonorariosDadosContrato) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<HonorariosDadosContrato[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de honorariosdadoscontrato:', error);
            this.notificationService.notify({
              entity: 'HonorariosDadosContrato',
              id: 0,
              action: NotifySystemActions.ERROR,
