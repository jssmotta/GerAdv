...
export const EnquadramentoEmpresaGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('enquadramentoempresa-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('enquadramentoempresa-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-enquadramentoempresa .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
...
const isColumnVisible = useCallback((field: string) => {
  ...
  const resetColumnsOrder = useCallback(() => {
    ...
    localStorage.removeItem(btoa('enquadramentoempresa-columns-order-mobile'));
  }, []);
