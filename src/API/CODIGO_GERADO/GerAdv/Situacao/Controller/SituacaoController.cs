...
public partial class SituacaoController(ISituacaoService situacaoService) : ControllerBase
{
    ...
    {
        _logger.Info("Situacao: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _situacaoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Situacao: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _situacaoService.UpdateColumns(parameters, uri);
        if (!result)
        {
