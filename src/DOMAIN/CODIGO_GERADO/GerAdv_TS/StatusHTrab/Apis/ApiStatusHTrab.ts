...
import { StatusHTrab } from '../../Models/StatusHTrab';
...
export class StatusHTrabApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class StatusHTrabApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/StatusHTrab`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusHTrab_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os StatusHTrab');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusHTrab_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Status H Trab com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterStatusHTrab): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusHTrab_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de StatusHTrab');
        }
    }

            public async filter(filtro: FilterStatusHTrab): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusHTrab_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar StatusHTrab');
                }
            }


    public async validation(regStatusHTrab: IStatusHTrab): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regStatusHTrab as StatusHTrab, this.getHeaders());
    ...
    public async addAndUpdate(regStatusHTrab: IStatusHTrab): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regStatusHTrab as StatusHTrab, this.getHeaders());
            var register = result.data as IStatusHTrab;        
            const action = regStatusHTrab.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Status H Trab');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o StatusHTrab. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'StatusHTrab',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Status H Trab com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterStatusHTrab) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<StatusHTrab[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de statushtrab:', error);
            this.notificationService.notify({
              entity: 'StatusHTrab',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos statushtrab'
            });
          }
        });
    }
