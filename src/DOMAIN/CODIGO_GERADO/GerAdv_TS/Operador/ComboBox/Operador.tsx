...
const OperadorComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOperador | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOperador | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOperador);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OperadorWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOperador={editar ?? addValue ?? OperadorEmpty()} /> 
            )}
