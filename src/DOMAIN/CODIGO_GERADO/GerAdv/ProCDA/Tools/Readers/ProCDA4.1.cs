...
public partial class ProCDAReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBProCDADicInfo.CampoNome}";
        ...
        var query = $@"SELECT TOP ({max})
                   {campos}
                   FROM {DBProCDA.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(pcdDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(pcdDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    pcdVisto as Visto
FROM {oCnn.UseDbo}.ProCDA dbq WITH (INDEX = idx_ProCDA_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.pcdQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.pcdQuemAtu
WHERE [pcdCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBProCDA.PTabelaNome,
...
FROM {oCnn.UseDbo}.ProCDA WITH (INDEX = idx_{DBProCDADicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [pcdCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBProCDADicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(pcdDtAtu, pcdDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBProCDADicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(pcdDtAtu, pcdDtCad) DESC;", oCnn.InnerConnection);
