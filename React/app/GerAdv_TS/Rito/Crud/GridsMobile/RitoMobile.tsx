...
import { useRitoGrid } from "../../Hooks/hookRitoGrid";
...
import RitoHelpCrudButton from "../../Help/RitoHelpCrudButton";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("rito-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector: ".grid-mobile-crud.grid-mobile-rito .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Rito-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Rito-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            <PageTitle
              title="Rito"
              helpButton={<RitoHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-rito"
                      key={`rito-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IRito);
