...
public partial class ApensoService(IOptions<AppSettings> appSettings, IFApensoFactory apensoFactory, IApensoReader reader, IApensoValidation validation, IApensoWriter writer, IProcessosReader processosReader, IHistoricoService historicoService, INENotasService nenotasService, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IApensoService, IDisposable
{
    ...
    private readonly IFApensoFactory apensoFactory = apensoFactory;
    private readonly IApensoReader reader = reader;
    private readonly IApensoValidation validation = validation;
    private readonly IApensoWriter writer = writer;
    ...
    public async Task<IEnumerable<ApensoResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-Apenso-Filter-{keyCache}";
    ...
    private async Task<ApensoResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<ApensoResponse?> AddAndUpdate([FromBody] Models.Apenso regApenso, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regApenso, operadorId, oCnn);
    ...
    public async Task<ApensoResponse?> Validation([FromBody] Models.Apenso regApenso, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regApenso.Id, oCnn);
    }
    ...
    public async Task<ApensoResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var apenso = await reader.Read(id, oCnn);
        try
        {
            if (apenso != null)
            {
                await writer.Delete(apenso, nOperador, oCnn);
