...
public partial class DBStatusTarefas
{
    ...
    public DBStatusTarefas(SqlDataReader? dbRec)
    {
        ...
        {
            throw new Exception($"Erro ao carregar dados do StatusTarefas: {ex.Message}", ex);
        }
    }
    ...
    private void InitFromRecord(Func<string, object?> getValue)
    {
        ...
        try
        {
            if (!DBNull.Value.Equals(getValue(DBStatusTarefasDicInfo.DtCad)))
                m_FDtCad = Convert.ToDateTime(getValue(DBStatusTarefasDicInfo.DtCad));
        }
        catch
        {
        }
