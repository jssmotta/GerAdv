...
public partial class FStatusTarefas : MenphisSI.SG.GerAdv.DBStatusTarefas, IDBStatusTarefas
{
    // DI-compatible default constructor
    public FStatusTarefas() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FStatusTarefas CreateFromReader(SqlDataReader? dbRec)
    {
        return new FStatusTarefas(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FStatusTarefas CreateFromDataRow(DataRow? dbRec)
    {
        return new FStatusTarefas(dbRec);
    }

    private FStatusTarefas(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FStatusTarefas(DataRow? dbRec) : base(dbRec)
    {
    }

    private FStatusTarefas(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FStatusTarefas CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FStatusTarefas(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FStatusTarefas> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fStatusTarefas = new FStatusTarefas();
        await fStatusTarefas.CarregarAsync(id, oCnn);
        return fStatusTarefas;
    }
