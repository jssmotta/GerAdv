...
import { useRegimeTributacaoComboBox } from '../Hooks/hookRegimeTributacao';
const RegimeTributacaoComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useRegimeTributacaoComboBox(regimetributacaoService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IRegimeTributacao | null>(null);
const [addRecord, setAddRecord] = useState<IRegimeTributacao | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await regimetributacaoService.fetchRegimeTributacaoById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = RegimeTributacaoEmpty();
...
const handleSuccess = (newRegimeTributacao?: IRegimeTributacao) => {
  if (newRegimeTributacao) {
    setValue({ id: newRegimeTributacao.id, nome: newRegimeTributacao.nome });
    loadRecordForEdit(newRegimeTributacao.id);
    handleValueChange(newRegimeTributacao);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .regimetributacao-dropdown-popup-msi {
  ...
  .regimetributacao-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .regimetributacao-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .regimetributacao-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedRegimeTributacao={editRecord || addRecord || RegimeTributacaoEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'regimetributacao-dropdown-popup-msi'
          }}
          />
