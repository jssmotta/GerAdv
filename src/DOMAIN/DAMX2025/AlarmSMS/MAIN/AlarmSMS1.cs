...
#region TableDefinition_AlarmSMS
...
#endregion
    public DBAlarmSMS(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBAlarmSMS(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBAlarmSMS(in string? sqlWhere, SqlConnection? oCnn)
    {
