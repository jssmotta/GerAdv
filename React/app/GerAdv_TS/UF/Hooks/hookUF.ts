...
export const useUFForm = (initialUF: IUF, dataService: IUFService) => {
  ...
  const loadUF = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar UF";
...
export const useUFList = (
  dataService: IUFService,
  currentFilter?: FilterUF | undefined | null,
) => {
  ...
  function validate(data: IUF): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.ddd.length > 10) {
        return {
          isValid: false,
          message: "O campo DDD não pode ter mais de 10 caracteres.",
        };
      }
      if (data.iduf.length > 4) {
        return {
          isValid: false,
          message: "O campo ID não pode ter mais de 4 caracteres.",
        };
      }
...
export const useUFComboBox = (dataService: IUFService, initialValue?: any) => {
