...
public partial class TipoEMailController(ITipoEMailService tipoemailService) : ControllerBase
{
    ...
    {
        _logger.Info("TipoEMail: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _tipoemailService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("TipoEMail: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _tipoemailService.UpdateColumns(parameters, uri);
        if (!result)
        {
