...
public partial class PoderJudiciarioAssociadoController(IPoderJudiciarioAssociadoService poderjudiciarioassociadoService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterPoderJudiciarioAssociado filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("PoderJudiciarioAssociado: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _poderjudiciarioassociadoService.Filter(max, filtro, uri);
        return Ok(result);
    }
