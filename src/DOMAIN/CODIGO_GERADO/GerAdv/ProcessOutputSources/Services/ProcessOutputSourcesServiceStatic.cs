...
public partial class ProcessOutputSourcesService
{
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterProcessOutputSources filtro)
    {
        ...
        cWhere.Append(filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutputSourcesDicInfo.PTabelaNome}].[{DBProcessOutputSourcesDicInfo.Nome}]  {DevourerConsts.MsiCollate} like @{nameof(DBProcessOutputSourcesDicInfo.Nome)}");
        cWhere.Append(filtro.GUID.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutputSourcesDicInfo.PTabelaNome}].[{DBProcessOutputSourcesDicInfo.GUID}]  {DevourerConsts.MsiCollate} like @{nameof(DBProcessOutputSourcesDicInfo.GUID)}");
        if (!filtro.Codigo_filtro.IsEmpty() && filtro.Codigo_filtro_end.IsEmpty())
        {
            cWhere.Append(filtro.Codigo_filtro <= 0 ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutputSourcesDicInfo.PTabelaNome}].[{DBProcessOutputSourcesDicInfo.CampoCodigo}] >= @{nameof(DBProcessOutputSourcesDicInfo.CampoCodigo)}");
        }
        else
        {
            cWhere.Append((filtro.Codigo_filtro <= 0 && filtro.Codigo_filtro_end <= 0) ? string.Empty : (!(filtro.Codigo_filtro <= 0) && !(filtro.Codigo_filtro_end <= 0)) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutputSourcesDicInfo.CampoCodigo} BETWEEN @{nameof(DBProcessOutputSourcesDicInfo.CampoCodigo)} AND @{nameof(DBProcessOutputSourcesDicInfo.CampoCodigo)}_end" : !(filtro.Codigo_filtro <= 0) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutputSourcesDicInfo.CampoCodigo} = @{nameof(DBProcessOutputSourcesDicInfo.CampoCodigo)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutputSourcesDicInfo.CampoCodigo} <= @{nameof(DBProcessOutputSourcesDicInfo.CampoCodigo)}_end");
        }
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter> parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarN(max, uri, where, parameters, DBProcessOutputSourcesDicInfo.CampoNome);
