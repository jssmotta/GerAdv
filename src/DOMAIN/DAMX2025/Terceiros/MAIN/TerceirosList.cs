...
#region ListarDados_Terceiros
    public static async IAsyncEnumerable<DBTerceiros> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTerceiros(dbRec);
        }
    }

    public static IEnumerable<DBTerceiros> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
