...
public partial class AgendaReader(IFAgendaFactory agendaFactory) : IAgendaReader
{
    ...
    public async Task<IEnumerable<AgendaResponseAll>> ListarAsync(MsiSqlConnection oCnn, int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = await ListarTabelaAsync(oCnn, BuildSqlQuery(DBAgenda.CamposSqlX, cWhere, order, max), parameters, uri, caching: false, max: max, cancellationToken: cancellationToken);
    ...
    private async Task<IEnumerable<AgendaResponseAll>> ListarTabelaAsync(MsiSqlConnection? oCnn, string sql, List<SqlParameter>? parameters, string uri, bool caching = false, int max = 200, CancellationToken cancellationToken = default)
    {
        var stopwatch = AgendaDatabaseMetrics.StartTimer();
