...
export const useOutrasPartesClienteForm = (
  ...
  const loadOutrasPartesCliente = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Outras Partes Cliente";
...
export const useOutrasPartesClienteList = (
  dataService: IOutrasPartesClienteService,
  currentFilter?: FilterOutrasPartesCliente | undefined | null,
) => {
  const [data, setData] = useState<IOutrasPartesCliente[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOutrasPartesCliente | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterOutrasPartesCliente | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar outraspartescliente";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsOutrasPartesCliente() {
  async function runValidation(
    ...
    const outraspartesclienteApi = new OutrasPartesClienteApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IOutrasPartesCliente): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.nomefantasia.length > 255) {
        return {
          isValid: false,
          message: "O campo NomeFantasia não pode ter mais de 255 caracteres.",
        };
      }
