...
public static class FilterProDespesasTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterProDespesas? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar ProDespesas em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProDespesas:

- O filter irá filtrar os ProDespesas conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProDespesas:(Pro Despesas)
- ligacaoid: número ou IDs;
- cliente: número ou IDs;
- corrigido: Corrigido;
- data: não é a data de cadastramento é a data do ProDespesas;
- valororiginal: valores monetários ou com decimais;
- processo: número ou IDs;
- quitado: número ou IDs;
...
- Se ouvir "todos os ProDespesas" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProDespesas length: {result.Length}");
#endif
