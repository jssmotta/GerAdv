...
import { useProcessOutPutIDsGrid } from "../../Hooks/hookProcessOutPutIDsGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("processoutputids-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-processoutputids .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("processoutputids-columns-order-mobile"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-processoutputids",
      columns: exportColumns,
      sheetName: "ProcessOutPutIDs",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-ProcessOutPutIDs-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-processoutputids",
      columns: exportColumnsPdf,
      title: "ProcessOutPutIDs",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-ProcessOutPutIDs-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-processoutputids"
              key={`processoutputids-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
