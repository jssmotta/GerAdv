...
const RitoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IRito | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IRito | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IRito);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <RitoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedRito={editar ?? addValue ?? RitoEmpty()} /> 
            )}
