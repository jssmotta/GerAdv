...
import { DocsRecebidosItens } from '../../Models/DocsRecebidosItens';
...
export class DocsRecebidosItensApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'DocsRecebidosItensApiError';
    this.status = status;
    this.code = code;
...
export class DocsRecebidosItensApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/DocsRecebidosItens`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DocsRecebidosItens_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os DocsRecebidosItens');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DocsRecebidosItens_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Docs Recebidos Itens com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterDocsRecebidosItens): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DocsRecebidosItens_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de DocsRecebidosItens');
        }
    }

            public async filter(filtro: FilterDocsRecebidosItens): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DocsRecebidosItens_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar DocsRecebidosItens');
                }
            }


    public async validation(regDocsRecebidosItens: IDocsRecebidosItens): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regDocsRecebidosItens as DocsRecebidosItens, this.getHeaders());
    ...
    public async addAndUpdate(regDocsRecebidosItens: IDocsRecebidosItens): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regDocsRecebidosItens as DocsRecebidosItens, this.getHeaders());
            var register = result.data as IDocsRecebidosItens;        
            const action = regDocsRecebidosItens.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Docs Recebidos Itens');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o DocsRecebidosItens. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'DocsRecebidosItens',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Docs Recebidos Itens com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterDocsRecebidosItens) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<DocsRecebidosItens[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de docsrecebidositens:', error);
            this.notificationService.notify({
              entity: 'DocsRecebidosItens',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos docsrecebidositens'
            });
          }
        });
    }
