...
import { ProSucumbencia } from '../../Models/ProSucumbencia';
...
export class ProSucumbenciaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ProSucumbenciaApiError';
    this.status = status;
    this.code = code;
...
export class ProSucumbenciaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProSucumbencia`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProSucumbencia_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProSucumbencia');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProSucumbencia_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Pro Sucumbencia com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterProSucumbencia): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProSucumbencia_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ProSucumbencia');
        }
    }

            public async filter(filtro: FilterProSucumbencia): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProSucumbencia_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProSucumbencia');
                }
            }


    public async validation(regProSucumbencia: IProSucumbencia): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProSucumbencia as ProSucumbencia, this.getHeaders());
    ...
    public async addAndUpdate(regProSucumbencia: IProSucumbencia): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProSucumbencia as ProSucumbencia, this.getHeaders());
            var register = result.data as IProSucumbencia;        
            const action = regProSucumbencia.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Pro Sucumbencia');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProSucumbencia. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProSucumbencia',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Pro Sucumbencia com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProSucumbencia) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProSucumbencia[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de prosucumbencia:', error);
            this.notificationService.notify({
              entity: 'ProSucumbencia',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos prosucumbencia'
            });
          }
        });
    }
