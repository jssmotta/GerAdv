...
[{DBServicosDicInfo.PTabelaNome}].[{DBServicosDicInfo.Descricao}]
                   ...
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBHorasTrabDicInfo.PTabelaNome}].[{DBHorasTrabDicInfo.Cliente}]
...
LEFT JOIN {DBAdvogadosDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.CampoCodigo}]=[{DBHorasTrabDicInfo.PTabelaNome}].[{DBHorasTrabDicInfo.Advogado}]
...
LEFT JOIN {DBFuncionariosDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBFuncionariosDicInfo.PTabelaNome}].[{DBFuncionariosDicInfo.CampoCodigo}]=[{DBHorasTrabDicInfo.PTabelaNome}].[{DBHorasTrabDicInfo.Funcionario}]
...
LEFT JOIN {DBServicosDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBServicosDicInfo.PTabelaNome}].[{DBServicosDicInfo.CampoCodigo}]=[{DBHorasTrabDicInfo.PTabelaNome}].[{DBHorasTrabDicInfo.Servico}]
...
FROM {oCnn.UseDbo}.HorasTrab WITH (INDEX = idx_{DBHorasTrabDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [htbCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBHorasTrabDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(htbDtAtu, htbDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBHorasTrabDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(htbDtAtu, htbDtCad) DESC;", oCnn.InnerConnection);
