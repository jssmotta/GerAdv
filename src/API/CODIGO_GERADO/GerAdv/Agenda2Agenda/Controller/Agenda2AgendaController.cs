...
public partial class Agenda2AgendaController(IAgenda2AgendaService agenda2agendaService) : ControllerBase
{
    ...
    {
        _logger.Info("Agenda2Agenda: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _agenda2agendaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Agenda2Agenda: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _agenda2agendaService.UpdateColumns(parameters, uri);
        if (!result)
        {
