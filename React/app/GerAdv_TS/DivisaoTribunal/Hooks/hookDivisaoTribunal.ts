...
export const useDivisaoTribunalForm = (
  ...
  const loadDivisaoTribunal = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Divisao Tribunal";
...
export const useDivisaoTribunalList = (
  dataService: IDivisaoTribunalService,
  currentFilter?: FilterDivisaoTribunal | undefined | null,
) => {
  const [data, setData] = useState<IDivisaoTribunal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterDivisaoTribunal | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterDivisaoTribunal | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar divisaotribunal";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsDivisaoTribunal() {
  ...
  function validate(data: IDivisaoTribunal): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      if (data.nomeespecial.length > 255) {
        return {
          isValid: false,
          message: "O campo NomeEspecial não pode ter mais de 255 caracteres.",
        };
      }
      if (data.codigodiv.length > 5) {
        return {
          isValid: false,
          message: "O campo CodigoDiv não pode ter mais de 5 caracteres.",
        };
      }
      if (data.endereco.length > 40) {
        return {
          isValid: false,
          message: "O campo Endereco não pode ter mais de 40 caracteres.",
        };
      }
      ...
      if (data.andar.length > 12) {
        return {
          isValid: false,
          message: "O campo Andar não pode ter mais de 12 caracteres.",
        };
      }
