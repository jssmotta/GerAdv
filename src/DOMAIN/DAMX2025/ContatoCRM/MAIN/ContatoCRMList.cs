...
#region ListarDados_ContatoCRM
    public static async IAsyncEnumerable<DBContatoCRM> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBContatoCRM(dbRec);
        }
    }

    public static IEnumerable<DBContatoCRM> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
