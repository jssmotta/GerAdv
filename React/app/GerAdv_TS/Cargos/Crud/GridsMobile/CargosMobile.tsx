...
import { useCargosGrid } from "../../Hooks/hookCargosGrid";
...
import CargosHelpCrudButton from "../../Help/CargosHelpCrudButton";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("cargos-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector: ".grid-mobile-crud.grid-mobile-cargos .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Cargos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Cargos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            <PageTitle
              title="Cargos"
              helpButton={<CargosHelpCrudButton isMobile={true} />}
            />
                ...
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      ...
                      key={`cargos-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as ICargos);
