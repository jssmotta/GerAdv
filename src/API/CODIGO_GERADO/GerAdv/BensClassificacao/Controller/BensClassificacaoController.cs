...
public partial class BensClassificacaoController(IBensClassificacaoService bensclassificacaoService) : ControllerBase
{
    ...
    {
        _logger.Info("BensClassificacao: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _bensclassificacaoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("BensClassificacao: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _bensclassificacaoService.UpdateColumns(parameters, uri);
        if (!result)
        {
