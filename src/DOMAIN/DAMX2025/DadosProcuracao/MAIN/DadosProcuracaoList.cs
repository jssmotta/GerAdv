...
#region ListarDados_DadosProcuracao
    public static async IAsyncEnumerable<DBDadosProcuracao> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBDadosProcuracao(dbRec);
        }
    }

    public static IEnumerable<DBDadosProcuracao> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
