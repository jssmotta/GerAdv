...
import { useAgendaRelatorioGrid } from "../../Hooks/hookAgendaRelatorioGrid";
...
import AgendaRelatorioHelpCrudButton from "../../Help/AgendaRelatorioHelpCrudButton";
...
export const AgendaRelatorioGridMobileComponent = React.memo(
  ({
    ...
    const [columnsOrder, setColumnsOrder] = useState<string[]>(["advnome"]);

    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("agendarelatorio-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-agendarelatorio .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-agendarelatorio",
      columns: exportColumns,
      sheetName: "AgendaRelatorio",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-AgendaRelatorio-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-agendarelatorio",
      columns: exportColumnsPdf,
      title: "AgendaRelatorio",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-AgendaRelatorio-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
        ...
        {initialized && (
          <>
            <PageTitle
              ...
              helpButton={<AgendaRelatorioHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-agendarelatorio"
                      key={`agendarelatorio-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IAgendaRelatorio);
