...
#region ListarDados_ProcessosObsReport
    public static async IAsyncEnumerable<DBProcessosObsReport> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProcessosObsReport(dbRec);
        }
    }

    public static IEnumerable<DBProcessosObsReport> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
