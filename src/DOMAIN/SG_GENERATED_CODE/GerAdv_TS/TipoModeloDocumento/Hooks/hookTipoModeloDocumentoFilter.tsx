...
import { FilterTipoModeloDocumento } from '@/app/GerAdv_TS/TipoModeloDocumento/Filters/TipoModeloDocumento';
...
interface UseTipoModeloDocumentoFilterProps {
    handleFetchWithFilter: (filtro?: FilterTipoModeloDocumento | undefined | null) => Promise<void>;
}

interface UseTipoModeloDocumentoFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterTipoModeloDocumento;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterTipoModeloDocumento>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterTipoModeloDocumento) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterTipoModeloDocumento>) => React.ReactNode;
...
export const useTipoModeloDocumentoFilter = ({ handleFetchWithFilter }: UseTipoModeloDocumentoFilterProps): UseTipoModeloDocumentoFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterTipoModeloDocumento>({} as FilterTipoModeloDocumento);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterTipoModeloDocumento;
    ...
    const handleConfirmSearch = async (filter: FilterTipoModeloDocumento) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterTipoModeloDocumento);
        sessionStorage.removeItem(btoa('TipoModeloDocumentoFilter'));
        handleFetchWithFilter({} as FilterTipoModeloDocumento);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterTipoModeloDocumento>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
