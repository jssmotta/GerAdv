...
describe('ProcessosObsReportApi', () => {
  let processosobsreportApi: ProcessosObsReportApi;
  ...
  beforeEach(() => {
    ...
    processosobsreportApi = new ProcessosObsReportApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processosobsreportApi).toBeInstanceOf(ProcessosObsReportApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessosObsReportApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessosObsReportApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ProcessosObsReportTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all processosobsreport items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosobsreportApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processosobsreportApi.getAll()).rejects.toThrow(ProcessosObsReportApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processosobsreportApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProcessosObsReport_last_getAll_${200}`)));
    });
    ...
    it('should throw ProcessosObsReportApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessosObsReport = ProcessosObsReportTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProcessosObsReport,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch processosobsreport by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosobsreportApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessosObsReport));
      ...
      expect(result.data).toEqual(mockProcessosObsReport);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosObsReport',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter processosobsreport items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processosobsreportApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new processosobsreport successfully', async () => {
      const responseProcessosObsReport = { ...mockProcessosObsReport, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessosObsReport,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosobsreportApi.addAndUpdate(mockProcessosObsReport);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockProcessosObsReport,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing processosobsreport successfully', async () => {
      const existingProcessosObsReport = { ...mockProcessosObsReport, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessosObsReport,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosobsreportApi.addAndUpdate(existingProcessosObsReport);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processosobsreportApi.addAndUpdate(mockProcessosObsReport)).rejects.toThrow(ProcessosObsReportApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosObsReport',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processosobsreport successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processosobsreportApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processosobsreportApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessosObsReport',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processosobsreportApi.delete(1)).rejects.toThrow(ProcessosObsReportApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await processosobsreportApi.delete(1);
  ...
  describe('ProcessosObsReportApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessosObsReportApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessosObsReportApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessosObsReportApiError('Test message', 500);
