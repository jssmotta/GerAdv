...
{
    Task<string> ValidateReg(Models.SMSAlice reg, ISMSAliceService service, IOperadorReader operadorReader, ITipoEMailReader tipoemailReader, [FromRoute, Required] string uri, MsiSqlConnection oCnn);
    Task<string> CanDelete(int id, ISMSAliceService service, [FromRoute, Required] string uri, MsiSqlConnection oCnn);
}

public class SMSAliceValidation : ISMSAliceValidation
{
    public async Task<string> CanDelete(int id, ISMSAliceService service, [FromRoute, Required] string uri, MsiSqlConnection oCnn)
    {
        if (id <= 0)
            return "Id inválido";
    ...
    public async Task<string> ValidateReg(Models.SMSAlice reg, ISMSAliceService service, IOperadorReader operadorReader, ITipoEMailReader tipoemailReader, [FromRoute, Required] string uri, MsiSqlConnection oCnn)
    {
        if (reg == null)
