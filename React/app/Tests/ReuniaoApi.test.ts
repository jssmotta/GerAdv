...
describe('ReuniaoApi', () => {
  ...
  beforeEach(() => {
    ...
    reuniaoApi = new ReuniaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(reuniaoApi).toBeInstanceOf(ReuniaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ReuniaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ReuniaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all reuniao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await reuniaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(reuniaoApi.getAll()).rejects.toThrow(ReuniaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await reuniaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Reuniao_last_getAll_${200}`)));
    });
    ...
    it('should throw ReuniaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockReuniao = ReuniaoTestEmpty();
    ...
    it('should fetch reuniao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await reuniaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockReuniao));
      ...
      expect(result.data).toEqual(mockReuniao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Reuniao',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter reuniao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await reuniaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new reuniao successfully', async () => {
      const responseReuniao = { ...mockReuniao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseReuniao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await reuniaoApi.addAndUpdate(mockReuniao);
    ...
    it('should update existing reuniao successfully', async () => {
      const existingReuniao = { ...mockReuniao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingReuniao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await reuniaoApi.addAndUpdate(existingReuniao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(reuniaoApi.addAndUpdate(mockReuniao)).rejects.toThrow(ReuniaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Reuniao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete reuniao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await reuniaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(reuniaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Reuniao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(reuniaoApi.delete(1)).rejects.toThrow(ReuniaoApiError);
    });
  });
  ...
  describe('ReuniaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ReuniaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ReuniaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ReuniaoApiError('Test message', 500);
