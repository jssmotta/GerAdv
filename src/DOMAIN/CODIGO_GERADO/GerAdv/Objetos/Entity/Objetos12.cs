...
public partial class FObjetos : MenphisSI.SG.GerAdv.DBObjetos, IDBObjetos
{
    // DI-compatible default constructor
    public FObjetos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FObjetos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FObjetos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FObjetos CreateFromDataRow(DataRow? dbRec)
    {
        return new FObjetos(dbRec);
    }

    private FObjetos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FObjetos(DataRow? dbRec) : base(dbRec)
    {
    }

    private FObjetos(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FObjetos CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FObjetos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FObjetos> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fObjetos = new FObjetos();
        await fObjetos.CarregarAsync(id, oCnn);
        return fObjetos;
    }
