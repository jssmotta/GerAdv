...
public static class FilterTipoCompromissoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterTipoCompromisso? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar TipoCompromisso em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoCompromisso:

- O filter irá filtrar os TipoCompromisso conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os TipoCompromisso" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoCompromisso length: {result.Length}");
#endif
