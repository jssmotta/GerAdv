...
public class FNECompromissosFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FNECompromissosFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FNECompromissosFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FNECompromissosFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "ncpCodigo")
        var dt = new DataTable();
        dt.Columns.Add("ncpCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["ncpCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FNECompromissos>();
    }
    ...
    {
        // Arrange
        var factory = new FNECompromissosFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("ncpCodigo", typeof(int));
        mockDataTable.Columns.Add("ncpTextoCompromisso", typeof(string));
        var row = mockDataTable.NewRow();
        row["ncpCodigo"] = testId;
        row["ncpTextoCompromisso"] = "Test NECompromissos";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["ncpCodigo"] = 123;
        row["ncpTextoCompromisso"] = "Test NECompromissos Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FNECompromissos properties
        }
    }
