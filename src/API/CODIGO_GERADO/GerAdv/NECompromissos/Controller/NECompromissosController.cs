...
public partial class NECompromissosController(INECompromissosService necompromissosService) : ControllerBase
{
    ...
    {
        _logger.Info("NECompromissos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _necompromissosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("NECompromissos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _necompromissosService.UpdateColumns(parameters, uri);
        if (!result)
        {
