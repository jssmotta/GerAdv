...
public class FColaboradoresFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FColaboradoresFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FColaboradoresFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FColaboradoresFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "colCodigo")
        var dt = new DataTable();
        dt.Columns.Add("colCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["colCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FColaboradores>();
    }
    ...
    {
        // Arrange
        var factory = new FColaboradoresFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("colCodigo", typeof(int));
        mockDataTable.Columns.Add("colNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["colCodigo"] = testId;
        row["colNome"] = "Test Colaboradores";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["colCodigo"] = 123;
        row["colNome"] = "Test Colaboradores Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FColaboradores properties
        }
    }
