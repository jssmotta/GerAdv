...
public partial class FServicos : MenphisSI.SG.GerAdv.DBServicos, IDBServicos
{
    // DI-compatible default constructor
    public FServicos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FServicos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FServicos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FServicos CreateFromDataRow(DataRow? dbRec)
    {
        return new FServicos(dbRec);
    }

    private FServicos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FServicos(DataRow? dbRec) : base(dbRec)
    {
    }

    private FServicos(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FServicos CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FServicos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FServicos> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fServicos = new FServicos();
        await fServicos.CarregarAsync(id, oCnn);
        return fServicos;
    }
