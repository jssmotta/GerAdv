...
#region ListarDados_TipoModeloDocumento
    public static async IAsyncEnumerable<DBTipoModeloDocumento> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoModeloDocumento(dbRec);
        }
    }

    public static IEnumerable<DBTipoModeloDocumento> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
