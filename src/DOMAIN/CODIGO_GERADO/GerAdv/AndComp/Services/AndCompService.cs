...
public partial class AndCompService(IOptions<AppSettings> appSettings, IFAndCompFactory andcompFactory, IAndCompReader reader, IAndCompValidation validation, IAndCompWriter writer, HybridCache cache, IMemoryCache memory) : IAndCompService, IDisposable
{
    ...
    public async Task<IEnumerable<AndCompResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterAndComp filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-{max}-AndComp-Filter-{where.GetHashCode2()}{filterHash}";
        var entryOptions = new HybridCacheEntryOptions
        {
