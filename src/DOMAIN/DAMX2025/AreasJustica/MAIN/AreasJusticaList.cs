...
#region ListarDados_AreasJustica
    public static async IAsyncEnumerable<DBAreasJustica> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAreasJustica(dbRec);
        }
    }

    public static IEnumerable<DBAreasJustica> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
