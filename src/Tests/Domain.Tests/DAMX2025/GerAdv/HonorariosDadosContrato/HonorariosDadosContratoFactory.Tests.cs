...
public class FHonorariosDadosContratoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FHonorariosDadosContratoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FHonorariosDadosContratoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FHonorariosDadosContratoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "hdcCodigo")
        var dt = new DataTable();
        dt.Columns.Add("hdcCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["hdcCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FHonorariosDadosContrato>();
    }
    ...
    {
        // Arrange
        var factory = new FHonorariosDadosContratoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("hdcCodigo", typeof(int));
        mockDataTable.Columns.Add("hdcGuid", typeof(string));
        var row = mockDataTable.NewRow();
        row["hdcCodigo"] = testId;
        row["hdcGuid"] = "Test HonorariosDadosContrato";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["hdcCodigo"] = 123;
        row["hdcGuid"] = "Test HonorariosDadosContrato Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FHonorariosDadosContrato properties
        }
    }
