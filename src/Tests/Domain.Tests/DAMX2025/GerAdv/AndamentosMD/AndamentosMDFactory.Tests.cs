...
public class FAndamentosMDFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FAndamentosMDFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FAndamentosMDFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FAndamentosMDFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "amdCodigo")
        var dt = new DataTable();
        dt.Columns.Add("amdCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["amdCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FAndamentosMD>();
    }
    ...
    {
        // Arrange
        var factory = new FAndamentosMDFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("amdCodigo", typeof(int));
        mockDataTable.Columns.Add("amdNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["amdCodigo"] = testId;
        row["amdNome"] = "Test AndamentosMD";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["amdCodigo"] = 123;
        row["amdNome"] = "Test AndamentosMD Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FAndamentosMD properties
        }
    }
