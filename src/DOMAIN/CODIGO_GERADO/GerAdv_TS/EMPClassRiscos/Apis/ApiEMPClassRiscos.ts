...
import { EMPClassRiscos } from '../../Models/EMPClassRiscos';
...
export class EMPClassRiscosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'EMPClassRiscosApiError';
    this.status = status;
    this.code = code;
...
export class EMPClassRiscosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/EMPClassRiscos`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EMPClassRiscos_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os EMPClassRiscos');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EMPClassRiscos_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar E M P Class Riscos com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterEMPClassRiscos): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EMPClassRiscos_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de EMPClassRiscos');
        }
    }

            public async filter(filtro: FilterEMPClassRiscos): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EMPClassRiscos_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar EMPClassRiscos');
                }
            }


    public async validation(regEMPClassRiscos: IEMPClassRiscos): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEMPClassRiscos as EMPClassRiscos, this.getHeaders());
    ...
    public async addAndUpdate(regEMPClassRiscos: IEMPClassRiscos): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regEMPClassRiscos as EMPClassRiscos, this.getHeaders());
            var register = result.data as IEMPClassRiscos;        
            const action = regEMPClassRiscos.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar E M P Class Riscos');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o EMPClassRiscos. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'EMPClassRiscos',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir E M P Class Riscos com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEMPClassRiscos) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<EMPClassRiscos[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de empclassriscos:', error);
            this.notificationService.notify({
              entity: 'EMPClassRiscos',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos empclassriscos'
            });
          }
        });
    }
