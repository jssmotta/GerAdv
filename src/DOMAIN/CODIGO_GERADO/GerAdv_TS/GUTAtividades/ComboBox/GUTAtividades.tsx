...
const GUTAtividadesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IGUTAtividades | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IGUTAtividades | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IGUTAtividades);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <GUTAtividadesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedGUTAtividades={editar ?? addValue ?? GUTAtividadesEmpty()} /> 
            )}
