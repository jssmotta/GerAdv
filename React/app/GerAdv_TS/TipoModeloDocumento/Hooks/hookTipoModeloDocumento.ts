...
export const useTipoModeloDocumentoForm = (
  ...
  const loadTipoModeloDocumento = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Modelo Documento";
...
export const useTipoModeloDocumentoList = (
  dataService: ITipoModeloDocumentoService,
  currentFilter?: FilterTipoModeloDocumento | undefined | null,
) => {
  const [data, setData] = useState<ITipoModeloDocumento[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoModeloDocumento | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterTipoModeloDocumento | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipomodelodocumento";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoModeloDocumento() {
  ...
  function validate(data: ITipoModeloDocumento): {
    isValid: boolean;
    message: string;
  } {
