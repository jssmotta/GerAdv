...
import { FilterOperadorGrupo } from '@/app/GerAdv_TS/OperadorGrupo/Filters/OperadorGrupo';
import OperadorComboBox from '@/app/GerAdv_TS/Operador/ComboBox/Operador';
interface UseOperadorGrupoFilterProps {
  handleFetchWithFilter: (filtro?: FilterOperadorGrupo | undefined | null) => Promise<void>;
}
interface UseOperadorGrupoFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterOperadorGrupo;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterOperadorGrupo>>;
  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterOperadorGrupo) => Promise<void>;
  // Render function
  renderInputFilters: (handlers: FilterHandlers<FilterOperadorGrupo>) => React.ReactNode;
  // Utilitários
  clearFilter: () => void;
  hasActiveFilter: boolean;
}
export const useOperadorGrupoFilter = ({ handleFetchWithFilter }: UseOperadorGrupoFilterProps): UseOperadorGrupoFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterOperadorGrupo>({} as FilterOperadorGrupo);
  // Handlers do Dialog
  const handleSearch = () => {
    setShowSearch(true);
    const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterOperadorGrupo;
    setWindowFilter(filterWildechar);
  };
  const handleCloseSearch = () => {
    setShowSearch(false);
  };
  const handleConfirmSearch = async (filter: FilterOperadorGrupo) => {
    await handleFetchWithFilter(filter);
  };
  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({});
    sessionStorage.removeItem(btoa('OperadorGrupoFilter'));
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterOperadorGrupo>) => (
  <>
  <OperadorComboBox
  name='operador'
  dataForm={null}
