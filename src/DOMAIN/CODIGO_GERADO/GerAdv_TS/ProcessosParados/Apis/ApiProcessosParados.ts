...
export class ProcessosParadosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ProcessosParadosApiError';
    this.status = status;
    this.code = code;
...
export class ProcessosParadosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProcessosParados`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosParados_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProcessosParados');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosParados_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Processos Parados com ID ${id}`);
        }
    }

            public async filter(filtro: FilterProcessosParados): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosParados_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProcessosParados');
                }
            }


    public async validation(regProcessosParados: IProcessosParados): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProcessosParados as ProcessosParados, this.getHeaders());
    ...
    public async addAndUpdate(regProcessosParados: IProcessosParados): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Processos Parados');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProcessosParados. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProcessosParados',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Processos Parados com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProcessosParados) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ProcessosParados[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de processosparados:', error);
            this.notificationService.notify({
              entity: 'ProcessosParados',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos processosparados'
            });
          }
        });
    }
