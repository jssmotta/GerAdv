...
public partial class OperadorGruposAgendaOperadoresController(IOperadorGruposAgendaOperadoresService operadorgruposagendaoperadoresService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> GetListN([FromQuery] int max, [FromBody] Filters.FilterOperadorGruposAgendaOperadores? filtro, [FromRoute, Required] string uri)
    {
        //_logger.Info($"OperadorGruposAgendaOperadores: GetListN called, max {max}, {filtro} uri");
        var result = await _operadorgruposagendaoperadoresService.GetListN(max, filtro, uri);
        return Ok(result);
    }
