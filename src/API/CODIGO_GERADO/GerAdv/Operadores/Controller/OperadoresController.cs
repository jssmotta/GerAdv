...
public partial class OperadoresController(IOperadoresService operadoresService) : ControllerBase
{
    ...
    {
        _logger.Info("Operadores: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _operadoresService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Operadores: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _operadoresService.UpdateColumns(parameters, uri);
        if (!result)
        {
