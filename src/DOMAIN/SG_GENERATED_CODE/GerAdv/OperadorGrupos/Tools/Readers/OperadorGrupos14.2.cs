...
public partial class OperadorGruposWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_OperadorGrupos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[OperadorGrupos]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_OperadorGrupos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[OperadorGrupos]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_OperadorGrupos_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_OperadorGrupos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[OperadorGrupos]'))
                BEGIN                    
