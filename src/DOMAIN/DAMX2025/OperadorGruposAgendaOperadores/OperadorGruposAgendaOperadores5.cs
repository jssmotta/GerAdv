...
#region AdministrativeMethods_OperadorGruposAgendaOperadores
    ...
    public bool DeletarItem(in int nOper, in int nId, SqlConnection? oCnn, SqlTransaction? oTrans) => nId > 0 && ConfiguracoesDBT.ExecuteSql($"{ConfiguracoesDBT.DeleteCommand(oCnn, true)} FROM dbo.{PTabelaNome} WHERE ogpCodigo={nId};", oCnn, oTrans);
#endregion
