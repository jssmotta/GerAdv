...
public static class FilterOperadorTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterOperador? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Operador em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterOperador:

- O filter irá filtrar os Operador conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Operador" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Operador length: {result.Length}");
#endif
