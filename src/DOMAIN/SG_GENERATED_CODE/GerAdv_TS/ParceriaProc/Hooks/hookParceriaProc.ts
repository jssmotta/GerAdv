...
export const useParceriaProcList = (dataService: IParceriaProcService, currentFilter?: FilterParceriaProc | undefined | null) => {
  const [data, setData] = useState<IParceriaProc[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterParceriaProc | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterParceriaProc | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterParceriaProc);
      setData(result);
    } catch (err) {
