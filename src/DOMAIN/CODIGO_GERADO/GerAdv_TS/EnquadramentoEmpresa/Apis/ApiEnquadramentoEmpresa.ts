...
export class EnquadramentoEmpresaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'EnquadramentoEmpresaApiError';
    this.status = status;
    this.code = code;
...
export class EnquadramentoEmpresaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/EnquadramentoEmpresa`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnquadramentoEmpresa_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os EnquadramentoEmpresa');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnquadramentoEmpresa_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Enquadramento Empresa com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnquadramentoEmpresa_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de EnquadramentoEmpresa');
        }
    }

            public async filter(filtro: FilterEnquadramentoEmpresa): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnquadramentoEmpresa_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar EnquadramentoEmpresa');
                }
            }


    public async validation(regEnquadramentoEmpresa: IEnquadramentoEmpresa): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEnquadramentoEmpresa as EnquadramentoEmpresa, this.getHeaders());
    ...
    public async addAndUpdate(regEnquadramentoEmpresa: IEnquadramentoEmpresa): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Enquadramento Empresa');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o EnquadramentoEmpresa. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'EnquadramentoEmpresa',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Enquadramento Empresa com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEnquadramentoEmpresa) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<EnquadramentoEmpresa[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de enquadramentoempresa:', error);
            this.notificationService.notify({
              entity: 'EnquadramentoEmpresa',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos enquadramentoempresa'
            });
          }
        });
    }
