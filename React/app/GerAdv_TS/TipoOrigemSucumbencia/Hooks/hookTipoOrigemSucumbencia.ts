...
export const useTipoOrigemSucumbenciaForm = (
  ...
  const loadTipoOrigemSucumbencia = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Origem Sucumbencia";
...
export const useTipoOrigemSucumbenciaList = (
  dataService: ITipoOrigemSucumbenciaService,
  currentFilter?: FilterTipoOrigemSucumbencia | undefined | null,
) => {
  const [data, setData] = useState<ITipoOrigemSucumbencia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    async (filtro?: FilterTipoOrigemSucumbencia | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipoorigemsucumbencia";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoOrigemSucumbencia() {
  ...
  function validate(data: ITipoOrigemSucumbencia): {
    isValid: boolean;
    message: string;
  } {
