...
public partial class DBRito : VAuditor, ICadastros, IAuditor
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Rito)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBRito();
...
#region TableDefinition_Rito
...
#endregion
    public DBRito(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBRito(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBRito(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_Rito
...
#endif
    {
        ...
        if (!isInsert)
            if (!(pFldFDescricao || pFldFTop || pFldFBold))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
        ...
        {
            throw new Exception("Campo 'ritDescricao' está vazio!");
        }
        ...
        if (pFldFDescricao)
            clsW.Fields(DBRitoDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFTop || ID.IsEmptyIDNumber())
            clsW.Fields(DBRitoDicInfo.Top, m_FTop, ETiposCampos.FBoolean);
        if (pFldFBold || ID.IsEmptyIDNumber())
            clsW.Fields(DBRitoDicInfo.Bold, m_FBold, ETiposCampos.FBoolean);
        if (pFldFGUID)
            clsW.Fields(DBRitoDicInfo.GUID, m_FGUID, ETiposCampos.FString);
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Rito)
        if (clsW.HasUpdates)
        {
            HasChanges = true;
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (pFldFQuemCad)
            clsW.Fields(DBRitoDicInfo.QuemCad, m_FQuemCad, ETiposCampos.FNumberNull);
        if (pFldFDtCad)
            clsW.Fields(DBRitoDicInfo.DtCad, m_FDtCad, ETiposCampos.FDate);
        if (pFldFQuemAtu)
            clsW.Fields(DBRitoDicInfo.QuemAtu, m_FQuemAtu, ETiposCampos.FNumberNull);
        if (pFldFDtAtu)
            clsW.Fields(DBRitoDicInfo.DtAtu, m_FDtAtu, ETiposCampos.FDate);
        if (pFldFVisto || ID.IsEmptyIDNumber())
            clsW.Fields(DBRitoDicInfo.Visto, m_FVisto, ETiposCampos.FBoolean);
        if (insertId != 0)
            return GravaNewId();
