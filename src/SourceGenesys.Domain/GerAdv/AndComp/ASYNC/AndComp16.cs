...
public partial class DBAndComp
{
    ...
    /// </summary>
    public static async Task<DBAndComp> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAndComp();
...
#if (!NOTSTORED_AndComp)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFAndamento || pFldFCompromisso;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFAndamento)
            updateTool.Fields(DBAndCompDicInfo.Andamento, m_FAndamento, ETiposCampos.FNumber);
        if (pFldFCompromisso)
            updateTool.Fields(DBAndCompDicInfo.Compromisso, m_FCompromisso, ETiposCampos.FNumber);
    }
