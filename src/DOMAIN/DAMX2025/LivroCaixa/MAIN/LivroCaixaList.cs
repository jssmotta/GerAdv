...
#region ListarDados_LivroCaixa
    public static async IAsyncEnumerable<DBLivroCaixa> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBLivroCaixa(dbRec);
        }
    }

    public static IEnumerable<DBLivroCaixa> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
