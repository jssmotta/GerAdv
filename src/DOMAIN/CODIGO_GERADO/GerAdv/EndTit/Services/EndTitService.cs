...
public partial class EndTitService(IOptions<AppSettings> appSettings, IFEndTitFactory endtitFactory, IEndTitReader reader, IEndTitValidation validation, IEndTitWriter writer, HybridCache cache, IMemoryCache memory) : IEndTitService, IDisposable
{
    private readonly IOptions<AppSettings> _appSettings = appSettings;
    private readonly HybridCache _cache = cache;
    private readonly IMemoryCache _memoryCache = memory;
    private bool _disposed;
    private readonly IFEndTitFactory endtitFactory = endtitFactory;
    private readonly IEndTitReader reader = reader;
    private readonly IEndTitValidation validation = validation;
    private readonly IEndTitWriter writer = writer;
    ...
    public async Task<IEnumerable<EndTitResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterEndTit filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-{max}-EndTit-Filter-{where.GetHashCode2()}{filterHash}";
        var entryOptions = new HybridCacheEntryOptions
        {
    ...
    private async Task<EndTitResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<EndTitResponse?> AddAndUpdate([FromBody] Models.EndTit regEndTit, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regEndTit, oCnn);
    ...
    public async Task<EndTitResponse?> Validation([FromBody] Models.EndTit regEndTit, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regEndTit.Id, oCnn);
    }
    ...
    public async Task<EndTitResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var endtit = await reader.Read(id, oCnn);
        try
        {
            if (endtit != null)
            {
                await writer.Delete(endtit, 0, oCnn);
