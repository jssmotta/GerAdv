...
import { SetorApiError } from '../GerAdv_TS/Setor/Apis/ApiSetor';
import { SetorApi } from '@/app/GerAdv_TS/Setor/Apis/ApiSetor';
import { FilterSetor } from '../GerAdv_TS/Setor/Filters/Setor';
...
describe('SetorApi', () => {
  ...
  beforeEach(() => {
    ...
    setorApi = new SetorApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(setorApi).toBeInstanceOf(SetorApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new SetorApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(SetorApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all setor items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await setorApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(setorApi.getAll()).rejects.toThrow(SetorApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await setorApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Setor_last_getAll_${200}`)));
    });
    ...
    it('should throw SetorApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockSetor = SetorTestEmpty();
    ...
    it('should fetch setor by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await setorApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockSetor));
      ...
      expect(result.data).toEqual(mockSetor);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Setor',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter setor items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await setorApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new setor successfully', async () => {
      const responseSetor = { ...mockSetor, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseSetor,
        status: 200,
        statusText: 'OK',
      ...
      const result = await setorApi.addAndUpdate(mockSetor);
    ...
    it('should update existing setor successfully', async () => {
      const existingSetor = { ...mockSetor, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingSetor,
        status: 200,
        statusText: 'OK',
      ...
      const result = await setorApi.addAndUpdate(existingSetor);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(setorApi.addAndUpdate(mockSetor)).rejects.toThrow(SetorApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Setor',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete setor successfully', async () => {
      ...
      const result = await setorApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(setorApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Setor',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(setorApi.delete(1)).rejects.toThrow(SetorApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterSetor = { funcionario: 1 };
      
      const result = setorApi.useFilter(mockFilter);
  ...
  describe('SetorApiError', () => {
    it('should create error with correct properties', () => {
      const error = new SetorApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('SetorApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new SetorApiError('Test message', 500);
