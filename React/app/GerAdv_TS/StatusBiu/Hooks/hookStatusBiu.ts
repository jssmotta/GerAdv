...
export const useStatusBiuList = (dataService: IStatusBiuService, currentFilter?: FilterStatusBiu | undefined | null) => {
  const [data, setData] = useState<IStatusBiu[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterStatusBiu | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterStatusBiu | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterStatusBiu);
      setData(result);
    } catch (err) {
