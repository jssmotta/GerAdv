...
import { ProDepositosApiError } from '../GerAdv_TS/ProDepositos/Apis/ApiProDepositos';
import { ProDepositosApi } from '@/app/GerAdv_TS/ProDepositos/Apis/ApiProDepositos';
import { FilterProDepositos } from '../GerAdv_TS/ProDepositos/Filters/ProDepositos';
...
describe('ProDepositosApi', () => {
  let prodepositosApi: ProDepositosApi;
  ...
  beforeEach(() => {
    ...
    prodepositosApi = new ProDepositosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(prodepositosApi).toBeInstanceOf(ProDepositosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProDepositosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProDepositosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all prodepositos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prodepositosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(prodepositosApi.getAll()).rejects.toThrow(ProDepositosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await prodepositosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProDepositos_last_getAll_${200}`)));
    });
    ...
    it('should throw ProDepositosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProDepositos = ProDepositosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProDepositos,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch prodepositos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prodepositosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProDepositos));
      ...
      expect(result.data).toEqual(mockProDepositos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProDepositos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProDepositos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter prodepositos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await prodepositosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProDepositos: IProDepositos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new prodepositos successfully', async () => {
      const responseProDepositos = { ...mockProDepositos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProDepositos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prodepositosApi.addAndUpdate(mockProDepositos);
    ...
    it('should update existing prodepositos successfully', async () => {
      const existingProDepositos = { ...mockProDepositos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProDepositos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prodepositosApi.addAndUpdate(existingProDepositos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(prodepositosApi.addAndUpdate(mockProDepositos)).rejects.toThrow(ProDepositosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProDepositos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete prodepositos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await prodepositosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(prodepositosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProDepositos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(prodepositosApi.delete(1)).rejects.toThrow(ProDepositosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProDepositos = { funcionario: 1 };
      
      const result = prodepositosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await prodepositosApi.delete(1);
  ...
  describe('ProDepositosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProDepositosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProDepositosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProDepositosApiError('Test message', 500);
