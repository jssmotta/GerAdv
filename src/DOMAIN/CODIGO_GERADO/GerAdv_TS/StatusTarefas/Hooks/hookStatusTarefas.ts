...
export const useStatusTarefasForm = (
  ...
  const loadStatusTarefas = useCallback(async (id: number) => {
    if (!id || id === 0) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Status Tarefas';
...
export const useStatusTarefasList = (dataService: IStatusTarefasService) => {
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar statustarefas';
      setError(errorMessage);
