...
public class FProcessOutPutIDsFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProcessOutPutIDsFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProcessOutPutIDsFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProcessOutPutIDsFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "poiCodigo")
        var dt = new DataTable();
        dt.Columns.Add("poiCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["poiCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProcessOutPutIDs>();
    }
    ...
    {
        // Arrange
        var factory = new FProcessOutPutIDsFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("poiCodigo", typeof(int));
        mockDataTable.Columns.Add("poiNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["poiCodigo"] = testId;
        row["poiNome"] = "Test ProcessOutPutIDs";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["poiCodigo"] = 123;
        row["poiNome"] = "Test ProcessOutPutIDs Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProcessOutPutIDs properties
        }
    }
