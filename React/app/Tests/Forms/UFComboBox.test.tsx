// app/TestsForms/UFComboBox.test.tsx - Vers�o Corrigida
...
import { UFEmpty, UFTestEmpty } from '@/app/GerAdv_TS/Models/UF';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/UF/Hooks/hookUF', () => ({
  useUFComboBox: jest.fn(() => ({
    options: [
    { ...UFEmpty(), id: 1, iduf: 'UF 1' },
    { ...UFEmpty(), id: 2, iduf: 'UF 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/UF/Apis/ApiUF', () => ({
  UFApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, iduf: 'Mock UF' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, iduf: 'Mock UF' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/UF/Services/UF.service', () => ({
  UFService: jest.fn().mockImplementation(() => ({
    fetchUFById: jest.fn().mockResolvedValue({ id: 1, iduf: 'Mock UF' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/UF/Crud/Grids/UFWindow', () => {
  return function MockUFWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='uf-window'>
      <button onClick={() => props.onClose()}>Close</button>
        <button onClick={() => props.onSuccess({ id: 1, iduf: 'New Item' })}>Success</button>
        </div>
        ) : null;
      };
    });
  ...
  >
  <option value=''>Select {props.textField === 'iduf' ? 'UF' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...UFTestEmpty(), id: 1, iduf: 'UF 1' },
  { ...UFTestEmpty(), id: 2, iduf: 'UF 2' },
];
...
describe('UFComboBox', () => {
  ...
  const { useUFComboBox } = require('@/app/GerAdv_TS/UF/Hooks/hookUF');
  (useUFComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<UFComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select UF')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...UFTestEmpty(), id: 1, iduf: 'UF 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useUFComboBox } = require('@/app/GerAdv_TS/UF/Hooks/hookUF');
    useUFComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens UFWindow on action button click (add)', async () => {
    render(<UFComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('uf-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('uf-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<UFComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles UFWindow onClose and onSuccess', () => {
    render(<UFComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    useUFComboBox.mockReturnValue({
      options: mockOptions, 
      loading: false, 
      selectedValue: { id: 1, iduf: 'Existing Item' },
      handleFilter: jest.fn(), 
      handleValueChange: jest.fn(), 
      clearValue: jest.fn(), 
    });
    render(<UFComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
