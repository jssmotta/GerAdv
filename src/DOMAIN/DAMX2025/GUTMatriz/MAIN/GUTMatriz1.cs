...
public partial class DBGUTMatriz : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_GUTMatriz)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBGUTMatriz();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_GUTMatriz
    [XmlIgnore]
    public string TabelaNome => "GUTMatriz";
...
#endregion
    public DBGUTMatriz(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBGUTMatriz(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBGUTMatriz(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_GUTMatriz
...
#endif
    {
        ...
        if (!isInsert)
            if (!(pFldFDescricao || pFldFGUTTipo || pFldFValor))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
        ...
        {
            throw new Exception("Campo 'gutDescricao' está vazio!");
        }
        ...
        if (pFldFDescricao)
            clsW.Fields(DBGUTMatrizDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFGUTTipo)
            clsW.Fields(DBGUTMatrizDicInfo.GUTTipo, m_FGUTTipo, ETiposCampos.FNumberNull);
        if (pFldFValor)
            clsW.Fields(DBGUTMatrizDicInfo.Valor, m_FValor, ETiposCampos.FNumberNull);
        if (insertId != 0)
            return GravaNewId();
