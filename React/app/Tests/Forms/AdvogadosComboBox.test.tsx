// app/TestsForms/AdvogadosComboBox.test.tsx - Vers�o Corrigida
...
import { AdvogadosEmpty, AdvogadosTestEmpty } from '@/app/GerAdv_TS/Models/Advogados';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Advogados/Hooks/hookAdvogados', () => ({
  useAdvogadosComboBox: jest.fn(() => ({
    options: [
    { ...AdvogadosEmpty(), id: 1, nome: 'Advogados 1' },
    { ...AdvogadosEmpty(), id: 2, nome: 'Advogados 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Advogados/Apis/ApiAdvogados', () => ({
  AdvogadosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Advogados' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Advogados' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Advogados/Services/Advogados.service', () => ({
  AdvogadosService: jest.fn().mockImplementation(() => ({
    fetchAdvogadosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Advogados' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Advogados/Crud/Grids/AdvogadosWindow', () => {
  return function MockAdvogadosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='advogados-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Advogados' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...AdvogadosTestEmpty(), id: 1, nome: 'Advogados 1' },
  { ...AdvogadosTestEmpty(), id: 2, nome: 'Advogados 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: AdvogadosEmpty(), 
};
describe('AdvogadosComboBox', () => {
  ...
  const { useAdvogadosComboBox } = require('@/app/GerAdv_TS/Advogados/Hooks/hookAdvogados');
  (useAdvogadosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<AdvogadosComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Advogados')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...AdvogadosTestEmpty(), id: 1, nome: 'Advogados 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useAdvogadosComboBox } = require('@/app/GerAdv_TS/Advogados/Hooks/hookAdvogados');
    useAdvogadosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens AdvogadosWindow on action button click (add)', async () => {
    render(<AdvogadosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('advogados-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('advogados-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<AdvogadosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles AdvogadosWindow onClose and onSuccess', () => {
    render(<AdvogadosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<AdvogadosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
