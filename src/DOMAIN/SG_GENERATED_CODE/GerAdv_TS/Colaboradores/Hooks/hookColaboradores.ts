...
export const useColaboradoresList = (dataService: IColaboradoresService, currentFilter?: FilterColaboradores | undefined | null) => {
  const [data, setData] = useState<IColaboradores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterColaboradores | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterColaboradores | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterColaboradores);
      setData(result);
    } catch (err) {
