...
public static class OperadorGruposAgendaOperadoresEndpoints
{
    ...
    private static void MapOperadorGruposAgendaOperadoresRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapGet("/GetAll", async (int max, string uri, IOperadorGruposAgendaOperadoresService service) =>
        {
            var result = await service.GetAll(max, uri);
            return Results.Ok(result);
        }).WithName("OperadorGruposAgendaOperadores_GetAll").WithDisplayName("Get All OperadorGruposAgendaOperadores");
        group.MapPost("/Filter", async (Filters.FilterOperadorGruposAgendaOperadores filtro, string uri, IOperadorGruposAgendaOperadoresService service) =>
        {
            var result = await service.Filter(filtro, uri);
            return Results.Ok(result);
        }).WithName("OperadorGruposAgendaOperadores_Filter").WithDisplayName("Filter OperadorGruposAgendaOperadores");
        group.MapGet("/GetById/{id}", async (int id, string uri, IOperadorGruposAgendaOperadoresService service, CancellationToken token) =>
        {
