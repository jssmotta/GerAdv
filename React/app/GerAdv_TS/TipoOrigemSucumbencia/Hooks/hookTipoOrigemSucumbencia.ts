...
export const useTipoOrigemSucumbenciaForm = (
  ...
  const loadTipoOrigemSucumbencia = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoOrigemSucumbencia);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Tipo Origem Sucumbencia';
      setError(errorMessage);
      console.log('Erro ao carregar Tipo Origem Sucumbencia');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialTipoOrigemSucumbencia]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoOrigemSucumbencia, resetForm]);
  return returnValue;
};


export const useTipoOrigemSucumbenciaNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('TipoOrigemSucumbencia', (entity) => {
      try {
...
export const useTipoOrigemSucumbenciaList = (dataService: ITipoOrigemSucumbenciaService) => {
  const [data, setData] = useState<ITipoOrigemSucumbencia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar tipoorigemsucumbencia';
      setError(errorMessage);
      console.log('Erro ao carregar tipoorigemsucumbencia');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useTipoOrigemSucumbenciaNotifications(
