...
#region ListarDados_StatusHTrab
    public static async IAsyncEnumerable<DBStatusHTrab> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBStatusHTrab(dbRec);
        }
    }

    public static IEnumerable<DBStatusHTrab> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
