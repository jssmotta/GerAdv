...
public partial class ServicosController(IServicosService servicosService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.Servicos regServicos, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Servicos: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Servicos: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.Servicos regServicos, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _servicosService.Validation(regServicos, uri);
            if (result == null)
            {
                _logger.Warn("Servicos: Validation failed to add or update Servicos, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "Servicos: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}