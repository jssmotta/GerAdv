...
export class OperadorApi {
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Operador-${uniqueKeyDay()}_lst_getAll_${max}`);
    ...
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Operador-${uniqueKeyDay()}_lst_getById_${id}`);
    ...
    public async getListN(max?: number, filtro?: FilterOperador): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Operador-${uniqueKeyDay()}_lst_listN_data`);
 ...
 public async filterPreload(max: number, filtro?: FilterOperador): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterOperadorDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Operador-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterOperador): Promise<AxiosResponse> {
    ...
    public async filterVoice(filtro?: FilterOperador, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterOperadorDefaults();

        // Create request object according to new backend structure
        const request: IFilterOperadorWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Operador-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
    ...
    public async validation(regOperador: IOperador): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regOperador as Operador, this.getHeaders());
            });
    ...
    public async addAndUpdate(regOperador: IOperador): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regOperador as Operador, this.getHeaders());
            });
    ...
    public useFilter(filtro: FilterOperador, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterOperadorWithVoiceRequest = {filter: filtro || new FilterOperadorDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<Operador[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de Operador');
            this.notificationService.notify({
              entity: 'Operador',
              id: 0,
              action: NotifySystemActions.ERROR,
