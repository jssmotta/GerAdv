...
#region ListarDados_GUTTipo
    public static async IAsyncEnumerable<DBGUTTipo> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTTipo(dbRec);
        }
    }

    public static IEnumerable<DBGUTTipo> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
