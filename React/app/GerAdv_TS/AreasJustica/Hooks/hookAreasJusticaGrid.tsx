...
// useAreasJusticaGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de AreasJustica
'use client';
...
interface UseAreasJusticaGridProps {
  data: IAreasJustica[];
...
export const useAreasJusticaGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('areasjustica-columns-order'), JSON.stringify(reorderableFields));
