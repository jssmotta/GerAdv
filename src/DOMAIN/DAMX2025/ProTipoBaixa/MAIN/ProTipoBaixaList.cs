...
#region ListarDados_ProTipoBaixa
    public static async IAsyncEnumerable<DBProTipoBaixa> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProTipoBaixa(dbRec);
        }
    }

    public static IEnumerable<DBProTipoBaixa> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
