...
import { Agenda2Agenda } from '../../Models/Agenda2Agenda';
...
export class Agenda2AgendaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'Agenda2AgendaApiError';
    this.status = status;
    this.code = code;
...
export class Agenda2AgendaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Agenda2Agenda`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Agenda2Agenda_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Agenda2Agenda');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Agenda2Agenda_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Agenda2 Agenda com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAgenda2Agenda): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Agenda2Agenda_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Agenda2Agenda');
                }
            }


    public async validation(regAgenda2Agenda: IAgenda2Agenda): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAgenda2Agenda as Agenda2Agenda, this.getHeaders());
    ...
    public async addAndUpdate(regAgenda2Agenda: IAgenda2Agenda): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAgenda2Agenda as Agenda2Agenda, this.getHeaders());
            var register = result.data as IAgenda2Agenda;        
            const action = regAgenda2Agenda.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Agenda2 Agenda');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Agenda2Agenda. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Agenda2Agenda',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Agenda2 Agenda com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAgenda2Agenda) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Agenda2Agenda[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de agenda2agenda:', error);
            this.notificationService.notify({
              entity: 'Agenda2Agenda',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos agenda2agenda'
            });
          }
        });
    }
