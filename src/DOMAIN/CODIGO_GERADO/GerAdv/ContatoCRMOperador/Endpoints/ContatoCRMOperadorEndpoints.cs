...
public static class ContatoCRMOperadorEndpoints
{
    ...
    private static void MapContatoCRMOperadorRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapGet("/GetAll", async (int max, string uri, IContatoCRMOperadorService service) =>
        {
            var result = await service.GetAll(max, uri);
            return Results.Ok(result);
        }).WithName("ContatoCRMOperador_GetAll").WithDisplayName("Get All ContatoCRMOperador");
        group.MapPost("/Filter", async (Filters.FilterContatoCRMOperador filtro, string uri, IContatoCRMOperadorService service) =>
        {
            var result = await service.Filter(filtro, uri);
            return Results.Ok(result);
        }).WithName("ContatoCRMOperador_Filter").WithDisplayName("Filter ContatoCRMOperador");
        group.MapGet("/GetById/{id}", async (int id, string uri, IContatoCRMOperadorService service, CancellationToken token) =>
        {
