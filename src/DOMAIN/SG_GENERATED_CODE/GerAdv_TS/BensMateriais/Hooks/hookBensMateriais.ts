...
export const useBensMateriaisList = (dataService: IBensMateriaisService, currentFilter?: FilterBensMateriais | undefined | null) => {
  const [data, setData] = useState<IBensMateriais[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterBensMateriais | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterBensMateriais | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterBensMateriais);
      setData(result);
    } catch (err) {
