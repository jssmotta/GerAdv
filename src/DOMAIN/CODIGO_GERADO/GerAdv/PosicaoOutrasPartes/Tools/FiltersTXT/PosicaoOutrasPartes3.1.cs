...
public static class FilterPosicaoOutrasPartesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar PosicaoOutrasPartes em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterPosicaoOutrasPartes:

- O filter irá filtrar os PosicaoOutrasPartes conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os PosicaoOutrasPartes" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterPosicaoOutrasPartes? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt PosicaoOutrasPartes length: {result.Length}");
#endif
