...
public class FPosicaoOutrasPartesFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FPosicaoOutrasPartesFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FPosicaoOutrasPartesFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FPosicaoOutrasPartesFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "posCodigo")
        var dt = new DataTable();
        dt.Columns.Add("posCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["posCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FPosicaoOutrasPartes>();
    }
    ...
    {
        // Arrange
        var factory = new FPosicaoOutrasPartesFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("posCodigo", typeof(int));
        mockDataTable.Columns.Add("posDescricao", typeof(string));
        var row = mockDataTable.NewRow();
        row["posCodigo"] = testId;
        row["posDescricao"] = "Test PosicaoOutrasPartes";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["posCodigo"] = 123;
        row["posDescricao"] = "Test PosicaoOutrasPartes Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FPosicaoOutrasPartes properties
        }
    }
