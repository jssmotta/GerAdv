...
#region ListarDados_Situacao
    public static async IAsyncEnumerable<DBSituacao> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBSituacao(dbRec);
        }
    }

    public static IEnumerable<DBSituacao> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
