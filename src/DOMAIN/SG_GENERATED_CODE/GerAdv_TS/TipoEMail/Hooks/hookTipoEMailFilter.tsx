...
import { FilterTipoEMail } from '@/app/GerAdv_TS/TipoEMail/Filters/TipoEMail';
import FilterDialogButton from '@/app/components/Cruds/FilterDialogButton';


interface UseTipoEMailFilterProps {
    handleFetchWithFilter: (filtro?: FilterTipoEMail | undefined | null) => Promise<void>;
}

interface UseTipoEMailFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterTipoEMail;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterTipoEMail>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterTipoEMail) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterTipoEMail>) => React.ReactNode;
...
export const useTipoEMailFilter = ({ handleFetchWithFilter }: UseTipoEMailFilterProps): UseTipoEMailFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterTipoEMail>({} as FilterTipoEMail);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterTipoEMail;
    ...
    const handleConfirmSearch = async (filter: FilterTipoEMail) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterTipoEMail);
        sessionStorage.removeItem(btoa('TipoEMailFilter'));
        handleFetchWithFilter({} as FilterTipoEMail);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterTipoEMail>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
