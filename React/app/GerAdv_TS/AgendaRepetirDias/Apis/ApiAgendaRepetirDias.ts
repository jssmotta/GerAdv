...
export class AgendaRepetirDiasApi {
 ...
 public async filterPreload(max: number, filtro: FilterAgendaRepetirDias): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRepetirDias-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterAgendaRepetirDias): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRepetirDias-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterAgendaRepetirDias) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AgendaRepetirDias[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de AgendaRepetirDias');
            this.notificationService.notify({
              entity: 'AgendaRepetirDias',
              id: 0,
              action: NotifySystemActions.ERROR,
