...
import { BensMateriais } from '../../Models/BensMateriais';
...
export class BensMateriaisApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'BensMateriaisApiError';
    this.status = status;
    this.code = code;
...
export class BensMateriaisApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/BensMateriais`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-BensMateriais_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os BensMateriais');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-BensMateriais_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Bens Materiais com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterBensMateriais): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-BensMateriais_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de BensMateriais');
        }
    }

            public async filter(filtro: FilterBensMateriais): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-BensMateriais_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar BensMateriais');
                }
            }


    public async validation(regBensMateriais: IBensMateriais): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regBensMateriais as BensMateriais, this.getHeaders());
    ...
    public async addAndUpdate(regBensMateriais: IBensMateriais): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regBensMateriais as BensMateriais, this.getHeaders());
            var register = result.data as IBensMateriais;        
            const action = regBensMateriais.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Bens Materiais');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o BensMateriais. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'BensMateriais',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Bens Materiais com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterBensMateriais) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<BensMateriais[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de bensmateriais:', error);
            this.notificationService.notify({
              entity: 'BensMateriais',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos bensmateriais'
            });
          }
        });
    }
