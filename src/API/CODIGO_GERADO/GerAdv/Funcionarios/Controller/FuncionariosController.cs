...
public partial class FuncionariosController(IFuncionariosService funcionariosService) : ControllerBase
{
    ...
    {
        _logger.Info("Funcionarios: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _funcionariosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Funcionarios: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _funcionariosService.UpdateColumns(parameters, uri);
        if (!result)
        {
