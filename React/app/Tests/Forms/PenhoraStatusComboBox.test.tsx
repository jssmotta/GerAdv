// app/TestsForms/PenhoraStatusComboBox.test.tsx - Vers�o Corrigida
...
import { PenhoraStatusEmpty, PenhoraStatusTestEmpty } from '@/app/GerAdv_TS/Models/PenhoraStatus';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/PenhoraStatus/Hooks/hookPenhoraStatus', () => ({
  usePenhoraStatusComboBox: jest.fn(() => ({
    options: [
    { ...PenhoraStatusEmpty(), id: 1, nome: 'Penhora Status 1' },
    { ...PenhoraStatusEmpty(), id: 2, nome: 'Penhora Status 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/PenhoraStatus/Apis/ApiPenhoraStatus', () => ({
  PenhoraStatusApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock PenhoraStatus' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock PenhoraStatus' }),
...
jest.mock('@/app/GerAdv_TS/PenhoraStatus/Services/PenhoraStatus.service', () => ({
  PenhoraStatusService: jest.fn().mockImplementation(() => ({
    fetchPenhoraStatusById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock PenhoraStatus' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/PenhoraStatus/Crud/Grids/PenhoraStatusWindow', () => {
  return function MockPenhoraStatusWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='penhorastatus-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Penhora Status' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...PenhoraStatusTestEmpty(), id: 1, nome: 'Penhora Status 1' },
  { ...PenhoraStatusTestEmpty(), id: 2, nome: 'Penhora Status 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: PenhoraStatusEmpty(), 
};
describe('PenhoraStatusComboBox', () => {
  ...
  const { usePenhoraStatusComboBox } = require('@/app/GerAdv_TS/PenhoraStatus/Hooks/hookPenhoraStatus');
  (usePenhoraStatusComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<PenhoraStatusComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Penhora Status')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Penhora Status')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...PenhoraStatusTestEmpty(), id: 1, nome: 'Penhora Status 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { usePenhoraStatusComboBox } = require('@/app/GerAdv_TS/PenhoraStatus/Hooks/hookPenhoraStatus');
    usePenhoraStatusComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens PenhoraStatusWindow on action button click (add)', async () => {
    render(<PenhoraStatusComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('penhorastatus-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('penhorastatus-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<PenhoraStatusComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles PenhoraStatusWindow onClose and onSuccess', () => {
    render(<PenhoraStatusComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<PenhoraStatusComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
