...
import { LigacoesApiError } from '../GerAdv_TS/Ligacoes/Apis/ApiLigacoes';
import { LigacoesApi } from '@/app/GerAdv_TS/Ligacoes/Apis/ApiLigacoes';
import { FilterLigacoes } from '../GerAdv_TS/Ligacoes/Filters/Ligacoes';
...
describe('LigacoesApi', () => {
  let ligacoesApi: LigacoesApi;
  ...
  beforeEach(() => {
    ...
    ligacoesApi = new LigacoesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(ligacoesApi).toBeInstanceOf(LigacoesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new LigacoesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(LigacoesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all ligacoes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ligacoesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(ligacoesApi.getAll()).rejects.toThrow(LigacoesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await ligacoesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Ligacoes_last_getAll_${200}`)));
    });
    ...
    it('should throw LigacoesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockLigacoes = LigacoesTestEmpty();
    ...
    it('should fetch ligacoes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ligacoesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockLigacoes));
      ...
      expect(result.data).toEqual(mockLigacoes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ligacoes',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterLigacoes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter ligacoes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await ligacoesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new ligacoes successfully', async () => {
      const responseLigacoes = { ...mockLigacoes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseLigacoes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await ligacoesApi.addAndUpdate(mockLigacoes);
    ...
    it('should update existing ligacoes successfully', async () => {
      const existingLigacoes = { ...mockLigacoes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingLigacoes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await ligacoesApi.addAndUpdate(existingLigacoes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(ligacoesApi.addAndUpdate(mockLigacoes)).rejects.toThrow(LigacoesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ligacoes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete ligacoes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await ligacoesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(ligacoesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ligacoes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(ligacoesApi.delete(1)).rejects.toThrow(LigacoesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterLigacoes = { funcionario: 1 };
      
      const result = ligacoesApi.useFilter(mockFilter);
  ...
  describe('LigacoesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new LigacoesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('LigacoesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new LigacoesApiError('Test message', 500);
