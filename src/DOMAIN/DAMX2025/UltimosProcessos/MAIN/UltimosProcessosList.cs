...
#region ListarDados_UltimosProcessos
    public static async IAsyncEnumerable<DBUltimosProcessos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBUltimosProcessos(dbRec);
        }
    }

    public static IEnumerable<DBUltimosProcessos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
