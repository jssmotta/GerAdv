...
export class GUTAtividadesMatrizApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'GUTAtividadesMatrizApiError';
    this.status = status;
    this.code = code;
...
export class GUTAtividadesMatrizApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/GUTAtividadesMatriz`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividadesMatriz_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTAtividadesMatriz');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividadesMatriz_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar G U T Atividades Matriz com ID ${id}`);
        }
    }

            public async filter(filtro: FilterGUTAtividadesMatriz): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividadesMatriz_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTAtividadesMatriz');
                }
            }


    public async validation(regGUTAtividadesMatriz: IGUTAtividadesMatriz): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTAtividadesMatriz as GUTAtividadesMatriz, this.getHeaders());
    ...
    public async addAndUpdate(regGUTAtividadesMatriz: IGUTAtividadesMatriz): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Atividades Matriz');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o GUTAtividadesMatriz. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'GUTAtividadesMatriz',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir G U T Atividades Matriz com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTAtividadesMatriz) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<GUTAtividadesMatriz[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de gutatividadesmatriz:', error);
            this.notificationService.notify({
              entity: 'GUTAtividadesMatriz',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gutatividadesmatriz'
            });
          }
        });
    }
