...
[{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBHonorariosDadosContratoDicInfo.PTabelaNome}].[{DBHonorariosDadosContratoDicInfo.Cliente}]
...
FROM {oCnn.UseDbo}.HonorariosDadosContrato WITH (INDEX = idx_{DBHonorariosDadosContratoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [hdcCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBHonorariosDadosContratoDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(hdcDtAtu, hdcDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBHonorariosDadosContratoDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(hdcDtAtu, hdcDtCad) DESC;", oCnn.InnerConnection);
