...
export const useFuncionariosList = (
  dataService: IFuncionariosService,
  currentFilter?: FilterFuncionarios | undefined | null,
) => {
  const [data, setData] = useState<IFuncionarios[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterFuncionarios | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterFuncionarios | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar funcionarios";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsFuncionarios() {
  ...
  function validate(data: IFuncionarios): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.nome.length > 60) {
        return {
          isValid: false,
          message: "O campo Nome não pode ter mais de 60 caracteres.",
        };
      }
      if (data.registro.length > 20) {
        return {
          isValid: false,
          message: "O campo Registro não pode ter mais de 20 caracteres.",
        };
      }
