...
export const useOutrasPartesClienteList = (dataService: IOutrasPartesClienteService, currentFilter?: FilterOutrasPartesCliente | undefined | null) => {
  const [data, setData] = useState<IOutrasPartesCliente[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterOutrasPartesCliente | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterOutrasPartesCliente | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterOutrasPartesCliente);
      setData(result);
    } catch (err) {
