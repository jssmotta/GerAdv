...
interface GraphGridProps {
  data: IGraph[];
  onRowClick: (graph: IGraph) => void;
  onDeleteClick: (e: any) => void;
  setSelectedId: (id: number | null) => void;
}
export const GraphGridMobileComponent = React.memo(
({
  data, 
  onRowClick, 
  onDeleteClick, 
...
const filteredData = useMemo(() => { return data.filter((data: any) => {
  const guidMatches = applyFilter(data, 'guid', columnFilters.guid);
  return guidMatches;
});
}, [data, columnFilters]);
const handleFilterChange = (event: GridFilterChangeEvent) => {
  const filters = event.filter?.filters || [];
  const newColumnFilters = { guid: '' };
