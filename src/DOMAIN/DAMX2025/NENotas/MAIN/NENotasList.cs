...
#region ListarDados_NENotas
    public static async IAsyncEnumerable<DBNENotas> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBNENotas(dbRec);
        }
    }

    public static IEnumerable<DBNENotas> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
