...
export const useStatusAndamentoForm = (
  ...
  const loadStatusAndamento = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Status Andamento";
...
export const useStatusAndamentoList = (
  dataService: IStatusAndamentoService,
  currentFilter?: FilterStatusAndamento | undefined | null,
) => {
  const [data, setData] = useState<IStatusAndamento[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterStatusAndamento | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterStatusAndamento | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar statusandamento";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsStatusAndamento() {
  ...
  function validate(data: IStatusAndamento): {
    isValid: boolean;
    message: string;
  } {
