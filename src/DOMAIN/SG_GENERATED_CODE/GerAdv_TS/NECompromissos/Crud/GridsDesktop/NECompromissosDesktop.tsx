...
export const NECompromissosGridDesktopComponent = React.memo(({
  ...
  const [columnsOrder, setColumnsOrder] = useState<string[]>(['descricaotipocompromisso']);

  // Carregar ordem salva do localStorage na inicialização
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('necompromissos-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('necompromissos-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
