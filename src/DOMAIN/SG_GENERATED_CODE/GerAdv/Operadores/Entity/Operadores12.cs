...
public partial class FOperadores : MenphisSI.SG.GerAdv.DBOperadores, IDBOperadores
{
    // DI-compatible default constructor
    public FOperadores() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FOperadores CreateFromReader(SqlDataReader? dbRec)
    {
        return new FOperadores(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FOperadores CreateFromDataRow(DataRow? dbRec)
    {
        return new FOperadores(dbRec);
    }

    private FOperadores(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FOperadores(DataRow? dbRec) : base(dbRec)
    {
    }

    private FOperadores(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FOperadores CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FOperadores(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FOperadores> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fOperadores = new FOperadores();
        await fOperadores.CarregarAsync(id, oCnn);
        return fOperadores;
    }
