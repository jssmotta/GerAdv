...
public static class EMPClassRiscosEndpoints
{
    ...
    private static void MapEMPClassRiscosRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IEMPClassRiscosValidation validation, IEMPClassRiscosWriter writer, IEMPClassRiscosService service) =>
        {
            logger.LogInfo("EMPClassRiscos", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("EMPClassRiscos", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("EMPClassRiscos_GetColumns").WithDisplayName("Get EMPClassRiscos Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IEMPClassRiscosValidation validation, IEMPClassRiscosWriter writer, IEMPClassRiscosService service) =>
        {
            logger.LogInfo("EMPClassRiscos", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("EMPClassRiscos", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("EMPClassRiscos_UpdateColumns").WithDisplayName("Update EMPClassRiscos Columns");
