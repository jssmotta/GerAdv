...
export const useOperadorGrupoList = (dataService: IOperadorGrupoService, currentFilter?: FilterOperadorGrupo | undefined | null) => {
  const [data, setData] = useState<IOperadorGrupo[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterOperadorGrupo | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterOperadorGrupo | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterOperadorGrupo);
      setData(result);
    } catch (err) {
