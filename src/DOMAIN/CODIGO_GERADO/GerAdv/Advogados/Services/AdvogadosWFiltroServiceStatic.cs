...
public partial class AdvogadosService
{
    ...
    /// <returns>FilterAdvogados processado</returns>
    private async Task<Filters.FilterAdvogados?> ProcessFilterVoiceCommandAsync(CommandSpeakerRequest message, Filters.FilterAdvogados? existingFilter, string uri)
    {
        try
        {
            // Usar o contexto do FilterAdvogadosTXT
            var context = FilterAdvogadosTXT.GetPrompt(existingFilter, FilterAdvogadosTXT.GetOptions());
            // Instruções específicas para Advogados
        ...
        catch (Exception)
        {
            ...
            return existingFilter ?? new Filters.FilterAdvogados();
        }
    }

    private async Task<IEnumerable<AdvogadosResponseAll>> GetDataAllAsync(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken cancellationToken)
    {
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter>? parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarNAsync(max, uri, where, parameters, DBAdvogadosDicInfo.CampoNome);
    ...
    private async Task<IEnumerable<AdvogadosResponseAll>> GetDataAllAsync2(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken token)
    {
        using var scope = _connectionService.CreateConnectionScope(uri);
        using var oCnn = scope.Connection;
        if (oCnn == null)
