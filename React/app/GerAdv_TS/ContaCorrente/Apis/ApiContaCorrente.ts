...
export class ContaCorrenteApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ContaCorrenteApiError';
    this.status = status;
    this.code = code;
...
export class ContaCorrenteApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ContaCorrente`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContaCorrente_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ContaCorrente');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContaCorrente_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Conta Corrente com ID ${id}`);
        }
    }

            public async filter(filtro: FilterContaCorrente): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContaCorrente_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ContaCorrente');
                }
            }


    public async validation(regContaCorrente: IContaCorrente): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regContaCorrente as ContaCorrente, this.getHeaders());
    ...
    public async addAndUpdate(regContaCorrente: IContaCorrente): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regContaCorrente as ContaCorrente, this.getHeaders());
            var register = result.data as IContaCorrente;        
            const action = regContaCorrente.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Conta Corrente');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ContaCorrente. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ContaCorrente',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Conta Corrente com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterContaCorrente) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ContaCorrente[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de contacorrente:', error);
            this.notificationService.notify({
              entity: 'ContaCorrente',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos contacorrente'
            });
          }
        });
    }
