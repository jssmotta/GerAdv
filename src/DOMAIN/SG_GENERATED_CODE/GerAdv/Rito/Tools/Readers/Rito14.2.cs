...
public partial class RitoWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        if (oCnn is null)
            return;
        Console.WriteLine($"WarmUp Rito: {tenantKey}");
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_Rito_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[Rito]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Rito_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Rito]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_Rito_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Rito_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Rito]'))
                BEGIN                    
