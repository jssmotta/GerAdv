...
describe('EscritoriosApi', () => {
  let escritoriosApi: EscritoriosApi;
  ...
  beforeEach(() => {
    ...
    escritoriosApi = new EscritoriosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(escritoriosApi).toBeInstanceOf(EscritoriosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EscritoriosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EscritoriosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all escritorios items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await escritoriosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(escritoriosApi.getAll()).rejects.toThrow(EscritoriosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await escritoriosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Escritorios_last_getAll_${200}`)));
    });
    ...
    it('should throw EscritoriosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEscritorios = EscritoriosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockEscritorios,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch escritorios by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await escritoriosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEscritorios));
      ...
      expect(result.data).toEqual(mockEscritorios);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Escritorios',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter escritorios items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await escritoriosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new escritorios successfully', async () => {
      const responseEscritorios = { ...mockEscritorios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEscritorios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await escritoriosApi.addAndUpdate(mockEscritorios);
    ...
    it('should update existing escritorios successfully', async () => {
      const existingEscritorios = { ...mockEscritorios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEscritorios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await escritoriosApi.addAndUpdate(existingEscritorios);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(escritoriosApi.addAndUpdate(mockEscritorios)).rejects.toThrow(EscritoriosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Escritorios',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete escritorios successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await escritoriosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(escritoriosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Escritorios',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(escritoriosApi.delete(1)).rejects.toThrow(EscritoriosApiError);
    });
  });
  ...
  describe('EscritoriosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EscritoriosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EscritoriosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EscritoriosApiError('Test message', 500);
