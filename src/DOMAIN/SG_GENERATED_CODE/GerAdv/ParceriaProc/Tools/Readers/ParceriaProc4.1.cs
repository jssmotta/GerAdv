...
[{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBAdvogadosDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.CampoCodigo}]=[{DBParceriaProcDicInfo.PTabelaNome}].[{DBParceriaProcDicInfo.Advogado}]
...
SELECT TOP (1) 
	FORMAT(parDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(parDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    parVisto as Visto
FROM {oCnn.UseDbo}.ParceriaProc dbq WITH (INDEX = idx_ParceriaProc_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.parQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.parQuemAtu
WHERE [parCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBParceriaProc.PTabelaNome,
...
FROM {oCnn.UseDbo}.ParceriaProc WITH (INDEX = idx_{DBParceriaProcDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [parCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBParceriaProcDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(parDtAtu, parDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBParceriaProcDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(parDtAtu, parDtCad) DESC;", oCnn.InnerConnection);
