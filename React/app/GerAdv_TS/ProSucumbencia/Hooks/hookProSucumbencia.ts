// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
"use client";
import { useState, useEffect, useCallback, useRef, useMemo } from "react";
import { IProSucumbenciaService } from "../Services/ProSucumbencia.service";
import {
  NotifySystemActions,
  subscribeToNotifications,
} from "@/app/tools/NotifySystem";
import { IProSucumbencia } from "../Interfaces/interface.ProSucumbencia";
import { FilterProSucumbencia } from "../Filters/ProSucumbencia";
import { isValidDate } from "@/app/tools/datetime";
import { ProSucumbenciaApi } from "../Apis/ApiProSucumbencia";

export const useProSucumbenciaForm = (
  initialProSucumbencia: IProSucumbencia,
  dataService: IProSucumbenciaService,
) => {
  const [data, setData] = useState<IProSucumbencia>(initialProSucumbencia);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleChange = useCallback((e: any) => {
    // Lazy require to avoid circular import surprises in SSR environments.
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { genericHandleChange } = require("../../../tools/formUtils");
    if (genericHandleChange) genericHandleChange(e, setData);
  }, []);

  const loadProSucumbencia = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
        setData(initialProSucumbencia);
        return;
      }

      setLoading(true);
      setError(null);

      try {
        const dados = await dataService.fetchProSucumbenciaById(id);
        setData(dados);
      } catch (err) {
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Pro Sucumbencia";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
          console.log("Erro ao carregar Pro Sucumbencia");
      } finally {
        setLoading(false);
      }
    },
    [dataService, initialProSucumbencia],
  );

  const resetForm = useCallback(() => {
    setData(initialProSucumbencia);
    setError(null);
  }, [initialProSucumbencia]);

  const returnValue = useMemo(
    () => ({
      data,
      loading,
      error,
      handleChange,
      loadProSucumbencia,
      resetForm,
      setData,
    }),
    [data, loading, error, handleChange, loadProSucumbencia, resetForm],
  );
  return returnValue;
};

export const useProSucumbenciaNotifications = (
  onUpdate?: (entity: any) => void,
  onDelete?: (entity: any) => void,
  onAdd?: (entity: any) => void,
) => {
  const callbacksRef = useRef({ onUpdate, onDelete, onAdd });

  useEffect(() => {
    callbacksRef.current = { onUpdate, onDelete, onAdd };
  }, [onUpdate, onDelete, onAdd]);

  useEffect(() => {
    const unsubscribe = subscribeToNotifications("ProSucumbencia", (entity) => {
      try {
        const { onUpdate, onDelete, onAdd } = callbacksRef.current;

        switch (entity.action) {
          case NotifySystemActions.DELETE:
            onDelete?.(entity);
            break;
          case NotifySystemActions.UPDATE:
            onUpdate?.(entity);
            break;
          case NotifySystemActions.ADD:
            onAdd?.(entity);
            break;
        }
      } catch (err) {
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
          console.log("Erro no listener de notificações.");
      }
    });

    return () => {
      unsubscribe();
    };
  }, []);
};

export const useProSucumbenciaList = (
  dataService: IProSucumbenciaService,
  currentFilter?: FilterProSucumbencia | undefined | null,
) => {
  const [data, setData] = useState<IProSucumbencia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProSucumbencia | undefined | null>(
    currentFilter,
  );

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(
    async (filtro?: FilterProSucumbencia | undefined | null) => {
      setLoading(true);
      setError(null);

      try {
        const result = await dataService.getAll(
          filtro ?? ({} as FilterProSucumbencia),
        );
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar prosucumbencia";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
          console.log("Erro ao carregar prosucumbencia");
      } finally {
        setLoading(false);
      }
    },
    [dataService],
  );

  const refreshData = useCallback(() => {
    fetchData(currentFilterRef.current);
  }, [fetchData]);

  useProSucumbenciaNotifications(
    refreshData, // onUpdate
    refreshData, // onDelete
    refreshData, // onAdd
  );

  return {
    data,
    loading,
    error,
    fetchData,
    refreshData,
  };
};

export function useValidationsProSucumbencia() {
  async function runValidation(
    data: IProSucumbencia,
    uri?: string,
    token?: string,
  ): Promise<{ isValid: boolean; message: string } | null> {
    const prosucumbenciaApi = new ProSucumbenciaApi(uri ?? "", token ?? "");

    const result = await prosucumbenciaApi.validation(data);

    return result;
  }

  function validate(data: IProSucumbencia): {
    isValid: boolean;
    message: string;
  } {
    if (!data) return { isValid: false, message: "Dados não informados." };

    try {
      if (data.data.includes("%")) {
        return { isValid: false, message: `Remova o caracter '%' inválido!` };
      }
      if (data.data.length <= 0) {
        return {
          isValid: false,
          message: "O campo Data não pode ficar vazio.",
        };
      }
      if (data.nome.length > 2048) {
        return {
          isValid: false,
          message: "O campo Nome não pode ter mais de 2048 caracteres.",
        };
      }
      if (data.percentual.length > 5) {
        return {
          isValid: false,
          message: "O campo Percentual não pode ter mais de 5 caracteres.",
        };
      }

      return { isValid: true, message: "" };
    } catch (error) {
      return { isValid: true, message: "Erro ao validar os dados." };
    }
  }

  return { validate, runValidation };
}
