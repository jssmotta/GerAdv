...
public static class FilterTipoEnderecoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoEndereco em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoEndereco:

- O filter irá filtrar os TipoEndereco conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterTipoEndereco:(Tipo Endereco)
- descricao: _nome ou descrição para Tipo Endereco;
...
- Se ouvir "todos os TipoEndereco" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoEndereco? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoEndereco length: {result.Length}");
#endif
