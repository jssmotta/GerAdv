...
public partial class DBProPartes
{
    ...
    /// </summary>
    public static async Task<DBProPartes> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBProPartes();
...
#if (!NOTSTORED_ProPartes)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFParte || pFldFProcesso;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFParte)
            updateTool.Fields(DBProPartesDicInfo.Parte, m_FParte, ETiposCampos.FNumber);
        if (pFldFProcesso)
            updateTool.Fields(DBProPartesDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
    }
