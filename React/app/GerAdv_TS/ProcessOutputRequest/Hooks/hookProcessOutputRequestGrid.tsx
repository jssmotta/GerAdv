...
// useProcessOutputRequestGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de ProcessOutputRequest
'use client';
...
interface UseProcessOutputRequestGridProps {
...
export const useProcessOutputRequestGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  initialTake = 10, 
  useCustomSort = false
}: UseProcessOutputRequestGridProps) => {
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('processoutputrequest-columns-order'), JSON.stringify(reorderableFields));
