...
export class ContatoCRMOperadorApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ContatoCRMOperadorApiError';
    this.status = status;
    this.code = code;
...
export class ContatoCRMOperadorApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMOperador_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ContatoCRMOperador');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMOperador_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Contato C R M Operador com ID ${id}`);
        }
    }

            public async filter(filtro: FilterContatoCRMOperador): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMOperador_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ContatoCRMOperador');
                }
            }


    public async validation(regContatoCRMOperador: IContatoCRMOperador): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regContatoCRMOperador as ContatoCRMOperador, this.getHeaders());
    ...
    public async addAndUpdate(regContatoCRMOperador: IContatoCRMOperador): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Contato C R M Operador');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Contato C R M Operador com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterContatoCRMOperador) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ContatoCRMOperador[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de contatocrmoperador:', error);
            this.notificationService.notify({
              entity: 'ContatoCRMOperador',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos contatocrmoperador'
            });
          }
        });
    }
