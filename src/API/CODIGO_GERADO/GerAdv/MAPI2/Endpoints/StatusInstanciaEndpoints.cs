...
public static class StatusInstanciaEndpoints
{
    ...
    private static void MapStatusInstanciaRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IStatusInstanciaValidation validation, IStatusInstanciaWriter writer, IStatusInstanciaService service) =>
        {
            logger.LogInfo("StatusInstancia", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("StatusInstancia", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("StatusInstancia_GetColumns").WithDisplayName("Get StatusInstancia Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IStatusInstanciaValidation validation, IStatusInstanciaWriter writer, IStatusInstanciaService service) =>
        {
            logger.LogInfo("StatusInstancia", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("StatusInstancia", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("StatusInstancia_UpdateColumns").WithDisplayName("Update StatusInstancia Columns");
