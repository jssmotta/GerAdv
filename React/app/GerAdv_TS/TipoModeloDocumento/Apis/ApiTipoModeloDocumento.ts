...
export class TipoModeloDocumentoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'TipoModeloDocumentoApiError';
    this.status = status;
    this.code = code;
...
export class TipoModeloDocumentoApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoModeloDocumento_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoModeloDocumento');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoModeloDocumento_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipo Modelo Documento com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoModeloDocumento_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoModeloDocumento');
        }
    }

            public async filter(filtro: FilterTipoModeloDocumento): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoModeloDocumento_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoModeloDocumento');
                }
            }


    public async validation(regTipoModeloDocumento: ITipoModeloDocumento): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoModeloDocumento as TipoModeloDocumento, this.getHeaders());
    ...
    public async addAndUpdate(regTipoModeloDocumento: ITipoModeloDocumento): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Tipo Modelo Documento');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Tipo Modelo Documento com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoModeloDocumento) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<TipoModeloDocumento[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de tipomodelodocumento:', error);
            this.notificationService.notify({
              entity: 'TipoModeloDocumento',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tipomodelodocumento'
            });
          }
        });
    }
