// app/TestsForms/BensMateriaisComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import BensMateriaisComboBox from '@/app/GerAdv_TS/BensMateriais/ComboBox/BensMateriais';
...
import { BensMateriaisEmpty, BensMateriaisTestEmpty } from '@/app/GerAdv_TS/Models/BensMateriais';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/BensMateriais/Hooks/hookBensMateriais', () => ({
  useBensMateriaisComboBox: jest.fn(() => ({
    options: [
    { ...BensMateriaisEmpty(), id: 1, nome: 'Bens Materiais 1' },
    { ...BensMateriaisEmpty(), id: 2, nome: 'Bens Materiais 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/BensMateriais/Apis/ApiBensMateriais', () => ({
  BensMateriaisApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock BensMateriais' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock BensMateriais' }),
...
jest.mock('@/app/GerAdv_TS/BensMateriais/Services/BensMateriais.service', () => ({
  BensMateriaisService: jest.fn().mockImplementation(() => ({
    fetchBensMateriaisById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock BensMateriais' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/BensMateriais/Crud/Grids/BensMateriaisWindow', () => {
  return function MockBensMateriaisWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='bensmateriais-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Bens Materiais' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...BensMateriaisTestEmpty(), id: 1, nome: 'Bens Materiais 1' },
  { ...BensMateriaisTestEmpty(), id: 2, nome: 'Bens Materiais 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: BensMateriaisEmpty(), 
};
describe('BensMateriaisComboBox', () => {
  ...
  const { useBensMateriaisComboBox } = require('@/app/GerAdv_TS/BensMateriais/Hooks/hookBensMateriais');
  (useBensMateriaisComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<BensMateriaisComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Bens Materiais')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Bens Materiais')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...BensMateriaisTestEmpty(), id: 1, nome: 'Bens Materiais 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useBensMateriaisComboBox } = require('@/app/GerAdv_TS/BensMateriais/Hooks/hookBensMateriais');
    useBensMateriaisComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens BensMateriaisWindow on action button click (add)', async () => {
    render(<BensMateriaisComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('bensmateriais-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('bensmateriais-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<BensMateriaisComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles BensMateriaisWindow onClose and onSuccess', () => {
    render(<BensMateriaisComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<BensMateriaisComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
