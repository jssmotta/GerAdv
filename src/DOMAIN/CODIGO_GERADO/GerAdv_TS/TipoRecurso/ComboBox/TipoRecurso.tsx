...
const TipoRecursoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoRecurso | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoRecurso | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoRecurso);
    ...
    React.useEffect(() => {
        const unsubscribe = subscribeToNotifications("TipoRecurso", (entity) => {
      try {
                fetchDados();                
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoRecursoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoRecurso={editar ?? addValue ?? TipoRecursoEmpty()} /> 
            )}
