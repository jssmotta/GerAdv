...
public class FGUTPeriodicidadeFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FGUTPeriodicidadeFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FGUTPeriodicidadeFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FGUTPeriodicidadeFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "pcgCodigo")
        var dt = new DataTable();
        dt.Columns.Add("pcgCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["pcgCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FGUTPeriodicidade>();
    }
    ...
    {
        // Arrange
        var factory = new FGUTPeriodicidadeFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("pcgCodigo", typeof(int));
        mockDataTable.Columns.Add("pcgNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["pcgCodigo"] = testId;
        row["pcgNome"] = "Test GUTPeriodicidade";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["pcgCodigo"] = 123;
        row["pcgNome"] = "Test GUTPeriodicidade Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FGUTPeriodicidade properties
        }
    }
