  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("tiporecurso-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-tiporecurso .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-tiporecurso",
      columns: exportColumns,
      sheetName: "TipoRecurso",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-TipoRecurso-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-TipoRecurso-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
                ...
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      ...
                      key={`tiporecurso-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as ITipoRecurso);
