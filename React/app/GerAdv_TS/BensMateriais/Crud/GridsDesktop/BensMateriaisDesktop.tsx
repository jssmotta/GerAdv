...
import { useBensMateriaisGrid } from "../../Hooks/hookBensMateriaisGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("bensmateriais-columns-order"),
        );
        if (savedOrder) {
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        nomebensclassificacao: (
          <GridColumn
            key="nomebensclassificacao"
            field="nomebensclassificacao"
            title="Bens Classificacao"
            sortable={false}
            filterable={false}
          />
        ) /* Track G.04 */,
        ...
        nomefornecedores: (
          <GridColumn
            key="nomefornecedores"
            field="nomefornecedores"
            title="Fornecedor"
            sortable={false}
            filterable={false}
          />
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("bensmateriais-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-bensmateriais",
      columns: exportColumns,
      sheetName: "BensMateriais",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-BensMateriais-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-BensMateriais-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`bensmateriais-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-bensmateriais"
