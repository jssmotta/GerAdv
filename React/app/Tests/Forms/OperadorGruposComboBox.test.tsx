// app/TestsForms/OperadorGruposComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import OperadorGruposComboBox from '@/app/GerAdv_TS/OperadorGrupos/ComboBox/OperadorGrupos';
...
import { OperadorGruposEmpty, OperadorGruposTestEmpty } from '@/app/GerAdv_TS/Models/OperadorGrupos';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/OperadorGrupos/Hooks/hookOperadorGrupos', () => ({
  useOperadorGruposComboBox: jest.fn(() => ({
    options: [
    { ...OperadorGruposEmpty(), id: 1, nome: 'Operador Grupos 1' },
    { ...OperadorGruposEmpty(), id: 2, nome: 'Operador Grupos 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/OperadorGrupos/Apis/ApiOperadorGrupos', () => ({
  OperadorGruposApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock OperadorGrupos' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock OperadorGrupos' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/OperadorGrupos/Services/OperadorGrupos.service', () => ({
  OperadorGruposService: jest.fn().mockImplementation(() => ({
    fetchOperadorGruposById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock OperadorGrupos' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/OperadorGrupos/Crud/Grids/OperadorGruposWindow', () => {
  return function MockOperadorGruposWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='operadorgrupos-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Operador Grupos' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...OperadorGruposTestEmpty(), id: 1, nome: 'Operador Grupos 1' },
  { ...OperadorGruposTestEmpty(), id: 2, nome: 'Operador Grupos 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Operador Grupos',
  name: 'operadorgrupos',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: OperadorGruposEmpty(), 
};
describe('OperadorGruposComboBox', () => {
  ...
  const { useOperadorGruposComboBox } = require('@/app/GerAdv_TS/OperadorGrupos/Hooks/hookOperadorGrupos');
  (useOperadorGruposComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<OperadorGruposComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Operador Grupos')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Operador Grupos')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...OperadorGruposTestEmpty(), id: 1, nome: 'Operador Grupos 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useOperadorGruposComboBox } = require('@/app/GerAdv_TS/OperadorGrupos/Hooks/hookOperadorGrupos');
    useOperadorGruposComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens OperadorGruposWindow on action button click (add)', async () => {
    render(<OperadorGruposComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('operadorgrupos-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('operadorgrupos-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<OperadorGruposComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles OperadorGruposWindow onClose and onSuccess', () => {
    render(<OperadorGruposComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<OperadorGruposComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
