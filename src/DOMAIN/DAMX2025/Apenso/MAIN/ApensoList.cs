...
#region ListarDados_Apenso
    public static async IAsyncEnumerable<DBApenso> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBApenso(dbRec);
        }
    }

    public static IEnumerable<DBApenso> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
