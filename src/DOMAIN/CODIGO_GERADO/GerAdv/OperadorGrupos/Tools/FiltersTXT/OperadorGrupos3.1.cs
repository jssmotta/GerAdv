...
public static class FilterOperadorGruposTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar OperadorGrupos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterOperadorGrupos:

- O filter irá filtrar os OperadorGrupos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterOperadorGrupos:(Operador Grupos)
- nome: _nome ou descrição para Operador Grupos;
...
- Se ouvir "todos os OperadorGrupos" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterOperadorGrupos? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt OperadorGrupos length: {result.Length}");
#endif
