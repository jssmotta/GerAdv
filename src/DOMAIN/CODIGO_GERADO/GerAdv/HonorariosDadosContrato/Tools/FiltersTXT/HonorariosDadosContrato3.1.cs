...
public static class FilterHonorariosDadosContratoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterHonorariosDadosContrato? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar HonorariosDadosContrato em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterHonorariosDadosContrato:

- O filter irá filtrar os HonorariosDadosContrato conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterHonorariosDadosContrato:(Honorarios Dados Contrato)
- cliente: número ou IDs;
- fixo: Fixo;
- variavel: Variavel;
- percsucesso: valores monetários ou com decimais;
- processo: número ou IDs;
- arquivocontrato: ArquivoContrato;
- textocontrato: TextoContrato;
- valorfixo: valores monetários ou com decimais;
- observacao: Observacao;
...
- Se ouvir "todos os HonorariosDadosContrato" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt HonorariosDadosContrato length: {result.Length}");
#endif
