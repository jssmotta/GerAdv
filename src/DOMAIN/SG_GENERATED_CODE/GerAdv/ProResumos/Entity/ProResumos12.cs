...
public partial class FProResumos : MenphisSI.SG.GerAdv.DBProResumos, IDBProResumos
{
    // DI-compatible default constructor
    public FProResumos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FProResumos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FProResumos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FProResumos CreateFromDataRow(DataRow? dbRec)
    {
        return new FProResumos(dbRec);
    }

    private FProResumos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FProResumos(DataRow? dbRec) : base(dbRec)
    {
    }

    private FProResumos(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FProResumos CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FProResumos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FProResumos> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fProResumos = new FProResumos();
        await fProResumos.CarregarAsync(id, oCnn);
        return fProResumos;
    }
