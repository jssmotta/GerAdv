...
public class FPenhoraStatusFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FPenhoraStatusFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FPenhoraStatusFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FPenhoraStatusFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "phsCodigo")
        var dt = new DataTable();
        dt.Columns.Add("phsCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["phsCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FPenhoraStatus>();
    }
    ...
    {
        // Arrange
        var factory = new FPenhoraStatusFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("phsCodigo", typeof(int));
        mockDataTable.Columns.Add("phsNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["phsCodigo"] = testId;
        row["phsNome"] = "Test PenhoraStatus";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["phsCodigo"] = 123;
        row["phsNome"] = "Test PenhoraStatus Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FPenhoraStatus properties
        }
    }
