...
import { BensClassificacaoApiError } from '../GerAdv_TS/BensClassificacao/Apis/ApiBensClassificacao';
import { BensClassificacaoApi } from '@/app/GerAdv_TS/BensClassificacao/Apis/ApiBensClassificacao';
import { FilterBensClassificacao } from '../GerAdv_TS/BensClassificacao/Filters/BensClassificacao';
...
describe('BensClassificacaoApi', () => {
  let bensclassificacaoApi: BensClassificacaoApi;
  ...
  beforeEach(() => {
    ...
    bensclassificacaoApi = new BensClassificacaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(bensclassificacaoApi).toBeInstanceOf(BensClassificacaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new BensClassificacaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(BensClassificacaoApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [BensClassificacaoTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all bensclassificacao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await bensclassificacaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(bensclassificacaoApi.getAll()).rejects.toThrow(BensClassificacaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await bensclassificacaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-BensClassificacao_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensClassificacao',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw BensClassificacaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockBensClassificacao = BensClassificacaoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockBensClassificacao,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch bensclassificacao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await bensclassificacaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockBensClassificacao));
      ...
      expect(result.data).toEqual(mockBensClassificacao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensClassificacao',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterBensClassificacao = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter bensclassificacao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await bensclassificacaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockBensClassificacao: IBensClassificacao = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new bensclassificacao successfully', async () => {
      const responseBensClassificacao = { ...mockBensClassificacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseBensClassificacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await bensclassificacaoApi.addAndUpdate(mockBensClassificacao);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockBensClassificacao,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing bensclassificacao successfully', async () => {
      const existingBensClassificacao = { ...mockBensClassificacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingBensClassificacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await bensclassificacaoApi.addAndUpdate(existingBensClassificacao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(bensclassificacaoApi.addAndUpdate(mockBensClassificacao)).rejects.toThrow(BensClassificacaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensClassificacao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete bensclassificacao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await bensclassificacaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(bensclassificacaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'BensClassificacao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(bensclassificacaoApi.delete(1)).rejects.toThrow(BensClassificacaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterBensClassificacao = { funcionario: 1 };
      
      const result = bensclassificacaoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await bensclassificacaoApi.delete(1);
  ...
  describe('BensClassificacaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new BensClassificacaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('BensClassificacaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new BensClassificacaoApiError('Test message', 500);
