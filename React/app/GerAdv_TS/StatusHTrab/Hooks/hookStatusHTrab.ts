...
export const useStatusHTrabForm = (
  ...
  const loadStatusHTrab = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Status H Trab";
...
export const useStatusHTrabList = (
  dataService: IStatusHTrabService,
  currentFilter?: FilterStatusHTrab | undefined | null,
) => {
  const [data, setData] = useState<IStatusHTrab[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterStatusHTrab | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterStatusHTrab | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar statushtrab";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
