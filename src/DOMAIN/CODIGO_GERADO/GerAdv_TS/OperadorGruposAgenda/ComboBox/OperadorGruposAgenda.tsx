...
const OperadorGruposAgendaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOperadorGruposAgenda | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOperadorGruposAgenda | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOperadorGruposAgenda);
    ...
    const handleAddClick = () => {
        ...
        const add = OperadorGruposAgendaEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OperadorGruposAgendaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOperadorGruposAgenda={editar ?? addValue ?? OperadorGruposAgendaEmpty()} /> 
            )}
