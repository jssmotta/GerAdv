...
public partial class EscritoriosController(IEscritoriosService escritoriosService) : ControllerBase
{
    ...
    {
        _logger.Info("Escritorios: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _escritoriosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Escritorios: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _escritoriosService.UpdateColumns(parameters, uri);
        if (!result)
        {
