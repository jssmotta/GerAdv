...
export const useContatoCRMOperadorList = (dataService: IContatoCRMOperadorService, currentFilter?: FilterContatoCRMOperador | undefined | null) => {
  const [data, setData] = useState<IContatoCRMOperador[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterContatoCRMOperador | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterContatoCRMOperador | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterContatoCRMOperador);
      setData(result);
    } catch (err) {
