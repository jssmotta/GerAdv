...
export const useParteClienteOutrasList = (dataService: IParteClienteOutrasService, currentFilter?: FilterParteClienteOutras | undefined | null) => {
  const [data, setData] = useState<IParteClienteOutras[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterParteClienteOutras | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterParteClienteOutras | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterParteClienteOutras);
      setData(result);
    } catch (err) {
