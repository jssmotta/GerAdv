...
export const useTipoEnderecoSistemaForm = (
  ...
  const loadTipoEnderecoSistema = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoEnderecoSistema);
      return;
    }
  ...
  }, [dataService, initialTipoEnderecoSistema]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoEnderecoSistema, resetForm]);
  return returnValue;
};


export const useTipoEnderecoSistemaNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('TipoEnderecoSistema', (entity) => {
      try {
...
export const useTipoEnderecoSistemaList = (dataService: ITipoEnderecoSistemaService) => {
  const [data, setData] = useState<ITipoEnderecoSistema[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  useTipoEnderecoSistemaNotifications(
