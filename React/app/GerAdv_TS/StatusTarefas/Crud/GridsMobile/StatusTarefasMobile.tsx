...
import { useStatusTarefasGrid } from "../../Hooks/hookStatusTarefasGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("statustarefas-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-statustarefas .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("statustarefas-columns-order-mobile"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-statustarefas",
      columns: exportColumns,
      sheetName: "StatusTarefas",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-StatusTarefas-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-StatusTarefas-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-statustarefas"
              key={`statustarefas-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
