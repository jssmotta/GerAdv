// app/TestsForms/StatusBiuComboBox.test.tsx - Vers�o Corrigida
...
import { StatusBiuEmpty, StatusBiuTestEmpty } from '@/app/GerAdv_TS/Models/StatusBiu';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/StatusBiu/Hooks/hookStatusBiu', () => ({
  useStatusBiuComboBox: jest.fn(() => ({
    options: [
    { ...StatusBiuEmpty(), id: 1, nome: 'Status Biu 1' },
    { ...StatusBiuEmpty(), id: 2, nome: 'Status Biu 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/StatusBiu/Apis/ApiStatusBiu', () => ({
  StatusBiuApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock StatusBiu' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock StatusBiu' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/StatusBiu/Services/StatusBiu.service', () => ({
  StatusBiuService: jest.fn().mockImplementation(() => ({
    fetchStatusBiuById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock StatusBiu' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/StatusBiu/Crud/Grids/StatusBiuWindow', () => {
  return function MockStatusBiuWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='statusbiu-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Status Biu' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...StatusBiuTestEmpty(), id: 1, nome: 'Status Biu 1' },
  { ...StatusBiuTestEmpty(), id: 2, nome: 'Status Biu 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: StatusBiuEmpty(), 
};
describe('StatusBiuComboBox', () => {
  ...
  const { useStatusBiuComboBox } = require('@/app/GerAdv_TS/StatusBiu/Hooks/hookStatusBiu');
  (useStatusBiuComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<StatusBiuComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Status Biu')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Status Biu')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...StatusBiuTestEmpty(), id: 1, nome: 'Status Biu 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useStatusBiuComboBox } = require('@/app/GerAdv_TS/StatusBiu/Hooks/hookStatusBiu');
    useStatusBiuComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens StatusBiuWindow on action button click (add)', async () => {
    render(<StatusBiuComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('statusbiu-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('statusbiu-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<StatusBiuComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles StatusBiuWindow onClose and onSuccess', () => {
    render(<StatusBiuComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<StatusBiuComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
