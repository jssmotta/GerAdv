...
// useTribEnderecosGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de TribEnderecos
'use client';
...
interface UseTribEnderecosGridProps {
  data: ITribEnderecos[];
...
export const useTribEnderecosGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('tribenderecos-columns-order'), JSON.stringify(reorderableFields));
