...
public partial class GUTAtividadesWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_GUTAtividades_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[GUTAtividades]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_GUTAtividades_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[GUTAtividades]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_GUTAtividades_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_GUTAtividades_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[GUTAtividades]'))
                BEGIN                    
