...
export const usePenhoraStatusForm = (
  ...
  const loadPenhoraStatus = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Penhora Status";
...
export const usePenhoraStatusList = (
  dataService: IPenhoraStatusService,
  currentFilter?: FilterPenhoraStatus | undefined | null,
) => {
  const [data, setData] = useState<IPenhoraStatus[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPenhoraStatus | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterPenhoraStatus | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar penhorastatus";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsPenhoraStatus() {
  ...
  function validate(data: IPenhoraStatus): {
    isValid: boolean;
    message: string;
  } {
