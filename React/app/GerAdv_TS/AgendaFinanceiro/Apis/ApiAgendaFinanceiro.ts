...
export class AgendaFinanceiroApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        public async getListN(max?: number, filtro?: FilterAgendaFinanceiro): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaFinanceiro_last_listN_data`);
        ...
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar lista de AgendaFinanceiro');
        }
    }
 public async filterPreload(filtro: FilterAgendaFinanceiro): Promise<AxiosResponse> {
    ...
    public useFilter(filtro: FilterAgendaFinanceiro) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AgendaFinanceiro[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de AgendaFinanceiro');
            this.notificationService.notify({
              entity: 'AgendaFinanceiro',
              id: 0,
              action: NotifySystemActions.ERROR,
