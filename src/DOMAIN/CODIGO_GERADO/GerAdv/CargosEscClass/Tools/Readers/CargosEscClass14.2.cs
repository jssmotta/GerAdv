...
public partial class CargosEscClassWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_CargosEscClass_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[CargosEscClass]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_CargosEscClass_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[CargosEscClass]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_CargosEscClass_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_CargosEscClass_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[CargosEscClass]'))
                BEGIN                    
