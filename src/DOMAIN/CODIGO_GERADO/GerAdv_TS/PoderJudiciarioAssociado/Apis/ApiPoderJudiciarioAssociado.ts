    ...
    private handleApiError(error: any, context: string): never {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterPoderJudiciarioAssociado): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PoderJudiciarioAssociado_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterPoderJudiciarioAssociado) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<PoderJudiciarioAssociado[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de poderjudiciarioassociado:', error);
            this.notificationService.notify({
              entity: 'PoderJudiciarioAssociado',
              id: 0,
              action: NotifySystemActions.ERROR,
