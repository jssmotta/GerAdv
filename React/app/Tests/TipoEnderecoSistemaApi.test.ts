...
import { TipoEnderecoSistemaApiError } from '../GerAdv_TS/TipoEnderecoSistema/Apis/ApiTipoEnderecoSistema';
import { TipoEnderecoSistemaApi } from '@/app/GerAdv_TS/TipoEnderecoSistema/Apis/ApiTipoEnderecoSistema';
import { FilterTipoEnderecoSistema } from '../GerAdv_TS/TipoEnderecoSistema/Filters/TipoEnderecoSistema';
...
describe('TipoEnderecoSistemaApi', () => {
  let tipoenderecosistemaApi: TipoEnderecoSistemaApi;
  ...
  beforeEach(() => {
    ...
    tipoenderecosistemaApi = new TipoEnderecoSistemaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipoenderecosistemaApi).toBeInstanceOf(TipoEnderecoSistemaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoEnderecoSistemaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoEnderecoSistemaApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [TipoEnderecoSistemaTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all tipoenderecosistema items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoenderecosistemaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipoenderecosistemaApi.getAll()).rejects.toThrow(TipoEnderecoSistemaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipoenderecosistemaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoEnderecoSistema_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEnderecoSistema',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw TipoEnderecoSistemaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoEnderecoSistema = TipoEnderecoSistemaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoEnderecoSistema,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipoenderecosistema by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoenderecosistemaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoEnderecoSistema));
      ...
      expect(result.data).toEqual(mockTipoEnderecoSistema);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEnderecoSistema',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoEnderecoSistema = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipoenderecosistema items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipoenderecosistemaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoEnderecoSistema: ITipoEnderecoSistema = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipoenderecosistema successfully', async () => {
      const responseTipoEnderecoSistema = { ...mockTipoEnderecoSistema, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoEnderecoSistema,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoenderecosistemaApi.addAndUpdate(mockTipoEnderecoSistema);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockTipoEnderecoSistema,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing tipoenderecosistema successfully', async () => {
      const existingTipoEnderecoSistema = { ...mockTipoEnderecoSistema, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoEnderecoSistema,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoenderecosistemaApi.addAndUpdate(existingTipoEnderecoSistema);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipoenderecosistemaApi.addAndUpdate(mockTipoEnderecoSistema)).rejects.toThrow(TipoEnderecoSistemaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEnderecoSistema',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipoenderecosistema successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipoenderecosistemaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipoenderecosistemaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEnderecoSistema',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipoenderecosistemaApi.delete(1)).rejects.toThrow(TipoEnderecoSistemaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoEnderecoSistema = { funcionario: 1 };
      
      const result = tipoenderecosistemaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipoenderecosistemaApi.delete(1);
  ...
  describe('TipoEnderecoSistemaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoEnderecoSistemaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoEnderecoSistemaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoEnderecoSistemaApiError('Test message', 500);
