...
public static class FilterProcessOutputEngineTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProcessOutputEngine em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProcessOutputEngine:

- O filter irá filtrar os ProcessOutputEngine conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProcessOutputEngine:(Process Output Engine)
- nome: _nome ou descrição para Process Output Engine;
- database: Database;
- tabela: Tabela;
- campo: Campo;
- valor: Valor;
...
- outputsource: número ou IDs;
- disableditem: DisabledItem;
- idmodulo: número ou IDs;
...
- Se ouvir "todos os ProcessOutputEngine" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProcessOutputEngine? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProcessOutputEngine length: {result.Length}");
#endif
