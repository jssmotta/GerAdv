// app/TestsForms/GruposEmpresasComboBox.test.tsx - Vers�o Corrigida
...
import { GruposEmpresasEmpty, GruposEmpresasTestEmpty } from '@/app/GerAdv_TS/Models/GruposEmpresas';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/GruposEmpresas/Hooks/hookGruposEmpresas', () => ({
  useGruposEmpresasComboBox: jest.fn(() => ({
    options: [
    { ...GruposEmpresasEmpty(), id: 1, descricao: 'Grupos Empresas 1' },
    { ...GruposEmpresasEmpty(), id: 2, descricao: 'Grupos Empresas 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/GruposEmpresas/Apis/ApiGruposEmpresas', () => ({
  GruposEmpresasApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GruposEmpresas' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GruposEmpresas' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/GruposEmpresas/Services/GruposEmpresas.service', () => ({
  GruposEmpresasService: jest.fn().mockImplementation(() => ({
    fetchGruposEmpresasById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GruposEmpresas' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/GruposEmpresas/Crud/Grids/GruposEmpresasWindow', () => {
  return function MockGruposEmpresasWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='gruposempresas-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'Grupos Empresas' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...GruposEmpresasTestEmpty(), id: 1, descricao: 'Grupos Empresas 1' },
  { ...GruposEmpresasTestEmpty(), id: 2, descricao: 'Grupos Empresas 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: GruposEmpresasEmpty(), 
};
describe('GruposEmpresasComboBox', () => {
  ...
  const { useGruposEmpresasComboBox } = require('@/app/GerAdv_TS/GruposEmpresas/Hooks/hookGruposEmpresas');
  (useGruposEmpresasComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<GruposEmpresasComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Grupos Empresas')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Grupos Empresas')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...GruposEmpresasTestEmpty(), id: 1, descricao: 'Grupos Empresas 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useGruposEmpresasComboBox } = require('@/app/GerAdv_TS/GruposEmpresas/Hooks/hookGruposEmpresas');
    useGruposEmpresasComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens GruposEmpresasWindow on action button click (add)', async () => {
    render(<GruposEmpresasComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('gruposempresas-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('gruposempresas-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<GruposEmpresasComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles GruposEmpresasWindow onClose and onSuccess', () => {
    render(<GruposEmpresasComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<GruposEmpresasComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
