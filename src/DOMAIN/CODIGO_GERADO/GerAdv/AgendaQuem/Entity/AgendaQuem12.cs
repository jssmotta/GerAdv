...
public partial class FAgendaQuem : MenphisSI.SG.GerAdv.DBAgendaQuem, IDBAgendaQuem
{
    // DI-compatible default constructor
    public FAgendaQuem() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FAgendaQuem CreateFromReader(SqlDataReader? dbRec)
    {
        return new FAgendaQuem(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FAgendaQuem CreateFromDataRow(DataRow? dbRec)
    {
        return new FAgendaQuem(dbRec);
    }

    private FAgendaQuem(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FAgendaQuem(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FAgendaQuem> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fAgendaQuem = new FAgendaQuem();
        await fAgendaQuem.CarregarAsync(id, oCnn);
        return fAgendaQuem;
    }

    private FAgendaQuem(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FAgendaQuem CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FAgendaQuem(parameters, oCnn, fullSql, sqlWhere, join);
    }
