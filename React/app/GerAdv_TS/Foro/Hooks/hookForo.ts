...
export const useForoForm = (initialForo: IForo, dataService: IForoService) => {
  ...
  const loadForo = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Foro";
...
export const useForoList = (
  dataService: IForoService,
  currentFilter?: FilterForo | undefined | null,
) => {
  const [data, setData] = useState<IForo[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterForo | undefined | null>(currentFilter);
  ...
  const fetchData = useCallback(
    async (filtro?: FilterForo | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterForo));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar foro";
  ...
  function validate(data: IForo): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.bairro.length > 255) {
        return {
          isValid: false,
          message: "O campo Bairro não pode ter mais de 255 caracteres.",
        };
      }
      ...
      if (data.web.length > 255) {
        return {
          isValid: false,
          message: "O campo Web não pode ter mais de 255 caracteres.",
        };
      }
