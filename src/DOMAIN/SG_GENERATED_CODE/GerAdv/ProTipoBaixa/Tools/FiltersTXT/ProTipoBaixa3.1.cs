...
public static class FilterProTipoBaixaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProTipoBaixa em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProTipoBaixa:

- O filter irá filtrar os ProTipoBaixa conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProTipoBaixa:(Pro Tipo Baixa)
- nome: _nome ou descrição para Pro Tipo Baixa;
- bold: Bold;
...
- Se ouvir "todos os ProTipoBaixa" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProTipoBaixa? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProTipoBaixa length: {result.Length}");
#endif
