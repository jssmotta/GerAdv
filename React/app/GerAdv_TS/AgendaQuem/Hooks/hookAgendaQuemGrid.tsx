...
// useAgendaQuemGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de AgendaQuem
'use client';
...
interface UseAgendaQuemGridProps {
  data: IAgendaQuem[];
...
export const useAgendaQuemGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('agendaquem-columns-order'), JSON.stringify(reorderableFields));
