...
// useProcessosObsReportGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de ProcessosObsReport
"use client";
import { useCallback, useMemo, useState, useEffect } from "react";
import { SortDescriptor } from "@progress/kendo-data-query";
import { IProcessosObsReport } from "../Interfaces/interface.ProcessosObsReport";
import { applyFilter, formatCurrency, CRUD_CONSTANTS } from "@/app/tools/crud";
...
interface UseProcessosObsReportGridProps {
...
export const useProcessosObsReportGrid = ({
  data,
  onRowClick,
  onDeleteClick,
  setSelectedId,
  ...
  useCustomSort = false,
}: UseProcessosObsReportGridProps) => {
  // ===== GERENCIAMENTO DE REORDENAÇÃO DE COLUNAS =====
  const [columnsOrder, setColumnsOrder] = useState<string[]>([]);

  // Definir ordem padrão das colunas
  const defaultColumnsOrder = useMemo(() => ["data", "nropastaprocessos"], []);
  ...
  const handleColumnsReorder = useCallback(
    (event: any) => {
      ...
      const reorderableFields = event.columns
        .filter(
          (col: any) =>
            col.field && !["index", "id_delete_row"].includes(col.field),
        )
        .map((col: any) => col.field);

      if (
        reorderableFields.length > 0 &&
        JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)
      ) {
        ...
        try {
          localStorage.setItem(
            ...
            JSON.stringify(reorderableFields),
          );
        } catch (error) {}
      }
    },
  ...
  const filterLogic = useCallback(
    (data: IProcessosObsReport, filters: Record<string, any>) => {
      const dataMatches = applyFilter(data, "data_date", filters.data);
  ...
  const {
    sort: mobileSort,
    sortedData: mobileSortedData,
    handleSortChange: mobileHandleSortChange,
  } = useGridSort({
    data: filteredData,
  });
  ...
  const handleDesktopSortChange = useCallback(
    (e: { sort: SortDescriptor[] }) => {
  ...
  const RowNumberCell = useCallback(
    (props: any) => <td>{props.dataIndex + 1}</td>,
    [],
  );
  ...
  const DeleteRow = useCallback(
    (e: any) => {
      return (
        <td>
          <span onClick={() => onDeleteClick(e)} title="Excluir item">
  ...
  return {
    ...
    handleSortChange: useCustomSort
      ? handleDesktopSortChange
      : mobileHandleSortChange,
    initialSort,
