...
import { TipoEMailApiError } from '../GerAdv_TS/TipoEMail/Apis/ApiTipoEMail';
import { TipoEMailApi } from '@/app/GerAdv_TS/TipoEMail/Apis/ApiTipoEMail';
import { FilterTipoEMail } from '../GerAdv_TS/TipoEMail/Filters/TipoEMail';
...
describe('TipoEMailApi', () => {
  let tipoemailApi: TipoEMailApi;
  ...
  beforeEach(() => {
    ...
    tipoemailApi = new TipoEMailApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipoemailApi).toBeInstanceOf(TipoEMailApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoEMailApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoEMailApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tipoemail items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoemailApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipoemailApi.getAll()).rejects.toThrow(TipoEMailApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipoemailApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoEMail_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoEMailApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoEMail = TipoEMailTestEmpty();
    ...
    it('should fetch tipoemail by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoemailApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoEMail));
      ...
      expect(result.data).toEqual(mockTipoEMail);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEMail',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoEMail = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipoemail items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipoemailApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoEMail: ITipoEMail = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipoemail successfully', async () => {
      const responseTipoEMail = { ...mockTipoEMail, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoEMail,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoemailApi.addAndUpdate(mockTipoEMail);
    ...
    it('should update existing tipoemail successfully', async () => {
      const existingTipoEMail = { ...mockTipoEMail, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoEMail,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoemailApi.addAndUpdate(existingTipoEMail);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipoemailApi.addAndUpdate(mockTipoEMail)).rejects.toThrow(TipoEMailApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEMail',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipoemail successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipoemailApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipoemailApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEMail',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipoemailApi.delete(1)).rejects.toThrow(TipoEMailApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoEMail = { funcionario: 1 };
      
      const result = tipoemailApi.useFilter(mockFilter);
  ...
  describe('TipoEMailApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoEMailApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoEMailApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoEMailApiError('Test message', 500);
