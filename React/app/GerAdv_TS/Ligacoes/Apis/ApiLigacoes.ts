...
export class LigacoesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class LigacoesApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Ligacoes`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Ligacoes_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Ligacoes');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Ligacoes_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Ligacoes com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Ligacoes_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Ligacoes');
        }
    }

            public async filter(filtro: FilterLigacoes): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Ligacoes_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Ligacoes');
                }
            }


    public async validation(regLigacoes: ILigacoes): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regLigacoes as Ligacoes, this.getHeaders());
    ...
    public async addAndUpdate(regLigacoes: ILigacoes): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Ligacoes');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Ligacoes. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Ligacoes',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Ligacoes com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterLigacoes) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Ligacoes[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de ligacoes:', error);
            this.notificationService.notify({
              entity: 'Ligacoes',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos ligacoes'
            });
          }
        });
    }
