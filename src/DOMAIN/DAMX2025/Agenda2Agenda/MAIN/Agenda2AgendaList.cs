...
#region ListarDados_Agenda2Agenda
    public static async IAsyncEnumerable<DBAgenda2Agenda> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAgenda2Agenda(dbRec);
        }
    }

    public static IEnumerable<DBAgenda2Agenda> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
