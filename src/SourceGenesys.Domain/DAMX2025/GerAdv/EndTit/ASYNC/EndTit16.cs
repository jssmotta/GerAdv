...
public partial class DBEndTit
{
    ...
    /// </summary>
    public static async Task<DBEndTit> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBEndTit();
...
#if (!NOTSTORED_EndTit)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFEndereco || pFldFTitulo;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFEndereco)
            updateTool.Fields(DBEndTitDicInfo.Endereco, m_FEndereco, ETiposCampos.FNumber);
        if (pFldFTitulo)
            updateTool.Fields(DBEndTitDicInfo.Titulo, m_FTitulo, ETiposCampos.FNumber);
    }
