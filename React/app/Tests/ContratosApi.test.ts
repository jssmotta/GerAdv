...
describe('ContratosApi', () => {
  let contratosApi: ContratosApi;
  ...
  beforeEach(() => {
    ...
    contratosApi = new ContratosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(contratosApi).toBeInstanceOf(ContratosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ContratosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ContratosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all contratos items successfully', async () => {
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(contratosApi.getAll()).rejects.toThrow(ContratosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await contratosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Contratos_last_getAll_${200}`)));
    });
    ...
    it('should throw ContratosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockContratos = ContratosTestEmpty();
    ...
    it('should fetch contratos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contratosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockContratos));
  ...
  describe('filter', () => {
    ...
    it('should filter contratos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await contratosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new contratos successfully', async () => {
      const responseContratos = { ...mockContratos, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await contratosApi.addAndUpdate(mockContratos);
    ...
    it('should update existing contratos successfully', async () => {
      const existingContratos = { ...mockContratos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingContratos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contratosApi.addAndUpdate(existingContratos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(contratosApi.addAndUpdate(mockContratos)).rejects.toThrow(ContratosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Contratos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete contratos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await contratosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(contratosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Contratos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(contratosApi.delete(1)).rejects.toThrow(ContratosApiError);
    });
  });
  ...
  describe('ContratosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ContratosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ContratosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ContratosApiError('Test message', 500);
