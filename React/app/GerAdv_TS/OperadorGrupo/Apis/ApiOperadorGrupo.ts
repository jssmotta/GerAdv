...
export class OperadorGrupoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'OperadorGrupoApiError';
    this.status = status;
    this.code = code;
...
export class OperadorGrupoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/OperadorGrupo`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupo_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os OperadorGrupo');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupo_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Operador Grupo com ID ${id}`);
        }
    }

            public async filter(filtro: FilterOperadorGrupo): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupo_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar OperadorGrupo');
                }
            }


    public async validation(regOperadorGrupo: IOperadorGrupo): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regOperadorGrupo as OperadorGrupo, this.getHeaders());
    ...
    public async addAndUpdate(regOperadorGrupo: IOperadorGrupo): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Operador Grupo');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o OperadorGrupo. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'OperadorGrupo',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Operador Grupo com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterOperadorGrupo) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<OperadorGrupo[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de operadorgrupo:', error);
            this.notificationService.notify({
              entity: 'OperadorGrupo',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos operadorgrupo'
            });
          }
        });
    }
