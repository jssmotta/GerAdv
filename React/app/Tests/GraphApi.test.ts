...
describe('GraphApi', () => {
  ...
  beforeEach(() => {
    ...
    graphApi = new GraphApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(graphApi).toBeInstanceOf(GraphApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GraphApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GraphApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all graph items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await graphApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(graphApi.getAll()).rejects.toThrow(GraphApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Graph_last_getAll_${200}`)));
    });
    ...
    it('should throw GraphApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGraph = GraphTestEmpty();
    ...
    it('should fetch graph by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await graphApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGraph));
  ...
  describe('filter', () => {
    ...
    it('should filter graph items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await graphApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new graph successfully', async () => {
      const responseGraph = { ...mockGraph, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGraph,
        status: 200,
        statusText: 'OK',
      ...
      const result = await graphApi.addAndUpdate(mockGraph);
    ...
    it('should update existing graph successfully', async () => {
      const existingGraph = { ...mockGraph, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGraph,
        status: 200,
        statusText: 'OK',
      ...
      const result = await graphApi.addAndUpdate(existingGraph);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(graphApi.addAndUpdate(mockGraph)).rejects.toThrow(GraphApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Graph',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete graph successfully', async () => {
      ...
      const result = await graphApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(graphApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Graph',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(graphApi.delete(1)).rejects.toThrow(GraphApiError);
    });
  });
  ...
  describe('GraphApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GraphApiError('Test message', 400, 'TEST_CODE', 'original');
    ...
    it('should create error without optional parameters', () => {
      const error = new GraphApiError('Test message', 500);
