...
#region ListarDados_Escritorios
    public static async IAsyncEnumerable<DBEscritorios> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEscritorios(dbRec);
        }
    }

    public static IEnumerable<DBEscritorios> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
