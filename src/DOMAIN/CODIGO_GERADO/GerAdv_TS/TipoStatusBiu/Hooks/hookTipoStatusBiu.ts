...
export const useTipoStatusBiuForm = (
  ...
  const loadTipoStatusBiu = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoStatusBiu);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Staus  Usuários';
      setError(errorMessage);
      console.log('Erro ao carregar Staus  Usuários');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialTipoStatusBiu]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoStatusBiu, resetForm]);
  return returnValue;
};


export const useTipoStatusBiuNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('TipoStatusBiu', (entity) => {
      try {
...
export const useTipoStatusBiuList = (dataService: ITipoStatusBiuService) => {
  const [data, setData] = useState<ITipoStatusBiu[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar tipostatusbiu';
      setError(errorMessage);
      console.log('Erro ao carregar tipostatusbiu');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
