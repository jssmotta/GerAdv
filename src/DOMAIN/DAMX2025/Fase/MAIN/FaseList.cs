...
#region ListarDados_Fase
    public static async IAsyncEnumerable<DBFase> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBFase(dbRec);
        }
    }

    public static IEnumerable<DBFase> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
