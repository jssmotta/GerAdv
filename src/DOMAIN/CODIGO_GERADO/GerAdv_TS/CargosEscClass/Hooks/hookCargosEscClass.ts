...
export const useCargosEscClassForm = (
  ...
  const loadCargosEscClass = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialCargosEscClass);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Cargos Esc Class';
      setError(errorMessage);
      console.log('Erro ao carregar Cargos Esc Class');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialCargosEscClass]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadCargosEscClass, resetForm]);
  return returnValue;
};


export const useCargosEscClassNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('CargosEscClass', (entity) => {
      try {
...
export const useCargosEscClassList = (dataService: ICargosEscClassService) => {
  const [data, setData] = useState<ICargosEscClass[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar cargosescclass';
      setError(errorMessage);
      console.log('Erro ao carregar cargosescclass');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useCargosEscClassNotifications(
