...
public partial class DBStatusBiu : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_StatusBiu)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBStatusBiu();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_StatusBiu
    [XmlIgnore]
    public string TabelaNome => "StatusBiu";
...
#endregion
    public DBStatusBiu(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBStatusBiu(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBStatusBiu(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_StatusBiu
...
#endif
    {
        ...
        if (!isInsert)
            if (!(pFldFNome || pFldFTipoStatusBiu || pFldFOperador || pFldFIcone))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
        ...
        {
            throw new Exception("Campo 'stbNome' está vazio!");
        }
        ...
        if (pFldFNome)
            clsW.Fields(DBStatusBiuDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFTipoStatusBiu)
            clsW.Fields(DBStatusBiuDicInfo.TipoStatusBiu, m_FTipoStatusBiu, ETiposCampos.FNumberNull);
        if (pFldFOperador)
            clsW.Fields(DBStatusBiuDicInfo.Operador, m_FOperador, ETiposCampos.FNumberNull);
        if (pFldFIcone)
            clsW.Fields(DBStatusBiuDicInfo.Icone, m_FIcone, ETiposCampos.FNumberNull);
        if (insertId != 0)
            return GravaNewId();
