...
const TipoCompromissoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoCompromisso | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoCompromisso | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoCompromisso);
    ...
    const handleAddClick = () => {
        ...
        const add = TipoCompromissoEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoCompromissoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoCompromisso={editar ?? addValue ?? TipoCompromissoEmpty()} /> 
            )}
