...
public partial class DBParceriaProc
{
    ...
    {
        if (pFldFAdvogado)
            updateTool.Fields(DBParceriaProcDicInfo.Advogado, m_FAdvogado, ETiposCampos.FNumber);
        if (pFldFProcesso)
            updateTool.Fields(DBParceriaProcDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBParceriaProcDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBParceriaProcDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
