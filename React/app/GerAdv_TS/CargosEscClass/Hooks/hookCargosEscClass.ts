...
export const useCargosEscClassForm = (
  ...
  const loadCargosEscClass = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Cargos Esc Class";
...
export const useCargosEscClassList = (
  dataService: ICargosEscClassService,
  currentFilter?: FilterCargosEscClass | undefined | null,
) => {
  const [data, setData] = useState<ICargosEscClass[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterCargosEscClass | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterCargosEscClass | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar cargosescclass";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsCargosEscClass() {
  ...
  function validate(data: ICargosEscClass): {
    isValid: boolean;
    message: string;
  } {
