...
#region Testes de Propriedades Computadas
    ...
    {
        _instance.FvqaData = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FvqaData?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }
    ...
    {
        var instance = new DBAgendaRelatorio();
        Assert.Equal(null !, instance.FvqaData);
    }
