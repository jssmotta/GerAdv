...
public partial class OperadorGruposAgendaController(IOperadorGruposAgendaService operadorgruposagendaService) : ControllerBase
{
    ...
    {
        _logger.Info("OperadorGruposAgenda: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _operadorgruposagendaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("OperadorGruposAgenda: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _operadorgruposagendaService.UpdateColumns(parameters, uri);
        if (!result)
        {
