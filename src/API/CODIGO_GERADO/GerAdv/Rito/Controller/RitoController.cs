...
public partial class RitoController(IRitoService ritoService) : ControllerBase
{
    ...
    {
        _logger.Info("Rito: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _ritoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Rito: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _ritoService.UpdateColumns(parameters, uri);
        if (!result)
        {
