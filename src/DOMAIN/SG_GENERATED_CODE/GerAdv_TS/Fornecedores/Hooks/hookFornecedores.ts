...
export const useFornecedoresList = (dataService: IFornecedoresService, currentFilter?: FilterFornecedores | undefined | null) => {
  const [data, setData] = useState<IFornecedores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterFornecedores | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterFornecedores | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterFornecedores);
      setData(result);
    } catch (err) {
