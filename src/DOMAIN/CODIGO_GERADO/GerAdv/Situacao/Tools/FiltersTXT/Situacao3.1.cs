...
public static class FilterSituacaoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Situacao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterSituacao:

- O filter irá filtrar os Situacao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterSituacao:(Situação)
- parte_int: _nome ou descrição para Situação;
- parte_opo: Parte_Opo;
- top: Top;
- bold: Bold;
...
- Se ouvir "todos os Situacao" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterSituacao? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Situacao length: {result.Length}");
#endif
