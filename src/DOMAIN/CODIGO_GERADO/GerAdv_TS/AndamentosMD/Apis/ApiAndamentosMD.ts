...
import { AndamentosMD } from '../../Models/AndamentosMD';
...
export class AndamentosMDApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AndamentosMDApiError';
    this.status = status;
    this.code = code;
...
export class AndamentosMDApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AndamentosMD`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndamentosMD_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AndamentosMD');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndamentosMD_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Andamentos M D com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterAndamentosMD): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndamentosMD_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de AndamentosMD');
        }
    }

            public async filter(filtro: FilterAndamentosMD): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndamentosMD_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AndamentosMD');
                }
            }


    public async validation(regAndamentosMD: IAndamentosMD): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAndamentosMD as AndamentosMD, this.getHeaders());
    ...
    public async addAndUpdate(regAndamentosMD: IAndamentosMD): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAndamentosMD as AndamentosMD, this.getHeaders());
            var register = result.data as IAndamentosMD;        
            const action = regAndamentosMD.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Andamentos M D');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AndamentosMD. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AndamentosMD',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Andamentos M D com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAndamentosMD) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AndamentosMD[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de andamentosmd:', error);
            this.notificationService.notify({
              entity: 'AndamentosMD',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos andamentosmd'
            });
          }
        });
    }
