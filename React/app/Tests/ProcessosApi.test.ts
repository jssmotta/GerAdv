...
import { ProcessosApiError } from '../GerAdv_TS/Processos/Apis/ApiProcessos';
import { ProcessosApi } from '@/app/GerAdv_TS/Processos/Apis/ApiProcessos';
import { FilterProcessos } from '../GerAdv_TS/Processos/Filters/Processos';
...
describe('ProcessosApi', () => {
  ...
  beforeEach(() => {
    ...
    processosApi = new ProcessosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processosApi).toBeInstanceOf(ProcessosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all processos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processosApi.getAll()).rejects.toThrow(ProcessosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Processos_last_getAll_${200}`)));
    });
    ...
    it('should throw ProcessosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessos = ProcessosTestEmpty();
    ...
    it('should fetch processos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessos));
      ...
      expect(result.data).toEqual(mockProcessos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Processos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProcessos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter processos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProcessos: IProcessos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new processos successfully', async () => {
      const responseProcessos = { ...mockProcessos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosApi.addAndUpdate(mockProcessos);
    ...
    it('should update existing processos successfully', async () => {
      const existingProcessos = { ...mockProcessos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processosApi.addAndUpdate(existingProcessos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processosApi.addAndUpdate(mockProcessos)).rejects.toThrow(ProcessosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Processos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Processos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processosApi.delete(1)).rejects.toThrow(ProcessosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProcessos = { funcionario: 1 };
      
      const result = processosApi.useFilter(mockFilter);
  ...
  describe('ProcessosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessosApiError('Test message', 500);
