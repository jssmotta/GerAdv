...
describe('NENotasApi', () => {
  ...
  beforeEach(() => {
    ...
    nenotasApi = new NENotasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(nenotasApi).toBeInstanceOf(NENotasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new NENotasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(NENotasApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all nenotas items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await nenotasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(nenotasApi.getAll()).rejects.toThrow(NENotasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await nenotasApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-NENotas_last_getAll_${200}`)));
    });
    ...
    it('should throw NENotasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockNENotas = NENotasTestEmpty();
    ...
    it('should fetch nenotas by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await nenotasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockNENotas));
      ...
      expect(result.data).toEqual(mockNENotas);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NENotas',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter nenotas items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await nenotasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new nenotas successfully', async () => {
      const responseNENotas = { ...mockNENotas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseNENotas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await nenotasApi.addAndUpdate(mockNENotas);
    ...
    it('should update existing nenotas successfully', async () => {
      const existingNENotas = { ...mockNENotas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingNENotas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await nenotasApi.addAndUpdate(existingNENotas);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(nenotasApi.addAndUpdate(mockNENotas)).rejects.toThrow(NENotasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NENotas',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete nenotas successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await nenotasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(nenotasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NENotas',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(nenotasApi.delete(1)).rejects.toThrow(NENotasApiError);
    });
  });
  ...
  describe('NENotasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new NENotasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('NENotasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new NENotasApiError('Test message', 500);
