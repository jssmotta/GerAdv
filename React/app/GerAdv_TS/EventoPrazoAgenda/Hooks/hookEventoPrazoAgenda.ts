...
export const useEventoPrazoAgendaForm = (
  ...
  const loadEventoPrazoAgenda = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Evento Prazo Agenda";
...
export const useEventoPrazoAgendaList = (
  dataService: IEventoPrazoAgendaService,
  currentFilter?: FilterEventoPrazoAgenda | undefined | null,
) => {
  const [data, setData] = useState<IEventoPrazoAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterEventoPrazoAgenda | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterEventoPrazoAgenda | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar eventoprazoagenda";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsEventoPrazoAgenda() {
  async function runValidation(
    ...
    const eventoprazoagendaApi = new EventoPrazoAgendaApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IEventoPrazoAgenda): {
    isValid: boolean;
    message: string;
  } {
