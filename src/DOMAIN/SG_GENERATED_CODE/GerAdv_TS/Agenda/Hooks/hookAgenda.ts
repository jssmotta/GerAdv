...
export const useAgendaList = (dataService: IAgendaService, currentFilter?: FilterAgenda | undefined | null) => {
  const [data, setData] = useState<IAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterAgenda | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterAgenda | undefined | null) => {
