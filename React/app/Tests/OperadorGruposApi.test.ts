...
import { OperadorGruposApiError } from '../GerAdv_TS/OperadorGrupos/Apis/ApiOperadorGrupos';
import { OperadorGruposApi } from '@/app/GerAdv_TS/OperadorGrupos/Apis/ApiOperadorGrupos';
import { FilterOperadorGrupos } from '../GerAdv_TS/OperadorGrupos/Filters/OperadorGrupos';
...
describe('OperadorGruposApi', () => {
  let operadorgruposApi: OperadorGruposApi;
  ...
  beforeEach(() => {
    ...
    operadorgruposApi = new OperadorGruposApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(operadorgruposApi).toBeInstanceOf(OperadorGruposApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OperadorGruposApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OperadorGruposApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all operadorgrupos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorgruposApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(operadorgruposApi.getAll()).rejects.toThrow(OperadorGruposApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await operadorgruposApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-OperadorGrupos_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupos',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw OperadorGruposApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOperadorGrupos = OperadorGruposTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockOperadorGrupos,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch operadorgrupos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorgruposApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOperadorGrupos));
      ...
      expect(result.data).toEqual(mockOperadorGrupos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterOperadorGrupos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter operadorgrupos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await operadorgruposApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockOperadorGrupos: IOperadorGrupos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new operadorgrupos successfully', async () => {
      const responseOperadorGrupos = { ...mockOperadorGrupos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOperadorGrupos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorgruposApi.addAndUpdate(mockOperadorGrupos);
    ...
    it('should update existing operadorgrupos successfully', async () => {
      const existingOperadorGrupos = { ...mockOperadorGrupos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOperadorGrupos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorgruposApi.addAndUpdate(existingOperadorGrupos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(operadorgruposApi.addAndUpdate(mockOperadorGrupos)).rejects.toThrow(OperadorGruposApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete operadorgrupos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await operadorgruposApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(operadorgruposApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(operadorgruposApi.delete(1)).rejects.toThrow(OperadorGruposApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterOperadorGrupos = { funcionario: 1 };
      
      const result = operadorgruposApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await operadorgruposApi.delete(1);
  ...
  describe('OperadorGruposApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OperadorGruposApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OperadorGruposApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OperadorGruposApiError('Test message', 500);
