...
import { TribunalApiError } from '../GerAdv_TS/Tribunal/Apis/ApiTribunal';
import { TribunalApi } from '@/app/GerAdv_TS/Tribunal/Apis/ApiTribunal';
import { FilterTribunal } from '../GerAdv_TS/Tribunal/Filters/Tribunal';
...
describe('TribunalApi', () => {
  let tribunalApi: TribunalApi;
  ...
  beforeEach(() => {
    ...
    tribunalApi = new TribunalApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tribunalApi).toBeInstanceOf(TribunalApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TribunalApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TribunalApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tribunal items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tribunalApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tribunalApi.getAll()).rejects.toThrow(TribunalApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tribunalApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Tribunal_last_getAll_${200}`)));
    });
    ...
    it('should throw TribunalApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTribunal = TribunalTestEmpty();
    ...
    it('should fetch tribunal by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tribunalApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTribunal));
      ...
      expect(result.data).toEqual(mockTribunal);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Tribunal',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTribunal = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tribunal items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tribunalApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new tribunal successfully', async () => {
      const responseTribunal = { ...mockTribunal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTribunal,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tribunalApi.addAndUpdate(mockTribunal);
    ...
    it('should update existing tribunal successfully', async () => {
      const existingTribunal = { ...mockTribunal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTribunal,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tribunalApi.addAndUpdate(existingTribunal);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tribunalApi.addAndUpdate(mockTribunal)).rejects.toThrow(TribunalApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Tribunal',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tribunal successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tribunalApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tribunalApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Tribunal',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tribunalApi.delete(1)).rejects.toThrow(TribunalApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTribunal = { funcionario: 1 };
      
      const result = tribunalApi.useFilter(mockFilter);
  ...
  describe('TribunalApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TribunalApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TribunalApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TribunalApiError('Test message', 500);
