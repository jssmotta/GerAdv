...
public static class FilterProResumosTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterProResumos? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar ProResumos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProResumos:

- O filter irá filtrar os ProResumos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProResumos:(Pro Resumos)
- processo: número ou IDs;
- data: não é a data de cadastramento é a data do ProResumos;
- resumo: Resumo;
- tiporesumo: número ou IDs;
- bold: Bold;
...
- Se ouvir "todos os ProResumos" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProResumos length: {result.Length}");
#endif
