...
const PenhoraStatusComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPenhoraStatus | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPenhoraStatus | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPenhoraStatus);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PenhoraStatusWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPenhoraStatus={editar ?? addValue ?? PenhoraStatusEmpty()} /> 
            )}
