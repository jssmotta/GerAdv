...
import { OponentesApiError } from '../GerAdv_TS/Oponentes/Apis/ApiOponentes';
import { OponentesApi } from '@/app/GerAdv_TS/Oponentes/Apis/ApiOponentes';
import { FilterOponentes } from '../GerAdv_TS/Oponentes/Filters/Oponentes';
...
describe('OponentesApi', () => {
  let oponentesApi: OponentesApi;
  ...
  beforeEach(() => {
    ...
    oponentesApi = new OponentesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(oponentesApi).toBeInstanceOf(OponentesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OponentesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OponentesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all oponentes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await oponentesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(oponentesApi.getAll()).rejects.toThrow(OponentesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await oponentesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Oponentes_last_getAll_${200}`)));
    });
    ...
    it('should throw OponentesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOponentes = OponentesTestEmpty();
    ...
    it('should fetch oponentes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await oponentesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOponentes));
      ...
      expect(result.data).toEqual(mockOponentes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Oponentes',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterOponentes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter oponentes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await oponentesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockOponentes: IOponentes = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new oponentes successfully', async () => {
      const responseOponentes = { ...mockOponentes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOponentes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await oponentesApi.addAndUpdate(mockOponentes);
    ...
    it('should update existing oponentes successfully', async () => {
      const existingOponentes = { ...mockOponentes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOponentes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await oponentesApi.addAndUpdate(existingOponentes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(oponentesApi.addAndUpdate(mockOponentes)).rejects.toThrow(OponentesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Oponentes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete oponentes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await oponentesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(oponentesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Oponentes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(oponentesApi.delete(1)).rejects.toThrow(OponentesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterOponentes = { funcionario: 1 };
      
      const result = oponentesApi.useFilter(mockFilter);
  ...
  describe('OponentesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OponentesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OponentesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OponentesApiError('Test message', 500);
