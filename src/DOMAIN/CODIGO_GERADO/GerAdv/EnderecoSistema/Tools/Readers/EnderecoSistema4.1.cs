...
public partial class EnderecoSistema
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(estDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.EnderecoSistema WITH (NOLOCK, INDEX = idx_EnderecoSistema_AuditorDtAtu)
WHERE estCodigo = @id
...
FROM {oCnn.UseDbo}.EnderecoSistema WITH (NOLOCK)
