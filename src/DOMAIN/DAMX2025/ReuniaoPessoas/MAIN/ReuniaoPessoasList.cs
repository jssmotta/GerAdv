...
#region ListarDados_ReuniaoPessoas
    public static async IAsyncEnumerable<DBReuniaoPessoas> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBReuniaoPessoas(dbRec);
        }
    }

    public static IEnumerable<DBReuniaoPessoas> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
