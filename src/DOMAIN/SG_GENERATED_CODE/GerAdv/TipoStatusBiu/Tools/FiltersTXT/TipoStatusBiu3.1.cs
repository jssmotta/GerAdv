...
public static class FilterTipoStatusBiuTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoStatusBiu em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoStatusBiu:

- O filter irá filtrar os TipoStatusBiu conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterTipoStatusBiu:(Staus  Usuários)
- nome: _nome ou descrição para Staus  Usuários;
...
- Se ouvir "todos os TipoStatusBiu" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoStatusBiu? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoStatusBiu length: {result.Length}");
#endif
