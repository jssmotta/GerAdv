...
const EnquadramentoEmpresaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IEnquadramentoEmpresa | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IEnquadramentoEmpresa | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IEnquadramentoEmpresa);
    ...
    const handleAddClick = () => {
        ...
        const add = EnquadramentoEmpresaEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <EnquadramentoEmpresaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedEnquadramentoEmpresa={editar ?? addValue ?? EnquadramentoEmpresaEmpty()} /> 
            )}
