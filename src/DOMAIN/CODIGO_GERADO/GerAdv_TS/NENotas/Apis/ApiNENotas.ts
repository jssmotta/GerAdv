...
import { NENotas } from '../../Models/NENotas';
...
export class NENotasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class NENotasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/NENotas`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NENotas_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os NENotas');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NENotas_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar N E Notas com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterNENotas): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NENotas_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de NENotas');
        }
    }

            public async filter(filtro: FilterNENotas): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-NENotas_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar NENotas');
                }
            }


    public async validation(regNENotas: INENotas): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regNENotas as NENotas, this.getHeaders());
    ...
    public async addAndUpdate(regNENotas: INENotas): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regNENotas as NENotas, this.getHeaders());
            var register = result.data as INENotas;        
            const action = regNENotas.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar N E Notas');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o NENotas. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'NENotas',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir N E Notas com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterNENotas) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<NENotas[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de nenotas:', error);
            this.notificationService.notify({
              entity: 'NENotas',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos nenotas'
            });
          }
        });
    }
