...
public class FGUTPeriodicidadeStatusFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FGUTPeriodicidadeStatusFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FGUTPeriodicidadeStatusFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FGUTPeriodicidadeStatusFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "pgsCodigo")
        var dt = new DataTable();
        dt.Columns.Add("pgsCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["pgsCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FGUTPeriodicidadeStatus>();
    }
    ...
    {
        // Arrange
        var factory = new FGUTPeriodicidadeStatusFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("pgsCodigo", typeof(int));
        mockDataTable.Columns.Add("pgsGuid", typeof(string));
        var row = mockDataTable.NewRow();
        row["pgsCodigo"] = testId;
        row["pgsGuid"] = "Test GUTPeriodicidadeStatus";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["pgsCodigo"] = 123;
        row["pgsGuid"] = "Test GUTPeriodicidadeStatus Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FGUTPeriodicidadeStatus properties
        }
    }
