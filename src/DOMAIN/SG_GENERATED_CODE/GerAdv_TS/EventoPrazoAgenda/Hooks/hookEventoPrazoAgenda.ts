...
export const useEventoPrazoAgendaList = (dataService: IEventoPrazoAgendaService, currentFilter?: FilterEventoPrazoAgenda | undefined | null) => {
  const [data, setData] = useState<IEventoPrazoAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterEventoPrazoAgenda | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterEventoPrazoAgenda | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterEventoPrazoAgenda);
      setData(result);
    } catch (err) {
