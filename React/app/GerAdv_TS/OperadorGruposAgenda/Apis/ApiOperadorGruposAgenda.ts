...
import { OperadorGruposAgenda } from '../../Models/OperadorGruposAgenda';
...
export class OperadorGruposAgendaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'OperadorGruposAgendaApiError';
    this.status = status;
    this.code = code;
...
export class OperadorGruposAgendaApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGruposAgenda_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os OperadorGruposAgenda');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGruposAgenda_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Operador Grupos Agenda com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGruposAgenda_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de OperadorGruposAgenda');
        }
    }

            public async filter(filtro: FilterOperadorGruposAgenda): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGruposAgenda_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar OperadorGruposAgenda');
                }
            }


    public async validation(regOperadorGruposAgenda: IOperadorGruposAgenda): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regOperadorGruposAgenda as OperadorGruposAgenda, this.getHeaders());
    ...
    public async addAndUpdate(regOperadorGruposAgenda: IOperadorGruposAgenda): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Operador Grupos Agenda');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Operador Grupos Agenda com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterOperadorGruposAgenda) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<OperadorGruposAgenda[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de operadorgruposagenda:', error);
            this.notificationService.notify({
              entity: 'OperadorGruposAgenda',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos operadorgruposagenda'
            });
          }
        });
    }
