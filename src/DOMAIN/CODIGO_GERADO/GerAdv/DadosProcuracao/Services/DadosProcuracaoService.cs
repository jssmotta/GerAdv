...
public partial class DadosProcuracaoService(IOptions<AppSettings> appSettings, IFDadosProcuracaoFactory dadosprocuracaoFactory, IDadosProcuracaoReader reader, IDadosProcuracaoValidation validation, IDadosProcuracaoWriter writer, IClientesReader clientesReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IDadosProcuracaoService, IDisposable
{
    ...
    private readonly IFDadosProcuracaoFactory dadosprocuracaoFactory = dadosprocuracaoFactory;
    private readonly IDadosProcuracaoReader reader = reader;
    private readonly IDadosProcuracaoValidation validation = validation;
    private readonly IDadosProcuracaoWriter writer = writer;
    ...
    public async Task<IEnumerable<DadosProcuracaoResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-DadosProcuracao-Filter-{max}-{keyCache}";
    ...
    public async Task<IEnumerable<DadosProcuracaoResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterDadosProcuracao filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        using var oCnn = Configuracoes.GetConnectionByUri(uri);
        if (oCnn == null)
        ...
        var cacheKey = $"{uri}-{max}DadosProcuracao-Filter-{where.GetHashCode2()}{filterHash}{keyCache}";
        var entryOptions = new HybridCacheEntryOptions
        {
    ...
    private async Task<DadosProcuracaoResponse?> GetDataByIdAsync(int id, MsiSqlConnection? oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<DadosProcuracaoResponse?> AddAndUpdate([FromBody] Models.DadosProcuracao? regDadosProcuracao, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regDadosProcuracao, operadorId, oCnn);
    ...
    public async Task<DadosProcuracaoResponse?> Validation([FromBody] Models.DadosProcuracao? regDadosProcuracao, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regDadosProcuracao.Id, oCnn);
    }
    ...
    public async Task<DadosProcuracaoResponse?> Delete([FromQuery] int? id, [FromRoute, Required] string uri)
    {
        ...
        var dadosprocuracao = await reader.Read(id ?? default, oCnn);
        try
        {
            if (dadosprocuracao != null)
            {
                await writer.Delete(dadosprocuracao, nOperador, oCnn);
