...
const DocsRecebidosItensComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, nome: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as DocsRecebidosItens;
        const novoDado = await dadoApi.addAndUpdate(nova as IDocsRecebidosItens);
