...
export const useOperadorEMailPopupList = (dataService: IOperadorEMailPopupService, currentFilter?: FilterOperadorEMailPopup | undefined | null) => {
  const [data, setData] = useState<IOperadorEMailPopup[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterOperadorEMailPopup | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterOperadorEMailPopup | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterOperadorEMailPopup);
      setData(result);
    } catch (err) {
