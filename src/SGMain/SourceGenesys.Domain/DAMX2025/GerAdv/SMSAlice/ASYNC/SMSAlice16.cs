...
public partial class DBSMSAlice
{
    ...
    /// </summary>
    public static async Task<DBSMSAlice> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBSMSAlice();
...
#if (!NOTSTORED_SMSAlice)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFOperador || pFldFNome || pFldFTipoEMail || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFOperador)
            updateTool.Fields(DBSMSAliceDicInfo.Operador, FOperador, EGenericTypeFields.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBSMSAliceDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFTipoEMail)
            updateTool.Fields(DBSMSAliceDicInfo.TipoEMail, FTipoEMail, EGenericTypeFields.FNumber);
        if (pFldFGuid)
            updateTool.Fields(DBSMSAliceDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBSMSAliceDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBSMSAliceDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
