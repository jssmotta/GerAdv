...
public partial class ReuniaoController(IReuniaoService reuniaoService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterReuniao filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Reuniao: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _reuniaoService.Filter(max, filtro, uri);
        return Ok(result);
    }
