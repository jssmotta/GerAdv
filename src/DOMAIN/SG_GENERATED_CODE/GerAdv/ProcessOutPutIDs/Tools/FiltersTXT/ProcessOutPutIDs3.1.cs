...
public static class FilterProcessOutPutIDsTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProcessOutPutIDs em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProcessOutPutIDs:

- O filter irá filtrar os ProcessOutPutIDs conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProcessOutPutIDs:(Process Out Put I Ds)
- nome: _nome ou descrição para Process Out Put I Ds;
...
- Se ouvir "todos os ProcessOutPutIDs" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProcessOutPutIDs? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProcessOutPutIDs length: {result.Length}");
#endif
