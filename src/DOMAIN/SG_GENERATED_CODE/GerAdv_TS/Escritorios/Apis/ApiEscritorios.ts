...
import { FilterEscritorios, FilterEscritoriosDefaults } from '../Filters/Escritorios'
...
export class EscritoriosApi {
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios-${uniqueKeyDay()}_lst_getAll_${max}`);
    ...
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios-${uniqueKeyDay()}_lst_getById_${id}`);
    ...
    public async getListN(max?: number, filtro?: FilterEscritorios): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios-${uniqueKeyDay()}_lst_listN_data`);
 ...
 public async filterPreload(max: number, filtro?: FilterEscritorios): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterEscritoriosDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterEscritorios): Promise<AxiosResponse> {
    ...
    public async filterVoice(filtro?: FilterEscritorios, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterEscritoriosDefaults();

        // Create request object according to new backend structure
        const request: IFilterEscritoriosWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
    ...
    public async validation(regEscritorios: IEscritorios): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regEscritorios as Escritorios, this.getHeaders());
            });
    ...
    public async addAndUpdate(regEscritorios: IEscritorios): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regEscritorios as Escritorios, this.getHeaders());
            });
    ...
    public useFilter(filtro: FilterEscritorios, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterEscritoriosWithVoiceRequest = {filter: filtro || new FilterEscritoriosDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<Escritorios[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de Escritorios');
            this.notificationService.notify({
              entity: 'Escritorios',
              id: 0,
              action: NotifySystemActions.ERROR,
