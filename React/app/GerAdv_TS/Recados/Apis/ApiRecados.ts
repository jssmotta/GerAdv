...
export class RecadosApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        public async getListN(max?: number, filtro?: FilterRecados): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Recados_last_listN_data`);
        ...
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Recados');
        }
    }
 public async filterPreload(filtro: FilterRecados): Promise<AxiosResponse> {
    ...
    public useFilter(filtro: FilterRecados) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Recados[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de Recados');
            this.notificationService.notify({
              entity: 'Recados',
              id: 0,
              action: NotifySystemActions.ERROR,
