...
public partial class FLivroCaixa : MenphisSI.SG.GerAdv.DBLivroCaixa, IDBLivroCaixa
{
    ...
    private FLivroCaixa(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FLivroCaixa CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FLivroCaixa(parameters, oCnn, fullSql, sqlWhere, join);
    }
