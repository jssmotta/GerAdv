...
import { FilterEMPClassRiscos } from '@/app/GerAdv_TS/EMPClassRiscos/Filters/EMPClassRiscos';
...
interface UseEMPClassRiscosFilterProps {
    handleFetchWithFilter: (filtro?: FilterEMPClassRiscos | undefined | null) => Promise<void>;
}

interface UseEMPClassRiscosFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterEMPClassRiscos;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterEMPClassRiscos>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterEMPClassRiscos) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterEMPClassRiscos>) => React.ReactNode;
...
export const useEMPClassRiscosFilter = ({ handleFetchWithFilter }: UseEMPClassRiscosFilterProps): UseEMPClassRiscosFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterEMPClassRiscos>({} as FilterEMPClassRiscos);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterEMPClassRiscos;
    ...
    const handleConfirmSearch = async (filter: FilterEMPClassRiscos) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterEMPClassRiscos);
        sessionStorage.removeItem(btoa('EMPClassRiscosFilter'));
        handleFetchWithFilter({} as FilterEMPClassRiscos);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterEMPClassRiscos>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
