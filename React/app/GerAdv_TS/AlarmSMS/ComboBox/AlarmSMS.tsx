...
import AlarmSMSWindow from '../Crud/Grids/AlarmSMSWindow';
...
import { useAlarmSMSComboBox } from '../Hooks/hookAlarmSMS';
const AlarmSMSComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useAlarmSMSComboBox(alarmsmsService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IAlarmSMS | null>(null);
const [addRecord, setAddRecord] = useState<IAlarmSMS | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await alarmsmsService.fetchAlarmSMSById(id);
...
const handleSuccess = (newAlarmSMS?: IAlarmSMS) => {
  if (newAlarmSMS) {
    setValue({ id: newAlarmSMS.id, nome: newAlarmSMS.descricao });
    loadRecordForEdit(newAlarmSMS.id);
    handleValueChange(newAlarmSMS);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .alarmsms-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .alarmsms-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .alarmsms-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedAlarmSMS={editRecord || addRecord || AlarmSMSEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'alarmsms-dropdown-popup-msi'
          }}
          />
...
export default AlarmSMSComboBox;