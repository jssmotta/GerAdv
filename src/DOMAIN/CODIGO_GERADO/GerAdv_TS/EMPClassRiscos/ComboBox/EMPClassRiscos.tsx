...
const EMPClassRiscosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IEMPClassRiscos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IEMPClassRiscos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IEMPClassRiscos);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <EMPClassRiscosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedEMPClassRiscos={editar ?? addValue ?? EMPClassRiscosEmpty()} /> 
            )}
