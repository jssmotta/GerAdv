...
import { ContatoCRMOperadorApiError } from '../GerAdv_TS/ContatoCRMOperador/Apis/ApiContatoCRMOperador';
import { ContatoCRMOperadorApi } from '@/app/GerAdv_TS/ContatoCRMOperador/Apis/ApiContatoCRMOperador';
import { FilterContatoCRMOperador } from '../GerAdv_TS/ContatoCRMOperador/Filters/ContatoCRMOperador';
...
describe('ContatoCRMOperadorApi', () => {
  let contatocrmoperadorApi: ContatoCRMOperadorApi;
  ...
  beforeEach(() => {
    ...
    contatocrmoperadorApi = new ContatoCRMOperadorApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(contatocrmoperadorApi).toBeInstanceOf(ContatoCRMOperadorApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ContatoCRMOperadorApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ContatoCRMOperadorApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ContatoCRMOperadorTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all contatocrmoperador items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmoperadorApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(contatocrmoperadorApi.getAll()).rejects.toThrow(ContatoCRMOperadorApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await contatocrmoperadorApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ContatoCRMOperador_last_getAll_${200}`)));
    });
    ...
    it('should throw ContatoCRMOperadorApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockContatoCRMOperador = ContatoCRMOperadorTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockContatoCRMOperador,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch contatocrmoperador by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await contatocrmoperadorApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockContatoCRMOperador));
      ...
      expect(result.data).toEqual(mockContatoCRMOperador);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMOperador',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterContatoCRMOperador = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter contatocrmoperador items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await contatocrmoperadorApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockContatoCRMOperador: IContatoCRMOperador = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new contatocrmoperador successfully', async () => {
      const responseContatoCRMOperador = { ...mockContatoCRMOperador, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseContatoCRMOperador,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmoperadorApi.addAndUpdate(mockContatoCRMOperador);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockContatoCRMOperador,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing contatocrmoperador successfully', async () => {
      const existingContatoCRMOperador = { ...mockContatoCRMOperador, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingContatoCRMOperador,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await contatocrmoperadorApi.addAndUpdate(existingContatoCRMOperador);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(contatocrmoperadorApi.addAndUpdate(mockContatoCRMOperador)).rejects.toThrow(ContatoCRMOperadorApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMOperador',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete contatocrmoperador successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await contatocrmoperadorApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(contatocrmoperadorApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ContatoCRMOperador',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(contatocrmoperadorApi.delete(1)).rejects.toThrow(ContatoCRMOperadorApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterContatoCRMOperador = { funcionario: 1 };
      
      const result = contatocrmoperadorApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await contatocrmoperadorApi.delete(1);
  ...
  describe('ContatoCRMOperadorApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ContatoCRMOperadorApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ContatoCRMOperadorApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ContatoCRMOperadorApiError('Test message', 500);
