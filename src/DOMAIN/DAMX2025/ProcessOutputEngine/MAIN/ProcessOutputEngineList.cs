...
#region ListarDados_ProcessOutputEngine
    public static async IAsyncEnumerable<DBProcessOutputEngine> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProcessOutputEngine(dbRec);
        }
    }

    public static IEnumerable<DBProcessOutputEngine> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
