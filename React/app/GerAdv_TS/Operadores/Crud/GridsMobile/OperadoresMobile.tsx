...
import { useOperadoresGrid } from "../../Hooks/hookOperadoresGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("operadores-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-operadores .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("operadores-columns-order-mobile"));
    }, []);
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Operadores-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Operadores-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-operadores"
              key={`operadores-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
