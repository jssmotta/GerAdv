...
export const useTipoEnderecoSistemaList = (dataService: ITipoEnderecoSistemaService, currentFilter?: FilterTipoEnderecoSistema | undefined | null) => {
  const [data, setData] = useState<ITipoEnderecoSistema[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoEnderecoSistema | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoEnderecoSistema | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoEnderecoSistema);
      setData(result);
    } catch (err) {
