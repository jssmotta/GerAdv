...
import FaseWindow from '../Crud/Grids/FaseWindow';
...
import { useFaseComboBox } from '../Hooks/hookFase';
const FaseComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useFaseComboBox(faseService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IFase | null>(null);
const [addRecord, setAddRecord] = useState<IFase | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await faseService.fetchFaseById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.descricao });
  } catch (error) {
  console.log('Erro ao carregar Fase:');
}
};
...
const handleSuccess = (newFase?: IFase) => {
  if (newFase) {
    setValue({ id: newFase.id, nome: newFase.descricao });
    loadRecordForEdit(newFase.id);
    handleValueChange(newFase);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .fase-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .fase-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .fase-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedFase={editRecord || addRecord || FaseEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'fase-dropdown-popup-msi'
          }}
          />
