...
export class GUTPeriodicidadeApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'GUTPeriodicidadeApiError';
    this.status = status;
    this.code = code;
...
export class GUTPeriodicidadeApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidade_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTPeriodicidade');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidade_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar G U T Periodicidade com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidade_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de GUTPeriodicidade');
        }
    }

            public async filter(filtro: FilterGUTPeriodicidade): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidade_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTPeriodicidade');
                }
            }


    public async validation(regGUTPeriodicidade: IGUTPeriodicidade): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTPeriodicidade as GUTPeriodicidade, this.getHeaders());
    ...
    public async addAndUpdate(regGUTPeriodicidade: IGUTPeriodicidade): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Periodicidade');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir G U T Periodicidade com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTPeriodicidade) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<GUTPeriodicidade[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de gutperiodicidade:', error);
            this.notificationService.notify({
              entity: 'GUTPeriodicidade',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gutperiodicidade'
            });
          }
        });
    }
