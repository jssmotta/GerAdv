...
public static class FilterStatusHTrabTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar StatusHTrab em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterStatusHTrab:

- O filter irá filtrar os StatusHTrab conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterStatusHTrab:(Status H Trab)
- descricao: _nome ou descrição para Status H Trab;
- resid: número ou IDs;
...
- Se ouvir "todos os StatusHTrab" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterStatusHTrab? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt StatusHTrab length: {result.Length}");
#endif
