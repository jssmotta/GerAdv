...
import { GUTPeriodicidadeApiError } from '../GerAdv_TS/GUTPeriodicidade/Apis/ApiGUTPeriodicidade';
import { GUTPeriodicidadeApi } from '@/app/GerAdv_TS/GUTPeriodicidade/Apis/ApiGUTPeriodicidade';
import { FilterGUTPeriodicidade } from '../GerAdv_TS/GUTPeriodicidade/Filters/GUTPeriodicidade';
...
describe('GUTPeriodicidadeApi', () => {
  let gutperiodicidadeApi: GUTPeriodicidadeApi;
  ...
  beforeEach(() => {
    ...
    gutperiodicidadeApi = new GUTPeriodicidadeApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gutperiodicidadeApi).toBeInstanceOf(GUTPeriodicidadeApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GUTPeriodicidadeApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GUTPeriodicidadeApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [GUTPeriodicidadeTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all gutperiodicidade items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutperiodicidadeApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gutperiodicidadeApi.getAll()).rejects.toThrow(GUTPeriodicidadeApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gutperiodicidadeApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GUTPeriodicidade_last_getAll_${200}`)));
    });
    ...
    it('should throw GUTPeriodicidadeApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGUTPeriodicidade = GUTPeriodicidadeTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockGUTPeriodicidade,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch gutperiodicidade by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutperiodicidadeApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGUTPeriodicidade));
      ...
      expect(result.data).toEqual(mockGUTPeriodicidade);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTPeriodicidade',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGUTPeriodicidade = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter gutperiodicidade items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gutperiodicidadeApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockGUTPeriodicidade: IGUTPeriodicidade = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new gutperiodicidade successfully', async () => {
      const responseGUTPeriodicidade = { ...mockGUTPeriodicidade, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGUTPeriodicidade,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutperiodicidadeApi.addAndUpdate(mockGUTPeriodicidade);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockGUTPeriodicidade,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing gutperiodicidade successfully', async () => {
      const existingGUTPeriodicidade = { ...mockGUTPeriodicidade, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGUTPeriodicidade,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutperiodicidadeApi.addAndUpdate(existingGUTPeriodicidade);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gutperiodicidadeApi.addAndUpdate(mockGUTPeriodicidade)).rejects.toThrow(GUTPeriodicidadeApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTPeriodicidade',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gutperiodicidade successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gutperiodicidadeApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gutperiodicidadeApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTPeriodicidade',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gutperiodicidadeApi.delete(1)).rejects.toThrow(GUTPeriodicidadeApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGUTPeriodicidade = { funcionario: 1 };
      
      const result = gutperiodicidadeApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await gutperiodicidadeApi.delete(1);
  ...
  describe('GUTPeriodicidadeApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GUTPeriodicidadeApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GUTPeriodicidadeApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GUTPeriodicidadeApiError('Test message', 500);
