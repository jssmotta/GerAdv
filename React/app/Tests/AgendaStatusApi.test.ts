...
import { AgendaStatusApiError } from '../GerAdv_TS/AgendaStatus/Apis/ApiAgendaStatus';
import { AgendaStatusApi } from '@/app/GerAdv_TS/AgendaStatus/Apis/ApiAgendaStatus';
import { FilterAgendaStatus } from '../GerAdv_TS/AgendaStatus/Filters/AgendaStatus';
...
describe('AgendaStatusApi', () => {
  let agendastatusApi: AgendaStatusApi;
  ...
  beforeEach(() => {
    ...
    agendastatusApi = new AgendaStatusApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendastatusApi).toBeInstanceOf(AgendaStatusApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaStatusApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaStatusApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all agendastatus items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendastatusApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendastatusApi.getAll()).rejects.toThrow(AgendaStatusApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendastatusApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaStatus_last_getAll_${200}`)));
    });
    ...
    it('should throw AgendaStatusApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaStatus = AgendaStatusTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAgendaStatus,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch agendastatus by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendastatusApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaStatus));
      ...
      expect(result.data).toEqual(mockAgendaStatus);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaStatus',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaStatus = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendastatus items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendastatusApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaStatus: IAgendaStatus = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendastatus successfully', async () => {
      const responseAgendaStatus = { ...mockAgendaStatus, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaStatus,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendastatusApi.addAndUpdate(mockAgendaStatus);
    ...
    it('should update existing agendastatus successfully', async () => {
      const existingAgendaStatus = { ...mockAgendaStatus, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaStatus,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendastatusApi.addAndUpdate(existingAgendaStatus);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendastatusApi.addAndUpdate(mockAgendaStatus)).rejects.toThrow(AgendaStatusApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaStatus',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendastatus successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendastatusApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendastatusApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaStatus',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendastatusApi.delete(1)).rejects.toThrow(AgendaStatusApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaStatus = { funcionario: 1 };
      
      const result = agendastatusApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await agendastatusApi.delete(1);
  ...
  describe('AgendaStatusApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaStatusApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaStatusApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaStatusApiError('Test message', 500);
