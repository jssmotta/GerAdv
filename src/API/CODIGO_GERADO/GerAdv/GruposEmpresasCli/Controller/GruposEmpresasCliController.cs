...
public partial class GruposEmpresasCliController(IGruposEmpresasCliService gruposempresascliService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.GruposEmpresasCli regGruposEmpresasCli, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "GruposEmpresasCli: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "GruposEmpresasCli: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    public async Task<IActionResult> Validation([FromBody] Models.GruposEmpresasCli regGruposEmpresasCli, [FromRoute, Required] string uri)
    {
        try
        {
            ...
            if (result == null)
            {
                _logger.Warn("GruposEmpresasCli: Validation failed to add or update GruposEmpresasCli, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "GruposEmpresasCli: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}