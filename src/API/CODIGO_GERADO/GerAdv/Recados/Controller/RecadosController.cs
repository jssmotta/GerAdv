...
public partial class RecadosController(IRecadosService recadosService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.Recados regRecados, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Recados: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Recados: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.Recados regRecados, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _recadosService.Validation(regRecados, uri);
            if (result == null)
            {
                _logger.Warn("Recados: Validation failed to add or update Recados, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "Recados: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}