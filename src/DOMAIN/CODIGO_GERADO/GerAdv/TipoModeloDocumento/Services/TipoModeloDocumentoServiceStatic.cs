...
public partial class TipoModeloDocumentoService
{
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterTipoModeloDocumento filtro)
    {
        ...
        cWhere.Append(filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoModeloDocumentoDicInfo.PTabelaNome}].[{DBTipoModeloDocumentoDicInfo.Nome}]  {DevourerConsts.MsiCollate} like @{nameof(DBTipoModeloDocumentoDicInfo.Nome)}");
        cWhere.Append(filtro.GUID.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoModeloDocumentoDicInfo.PTabelaNome}].[{DBTipoModeloDocumentoDicInfo.GUID}]  {DevourerConsts.MsiCollate} like @{nameof(DBTipoModeloDocumentoDicInfo.GUID)}");
        if (!filtro.Codigo_filtro.IsEmpty() && filtro.Codigo_filtro_end.IsEmpty())
        {
            cWhere.Append(filtro.Codigo_filtro <= 0 ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoModeloDocumentoDicInfo.PTabelaNome}].[{DBTipoModeloDocumentoDicInfo.CampoCodigo}] >= @{nameof(DBTipoModeloDocumentoDicInfo.CampoCodigo)}");
        }
        else
        {
            cWhere.Append((filtro.Codigo_filtro <= 0 && filtro.Codigo_filtro_end <= 0) ? string.Empty : (!(filtro.Codigo_filtro <= 0) && !(filtro.Codigo_filtro_end <= 0)) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoModeloDocumentoDicInfo.CampoCodigo} BETWEEN @{nameof(DBTipoModeloDocumentoDicInfo.CampoCodigo)} AND @{nameof(DBTipoModeloDocumentoDicInfo.CampoCodigo)}_end" : !(filtro.Codigo_filtro <= 0) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoModeloDocumentoDicInfo.CampoCodigo} = @{nameof(DBTipoModeloDocumentoDicInfo.CampoCodigo)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoModeloDocumentoDicInfo.CampoCodigo} <= @{nameof(DBTipoModeloDocumentoDicInfo.CampoCodigo)}_end");
        }
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter> parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarN(max, uri, where, parameters, DBTipoModeloDocumentoDicInfo.CampoNome);
