...
const TipoModeloDocumentoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoModeloDocumento | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoModeloDocumento | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoModeloDocumento);
    ...
    const handleAddClick = () => {
        ...
        const add = TipoModeloDocumentoEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoModeloDocumentoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoModeloDocumento={editar ?? addValue ?? TipoModeloDocumentoEmpty()} /> 
            )}
