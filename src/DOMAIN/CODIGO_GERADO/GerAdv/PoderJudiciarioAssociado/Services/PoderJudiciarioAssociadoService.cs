...
public partial class PoderJudiciarioAssociadoService(IOptions<AppSettings> appSettings, IPoderJudiciarioAssociadoReader reader, IPoderJudiciarioAssociadoValidation validation, IPoderJudiciarioAssociadoWriter writer, IJusticaReader justicaReader, IAreaReader areaReader, ITribunalReader tribunalReader, IForoReader foroReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IPoderJudiciarioAssociadoService, IDisposable
{
    ...
    public async Task<PoderJudiciarioAssociadoResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-PoderJudiciarioAssociado-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("PoderJudiciarioAssociado: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBPoderJudiciarioAssociado(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBPoderJudiciarioAssociadoDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("PoderJudiciarioAssociado: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBPoderJudiciarioAssociado(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBPoderJudiciarioAssociadoDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
