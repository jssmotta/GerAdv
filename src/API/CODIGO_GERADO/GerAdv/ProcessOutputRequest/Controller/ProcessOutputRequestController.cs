...
public partial class ProcessOutputRequestController(IProcessOutputRequestService processoutputrequestService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> GetListN([FromQuery] int max, [FromBody] Filters.FilterProcessOutputRequest? filtro, [FromRoute, Required] string uri)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        //_logger.Info($"ProcessOutputRequest: GetListN called, max {max}, {filtro} uri");
        var result = await _processoutputrequestService.GetListN(max, filtro, uri);
        return Ok(result);
    }
