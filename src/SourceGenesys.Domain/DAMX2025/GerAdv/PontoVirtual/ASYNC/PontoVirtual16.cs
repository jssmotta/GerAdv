...
public partial class DBPontoVirtual
{
    ...
    /// </summary>
    public static async Task<DBPontoVirtual> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBPontoVirtual();
...
#if (!NOTSTORED_PontoVirtual)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFHoraEntrada || pFldFHoraSaida || pFldFOperador || pFldFKey;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFHoraEntrada)
            updateTool.Fields(DBPontoVirtualDicInfo.HoraEntrada, m_FHoraEntrada, ETiposCampos.FDate);
        if (pFldFHoraSaida)
            updateTool.Fields(DBPontoVirtualDicInfo.HoraSaida, m_FHoraSaida, ETiposCampos.FDate);
        if (pFldFOperador)
            updateTool.Fields(DBPontoVirtualDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFKey)
            updateTool.Fields(DBPontoVirtualDicInfo.Key, m_FKey, ETiposCampos.FString);
    }
