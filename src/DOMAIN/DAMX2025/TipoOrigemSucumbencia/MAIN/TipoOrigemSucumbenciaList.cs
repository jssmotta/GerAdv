...
#region ListarDados_TipoOrigemSucumbencia
    public static async IAsyncEnumerable<DBTipoOrigemSucumbencia> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoOrigemSucumbencia(dbRec);
        }
    }

    public static IEnumerable<DBTipoOrigemSucumbencia> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
