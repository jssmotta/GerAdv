...
SELECT TOP (1) 
	FORMAT(areDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(areDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    areVisto as Visto
FROM {oCnn.UseDbo}.Area dbq WITH (INDEX = idx_Area_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.areQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.areQuemAtu
WHERE [areCodigo] = @id
...
FROM {oCnn.UseDbo}.Area WITH (INDEX = idx_{DBAreaDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [areCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBAreaDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(areDtAtu, areDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBAreaDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(areDtAtu, areDtCad) DESC;", oCnn.InnerConnection);
