...
public static class FilterDocumentosTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Documentos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterDocumentos:

- O filter irá filtrar os Documentos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterDocumentos:(Documentos)
- processo: número ou IDs;
- data: não é a data de cadastramento é a data do Documentos;
...
- Se ouvir "todos os Documentos" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterDocumentos? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Documentos length: {result.Length}");
#endif
