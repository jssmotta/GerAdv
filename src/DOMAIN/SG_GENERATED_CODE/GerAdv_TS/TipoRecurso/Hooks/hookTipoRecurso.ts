...
export const useTipoRecursoList = (dataService: ITipoRecursoService, currentFilter?: FilterTipoRecurso | undefined | null) => {
  const [data, setData] = useState<ITipoRecurso[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoRecurso | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoRecurso | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoRecurso);
      setData(result);
    } catch (err) {
