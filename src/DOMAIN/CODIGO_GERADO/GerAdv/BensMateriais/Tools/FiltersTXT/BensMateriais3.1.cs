...
public static class FilterBensMateriaisTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar BensMateriais em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterBensMateriais:

- O filter irá filtrar os BensMateriais conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
BensClassificacao é Classificação de Bens
Fornecedores é Fornecedor
...
- filterBensClassificacao: filtro específico para Bens Classificacao
- filterFornecedores: filtro específico para Fornecedores
...
CAMPOS filterBensClassificacao (Bens Classificacao):
- nome: _nome ou descrição para Bens Classificacao;
- bold: Bold;
...
CAMPOS filterFornecedores (Fornecedores):
- grupo: número ou IDs;
- nome: _nome ou descrição para Fornecedores;
- subgrupo: número ou IDs;
- tipo: Tipo;
- sexo: sexo do Fornecedores: 1=Homem; 0=Mulher; -2147483648=ambos;
...
CAMPOS filterBensMateriais:(Bens Materiais)
- nome: _nome ou descrição para Bens Materiais;
- bensclassificacao: número ou IDs;
- datacompra: DataCompra;
- datafimdagarantia: DataFimDaGarantia;
- nfnro: NFNRO;
- fornecedor: número ou IDs;
- valorbem: valores monetários ou com decimais;
...
- Se ouvir "todos os BensMateriais" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterBensMateriais? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt BensMateriais length: {result.Length}");
#endif
