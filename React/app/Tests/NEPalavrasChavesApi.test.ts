...
import { NEPalavrasChavesApiError } from '../GerAdv_TS/NEPalavrasChaves/Apis/ApiNEPalavrasChaves';
import { NEPalavrasChavesApi } from '@/app/GerAdv_TS/NEPalavrasChaves/Apis/ApiNEPalavrasChaves';
import { FilterNEPalavrasChaves } from '../GerAdv_TS/NEPalavrasChaves/Filters/NEPalavrasChaves';
...
describe('NEPalavrasChavesApi', () => {
  let nepalavraschavesApi: NEPalavrasChavesApi;
  ...
  beforeEach(() => {
    ...
    nepalavraschavesApi = new NEPalavrasChavesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(nepalavraschavesApi).toBeInstanceOf(NEPalavrasChavesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new NEPalavrasChavesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(NEPalavrasChavesApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [NEPalavrasChavesTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all nepalavraschaves items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await nepalavraschavesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(nepalavraschavesApi.getAll()).rejects.toThrow(NEPalavrasChavesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await nepalavraschavesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-NEPalavrasChaves_last_getAll_${200}`)));
    });
    ...
    it('should throw NEPalavrasChavesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockNEPalavrasChaves = NEPalavrasChavesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockNEPalavrasChaves,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch nepalavraschaves by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await nepalavraschavesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockNEPalavrasChaves));
      ...
      expect(result.data).toEqual(mockNEPalavrasChaves);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NEPalavrasChaves',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterNEPalavrasChaves = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter nepalavraschaves items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await nepalavraschavesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockNEPalavrasChaves: INEPalavrasChaves = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new nepalavraschaves successfully', async () => {
      const responseNEPalavrasChaves = { ...mockNEPalavrasChaves, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseNEPalavrasChaves,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await nepalavraschavesApi.addAndUpdate(mockNEPalavrasChaves);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockNEPalavrasChaves,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing nepalavraschaves successfully', async () => {
      const existingNEPalavrasChaves = { ...mockNEPalavrasChaves, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingNEPalavrasChaves,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await nepalavraschavesApi.addAndUpdate(existingNEPalavrasChaves);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(nepalavraschavesApi.addAndUpdate(mockNEPalavrasChaves)).rejects.toThrow(NEPalavrasChavesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NEPalavrasChaves',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete nepalavraschaves successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await nepalavraschavesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(nepalavraschavesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'NEPalavrasChaves',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(nepalavraschavesApi.delete(1)).rejects.toThrow(NEPalavrasChavesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterNEPalavrasChaves = { funcionario: 1 };
      
      const result = nepalavraschavesApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await nepalavraschavesApi.delete(1);
  ...
  describe('NEPalavrasChavesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new NEPalavrasChavesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('NEPalavrasChavesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new NEPalavrasChavesApiError('Test message', 500);
