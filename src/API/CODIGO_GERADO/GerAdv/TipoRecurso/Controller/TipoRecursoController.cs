...
public partial class TipoRecursoController(ITipoRecursoService tiporecursoService) : ControllerBase
{
    ...
    {
        _logger.Info("TipoRecurso: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _tiporecursoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("TipoRecurso: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _tiporecursoService.UpdateColumns(parameters, uri);
        if (!result)
        {
