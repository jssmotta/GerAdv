...
const OperadoresComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IOperadores | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IOperadores | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IOperadores);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <OperadoresWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedOperadores={editar ?? addValue ?? OperadoresEmpty()} /> 
            )}
