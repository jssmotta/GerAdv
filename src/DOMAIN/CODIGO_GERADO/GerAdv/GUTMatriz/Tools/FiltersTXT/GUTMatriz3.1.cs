...
public static class FilterGUTMatrizTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar GUTMatriz em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGUTMatriz:

- O filter irá filtrar os GUTMatriz conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterGUTTipo: filtro específico para G U T Tipo
...
CAMPOS filterGUTTipo (G U T Tipo):
- nome: _nome ou descrição para G U T Tipo;
- ordem: número ou IDs;
...
- Se ouvir "todos os GUTMatriz" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterGUTMatriz? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt GUTMatriz length: {result.Length}");
#endif
