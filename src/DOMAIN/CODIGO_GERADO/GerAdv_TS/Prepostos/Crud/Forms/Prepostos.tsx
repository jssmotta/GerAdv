...
if (getParamFromUrl("setor") > 0) {
 ...
 const addValorSetor = (e: any) => {
  ...
  <>
        ...
        <div className={isMobile ? 'form-container-Prepostos' : 'form-container form-container-Prepostos'}>
       
            <form className='formInputCadInc' id={`PrepostosForm-${prepostosData.id}`} onSubmit={onConfirm}>

                <ButtonsCrud data={prepostosData} isSubmitting={isSubmitting} onClose={onClose} formId={`PrepostosForm-${prepostosData.id}`} preventPropagation={true} onSave={handleDirectSave} />
