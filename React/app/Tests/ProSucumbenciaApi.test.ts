...
describe('ProSucumbenciaApi', () => {
  let prosucumbenciaApi: ProSucumbenciaApi;
  ...
  beforeEach(() => {
    ...
    prosucumbenciaApi = new ProSucumbenciaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(prosucumbenciaApi).toBeInstanceOf(ProSucumbenciaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProSucumbenciaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProSucumbenciaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all prosucumbencia items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prosucumbenciaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(prosucumbenciaApi.getAll()).rejects.toThrow(ProSucumbenciaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await prosucumbenciaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProSucumbencia_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProSucumbencia',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw ProSucumbenciaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProSucumbencia = ProSucumbenciaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProSucumbencia,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch prosucumbencia by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prosucumbenciaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProSucumbencia));
      ...
      expect(result.data).toEqual(mockProSucumbencia);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProSucumbencia',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter prosucumbencia items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await prosucumbenciaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new prosucumbencia successfully', async () => {
      const responseProSucumbencia = { ...mockProSucumbencia, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProSucumbencia,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prosucumbenciaApi.addAndUpdate(mockProSucumbencia);
    ...
    it('should update existing prosucumbencia successfully', async () => {
      const existingProSucumbencia = { ...mockProSucumbencia, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProSucumbencia,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prosucumbenciaApi.addAndUpdate(existingProSucumbencia);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(prosucumbenciaApi.addAndUpdate(mockProSucumbencia)).rejects.toThrow(ProSucumbenciaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProSucumbencia',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete prosucumbencia successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await prosucumbenciaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(prosucumbenciaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProSucumbencia',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(prosucumbenciaApi.delete(1)).rejects.toThrow(ProSucumbenciaApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await prosucumbenciaApi.delete(1);
  ...
  describe('ProSucumbenciaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProSucumbenciaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProSucumbenciaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProSucumbenciaApiError('Test message', 500);
