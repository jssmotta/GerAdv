...
import { GruposEmpresas } from '../../Models/GruposEmpresas';
...
export class GruposEmpresasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/GruposEmpresas`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GruposEmpresas_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GruposEmpresas');
        }
    }
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Grupos Empresas com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterGruposEmpresas): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GruposEmpresas_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de GruposEmpresas');
        }
    }

            public async filter(filtro: FilterGruposEmpresas): Promise<AxiosResponse> {
    ...
    public async addAndUpdate(regGruposEmpresas: IGruposEmpresas): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regGruposEmpresas as GruposEmpresas, this.getHeaders());
            var register = result.data as IGruposEmpresas;        
            const action = regGruposEmpresas.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Grupos Empresas');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o GruposEmpresas. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'GruposEmpresas',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Grupos Empresas com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGruposEmpresas) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<GruposEmpresas[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de gruposempresas:', error);
            this.notificationService.notify({
              entity: 'GruposEmpresas',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gruposempresas'
            });
          }
        });
    }
