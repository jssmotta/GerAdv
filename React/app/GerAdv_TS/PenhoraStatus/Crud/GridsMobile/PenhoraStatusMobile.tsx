...
import { usePenhoraStatusGrid } from "../../Hooks/hookPenhoraStatusGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("penhorastatus-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    const SearchFromCellPenhora = (props: any) => {
      return (
        <>
          <td>
            <div onClick={() => openSearchCellPenhora(props.dataItem.id)}>
              <span title="Pesquisar relacionados em Penhora">
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-penhorastatus .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("penhorastatus-columns-order-mobile"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-penhorastatus",
      columns: exportColumns,
      sheetName: "PenhoraStatus",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-PenhoraStatus-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-PenhoraStatus-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-penhorastatus"
              key={`penhorastatus-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
