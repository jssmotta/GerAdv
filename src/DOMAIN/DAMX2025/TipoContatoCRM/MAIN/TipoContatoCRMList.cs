...
#region ListarDados_TipoContatoCRM
    public static async IAsyncEnumerable<DBTipoContatoCRM> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoContatoCRM(dbRec);
        }
    }

    public static IEnumerable<DBTipoContatoCRM> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
