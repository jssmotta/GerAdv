...
public partial class UFController(IUFService ufService) : ControllerBase
{
    ...
    {
        _logger.Info("UF: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _ufService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("UF: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _ufService.UpdateColumns(parameters, uri);
        if (!result)
        {
