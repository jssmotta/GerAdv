...
public partial class DBGUTPeriodicidade
{
    ...
    /// </summary>
    public static async Task<DBGUTPeriodicidade> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBGUTPeriodicidade();
...
#if (!NOTSTORED_GUTPeriodicidade)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFNome || pFldFIntervaloDias || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFNome)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFIntervaloDias)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.IntervaloDias, FIntervaloDias, EGenericTypeFields.FNumber);
        if (pFldFGuid)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBGUTPeriodicidadeDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
