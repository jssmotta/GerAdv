...
export const useStatusTarefasList = (dataService: IStatusTarefasService, currentFilter?: FilterStatusTarefas | undefined | null) => {
  const [data, setData] = useState<IStatusTarefas[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterStatusTarefas | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterStatusTarefas | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterStatusTarefas);
      setData(result);
    } catch (err) {
