...
import EventoPrazoAgendaWindow from '../Crud/Grids/EventoPrazoAgendaWindow';
...
import { useEventoPrazoAgendaComboBox } from '../Hooks/hookEventoPrazoAgenda';
const EventoPrazoAgendaComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useEventoPrazoAgendaComboBox(eventoprazoagendaService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IEventoPrazoAgenda | null>(null);
const [addRecord, setAddRecord] = useState<IEventoPrazoAgenda | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await eventoprazoagendaService.fetchEventoPrazoAgendaById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nome });
  } catch (error) {
  console.log('Erro ao carregar Evento Prazo Agenda:');
}
};
...
const handleActionClick = () => {
  ...
  const newRecord = EventoPrazoAgendaEmpty();
...
const handleSuccess = (newEventoPrazoAgenda?: IEventoPrazoAgenda) => {
  if (newEventoPrazoAgenda) {
    setValue({ id: newEventoPrazoAgenda.id, nome: newEventoPrazoAgenda.nome });
    loadRecordForEdit(newEventoPrazoAgenda.id);
    handleValueChange(newEventoPrazoAgenda);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .eventoprazoagenda-dropdown-popup-msi {
  ...
  .eventoprazoagenda-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .eventoprazoagenda-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .eventoprazoagenda-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedEventoPrazoAgenda={editRecord || addRecord || EventoPrazoAgendaEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'eventoprazoagenda-dropdown-popup-msi'
          }}
          />
