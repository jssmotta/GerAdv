...
import { Precatoria } from '../../Models/Precatoria';
...
export class PrecatoriaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class PrecatoriaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Precatoria`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Precatoria_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Precatoria');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Precatoria_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Precatoria com ID ${id}`);
        }
    }

            public async filter(filtro: FilterPrecatoria): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Precatoria_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Precatoria');
                }
            }


    public async validation(regPrecatoria: IPrecatoria): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regPrecatoria as Precatoria, this.getHeaders());
    ...
    public async addAndUpdate(regPrecatoria: IPrecatoria): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regPrecatoria as Precatoria, this.getHeaders());
            var register = result.data as IPrecatoria;        
            const action = regPrecatoria.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Precatoria');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Precatoria. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Precatoria',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Precatoria com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterPrecatoria) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Precatoria[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de precatoria:', error);
            this.notificationService.notify({
              entity: 'Precatoria',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos precatoria'
            });
          }
        });
    }
