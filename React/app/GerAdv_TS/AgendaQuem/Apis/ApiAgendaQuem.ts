...
import { AgendaQuem } from '../../Models/AgendaQuem';
...
export class AgendaQuemApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class AgendaQuemApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AgendaQuem`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaQuem_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AgendaQuem');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaQuem_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Agenda Quem com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAgendaQuem): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaQuem_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AgendaQuem');
                }
            }


    public async validation(regAgendaQuem: IAgendaQuem): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAgendaQuem as AgendaQuem, this.getHeaders());
    ...
    public async addAndUpdate(regAgendaQuem: IAgendaQuem): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAgendaQuem as AgendaQuem, this.getHeaders());
            var register = result.data as IAgendaQuem;        
            const action = regAgendaQuem.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Agenda Quem');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AgendaQuem. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AgendaQuem',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Agenda Quem com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAgendaQuem) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AgendaQuem[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de agendaquem:', error);
            this.notificationService.notify({
              entity: 'AgendaQuem',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos agendaquem'
            });
          }
        });
    }
