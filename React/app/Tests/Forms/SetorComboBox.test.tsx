// app/TestsForms/SetorComboBox.test.tsx - Vers�o Corrigida
...
import { SetorEmpty, SetorTestEmpty } from '@/app/GerAdv_TS/Models/Setor';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Setor/Hooks/hookSetor', () => ({
  useSetorComboBox: jest.fn(() => ({
    options: [
    { ...SetorEmpty(), id: 1, descricao: 'Setor 1' },
    { ...SetorEmpty(), id: 2, descricao: 'Setor 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Setor/Apis/ApiSetor', () => ({
  SetorApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Setor' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Setor' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Setor/Services/Setor.service', () => ({
  SetorService: jest.fn().mockImplementation(() => ({
    fetchSetorById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Setor' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Setor/Crud/Grids/SetorWindow', () => {
  return function MockSetorWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='setor-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'Setor' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...SetorTestEmpty(), id: 1, descricao: 'Setor 1' },
  { ...SetorTestEmpty(), id: 2, descricao: 'Setor 2' },
];
...
describe('SetorComboBox', () => {
  ...
  const { useSetorComboBox } = require('@/app/GerAdv_TS/Setor/Hooks/hookSetor');
  (useSetorComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<SetorComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Setor')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...SetorTestEmpty(), id: 1, descricao: 'Setor 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useSetorComboBox } = require('@/app/GerAdv_TS/Setor/Hooks/hookSetor');
    useSetorComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens SetorWindow on action button click (add)', async () => {
    render(<SetorComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('setor-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('setor-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<SetorComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles SetorWindow onClose and onSuccess', () => {
    render(<SetorComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<SetorComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
