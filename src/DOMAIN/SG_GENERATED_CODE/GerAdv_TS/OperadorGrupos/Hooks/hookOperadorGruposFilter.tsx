...
import { FilterOperadorGrupos } from '@/app/GerAdv_TS/OperadorGrupos/Filters/OperadorGrupos';
import FilterDialogButton from '@/app/components/Cruds/FilterDialogButton';


interface UseOperadorGruposFilterProps {
    handleFetchWithFilter: (filtro?: FilterOperadorGrupos | undefined | null) => Promise<void>;
}

interface UseOperadorGruposFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterOperadorGrupos;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterOperadorGrupos>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterOperadorGrupos) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterOperadorGrupos>) => React.ReactNode;
...
export const useOperadorGruposFilter = ({ handleFetchWithFilter }: UseOperadorGruposFilterProps): UseOperadorGruposFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterOperadorGrupos>({} as FilterOperadorGrupos);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterOperadorGrupos;
    ...
    const handleConfirmSearch = async (filter: FilterOperadorGrupos) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterOperadorGrupos);
        sessionStorage.removeItem(btoa('OperadorGruposFilter'));
        handleFetchWithFilter({} as FilterOperadorGrupos);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterOperadorGrupos>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
