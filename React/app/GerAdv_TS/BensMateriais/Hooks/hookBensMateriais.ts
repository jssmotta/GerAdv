...
export const useBensMateriaisForm = (
  ...
  const loadBensMateriais = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
...
export const useBensMateriaisList = (
  dataService: IBensMateriaisService,
  currentFilter?: FilterBensMateriais | undefined | null,
) => {
  const [data, setData] = useState<IBensMateriais[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterBensMateriais | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterBensMateriais | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar bensmateriais";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsBensMateriais() {
  ...
  function validate(data: IBensMateriais): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.nfnro.length > 255) {
        return {
          isValid: false,
          message: "O campo NFNRO não pode ter mais de 255 caracteres.",
        };
      }
      if (data.nroserieproduto.length > 100) {
        return {
          isValid: false,
          message:
            "O campo NroSerieProduto não pode ter mais de 100 caracteres.",
        };
      }
      if (data.comprador.length > 100) {
        return {
          isValid: false,
          message: "O campo Comprador não pode ter mais de 100 caracteres.",
        };
      }
      if (data.observacoes.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Observacoes não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.nomevendedor.length > 255) {
        return {
          isValid: false,
          message: "O campo NomeVendedor não pode ter mais de 255 caracteres.",
        };
      }
