...
import { AgendaApiError } from '../GerAdv_TS/Agenda/Apis/ApiAgenda';
import { AgendaApi } from '@/app/GerAdv_TS/Agenda/Apis/ApiAgenda';
import { FilterAgenda } from '../GerAdv_TS/Agenda/Filters/Agenda';
...
describe('AgendaApi', () => {
  ...
  beforeEach(() => {
    ...
    agendaApi = new AgendaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendaApi).toBeInstanceOf(AgendaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all agenda items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendaApi.getAll()).rejects.toThrow(AgendaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Agenda_last_getAll_${200}`)));
    });
    ...
    it('should throw AgendaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgenda = AgendaTestEmpty();
    ...
    it('should fetch agenda by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgenda));
  ...
  describe('filter', () => {
    ...
    it('should filter agenda items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new agenda successfully', async () => {
      const responseAgenda = { ...mockAgenda, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgenda,
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendaApi.addAndUpdate(mockAgenda);
    ...
    it('should update existing agenda successfully', async () => {
      const existingAgenda = { ...mockAgenda, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgenda,
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendaApi.addAndUpdate(existingAgenda);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendaApi.addAndUpdate(mockAgenda)).rejects.toThrow(AgendaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Agenda',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agenda successfully', async () => {
      ...
      const result = await agendaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Agenda',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendaApi.delete(1)).rejects.toThrow(AgendaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgenda = { funcionario: 1 };
      
      const result = agendaApi.useFilter(mockFilter);
  ...
  describe('AgendaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaApiError('Test message', 500);
