 ...
 const addValorTipoValorProcesso = (e: any) => {
    ...
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao submeter formulário de ProValores:", error);
  ...
  const handleDirectSave = () => {
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao salvar ProValores diretamente:", error);
        setIsSubmitting(false);
        if (onError) onError();
      }
    }
  ...
  <>
        ...
        <div className={isMobile ? 'form-container-ProValores' : 'form-container form-container-ProValores'}>
       
            <form className='formInputCadInc' id={`ProValoresForm-${provaloresData.id}`} onSubmit={onConfirm}>

                <ButtonsCrud data={provaloresData} isSubmitting={isSubmitting} onClose={onClose} formId={`ProValoresForm-${provaloresData.id}`} preventPropagation={true} onSave={handleDirectSave} />
