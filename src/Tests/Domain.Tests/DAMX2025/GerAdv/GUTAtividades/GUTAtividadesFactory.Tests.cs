...
public class FGUTAtividadesFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FGUTAtividadesFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FGUTAtividadesFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FGUTAtividadesFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "agtCodigo")
        var dt = new DataTable();
        dt.Columns.Add("agtCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["agtCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FGUTAtividades>();
    }
    ...
    {
        // Arrange
        var factory = new FGUTAtividadesFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("agtCodigo", typeof(int));
        mockDataTable.Columns.Add("agtNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["agtCodigo"] = testId;
        row["agtNome"] = "Test GUTAtividades";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["agtCodigo"] = 123;
        row["agtNome"] = "Test GUTAtividades Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FGUTAtividades properties
        }
    }
