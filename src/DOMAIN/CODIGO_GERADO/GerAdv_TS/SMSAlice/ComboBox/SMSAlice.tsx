...
const SMSAliceComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ISMSAlice | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ISMSAlice | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ISMSAlice);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <SMSAliceWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedSMSAlice={editar ?? addValue ?? SMSAliceEmpty()} /> 
            )}
