...
export const useFuncaoList = (
  dataService: IFuncaoService,
  currentFilter?: FilterFuncao | undefined | null,
) => {
  const [data, setData] = useState<IFuncao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterFuncao | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterFuncao | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterFuncao));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar funcao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
