...
export class SetorApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class SetorApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Setor`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Setor_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Setor');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Setor_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Setor com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Setor_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Setor');
        }
    }

            public async filter(filtro: FilterSetor): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Setor_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Setor');
                }
            }


    public async validation(regSetor: ISetor): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regSetor as Setor, this.getHeaders());
    ...
    public async addAndUpdate(regSetor: ISetor): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Setor');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Setor. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Setor',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Setor com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterSetor) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Setor[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de setor:', error);
            this.notificationService.notify({
              entity: 'Setor',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos setor'
            });
          }
        });
    }
