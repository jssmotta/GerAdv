...
public partial class AdvogadosController(IAdvogadosService advogadosService) : ControllerBase
{
    ...
    {
        _logger.Info("Advogados: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _advogadosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Advogados: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _advogadosService.UpdateColumns(parameters, uri);
        if (!result)
        {
