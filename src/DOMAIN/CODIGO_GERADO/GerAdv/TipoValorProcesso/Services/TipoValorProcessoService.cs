...
public partial class TipoValorProcessoService(IOptions<AppSettings> appSettings, IFTipoValorProcessoFactory tipovalorprocessoFactory, ITipoValorProcessoReader reader, ITipoValorProcessoValidation validation, ITipoValorProcessoWriter writer, IProValoresService provaloresService, HybridCache cache, IMemoryCache memory) : ITipoValorProcessoService, IDisposable
{
    private readonly IOptions<AppSettings> _appSettings = appSettings;
    private readonly HybridCache _cache = cache;
    private readonly IMemoryCache _memoryCache = memory;
    private bool _disposed;
    private readonly IFTipoValorProcessoFactory tipovalorprocessoFactory = tipovalorprocessoFactory;
    private readonly ITipoValorProcessoReader reader = reader;
    private readonly ITipoValorProcessoValidation validation = validation;
    private readonly ITipoValorProcessoWriter writer = writer;
    ...
    public async Task<IEnumerable<TipoValorProcessoResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterTipoValorProcesso filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-{max}-TipoValorProcesso-Filter-{where.GetHashCode2()}{filterHash}";
        var entryOptions = new HybridCacheEntryOptions
        {
    ...
    private async Task<TipoValorProcessoResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<TipoValorProcessoResponse?> AddAndUpdate([FromBody] Models.TipoValorProcesso regTipoValorProcesso, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regTipoValorProcesso, oCnn);
    ...
    public async Task<TipoValorProcessoResponse?> Validation([FromBody] Models.TipoValorProcesso regTipoValorProcesso, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regTipoValorProcesso.Id, oCnn);
    }
    ...
    public async Task<TipoValorProcessoResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var tipovalorprocesso = await reader.Read(id, oCnn);
        try
        {
            if (tipovalorprocesso != null)
            {
                await writer.Delete(tipovalorprocesso, 0, oCnn);
