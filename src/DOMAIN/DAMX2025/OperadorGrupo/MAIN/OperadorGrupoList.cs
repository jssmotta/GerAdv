...
#region ListarDados_OperadorGrupo
    public static async IAsyncEnumerable<DBOperadorGrupo> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperadorGrupo(dbRec);
        }
    }

    public static IEnumerable<DBOperadorGrupo> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
