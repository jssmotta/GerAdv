...
#region ListarDados_DocsRecebidosItens
    public static async IAsyncEnumerable<DBDocsRecebidosItens> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBDocsRecebidosItens(dbRec);
        }
    }

    public static IEnumerable<DBDocsRecebidosItens> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
