// app/TestsForms/AlarmSMSComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import AlarmSMSComboBox from '@/app/GerAdv_TS/AlarmSMS/ComboBox/AlarmSMS';
...
import { AlarmSMSEmpty, AlarmSMSTestEmpty } from '@/app/GerAdv_TS/Models/AlarmSMS';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/AlarmSMS/Hooks/hookAlarmSMS', () => ({
  useAlarmSMSComboBox: jest.fn(() => ({
    options: [
    { ...AlarmSMSEmpty(), id: 1, descricao: 'Alarm S M S 1' },
    { ...AlarmSMSEmpty(), id: 2, descricao: 'Alarm S M S 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/AlarmSMS/Apis/ApiAlarmSMS', () => ({
  AlarmSMSApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock AlarmSMS' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock AlarmSMS' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/AlarmSMS/Services/AlarmSMS.service', () => ({
  AlarmSMSService: jest.fn().mockImplementation(() => ({
    fetchAlarmSMSById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock AlarmSMS' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/AlarmSMS/Crud/Grids/AlarmSMSWindow', () => {
  return function MockAlarmSMSWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='alarmsms-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'Alarm S M S' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...AlarmSMSTestEmpty(), id: 1, descricao: 'Alarm S M S 1' },
  { ...AlarmSMSTestEmpty(), id: 2, descricao: 'Alarm S M S 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: AlarmSMSEmpty(), 
};
describe('AlarmSMSComboBox', () => {
  ...
  const { useAlarmSMSComboBox } = require('@/app/GerAdv_TS/AlarmSMS/Hooks/hookAlarmSMS');
  (useAlarmSMSComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<AlarmSMSComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Alarm S M S')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Alarm S M S')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...AlarmSMSTestEmpty(), id: 1, descricao: 'Alarm S M S 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useAlarmSMSComboBox } = require('@/app/GerAdv_TS/AlarmSMS/Hooks/hookAlarmSMS');
    useAlarmSMSComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens AlarmSMSWindow on action button click (add)', async () => {
    render(<AlarmSMSComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alarmsms-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('alarmsms-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<AlarmSMSComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles AlarmSMSWindow onClose and onSuccess', () => {
    render(<AlarmSMSComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<AlarmSMSComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
