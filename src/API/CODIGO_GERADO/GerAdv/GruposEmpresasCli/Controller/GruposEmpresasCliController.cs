...
public partial class GruposEmpresasCliController(IGruposEmpresasCliService gruposempresascliService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterGruposEmpresasCli filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("GruposEmpresasCli: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _gruposempresascliService.Filter(max, filtro, uri);
        return Ok(result);
    }
