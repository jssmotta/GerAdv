...
{
    Task<GUTMatrizResponse?> ReadAsync(int id, MsiSqlConnection? oCnn);
    Task<Models.GUTMatriz?> ReadMAsync(int id, MsiSqlConnection? oCnn);
    ...
    Task<IEnumerable<GUTMatrizResponseAll>> ListarAsync(MsiSqlConnection oCnn, int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken);
}