...
#region ListarDados_TipoProDesposito
    public static async IAsyncEnumerable<DBTipoProDesposito> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoProDesposito(dbRec);
        }
    }

    public static IEnumerable<DBTipoProDesposito> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
