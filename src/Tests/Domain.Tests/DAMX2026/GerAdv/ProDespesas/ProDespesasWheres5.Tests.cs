...
#region Test Data Setup Helpers
    ...
    private void SetupMockFProDespesas(int? LigacaoID = 1, int? Cliente = 1, bool? Corrigido = false, string? Data = "24/04/1975", decimal? ValorOriginal = 1m, int? Processo = 1, int? Quitado = 1, string? DataCorrecao = "24/04/1975", decimal? Valor = 1m, bool? Tipo = true, string? Historico = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", bool? LivroCaixa = false)
    {
        ...
        {
            _mockFProDespesas.Setup(f => f.FData).Returns((DateOnly? )null);
        }
        else
        {
            ...
            _mockFProDespesas.Setup(f => f.FData).Returns(dateOnly);
        }
        ...
        if (string.IsNullOrEmpty(DataCorrecao))
        {
            _mockFProDespesas.Setup(f => f.FDataCorrecao).Returns((DateOnly? )null);
        }
        else
        {
            DateTime parsedDateTime;
            if (!DateTime.TryParse(DataCorrecao, new CultureInfo("pt-BR"), DateTimeStyles.None, out parsedDateTime))
            {
                parsedDateTime = DateTime.Parse(DataCorrecao, CultureInfo.InvariantCulture);
            }

            var dateOnly = DateOnly.FromDateTime(parsedDateTime);
            _mockFProDespesas.Setup(f => f.FDataCorrecao).Returns(dateOnly);
        }
...
#region Integration Tests
    ...
    public void Read_ShouldPassCorrectParametersToFactory()
    {
        ...
        var parameters = new List<SqlParameter>
        {
            new SqlParameter($"@{DBProDespesasDicInfo.CampoNome}", TConstantes.TestData),
        };
        SetupMockFProDespesas();
        _mockProDespesasFactory.Setup(f => f.CreateFromParameters(parameters, _mockConnection.Object, "", "", where, "")).Returns(_mockFProDespesas.Object);
        // Act
...
#region DateTime Tests
    ...
    public void Read_WithValidDate_DataCorrecao_Formats_ShouldParseCorrectly(string dateString)
    {
        // Arrange
        var where = "Id = @Id";
        var parameters = CreateTestParameters();
        ...
        var result = _prodespesasWhere.Read(where, parameters, _mockConnection.Object);
        // Assert
        result.DataCorrecao.Should().Be(expectedDate.ToString("dd/MM/yyyy"));
    }
#endregion
#endregion
}