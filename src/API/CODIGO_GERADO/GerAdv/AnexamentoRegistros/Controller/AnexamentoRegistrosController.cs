...
public partial class AnexamentoRegistrosController(IAnexamentoRegistrosService anexamentoregistrosService) : ControllerBase
{
    ...
    {
        _logger.Info("AnexamentoRegistros: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _anexamentoregistrosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AnexamentoRegistros: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _anexamentoregistrosService.UpdateColumns(parameters, uri);
        if (!result)
        {
