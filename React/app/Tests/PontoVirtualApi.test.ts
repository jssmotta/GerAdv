...
import { PontoVirtualApiError } from '../GerAdv_TS/PontoVirtual/Apis/ApiPontoVirtual';
import { PontoVirtualApi } from '@/app/GerAdv_TS/PontoVirtual/Apis/ApiPontoVirtual';
import { FilterPontoVirtual } from '../GerAdv_TS/PontoVirtual/Filters/PontoVirtual';
...
describe('PontoVirtualApi', () => {
  let pontovirtualApi: PontoVirtualApi;
  ...
  beforeEach(() => {
    ...
    pontovirtualApi = new PontoVirtualApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(pontovirtualApi).toBeInstanceOf(PontoVirtualApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PontoVirtualApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PontoVirtualApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all pontovirtual items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await pontovirtualApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(pontovirtualApi.getAll()).rejects.toThrow(PontoVirtualApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await pontovirtualApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-PontoVirtual_last_getAll_${200}`)));
    });
    ...
    it('should throw PontoVirtualApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPontoVirtual = PontoVirtualTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockPontoVirtual,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch pontovirtual by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await pontovirtualApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPontoVirtual));
      ...
      expect(result.data).toEqual(mockPontoVirtual);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PontoVirtual',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterPontoVirtual = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter pontovirtual items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await pontovirtualApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockPontoVirtual: IPontoVirtual = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new pontovirtual successfully', async () => {
      const responsePontoVirtual = { ...mockPontoVirtual, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePontoVirtual,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await pontovirtualApi.addAndUpdate(mockPontoVirtual);
    ...
    it('should update existing pontovirtual successfully', async () => {
      const existingPontoVirtual = { ...mockPontoVirtual, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPontoVirtual,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await pontovirtualApi.addAndUpdate(existingPontoVirtual);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(pontovirtualApi.addAndUpdate(mockPontoVirtual)).rejects.toThrow(PontoVirtualApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PontoVirtual',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete pontovirtual successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await pontovirtualApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(pontovirtualApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PontoVirtual',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(pontovirtualApi.delete(1)).rejects.toThrow(PontoVirtualApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterPontoVirtual = { funcionario: 1 };
      
      const result = pontovirtualApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await pontovirtualApi.delete(1);
  ...
  describe('PontoVirtualApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PontoVirtualApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PontoVirtualApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PontoVirtualApiError('Test message', 500);
