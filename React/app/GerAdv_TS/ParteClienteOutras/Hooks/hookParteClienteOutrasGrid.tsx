...
// useParteClienteOutrasGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de ParteClienteOutras
'use client';
...
interface UseParteClienteOutrasGridProps {
...
export const useParteClienteOutrasGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  initialTake = 10, 
  useCustomSort = false
}: UseParteClienteOutrasGridProps) => {
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('parteclienteoutras-columns-order'), JSON.stringify(reorderableFields));
