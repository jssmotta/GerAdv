...
export const ClientesForm: React.FC<ClientesFormProps> = ({
  ...
  const [_nomeRegimeTributacao, setNomeRegimeTributacao] = useState("");
  const regimetributacaoApi = new RegimeTributacaoApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  const [_nomeEnquadramentoEmpresa, setNomeEnquadramentoEmpresa] = useState("");
  const enquadramentoempresaApi = new EnquadramentoEmpresaApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  if (getParamFromUrl("enquadramentoempresa") > 0) {
    if (clientesData.id === 0 && clientesData.enquadramentoempresa == 0) {
      ...
      clientesData.enquadramentoempresa = getParamFromUrl(
