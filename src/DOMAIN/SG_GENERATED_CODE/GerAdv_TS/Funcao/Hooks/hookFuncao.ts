...
export const useFuncaoList = (dataService: IFuncaoService, currentFilter?: FilterFuncao | undefined | null) => {
  const [data, setData] = useState<IFuncao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterFuncao | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterFuncao | undefined | null) => {
