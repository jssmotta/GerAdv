...
public class FNEPalavrasChavesFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FNEPalavrasChavesFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FNEPalavrasChavesFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FNEPalavrasChavesFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "npcCodigo")
        var dt = new DataTable();
        dt.Columns.Add("npcCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["npcCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FNEPalavrasChaves>();
    }
    ...
    {
        // Arrange
        var factory = new FNEPalavrasChavesFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("npcCodigo", typeof(int));
        mockDataTable.Columns.Add("npcNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["npcCodigo"] = testId;
        row["npcNome"] = "Test NEPalavrasChaves";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["npcCodigo"] = 123;
        row["npcNome"] = "Test NEPalavrasChaves Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FNEPalavrasChaves properties
        }
    }
