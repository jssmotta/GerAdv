...
describe('DivisaoTribunalApi', () => {
  let divisaotribunalApi: DivisaoTribunalApi;
  ...
  beforeEach(() => {
    ...
    divisaotribunalApi = new DivisaoTribunalApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(divisaotribunalApi).toBeInstanceOf(DivisaoTribunalApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new DivisaoTribunalApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(DivisaoTribunalApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all divisaotribunal items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await divisaotribunalApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(divisaotribunalApi.getAll()).rejects.toThrow(DivisaoTribunalApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await divisaotribunalApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-DivisaoTribunal_last_getAll_${200}`)));
    });
    ...
    it('should throw DivisaoTribunalApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockDivisaoTribunal = DivisaoTribunalTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockDivisaoTribunal,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch divisaotribunal by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await divisaotribunalApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockDivisaoTribunal));
      ...
      expect(result.data).toEqual(mockDivisaoTribunal);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DivisaoTribunal',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter divisaotribunal items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await divisaotribunalApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new divisaotribunal successfully', async () => {
      const responseDivisaoTribunal = { ...mockDivisaoTribunal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseDivisaoTribunal,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await divisaotribunalApi.addAndUpdate(mockDivisaoTribunal);
    ...
    it('should update existing divisaotribunal successfully', async () => {
      const existingDivisaoTribunal = { ...mockDivisaoTribunal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingDivisaoTribunal,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await divisaotribunalApi.addAndUpdate(existingDivisaoTribunal);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(divisaotribunalApi.addAndUpdate(mockDivisaoTribunal)).rejects.toThrow(DivisaoTribunalApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DivisaoTribunal',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete divisaotribunal successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await divisaotribunalApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(divisaotribunalApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DivisaoTribunal',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(divisaotribunalApi.delete(1)).rejects.toThrow(DivisaoTribunalApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await divisaotribunalApi.delete(1);
  ...
  describe('DivisaoTribunalApiError', () => {
    it('should create error with correct properties', () => {
      const error = new DivisaoTribunalApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('DivisaoTribunalApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new DivisaoTribunalApiError('Test message', 500);
