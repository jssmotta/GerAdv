...
const StatusBiuComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IStatusBiu | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IStatusBiu | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IStatusBiu);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <StatusBiuWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedStatusBiu={editar ?? addValue ?? StatusBiuEmpty()} /> 
            )}
