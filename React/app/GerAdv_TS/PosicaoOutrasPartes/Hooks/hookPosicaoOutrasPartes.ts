...
export const usePosicaoOutrasPartesForm = (
  ...
  const loadPosicaoOutrasPartes = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Posicao Outras Partes";
...
export const usePosicaoOutrasPartesList = (
  dataService: IPosicaoOutrasPartesService,
  currentFilter?: FilterPosicaoOutrasPartes | undefined | null,
) => {
  const [data, setData] = useState<IPosicaoOutrasPartes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPosicaoOutrasPartes | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterPosicaoOutrasPartes | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar posicaooutraspartes";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsPosicaoOutrasPartes() {
  ...
  function validate(data: IPosicaoOutrasPartes): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.descricao.length > 30) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 30 caracteres.",
        };
      }
