...
import { useTipoProDespositoGrid } from "../../Hooks/hookTipoProDespositoGrid";
...
import TipoProDespositoHelpCrudButton from "../../Help/TipoProDespositoHelpCrudButton";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("tipoprodesposito-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-tipoprodesposito .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-tipoprodesposito",
      columns: exportColumns,
      sheetName: "TipoProDesposito",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-TipoProDesposito-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-tipoprodesposito",
      columns: exportColumnsPdf,
      title: "TipoProDesposito",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-TipoProDesposito-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
        ...
        {initialized && (
          <>
            <PageTitle
              ...
              helpButton={<TipoProDespositoHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-tipoprodesposito"
                      key={`tipoprodesposito-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as ITipoProDesposito);
