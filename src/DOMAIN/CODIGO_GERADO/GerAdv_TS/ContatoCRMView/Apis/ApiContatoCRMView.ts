...
import { ContatoCRMView } from '../../Models/ContatoCRMView';
...
export class ContatoCRMViewApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ContatoCRMViewApiError';
    this.status = status;
    this.code = code;
...
export class ContatoCRMViewApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ContatoCRMView`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMView_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ContatoCRMView');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMView_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Contato C R M View com ID ${id}`);
        }
    }

            public async filter(filtro: FilterContatoCRMView): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRMView_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ContatoCRMView');
                }
            }


    public async validation(regContatoCRMView: IContatoCRMView): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regContatoCRMView as ContatoCRMView, this.getHeaders());
    ...
    public async addAndUpdate(regContatoCRMView: IContatoCRMView): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regContatoCRMView as ContatoCRMView, this.getHeaders());
            var register = result.data as IContatoCRMView;        
            const action = regContatoCRMView.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Contato C R M View');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ContatoCRMView. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ContatoCRMView',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Contato C R M View com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterContatoCRMView) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ContatoCRMView[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de contatocrmview:', error);
            this.notificationService.notify({
              entity: 'ContatoCRMView',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos contatocrmview'
            });
          }
        });
    }
