...
export class ParteClienteOutrasApi {
 ...
 public async filterPreload(max: number, filtro: FilterParteClienteOutras): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParteClienteOutras-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterParteClienteOutras): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParteClienteOutras-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterParteClienteOutras) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ParteClienteOutras[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de ParteClienteOutras');
            this.notificationService.notify({
              entity: 'ParteClienteOutras',
              id: 0,
              action: NotifySystemActions.ERROR,
