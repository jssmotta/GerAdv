...
import { useProSucumbenciaGrid } from '@/app/GerAdv_TS/ProSucumbencia/Hooks/hookProSucumbenciaGrid';
import { IProSucumbencia } from '@/app/GerAdv_TS/ProSucumbencia/Interfaces/interface.ProSucumbencia';

// Sample minimal prosucumbencia item for tests
const sampleProSucumbencia = (overrides?: Partial<IProSucumbencia>): IProSucumbencia => ({
  id: 1,
...
describe('useProSucumbenciaGrid', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('inicializa com dados processados e paginação', () => {
    const data = [sampleProSucumbencia({ id: 1 }), sampleProSucumbencia({ id: 2 })];
  ...
  test('handleRowClick chama onRowClick com item correto', () => {
    const data = [sampleProSucumbencia({ id: 5 })];
    const onRowClick = jest.fn();
    const onDeleteClick = jest.fn();
    const setSelectedId = jest.fn();

    const { result } = renderHook(() =>
      useProSucumbenciaGrid({ data, onRowClick, onDeleteClick, setSelectedId })
    );
  ...
  test('resetColumnsOrder restaura ordem padrão e limpa localStorage', () => {
    const data = [sampleProSucumbencia()];
    const onRowClick = jest.fn();
    const onDeleteClick = jest.fn();
    const setSelectedId = jest.fn();
    ...
    try {
      localStorage.setItem(btoa('prosucumbencia-columns-order'), JSON.stringify(['data','data']));
    } catch(e) {}
    ...
    try {
      const v = localStorage.getItem(btoa('prosucumbencia-columns-order'));
      expect(v).toBeNull();
    } catch (e) {}
  });

  test('filterLogic filtra por data', async () => {
    const matching = sampleProSucumbencia({ data: '27/05/2022' });
    const notMatching = sampleProSucumbencia({ data: 'Outro' });
