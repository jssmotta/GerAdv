...
const FuncaoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IFuncao | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IFuncao | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IFuncao);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <FuncaoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedFuncao={editar ?? addValue ?? FuncaoEmpty()} /> 
            )}
