...
import FuncionariosWindow from '../Crud/Grids/FuncionariosWindow';
...
import { useFuncionariosComboBox } from '../Hooks/hookFuncionarios';
const FuncionariosComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'funcionariosInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useFuncionariosComboBox(funcionariosService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IFuncionarios | null>(null);
const [addRecord, setAddRecord] = useState<IFuncionarios | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await funcionariosService.fetchFuncionariosById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = FuncionariosEmpty();
...
const handleSuccess = (newFuncionarios?: IFuncionarios) => {
  if (newFuncionarios) {
    setValue({ id: newFuncionarios.id, nome: newFuncionarios.nome });
    loadRecordForEdit(newFuncionarios.id);
    handleValueChange(newFuncionarios);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .funcionarios-dropdown-popup-msi {
  ...
  .funcionarios-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .funcionarios-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .funcionarios-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedFuncionarios={editRecord || addRecord || FuncionariosEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'funcionarios-dropdown-popup-msi'
          }}
          />
...
export default FuncionariosComboBox;