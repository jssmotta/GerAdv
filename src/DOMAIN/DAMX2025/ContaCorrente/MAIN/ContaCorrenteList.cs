...
#region ListarDados_ContaCorrente
    public static async IAsyncEnumerable<DBContaCorrente> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBContaCorrente(dbRec);
        }
    }

    public static IEnumerable<DBContaCorrente> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
