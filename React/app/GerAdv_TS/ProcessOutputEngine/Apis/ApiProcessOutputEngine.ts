...
import { ProcessOutputEngine } from '../../Models/ProcessOutputEngine';
...
export class ProcessOutputEngineApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ProcessOutputEngineApiError';
    this.status = status;
    this.code = code;
...
export class ProcessOutputEngineApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProcessOutputEngine`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputEngine_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProcessOutputEngine');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputEngine_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Process Output Engine com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterProcessOutputEngine): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputEngine_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ProcessOutputEngine');
        }
    }

            public async filter(filtro: FilterProcessOutputEngine): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutputEngine_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProcessOutputEngine');
                }
            }


    public async validation(regProcessOutputEngine: IProcessOutputEngine): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProcessOutputEngine as ProcessOutputEngine, this.getHeaders());
    ...
    public async addAndUpdate(regProcessOutputEngine: IProcessOutputEngine): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProcessOutputEngine as ProcessOutputEngine, this.getHeaders());
            var register = result.data as IProcessOutputEngine;        
            const action = regProcessOutputEngine.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Process Output Engine');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProcessOutputEngine. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProcessOutputEngine',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Process Output Engine com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProcessOutputEngine) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProcessOutputEngine[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de processoutputengine:', error);
            this.notificationService.notify({
              entity: 'ProcessOutputEngine',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos processoutputengine'
            });
          }
        });
    }
