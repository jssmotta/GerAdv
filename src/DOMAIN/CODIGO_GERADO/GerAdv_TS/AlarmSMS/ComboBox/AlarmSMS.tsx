...
const AlarmSMSComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IAlarmSMS | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IAlarmSMS | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IAlarmSMS);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <AlarmSMSWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedAlarmSMS={editar ?? addValue ?? AlarmSMSEmpty()} /> 
            )}
