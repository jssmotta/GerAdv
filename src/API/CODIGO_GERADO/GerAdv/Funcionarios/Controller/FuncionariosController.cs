...
public partial class FuncionariosController(IFuncionariosService funcionariosService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterFuncionarios filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Funcionarios: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _funcionariosService.Filter(max, filtro, uri);
        return Ok(result);
    }
