...
#region ListarDados_OponentesRepLegal
    public static async IAsyncEnumerable<DBOponentesRepLegal> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOponentesRepLegal(dbRec);
        }
    }

    public static IEnumerable<DBOponentesRepLegal> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
