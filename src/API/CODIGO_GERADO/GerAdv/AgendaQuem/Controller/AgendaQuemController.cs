...
public partial class AgendaQuemController(IAgendaQuemService agendaquemService) : ControllerBase
{
    ...
    {
        _logger.Info("AgendaQuem: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _agendaquemService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AgendaQuem: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _agendaquemService.UpdateColumns(parameters, uri);
        if (!result)
        {
