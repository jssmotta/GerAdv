...
export const usePontoVirtualAcessosForm = (
  ...
  const loadPontoVirtualAcessos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Ponto Virtual Acessos";
...
export const usePontoVirtualAcessosList = (
  dataService: IPontoVirtualAcessosService,
  currentFilter?: FilterPontoVirtualAcessos | undefined | null,
) => {
  const [data, setData] = useState<IPontoVirtualAcessos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPontoVirtualAcessos | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterPontoVirtualAcessos | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar pontovirtualacessos";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsPontoVirtualAcessos() {
  async function runValidation(
    ...
    const pontovirtualacessosApi = new PontoVirtualAcessosApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IPontoVirtualAcessos): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      if (data.origem.length > 150) {
        return {
          isValid: false,
          message: "O campo Origem não pode ter mais de 150 caracteres.",
        };
      }
