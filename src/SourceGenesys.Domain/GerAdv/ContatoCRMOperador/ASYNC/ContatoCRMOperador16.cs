...
public partial class DBContatoCRMOperador
{
    ...
    /// </summary>
    public static async Task<DBContatoCRMOperador> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBContatoCRMOperador();
...
#if (!NOTSTORED_ContatoCRMOperador)
    /// <summary>
...
#if (!NOTSTORED_ContatoCRMOperador)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFContatoCRM || pFldFCargoEsc || pFldFOperador;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFContatoCRM)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.ContatoCRM, m_FContatoCRM, ETiposCampos.FNumber);
        if (pFldFCargoEsc)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.CargoEsc, m_FCargoEsc, ETiposCampos.FNumber);
        if (pFldFOperador)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBContatoCRMOperadorDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBContatoCRMOperadorDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
