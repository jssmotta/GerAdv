...
export class PontoVirtualApi {
 ...
 public async filterPreload(max: number, filtro: FilterPontoVirtual): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PontoVirtual-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterPontoVirtual): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PontoVirtual-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
