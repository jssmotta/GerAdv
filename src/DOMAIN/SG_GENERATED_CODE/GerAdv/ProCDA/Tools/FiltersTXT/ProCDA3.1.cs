...
public static class FilterProCDATXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProCDA em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProCDA:

- O filter irá filtrar os ProCDA conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProCDA:(Pro C D A)
- processo: número ou IDs;
- nome: _nome ou descrição para Pro C D A;
...
- Se ouvir "todos os ProCDA" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProCDA? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProCDA length: {result.Length}");
#endif
