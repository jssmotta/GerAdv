...
public class DadosProcuracaoHealthCheck(IOptions<AppSettings> appSettings, DadosProcuracaoService dadosprocuracaoService, HybridCache cache) : IHealthCheck, IDisposable
{
    ...
    private readonly DadosProcuracaoService _dadosprocuracaoService = dadosprocuracaoService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = "SELECT TOP (1) prcCliente,prcEstadoCivil,prcNacionalidade,prcProfissao,prcCTPS,prcPisPasep,prcRemuneracao,prcObjeto FROM dbo.DadosProcuracao (NOLOCK);";
                            tableCheck.CommandTimeout = 5;
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT DadosProcuracao successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT DadosProcuracao failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _dadosprocuracaoService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-DadosProcuracao";
                    ...
                    static async Task<IEnumerable<Models.DadosProcuracao>> GetData(string uri, CancellationToken token)
                    {
            ...
            return isHealthy ? HealthCheckResult.Healthy("DadosProcuracao is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
