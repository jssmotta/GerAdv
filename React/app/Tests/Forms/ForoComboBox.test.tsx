// app/TestsForms/ForoComboBox.test.tsx - Vers�o Corrigida
...
import { ForoEmpty, ForoTestEmpty } from '@/app/GerAdv_TS/Models/Foro';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Foro/Hooks/hookForo', () => ({
  useForoComboBox: jest.fn(() => ({
    options: [
    { ...ForoEmpty(), id: 1, nome: 'Foro 1' },
    { ...ForoEmpty(), id: 2, nome: 'Foro 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Foro/Apis/ApiForo', () => ({
  ForoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Foro' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Foro' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Foro/Services/Foro.service', () => ({
  ForoService: jest.fn().mockImplementation(() => ({
    fetchForoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Foro' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Foro/Crud/Grids/ForoWindow', () => {
  return function MockForoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='foro-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Foro' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ForoTestEmpty(), id: 1, nome: 'Foro 1' },
  { ...ForoTestEmpty(), id: 2, nome: 'Foro 2' },
];
...
describe('ForoComboBox', () => {
  ...
  const { useForoComboBox } = require('@/app/GerAdv_TS/Foro/Hooks/hookForo');
  (useForoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ForoComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Foro')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ForoTestEmpty(), id: 1, nome: 'Foro 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useForoComboBox } = require('@/app/GerAdv_TS/Foro/Hooks/hookForo');
    useForoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ForoWindow on action button click (add)', async () => {
    render(<ForoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('foro-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('foro-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ForoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ForoWindow onClose and onSuccess', () => {
    render(<ForoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ForoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
