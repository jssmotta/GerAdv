...
public partial class ClientesReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBClientesDicInfo.CampoNome}";
...
SELECT TOP (1) 
    ...
    cliVisto as Visto
FROM {oCnn.UseDbo}.Clientes dbq WITH (INDEX = idx_Clientes_AuditorDtAtu)
...
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBClientes.PTabelaNome,
...
FROM {oCnn.UseDbo}.Clientes WITH (INDEX = idx_{DBClientesDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [cliCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBClientesDicInfo.PTabelaNome}-RRSAA";
...
FROM [{oCnn.UseDbo}].[{DBClientesDicInfo.PTabelaNome}] (NOLOCK)
