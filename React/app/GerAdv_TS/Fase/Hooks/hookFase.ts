...
export const useFaseForm = (
  ...
  const loadFase = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialFase);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Fase';
      setError(errorMessage);
      console.log('Erro ao carregar Fase');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialFase]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadFase, resetForm]);
  return returnValue;
};


export const useFaseNotifications = (
...
export const useFaseList = (dataService: IFaseService) => {
  const [data, setData] = useState<IFase[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
