...
const TiposAcaoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITiposAcao | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITiposAcao | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITiposAcao);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TiposAcaoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTiposAcao={editar ?? addValue ?? TiposAcaoEmpty()} /> 
            )}
