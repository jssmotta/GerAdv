...
import { StatusBiu } from '../../Models/StatusBiu';
...
export class StatusBiuApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class StatusBiuApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/StatusBiu`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusBiu_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os StatusBiu');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusBiu_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Status Biu com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterStatusBiu): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusBiu_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de StatusBiu');
        }
    }

            public async filter(filtro: FilterStatusBiu): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusBiu_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar StatusBiu');
                }
            }


    public async validation(regStatusBiu: IStatusBiu): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regStatusBiu as StatusBiu, this.getHeaders());
    ...
    public async addAndUpdate(regStatusBiu: IStatusBiu): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regStatusBiu as StatusBiu, this.getHeaders());
            var register = result.data as IStatusBiu;        
            const action = regStatusBiu.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Status Biu');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o StatusBiu. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'StatusBiu',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Status Biu com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterStatusBiu) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<StatusBiu[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de statusbiu:', error);
            this.notificationService.notify({
              entity: 'StatusBiu',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos statusbiu'
            });
          }
        });
    }
