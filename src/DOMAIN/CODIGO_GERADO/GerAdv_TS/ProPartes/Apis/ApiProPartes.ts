...
import { ProPartes } from '../../Models/ProPartes';
...
export class ProPartesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class ProPartesApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProPartes`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProPartes_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProPartes');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProPartes_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Pro Partes com ID ${id}`);
        }
    }

            public async filter(filtro: FilterProPartes): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProPartes_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProPartes');
                }
            }


    public async validation(regProPartes: IProPartes): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProPartes as ProPartes, this.getHeaders());
    ...
    public async addAndUpdate(regProPartes: IProPartes): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProPartes as ProPartes, this.getHeaders());
            var register = result.data as IProPartes;        
            const action = regProPartes.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Pro Partes');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProPartes. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProPartes',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Pro Partes com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProPartes) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProPartes[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de propartes:', error);
            this.notificationService.notify({
              entity: 'ProPartes',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos propartes'
            });
          }
        });
    }
