...
public static partial class DevourerOne
{
    ...
    public static (bool setUpNow, bool changed, DateTime? data) DateUp7(in bool hasChange, DateTime? currDate, string? value)
    {
        if (value.IsEquals("now"))
        {
            return (true, true, DateTimeUtc);
        }

        if (string.IsNullOrEmpty(value))
        {
            return (currDate != null, currDate != null, null);
        }

        if (currDate != null && value.IsEquals(Convert.ToDateTime(currDate).ToShortDateString()))
        {
            return (false, false, null);
        }

        if (!DateTime.TryParse(value, out var dia) || dia.ToShortDateString().Equals("01/01/0001"))
        {
            return (false, false, null);
        }

        return (true, true, dia);
    }
    ...
    public static string AppendDataSqlMaiorOuIgual(string dateTime1, string cCampo)
    {
        ...
        return AppendDataSqlMaiorOuIgual(IsValidDate2(DateTimeUtc.ToString()) ? Convert.ToDateTime(dateTime1) : DateTimeUtc, cCampo);
    }
