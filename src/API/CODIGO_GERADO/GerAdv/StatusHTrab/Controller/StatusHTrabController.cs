...
public partial class StatusHTrabController(IStatusHTrabService statushtrabService) : ControllerBase
{
    ...
    {
        _logger.Info("StatusHTrab: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _statushtrabService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("StatusHTrab: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _statushtrabService.UpdateColumns(parameters, uri);
        if (!result)
        {
