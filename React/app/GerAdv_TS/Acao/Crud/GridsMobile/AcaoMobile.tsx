...
import { useAcaoGrid } from "../../Hooks/hookAcaoGrid";
...
import AcaoHelpCrudButton from "../../Help/AcaoHelpCrudButton";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("acao-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector: ".grid-mobile-crud.grid-mobile-acao .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Acao-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Acao-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            <PageTitle
              title="Acao"
              helpButton={<AcaoHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                        ...
                        <>
                          {sortedData.map((data) => (
                            <ViewMobile
                              ...
                              name={data.descricao}
                              data={data}
                              related={SearchFromCellInstanciaMobile({
                                dataItem: data,
                              })}
                            />
                          ))}
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-acao"
                      key={`acao-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IAcao);
