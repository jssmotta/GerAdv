...
#region Class Structure Tests
    ...
    private static GerAdv.Models.Penhora CreateValidPenhora()
    {
        return new GerAdv.Models.Penhora
        {
            Id = 1,
            Processo = 0,
            Nome = "João",
    ...
    private void SetupValidMocks()
    {
        ...
        _ = _mockPenhoraStatusReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<PenhoraStatusResponse?>(new PenhoraStatusResponse { Id = id }));
    }
    ...
    private void SetupValidMocksInvalid()
    {
        ...
        _mockPenhoraService.Setup(x => x.Filter(It.IsAny<int>(), It.IsAny<FilterPenhora>(), It.IsAny<string>())).ReturnsAsync([]);
        // Setup other mocks but don't override the Penhoras service mock
        _ = _mockPenhoraStatusReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<PenhoraStatusResponse?>(new PenhoraStatusResponse { Id = 0 }));
    }
...
#region DataPenhora Validation Tests
    ...
    [Fact]
    public async Task ValidateReg_WithNull_DataPenhora_ShouldPass()
    {
        // Arrange
        var penhora = CreateValidPenhora();
...
#region Foreign Key Validation Tests - PenhoraStatus
    ...
    public async Task ValidateReg_WithInvalidPenhoraStatus_ShouldThrowSGValidationException()
    {
        ...
        _mockPenhoraStatusReader.Setup(x => x.ReadAsync(999, _mockConnection.Object)).Returns(Task.FromResult<GerAdv.Models.Response.PenhoraStatusResponse?>(null !));
    ...
    public async Task ValidateReg_WithMismatchedPenhoraStatusId_ShouldThrowSGValidationException()
    {
        ...
        var penhora = CreateValidPenhora();
        penhora.PenhoraStatus = 999;
        var reg888 = new GerAdv.Models.Response.PenhoraStatusResponse
        {
            Id = 888
        }; // Different ID
        _mockPenhoraStatusReader.Setup(x => x.ReadAsync(999, _mockConnection.Object)).Returns(Task.FromResult<GerAdv.Models.Response.PenhoraStatusResponse?>(reg888));
        SetupValidMocksInvalid();
        // Act & Assert
        var exception = await Assert.ThrowsAsync<SGValidationException>(() => _validation.ValidateReg(penhora, _mockPenhoraService.Object, _mockPenhoraStatusReader.Object, _validUri, _mockConnection.Object));
        exception.Message.Should().Contain("não encontrado");
    ...
    public async Task ValidateReg_WithValidPenhoraStatus_ShouldPass()
    {
        ...
        var reg123 = new GerAdv.Models.Response.PenhoraStatusResponse
        {
            Id = 123
        };
        _mockPenhoraStatusReader.Setup(x => x.ReadAsync(123, _mockConnection.Object)).Returns(Task.FromResult<GerAdv.Models.Response.PenhoraStatusResponse?>(reg123));
        SetupValidMocks();
        // Act
        var result = await _validation.ValidateReg(penhora, _mockPenhoraService.Object, _mockPenhoraStatusReader.Object, _validUri, _mockConnection.Object);
        // Assert
