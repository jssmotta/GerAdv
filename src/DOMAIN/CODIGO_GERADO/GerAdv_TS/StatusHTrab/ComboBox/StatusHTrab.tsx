...
const StatusHTrabComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IStatusHTrab | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IStatusHTrab | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setDisplayValue({ id: response.data.id, nome: response.data.descricao });
                setEditar(response.data as IStatusHTrab);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <StatusHTrabWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedStatusHTrab={editar ?? addValue ?? StatusHTrabEmpty()} /> 
            )}
