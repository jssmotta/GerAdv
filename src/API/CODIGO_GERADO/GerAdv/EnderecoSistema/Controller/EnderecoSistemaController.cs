...
public partial class EnderecoSistemaController(IEnderecoSistemaService enderecosistemaService) : ControllerBase
{
    ...
    {
        _logger.Info("EnderecoSistema: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _enderecosistemaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("EnderecoSistema: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _enderecosistemaService.UpdateColumns(parameters, uri);
        if (!result)
        {
