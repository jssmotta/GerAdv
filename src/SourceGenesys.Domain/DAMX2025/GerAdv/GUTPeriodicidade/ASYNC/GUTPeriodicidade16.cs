...
public partial class DBGUTPeriodicidade
{
    ...
    {
        if (pFldFNome)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFIntervaloDias)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.IntervaloDias, m_FIntervaloDias, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBGUTPeriodicidadeDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBGUTPeriodicidadeDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
