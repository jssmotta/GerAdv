...
// useOperadorGrupoGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de OperadorGrupo
'use client';
...
interface UseOperadorGrupoGridProps {
  data: IOperadorGrupo[];
...
export const useOperadorGrupoGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('operadorgrupo-columns-order'), JSON.stringify(reorderableFields));
