...
public partial class ClientesService
{
    ...
    /// <returns>FilterClientes processado</returns>
    private async Task<Filters.FilterClientes?> ProcessFilterVoiceCommandAsync(CommandSpeakerRequest message, Filters.FilterClientes? existingFilter, string uri)
    {
        try
        {
            // Usar o contexto do FilterClientesTXT
            var context = FilterClientesTXT.GetPrompt(existingFilter, FilterClientesTXT.GetOptions());
        ...
        catch (Exception)
        {
            ...
            return existingFilter ?? new Filters.FilterClientes();
        }
    }

    private async Task<IEnumerable<ClientesResponseAll>> GetDataAllAsync(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken cancellationToken)
    {
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter>? parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarNAsync(max, uri, where, parameters, DBClientesDicInfo.CampoNome);
    ...
    private async Task<IEnumerable<ClientesResponseAll>> GetDataAllAsync2(int max, string where, List<SqlParameter>? parameters, string uri, CancellationToken token)
    {
        using var scope = _connectionService.CreateConnectionScope(uri);
        using var oCnn = scope.Connection;
        if (oCnn == null)
