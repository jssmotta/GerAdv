...
public static class FilterInstanciaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Instancia em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterInstancia:

- O filter irá filtrar os Instancia conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterAcao: filtro específico para Acao
- filterForo: filtro específico para Foro
- filterTipoRecurso: filtro específico para Tipo Recurso
...
CAMPOS filterAcao (Acao):
- justica: número ou IDs;
- area: número ou IDs;
- descricao: _nome ou descrição para Acao;
...
CAMPOS filterForo (Foro):
- email: EMail;
- nome: _nome ou descrição para Foro;
- unico: Unico;
...
CAMPOS filterTipoRecurso (Tipo Recurso):
- justica: número ou IDs;
- area: número ou IDs;
- descricao: _nome ou descrição para Tipo Recurso;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
- quemcad, quemcad_end: ID do operador que cadastrou
...
- Se ouvir "todos os Instancia" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterInstancia? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Instancia length: {result.Length}");
#endif
