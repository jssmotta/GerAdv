...
interface UseParceriaProcFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterParceriaProc | undefined | null,
  ) => Promise<void>;
}

interface UseParceriaProcFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterParceriaProc;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterParceriaProc>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterParceriaProc) => Promise<void>;

  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterParceriaProc>,
  ) => React.ReactNode;
...
export const useParceriaProcFilter = ({
  handleFetchWithFilter,
}: UseParceriaProcFilterProps): UseParceriaProcFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterParceriaProc>(
  ...
  const handleConfirmSearch = async (filter: FilterParceriaProc) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterParceriaProc);
    sessionStorage.removeItem(btoa("ParceriaProcFilter"));
    handleFetchWithFilter({} as FilterParceriaProc);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterParceriaProc>) => (
    <>
      <AdvogadosComboBox
        name="advogado"
        dataForm={null}
