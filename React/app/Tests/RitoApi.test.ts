...
import { RitoApiError } from '../GerAdv_TS/Rito/Apis/ApiRito';
import { RitoApi } from '@/app/GerAdv_TS/Rito/Apis/ApiRito';
import { FilterRito } from '../GerAdv_TS/Rito/Filters/Rito';
...
describe('RitoApi', () => {
  ...
  beforeEach(() => {
    ...
    ritoApi = new RitoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(ritoApi).toBeInstanceOf(RitoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new RitoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(RitoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all rito items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ritoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(ritoApi.getAll()).rejects.toThrow(RitoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await ritoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Rito_last_getAll_${200}`)));
    });
    ...
    it('should throw RitoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    ...
    it('should fetch rito by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ritoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockRito));
      ...
      expect(result.data).toEqual(mockRito);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Rito',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter rito items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await ritoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new rito successfully', async () => {
      const responseRito = { ...mockRito, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseRito,
        status: 200,
        statusText: 'OK',
      ...
      const result = await ritoApi.addAndUpdate(mockRito);
    ...
    it('should update existing rito successfully', async () => {
      const existingRito = { ...mockRito, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingRito,
        status: 200,
        statusText: 'OK',
      ...
      const result = await ritoApi.addAndUpdate(existingRito);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(ritoApi.addAndUpdate(mockRito)).rejects.toThrow(RitoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Rito',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete rito successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await ritoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(ritoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Rito',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(ritoApi.delete(1)).rejects.toThrow(RitoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterRito = { funcionario: 1 };
      
      const result = ritoApi.useFilter(mockFilter);
  ...
  describe('RitoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new RitoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('RitoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new RitoApiError('Test message', 500);
