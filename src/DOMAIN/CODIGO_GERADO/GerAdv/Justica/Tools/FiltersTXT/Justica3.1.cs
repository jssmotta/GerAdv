...
public static class FilterJusticaTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterJustica? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Justica em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterJustica:

- O filter irá filtrar os Justica conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Justica" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Justica length: {result.Length}");
#endif
