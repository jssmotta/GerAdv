  ...
  const loadRamal = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Ramal";
...
export const useRamalList = (
  dataService: IRamalService,
  currentFilter?: FilterRamal | undefined | null,
) => {
  const [data, setData] = useState<IRamal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterRamal | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterRamal | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterRamal));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar ramal";
