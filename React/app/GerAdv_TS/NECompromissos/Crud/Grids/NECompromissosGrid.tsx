...
import { useNECompromissosList } from '../../Hooks/hookNECompromissos';
...
const {
  ...
  fetchData, 
  refreshData
} = useNECompromissosList(necompromissosService);
...
const handleFetchWithFilter = async (filtro?: FilterNECompromissos | undefined | null) => {
  setCurrFilter(filtro);
  await fetchData(filtro ?? {} as FilterNECompromissos);
};
...
const confirmDelete = async () => {
  if (deleteId !== null) {
    try {
      await necompromissosService.deleteNECompromissos(deleteId);
      // O hook já escuta as notificações e recarrega automaticamente
    } catch (error) {
...
{loading && (
  ...
  {isMobile ? (
    <NECompromissosGridMobileComponent
    data={necompromissos}
    ...
    <NECompromissosGridDesktopComponent
    data={necompromissos}
    onRowClick={handleRowClick}
    onDeleteClick={onDeleteClick}
    setSelectedId={setSelectedId}
