...
import PreClientesWindow from '../Crud/Grids/PreClientesWindow';
...
import { usePreClientesComboBox } from '../Hooks/hookPreClientes';
const PreClientesComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'preclientesInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = usePreClientesComboBox(preclientesService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IPreClientes | null>(null);
const [addRecord, setAddRecord] = useState<IPreClientes | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await preclientesService.fetchPreClientesById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = PreClientesEmpty();
...
const handleSuccess = (newPreClientes?: IPreClientes) => {
  if (newPreClientes) {
    setValue({ id: newPreClientes.id, nome: newPreClientes.nome });
    loadRecordForEdit(newPreClientes.id);
    handleValueChange(newPreClientes);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .preclientes-dropdown-popup-msi {
  ...
  .preclientes-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .preclientes-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .preclientes-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedPreClientes={editRecord || addRecord || PreClientesEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'preclientes-dropdown-popup-msi'
          }}
          />
...
export default PreClientesComboBox;