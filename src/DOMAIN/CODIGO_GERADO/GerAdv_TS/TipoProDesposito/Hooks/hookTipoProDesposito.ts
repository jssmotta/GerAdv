...
export const useTipoProDespositoForm = (
  ...
  const loadTipoProDesposito = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialTipoProDesposito);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Tipo Pro Desposito';
      setError(errorMessage);
      console.log('Erro ao carregar Tipo Pro Desposito');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialTipoProDesposito]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadTipoProDesposito, resetForm]);
  return returnValue;
};


export const useTipoProDespositoNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('TipoProDesposito', (entity) => {
      try {
...
export const useTipoProDespositoList = (dataService: ITipoProDespositoService) => {
  const [data, setData] = useState<ITipoProDesposito[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar tipoprodesposito';
      setError(errorMessage);
      console.log('Erro ao carregar tipoprodesposito');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useTipoProDespositoNotifications(
