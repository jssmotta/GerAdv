...
public partial class FornecedoresReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBFornecedoresDicInfo.CampoNome}";
...
[{DBCidadeDicInfo.PTabelaNome}].[{DBCidadeDicInfo.Nome}]
                   FROM {DBFornecedores.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(forDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(forDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    forVisto as Visto
FROM {oCnn.UseDbo}.Fornecedores dbq WITH (INDEX = idx_Fornecedores_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.forQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.forQuemAtu
WHERE [forCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBFornecedores.PTabelaNome,
...
FROM {oCnn.UseDbo}.Fornecedores WITH (INDEX = idx_{DBFornecedoresDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [forCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBFornecedoresDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(forDtAtu, forDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBFornecedoresDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(forDtAtu, forDtCad) DESC;", oCnn.InnerConnection);
