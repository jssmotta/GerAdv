// app/TestsForms/AcaoComboBox.test.tsx - Vers�o Corrigida
...
import { AcaoEmpty, AcaoTestEmpty } from '@/app/GerAdv_TS/Models/Acao';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Acao/Hooks/hookAcao', () => ({
  useAcaoComboBox: jest.fn(() => ({
    options: [
    { ...AcaoEmpty(), id: 1, descricao: 'Acao 1' },
    { ...AcaoEmpty(), id: 2, descricao: 'Acao 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Acao/Apis/ApiAcao', () => ({
  AcaoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Acao' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Acao' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Acao/Services/Acao.service', () => ({
  AcaoService: jest.fn().mockImplementation(() => ({
    fetchAcaoById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock Acao' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Acao/Crud/Grids/AcaoWindow', () => {
  return function MockAcaoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='acao-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'Acao' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...AcaoTestEmpty(), id: 1, descricao: 'Acao 1' },
  { ...AcaoTestEmpty(), id: 2, descricao: 'Acao 2' },
];
...
describe('AcaoComboBox', () => {
  ...
  const { useAcaoComboBox } = require('@/app/GerAdv_TS/Acao/Hooks/hookAcao');
  (useAcaoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<AcaoComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Acao')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...AcaoTestEmpty(), id: 1, descricao: 'Acao 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useAcaoComboBox } = require('@/app/GerAdv_TS/Acao/Hooks/hookAcao');
    useAcaoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens AcaoWindow on action button click (add)', async () => {
    render(<AcaoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('acao-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('acao-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<AcaoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles AcaoWindow onClose and onSuccess', () => {
    render(<AcaoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<AcaoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
