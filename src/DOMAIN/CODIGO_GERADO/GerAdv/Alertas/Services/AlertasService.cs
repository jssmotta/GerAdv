...
public partial class AlertasService(IOptions<AppSettings> appSettings, IAlertasReader reader, IAlertasValidation validation, IAlertasWriter writer, IOperadorReader operadorReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IAlertasService, IDisposable
{
    ...
    public async Task<AlertasResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-Alertas-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    public async Task<AlertasResponse?> AddAndUpdate([FromBody] Models.Alertas regAlertas, [FromRoute, Required] string uri)
    {
        ...
        return await Task.Run(async () =>
        {
            ...
            {
                throw new Exception($"Alertas: {validade}");
            }
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("Alertas: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAlertas(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBAlertasDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("Alertas: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAlertas(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBAlertasDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
