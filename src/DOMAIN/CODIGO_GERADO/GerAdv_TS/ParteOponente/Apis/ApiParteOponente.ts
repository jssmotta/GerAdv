...
import { ParteOponente } from "../../Models/ParteOponente";
...
export class ParteOponenteApi {
    ...
    constructor(uri: string, authorization: string) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API}/${uri}/ParteOponente`;
    ...
    public async addAndUpdate(regParteOponente: IParteOponente): Promise<AxiosResponse> {
        var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regParteOponente as ParteOponente, this.getHeaders());
        var register = result.data as IParteOponente;        
        const action = regParteOponente.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
    ...
    public useGetAll(max: number = 1000) {
        ...
        return useSWR<ParteOponente[]>(key, fetcher, {
