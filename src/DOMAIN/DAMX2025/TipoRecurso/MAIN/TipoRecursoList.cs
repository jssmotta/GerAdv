...
#region ListarDados_TipoRecurso
    public static async IAsyncEnumerable<DBTipoRecurso> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoRecurso(dbRec);
        }
    }

    public static IEnumerable<DBTipoRecurso> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
