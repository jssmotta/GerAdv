...
const CargosEscClassComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ICargosEscClass | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ICargosEscClass | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ICargosEscClass);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <CargosEscClassWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedCargosEscClass={editar ?? addValue ?? CargosEscClassEmpty()} /> 
            )}
