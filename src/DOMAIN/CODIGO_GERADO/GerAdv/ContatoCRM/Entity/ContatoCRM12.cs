...
public partial class FContatoCRM : MenphisSI.SG.GerAdv.DBContatoCRM, IDBContatoCRM
{
    ...
    private FContatoCRM(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FContatoCRM CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FContatoCRM(parameters, oCnn, fullSql, sqlWhere, join);
    }
