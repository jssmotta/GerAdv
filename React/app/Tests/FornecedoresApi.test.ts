...
describe('FornecedoresApi', () => {
  let fornecedoresApi: FornecedoresApi;
  ...
  beforeEach(() => {
    ...
    fornecedoresApi = new FornecedoresApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(fornecedoresApi).toBeInstanceOf(FornecedoresApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new FornecedoresApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(FornecedoresApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all fornecedores items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await fornecedoresApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(fornecedoresApi.getAll()).rejects.toThrow(FornecedoresApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Fornecedores_last_getAll_${200}`)));
    });
    ...
    it('should throw FornecedoresApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockFornecedores = FornecedoresTestEmpty();
    ...
    it('should fetch fornecedores by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await fornecedoresApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockFornecedores));
      ...
      expect(result.data).toEqual(mockFornecedores);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fornecedores',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter fornecedores items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await fornecedoresApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new fornecedores successfully', async () => {
      const responseFornecedores = { ...mockFornecedores, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await fornecedoresApi.addAndUpdate(mockFornecedores);
    ...
    it('should update existing fornecedores successfully', async () => {
      const existingFornecedores = { ...mockFornecedores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingFornecedores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await fornecedoresApi.addAndUpdate(existingFornecedores);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(fornecedoresApi.addAndUpdate(mockFornecedores)).rejects.toThrow(FornecedoresApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fornecedores',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete fornecedores successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await fornecedoresApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(fornecedoresApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fornecedores',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(fornecedoresApi.delete(1)).rejects.toThrow(FornecedoresApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await fornecedoresApi.delete(1);
  ...
  describe('FornecedoresApiError', () => {
    it('should create error with correct properties', () => {
      const error = new FornecedoresApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('FornecedoresApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new FornecedoresApiError('Test message', 500);
