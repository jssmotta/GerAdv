...
public partial class DBPosicaoOutrasPartes
{
    ...
    {
        if (pFldFDescricao)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBPosicaoOutrasPartesDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBPosicaoOutrasPartesDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
