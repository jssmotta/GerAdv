...
#region ListarDados_Alertas
    public static async IAsyncEnumerable<DBAlertas> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAlertas(dbRec);
        }
    }

    public static IEnumerable<DBAlertas> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
