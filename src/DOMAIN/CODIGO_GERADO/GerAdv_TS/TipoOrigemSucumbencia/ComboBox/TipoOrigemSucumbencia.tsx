...
const TipoOrigemSucumbenciaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoOrigemSucumbencia | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoOrigemSucumbencia | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoOrigemSucumbencia);
    ...
    const handleAddClick = () => {
        ...
        const add = TipoOrigemSucumbenciaEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoOrigemSucumbenciaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoOrigemSucumbencia={editar ?? addValue ?? TipoOrigemSucumbenciaEmpty()} /> 
            )}
