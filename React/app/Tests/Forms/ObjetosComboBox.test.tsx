// app/TestsForms/ObjetosComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import ObjetosComboBox from '@/app/GerAdv_TS/Objetos/ComboBox/Objetos';
...
import { ObjetosEmpty, ObjetosTestEmpty } from '@/app/GerAdv_TS/Models/Objetos';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Objetos/Hooks/hookObjetos', () => ({
  useObjetosComboBox: jest.fn(() => ({
    options: [
    { ...ObjetosEmpty(), id: 1, nome: 'Objetos 1' },
    { ...ObjetosEmpty(), id: 2, nome: 'Objetos 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Objetos/Apis/ApiObjetos', () => ({
  ObjetosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Objetos' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Objetos' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Objetos/Services/Objetos.service', () => ({
  ObjetosService: jest.fn().mockImplementation(() => ({
    fetchObjetosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Objetos' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Objetos/Crud/Grids/ObjetosWindow', () => {
  return function MockObjetosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='objetos-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Objetos' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ObjetosTestEmpty(), id: 1, nome: 'Objetos 1' },
  { ...ObjetosTestEmpty(), id: 2, nome: 'Objetos 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: ObjetosEmpty(), 
};
describe('ObjetosComboBox', () => {
  ...
  const { useObjetosComboBox } = require('@/app/GerAdv_TS/Objetos/Hooks/hookObjetos');
  (useObjetosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ObjetosComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Objetos')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ObjetosTestEmpty(), id: 1, nome: 'Objetos 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useObjetosComboBox } = require('@/app/GerAdv_TS/Objetos/Hooks/hookObjetos');
    useObjetosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ObjetosWindow on action button click (add)', async () => {
    render(<ObjetosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('objetos-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('objetos-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ObjetosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ObjetosWindow onClose and onSuccess', () => {
    render(<ObjetosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ObjetosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
