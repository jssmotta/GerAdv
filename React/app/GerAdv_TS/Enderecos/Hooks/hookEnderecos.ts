  ...
  const loadEnderecos = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Endereço";
...
export const useEnderecosList = (
  dataService: IEnderecosService,
  currentFilter?: FilterEnderecos | undefined | null,
) => {
  const [data, setData] = useState<IEnderecos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterEnderecos | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IEnderecos): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.descricao.length > 50) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 50 caracteres.",
        };
      }
      ...
      if (data.oab.length > 20) {
        return {
          isValid: false,
          message: "O campo OAB não pode ter mais de 20 caracteres.",
        };
      }
      ...
      if (data.tratamento.length > 20) {
        return {
          isValid: false,
          message: "O campo Tratamento não pode ter mais de 20 caracteres.",
        };
      }
      ...
      if (data.quemindicou.length > 150) {
        return {
          isValid: false,
          message: "O campo QuemIndicou não pode ter mais de 150 caracteres.",
        };
      }
