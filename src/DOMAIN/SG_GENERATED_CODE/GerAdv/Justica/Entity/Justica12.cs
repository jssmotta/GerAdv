...
public partial class FJustica : MenphisSI.SG.GerAdv.DBJustica, IDBJustica
{
    // DI-compatible default constructor
    public FJustica() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FJustica CreateFromReader(SqlDataReader? dbRec)
    {
        return new FJustica(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FJustica CreateFromDataRow(DataRow? dbRec)
    {
        return new FJustica(dbRec);
    }

    private FJustica(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FJustica(DataRow? dbRec) : base(dbRec)
    {
    }

    private FJustica(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FJustica CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FJustica(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FJustica> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fJustica = new FJustica();
        await fJustica.CarregarAsync(id, oCnn);
        return fJustica;
    }
