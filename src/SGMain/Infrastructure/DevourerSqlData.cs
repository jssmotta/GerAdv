...
public static partial class DevourerSqlData
{
    ...
    public async static Task<List<DBNomeID>?> ListarNomeID(string sql, List<SqlParameter>? parameters, string uri, bool caching = false, int max = 200)
    {
        try
        {
            ...
            await using var connection = ConfiguracoesSys.GetConnectionByUriAsync(uri).GetAwaiter().GetResult();
            ...
            await using var cmd = new SqlCommand(cmdText: ConfiguracoesDBT2.CmdSql(sql), connection: connection?.InnerConnection)
            {
                CommandTimeout = 0
            };
            if (parameters != null)
