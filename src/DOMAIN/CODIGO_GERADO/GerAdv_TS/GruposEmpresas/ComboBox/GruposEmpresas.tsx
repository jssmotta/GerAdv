...
const GruposEmpresasComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IGruposEmpresas | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IGruposEmpresas | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IGruposEmpresas);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <GruposEmpresasWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedGruposEmpresas={editar ?? addValue ?? GruposEmpresasEmpty()} /> 
            )}
