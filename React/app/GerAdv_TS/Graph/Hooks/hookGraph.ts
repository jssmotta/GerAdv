  ...
  const loadGraph = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Graph";
...
export const useGraphList = (
  dataService: IGraphService,
  currentFilter?: FilterGraph | undefined | null,
) => {
  const [data, setData] = useState<IGraph[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGraph | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IGraph): { isValid: boolean; message: string } {
    ...
    try {
      if (data.tabela.length > 80) {
        return {
          isValid: false,
          message: "O campo Tabela não pode ter mais de 80 caracteres.",
        };
      }
