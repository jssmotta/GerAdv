  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("divisaotribunal-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-divisaotribunal .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-divisaotribunal",
      columns: exportColumns,
      sheetName: "DivisaoTribunal",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-DivisaoTribunal-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-divisaotribunal",
      columns: exportColumnsPdf,
      title: "DivisaoTribunal",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-DivisaoTribunal-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
        ...
        {initialized && (
          <>
            <PageTitle
              ...
              helpButton={<DivisaoTribunalHelpCrudButton isMobile={true} />}
            />
                ...
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      ...
                      key={`divisaotribunal-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IDivisaoTribunal);
