...
public partial class FPreClientes : MenphisSI.SG.GerAdv.DBPreClientes, IDBPreClientes
{
    // DI-compatible default constructor
    public FPreClientes() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FPreClientes CreateFromReader(SqlDataReader? dbRec)
    {
        return new FPreClientes(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FPreClientes CreateFromDataRow(DataRow? dbRec)
    {
        return new FPreClientes(dbRec);
    }

    private FPreClientes(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FPreClientes(DataRow? dbRec) : base(dbRec)
    {
    }

    private FPreClientes(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FPreClientes CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FPreClientes(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FPreClientes> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fPreClientes = new FPreClientes();
        await fPreClientes.CarregarAsync(id, oCnn);
        return fPreClientes;
    }
