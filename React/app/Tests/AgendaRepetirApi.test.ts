...
import { AgendaRepetirApiError } from '../GerAdv_TS/AgendaRepetir/Apis/ApiAgendaRepetir';
import { AgendaRepetirApi } from '@/app/GerAdv_TS/AgendaRepetir/Apis/ApiAgendaRepetir';
import { FilterAgendaRepetir } from '../GerAdv_TS/AgendaRepetir/Filters/AgendaRepetir';
...
describe('AgendaRepetirApi', () => {
  let agendarepetirApi: AgendaRepetirApi;
  ...
  beforeEach(() => {
    ...
    agendarepetirApi = new AgendaRepetirApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendarepetirApi).toBeInstanceOf(AgendaRepetirApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaRepetirApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaRepetirApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all agendarepetir items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarepetirApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendarepetirApi.getAll()).rejects.toThrow(AgendaRepetirApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendarepetirApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaRepetir_last_getAll_${200}`)));
    });
    ...
    it('should throw AgendaRepetirApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaRepetir = AgendaRepetirTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAgendaRepetir,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch agendarepetir by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarepetirApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaRepetir));
      ...
      expect(result.data).toEqual(mockAgendaRepetir);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetir',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaRepetir = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendarepetir items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendarepetirApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaRepetir: IAgendaRepetir = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendarepetir successfully', async () => {
      const responseAgendaRepetir = { ...mockAgendaRepetir, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaRepetir,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarepetirApi.addAndUpdate(mockAgendaRepetir);
    ...
    it('should update existing agendarepetir successfully', async () => {
      const existingAgendaRepetir = { ...mockAgendaRepetir, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaRepetir,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarepetirApi.addAndUpdate(existingAgendaRepetir);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendarepetirApi.addAndUpdate(mockAgendaRepetir)).rejects.toThrow(AgendaRepetirApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetir',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendarepetir successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendarepetirApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendarepetirApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetir',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendarepetirApi.delete(1)).rejects.toThrow(AgendaRepetirApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaRepetir = { funcionario: 1 };
      
      const result = agendarepetirApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await agendarepetirApi.delete(1);
  ...
  describe('AgendaRepetirApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaRepetirApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaRepetirApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaRepetirApiError('Test message', 500);
