...
public partial class RegimeTributacaoWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_RegimeTributacao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[RegimeTributacao]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_RegimeTributacao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[RegimeTributacao]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_RegimeTributacao_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_RegimeTributacao_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[RegimeTributacao]'))
                BEGIN                    
