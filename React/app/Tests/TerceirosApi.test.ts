...
describe('TerceirosApi', () => {
  let terceirosApi: TerceirosApi;
  ...
  beforeEach(() => {
    ...
    terceirosApi = new TerceirosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(terceirosApi).toBeInstanceOf(TerceirosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TerceirosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TerceirosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all terceiros items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await terceirosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(terceirosApi.getAll()).rejects.toThrow(TerceirosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await terceirosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Terceiros_last_getAll_${200}`)));
    });
    ...
    it('should throw TerceirosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTerceiros = TerceirosTestEmpty();
    ...
    it('should fetch terceiros by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await terceirosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTerceiros));
      ...
      expect(result.data).toEqual(mockTerceiros);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Terceiros',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter terceiros items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await terceirosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new terceiros successfully', async () => {
      const responseTerceiros = { ...mockTerceiros, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTerceiros,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await terceirosApi.addAndUpdate(mockTerceiros);
    ...
    it('should update existing terceiros successfully', async () => {
      const existingTerceiros = { ...mockTerceiros, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTerceiros,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await terceirosApi.addAndUpdate(existingTerceiros);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(terceirosApi.addAndUpdate(mockTerceiros)).rejects.toThrow(TerceirosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Terceiros',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete terceiros successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await terceirosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(terceirosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Terceiros',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(terceirosApi.delete(1)).rejects.toThrow(TerceirosApiError);
    });
  });
  ...
  describe('TerceirosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TerceirosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TerceirosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TerceirosApiError('Test message', 500);
