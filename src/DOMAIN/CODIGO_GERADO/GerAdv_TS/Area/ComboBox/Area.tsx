...
const AreaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IArea | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IArea | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IArea);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <AreaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedArea={editar ?? addValue ?? AreaEmpty()} /> 
            )}
