...
#region ListarDados_AlarmSMS
    public static async IAsyncEnumerable<DBAlarmSMS> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAlarmSMS(dbRec);
        }
    }

    public static IEnumerable<DBAlarmSMS> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
