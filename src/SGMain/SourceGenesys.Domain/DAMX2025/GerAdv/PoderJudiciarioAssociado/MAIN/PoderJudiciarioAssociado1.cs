...
public partial class DBPoderJudiciarioAssociado : VAuditor, ICadastros
{
...
#region GravarDados_PoderJudiciarioAssociado
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (!isInsert)
            if (!(pFldFJustica || pFldFJusticaNome || pFldFArea || pFldFAreaNome || pFldFTribunal || pFldFTribunalNome || pFldFForo || pFldFForoNome || pFldFCidade || pFldFSubDivisaoNome || pFldFCidadeNome || pFldFSubDivisao || pFldFGUID || pFldFTipo))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
