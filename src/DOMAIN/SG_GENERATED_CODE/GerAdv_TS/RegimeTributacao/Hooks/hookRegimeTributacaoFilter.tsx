...
import { FilterRegimeTributacao } from '@/app/GerAdv_TS/RegimeTributacao/Filters/RegimeTributacao';
...
interface UseRegimeTributacaoFilterProps {
    handleFetchWithFilter: (filtro?: FilterRegimeTributacao | undefined | null) => Promise<void>;
}

interface UseRegimeTributacaoFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterRegimeTributacao;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterRegimeTributacao>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterRegimeTributacao) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterRegimeTributacao>) => React.ReactNode;
...
export const useRegimeTributacaoFilter = ({ handleFetchWithFilter }: UseRegimeTributacaoFilterProps): UseRegimeTributacaoFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterRegimeTributacao>({} as FilterRegimeTributacao);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterRegimeTributacao;
    ...
    const handleConfirmSearch = async (filter: FilterRegimeTributacao) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterRegimeTributacao);
        sessionStorage.removeItem(btoa('RegimeTributacaoFilter'));
        handleFetchWithFilter({} as FilterRegimeTributacao);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterRegimeTributacao>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
