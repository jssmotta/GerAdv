...
export const useTipoProDespositoList = (dataService: ITipoProDespositoService, currentFilter?: FilterTipoProDesposito | undefined | null) => {
  const [data, setData] = useState<ITipoProDesposito[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoProDesposito | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoProDesposito | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoProDesposito);
      setData(result);
    } catch (err) {
