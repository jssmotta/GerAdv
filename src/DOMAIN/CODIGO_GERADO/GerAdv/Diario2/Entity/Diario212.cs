...
public partial class FDiario2 : MenphisSI.SG.GerAdv.DBDiario2, IDBDiario2
{
    // DI-compatible default constructor
    public FDiario2() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FDiario2 CreateFromReader(SqlDataReader? dbRec)
    {
        return new FDiario2(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FDiario2 CreateFromDataRow(DataRow? dbRec)
    {
        return new FDiario2(dbRec);
    }

    private FDiario2(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FDiario2(DataRow? dbRec) : base(dbRec)
    {
    }

    private FDiario2(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FDiario2 CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FDiario2(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FDiario2> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fDiario2 = new FDiario2();
        await fDiario2.CarregarAsync(id, oCnn);
        return fDiario2;
    }
