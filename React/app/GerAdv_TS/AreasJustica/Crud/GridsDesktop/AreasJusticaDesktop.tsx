...
import { useAreasJusticaGrid } from '../../Hooks/hookAreasJusticaGrid';
...
const [columnsOrder, setColumnsOrder] = useState<string[]>(['descricaoarea',
'nomejustica']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('areasjustica-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('areasjustica-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('areasjustica-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-areasjustica',
  columns: exportColumns, 
  sheetName: 'AreasJustica'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-AreasJustica-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-areasjustica',
  columns: exportColumnsPdf, 
  title: 'AreasJustica'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-AreasJustica-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`areasjustica-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-areasjustica'
