...
public partial class ClientesSociosReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBClientesSociosDicInfo.CampoNome}";
...
[{DBCidadeDicInfo.PTabelaNome}].[{DBCidadeDicInfo.Nome}]
                   FROM {DBClientesSocios.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBClientesSociosDicInfo.PTabelaNome}].[{DBClientesSociosDicInfo.Cliente}]
...
SELECT TOP (1) 
	FORMAT(cscDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(cscDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    cscVisto as Visto
FROM {oCnn.UseDbo}.ClientesSocios dbq WITH (INDEX = idx_ClientesSocios_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.cscQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.cscQuemAtu
WHERE [cscCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBClientesSocios.PTabelaNome,
...
FROM {oCnn.UseDbo}.ClientesSocios WITH (INDEX = idx_{DBClientesSociosDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [cscCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBClientesSociosDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(cscDtAtu, cscDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBClientesSociosDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(cscDtAtu, cscDtCad) DESC;", oCnn.InnerConnection);
