...
public partial class FNENotas : MenphisSI.SG.GerAdv.DBNENotas, IDBNENotas
{
    // DI-compatible default constructor
    public FNENotas() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FNENotas CreateFromReader(SqlDataReader? dbRec)
    {
        return new FNENotas(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FNENotas CreateFromDataRow(DataRow? dbRec)
    {
        return new FNENotas(dbRec);
    }

    private FNENotas(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FNENotas(DataRow? dbRec) : base(dbRec)
    {
    }

    private FNENotas(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FNENotas CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FNENotas(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FNENotas> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fNENotas = new FNENotas();
        await fNENotas.CarregarAsync(id, oCnn);
        return fNENotas;
    }
