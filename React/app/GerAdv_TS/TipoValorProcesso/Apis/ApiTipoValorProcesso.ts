...
import { TipoValorProcesso } from '../../Models/TipoValorProcesso';
...
export class TipoValorProcessoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'TipoValorProcessoApiError';
    this.status = status;
    this.code = code;
...
export class TipoValorProcessoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/TipoValorProcesso`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoValorProcesso');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipo Valor Processo com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterTipoValorProcesso): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoValorProcesso');
        }
    }

            public async filter(filtro: FilterTipoValorProcesso): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoValorProcesso_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoValorProcesso');
                }
            }


    public async validation(regTipoValorProcesso: ITipoValorProcesso): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoValorProcesso as TipoValorProcesso, this.getHeaders());
    ...
    public async addAndUpdate(regTipoValorProcesso: ITipoValorProcesso): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regTipoValorProcesso as TipoValorProcesso, this.getHeaders());
            var register = result.data as ITipoValorProcesso;        
            const action = regTipoValorProcesso.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Tipo Valor Processo');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o TipoValorProcesso. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'TipoValorProcesso',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Tipo Valor Processo com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoValorProcesso) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<TipoValorProcesso[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de tipovalorprocesso:', error);
            this.notificationService.notify({
              entity: 'TipoValorProcesso',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tipovalorprocesso'
            });
          }
        });
    }
