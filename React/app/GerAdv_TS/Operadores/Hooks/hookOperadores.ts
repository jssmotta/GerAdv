...
export const useOperadoresForm = (
  ...
  const loadOperadores = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
...
export const useOperadoresList = (
  dataService: IOperadoresService,
  currentFilter?: FilterOperadores | undefined | null,
) => {
  const [data, setData] = useState<IOperadores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOperadores | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterOperadores | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar operadores";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
