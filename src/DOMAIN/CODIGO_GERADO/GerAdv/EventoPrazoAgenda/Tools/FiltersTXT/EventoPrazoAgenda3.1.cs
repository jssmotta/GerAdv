...
public static class FilterEventoPrazoAgendaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar EventoPrazoAgenda em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterEventoPrazoAgenda:

- O filter irá filtrar os EventoPrazoAgenda conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterEventoPrazoAgenda:(Evento Prazo Agenda)
- nome: _nome ou descrição para Evento Prazo Agenda;
- bold: Bold;
...
- Se ouvir "todos os EventoPrazoAgenda" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterEventoPrazoAgenda? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt EventoPrazoAgenda length: {result.Length}");
#endif
