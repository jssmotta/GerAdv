...
#region ListarDados_BensMateriais
    public static async IAsyncEnumerable<DBBensMateriais> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBBensMateriais(dbRec);
        }
    }

    public static IEnumerable<DBBensMateriais> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
