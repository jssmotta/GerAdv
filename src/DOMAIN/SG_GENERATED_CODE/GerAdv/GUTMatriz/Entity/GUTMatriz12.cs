...
public partial class FGUTMatriz : MenphisSI.SG.GerAdv.DBGUTMatriz, IDBGUTMatriz
{
    // DI-compatible default constructor
    public FGUTMatriz() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FGUTMatriz CreateFromReader(SqlDataReader? dbRec)
    {
        return new FGUTMatriz(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FGUTMatriz CreateFromDataRow(DataRow? dbRec)
    {
        return new FGUTMatriz(dbRec);
    }

    private FGUTMatriz(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FGUTMatriz(DataRow? dbRec) : base(dbRec)
    {
    }

    private FGUTMatriz(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FGUTMatriz CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FGUTMatriz(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FGUTMatriz> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fGUTMatriz = new FGUTMatriz();
        await fGUTMatriz.CarregarAsync(id, oCnn);
        return fGUTMatriz;
    }
