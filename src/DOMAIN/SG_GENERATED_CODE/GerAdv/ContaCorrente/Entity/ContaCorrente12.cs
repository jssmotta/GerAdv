...
public partial class FContaCorrente : MenphisSI.SG.GerAdv.DBContaCorrente, IDBContaCorrente
{
    // DI-compatible default constructor
    public FContaCorrente() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FContaCorrente CreateFromReader(SqlDataReader? dbRec)
    {
        return new FContaCorrente(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FContaCorrente CreateFromDataRow(DataRow? dbRec)
    {
        return new FContaCorrente(dbRec);
    }

    private FContaCorrente(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FContaCorrente(DataRow? dbRec) : base(dbRec)
    {
    }

    private FContaCorrente(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FContaCorrente CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FContaCorrente(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FContaCorrente> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fContaCorrente = new FContaCorrente();
        await fContaCorrente.CarregarAsync(id, oCnn);
        return fContaCorrente;
    }
