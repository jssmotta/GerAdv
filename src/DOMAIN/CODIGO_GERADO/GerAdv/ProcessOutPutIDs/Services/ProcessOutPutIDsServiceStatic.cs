...
public partial class ProcessOutPutIDsService
{
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterProcessOutPutIDs filtro)
    {
        ...
        cWhere.Append(filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutPutIDsDicInfo.PTabelaNome}].[{DBProcessOutPutIDsDicInfo.Nome}]  {DevourerConsts.MsiCollate} like @{nameof(DBProcessOutPutIDsDicInfo.Nome)}");
        cWhere.Append(filtro.GUID.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutPutIDsDicInfo.PTabelaNome}].[{DBProcessOutPutIDsDicInfo.GUID}]  {DevourerConsts.MsiCollate} like @{nameof(DBProcessOutPutIDsDicInfo.GUID)}");
        if (!filtro.Codigo_filtro.IsEmpty() && filtro.Codigo_filtro_end.IsEmpty())
        {
            cWhere.Append(filtro.Codigo_filtro <= 0 ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBProcessOutPutIDsDicInfo.PTabelaNome}].[{DBProcessOutPutIDsDicInfo.CampoCodigo}] >= @{nameof(DBProcessOutPutIDsDicInfo.CampoCodigo)}");
        }
        else
        {
            cWhere.Append((filtro.Codigo_filtro <= 0 && filtro.Codigo_filtro_end <= 0) ? string.Empty : (!(filtro.Codigo_filtro <= 0) && !(filtro.Codigo_filtro_end <= 0)) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutPutIDsDicInfo.CampoCodigo} BETWEEN @{nameof(DBProcessOutPutIDsDicInfo.CampoCodigo)} AND @{nameof(DBProcessOutPutIDsDicInfo.CampoCodigo)}_end" : !(filtro.Codigo_filtro <= 0) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutPutIDsDicInfo.CampoCodigo} = @{nameof(DBProcessOutPutIDsDicInfo.CampoCodigo)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBProcessOutPutIDsDicInfo.CampoCodigo} <= @{nameof(DBProcessOutPutIDsDicInfo.CampoCodigo)}_end");
        }
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter> parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarN(max, uri, where, parameters, DBProcessOutPutIDsDicInfo.CampoNome);
