...
public partial class TipoEnderecoSistema
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(tesDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.TipoEnderecoSistema WITH (NOLOCK, INDEX = idx_TipoEnderecoSistema_AuditorDtAtu)
WHERE tesCodigo = @id
...
FROM {oCnn.UseDbo}.TipoEnderecoSistema WITH (NOLOCK)
