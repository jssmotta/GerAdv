...
public static class ProCDAEndpoints
{
    ...
    private static void MapProCDARoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IProCDAValidation validation, IProCDAWriter writer, IProcessosReader processosReader, IProCDAService service) =>
        {
            logger.LogInfo("ProCDA", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("ProCDA", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("ProCDA_GetColumns").WithDisplayName("Get ProCDA Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IProCDAValidation validation, IProCDAWriter writer, IProcessosReader processosReader, IProCDAService service) =>
        {
            logger.LogInfo("ProCDA", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("ProCDA", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("ProCDA_UpdateColumns").WithDisplayName("Update ProCDA Columns");
