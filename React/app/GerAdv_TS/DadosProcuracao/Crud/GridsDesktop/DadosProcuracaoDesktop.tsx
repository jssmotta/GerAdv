...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('dadosprocuracao-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('dadosprocuracao-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('dadosprocuracao-columns-order'));
}, []);
...
{initialized && (
  ...
  className='grid-desktop-crud grid-desktop-dadosprocuracao'
