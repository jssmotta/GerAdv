...
public static class FilterPenhoraStatusTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar PenhoraStatus em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterPenhoraStatus:

- O filter irá filtrar os PenhoraStatus conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os PenhoraStatus" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterPenhoraStatus? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt PenhoraStatus length: {result.Length}");
#endif
