...
export const useTiposAcaoList = (dataService: ITiposAcaoService, currentFilter?: FilterTiposAcao | undefined | null) => {
  const [data, setData] = useState<ITiposAcao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTiposAcao | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTiposAcao | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTiposAcao);
      setData(result);
    } catch (err) {
