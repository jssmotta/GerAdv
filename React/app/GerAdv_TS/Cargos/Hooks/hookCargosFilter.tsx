...
import { FilterCargos } from "@/app/GerAdv_TS/Cargos/Filters/Cargos";
import FilterDialogButton from "@/app/components/Cruds/FilterDialogButton";

interface UseCargosFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterCargos | undefined | null,
  ) => Promise<void>;
}

interface UseCargosFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterCargos;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterCargos>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterCargos) => Promise<void>;

  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterCargos>,
  ) => React.ReactNode;
...
export const useCargosFilter = ({
  handleFetchWithFilter,
}: UseCargosFilterProps): UseCargosFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterCargos>(
    {} as FilterCargos,
  );
  ...
  const handleConfirmSearch = async (filter: FilterCargos) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterCargos);
    sessionStorage.removeItem(btoa("CargosFilter"));
    handleFetchWithFilter({} as FilterCargos);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterCargos>) => (
    <>
      <InputInput
        tabIndex={0}
        type="text"
