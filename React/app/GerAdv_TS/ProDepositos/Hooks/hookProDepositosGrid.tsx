...
// useProDepositosGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de ProDepositos
'use client';
...
interface UseProDepositosGridProps {
  data: IProDepositos[];
...
export const useProDepositosGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('prodepositos-columns-order'), JSON.stringify(reorderableFields));
