...
import DocsRecebidosItensWindow from '../Crud/Grids/DocsRecebidosItensWindow';
...
import { useDocsRecebidosItensComboBox } from '../Hooks/hookDocsRecebidosItens';
const DocsRecebidosItensComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'docsrecebidositensInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useDocsRecebidosItensComboBox(docsrecebidositensService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IDocsRecebidosItens | null>(null);
const [addRecord, setAddRecord] = useState<IDocsRecebidosItens | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await docsrecebidositensService.fetchDocsRecebidosItensById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nome });
  } catch (error) {
  console.log('Erro ao carregar Docs Recebidos Itens:');
}
};
...
const handleActionClick = () => {
  ...
  const newRecord = DocsRecebidosItensEmpty();
...
const handleSuccess = (newDocsRecebidosItens?: IDocsRecebidosItens) => {
  if (newDocsRecebidosItens) {
    setValue({ id: newDocsRecebidosItens.id, nome: newDocsRecebidosItens.nome });
    loadRecordForEdit(newDocsRecebidosItens.id);
    handleValueChange(newDocsRecebidosItens);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .docsrecebidositens-dropdown-popup-msi {
  ...
  .docsrecebidositens-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .docsrecebidositens-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .docsrecebidositens-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedDocsRecebidosItens={editRecord || addRecord || DocsRecebidosItensEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'docsrecebidositens-dropdown-popup-msi'
          }}
          />
...
export default DocsRecebidosItensComboBox;