...
public static class FilterColaboradoresTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Colaboradores em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterColaboradores:

- O filter irá filtrar os Colaboradores conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterColaboradores:(Colaboradores)
- cargo: número ou IDs;
- cliente: número ou IDs;
- sexo: sexo do Colaboradores: 1=Homem; 0=Mulher; -2147483648=ambos;
- nome: _nome ou descrição para Colaboradores;
- cpf: somente os 11 caracteres sem mascara;
- rg: RG;
- dtnasc: DtNasc;
- idade: número ou IDs;
...
- Se ouvir "todos os Colaboradores" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterColaboradores? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Colaboradores length: {result.Length}");
#endif
