...
export const useOperadorGruposAgendaForm = (
  ...
  const loadOperadorGruposAgenda = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Operador Grupos Agenda";
...
export const useOperadorGruposAgendaList = (
  dataService: IOperadorGruposAgendaService,
  currentFilter?: FilterOperadorGruposAgenda | undefined | null,
) => {
  const [data, setData] = useState<IOperadorGruposAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    async (filtro?: FilterOperadorGruposAgenda | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar operadorgruposagenda";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsOperadorGruposAgenda() {
  async function runValidation(
    ...
    const operadorgruposagendaApi = new OperadorGruposAgendaApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IOperadorGruposAgenda): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.sqlwhere.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo SQLWhere não pode ter mais de 2147483647 caracteres.",
        };
      }
