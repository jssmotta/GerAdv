...
#region ListarDados_AgendaStatus
    public static async IAsyncEnumerable<DBAgendaStatus> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAgendaStatus(dbRec);
        }
    }

    public static IEnumerable<DBAgendaStatus> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
