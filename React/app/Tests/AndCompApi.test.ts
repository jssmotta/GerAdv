...
import { AndCompApiError } from '../GerAdv_TS/AndComp/Apis/ApiAndComp';
import { AndCompApi } from '@/app/GerAdv_TS/AndComp/Apis/ApiAndComp';
import { FilterAndComp } from '../GerAdv_TS/AndComp/Filters/AndComp';
...
describe('AndCompApi', () => {
  ...
  beforeEach(() => {
    ...
    andcompApi = new AndCompApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(andcompApi).toBeInstanceOf(AndCompApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AndCompApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AndCompApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all andcomp items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await andcompApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(andcompApi.getAll()).rejects.toThrow(AndCompApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await andcompApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AndComp_last_getAll_${200}`)));
    });
    ...
    it('should throw AndCompApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAndComp = AndCompTestEmpty();
    ...
    it('should fetch andcomp by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await andcompApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAndComp));
      ...
      expect(result.data).toEqual(mockAndComp);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndComp',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAndComp = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter andcomp items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await andcompApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new andcomp successfully', async () => {
      const responseAndComp = { ...mockAndComp, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAndComp,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await andcompApi.addAndUpdate(mockAndComp);
    ...
    it('should update existing andcomp successfully', async () => {
      const existingAndComp = { ...mockAndComp, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAndComp,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await andcompApi.addAndUpdate(existingAndComp);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(andcompApi.addAndUpdate(mockAndComp)).rejects.toThrow(AndCompApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndComp',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete andcomp successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await andcompApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(andcompApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AndComp',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(andcompApi.delete(1)).rejects.toThrow(AndCompApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAndComp = { funcionario: 1 };
      
      const result = andcompApi.useFilter(mockFilter);
  ...
  describe('AndCompApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AndCompApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AndCompApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AndCompApiError('Test message', 500);
