...
SELECT TOP (1) 
	FORMAT(ufDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(ufDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    ufVisto as Visto
FROM {oCnn.UseDbo}.UF dbq WITH (INDEX = idx_UF_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.ufQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.ufQuemAtu
WHERE [ufCodigo] = @id
...
FROM {oCnn.UseDbo}.UF WITH (INDEX = idx_{DBUFDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [ufCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBUFDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(ufDtAtu, ufDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBUFDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(ufDtAtu, ufDtCad) DESC;", oCnn.InnerConnection);
