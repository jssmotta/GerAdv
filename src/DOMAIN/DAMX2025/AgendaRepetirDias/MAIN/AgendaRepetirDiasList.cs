...
#region ListarDados_AgendaRepetirDias
    public static async IAsyncEnumerable<DBAgendaRepetirDias> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAgendaRepetirDias(dbRec);
        }
    }

    public static IEnumerable<DBAgendaRepetirDias> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
