...
public static class FilterFornecedoresTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Fornecedores em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterFornecedores:

- O filter irá filtrar os Fornecedores conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Fornecedores" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterFornecedores? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Fornecedores length: {result.Length}");
#endif
