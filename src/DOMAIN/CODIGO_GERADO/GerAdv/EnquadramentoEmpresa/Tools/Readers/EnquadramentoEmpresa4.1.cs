...
FROM {oCnn.UseDbo}.EnquadramentoEmpresa WITH (INDEX = idx_{DBEnquadramentoEmpresaDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [eqeCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBEnquadramentoEmpresaDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(eqeDtAtu, eqeDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBEnquadramentoEmpresaDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(eqeDtAtu, eqeDtCad) DESC;", oCnn.InnerConnection);
