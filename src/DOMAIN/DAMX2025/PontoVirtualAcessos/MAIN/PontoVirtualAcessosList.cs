...
#region ListarDados_PontoVirtualAcessos
    public static async IAsyncEnumerable<DBPontoVirtualAcessos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPontoVirtualAcessos(dbRec);
        }
    }

    public static IEnumerable<DBPontoVirtualAcessos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
