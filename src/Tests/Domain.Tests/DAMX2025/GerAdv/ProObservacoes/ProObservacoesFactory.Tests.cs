...
public class FProObservacoesFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProObservacoesFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProObservacoesFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProObservacoesFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "pobCodigo")
        var dt = new DataTable();
        dt.Columns.Add("pobCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["pobCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProObservacoes>();
    }
    ...
    {
        // Arrange
        var factory = new FProObservacoesFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("pobCodigo", typeof(int));
        mockDataTable.Columns.Add("pobNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["pobCodigo"] = testId;
        row["pobNome"] = "Test ProObservacoes";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["pobCodigo"] = 123;
        row["pobNome"] = "Test ProObservacoes Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProObservacoes properties
        }
    }
