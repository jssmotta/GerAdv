...
public partial class AgendaQuemService(IOptions<AppSettings> appSettings, IFAgendaQuemFactory agendaquemFactory, IAgendaQuemReader reader, IAgendaQuemValidation validation, IAgendaQuemWriter writer, IAdvogadosReader advogadosReader, IFuncionariosReader funcionariosReader, IPrepostosReader prepostosReader, HybridCache cache, IMemoryCache memory) : IAgendaQuemService, IDisposable
{
    ...
    public async Task<IEnumerable<AgendaQuemResponseAll>> Filter([FromQuery] int max, [FromBody] Filters.FilterAgendaQuem filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-{max}-AgendaQuem-Filter-{where.GetHashCode2()}{filterHash}";
        var entryOptions = new HybridCacheEntryOptions
        {
