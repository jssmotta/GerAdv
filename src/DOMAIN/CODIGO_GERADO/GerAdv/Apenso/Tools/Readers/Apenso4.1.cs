...
public partial class ApensoReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBApensoDicInfo.CampoCodigo}";
        ...
        var query = $@"SELECT TOP ({max})
                   ...
                   FROM {DBApenso.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBProcessosDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBProcessosDicInfo.PTabelaNome}].[{DBProcessos.CampoCodigo}]=[{DBApensoDicInfo.PTabelaNome}].[{DBApensoDicInfo.Processo}]
