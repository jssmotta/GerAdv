...
public class FRegimeTributacaoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FRegimeTributacaoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FRegimeTributacaoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FRegimeTributacaoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "rdtCodigo")
        var dt = new DataTable();
        dt.Columns.Add("rdtCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["rdtCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FRegimeTributacao>();
    }
    ...
    {
        // Arrange
        var factory = new FRegimeTributacaoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("rdtCodigo", typeof(int));
        mockDataTable.Columns.Add("rdtNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["rdtCodigo"] = testId;
        row["rdtNome"] = "Test RegimeTributacao";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["rdtCodigo"] = 123;
        row["rdtNome"] = "Test RegimeTributacao Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FRegimeTributacao properties
        }
    }
