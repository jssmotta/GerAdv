...
#region ListarDados_Cidade
    public static async IAsyncEnumerable<DBCidade> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBCidade(dbRec);
        }
    }

    public static IEnumerable<DBCidade> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
