...
export const GUTTipoGridDesktopComponent = React.memo(
({
  data, 
  onRowClick, 
  onDeleteClick, 
...
const filterLogic = useCallback((data: IGUTTipo, filters: Record<string, any>) => {
...
const openConsultaGUTMatriz = (id: number) => {
  router.push(`/pages/gutmatriz/?guttipo=${id}`);
};
const EditarCellGUTMatriz = (props: any) => {
  return (
  <>
  <td>
    <div onClick={() => openConsultaGUTMatriz(props.dataItem.id)}><span title='Editar G U T Matriz'><SvgIcon icon={pencilIcon} /></span></div>
  </td>
</>
);
};
...
const gridColumns = useMemo(() => [
  ...
  width={'65px'}
  title='G U T Matriz'
  cells={{ data: EditarCellGUTMatriz }}
  />, 
  <GridColumn
  field='id'
  width={'55px'}
...
{initialized && (
  ...
  className='grid-desktop-guttipo'
