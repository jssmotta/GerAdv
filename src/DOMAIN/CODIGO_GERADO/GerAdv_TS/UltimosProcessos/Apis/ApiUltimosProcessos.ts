...
import { UltimosProcessos } from '../../Models/UltimosProcessos';
...
export class UltimosProcessosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'UltimosProcessosApiError';
    this.status = status;
    this.code = code;
...
export class UltimosProcessosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/UltimosProcessos`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UltimosProcessos_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os UltimosProcessos');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UltimosProcessos_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Ultimos Processos com ID ${id}`);
        }
    }

            public async filter(filtro: FilterUltimosProcessos): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UltimosProcessos_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar UltimosProcessos');
                }
            }


    public async validation(regUltimosProcessos: IUltimosProcessos): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regUltimosProcessos as UltimosProcessos, this.getHeaders());
    ...
    public async addAndUpdate(regUltimosProcessos: IUltimosProcessos): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regUltimosProcessos as UltimosProcessos, this.getHeaders());
            var register = result.data as IUltimosProcessos;        
            const action = regUltimosProcessos.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Ultimos Processos');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o UltimosProcessos. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'UltimosProcessos',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Ultimos Processos com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterUltimosProcessos) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<UltimosProcessos[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de ultimosprocessos:', error);
            this.notificationService.notify({
              entity: 'UltimosProcessos',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos ultimosprocessos'
            });
          }
        });
    }
