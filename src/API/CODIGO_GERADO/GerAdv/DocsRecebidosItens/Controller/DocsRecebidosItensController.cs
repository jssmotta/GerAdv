...
public partial class DocsRecebidosItensController(IDocsRecebidosItensService docsrecebidositensService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.DocsRecebidosItens regDocsRecebidosItens, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "DocsRecebidosItens: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "DocsRecebidosItens: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.DocsRecebidosItens regDocsRecebidosItens, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _docsrecebidositensService.Validation(regDocsRecebidosItens, uri);
            if (result == null)
            {
                _logger.Warn("DocsRecebidosItens: Validation failed to add or update DocsRecebidosItens, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "DocsRecebidosItens: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}