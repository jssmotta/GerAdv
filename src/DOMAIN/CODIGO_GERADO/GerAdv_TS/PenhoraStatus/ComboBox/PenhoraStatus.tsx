...
import PenhoraStatusWindow from '../Crud/Grids/PenhoraStatusWindow';
...
import { usePenhoraStatusComboBox } from '../Hooks/hookPenhoraStatus';
const PenhoraStatusComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'penhorastatusInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = usePenhoraStatusComboBox(penhorastatusService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IPenhoraStatus | null>(null);
const [addRecord, setAddRecord] = useState<IPenhoraStatus | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await penhorastatusService.fetchPenhoraStatusById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = PenhoraStatusEmpty();
...
const handleSuccess = (newPenhoraStatus?: IPenhoraStatus) => {
  if (newPenhoraStatus) {
    setValue({ id: newPenhoraStatus.id, nome: newPenhoraStatus.nome });
    loadRecordForEdit(newPenhoraStatus.id);
    handleValueChange(newPenhoraStatus);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .penhorastatus-dropdown-popup-msi {
  ...
  .penhorastatus-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .penhorastatus-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .penhorastatus-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedPenhoraStatus={editRecord || addRecord || PenhoraStatusEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'penhorastatus-dropdown-popup-msi'
          }}
          />
...
export default PenhoraStatusComboBox;