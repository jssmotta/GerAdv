...
import { AgendaQuemApiError } from '../GerAdv_TS/AgendaQuem/Apis/ApiAgendaQuem';
import { AgendaQuemApi } from '@/app/GerAdv_TS/AgendaQuem/Apis/ApiAgendaQuem';
import { FilterAgendaQuem } from '../GerAdv_TS/AgendaQuem/Filters/AgendaQuem';
...
describe('AgendaQuemApi', () => {
  let agendaquemApi: AgendaQuemApi;
  ...
  beforeEach(() => {
    ...
    agendaquemApi = new AgendaQuemApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendaquemApi).toBeInstanceOf(AgendaQuemApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaQuemApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaQuemApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all agendaquem items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendaquemApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendaquemApi.getAll()).rejects.toThrow(AgendaQuemApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendaquemApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaQuem_last_getAll_${200}`)));
    });
    ...
    it('should throw AgendaQuemApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaQuem = AgendaQuemTestEmpty();
    ...
    it('should fetch agendaquem by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendaquemApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaQuem));
      ...
      expect(result.data).toEqual(mockAgendaQuem);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaQuem',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaQuem = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendaquem items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendaquemApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaQuem: IAgendaQuem = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendaquem successfully', async () => {
      const responseAgendaQuem = { ...mockAgendaQuem, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaQuem,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendaquemApi.addAndUpdate(mockAgendaQuem);
    ...
    it('should update existing agendaquem successfully', async () => {
      const existingAgendaQuem = { ...mockAgendaQuem, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaQuem,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendaquemApi.addAndUpdate(existingAgendaQuem);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendaquemApi.addAndUpdate(mockAgendaQuem)).rejects.toThrow(AgendaQuemApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaQuem',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendaquem successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendaquemApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendaquemApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaQuem',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendaquemApi.delete(1)).rejects.toThrow(AgendaQuemApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaQuem = { funcionario: 1 };
      
      const result = agendaquemApi.useFilter(mockFilter);
  ...
  describe('AgendaQuemApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaQuemApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaQuemApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaQuemApiError('Test message', 500);
