...
export const useAlarmSMSList = (dataService: IAlarmSMSService, currentFilter?: FilterAlarmSMS | undefined | null) => {
  const [data, setData] = useState<IAlarmSMS[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterAlarmSMS | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterAlarmSMS | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterAlarmSMS);
      setData(result);
    } catch (err) {
