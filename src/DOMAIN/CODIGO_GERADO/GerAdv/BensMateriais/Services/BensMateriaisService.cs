...
public partial class BensMateriaisService(IOptions<AppSettings> appSettings, IBensMateriaisReader reader, IBensMateriaisValidation validation, IBensMateriaisWriter writer, IBensClassificacaoReader bensclassificacaoReader, IFornecedoresReader fornecedoresReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IBensMateriaisService, IDisposable
{
    ...
    public async Task<BensMateriaisResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-BensMateriais-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("BensMateriais: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBBensMateriais(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBBensMateriaisDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("BensMateriais: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBBensMateriais(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBBensMateriaisDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
