...
#region ListarDados_ModelosDocumentos
    public static async IAsyncEnumerable<DBModelosDocumentos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBModelosDocumentos(dbRec);
        }
    }

    public static IEnumerable<DBModelosDocumentos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
