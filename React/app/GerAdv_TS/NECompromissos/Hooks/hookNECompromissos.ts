...
export const useNECompromissosForm = (
  ...
  const loadNECompromissos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar N E Compromissos";
...
export const useNECompromissosList = (
  dataService: INECompromissosService,
  currentFilter?: FilterNECompromissos | undefined | null,
) => {
  const [data, setData] = useState<INECompromissos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterNECompromissos | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterNECompromissos | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar necompromissos";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsNECompromissos() {
  ...
  function validate(data: INECompromissos): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      if (data.textocompromisso.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo TextoCompromisso não pode ter mais de 2147483647 caracteres.",
        };
      }
