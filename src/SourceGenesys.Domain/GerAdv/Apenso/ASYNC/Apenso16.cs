...
public partial class DBApenso
{
    ...
    /// </summary>
    public static async Task<DBApenso> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBApenso();
...
#if (!NOTSTORED_Apenso)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFApenso || pFldFAcao || pFldFDtDist || pFldFOBS || pFldFValorCausa;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBApensoDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFApenso)
            updateTool.Fields(DBApensoDicInfo.Apenso, m_FApenso, ETiposCampos.FString);
        if (pFldFAcao)
            updateTool.Fields(DBApensoDicInfo.Acao, m_FAcao, ETiposCampos.FString);
        if (pFldFDtDist)
            updateTool.Fields(DBApensoDicInfo.DtDist, m_FDtDist, ETiposCampos.FDate);
        if (pFldFOBS)
            updateTool.Fields(DBApensoDicInfo.OBS, m_FOBS, ETiposCampos.FString);
        if (pFldFValorCausa)
            updateTool.Fields(DBApensoDicInfo.ValorCausa, m_FValorCausa, ETiposCampos.FDecimal);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBApensoDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBApensoDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBApensoDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBApensoDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBApensoDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
