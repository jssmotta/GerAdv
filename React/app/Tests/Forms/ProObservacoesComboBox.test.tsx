// app/TestsForms/ProObservacoesComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import ProObservacoesComboBox from '@/app/GerAdv_TS/ProObservacoes/ComboBox/ProObservacoes';
...
import { ProObservacoesEmpty, ProObservacoesTestEmpty } from '@/app/GerAdv_TS/Models/ProObservacoes';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/ProObservacoes/Hooks/hookProObservacoes', () => ({
  useProObservacoesComboBox: jest.fn(() => ({
    options: [
    { ...ProObservacoesEmpty(), id: 1, nome: 'Pro Observacoes 1' },
    { ...ProObservacoesEmpty(), id: 2, nome: 'Pro Observacoes 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/ProObservacoes/Apis/ApiProObservacoes', () => ({
  ProObservacoesApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProObservacoes' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProObservacoes' }),
...
jest.mock('@/app/GerAdv_TS/ProObservacoes/Services/ProObservacoes.service', () => ({
  ProObservacoesService: jest.fn().mockImplementation(() => ({
    fetchProObservacoesById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProObservacoes' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/ProObservacoes/Crud/Grids/ProObservacoesWindow', () => {
  return function MockProObservacoesWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='proobservacoes-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Pro Observacoes' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ProObservacoesTestEmpty(), id: 1, nome: 'Pro Observacoes 1' },
  { ...ProObservacoesTestEmpty(), id: 2, nome: 'Pro Observacoes 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Pro Observacoes',
  name: 'proobservacoes',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: ProObservacoesEmpty(), 
};
describe('ProObservacoesComboBox', () => {
  ...
  const { useProObservacoesComboBox } = require('@/app/GerAdv_TS/ProObservacoes/Hooks/hookProObservacoes');
  (useProObservacoesComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ProObservacoesComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Pro Observacoes')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Pro Observacoes')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ProObservacoesTestEmpty(), id: 1, nome: 'Pro Observacoes 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useProObservacoesComboBox } = require('@/app/GerAdv_TS/ProObservacoes/Hooks/hookProObservacoes');
    useProObservacoesComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ProObservacoesWindow on action button click (add)', async () => {
    render(<ProObservacoesComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('proobservacoes-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('proobservacoes-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ProObservacoesComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ProObservacoesWindow onClose and onSuccess', () => {
    render(<ProObservacoesComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ProObservacoesComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
