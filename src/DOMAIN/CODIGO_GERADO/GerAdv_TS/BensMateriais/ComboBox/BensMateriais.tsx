...
const BensMateriaisComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IBensMateriais | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IBensMateriais | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IBensMateriais);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <BensMateriaisWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedBensMateriais={editar ?? addValue ?? BensMateriaisEmpty()} /> 
            )}
