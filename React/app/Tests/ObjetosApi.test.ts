...
import { ObjetosApiError } from '../GerAdv_TS/Objetos/Apis/ApiObjetos';
import { ObjetosApi } from '@/app/GerAdv_TS/Objetos/Apis/ApiObjetos';
import { FilterObjetos } from '../GerAdv_TS/Objetos/Filters/Objetos';
...
describe('ObjetosApi', () => {
  ...
  beforeEach(() => {
    ...
    objetosApi = new ObjetosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(objetosApi).toBeInstanceOf(ObjetosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ObjetosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ObjetosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all objetos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await objetosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(objetosApi.getAll()).rejects.toThrow(ObjetosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await objetosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Objetos_last_getAll_${200}`)));
    });
    ...
    it('should throw ObjetosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockObjetos = ObjetosTestEmpty();
    ...
    it('should fetch objetos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await objetosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockObjetos));
      ...
      expect(result.data).toEqual(mockObjetos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Objetos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterObjetos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter objetos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await objetosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new objetos successfully', async () => {
      const responseObjetos = { ...mockObjetos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseObjetos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await objetosApi.addAndUpdate(mockObjetos);
    ...
    it('should update existing objetos successfully', async () => {
      const existingObjetos = { ...mockObjetos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingObjetos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await objetosApi.addAndUpdate(existingObjetos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(objetosApi.addAndUpdate(mockObjetos)).rejects.toThrow(ObjetosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Objetos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete objetos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await objetosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(objetosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Objetos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(objetosApi.delete(1)).rejects.toThrow(ObjetosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterObjetos = { funcionario: 1 };
      
      const result = objetosApi.useFilter(mockFilter);
  ...
  describe('ObjetosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ObjetosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ObjetosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ObjetosApiError('Test message', 500);
