...
public static class FilterPenhoraTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Penhora em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterPenhora:

- O filter irá filtrar os Penhora conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterPenhoraStatus: filtro específico para Penhora Status
...
CAMPOS filterPenhoraStatus (Penhora Status):
- nome: _nome ou descrição para Penhora Status;
...
CAMPOS filterPenhora:(Penhora)
- processo: número ou IDs;
- nome: _nome ou descrição para Penhora;
- descricao: Descrições;
- datapenhora: DataPenhora;
- penhorastatus: número ou IDs;
- master: número ou IDs;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
- quemcad, quemcad_end: ID do operador que cadastrou
...
- Se ouvir "todos os Penhora" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterPenhora? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Penhora length: {result.Length}");
#endif
