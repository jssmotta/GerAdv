...
export const useOponentesRepLegalForm = (
  ...
  const loadOponentesRepLegal = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Oponentes Rep Legal";
...
export const useOponentesRepLegalList = (
  dataService: IOponentesRepLegalService,
  currentFilter?: FilterOponentesRepLegal | undefined | null,
) => {
  const [data, setData] = useState<IOponentesRepLegal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOponentesRepLegal | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterOponentesRepLegal | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar oponentesreplegal";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsOponentesRepLegal() {
  async function runValidation(
    ...
    const oponentesreplegalApi = new OponentesRepLegalApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IOponentesRepLegal): {
    isValid: boolean;
    message: string;
  } {
