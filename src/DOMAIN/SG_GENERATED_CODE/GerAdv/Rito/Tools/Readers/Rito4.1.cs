...
public partial class RitoReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBRitoDicInfo.CampoNome}";
        ...
        var query = $@"SELECT TOP ({max})
                   {campos}
                   FROM {DBRito.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(ritDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(ritDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    ritVisto as Visto
FROM {oCnn.UseDbo}.Rito dbq WITH (INDEX = idx_Rito_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.ritQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.ritQuemAtu
WHERE [ritCodigo] = @id
...
FROM {oCnn.UseDbo}.Rito WITH (INDEX = idx_{DBRitoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [ritCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBRitoDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(ritDtAtu, ritDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBRitoDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(ritDtAtu, ritDtCad) DESC;", oCnn.InnerConnection);
