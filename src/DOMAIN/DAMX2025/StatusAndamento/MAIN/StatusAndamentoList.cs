...
#region ListarDados_StatusAndamento
    public static async IAsyncEnumerable<DBStatusAndamento> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBStatusAndamento(dbRec);
        }
    }

    public static IEnumerable<DBStatusAndamento> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
