...
#region TableDefinition_ProcessOutputEngine
...
#endregion
    public DBProcessOutputEngine(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBProcessOutputEngine(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBProcessOutputEngine(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_ProcessOutputEngine
    public int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        {
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_ProcessOutputEngine)
#endif
