...
import { ProPartesApiError } from '../GerAdv_TS/ProPartes/Apis/ApiProPartes';
import { ProPartesApi } from '@/app/GerAdv_TS/ProPartes/Apis/ApiProPartes';
import { FilterProPartes } from '../GerAdv_TS/ProPartes/Filters/ProPartes';
...
describe('ProPartesApi', () => {
  let propartesApi: ProPartesApi;
  ...
  beforeEach(() => {
    ...
    propartesApi = new ProPartesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(propartesApi).toBeInstanceOf(ProPartesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProPartesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProPartesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all propartes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await propartesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(propartesApi.getAll()).rejects.toThrow(ProPartesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await propartesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProPartes_last_getAll_${200}`)));
    });
    ...
    it('should throw ProPartesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProPartes = ProPartesTestEmpty();
    ...
    it('should fetch propartes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await propartesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProPartes));
      ...
      expect(result.data).toEqual(mockProPartes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProPartes',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProPartes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter propartes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await propartesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProPartes: IProPartes = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new propartes successfully', async () => {
      const responseProPartes = { ...mockProPartes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProPartes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await propartesApi.addAndUpdate(mockProPartes);
    ...
    it('should update existing propartes successfully', async () => {
      const existingProPartes = { ...mockProPartes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProPartes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await propartesApi.addAndUpdate(existingProPartes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(propartesApi.addAndUpdate(mockProPartes)).rejects.toThrow(ProPartesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProPartes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete propartes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await propartesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(propartesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProPartes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(propartesApi.delete(1)).rejects.toThrow(ProPartesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProPartes = { funcionario: 1 };
      
      const result = propartesApi.useFilter(mockFilter);
  ...
  describe('ProPartesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProPartesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProPartesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProPartesApiError('Test message', 500);
