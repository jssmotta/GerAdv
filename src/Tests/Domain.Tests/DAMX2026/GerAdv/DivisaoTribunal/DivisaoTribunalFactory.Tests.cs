...
// Source:EntityFactoryTestGenerator
using MenphisSI.DB;
using System.Data;
using Xunit.Abstractions;

namespace MenphisSI.GerAdv.Factory.Tests;
public class FDivisaoTribunalFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FDivisaoTribunalFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FDivisaoTribunalFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FDivisaoTribunalFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "divCodigo")
        var dt = new DataTable();
        dt.Columns.Add("divCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["divCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FDivisaoTribunal>();
    }
    ...
    {
        // Arrange
        var factory = new FDivisaoTribunalFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("divCodigo", typeof(int));
        mockDataTable.Columns.Add("divGuid", typeof(string));
        var row = mockDataTable.NewRow();
        row["divCodigo"] = testId;
        row["divGuid"] = "Test DivisaoTribunal";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["divCodigo"] = 123;
        row["divGuid"] = "Test DivisaoTribunal Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FDivisaoTribunal properties
        }
    }
