...
export class AlertasEnviadosApi {
 ...
 public async filterPreload(max: number, filtro: FilterAlertasEnviados): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlertasEnviados-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterAlertasEnviados): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlertasEnviados-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterAlertasEnviados) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AlertasEnviados[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de AlertasEnviados');
            this.notificationService.notify({
              entity: 'AlertasEnviados',
              id: 0,
              action: NotifySystemActions.ERROR,
