...
#region ListarDados_OperadorEMailPopup
    public static async IAsyncEnumerable<DBOperadorEMailPopup> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperadorEMailPopup(dbRec);
        }
    }

    public static IEnumerable<DBOperadorEMailPopup> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
