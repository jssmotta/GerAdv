// app/TestsForms/TiposAcaoComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import TiposAcaoComboBox from '@/app/GerAdv_TS/TiposAcao/ComboBox/TiposAcao';
...
import { TiposAcaoEmpty, TiposAcaoTestEmpty } from '@/app/GerAdv_TS/Models/TiposAcao';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TiposAcao/Hooks/hookTiposAcao', () => ({
  useTiposAcaoComboBox: jest.fn(() => ({
    options: [
    { ...TiposAcaoEmpty(), id: 1, nome: 'Tipos Acao 1' },
    { ...TiposAcaoEmpty(), id: 2, nome: 'Tipos Acao 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TiposAcao/Apis/ApiTiposAcao', () => ({
  TiposAcaoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TiposAcao' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TiposAcao' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/TiposAcao/Services/TiposAcao.service', () => ({
  TiposAcaoService: jest.fn().mockImplementation(() => ({
    fetchTiposAcaoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TiposAcao' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TiposAcao/Crud/Grids/TiposAcaoWindow', () => {
  return function MockTiposAcaoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tiposacao-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Tipos Acao' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TiposAcaoTestEmpty(), id: 1, nome: 'Tipos Acao 1' },
  { ...TiposAcaoTestEmpty(), id: 2, nome: 'Tipos Acao 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TiposAcaoEmpty(), 
};
describe('TiposAcaoComboBox', () => {
  ...
  const { useTiposAcaoComboBox } = require('@/app/GerAdv_TS/TiposAcao/Hooks/hookTiposAcao');
  (useTiposAcaoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TiposAcaoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Tipos Acao')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Tipos Acao')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TiposAcaoTestEmpty(), id: 1, nome: 'Tipos Acao 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTiposAcaoComboBox } = require('@/app/GerAdv_TS/TiposAcao/Hooks/hookTiposAcao');
    useTiposAcaoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TiposAcaoWindow on action button click (add)', async () => {
    render(<TiposAcaoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tiposacao-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tiposacao-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TiposAcaoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TiposAcaoWindow onClose and onSuccess', () => {
    render(<TiposAcaoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TiposAcaoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
