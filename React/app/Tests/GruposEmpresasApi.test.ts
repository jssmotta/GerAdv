...
import { GruposEmpresasApiError } from '../GerAdv_TS/GruposEmpresas/Apis/ApiGruposEmpresas';
import { GruposEmpresasApi } from '@/app/GerAdv_TS/GruposEmpresas/Apis/ApiGruposEmpresas';
import { FilterGruposEmpresas } from '../GerAdv_TS/GruposEmpresas/Filters/GruposEmpresas';
...
describe('GruposEmpresasApi', () => {
  let gruposempresasApi: GruposEmpresasApi;
  ...
  beforeEach(() => {
    ...
    gruposempresasApi = new GruposEmpresasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gruposempresasApi).toBeInstanceOf(GruposEmpresasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GruposEmpresasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GruposEmpresasApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all gruposempresas items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gruposempresasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gruposempresasApi.getAll()).rejects.toThrow(GruposEmpresasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gruposempresasApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GruposEmpresas_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresas',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw GruposEmpresasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGruposEmpresas = GruposEmpresasTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockGruposEmpresas,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch gruposempresas by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gruposempresasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGruposEmpresas));
      ...
      expect(result.data).toEqual(mockGruposEmpresas);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresas',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGruposEmpresas = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter gruposempresas items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gruposempresasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockGruposEmpresas: IGruposEmpresas = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new gruposempresas successfully', async () => {
      const responseGruposEmpresas = { ...mockGruposEmpresas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGruposEmpresas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gruposempresasApi.addAndUpdate(mockGruposEmpresas);
    ...
    it('should update existing gruposempresas successfully', async () => {
      const existingGruposEmpresas = { ...mockGruposEmpresas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGruposEmpresas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gruposempresasApi.addAndUpdate(existingGruposEmpresas);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gruposempresasApi.addAndUpdate(mockGruposEmpresas)).rejects.toThrow(GruposEmpresasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresas',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gruposempresas successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gruposempresasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gruposempresasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresas',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gruposempresasApi.delete(1)).rejects.toThrow(GruposEmpresasApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGruposEmpresas = { funcionario: 1 };
      
      const result = gruposempresasApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await gruposempresasApi.delete(1);
  ...
  describe('GruposEmpresasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GruposEmpresasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GruposEmpresasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GruposEmpresasApiError('Test message', 500);
