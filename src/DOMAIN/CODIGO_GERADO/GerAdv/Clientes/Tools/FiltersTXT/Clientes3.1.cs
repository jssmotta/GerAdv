...
public static class FilterClientesTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterClientes? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Clientes em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterClientes:

- O filter irá filtrar os Clientes conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Clientes" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Clientes length: {result.Length}");
#endif
