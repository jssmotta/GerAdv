...
public static class FilterTipoContatoCRMTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoContatoCRM em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoContatoCRM:

- O filter irá filtrar os TipoContatoCRM conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterTipoContatoCRM:(Tipo Contato C R M)
- nome: _nome ou descrição para Tipo Contato C R M;
- bold: Bold;
...
- Se ouvir "todos os TipoContatoCRM" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoContatoCRM? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoContatoCRM length: {result.Length}");
#endif
