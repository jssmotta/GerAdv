...
export const OperadorGruposAgendaGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('operadorgruposagenda-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('operadorgruposagenda-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-operadorgruposagenda .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
...
const isColumnVisible = useCallback((field: string) => {
  ...
  const resetColumnsOrder = useCallback(() => {
    ...
    localStorage.removeItem(btoa('operadorgruposagenda-columns-order-mobile'));
  }, []);
