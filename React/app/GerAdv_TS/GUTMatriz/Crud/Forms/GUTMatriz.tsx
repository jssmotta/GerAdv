...
export const GUTMatrizForm: React.FC<GUTMatrizFormProps> = ({
  ...
  const dadoApi = new GUTMatrizApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const [_nomeGUTTipo, setNomeGUTTipo] = useState("");
  const guttipoApi = new GUTTipoApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const addValorGUTTipo = (e: any) => {
    if (e?.id > 0) onChange({ target: { name: "guttipo", value: e.id } });
  };
  ...
  useEffect(() => {
    ...
    if (el) {
      el.textContent =
        gutmatrizData?.id == 0 ? "Editar GUTMatriz" : "Adicionar G U T Matriz";
    }
  }, [gutmatrizData.id]);
  ...
  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-GUTMatriz"
            : "form-container5 form-container-GUTMatriz"
        }
      >
        <form
          className="formInputCadInc"
          id={`GUTMatrizForm-${gutmatrizData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
              ...
              onClose={onClose}
              formId={`GUTMatrizForm-${gutmatrizData.id}`}
          ...
          <div className="grid-container">
            <InputDescription
              ...
              placeholder={`Digite _nome g u t matriz`}
              onChange={onChange}
              required
            />
        ...
        <FooterIncDocker>
          <CopyButton formId={`GUTMatrizForm-${gutmatrizData.id}`} />

          <PrintButton formId={`GUTMatrizForm-${gutmatrizData.id}`} />

          <AuditorButton
            tableName="GUTMatriz"
            coolName="G U T Matriz"
            id={gutmatrizData.id}
          />

          <DeleteButton
            page={"/pages/gutmatriz"}
            id={gutmatrizData.id}
            dadoApi={dadoApi}
          />
        </FooterIncDocker>
