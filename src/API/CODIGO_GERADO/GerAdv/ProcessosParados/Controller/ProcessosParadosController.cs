...
public partial class ProcessosParadosController(IProcessosParadosService processosparadosService) : ControllerBase
{
    ...
    {
        _logger.Info("ProcessosParados: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _processosparadosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProcessosParados: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _processosparadosService.UpdateColumns(parameters, uri);
        if (!result)
        {
