...
export const useGUTAtividadesMatrizList = (dataService: IGUTAtividadesMatrizService, currentFilter?: FilterGUTAtividadesMatriz | undefined | null) => {
  const [data, setData] = useState<IGUTAtividadesMatriz[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterGUTAtividadesMatriz | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterGUTAtividadesMatriz | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterGUTAtividadesMatriz);
      setData(result);
    } catch (err) {
