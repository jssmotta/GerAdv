...
public partial class DBPrecatoria
{
    ...
    /// </summary>
    public static async Task<DBPrecatoria> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBPrecatoria();
...
#if (!NOTSTORED_Precatoria)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFDtDist || pFldFProcesso || pFldFPrecatoria || pFldFDeprecante || pFldFDeprecado || pFldFOBS || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFDtDist)
            updateTool.Fields(DBPrecatoriaDicInfo.DtDist, m_FDtDist, ETiposCampos.FDate);
        if (pFldFProcesso)
            updateTool.Fields(DBPrecatoriaDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFPrecatoria)
            updateTool.Fields(DBPrecatoriaDicInfo.Precatoria, m_FPrecatoria, ETiposCampos.FString);
        if (pFldFDeprecante)
            updateTool.Fields(DBPrecatoriaDicInfo.Deprecante, m_FDeprecante, ETiposCampos.FString);
        if (pFldFDeprecado)
            updateTool.Fields(DBPrecatoriaDicInfo.Deprecado, m_FDeprecado, ETiposCampos.FString);
        if (pFldFOBS)
            updateTool.Fields(DBPrecatoriaDicInfo.OBS, m_FOBS, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBPrecatoriaDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBPrecatoriaDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBPrecatoriaDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBPrecatoriaDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBPrecatoriaDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBPrecatoriaDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
