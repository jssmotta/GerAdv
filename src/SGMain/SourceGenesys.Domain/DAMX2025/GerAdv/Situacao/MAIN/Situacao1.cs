...
public partial class DBSituacao : VAuditor, ICrud
{
#region TableDefinition_Situacao
...
#endregion
    public DBSituacao(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBSituacao(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
        if (oCnn == null)
            return;
...
#region GravarDados_Situacao
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (!isInsert)
            if (!(pFldFParte_Int || pFldFParte_Opo || pFldFTop || pFldFBold || pFldFGuid))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
...
#endif
        if (this.FParte_Int.IsEmpty())
        {
            // Validação preventiva por que ao chegar aqui já passou por outras fases
            throw new Exception("Campo 'Parte_Int' está vazio!");
        }
        ...
        if (pFldFParte_Int)
            clsW.Fields(DBSituacaoDicInfo.Parte_Int, FParte_Int, EGenericTypeFields.FString);
        if (pFldFParte_Opo)
            clsW.Fields(DBSituacaoDicInfo.Parte_Opo, FParte_Opo, EGenericTypeFields.FString);
        if (pFldFTop || ID.IsEmptyIDNumber())
            clsW.Fields(DBSituacaoDicInfo.Top, FTop, EGenericTypeFields.FBoolean);
        if (pFldFBold || ID.IsEmptyIDNumber())
            clsW.Fields(DBSituacaoDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            clsW.Fields(DBSituacaoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (pFldFQuemCad)
            clsW.Fields(DBSituacaoDicInfo.QuemCad, FQuemCad, EGenericTypeFields.FNumberNull);
        if (pFldFDtCad)
            clsW.Fields(DBSituacaoDicInfo.DtCad, FDtCad, EGenericTypeFields.FDate);
        if (pFldFQuemAtu)
            clsW.Fields(DBSituacaoDicInfo.QuemAtu, FQuemAtu, EGenericTypeFields.FNumberNull);
        if (pFldFDtAtu)
            clsW.Fields(DBSituacaoDicInfo.DtAtu, FDtAtu, EGenericTypeFields.FDate);
        if (pFldFVisto || ID.IsEmptyIDNumber())
            clsW.Fields(DBSituacaoDicInfo.Visto, FVisto, EGenericTypeFields.FBoolean);
        if (insertId != 0)
            return GravaNewId();
