...
public partial class ProCDAController(IProCDAService procdaService) : ControllerBase
{
    ...
    {
        _logger.Info("ProCDA: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _procdaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProCDA: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _procdaService.UpdateColumns(parameters, uri);
        if (!result)
        {
