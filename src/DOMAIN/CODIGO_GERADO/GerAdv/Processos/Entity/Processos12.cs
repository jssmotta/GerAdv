...
public partial class FProcessos : MenphisSI.SG.GerAdv.DBProcessos, IDBProcessos
{
    // DI-compatible default constructor
    public FProcessos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FProcessos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FProcessos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FProcessos CreateFromDataRow(DataRow? dbRec)
    {
        return new FProcessos(dbRec);
    }

    private FProcessos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FProcessos(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FProcessos> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fProcessos = new FProcessos();
        await fProcessos.CarregarAsync(id, oCnn);
        return fProcessos;
    }

    private FProcessos(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FProcessos CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FProcessos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
