...
export class TipoRecursoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class TipoRecursoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/TipoRecurso`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoRecurso_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoRecurso');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoRecurso_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipo Recurso com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterTipoRecurso): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoRecurso_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoRecurso');
        }
    }

            public async filter(filtro: FilterTipoRecurso): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoRecurso_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoRecurso');
                }
            }


    public async validation(regTipoRecurso: ITipoRecurso): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoRecurso as TipoRecurso, this.getHeaders());
    ...
    public async addAndUpdate(regTipoRecurso: ITipoRecurso): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regTipoRecurso as TipoRecurso, this.getHeaders());
            var register = result.data as ITipoRecurso;        
            const action = regTipoRecurso.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Tipo Recurso');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o TipoRecurso. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'TipoRecurso',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Tipo Recurso com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoRecurso) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<TipoRecurso[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de tiporecurso:', error);
            this.notificationService.notify({
              entity: 'TipoRecurso',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tiporecurso'
            });
          }
        });
    }
