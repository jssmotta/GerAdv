// app/TestsForms/ProTipoBaixaComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import ProTipoBaixaComboBox from '@/app/GerAdv_TS/ProTipoBaixa/ComboBox/ProTipoBaixa';
...
import { ProTipoBaixaEmpty, ProTipoBaixaTestEmpty } from '@/app/GerAdv_TS/Models/ProTipoBaixa';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/ProTipoBaixa/Hooks/hookProTipoBaixa', () => ({
  useProTipoBaixaComboBox: jest.fn(() => ({
    options: [
    { ...ProTipoBaixaEmpty(), id: 1, nome: 'Pro Tipo Baixa 1' },
    { ...ProTipoBaixaEmpty(), id: 2, nome: 'Pro Tipo Baixa 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/ProTipoBaixa/Apis/ApiProTipoBaixa', () => ({
  ProTipoBaixaApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProTipoBaixa' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProTipoBaixa' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/ProTipoBaixa/Services/ProTipoBaixa.service', () => ({
  ProTipoBaixaService: jest.fn().mockImplementation(() => ({
    fetchProTipoBaixaById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ProTipoBaixa' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/ProTipoBaixa/Crud/Grids/ProTipoBaixaWindow', () => {
  return function MockProTipoBaixaWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='protipobaixa-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Pro Tipo Baixa' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ProTipoBaixaTestEmpty(), id: 1, nome: 'Pro Tipo Baixa 1' },
  { ...ProTipoBaixaTestEmpty(), id: 2, nome: 'Pro Tipo Baixa 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Pro Tipo Baixa',
  name: 'protipobaixa',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: ProTipoBaixaEmpty(), 
};
describe('ProTipoBaixaComboBox', () => {
  ...
  const { useProTipoBaixaComboBox } = require('@/app/GerAdv_TS/ProTipoBaixa/Hooks/hookProTipoBaixa');
  (useProTipoBaixaComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ProTipoBaixaComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Pro Tipo Baixa')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Pro Tipo Baixa')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ProTipoBaixaTestEmpty(), id: 1, nome: 'Pro Tipo Baixa 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useProTipoBaixaComboBox } = require('@/app/GerAdv_TS/ProTipoBaixa/Hooks/hookProTipoBaixa');
    useProTipoBaixaComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ProTipoBaixaWindow on action button click (add)', async () => {
    render(<ProTipoBaixaComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('protipobaixa-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('protipobaixa-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ProTipoBaixaComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ProTipoBaixaWindow onClose and onSuccess', () => {
    render(<ProTipoBaixaComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ProTipoBaixaComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
