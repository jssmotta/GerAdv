...
SELECT TOP (1) 
	FORMAT(livDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(livDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    livVisto as Visto
FROM {oCnn.UseDbo}.LivroCaixa dbq WITH (INDEX = idx_LivroCaixa_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.livQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.livQuemAtu
WHERE [livCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBLivroCaixa.PTabelaNome,
...
FROM {oCnn.UseDbo}.LivroCaixa WITH (INDEX = idx_{DBLivroCaixaDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [livCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBLivroCaixaDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(livDtAtu, livDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBLivroCaixaDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(livDtAtu, livDtCad) DESC;", oCnn.InnerConnection);
