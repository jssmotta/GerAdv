...
public partial class ProPartesService(IOptions<AppSettings> appSettings, IProPartesReader reader, IProPartesValidation validation, IProPartesWriter writer, IProcessosReader processosReader, HybridCache cache) : IProPartesService, IDisposable
{
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("ProPartes: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBProPartes(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBProPartesDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("ProPartes: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBProPartes(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBProPartesDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
