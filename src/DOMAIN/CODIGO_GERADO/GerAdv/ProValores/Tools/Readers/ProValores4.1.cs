...
public partial class ProValoresReader
{
...
SELECT TOP (1) 
	FORMAT(prvDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(prvDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    prvVisto as Visto
FROM {oCnn.UseDbo}.ProValores dbq WITH (INDEX = idx_ProValores_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.prvQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.prvQuemAtu
WHERE [prvCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBProValores.PTabelaNome,
...
FROM {oCnn.UseDbo}.ProValores WITH (INDEX = idx_{DBProValoresDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [prvCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBProValoresDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(prvDtAtu, prvDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBProValoresDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(prvDtAtu, prvDtCad) DESC;", oCnn.InnerConnection);
