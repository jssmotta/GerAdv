...
SELECT TOP (1) 
	FORMAT(prsDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(prsDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    prsVisto as Visto
FROM {oCnn.UseDbo}.ProResumos dbq WITH (INDEX = idx_ProResumos_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.prsQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.prsQuemAtu
WHERE [prsCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBProResumos.PTabelaNome,
...
FROM {oCnn.UseDbo}.ProResumos WITH (INDEX = idx_{DBProResumosDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [prsCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBProResumosDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(prsDtAtu, prsDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBProResumosDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(prsDtAtu, prsDtCad) DESC;", oCnn.InnerConnection);
