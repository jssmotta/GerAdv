...
#region ListarDados_Paises
    public static async IAsyncEnumerable<DBPaises> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPaises(dbRec);
        }
    }

    public static IEnumerable<DBPaises> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
