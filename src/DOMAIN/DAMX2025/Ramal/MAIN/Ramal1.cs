...
public partial class DBRamal : VAuditor, ICadastros, IAuditor
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Ramal)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBRamal();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_Ramal
    [XmlIgnore]
    public string TabelaNome => "Ramal";
...
#endregion
    public DBRamal(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBRamal(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBRamal(in string? sqlWhere, SqlConnection? oCnn)
    {
...
#region GravarDados_Ramal
...
#endif
    {
        ...
        if (pFldFNome)
            clsW.Fields(DBRamalDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFObs)
            clsW.Fields(DBRamalDicInfo.Obs, m_FObs, ETiposCampos.FString);
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Ramal)
        if (clsW.HasUpdates)
        {
            HasChanges = true;
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (pFldFQuemCad)
            clsW.Fields(DBRamalDicInfo.QuemCad, m_FQuemCad, ETiposCampos.FNumberNull);
        if (pFldFDtCad)
            clsW.Fields(DBRamalDicInfo.DtCad, m_FDtCad, ETiposCampos.FDate);
        if (pFldFQuemAtu)
            clsW.Fields(DBRamalDicInfo.QuemAtu, m_FQuemAtu, ETiposCampos.FNumberNull);
        if (pFldFDtAtu)
            clsW.Fields(DBRamalDicInfo.DtAtu, m_FDtAtu, ETiposCampos.FDate);
        if (pFldFVisto || ID.IsEmptyIDNumber())
            clsW.Fields(DBRamalDicInfo.Visto, m_FVisto, ETiposCampos.FBoolean);
        if (insertId != 0)
            return GravaNewId();
