...
public partial class DBSituacao
{
    ...
    /// </summary>
    public static async Task<DBSituacao> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBSituacao();
...
#if (!NOTSTORED_Situacao)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFParte_Int || pFldFParte_Opo || pFldFTop || pFldFBold || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFParte_Int)
            updateTool.Fields(DBSituacaoDicInfo.Parte_Int, FParte_Int, EGenericTypeFields.FString);
        if (pFldFParte_Opo)
            updateTool.Fields(DBSituacaoDicInfo.Parte_Opo, FParte_Opo, EGenericTypeFields.FString);
        if (pFldFTop || updateTool.Insert)
            updateTool.Fields(DBSituacaoDicInfo.Top, FTop, EGenericTypeFields.FBoolean);
        if (pFldFBold || updateTool.Insert)
            updateTool.Fields(DBSituacaoDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            updateTool.Fields(DBSituacaoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBSituacaoDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBSituacaoDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBSituacaoDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBSituacaoDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBSituacaoDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBSituacaoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
