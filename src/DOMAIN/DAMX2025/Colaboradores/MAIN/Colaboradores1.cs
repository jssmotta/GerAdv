...
public partial class DBColaboradores : VSexo, ICadastrosAuditor, IAuditor, IListagemCidade
{
#if (DEBUG)
...
#region TableDefinition_Colaboradores
...
#endregion
    public DBColaboradores(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBColaboradores(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBColaboradores(in string? sqlWhere, SqlConnection? oCnn)
    {
