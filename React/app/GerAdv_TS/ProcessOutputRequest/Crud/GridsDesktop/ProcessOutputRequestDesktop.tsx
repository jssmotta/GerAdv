...
import { useProcessOutputRequestGrid } from '../../Hooks/hookProcessOutputRequestGrid';
...
const [columnsOrder, setColumnsOrder] = useState<string[]>(['nomeprocessoutputengine',
'rnomeoperador',
'nropastaprocessos']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('processoutputrequest-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('processoutputrequest-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const basicColumnMap: Record<string, React.ReactElement> = useMemo(() => ({
  'nomeprocessoutputengine': (
  <GridColumn
  key='nomeprocessoutputengine'
  field='nomeprocessoutputengine'
  title='Process Output Engine'
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('processoutputrequest-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-processoutputrequest',
  columns: exportColumns, 
  sheetName: 'ProcessOutputRequest'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-ProcessOutputRequest-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-processoutputrequest',
  columns: exportColumnsPdf, 
  title: 'ProcessOutputRequest'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-ProcessOutputRequest-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`processoutputrequest-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-processoutputrequest'
