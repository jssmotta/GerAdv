...
public static class FilterGraphTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterGraph? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Graph em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGraph:

- O filter irá filtrar os Graph conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterGraph:(Graph)
- tabela: Tabela;
- tabelaid: número ou IDs;
...
- Se ouvir "todos os Graph" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Graph length: {result.Length}");
#endif
