...
import { TipoValorProcessoApiError } from '../GerAdv_TS/TipoValorProcesso/Apis/ApiTipoValorProcesso';
import { TipoValorProcessoApi } from '@/app/GerAdv_TS/TipoValorProcesso/Apis/ApiTipoValorProcesso';
import { FilterTipoValorProcesso } from '../GerAdv_TS/TipoValorProcesso/Filters/TipoValorProcesso';
...
describe('TipoValorProcessoApi', () => {
  let tipovalorprocessoApi: TipoValorProcessoApi;
  ...
  beforeEach(() => {
    ...
    tipovalorprocessoApi = new TipoValorProcessoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipovalorprocessoApi).toBeInstanceOf(TipoValorProcessoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoValorProcessoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoValorProcessoApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [TipoValorProcessoTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all tipovalorprocesso items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipovalorprocessoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipovalorprocessoApi.getAll()).rejects.toThrow(TipoValorProcessoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipovalorprocessoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoValorProcesso_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoValorProcesso',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw TipoValorProcessoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoValorProcesso = TipoValorProcessoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoValorProcesso,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipovalorprocesso by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipovalorprocessoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoValorProcesso));
      ...
      expect(result.data).toEqual(mockTipoValorProcesso);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoValorProcesso',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoValorProcesso = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipovalorprocesso items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipovalorprocessoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoValorProcesso: ITipoValorProcesso = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipovalorprocesso successfully', async () => {
      const responseTipoValorProcesso = { ...mockTipoValorProcesso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoValorProcesso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipovalorprocessoApi.addAndUpdate(mockTipoValorProcesso);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockTipoValorProcesso,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing tipovalorprocesso successfully', async () => {
      const existingTipoValorProcesso = { ...mockTipoValorProcesso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoValorProcesso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipovalorprocessoApi.addAndUpdate(existingTipoValorProcesso);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipovalorprocessoApi.addAndUpdate(mockTipoValorProcesso)).rejects.toThrow(TipoValorProcessoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoValorProcesso',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipovalorprocesso successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipovalorprocessoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipovalorprocessoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoValorProcesso',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipovalorprocessoApi.delete(1)).rejects.toThrow(TipoValorProcessoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoValorProcesso = { funcionario: 1 };
      
      const result = tipovalorprocessoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipovalorprocessoApi.delete(1);
  ...
  describe('TipoValorProcessoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoValorProcessoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoValorProcessoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoValorProcessoApiError('Test message', 500);
