...
public partial class TiposAcaoReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBTiposAcaoDicInfo.CampoNome}";
        ...
        var query = $@"SELECT TOP ({max})
                   {campos}
                   FROM {DBTiposAcao.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(tacDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(tacDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    tacVisto as Visto
FROM {oCnn.UseDbo}.TiposAcao dbq WITH (INDEX = idx_TiposAcao_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.tacQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.tacQuemAtu
WHERE [tacCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBTiposAcao.PTabelaNome,
...
FROM {oCnn.UseDbo}.TiposAcao WITH (INDEX = idx_{DBTiposAcaoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [tacCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBTiposAcaoDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(tacDtAtu, tacDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBTiposAcaoDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(tacDtAtu, tacDtCad) DESC;", oCnn.InnerConnection);
