...
#region ListarDados_LivroCaixaClientes
    public static async IAsyncEnumerable<DBLivroCaixaClientes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBLivroCaixaClientes(dbRec);
        }
    }

    public static IEnumerable<DBLivroCaixaClientes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
