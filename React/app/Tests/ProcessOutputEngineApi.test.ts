...
import { ProcessOutputEngineApiError } from '../GerAdv_TS/ProcessOutputEngine/Apis/ApiProcessOutputEngine';
import { ProcessOutputEngineApi } from '@/app/GerAdv_TS/ProcessOutputEngine/Apis/ApiProcessOutputEngine';
import { FilterProcessOutputEngine } from '../GerAdv_TS/ProcessOutputEngine/Filters/ProcessOutputEngine';
...
describe('ProcessOutputEngineApi', () => {
  let processoutputengineApi: ProcessOutputEngineApi;
  ...
  beforeEach(() => {
    ...
    processoutputengineApi = new ProcessOutputEngineApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(processoutputengineApi).toBeInstanceOf(ProcessOutputEngineApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProcessOutputEngineApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProcessOutputEngineApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [ProcessOutputEngineTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all processoutputengine items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputengineApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(processoutputengineApi.getAll()).rejects.toThrow(ProcessOutputEngineApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await processoutputengineApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProcessOutputEngine_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputEngine',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw ProcessOutputEngineApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProcessOutputEngine = ProcessOutputEngineTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProcessOutputEngine,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch processoutputengine by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await processoutputengineApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProcessOutputEngine));
      ...
      expect(result.data).toEqual(mockProcessOutputEngine);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputEngine',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProcessOutputEngine = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter processoutputengine items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await processoutputengineApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProcessOutputEngine: IProcessOutputEngine = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new processoutputengine successfully', async () => {
      const responseProcessOutputEngine = { ...mockProcessOutputEngine, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProcessOutputEngine,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputengineApi.addAndUpdate(mockProcessOutputEngine);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockProcessOutputEngine,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing processoutputengine successfully', async () => {
      const existingProcessOutputEngine = { ...mockProcessOutputEngine, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProcessOutputEngine,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await processoutputengineApi.addAndUpdate(existingProcessOutputEngine);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(processoutputengineApi.addAndUpdate(mockProcessOutputEngine)).rejects.toThrow(ProcessOutputEngineApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputEngine',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete processoutputengine successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await processoutputengineApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(processoutputengineApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProcessOutputEngine',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(processoutputengineApi.delete(1)).rejects.toThrow(ProcessOutputEngineApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProcessOutputEngine = { funcionario: 1 };
      
      const result = processoutputengineApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await processoutputengineApi.delete(1);
  ...
  describe('ProcessOutputEngineApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProcessOutputEngineApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProcessOutputEngineApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProcessOutputEngineApiError('Test message', 500);
