...
#region Test Data Setup Helpers
    ...
    private void SetupMockFContaCorrente(int? CIAcordo = 1, bool? Quitado = false, int? IDContrato = 1, int? QuitadoID = 1, int? DebitoID = 1, int? LivroCaixaID = 1, bool? Sucumbencia = true, bool? DistRegra = false, string? DtOriginal = "24/04/1975", int? Processo = 1, int? ParcelaX = 1, decimal? Valor = 1m, string? Data = "24/04/1975", int? Cliente = 1, string? Historico = "", bool? Contrato = true, bool? Pago = false, bool? Distribuir = true, bool? LC = false, int? IDHTrab = 1, int? NroParcelas = 1, decimal? ValorPrincipal = 1m, int? ParcelaPrincipalID = 1, bool? Hide = true, string? DataPgto = "24/04/1975")
    {
        ...
        {
            _mockFContaCorrente.Setup(f => f.FDtOriginal).Returns((DateOnly? )null);
        }
        else
        {
            DateTime parsedDateTime;
            if (!DateTime.TryParse(DtOriginal, new CultureInfo("pt-BR"), DateTimeStyles.None, out parsedDateTime))
            {
                parsedDateTime = DateTime.Parse(DtOriginal, CultureInfo.InvariantCulture);
            }

            var dateOnly = DateOnly.FromDateTime(parsedDateTime);
            _mockFContaCorrente.Setup(f => f.FDtOriginal).Returns(dateOnly);
        }
        ...
        {
            _mockFContaCorrente.Setup(f => f.FData).Returns((DateOnly? )null);
        }
        else
        {
            DateTime parsedDateTime;
            ...
            var dateOnly = DateOnly.FromDateTime(parsedDateTime);
            _mockFContaCorrente.Setup(f => f.FData).Returns(dateOnly);
        }
        ...
        {
            _mockFContaCorrente.Setup(f => f.FDataPgto).Returns((DateOnly? )null);
        }
        else
        {
            DateTime parsedDateTime;
            if (!DateTime.TryParse(DataPgto, new CultureInfo("pt-BR"), DateTimeStyles.None, out parsedDateTime))
            {
                parsedDateTime = DateTime.Parse(DataPgto, CultureInfo.InvariantCulture);
            }

            var dateOnly = DateOnly.FromDateTime(parsedDateTime);
            _mockFContaCorrente.Setup(f => f.FDataPgto).Returns(dateOnly);
        }
    }
...
#region Integration Tests
    ...
    public void Read_ShouldPassCorrectParametersToFactory()
    {
        ...
        var parameters = new List<SqlParameter>
        {
            new SqlParameter($"@{DBContaCorrenteDicInfo.CampoNome}", TConstantes.TestData),
        };
        SetupMockFContaCorrente();
        _mockContaCorrenteFactory.Setup(f => f.CreateFromParameters(parameters, _mockConnection.Object, "", "", where, "")).Returns(_mockFContaCorrente.Object);
        // Act
...
#region DateTime Tests
    ...
    public void Read_WithValidDate_DtOriginal_Formats_ShouldParseCorrectly(string dateString)
    {
        // Arrange
        var where = "Id = @Id";
        var parameters = CreateTestParameters();
        ...
        var result = _contacorrenteWhere.Read(where, parameters, _mockConnection.Object);
        // Assert
        result.DtOriginal.Should().Be(expectedDate.ToString("dd/MM/yyyy"));
    }
    ...
    public void Read_WithValidDate_DataPgto_Formats_ShouldParseCorrectly(string dateString)
    {
        // Arrange
        var where = "Id = @Id";
        var parameters = CreateTestParameters();
        ...
        var result = _contacorrenteWhere.Read(where, parameters, _mockConnection.Object);
        // Assert
        result.DataPgto.Should().Be(expectedDate.ToString("dd/MM/yyyy"));
    }
#endregion
#endregion
}