  ...
  const loadCidade = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Cidade";
...
export const useCidadeList = (
  dataService: ICidadeService,
  currentFilter?: FilterCidade | undefined | null,
) => {
  const [data, setData] = useState<ICidade[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterCidade | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: ICidade): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.sigla.length > 10) {
        return {
          isValid: false,
          message: "O campo Sigla não pode ter mais de 10 caracteres.",
        };
      }
