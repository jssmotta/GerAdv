...
const StatusTarefasComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IStatusTarefas | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IStatusTarefas | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IStatusTarefas);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <StatusTarefasWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedStatusTarefas={editar ?? addValue ?? StatusTarefasEmpty()} /> 
            )}
