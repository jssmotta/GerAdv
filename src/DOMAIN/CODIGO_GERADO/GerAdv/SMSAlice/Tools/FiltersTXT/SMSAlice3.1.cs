...
public static class FilterSMSAliceTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar SMSAlice em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterSMSAlice:

- O filter irá filtrar os SMSAlice conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterTipoEMail: filtro específico para Tipo E Mail
...
CAMPOS filterTipoEMail (Tipo E Mail):
- nome: _nome ou descrição para Tipo E Mail;

CAMPOS filterSMSAlice:(S M S Alice)
- operador: número ou IDs;
- nome: _nome ou descrição para S M S Alice;
- tipoemail: número ou IDs;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
- quemcad, quemcad_end: ID do operador que cadastrou
...
- Se ouvir "todos os SMSAlice" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterSMSAlice? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt SMSAlice length: {result.Length}");
#endif
