// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Tabela:Prepostos
// Fonte:SGDomainTests
/// <summary>
/// Template de testes unitários para classes geradas pelo Source Genesys
/// Variáveis de substituição:
/// MenphisSI.GerAdv - Namespace da classe
/// DBPrepostos - Nome da classe (ex: DBClientes)
/// Prepostos - Nome da tabela (ex: Clientes)
/// pre - Prefixo dos campos (ex: cli)
/// preCodigo - Nome do campo código (ex: cliCodigo)
/// preNome - Nome do campo nome (ex: cliNome)
/// DBPrepostos.CadastroGuid - GUID do cadastro
/// </summary>
namespace MenphisSI.GerAdv.Tests;
public class DBPrepostosTests : IDisposable
{
    private readonly DBPrepostos _instance;
    private readonly Mock<MsiSqlConnection> _mockConnection;
    private readonly DataTable _testDataTable;
    public DBPrepostosTests()
    {
        _instance = new DBPrepostos();
        _mockConnection = new Mock<MsiSqlConnection>();
        _testDataTable = CreateTestDataTable();
    }

    private DataTable CreateTestDataTable()
    {
        var dt = new DataTable();
        // Campos obrigatórios Source Genesys
        dt.Columns.Add("preCodigo", typeof(int));
        dt.Columns.Add("preQuemCad", typeof(int));
        dt.Columns.Add("preDtCad", typeof(DateTime));
        dt.Columns.Add("preQuemAtu", typeof(int));
        dt.Columns.Add("preDtAtu", typeof(DateTime));
        dt.Columns.Add("preVisto", typeof(bool));
        dt.Columns.Add("preNome", typeof(string));
        dt.Columns.Add("preFuncao", typeof(int));
        dt.Columns.Add("preSetor", typeof(int));
        dt.Columns.Add("preDtNasc", typeof(DateTime));
        dt.Columns.Add("preQualificacao", typeof(string));
        dt.Columns.Add("preSexo", typeof(string));
        dt.Columns.Add("preIdade", typeof(int));
        dt.Columns.Add("preCPF", typeof(string));
        dt.Columns.Add("preRG", typeof(string));
        dt.Columns.Add("prePeriodo_Ini", typeof(DateTime));
        dt.Columns.Add("prePeriodo_Fim", typeof(DateTime));
        dt.Columns.Add("preRegistro", typeof(string));
        dt.Columns.Add("preCTPSNumero", typeof(string));
        dt.Columns.Add("preCTPSSerie", typeof(string));
        dt.Columns.Add("preCTPSDtEmissao", typeof(DateTime));
        dt.Columns.Add("prePIS", typeof(string));
        dt.Columns.Add("preSalario", typeof(decimal));
        dt.Columns.Add("preLiberaAgenda", typeof(string));
        dt.Columns.Add("preObservacao", typeof(string));
        dt.Columns.Add("preEndereco", typeof(string));
        dt.Columns.Add("preBairro", typeof(string));
        dt.Columns.Add("preCidade", typeof(int));
        dt.Columns.Add("preCEP", typeof(string));
        dt.Columns.Add("preFone", typeof(string));
        dt.Columns.Add("preFax", typeof(string));
        dt.Columns.Add("preEMail", typeof(string));
        dt.Columns.Add("prePai", typeof(string));
        dt.Columns.Add("preMae", typeof(string));
        dt.Columns.Add("preClass", typeof(string));
        dt.Columns.Add("preEtiqueta", typeof(string));
        dt.Columns.Add("preAni", typeof(string));
        dt.Columns.Add("preBold", typeof(string));
        dt.Columns.Add("preGuid", typeof(string));
        return dt;
    }

    [Fact]
    public void Constructor_WithValidDataRow_ShouldLoadData()
    {
        // Arrange
        var row = _testDataTable.NewRow();
        row["preCodigo"] = 123;
        _testDataTable.Rows.Add(row);
        // Act
        var instance = new DBPrepostos(_testDataTable.Rows[0]);
        // Assert
        Assert.Equal(123, instance.ID);
    }

#region Testes de Constantes e Propriedades Estáticas
    [Fact]
    public void Constants_ShouldHaveCorrectValues()
    {
        Assert.Equal("Prepostos", DBPrepostos.PTabelaNome);
        Assert.Equal(" Prepostos.* ", DBPrepostos.CamposSqlX);
        Assert.Equal("preCodigo", DBPrepostos.CampoCodigo);
        Assert.Equal("preNome", DBPrepostos.CampoNome);
        Assert.Equal("pre", DBPrepostos.PTabelaPrefixo);
    }

#endregion
#region Testes de Construtores
    [Fact]
    public void Constructor_Default_ShouldInitializeCorrectly()
    {
        var instance = new DBPrepostos();
        Assert.Equal(0, instance.ID);
        Assert.Equal("Prepostos", instance.ITableName());
        Assert.Equal("pre", instance.Prefixo);
    }

    [Fact]
    public void Constructor_WithNullDataRow_ShouldNotThrow()
    {
        var instance = new DBPrepostos((DataRow)null !);
        Assert.Equal(0, instance.ID);
    }

    [Fact]
    public void Constructor_WithNullSqlDataReader_ShouldNotThrow()
    {
        var instance = new DBPrepostos((SqlDataReader)null !);
        Assert.Equal(0, instance.ID);
    }

#endregion
#region Testes de Interfaces
    [Fact]
    public void ICrud_Implementation_ShouldWork()
    {
        ICrud cadastro = (ICrud)_instance;
        Assert.Equal("Prepostos", cadastro.ITableName());
        Assert.Equal("preCodigo", cadastro.IFieldId());
        Assert.Equal("preNome", cadastro.IFieldNameDescription());
        Assert.Equal("pre", cadastro.IPrefix());
    }

#endregion
#region Testes de Métodos Assíncronos
    [Fact]
    public async Task CreateAsync_WithInvalidId_ShouldReturnEmptyInstance()
    {
        var instance = await DBPrepostos.CreateAsync(0, null);
        Assert.Equal(0, instance.ID);
    }

    [Fact]
    public async Task ExisteAsync_WithNullConnection_ShouldReturnFalse()
    {
        var exists = await DBPrepostos.ExisteAsync(1, null);
        Assert.False(exists);
    }

    [Fact]
    public async Task DeleteRecordAsync_WithInvalidId_ShouldReturnFalse()
    {
        var result = await DBPrepostos.DeleteRecordAsync(1, 0, null);
        Assert.False(result);
    }

#endregion
#region Testes de Dispose
    [Fact]
    public void Dispose_ShouldNotThrow()
    {
        var instance = new DBPrepostos();
        var exception = Record.Exception(() => instance.Dispose());
        Assert.Null(exception);
    }

    [Fact]
    public void Dispose_MultipleCalls_ShouldBeIdempotent()
    {
        var instance = new DBPrepostos();
        instance.Dispose();
        var exception = Record.Exception(() => instance.Dispose());
        Assert.Null(exception);
    }

#endregion
#region Testes de Propriedades Computadas
    [Fact]
    public void HasAuditor_ShouldReturnTrue()
    {
        Assert.True(_instance.HasAuditor());
    }

    [Fact]
    public void HasNameId_ShouldReturnTrue()
    {
        Assert.True(_instance.HasNameId());
    }

    [Fact]
    public void IsStoredProcedureOrView_ShouldReturnFalse()
    {
        Assert.False(_instance.IsStoredProcedureOrView());
    }

#endregion
    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Nome_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FNome = input;
        Assert.Equal(expected, _instance.FNome);
    }

    [Fact]
    public void Nome_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 80 + 10);
        _instance.FNome = longString;
        Assert.True(_instance.FNome.Length <= 80);
    }

    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    [InlineData(-1)]
    [InlineData(int.MaxValue)]
    [InlineData(int.MinValue)]
    public void Funcao_ShouldAcceptIntegerValues(int value)
    {
        _instance.FFuncao = value;
        Assert.Equal(value, _instance.FFuncao);
    }

    [Fact]
    public void Funcao_DefaultValue_ShouldBeZero()
    {
        var instance = new DBPrepostos();
        Assert.Equal(0, instance.FFuncao);
    }

    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    [InlineData(-1)]
    [InlineData(int.MaxValue)]
    [InlineData(int.MinValue)]
    public void Setor_ShouldAcceptIntegerValues(int value)
    {
        _instance.FSetor = value;
        Assert.Equal(value, _instance.FSetor);
    }

    [Fact]
    public void Setor_DefaultValue_ShouldBeZero()
    {
        var instance = new DBPrepostos();
        Assert.Equal(0, instance.FSetor);
    }

    [Theory]
    [InlineData("01/01/2000")]
    [InlineData("31/12/2023")]
    [InlineData("15/08/2024")]
    public void DtNasc_ShouldFormatDateCorrectly(string dateString)
    {
        _instance.FDtNasc = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FDtNasc?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }

    [Fact]
    public void DtNasc_EmptyDate_ShouldReturnEmptyString()
    {
        var instance = new DBPrepostos();
        Assert.Equal(null !, instance.FDtNasc);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Qualificacao_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FQualificacao = input;
        Assert.Equal(expected, _instance.FQualificacao);
    }

    [Fact]
    public void Qualificacao_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 100 + 10);
        _instance.FQualificacao = longString;
        Assert.True(_instance.FQualificacao.Length <= 100);
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void Sexo_ShouldAcceptBooleanValues(bool value)
    {
        _instance.FSexo = value;
        Assert.Equal(value, _instance.FSexo);
    }

    [Fact]
    public void Sexo_DefaultValue_ShouldBeFalse()
    {
        var instance = new DBPrepostos();
        Assert.False(instance.FSexo);
    }

    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    [InlineData(-1)]
    [InlineData(int.MaxValue)]
    [InlineData(int.MinValue)]
    public void Idade_ShouldAcceptIntegerValues(int value)
    {
        _instance.FIdade = value;
        Assert.Equal(value, _instance.FIdade);
    }

    [Fact]
    public void Idade_DefaultValue_ShouldBeZero()
    {
        var instance = new DBPrepostos();
        Assert.Equal(0, instance.FIdade);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void RG_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FRG = input;
        Assert.Equal(expected, _instance.FRG);
    }

    [Fact]
    public void RG_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 30 + 10);
        _instance.FRG = longString;
        Assert.True(_instance.FRG.Length <= 30);
    }

    [Theory]
    [InlineData("01/01/2000")]
    [InlineData("31/12/2023")]
    [InlineData("15/08/2024")]
    public void Periodo_Ini_ShouldFormatDateCorrectly(string dateString)
    {
        _instance.FPeriodo_Ini = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FPeriodo_Ini?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }

    [Fact]
    public void Periodo_Ini_EmptyDate_ShouldReturnEmptyString()
    {
        var instance = new DBPrepostos();
        Assert.Equal(null !, instance.FPeriodo_Ini);
    }

    [Theory]
    [InlineData("01/01/2000")]
    [InlineData("31/12/2023")]
    [InlineData("15/08/2024")]
    public void Periodo_Fim_ShouldFormatDateCorrectly(string dateString)
    {
        _instance.FPeriodo_Fim = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FPeriodo_Fim?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }

    [Fact]
    public void Periodo_Fim_EmptyDate_ShouldReturnEmptyString()
    {
        var instance = new DBPrepostos();
        Assert.Equal(null !, instance.FPeriodo_Fim);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Registro_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FRegistro = input;
        Assert.Equal(expected, _instance.FRegistro);
    }

    [Fact]
    public void Registro_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 30 + 10);
        _instance.FRegistro = longString;
        Assert.True(_instance.FRegistro.Length <= 30);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void CTPSNumero_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FCTPSNumero = input;
        Assert.Equal(expected, _instance.FCTPSNumero);
    }

    [Fact]
    public void CTPSNumero_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 15 + 10);
        _instance.FCTPSNumero = longString;
        Assert.True(_instance.FCTPSNumero.Length <= 15);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void CTPSSerie_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FCTPSSerie = input;
        Assert.Equal(expected, _instance.FCTPSSerie);
    }

    [Fact]
    public void CTPSSerie_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 10 + 10);
        _instance.FCTPSSerie = longString;
        Assert.True(_instance.FCTPSSerie.Length <= 10);
    }

    [Theory]
    [InlineData("01/01/2000")]
    [InlineData("31/12/2023")]
    [InlineData("15/08/2024")]
    public void CTPSDtEmissao_ShouldFormatDateCorrectly(string dateString)
    {
        _instance.FCTPSDtEmissao = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FCTPSDtEmissao?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }

    [Fact]
    public void CTPSDtEmissao_EmptyDate_ShouldReturnEmptyString()
    {
        var instance = new DBPrepostos();
        Assert.Equal(null !, instance.FCTPSDtEmissao);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void PIS_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FPIS = input;
        Assert.Equal(expected, _instance.FPIS);
    }

    [Fact]
    public void PIS_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 20 + 10);
        _instance.FPIS = longString;
        Assert.True(_instance.FPIS.Length <= 20);
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void LiberaAgenda_ShouldAcceptBooleanValues(bool value)
    {
        _instance.FLiberaAgenda = value;
        Assert.Equal(value, _instance.FLiberaAgenda);
    }

    [Fact]
    public void LiberaAgenda_DefaultValue_ShouldBeFalse()
    {
        var instance = new DBPrepostos();
        Assert.False(instance.FLiberaAgenda);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Observacao_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FObservacao = input;
        Assert.Equal(expected, _instance.FObservacao);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Endereco_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FEndereco = input;
        Assert.Equal(expected, _instance.FEndereco);
    }

    [Fact]
    public void Endereco_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 80 + 10);
        _instance.FEndereco = longString;
        Assert.True(_instance.FEndereco.Length <= 80);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Bairro_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FBairro = input;
        Assert.Equal(expected, _instance.FBairro);
    }

    [Fact]
    public void Bairro_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 50 + 10);
        _instance.FBairro = longString;
        Assert.True(_instance.FBairro.Length <= 50);
    }

    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    [InlineData(-1)]
    [InlineData(int.MaxValue)]
    [InlineData(int.MinValue)]
    public void Cidade_ShouldAcceptIntegerValues(int value)
    {
        _instance.FCidade = value;
        Assert.Equal(value, _instance.FCidade);
    }

    [Fact]
    public void Cidade_DefaultValue_ShouldBeZero()
    {
        var instance = new DBPrepostos();
        Assert.Equal(0, instance.FCidade);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Fone_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FFone = input;
        Assert.Equal(expected, _instance.FFone);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Fax_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FFax = input;
        Assert.Equal(expected, _instance.FFax);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Pai_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FPai = input;
        Assert.Equal(expected, _instance.FPai);
    }

    [Fact]
    public void Pai_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 50 + 10);
        _instance.FPai = longString;
        Assert.True(_instance.FPai.Length <= 50);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData("  Teste  ", "Teste")]
    public void Mae_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FMae = input;
        Assert.Equal(expected, _instance.FMae);
    }

    [Fact]
    public void Mae_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 50 + 10);
        _instance.FMae = longString;
        Assert.True(_instance.FMae.Length <= 50);
    }

    [Theory]
    [InlineData("", "")]
    [InlineData(null, "")]
    [InlineData(" A", "A")]
    public void Class_ShouldTrimAndHandleNulls(string? input, string expected)
    {
        _instance.FClass = input;
        Assert.Equal(expected, _instance.FClass);
    }

    [Fact]
    public void Class_ShouldRespectMaxLength()
    {
        var longString = new string ('A', 1 + 10);
        _instance.FClass = longString;
        Assert.True(_instance.FClass.Length <= 1);
    }

    public virtual void Dispose()
    {
        _instance?.Dispose();
    }
}