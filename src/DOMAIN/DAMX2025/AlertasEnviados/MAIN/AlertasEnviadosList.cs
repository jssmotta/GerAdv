...
#region ListarDados_AlertasEnviados
    public static async IAsyncEnumerable<DBAlertasEnviados> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAlertasEnviados(dbRec);
        }
    }

    public static IEnumerable<DBAlertasEnviados> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
