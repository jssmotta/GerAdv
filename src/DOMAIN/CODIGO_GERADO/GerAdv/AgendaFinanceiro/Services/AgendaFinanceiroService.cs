...
public partial class AgendaFinanceiroService(IOptions<AppSettings> appSettings, IAgendaFinanceiroReader reader, IAgendaFinanceiroValidation validation, IAgendaFinanceiroWriter writer, IAdvogadosReader advogadosReader, IFuncionariosReader funcionariosReader, ITipoCompromissoReader tipocompromissoReader, IClientesReader clientesReader, IAreaReader areaReader, IJusticaReader justicaReader, IProcessosReader processosReader, IOperadorReader operadorReader, IPrepostosReader prepostosReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IAgendaFinanceiroService, IDisposable
{
    ...
    public async Task<AgendaFinanceiroResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-AgendaFinanceiro-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AgendaFinanceiro: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAgendaFinanceiro(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBAgendaFinanceiroDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AgendaFinanceiro: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAgendaFinanceiro(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBAgendaFinanceiroDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
