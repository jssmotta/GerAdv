...
#region ListarDados_PontoVirtual
    public static async IAsyncEnumerable<DBPontoVirtual> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPontoVirtual(dbRec);
        }
    }

    public static IEnumerable<DBPontoVirtual> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
