...
import { useAcaoComboBox } from '../Hooks/hookAcao';
const AcaoComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useAcaoComboBox(acaoService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IAcao | null>(null);
const [addRecord, setAddRecord] = useState<IAcao | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await acaoService.fetchAcaoById(id);
...
const handleSuccess = (newAcao?: IAcao) => {
  if (newAcao) {
    setValue({ id: newAcao.id, nome: newAcao.descricao });
    loadRecordForEdit(newAcao.id);
    handleValueChange(newAcao);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .acao-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .acao-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .acao-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedAcao={editRecord || addRecord || AcaoEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'acao-dropdown-popup-msi'
          }}
          />
