...
export class HistoricoApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        public async getListN(max?: number, filtro?: FilterHistorico): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Historico_last_listN_data`);
        ...
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Historico');
        }
    }
 public async filterPreload(filtro: FilterHistorico): Promise<AxiosResponse> {
    ...
    public useFilter(filtro: FilterHistorico) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Historico[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de Historico');
            this.notificationService.notify({
              entity: 'Historico',
              id: 0,
              action: NotifySystemActions.ERROR,
