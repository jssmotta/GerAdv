...
export const useContaCorrenteForm = (
  ...
  const loadContaCorrente = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Conta Corrente";
...
export const useContaCorrenteList = (
  dataService: IContaCorrenteService,
  currentFilter?: FilterContaCorrente | undefined | null,
) => {
  const [data, setData] = useState<IContaCorrente[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterContaCorrente | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterContaCorrente | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar contacorrente";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsContaCorrente() {
  ...
  function validate(data: IContaCorrente): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.historico.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Historico não pode ter mais de 2147483647 caracteres.",
        };
      }
