...
public static class FilterNEPalavrasChavesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar NEPalavrasChaves em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterNEPalavrasChaves:

- O filter irá filtrar os NEPalavrasChaves conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterNEPalavrasChaves:(N E Palavras Chaves)
- nome: _nome ou descrição para N E Palavras Chaves;
- bold: Bold;
...
- Se ouvir "todos os NEPalavrasChaves" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterNEPalavrasChaves? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt NEPalavrasChaves length: {result.Length}");
#endif
