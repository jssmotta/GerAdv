...
public class FEventoPrazoAgendaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FEventoPrazoAgendaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FEventoPrazoAgendaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FEventoPrazoAgendaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "epaCodigo")
        var dt = new DataTable();
        dt.Columns.Add("epaCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["epaCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FEventoPrazoAgenda>();
    }
    ...
    {
        // Arrange
        var factory = new FEventoPrazoAgendaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("epaCodigo", typeof(int));
        mockDataTable.Columns.Add("epaNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["epaCodigo"] = testId;
        row["epaNome"] = "Test EventoPrazoAgenda";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["epaCodigo"] = 123;
        row["epaNome"] = "Test EventoPrazoAgenda Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FEventoPrazoAgenda properties
        }
    }
