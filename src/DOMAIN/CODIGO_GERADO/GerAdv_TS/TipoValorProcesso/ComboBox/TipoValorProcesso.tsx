...
const TipoValorProcessoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoValorProcesso | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoValorProcesso | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoValorProcesso);
    ...
    const handleAddClick = () => {
        ...
        const add = TipoValorProcessoEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoValorProcessoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoValorProcesso={editar ?? addValue ?? TipoValorProcessoEmpty()} /> 
            )}
