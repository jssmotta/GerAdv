...
public partial class EventoPrazoAgendaController(IEventoPrazoAgendaService eventoprazoagendaService) : ControllerBase
{
    ...
    {
        _logger.Info("EventoPrazoAgenda: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _eventoprazoagendaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("EventoPrazoAgenda: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _eventoprazoagendaService.UpdateColumns(parameters, uri);
        if (!result)
        {
