...
import { AlarmSMS } from '../../Models/AlarmSMS';
...
export class AlarmSMSApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class AlarmSMSApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AlarmSMS`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlarmSMS_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AlarmSMS');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlarmSMS_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Alarm S M S com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterAlarmSMS): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlarmSMS_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de AlarmSMS');
        }
    }

            public async filter(filtro: FilterAlarmSMS): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AlarmSMS_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AlarmSMS');
                }
            }


    public async validation(regAlarmSMS: IAlarmSMS): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAlarmSMS as AlarmSMS, this.getHeaders());
    ...
    public async addAndUpdate(regAlarmSMS: IAlarmSMS): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regAlarmSMS as AlarmSMS, this.getHeaders());
            var register = result.data as IAlarmSMS;        
            const action = regAlarmSMS.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Alarm S M S');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AlarmSMS. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AlarmSMS',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Alarm S M S com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAlarmSMS) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<AlarmSMS[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de alarmsms:', error);
            this.notificationService.notify({
              entity: 'AlarmSMS',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos alarmsms'
            });
          }
        });
    }
