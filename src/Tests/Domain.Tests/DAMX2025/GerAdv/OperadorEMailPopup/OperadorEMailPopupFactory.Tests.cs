...
public class FOperadorEMailPopupFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FOperadorEMailPopupFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FOperadorEMailPopupFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FOperadorEMailPopupFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "oepCodigo")
        var dt = new DataTable();
        dt.Columns.Add("oepCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["oepCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FOperadorEMailPopup>();
    }
    ...
    {
        // Arrange
        var factory = new FOperadorEMailPopupFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("oepCodigo", typeof(int));
        mockDataTable.Columns.Add("oepNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["oepCodigo"] = testId;
        row["oepNome"] = "Test OperadorEMailPopup";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["oepCodigo"] = 123;
        row["oepNome"] = "Test OperadorEMailPopup Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FOperadorEMailPopup properties
        }
    }
