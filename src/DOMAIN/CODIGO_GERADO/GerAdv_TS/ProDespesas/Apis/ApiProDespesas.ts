...
import { ProDespesas } from '../../Models/ProDespesas';
...
export class ProDespesasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class ProDespesasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProDespesas`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProDespesas_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProDespesas');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProDespesas_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Pro Despesas com ID ${id}`);
        }
    }

            public async filter(filtro: FilterProDespesas): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProDespesas_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProDespesas');
                }
            }


    public async validation(regProDespesas: IProDespesas): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProDespesas as ProDespesas, this.getHeaders());
    ...
    public async addAndUpdate(regProDespesas: IProDespesas): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProDespesas as ProDespesas, this.getHeaders());
            var register = result.data as IProDespesas;        
            const action = regProDespesas.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Pro Despesas');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProDespesas. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProDespesas',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Pro Despesas com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProDespesas) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProDespesas[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de prodespesas:', error);
            this.notificationService.notify({
              entity: 'ProDespesas',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos prodespesas'
            });
          }
        });
    }
