...
import { InstanciaApiError } from '../GerAdv_TS/Instancia/Apis/ApiInstancia';
import { InstanciaApi } from '@/app/GerAdv_TS/Instancia/Apis/ApiInstancia';
import { FilterInstancia } from '../GerAdv_TS/Instancia/Filters/Instancia';
...
describe('InstanciaApi', () => {
  ...
  beforeEach(() => {
    ...
    instanciaApi = new InstanciaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(instanciaApi).toBeInstanceOf(InstanciaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new InstanciaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(InstanciaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all instancia items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await instanciaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(instanciaApi.getAll()).rejects.toThrow(InstanciaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await instanciaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Instancia_last_getAll_${200}`)));
    });
    ...
    it('should throw InstanciaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockInstancia = InstanciaTestEmpty();
    ...
    it('should fetch instancia by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await instanciaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockInstancia));
      ...
      expect(result.data).toEqual(mockInstancia);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Instancia',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterInstancia = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter instancia items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await instanciaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockInstancia: IInstancia = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new instancia successfully', async () => {
      const responseInstancia = { ...mockInstancia, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await instanciaApi.addAndUpdate(mockInstancia);
    ...
    it('should update existing instancia successfully', async () => {
      const existingInstancia = { ...mockInstancia, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingInstancia,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await instanciaApi.addAndUpdate(existingInstancia);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(instanciaApi.addAndUpdate(mockInstancia)).rejects.toThrow(InstanciaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Instancia',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete instancia successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await instanciaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(instanciaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Instancia',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(instanciaApi.delete(1)).rejects.toThrow(InstanciaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterInstancia = { funcionario: 1 };
      
      const result = instanciaApi.useFilter(mockFilter);
  ...
  describe('InstanciaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new InstanciaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('InstanciaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new InstanciaApiError('Test message', 500);
