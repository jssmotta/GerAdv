...
//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBOperadorEMailPopup();
...
#region TableDefinition_OperadorEMailPopup
...
#endregion
    public DBOperadorEMailPopup(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBOperadorEMailPopup(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBOperadorEMailPopup(in string? sqlWhere, SqlConnection? oCnn)
    {
