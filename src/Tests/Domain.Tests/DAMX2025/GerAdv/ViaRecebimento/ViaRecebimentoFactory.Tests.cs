...
public class FViaRecebimentoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FViaRecebimentoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FViaRecebimentoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FViaRecebimentoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "vrbCodigo")
        var dt = new DataTable();
        dt.Columns.Add("vrbCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["vrbCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FViaRecebimento>();
    }
    ...
    {
        // Arrange
        var factory = new FViaRecebimentoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("vrbCodigo", typeof(int));
        mockDataTable.Columns.Add("vrbNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["vrbCodigo"] = testId;
        row["vrbNome"] = "Test ViaRecebimento";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["vrbCodigo"] = 123;
        row["vrbNome"] = "Test ViaRecebimento Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FViaRecebimento properties
        }
    }
