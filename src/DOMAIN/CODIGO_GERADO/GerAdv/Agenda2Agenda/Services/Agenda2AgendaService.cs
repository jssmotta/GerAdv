...
public partial class Agenda2AgendaService(IOptions<AppSettings> appSettings, IFAgenda2AgendaFactory agenda2agendaFactory, IAgenda2AgendaReader reader, IAgenda2AgendaValidation validation, IAgenda2AgendaWriter writer, IAgendaReader agendaReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IAgenda2AgendaService, IDisposable
{
    ...
    private readonly IFAgenda2AgendaFactory agenda2agendaFactory = agenda2agendaFactory;
    private readonly IAgenda2AgendaReader reader = reader;
    private readonly IAgenda2AgendaValidation validation = validation;
    private readonly IAgenda2AgendaWriter writer = writer;
    ...
    public async Task<IEnumerable<Agenda2AgendaResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-Agenda2Agenda-Filter-{keyCache}";
    ...
    private async Task<Agenda2AgendaResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<Agenda2AgendaResponse?> AddAndUpdate([FromBody] Models.Agenda2Agenda regAgenda2Agenda, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regAgenda2Agenda, operadorId, oCnn);
    ...
    public async Task<Agenda2AgendaResponse?> Validation([FromBody] Models.Agenda2Agenda regAgenda2Agenda, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regAgenda2Agenda.Id, oCnn);
    }
    ...
    public async Task<Agenda2AgendaResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var agenda2agenda = await reader.Read(id, oCnn);
        try
        {
            if (agenda2agenda != null)
            {
                await writer.Delete(agenda2agenda, nOperador, oCnn);
