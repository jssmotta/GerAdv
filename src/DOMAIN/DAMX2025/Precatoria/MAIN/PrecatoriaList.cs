...
#region ListarDados_Precatoria
    public static async IAsyncEnumerable<DBPrecatoria> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPrecatoria(dbRec);
        }
    }

    public static IEnumerable<DBPrecatoria> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
