...
public static class FilterNECompromissosTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterNECompromissos? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar NECompromissos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterNECompromissos:

- O filter irá filtrar os NECompromissos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterNECompromissos:(N E Compromissos)
- palavrachave: número ou IDs;
...
- Se ouvir "todos os NECompromissos" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt NECompromissos length: {result.Length}");
#endif
