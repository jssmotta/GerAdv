...
export const useTribunalForm = (
  ...
  const loadTribunal = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Tribunal";
...
export const useTribunalList = (
  dataService: ITribunalService,
  currentFilter?: FilterTribunal | undefined | null,
) => {
  const [data, setData] = useState<ITribunal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTribunal | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: ITribunal): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.sigla.length > 20) {
        return {
          isValid: false,
          message: "O campo Sigla não pode ter mais de 20 caracteres.",
        };
      }
