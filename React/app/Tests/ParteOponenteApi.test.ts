...
import { ParteOponenteApiError } from '../GerAdv_TS/ParteOponente/Apis/ApiParteOponente';
import { ParteOponenteApi } from '@/app/GerAdv_TS/ParteOponente/Apis/ApiParteOponente';
import { FilterParteOponente } from '../GerAdv_TS/ParteOponente/Filters/ParteOponente';
...
describe('ParteOponenteApi', () => {
  let parteoponenteApi: ParteOponenteApi;
  ...
  beforeEach(() => {
    ...
    parteoponenteApi = new ParteOponenteApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(parteoponenteApi).toBeInstanceOf(ParteOponenteApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ParteOponenteApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ParteOponenteApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all parteoponente items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteoponenteApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(parteoponenteApi.getAll()).rejects.toThrow(ParteOponenteApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await parteoponenteApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ParteOponente_last_getAll_${200}`)));
    });
    ...
    it('should throw ParteOponenteApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockParteOponente = ParteOponenteTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockParteOponente,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch parteoponente by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteoponenteApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockParteOponente));
      ...
      expect(result.data).toEqual(mockParteOponente);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteOponente',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterParteOponente = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter parteoponente items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await parteoponenteApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockParteOponente: IParteOponente = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new parteoponente successfully', async () => {
      const responseParteOponente = { ...mockParteOponente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseParteOponente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteoponenteApi.addAndUpdate(mockParteOponente);
    ...
    it('should update existing parteoponente successfully', async () => {
      const existingParteOponente = { ...mockParteOponente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingParteOponente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteoponenteApi.addAndUpdate(existingParteOponente);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(parteoponenteApi.addAndUpdate(mockParteOponente)).rejects.toThrow(ParteOponenteApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteOponente',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete parteoponente successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await parteoponenteApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(parteoponenteApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteOponente',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(parteoponenteApi.delete(1)).rejects.toThrow(ParteOponenteApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterParteOponente = { funcionario: 1 };
      
      const result = parteoponenteApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await parteoponenteApi.delete(1);
  ...
  describe('ParteOponenteApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ParteOponenteApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ParteOponenteApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ParteOponenteApiError('Test message', 500);
