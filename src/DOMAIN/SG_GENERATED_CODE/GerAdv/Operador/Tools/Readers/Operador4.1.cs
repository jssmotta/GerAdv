...
SELECT TOP (1) 
    ...
    operVisto as Visto
FROM {oCnn.UseDbo}.Operador dbq WITH (INDEX = idx_Operador_AuditorDtAtu)
...
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBOperador.PTabelaNome,
...
FROM {oCnn.UseDbo}.Operador WITH (INDEX = idx_{DBOperadorDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [operCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string tenantKey, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{tenantKey}-{DBOperadorDicInfo.PTabelaNome}-RRSAA";
...
FROM [{oCnn.UseDbo}].[{DBOperadorDicInfo.PTabelaNome}] (NOLOCK)
