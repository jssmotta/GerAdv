...
import { FilterGUTMatriz, FilterGUTMatrizDefaults } from '../Filters/GUTMatriz'
import { IFilterGUTMatrizWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for GUTMatriz API */
const GUTMATRIZ_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class GUTMatrizApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `GUTMatriz-${uri}`,
          {
            ...GUTMATRIZ_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${GUTMATRIZ_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os GUTMatriz'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar G U T Matriz com ID ${id}`,
          id
        );
    }
    ...
    public async getListN(max?: number, filtro?: FilterGUTMatriz): Promise<AxiosResponse> {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz-${uniqueKeyDay()}_lst_listN_data`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar lista de GUTMatriz'
        );
    }
 public async filterPreload(max: number, filtro?: FilterGUTMatriz): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterGUTMatrizDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterGUTMatriz): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar GUTMatriz'
        );
    }


    public async filterVoice(filtro?: FilterGUTMatriz, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterGUTMatrizDefaults();

        // Create request object according to new backend structure
        const request: IFilterGUTMatrizWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter GUTMatriz'
        );
    }
    ...
    public async validation(regGUTMatriz: IGUTMatriz): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regGUTMatriz as GUTMatriz, this.getHeaders());
            });
    ...
    public async addAndUpdate(regGUTMatriz: IGUTMatriz): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regGUTMatriz as GUTMatriz, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new GUTMatrizApiError(
                'Erro ao salvar G U T Matriz: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new GUTMatrizApiError(
                `Erro ao excluir G U T Matriz com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterGUTMatriz, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterGUTMatrizWithVoiceRequest = {filter: filtro || new FilterGUTMatrizDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<GUTMatriz[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de GUTMatriz');
            this.notificationService.notify({
              entity: 'GUTMatriz',
              id: 0,
              action: NotifySystemActions.ERROR,
