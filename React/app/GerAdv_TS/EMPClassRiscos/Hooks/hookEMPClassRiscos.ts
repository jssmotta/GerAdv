...
export const useEMPClassRiscosForm = (
  ...
  const loadEMPClassRiscos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar E M P Class Riscos";
...
export const useEMPClassRiscosList = (
  dataService: IEMPClassRiscosService,
  currentFilter?: FilterEMPClassRiscos | undefined | null,
) => {
  const [data, setData] = useState<IEMPClassRiscos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterEMPClassRiscos | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterEMPClassRiscos | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar empclassriscos";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsEMPClassRiscos() {
  ...
  function validate(data: IEMPClassRiscos): {
    isValid: boolean;
    message: string;
  } {
