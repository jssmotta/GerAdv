...
#region ListarDados_StatusBiu
    public static async IAsyncEnumerable<DBStatusBiu> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBStatusBiu(dbRec);
        }
    }

    public static IEnumerable<DBStatusBiu> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
