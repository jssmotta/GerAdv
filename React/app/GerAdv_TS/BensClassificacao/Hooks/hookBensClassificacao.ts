...
export const useBensClassificacaoForm = (
  ...
  const loadBensClassificacao = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Bens Classificacao";
...
export const useBensClassificacaoList = (
  dataService: IBensClassificacaoService,
  currentFilter?: FilterBensClassificacao | undefined | null,
) => {
  const [data, setData] = useState<IBensClassificacao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterBensClassificacao | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterBensClassificacao | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar bensclassificacao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsBensClassificacao() {
  ...
  function validate(data: IBensClassificacao): {
    isValid: boolean;
    message: string;
  } {
