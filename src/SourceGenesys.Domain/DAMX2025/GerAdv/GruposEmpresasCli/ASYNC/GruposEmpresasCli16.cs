...
public partial class DBGruposEmpresasCli
{
    ...
    /// </summary>
    public static async Task<DBGruposEmpresasCli> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBGruposEmpresasCli();
...
#if (!NOTSTORED_GruposEmpresasCli)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFGrupo || pFldFCliente || pFldFOculto;
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBGruposEmpresasCliDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBGruposEmpresasCliDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBGruposEmpresasCliDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBGruposEmpresasCliDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBGruposEmpresasCliDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
