...
public partial interface IAdvogadosReader
{
    ...
    AdvogadosResponse? Read(Entity.DBAdvogados dbRec, MsiSqlConnection oCnn);
    ...
    AdvogadosResponseAll? ReadAll(DBAdvogados dbRec, SqlDataReader dr);
    IEnumerable<DBNomeID> ListarN(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
    IEnumerable<AdvogadosResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
}

public partial class Advogados : IAdvogadosReader
{
    public IEnumerable<DBNomeID> ListarN(int max, string uri, string cWhere, List<SqlParameter> parameters, string order) => DevourerSqlData.ListarNomeID(BuildSqlQuery("{advCodigo, advNome}", cWhere, order, max), parameters, uri, caching: DevourerOne.PCachingDefault, max: max);
    public IEnumerable<AdvogadosResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order) => ListarTabela(BuildSqlQuery("*", cWhere, order, max), parameters, uri, caching: DevourerOne.PCachingDefault, max: max);
    private IEnumerable<AdvogadosResponseAll> ListarTabela(string sql, List<SqlParameter> parameters, string uri, bool caching = DevourerOne.PCachingDefault, int max = 200)
    {
        using var oCnn = new MsiSqlConnection(uri);
        return ReadLocalAsync().Result;
        async Task<List<AdvogadosResponseAll>> ReadLocalAsync()
        {
            ...
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                if (await reader.IsDBNullAsync(1).ConfigureAwait(false))
                    continue;
                result.Add(ReadAll(new Entity.DBAdvogados(reader), reader)!);
            }
    ...
    static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var query = new StringBuilder($"SELECT TOP ({max}) {campos}  FROM {"Advogados".dbo(oCnn)} (NOLOCK) ");
    ...
    public AdvogadosResponse? Read(Entity.DBAdvogados dbRec, MsiSqlConnection oCnn)
    {
        return Read(dbRec);
    }
    ...
    public AdvogadosResponseAll? ReadAll(DBAdvogados dbRec, SqlDataReader dr)
    {
        if (dbRec == null)
        {
            return null;
