...
describe('PrepostosApi', () => {
  let prepostosApi: PrepostosApi;
  ...
  beforeEach(() => {
    ...
    prepostosApi = new PrepostosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(prepostosApi).toBeInstanceOf(PrepostosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PrepostosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PrepostosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all prepostos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prepostosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(prepostosApi.getAll()).rejects.toThrow(PrepostosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await prepostosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Prepostos_last_getAll_${200}`)));
    });
    ...
    it('should throw PrepostosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPrepostos = PrepostosTestEmpty();
    ...
    it('should fetch prepostos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await prepostosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPrepostos));
      ...
      expect(result.data).toEqual(mockPrepostos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Prepostos',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter prepostos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await prepostosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new prepostos successfully', async () => {
      const responsePrepostos = { ...mockPrepostos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePrepostos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prepostosApi.addAndUpdate(mockPrepostos);
    ...
    it('should update existing prepostos successfully', async () => {
      const existingPrepostos = { ...mockPrepostos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPrepostos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await prepostosApi.addAndUpdate(existingPrepostos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(prepostosApi.addAndUpdate(mockPrepostos)).rejects.toThrow(PrepostosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Prepostos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete prepostos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await prepostosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(prepostosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Prepostos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(prepostosApi.delete(1)).rejects.toThrow(PrepostosApiError);
    });
  });
  ...
  describe('PrepostosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PrepostosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PrepostosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PrepostosApiError('Test message', 500);
