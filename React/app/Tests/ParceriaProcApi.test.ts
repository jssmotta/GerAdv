...
import { ParceriaProcApiError } from '../GerAdv_TS/ParceriaProc/Apis/ApiParceriaProc';
import { ParceriaProcApi } from '@/app/GerAdv_TS/ParceriaProc/Apis/ApiParceriaProc';
import { FilterParceriaProc } from '../GerAdv_TS/ParceriaProc/Filters/ParceriaProc';
...
describe('ParceriaProcApi', () => {
  let parceriaprocApi: ParceriaProcApi;
  ...
  beforeEach(() => {
    ...
    parceriaprocApi = new ParceriaProcApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(parceriaprocApi).toBeInstanceOf(ParceriaProcApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ParceriaProcApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ParceriaProcApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all parceriaproc items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parceriaprocApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(parceriaprocApi.getAll()).rejects.toThrow(ParceriaProcApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await parceriaprocApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ParceriaProc_last_getAll_${200}`)));
    });
    ...
    it('should throw ParceriaProcApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockParceriaProc = ParceriaProcTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockParceriaProc,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch parceriaproc by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parceriaprocApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockParceriaProc));
      ...
      expect(result.data).toEqual(mockParceriaProc);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParceriaProc',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterParceriaProc = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter parceriaproc items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await parceriaprocApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockParceriaProc: IParceriaProc = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new parceriaproc successfully', async () => {
      const responseParceriaProc = { ...mockParceriaProc, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseParceriaProc,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parceriaprocApi.addAndUpdate(mockParceriaProc);
    ...
    it('should update existing parceriaproc successfully', async () => {
      const existingParceriaProc = { ...mockParceriaProc, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingParceriaProc,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parceriaprocApi.addAndUpdate(existingParceriaProc);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(parceriaprocApi.addAndUpdate(mockParceriaProc)).rejects.toThrow(ParceriaProcApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParceriaProc',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete parceriaproc successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await parceriaprocApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(parceriaprocApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParceriaProc',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(parceriaprocApi.delete(1)).rejects.toThrow(ParceriaProcApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterParceriaProc = { funcionario: 1 };
      
      const result = parceriaprocApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await parceriaprocApi.delete(1);
  ...
  describe('ParceriaProcApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ParceriaProcApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ParceriaProcApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ParceriaProcApiError('Test message', 500);
