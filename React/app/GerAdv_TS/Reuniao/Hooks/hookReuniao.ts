...
export const useReuniaoList = (
  dataService: IReuniaoService,
  currentFilter?: FilterReuniao | undefined | null,
) => {
  const [data, setData] = useState<IReuniao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterReuniao | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterReuniao | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar reuniao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IReuniao): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.pauta.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo Pauta não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.ata.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo ATA não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.principaisdecisoes.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo PrincipaisDecisoes não pode ter mais de 2147483647 caracteres.",
        };
      }
