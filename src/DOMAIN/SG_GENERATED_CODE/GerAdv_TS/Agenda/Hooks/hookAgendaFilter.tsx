...
interface UseAgendaFilterProps {
    handleFetchWithFilter: (filtro?: FilterAgenda | undefined | null) => Promise<void>;
}
...
interface UseAgendaFilterReturn {
    ...
    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterAgenda>) => React.ReactNode;
...
export const useAgendaFilter = ({ handleFetchWithFilter }: UseAgendaFilterProps): UseAgendaFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterAgenda>({} as FilterAgenda);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterAgenda;
...
<FilterDialogButton        
        ...
        setFilter={(f: any) =>
          handlers.setWindowFilter((prev) => ({ ...prev, filterFuncionarios: f }))
        }        
        onConfirm={async (f: any) => {          
        }}
      ...
      />
<FilterDialogButton        
        filter={handlers.windowFilter.filterTipoCompromisso ?? ({} as any) as any}
        setFilter={(f: any) =>
          handlers.setWindowFilter((prev) => ({ ...prev, filterTipoCompromisso: f }))
        }        
        onConfirm={async (f: any) => {          
        }}
...
const { renderInputFilters: renderFuncionariosInputs } = useFuncionariosFilter({
    handleFetchWithFilter: async () => {},
  });
const { renderInputFilters: renderTipoCompromissoInputs } = useTipoCompromissoFilter({
    handleFetchWithFilter: async () => {},
  });
