...
public partial class PontoVirtualAcessosReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBPontoVirtualAcessosDicInfo.CampoCodigo}";
...
[{DBOperadorDicInfo.PTabelaNome}].[{DBOperadorDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBOperadorDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBOperadorDicInfo.PTabelaNome}].[{DBOperadorDicInfo.CampoCodigo}]=[{DBPontoVirtualAcessosDicInfo.PTabelaNome}].[{DBPontoVirtualAcessosDicInfo.Operador}]
