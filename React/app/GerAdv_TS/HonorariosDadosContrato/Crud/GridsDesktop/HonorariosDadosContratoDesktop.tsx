...
import { useHonorariosDadosContratoGrid } from "../../Hooks/hookHonorariosDadosContratoGrid";
...
export const HonorariosDadosContratoGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("honorariosdadoscontrato-columns-order"),
        );
        if (savedOrder) {
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("honorariosdadoscontrato-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-honorariosdadoscontrato",
      columns: exportColumns,
      sheetName: "HonorariosDadosContrato",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-HonorariosDadosContrato-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-honorariosdadoscontrato",
      columns: exportColumnsPdf,
      title: "HonorariosDadosContrato",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-HonorariosDadosContrato-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`honorariosdadoscontrato-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-honorariosdadoscontrato"
