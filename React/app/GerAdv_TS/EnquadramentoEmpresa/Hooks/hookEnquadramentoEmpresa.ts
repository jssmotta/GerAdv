...
export const useEnquadramentoEmpresaForm = (
  ...
  const loadEnquadramentoEmpresa = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialEnquadramentoEmpresa);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Enquadramento Empresa';
      setError(errorMessage);
      console.log('Erro ao carregar Enquadramento Empresa');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialEnquadramentoEmpresa]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadEnquadramentoEmpresa, resetForm]);
  return returnValue;
};


export const useEnquadramentoEmpresaNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('EnquadramentoEmpresa', (entity) => {
      try {
...
export const useEnquadramentoEmpresaList = (dataService: IEnquadramentoEmpresaService) => {
  const [data, setData] = useState<IEnquadramentoEmpresa[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar enquadramentoempresa';
      setError(errorMessage);
      console.log('Erro ao carregar enquadramentoempresa');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useEnquadramentoEmpresaNotifications(
