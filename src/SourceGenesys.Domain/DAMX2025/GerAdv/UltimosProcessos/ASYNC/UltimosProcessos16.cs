...
public partial class DBUltimosProcessos
{
    ...
    /// </summary>
    public static async Task<DBUltimosProcessos> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBUltimosProcessos();
...
#if (!NOTSTORED_UltimosProcessos)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFQuando || pFldFQuem;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBUltimosProcessosDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFQuando)
            updateTool.Fields(DBUltimosProcessosDicInfo.Quando, m_FQuando, ETiposCampos.FDate);
        if (pFldFQuem)
            updateTool.Fields(DBUltimosProcessosDicInfo.Quem, m_FQuem, ETiposCampos.FNumber);
    }
