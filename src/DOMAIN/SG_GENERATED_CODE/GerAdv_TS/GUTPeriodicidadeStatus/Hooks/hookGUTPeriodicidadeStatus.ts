...
export const useGUTPeriodicidadeStatusList = (dataService: IGUTPeriodicidadeStatusService, currentFilter?: FilterGUTPeriodicidadeStatus | undefined | null) => {
  const [data, setData] = useState<IGUTPeriodicidadeStatus[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterGUTPeriodicidadeStatus | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterGUTPeriodicidadeStatus | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterGUTPeriodicidadeStatus);
      setData(result);
    } catch (err) {
