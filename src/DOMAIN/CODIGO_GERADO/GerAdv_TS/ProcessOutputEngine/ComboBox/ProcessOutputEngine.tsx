...
const ProcessOutputEngineComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProcessOutputEngine | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProcessOutputEngine | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProcessOutputEngine);
    ...
    const handleAddClick = () => {
        ...
        const add = ProcessOutputEngineEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProcessOutputEngineWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProcessOutputEngine={editar ?? addValue ?? ProcessOutputEngineEmpty()} /> 
            )}
