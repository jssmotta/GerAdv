...
interface UseGUTPeriodicidadeFilterProps {
    handleFetchWithFilter: (filtro?: FilterGUTPeriodicidade | undefined | null) => Promise<void>;
}

interface UseGUTPeriodicidadeFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterGUTPeriodicidade;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterGUTPeriodicidade>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterGUTPeriodicidade) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterGUTPeriodicidade>) => React.ReactNode;
...
export const useGUTPeriodicidadeFilter = ({ handleFetchWithFilter }: UseGUTPeriodicidadeFilterProps): UseGUTPeriodicidadeFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterGUTPeriodicidade>({} as FilterGUTPeriodicidade);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterGUTPeriodicidade;
    ...
    const handleConfirmSearch = async (filter: FilterGUTPeriodicidade) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterGUTPeriodicidade);
        sessionStorage.removeItem(btoa('GUTPeriodicidadeFilter'));
        handleFetchWithFilter({} as FilterGUTPeriodicidade);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterGUTPeriodicidade>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
...
<InputInput
                ...
                id='intervalodias'
                name='intervalodias'
                value={handlers.windowFilter?.intervalodias ?? ''}
                onChange={handlers.handleInputChange}
                placeholder='Informe IntervaloDias'
                label='IntervaloDias (igual ou inicial)'
                className='input-default-main inputSearch'
                />
<InputInput
                type='text'
                id='intervalodias_end'
                name='intervalodias_end'
                value={handlers.windowFilter?.intervalodias_end ?? ''}
                onChange={handlers.handleInputChange}
                placeholder='Informe IntervaloDias final'
                label='IntervaloDias final'
                disabled={handlers.windowFilter?.intervalodias ? false : true}
                className='input-default-main inputSearch'
                />
<InputInput
                type='text'
