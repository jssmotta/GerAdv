...
export const useContatoCRMList = (dataService: IContatoCRMService, currentFilter?: FilterContatoCRM | undefined | null) => {
  const [data, setData] = useState<IContatoCRM[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterContatoCRM | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterContatoCRM | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterContatoCRM);
      setData(result);
    } catch (err) {
