...
const FaseComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IFase | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IFase | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IFase);
    ...
    React.useEffect(() => {
        const unsubscribe = subscribeToNotifications("Fase", (entity) => {
      try {
                fetchDados();                
        ...
        <>
          {(editar || addValue) && isOpen && (
            <FaseWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedFase={editar ?? addValue ?? FaseEmpty()} /> 
            )}
