...
export const usePosicaoOutrasPartesList = (dataService: IPosicaoOutrasPartesService, currentFilter?: FilterPosicaoOutrasPartes | undefined | null) => {
  const [data, setData] = useState<IPosicaoOutrasPartes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterPosicaoOutrasPartes | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterPosicaoOutrasPartes | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterPosicaoOutrasPartes);
      setData(result);
    } catch (err) {
