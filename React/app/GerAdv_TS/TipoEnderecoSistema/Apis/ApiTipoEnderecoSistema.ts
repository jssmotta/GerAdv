...
import { TipoEnderecoSistema } from '../../Models/TipoEnderecoSistema';
...
export class TipoEnderecoSistemaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'TipoEnderecoSistemaApiError';
    this.status = status;
    this.code = code;
...
export class TipoEnderecoSistemaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/TipoEnderecoSistema`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoEnderecoSistema_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os TipoEnderecoSistema');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoEnderecoSistema_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Tipo Endereco Sistema com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterTipoEnderecoSistema): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoEnderecoSistema_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de TipoEnderecoSistema');
        }
    }

            public async filter(filtro: FilterTipoEnderecoSistema): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-TipoEnderecoSistema_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar TipoEnderecoSistema');
                }
            }


    public async validation(regTipoEnderecoSistema: ITipoEnderecoSistema): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regTipoEnderecoSistema as TipoEnderecoSistema, this.getHeaders());
    ...
    public async addAndUpdate(regTipoEnderecoSistema: ITipoEnderecoSistema): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regTipoEnderecoSistema as TipoEnderecoSistema, this.getHeaders());
            var register = result.data as ITipoEnderecoSistema;        
            const action = regTipoEnderecoSistema.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Tipo Endereco Sistema');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o TipoEnderecoSistema. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'TipoEnderecoSistema',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Tipo Endereco Sistema com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterTipoEnderecoSistema) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<TipoEnderecoSistema[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de tipoenderecosistema:', error);
            this.notificationService.notify({
              entity: 'TipoEnderecoSistema',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos tipoenderecosistema'
            });
          }
        });
    }
