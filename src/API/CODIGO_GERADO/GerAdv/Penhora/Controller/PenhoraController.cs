...
public partial class PenhoraController(IPenhoraService penhoraService) : ControllerBase
{
    ...
    {
        _logger.Info("Penhora: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _penhoraService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Penhora: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _penhoraService.UpdateColumns(parameters, uri);
        if (!result)
        {
