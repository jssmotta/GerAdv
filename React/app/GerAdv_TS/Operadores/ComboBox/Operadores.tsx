...
import OperadoresWindow from '../Crud/Grids/OperadoresWindow';
...
import { useOperadoresComboBox } from '../Hooks/hookOperadores';
const OperadoresComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useOperadoresComboBox(operadoresService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IOperadores | null>(null);
const [addRecord, setAddRecord] = useState<IOperadores | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await operadoresService.fetchOperadoresById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = OperadoresEmpty();
...
const handleSuccess = (newOperadores?: IOperadores) => {
  if (newOperadores) {
    setValue({ id: newOperadores.id, nome: newOperadores.nome });
    loadRecordForEdit(newOperadores.id);
    handleValueChange(newOperadores);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .operadores-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .operadores-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .operadores-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedOperadores={editRecord || addRecord || OperadoresEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'operadores-dropdown-popup-msi'
          }}
          />
...
export default OperadoresComboBox;