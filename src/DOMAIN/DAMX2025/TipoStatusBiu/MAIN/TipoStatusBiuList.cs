...
#region ListarDados_TipoStatusBiu
    public static async IAsyncEnumerable<DBTipoStatusBiu> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoStatusBiu(dbRec);
        }
    }

    public static IEnumerable<DBTipoStatusBiu> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
