  ...
  ({
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        descricaoservicos: (
          <GridColumn
            key="descricaoservicos"
            field="descricaoservicos"
            title="Serviço"
            sortable={false}
            filterable={false}
          />
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("horastrab-columns-order"));
    }, []);
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-HorasTrab-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-HorasTrab-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`horastrab-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-horastrab"
