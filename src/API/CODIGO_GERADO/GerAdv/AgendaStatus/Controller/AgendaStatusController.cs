...
public partial class AgendaStatusController(IAgendaStatusService agendastatusService) : ControllerBase
{
    ...
    {
        _logger.Info("AgendaStatus: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _agendastatusService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("AgendaStatus: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _agendastatusService.UpdateColumns(parameters, uri);
        if (!result)
        {
