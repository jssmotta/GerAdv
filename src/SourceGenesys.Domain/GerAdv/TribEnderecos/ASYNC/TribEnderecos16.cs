...
public partial class DBTribEnderecos
{
    ...
    /// </summary>
    public static async Task<DBTribEnderecos> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBTribEnderecos();
...
#if (!NOTSTORED_TribEnderecos)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFTribunal || pFldFCidade || pFldFEndereco || pFldFCEP || pFldFFone || pFldFFax || pFldFOBS;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFTribunal)
            updateTool.Fields(DBTribEnderecosDicInfo.Tribunal, m_FTribunal, ETiposCampos.FNumber);
        if (pFldFCidade)
            updateTool.Fields(DBTribEnderecosDicInfo.Cidade, m_FCidade, ETiposCampos.FNumber);
        if (pFldFEndereco)
            updateTool.Fields(DBTribEnderecosDicInfo.Endereco, m_FEndereco, ETiposCampos.FString);
        if (pFldFCEP)
            updateTool.Fields(DBTribEnderecosDicInfo.CEP, m_FCEP, ETiposCampos.FString);
        if (pFldFFone)
            updateTool.Fields(DBTribEnderecosDicInfo.Fone, m_FFone, ETiposCampos.FString);
        if (pFldFFax)
            updateTool.Fields(DBTribEnderecosDicInfo.Fax, m_FFax, ETiposCampos.FString);
        if (pFldFOBS)
            updateTool.Fields(DBTribEnderecosDicInfo.OBS, m_FOBS, ETiposCampos.FString);
    }
