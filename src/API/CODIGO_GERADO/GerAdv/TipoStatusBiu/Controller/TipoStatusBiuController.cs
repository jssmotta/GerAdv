...
public partial class TipoStatusBiuController(ITipoStatusBiuService tipostatusbiuService) : ControllerBase
{
    ...
    {
        _logger.Info("TipoStatusBiu: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _tipostatusbiuService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("TipoStatusBiu: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _tipostatusbiuService.UpdateColumns(parameters, uri);
        if (!result)
        {
