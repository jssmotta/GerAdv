...
public partial class DBEscritorios : VAuditor, ICadastros, IAuditor
{
#region TableDefinition_Escritorios
...
#endregion
    public DBEscritorios(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBEscritorios(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBEscritorios(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_Escritorios)
        if (clsW.HasUpdates)
        {
            HasChanges = true;
        }
