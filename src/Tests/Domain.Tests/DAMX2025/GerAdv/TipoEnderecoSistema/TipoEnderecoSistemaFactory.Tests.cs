...
public class FTipoEnderecoSistemaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FTipoEnderecoSistemaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FTipoEnderecoSistemaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FTipoEnderecoSistemaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "tesCodigo")
        var dt = new DataTable();
        dt.Columns.Add("tesCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["tesCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FTipoEnderecoSistema>();
    }
    ...
    {
        // Arrange
        var factory = new FTipoEnderecoSistemaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("tesCodigo", typeof(int));
        mockDataTable.Columns.Add("tesNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["tesCodigo"] = testId;
        row["tesNome"] = "Test TipoEnderecoSistema";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["tesCodigo"] = 123;
        row["tesNome"] = "Test TipoEnderecoSistema Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FTipoEnderecoSistema properties
        }
    }
