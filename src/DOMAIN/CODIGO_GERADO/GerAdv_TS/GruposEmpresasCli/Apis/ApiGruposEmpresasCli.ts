...
export class GruposEmpresasCliApi {
    ...
    public async validation(regGruposEmpresasCli: IGruposEmpresasCli): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGruposEmpresasCli as GruposEmpresasCli, this.getHeaders());
