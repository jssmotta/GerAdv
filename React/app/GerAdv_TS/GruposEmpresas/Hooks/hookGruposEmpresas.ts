...
export const useGruposEmpresasForm = (
  ...
  const loadGruposEmpresas = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Grupos Empresas";
...
export const useGruposEmpresasList = (
  dataService: IGruposEmpresasService,
  currentFilter?: FilterGruposEmpresas | undefined | null,
) => {
  const [data, setData] = useState<IGruposEmpresas[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGruposEmpresas | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterGruposEmpresas | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar gruposempresas";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsGruposEmpresas() {
  ...
  function validate(data: IGruposEmpresas): {
    isValid: boolean;
    message: string;
  } {
