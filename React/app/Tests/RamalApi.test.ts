...
import { RamalApiError } from '../GerAdv_TS/Ramal/Apis/ApiRamal';
import { RamalApi } from '@/app/GerAdv_TS/Ramal/Apis/ApiRamal';
import { FilterRamal } from '../GerAdv_TS/Ramal/Filters/Ramal';
...
describe('RamalApi', () => {
  ...
  beforeEach(() => {
    ...
    ramalApi = new RamalApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(ramalApi).toBeInstanceOf(RamalApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new RamalApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(RamalApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all ramal items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ramalApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(ramalApi.getAll()).rejects.toThrow(RamalApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await ramalApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Ramal_last_getAll_${200}`)));
    });
    ...
    it('should throw RamalApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockRamal = RamalTestEmpty();
    ...
    it('should fetch ramal by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ramalApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockRamal));
      ...
      expect(result.data).toEqual(mockRamal);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ramal',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter ramal items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await ramalApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new ramal successfully', async () => {
      const responseRamal = { ...mockRamal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseRamal,
        status: 200,
        statusText: 'OK',
      ...
      const result = await ramalApi.addAndUpdate(mockRamal);
    ...
    it('should update existing ramal successfully', async () => {
      const existingRamal = { ...mockRamal, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingRamal,
        status: 200,
        statusText: 'OK',
      ...
      const result = await ramalApi.addAndUpdate(existingRamal);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(ramalApi.addAndUpdate(mockRamal)).rejects.toThrow(RamalApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ramal',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete ramal successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await ramalApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(ramalApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Ramal',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(ramalApi.delete(1)).rejects.toThrow(RamalApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterRamal = { funcionario: 1 };
      
      const result = ramalApi.useFilter(mockFilter);
  ...
  describe('RamalApiError', () => {
    it('should create error with correct properties', () => {
      const error = new RamalApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('RamalApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new RamalApiError('Test message', 500);
