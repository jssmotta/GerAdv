...
public static class FilterProObservacoesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProObservacoes em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProObservacoes:

- O filter irá filtrar os ProObservacoes conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProObservacoes:(Pro Observacoes)
- processo: número ou IDs;
- nome: _nome ou descrição para Pro Observacoes;
- observacoes: Observacoes;
- data: não é a data de cadastramento é a data do ProObservacoes;
...
- Se ouvir "todos os ProObservacoes" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProObservacoes? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProObservacoes length: {result.Length}");
#endif
