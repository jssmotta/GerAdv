...
export const useProValoresForm = (
  ...
  const loadProValores = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Pro Valores";
...
export const useProValoresList = (
  dataService: IProValoresService,
  currentFilter?: FilterProValores | undefined | null,
) => {
  const [data, setData] = useState<IProValores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProValores | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterProValores | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar provalores";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IProValores): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.indice.length > 20) {
        return {
          isValid: false,
          message: "O campo Indice não pode ter mais de 20 caracteres.",
        };
      }
