...
public static class FilterProcessOutputSourcesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProcessOutputSources em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProcessOutputSources:

- O filter irá filtrar os ProcessOutputSources conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProcessOutputSources:(Process Output Sources)
- nome: _nome ou descrição para Process Output Sources;
...
- Se ouvir "todos os ProcessOutputSources" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProcessOutputSources? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProcessOutputSources length: {result.Length}");
#endif
