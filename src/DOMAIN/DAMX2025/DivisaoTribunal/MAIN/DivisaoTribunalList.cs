...
#region ListarDados_DivisaoTribunal
    public static async IAsyncEnumerable<DBDivisaoTribunal> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBDivisaoTribunal(dbRec);
        }
    }

    public static IEnumerable<DBDivisaoTribunal> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
