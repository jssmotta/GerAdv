...
// usePoderJudiciarioAssociadoGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de PoderJudiciarioAssociado
'use client';
...
interface UsePoderJudiciarioAssociadoGridProps {
...
export const usePoderJudiciarioAssociadoGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  initialTake = 10, 
  useCustomSort = false
}: UsePoderJudiciarioAssociadoGridProps) => {
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('poderjudiciarioassociado-columns-order'), JSON.stringify(reorderableFields));
