...
import { GruposEmpresasCliApiError } from '../GerAdv_TS/GruposEmpresasCli/Apis/ApiGruposEmpresasCli';
import { GruposEmpresasCliApi } from '@/app/GerAdv_TS/GruposEmpresasCli/Apis/ApiGruposEmpresasCli';
import { FilterGruposEmpresasCli } from '../GerAdv_TS/GruposEmpresasCli/Filters/GruposEmpresasCli';
...
describe('GruposEmpresasCliApi', () => {
  let gruposempresascliApi: GruposEmpresasCliApi;
  ...
  beforeEach(() => {
    ...
    gruposempresascliApi = new GruposEmpresasCliApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gruposempresascliApi).toBeInstanceOf(GruposEmpresasCliApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GruposEmpresasCliApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GruposEmpresasCliApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [GruposEmpresasCliTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all gruposempresascli items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gruposempresascliApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gruposempresascliApi.getAll()).rejects.toThrow(GruposEmpresasCliApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gruposempresascliApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GruposEmpresasCli_last_getAll_${200}`)));
    });
    ...
    it('should throw GruposEmpresasCliApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGruposEmpresasCli = GruposEmpresasCliTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockGruposEmpresasCli,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch gruposempresascli by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gruposempresascliApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGruposEmpresasCli));
      ...
      expect(result.data).toEqual(mockGruposEmpresasCli);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresasCli',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGruposEmpresasCli = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter gruposempresascli items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gruposempresascliApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockGruposEmpresasCli: IGruposEmpresasCli = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new gruposempresascli successfully', async () => {
      const responseGruposEmpresasCli = { ...mockGruposEmpresasCli, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGruposEmpresasCli,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gruposempresascliApi.addAndUpdate(mockGruposEmpresasCli);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockGruposEmpresasCli,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing gruposempresascli successfully', async () => {
      const existingGruposEmpresasCli = { ...mockGruposEmpresasCli, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGruposEmpresasCli,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gruposempresascliApi.addAndUpdate(existingGruposEmpresasCli);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gruposempresascliApi.addAndUpdate(mockGruposEmpresasCli)).rejects.toThrow(GruposEmpresasCliApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresasCli',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gruposempresascli successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gruposempresascliApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gruposempresascliApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GruposEmpresasCli',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gruposempresascliApi.delete(1)).rejects.toThrow(GruposEmpresasCliApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGruposEmpresasCli = { funcionario: 1 };
      
      const result = gruposempresascliApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await gruposempresascliApi.delete(1);
  ...
  describe('GruposEmpresasCliApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GruposEmpresasCliApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GruposEmpresasCliApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GruposEmpresasCliApiError('Test message', 500);
