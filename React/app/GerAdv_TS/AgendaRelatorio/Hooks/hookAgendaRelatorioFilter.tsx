...
interface UseAgendaRelatorioFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterAgendaRelatorio | undefined | null,
  ) => Promise<void>;
}
...
interface UseAgendaRelatorioFilterReturn {
  ...
  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterAgendaRelatorio>,
  ) => React.ReactNode;
...
export const useAgendaRelatorioFilter = ({
  handleFetchWithFilter,
}: UseAgendaRelatorioFilterProps): UseAgendaRelatorioFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterAgendaRelatorio>(
  ...
  const clearFilter = () => {
    setWindowFilter({} as FilterAgendaRelatorio);
    sessionStorage.removeItem(btoa("AgendaRelatorioFilter"));
    handleFetchWithFilter({} as FilterAgendaRelatorio);
  };
    ...
    <>
      <InputDate
        ...
        onChange={(value: string) =>
          handlers.handleDateChange("vqadata", value)
        }
      />
      <InputDate
        tabIndex={1}
        ...
        onChange={(value: string) =>
          handlers.handleDateChange("vqadata_end", value)
        }
      />
      <InputInput
        tabIndex={2}
