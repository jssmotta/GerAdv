...
public partial class ParteClienteOutrasController(IParteClienteOutrasService parteclienteoutrasService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.ParteClienteOutras regParteClienteOutras, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "ParteClienteOutras: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "ParteClienteOutras: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    public async Task<IActionResult> Validation([FromBody] Models.ParteClienteOutras regParteClienteOutras, [FromRoute, Required] string uri)
    {
        try
        {
            ...
            if (result == null)
            {
                _logger.Warn("ParteClienteOutras: Validation failed to add or update ParteClienteOutras, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "ParteClienteOutras: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}