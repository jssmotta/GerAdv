...
public static class FilterTipoEMailTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoEMail em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoEMail:

- O filter irá filtrar os TipoEMail conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os TipoEMail" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoEMail? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoEMail length: {result.Length}");
#endif
