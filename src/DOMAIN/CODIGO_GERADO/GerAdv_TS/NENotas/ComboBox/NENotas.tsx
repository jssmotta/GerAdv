...
const NENotasComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<INENotas | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<INENotas | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as INENotas);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <NENotasWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedNENotas={editar ?? addValue ?? NENotasEmpty()} /> 
            )}
