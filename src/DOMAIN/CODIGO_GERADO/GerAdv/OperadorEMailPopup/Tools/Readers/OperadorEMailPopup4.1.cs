...
public partial class OperadorEMailPopup
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(oepDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.OperadorEMailPopup WITH (NOLOCK, INDEX = idx_OperadorEMailPopup_AuditorDtAtu)
WHERE oepCodigo = @id
...
FROM {oCnn.UseDbo}.OperadorEMailPopup WITH (NOLOCK)
