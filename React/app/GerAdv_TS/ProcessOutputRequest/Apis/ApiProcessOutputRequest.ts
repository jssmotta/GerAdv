...
export class ProcessOutputRequestApi {
    ...
    public async validation(regProcessOutputRequest: IProcessOutputRequest): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProcessOutputRequest as ProcessOutputRequest, this.getHeaders());
