...
export const useEMPClassRiscosList = (dataService: IEMPClassRiscosService, currentFilter?: FilterEMPClassRiscos | undefined | null) => {
  const [data, setData] = useState<IEMPClassRiscos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterEMPClassRiscos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterEMPClassRiscos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterEMPClassRiscos);
      setData(result);
    } catch (err) {
