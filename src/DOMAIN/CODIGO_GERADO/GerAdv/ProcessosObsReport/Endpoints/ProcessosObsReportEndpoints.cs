...
public static class ProcessosObsReportEndpoints
{
    ...
    private static void MapProcessosObsReportRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapGet("/GetAll", async (int max, string uri, IProcessosObsReportService service) =>
        {
            var result = await service.GetAll(max, uri);
            return Results.Ok(result);
        }).WithName("ProcessosObsReport_GetAll").WithDisplayName("Get All ProcessosObsReport");
        group.MapPost("/Filter", async (Filters.FilterProcessosObsReport filtro, string uri, IProcessosObsReportService service) =>
        {
            var result = await service.Filter(filtro, uri);
            return Results.Ok(result);
        }).WithName("ProcessosObsReport_Filter").WithDisplayName("Filter ProcessosObsReport");
        group.MapGet("/GetById/{id}", async (int id, string uri, IProcessosObsReportService service, CancellationToken token) =>
        {
