...
const ProcessOutputSourcesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProcessOutputSources | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProcessOutputSources | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProcessOutputSources);
    ...
    const handleAddClick = () => {
        ...
        const add = ProcessOutputSourcesEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProcessOutputSourcesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProcessOutputSources={editar ?? addValue ?? ProcessOutputSourcesEmpty()} /> 
            )}
