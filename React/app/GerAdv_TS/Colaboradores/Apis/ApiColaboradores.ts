...
export class ColaboradoresApiError extends Error {
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ColaboradoresApiError';
    this.status = status;
    this.code = code;
...
export class ColaboradoresApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Colaboradores`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Colaboradores_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Colaboradores');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Colaboradores_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Colaborador com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterColaboradores): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Colaboradores_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Colaboradores');
        }
    }

            public async filter(filtro: FilterColaboradores): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Colaboradores_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Colaboradores');
                }
            }


    public async validation(regColaboradores: IColaboradores): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regColaboradores as Colaboradores, this.getHeaders());
    ...
    public async addAndUpdate(regColaboradores: IColaboradores): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regColaboradores as Colaboradores, this.getHeaders());
            var register = result.data as IColaboradores;        
            const action = regColaboradores.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Colaboradores. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Colaboradores',
                        id: id,
                        action: NotifySystemActions.ERROR,
    ...
    public useFilter(filtro: FilterColaboradores) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Colaboradores[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de colaboradores:', error);
            this.notificationService.notify({
              entity: 'Colaboradores',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos colaboradores'
            });
          }
        });
    }
