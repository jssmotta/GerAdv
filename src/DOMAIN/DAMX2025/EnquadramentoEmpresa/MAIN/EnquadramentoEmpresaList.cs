...
#region ListarDados_EnquadramentoEmpresa
    public static async IAsyncEnumerable<DBEnquadramentoEmpresa> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEnquadramentoEmpresa(dbRec);
        }
    }

    public static IEnumerable<DBEnquadramentoEmpresa> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
