...
describe('TipoCompromissoApi', () => {
  let tipocompromissoApi: TipoCompromissoApi;
  ...
  beforeEach(() => {
    ...
    tipocompromissoApi = new TipoCompromissoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipocompromissoApi).toBeInstanceOf(TipoCompromissoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoCompromissoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoCompromissoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tipocompromisso items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipocompromissoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipocompromissoApi.getAll()).rejects.toThrow(TipoCompromissoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipocompromissoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoCompromisso_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoCompromissoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoCompromisso = TipoCompromissoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoCompromisso,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipocompromisso by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipocompromissoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoCompromisso));
      ...
      expect(result.data).toEqual(mockTipoCompromisso);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoCompromisso',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter tipocompromisso items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipocompromissoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new tipocompromisso successfully', async () => {
      const responseTipoCompromisso = { ...mockTipoCompromisso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoCompromisso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipocompromissoApi.addAndUpdate(mockTipoCompromisso);
    ...
    it('should update existing tipocompromisso successfully', async () => {
      const existingTipoCompromisso = { ...mockTipoCompromisso, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoCompromisso,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipocompromissoApi.addAndUpdate(existingTipoCompromisso);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipocompromissoApi.addAndUpdate(mockTipoCompromisso)).rejects.toThrow(TipoCompromissoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoCompromisso',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipocompromisso successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipocompromissoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipocompromissoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoCompromisso',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipocompromissoApi.delete(1)).rejects.toThrow(TipoCompromissoApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipocompromissoApi.delete(1);
  ...
  describe('TipoCompromissoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoCompromissoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoCompromissoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoCompromissoApiError('Test message', 500);
