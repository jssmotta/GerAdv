...
describe('ProObservacoesApi', () => {
  let proobservacoesApi: ProObservacoesApi;
  ...
  beforeEach(() => {
    ...
    proobservacoesApi = new ProObservacoesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(proobservacoesApi).toBeInstanceOf(ProObservacoesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProObservacoesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProObservacoesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all proobservacoes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proobservacoesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(proobservacoesApi.getAll()).rejects.toThrow(ProObservacoesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await proobservacoesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProObservacoes_last_getAll_${200}`)));
    });
    ...
    it('should throw ProObservacoesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProObservacoes = ProObservacoesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProObservacoes,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch proobservacoes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proobservacoesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProObservacoes));
      ...
      expect(result.data).toEqual(mockProObservacoes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProObservacoes',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter proobservacoes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await proobservacoesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new proobservacoes successfully', async () => {
      const responseProObservacoes = { ...mockProObservacoes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProObservacoes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proobservacoesApi.addAndUpdate(mockProObservacoes);
    ...
    it('should update existing proobservacoes successfully', async () => {
      const existingProObservacoes = { ...mockProObservacoes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProObservacoes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proobservacoesApi.addAndUpdate(existingProObservacoes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(proobservacoesApi.addAndUpdate(mockProObservacoes)).rejects.toThrow(ProObservacoesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProObservacoes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete proobservacoes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await proobservacoesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(proobservacoesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProObservacoes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(proobservacoesApi.delete(1)).rejects.toThrow(ProObservacoesApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await proobservacoesApi.delete(1);
  ...
  describe('ProObservacoesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProObservacoesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProObservacoesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProObservacoesApiError('Test message', 500);
