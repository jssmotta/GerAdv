...
const FuncionariosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IFuncionarios | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IFuncionarios | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IFuncionarios);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <FuncionariosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedFuncionarios={editar ?? addValue ?? FuncionariosEmpty()} /> 
            )}
