...
export const AgendaSemanaGridMobileComponent = React.memo(
  ({
    ...
    const [columnsOrder, setColumnsOrder] = useState<string[]>(["paranome"]);

    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("agendasemana-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-agendasemana .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-agendasemana",
      columns: exportColumns,
      sheetName: "AgendaSemana",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-AgendaSemana-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-AgendaSemana-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            <PageTitle
              title="Agenda Semana"
              helpButton={<AgendaSemanaHelpCrudButton isMobile={true} />}
            />
            ...
            >
              {(typeView) => (
                <>
                  ...
                  {typeView === "grid" && (
                    <Grid
                      className="grid-mobile-crud grid-mobile-agendasemana"
                      key={`agendasemana-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
                      ...
                      onRowClick={(e) => {
                        if (selectItem) {
                          selectItem(e.dataItem as IAgendaSemana);
