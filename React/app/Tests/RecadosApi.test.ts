...
import { RecadosApiError } from '../GerAdv_TS/Recados/Apis/ApiRecados';
import { RecadosApi } from '@/app/GerAdv_TS/Recados/Apis/ApiRecados';
import { FilterRecados } from '../GerAdv_TS/Recados/Filters/Recados';
...
describe('RecadosApi', () => {
  ...
  beforeEach(() => {
    ...
    recadosApi = new RecadosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(recadosApi).toBeInstanceOf(RecadosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new RecadosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(RecadosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all recados items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await recadosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(recadosApi.getAll()).rejects.toThrow(RecadosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await recadosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Recados_last_getAll_${200}`)));
    });
    ...
    it('should throw RecadosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockRecados = RecadosTestEmpty();
    ...
    it('should fetch recados by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await recadosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockRecados));
      ...
      expect(result.data).toEqual(mockRecados);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Recados',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterRecados = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter recados items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await recadosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new recados successfully', async () => {
      const responseRecados = { ...mockRecados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseRecados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await recadosApi.addAndUpdate(mockRecados);
    ...
    it('should update existing recados successfully', async () => {
      const existingRecados = { ...mockRecados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingRecados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await recadosApi.addAndUpdate(existingRecados);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(recadosApi.addAndUpdate(mockRecados)).rejects.toThrow(RecadosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Recados',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete recados successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await recadosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(recadosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Recados',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(recadosApi.delete(1)).rejects.toThrow(RecadosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterRecados = { funcionario: 1 };
      
      const result = recadosApi.useFilter(mockFilter);
  ...
  describe('RecadosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new RecadosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('RecadosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new RecadosApiError('Test message', 500);
