...
const UFComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IUF | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IUF | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setDisplayValue({ id: response.data.id, nome: response.data.iduf });
                setEditar(response.data as IUF);
    ...
    const handleAddClick = () => {
        ...
        const add = UFEmpty();
        add.iduf = inputValue.toUpperCase();       
        setEditar(null);
        setAddValue(add);
        setIsOpen(true);
    }
        ...
        <>
          {(editar || addValue) && isOpen && (
            <UFWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedUF={editar ?? addValue ?? UFEmpty()} /> 
            )}
