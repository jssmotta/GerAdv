...
#region ListarDados_Area
    public static async IAsyncEnumerable<DBArea> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBArea(dbRec);
        }
    }

    public static IEnumerable<DBArea> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
