...
import { ProCDA } from '../../Models/ProCDA';
...
export class ProCDAApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class ProCDAApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProCDA`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProCDA_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProCDA');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProCDA_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Pro C D A com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterProCDA): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProCDA_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ProCDA');
        }
    }

            public async filter(filtro: FilterProCDA): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProCDA_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProCDA');
                }
            }


    public async validation(regProCDA: IProCDA): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProCDA as ProCDA, this.getHeaders());
    ...
    public async addAndUpdate(regProCDA: IProCDA): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProCDA as ProCDA, this.getHeaders());
            var register = result.data as IProCDA;        
            const action = regProCDA.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Pro C D A');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProCDA. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProCDA',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Pro C D A com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProCDA) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProCDA[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de procda:', error);
            this.notificationService.notify({
              entity: 'ProCDA',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos procda'
            });
          }
        });
    }
