...
export const useTipoEMailForm = (
  ...
  const loadTipoEMail = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Tipo E Mail";
...
export const useTipoEMailList = (
  dataService: ITipoEMailService,
  currentFilter?: FilterTipoEMail | undefined | null,
) => {
  const [data, setData] = useState<ITipoEMail[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoEMail | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterTipoEMail | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar tipoemail";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
