...
public class FOutrasPartesClienteFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FOutrasPartesClienteFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FOutrasPartesClienteFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FOutrasPartesClienteFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "opcCodigo")
        var dt = new DataTable();
        dt.Columns.Add("opcCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["opcCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FOutrasPartesCliente>();
    }
    ...
    {
        // Arrange
        var factory = new FOutrasPartesClienteFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("opcCodigo", typeof(int));
        mockDataTable.Columns.Add("opcNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["opcCodigo"] = testId;
        row["opcNome"] = "Test OutrasPartesCliente";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["opcCodigo"] = 123;
        row["opcNome"] = "Test OutrasPartesCliente Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FOutrasPartesCliente properties
        }
    }
