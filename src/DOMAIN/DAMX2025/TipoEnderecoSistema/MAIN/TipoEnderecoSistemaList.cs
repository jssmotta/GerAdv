...
#region ListarDados_TipoEnderecoSistema
    public static async IAsyncEnumerable<DBTipoEnderecoSistema> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoEnderecoSistema(dbRec);
        }
    }

    public static IEnumerable<DBTipoEnderecoSistema> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
