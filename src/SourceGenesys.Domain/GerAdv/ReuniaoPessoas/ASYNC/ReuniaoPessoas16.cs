...
public partial class DBReuniaoPessoas
{
    ...
    /// </summary>
    public static async Task<DBReuniaoPessoas> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBReuniaoPessoas();
...
#if (!NOTSTORED_ReuniaoPessoas)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFReuniao || pFldFOperador;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFReuniao)
            updateTool.Fields(DBReuniaoPessoasDicInfo.Reuniao, m_FReuniao, ETiposCampos.FNumber);
        if (pFldFOperador)
            updateTool.Fields(DBReuniaoPessoasDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBReuniaoPessoasDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBReuniaoPessoasDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBReuniaoPessoasDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBReuniaoPessoasDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBReuniaoPessoasDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
