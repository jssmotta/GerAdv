...
import { ClientesApiError } from '../GerAdv_TS/Clientes/Apis/ApiClientes';
import { ClientesApi } from '@/app/GerAdv_TS/Clientes/Apis/ApiClientes';
import { FilterClientes } from '../GerAdv_TS/Clientes/Filters/Clientes';
...
describe('ClientesApi', () => {
  ...
  beforeEach(() => {
    ...
    clientesApi = new ClientesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(clientesApi).toBeInstanceOf(ClientesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ClientesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ClientesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all clientes items successfully', async () => {
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(clientesApi.getAll()).rejects.toThrow(ClientesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Clientes_last_getAll_${200}`)));
    });
    ...
    it('should throw ClientesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockClientes = ClientesTestEmpty();
    ...
    it('should fetch clientes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await clientesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockClientes));
  ...
  describe('filter', () => {
    const mockFilter: FilterClientes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter clientes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await clientesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new clientes successfully', async () => {
      const responseClientes = { ...mockClientes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseClientes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await clientesApi.addAndUpdate(mockClientes);
    ...
    it('should update existing clientes successfully', async () => {
      const existingClientes = { ...mockClientes, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await clientesApi.addAndUpdate(existingClientes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(clientesApi.addAndUpdate(mockClientes)).rejects.toThrow(ClientesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Clientes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete clientes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await clientesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(clientesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Clientes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(clientesApi.delete(1)).rejects.toThrow(ClientesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterClientes = { funcionario: 1 };
      
      const result = clientesApi.useFilter(mockFilter);
  ...
  describe('ClientesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ClientesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ClientesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ClientesApiError('Test message', 500);
