...
public partial class ClientesController(IClientesService clientesService) : ControllerBase
{
    ...
    {
        _logger.Info("Clientes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _clientesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Clientes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _clientesService.UpdateColumns(parameters, uri);
        if (!result)
        {
