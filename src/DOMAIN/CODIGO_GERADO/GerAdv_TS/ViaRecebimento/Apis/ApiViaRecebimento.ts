...
import { ViaRecebimento } from '../../Models/ViaRecebimento';
...
export class ViaRecebimentoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ViaRecebimentoApiError';
    this.status = status;
    this.code = code;
...
export class ViaRecebimentoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ViaRecebimento`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ViaRecebimento_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ViaRecebimento');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ViaRecebimento_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Via Recebimento com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterViaRecebimento): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ViaRecebimento_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ViaRecebimento');
        }
    }

            public async filter(filtro: FilterViaRecebimento): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ViaRecebimento_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ViaRecebimento');
                }
            }


    public async validation(regViaRecebimento: IViaRecebimento): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regViaRecebimento as ViaRecebimento, this.getHeaders());
    ...
    public async addAndUpdate(regViaRecebimento: IViaRecebimento): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regViaRecebimento as ViaRecebimento, this.getHeaders());
            var register = result.data as IViaRecebimento;        
            const action = regViaRecebimento.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Via Recebimento');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ViaRecebimento. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ViaRecebimento',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Via Recebimento com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterViaRecebimento) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ViaRecebimento[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de viarecebimento:', error);
            this.notificationService.notify({
              entity: 'ViaRecebimento',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos viarecebimento'
            });
          }
        });
    }
