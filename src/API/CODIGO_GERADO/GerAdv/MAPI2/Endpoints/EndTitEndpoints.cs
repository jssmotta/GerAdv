...
public static class EndTitEndpoints
{
    ...
    private static void MapEndTitRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IEndTitValidation validation, IEndTitWriter writer, IEndTitService service) =>
        {
            logger.LogInfo("EndTit", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("EndTit", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("EndTit_GetColumns").WithDisplayName("Get EndTit Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IEndTitValidation validation, IEndTitWriter writer, IEndTitService service) =>
        {
            logger.LogInfo("EndTit", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("EndTit", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("EndTit_UpdateColumns").WithDisplayName("Update EndTit Columns");
