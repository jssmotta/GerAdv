...
export const useStatusInstanciaForm = (
  ...
  const loadStatusInstancia = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Status Instancia";
...
export const useStatusInstanciaList = (
  dataService: IStatusInstanciaService,
  currentFilter?: FilterStatusInstancia | undefined | null,
) => {
  const [data, setData] = useState<IStatusInstancia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterStatusInstancia | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterStatusInstancia | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar statusinstancia";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsStatusInstancia() {
  ...
  function validate(data: IStatusInstancia): {
    isValid: boolean;
    message: string;
  } {
