...
import { ContatoCRM } from '../../Models/ContatoCRM';
...
export class ContatoCRMApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class ContatoCRMApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ContatoCRM`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRM_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ContatoCRM');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRM_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Contato C R M com ID ${id}`);
        }
    }

            public async filter(filtro: FilterContatoCRM): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRM_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ContatoCRM');
                }
            }


    public async validation(regContatoCRM: IContatoCRM): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regContatoCRM as ContatoCRM, this.getHeaders());
    ...
    public async addAndUpdate(regContatoCRM: IContatoCRM): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regContatoCRM as ContatoCRM, this.getHeaders());
            var register = result.data as IContatoCRM;        
            const action = regContatoCRM.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Contato C R M');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ContatoCRM. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ContatoCRM',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Contato C R M com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterContatoCRM) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ContatoCRM[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de contatocrm:', error);
            this.notificationService.notify({
              entity: 'ContatoCRM',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos contatocrm'
            });
          }
        });
    }
