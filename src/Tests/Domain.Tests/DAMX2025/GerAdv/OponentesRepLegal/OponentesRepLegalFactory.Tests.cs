...
public class FOponentesRepLegalFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FOponentesRepLegalFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FOponentesRepLegalFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FOponentesRepLegalFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "oprCodigo")
        var dt = new DataTable();
        dt.Columns.Add("oprCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["oprCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FOponentesRepLegal>();
    }
    ...
    {
        // Arrange
        var factory = new FOponentesRepLegalFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("oprCodigo", typeof(int));
        mockDataTable.Columns.Add("oprNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["oprCodigo"] = testId;
        row["oprNome"] = "Test OponentesRepLegal";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["oprCodigo"] = 123;
        row["oprNome"] = "Test OponentesRepLegal Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FOponentesRepLegal properties
        }
    }
