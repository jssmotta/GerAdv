...
public partial class DBObjetos
{
    ...
    /// </summary>
    public static async Task<DBObjetos> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBObjetos();
...
#if (!NOTSTORED_Objetos)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFJustica || pFldFArea || pFldFNome || pFldFBold || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFJustica)
            updateTool.Fields(DBObjetosDicInfo.Justica, FJustica, EGenericTypeFields.FNumber);
        if (pFldFArea)
            updateTool.Fields(DBObjetosDicInfo.Area, FArea, EGenericTypeFields.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBObjetosDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFBold || updateTool.Insert)
            updateTool.Fields(DBObjetosDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            updateTool.Fields(DBObjetosDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBObjetosDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBObjetosDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBObjetosDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBObjetosDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBObjetosDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBObjetosDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
