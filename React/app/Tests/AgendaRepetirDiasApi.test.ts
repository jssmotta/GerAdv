...
import { AgendaRepetirDiasApiError } from '../GerAdv_TS/AgendaRepetirDias/Apis/ApiAgendaRepetirDias';
import { AgendaRepetirDiasApi } from '@/app/GerAdv_TS/AgendaRepetirDias/Apis/ApiAgendaRepetirDias';
import { FilterAgendaRepetirDias } from '../GerAdv_TS/AgendaRepetirDias/Filters/AgendaRepetirDias';
import { IAgendaRepetirDias } from '../GerAdv_TS/AgendaRepetirDias/Interfaces/interface.AgendaRepetirDias';
import { AgendaRepetirDiasTestEmpty } from '../GerAdv_TS/Models/AgendaRepetirDias';
...
describe('AgendaRepetirDiasApi', () => {
  let agendarepetirdiasApi: AgendaRepetirDiasApi;
  ...
  beforeEach(() => {
    ...
    agendarepetirdiasApi = new AgendaRepetirDiasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendarepetirdiasApi).toBeInstanceOf(AgendaRepetirDiasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaRepetirDiasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaRepetirDiasApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [AgendaRepetirDiasTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all agendarepetirdias items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarepetirdiasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendarepetirdiasApi.getAll()).rejects.toThrow(AgendaRepetirDiasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendarepetirdiasApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaRepetirDias_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetirDias',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw AgendaRepetirDiasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaRepetirDias = AgendaRepetirDiasTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAgendaRepetirDias,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch agendarepetirdias by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarepetirdiasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaRepetirDias));
      ...
      expect(result.data).toEqual(mockAgendaRepetirDias);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetirDias',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaRepetirDias = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendarepetirdias items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendarepetirdiasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaRepetirDias: IAgendaRepetirDias = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendarepetirdias successfully', async () => {
      const responseAgendaRepetirDias = { ...mockAgendaRepetirDias, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaRepetirDias,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarepetirdiasApi.addAndUpdate(mockAgendaRepetirDias);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockAgendaRepetirDias,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing agendarepetirdias successfully', async () => {
      const existingAgendaRepetirDias = { ...mockAgendaRepetirDias, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaRepetirDias,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarepetirdiasApi.addAndUpdate(existingAgendaRepetirDias);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendarepetirdiasApi.addAndUpdate(mockAgendaRepetirDias)).rejects.toThrow(AgendaRepetirDiasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetirDias',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendarepetirdias successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendarepetirdiasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendarepetirdiasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRepetirDias',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendarepetirdiasApi.delete(1)).rejects.toThrow(AgendaRepetirDiasApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaRepetirDias = { funcionario: 1 };
      
      const result = agendarepetirdiasApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await agendarepetirdiasApi.delete(1);
  ...
  describe('AgendaRepetirDiasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaRepetirDiasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaRepetirDiasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaRepetirDiasApiError('Test message', 500);
