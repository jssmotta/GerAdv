...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('processoutputrequest-columns-order-mobile'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('processoutputrequest-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('processoutputrequest-columns-order-mobile'));
}, []);
...
{initialized && (
  <Grid
  className='grid-mobile-crud grid-mobile-processoutputrequest'
