// app/TestsForms/ViaRecebimentoComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import ViaRecebimentoComboBox from '@/app/GerAdv_TS/ViaRecebimento/ComboBox/ViaRecebimento';
...
import { ViaRecebimentoEmpty, ViaRecebimentoTestEmpty } from '@/app/GerAdv_TS/Models/ViaRecebimento';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/ViaRecebimento/Hooks/hookViaRecebimento', () => ({
  useViaRecebimentoComboBox: jest.fn(() => ({
    options: [
    { ...ViaRecebimentoEmpty(), id: 1, nome: 'Via Recebimento 1' },
    { ...ViaRecebimentoEmpty(), id: 2, nome: 'Via Recebimento 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/ViaRecebimento/Apis/ApiViaRecebimento', () => ({
  ViaRecebimentoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ViaRecebimento' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ViaRecebimento' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/ViaRecebimento/Services/ViaRecebimento.service', () => ({
  ViaRecebimentoService: jest.fn().mockImplementation(() => ({
    fetchViaRecebimentoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock ViaRecebimento' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/ViaRecebimento/Crud/Grids/ViaRecebimentoWindow', () => {
  return function MockViaRecebimentoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='viarecebimento-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Via Recebimento' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...ViaRecebimentoTestEmpty(), id: 1, nome: 'Via Recebimento 1' },
  { ...ViaRecebimentoTestEmpty(), id: 2, nome: 'Via Recebimento 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Via Recebimento',
  name: 'viarecebimento',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: ViaRecebimentoEmpty(), 
};
describe('ViaRecebimentoComboBox', () => {
  ...
  const { useViaRecebimentoComboBox } = require('@/app/GerAdv_TS/ViaRecebimento/Hooks/hookViaRecebimento');
  (useViaRecebimentoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<ViaRecebimentoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Via Recebimento')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Via Recebimento')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...ViaRecebimentoTestEmpty(), id: 1, nome: 'Via Recebimento 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useViaRecebimentoComboBox } = require('@/app/GerAdv_TS/ViaRecebimento/Hooks/hookViaRecebimento');
    useViaRecebimentoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens ViaRecebimentoWindow on action button click (add)', async () => {
    render(<ViaRecebimentoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('viarecebimento-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('viarecebimento-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<ViaRecebimentoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles ViaRecebimentoWindow onClose and onSuccess', () => {
    render(<ViaRecebimentoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<ViaRecebimentoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
