...
import OperadorEMailPopupWindow from '../Crud/Grids/OperadorEMailPopupWindow';
...
import { useOperadorEMailPopupComboBox } from '../Hooks/hookOperadorEMailPopup';
const OperadorEMailPopupComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'operadoremailpopupInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useOperadorEMailPopupComboBox(operadoremailpopupService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IOperadorEMailPopup | null>(null);
const [addRecord, setAddRecord] = useState<IOperadorEMailPopup | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await operadoremailpopupService.fetchOperadorEMailPopupById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = OperadorEMailPopupEmpty();
...
const handleSuccess = (newOperadorEMailPopup?: IOperadorEMailPopup) => {
  if (newOperadorEMailPopup) {
    setValue({ id: newOperadorEMailPopup.id, nome: newOperadorEMailPopup.nome });
    loadRecordForEdit(newOperadorEMailPopup.id);
    handleValueChange(newOperadorEMailPopup);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .operadoremailpopup-dropdown-popup-msi {
  ...
  .operadoremailpopup-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .operadoremailpopup-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .operadoremailpopup-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedOperadorEMailPopup={editRecord || addRecord || OperadorEMailPopupEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'operadoremailpopup-dropdown-popup-msi'
          }}
          />
...
export default OperadorEMailPopupComboBox;