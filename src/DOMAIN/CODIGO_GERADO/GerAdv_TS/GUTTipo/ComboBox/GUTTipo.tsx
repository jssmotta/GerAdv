...
const GUTTipoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IGUTTipo | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IGUTTipo | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IGUTTipo);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <GUTTipoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedGUTTipo={editar ?? addValue ?? GUTTipoEmpty()} /> 
            )}
