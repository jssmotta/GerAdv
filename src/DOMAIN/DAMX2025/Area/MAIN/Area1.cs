...
#region TableDefinition_Area
...
#endregion
    public DBArea(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBArea(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBArea(in string? sqlWhere, SqlConnection? oCnn)
    {
