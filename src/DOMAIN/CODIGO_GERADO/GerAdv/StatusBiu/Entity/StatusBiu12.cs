...
public partial class FStatusBiu : MenphisSI.SG.GerAdv.DBStatusBiu, IDBStatusBiu
{
    // DI-compatible default constructor
    public FStatusBiu() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FStatusBiu CreateFromReader(SqlDataReader? dbRec)
    {
        return new FStatusBiu(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FStatusBiu CreateFromDataRow(DataRow? dbRec)
    {
        return new FStatusBiu(dbRec);
    }

    private FStatusBiu(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FStatusBiu(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FStatusBiu> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fStatusBiu = new FStatusBiu();
        await fStatusBiu.CarregarAsync(id, oCnn);
        return fStatusBiu;
    }

    private FStatusBiu(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FStatusBiu CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FStatusBiu(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
