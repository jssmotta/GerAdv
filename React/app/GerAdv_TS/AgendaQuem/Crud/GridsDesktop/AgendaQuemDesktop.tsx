...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('agendaquem-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('agendaquem-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('agendaquem-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-agendaquem',
  columns: exportColumns, 
  sheetName: 'AgendaQuem'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-AgendaQuem-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-agendaquem',
  columns: exportColumnsPdf, 
  title: 'AgendaQuem'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-AgendaQuem-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`agendaquem-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-agendaquem'
