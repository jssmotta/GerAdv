...
export const useProObservacoesForm = (
  ...
  const loadProObservacoes = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Pro Observacoes";
...
export const useProObservacoesList = (
  dataService: IProObservacoesService,
  currentFilter?: FilterProObservacoes | undefined | null,
) => {
  const [data, setData] = useState<IProObservacoes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProObservacoes | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterProObservacoes | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar proobservacoes";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProObservacoes() {
  ...
  function validate(data: IProObservacoes): {
    isValid: boolean;
    message: string;
  } {
