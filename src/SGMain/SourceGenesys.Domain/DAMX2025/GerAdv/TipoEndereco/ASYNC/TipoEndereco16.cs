...
public partial class DBTipoEndereco
{
    ...
    /// </summary>
    public static async Task<DBTipoEndereco> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBTipoEndereco();
...
#if (!NOTSTORED_TipoEndereco)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFDescricao || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFDescricao)
            updateTool.Fields(DBTipoEnderecoDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFGuid)
            updateTool.Fields(DBTipoEnderecoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBTipoEnderecoDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBTipoEnderecoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
