...
export const useProTipoBaixaList = (dataService: IProTipoBaixaService, currentFilter?: FilterProTipoBaixa | undefined | null) => {
  const [data, setData] = useState<IProTipoBaixa[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterProTipoBaixa | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterProTipoBaixa | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterProTipoBaixa);
      setData(result);
    } catch (err) {
