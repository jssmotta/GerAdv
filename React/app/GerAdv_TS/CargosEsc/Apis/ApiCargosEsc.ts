...
export class CargosEscApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class CargosEscApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/CargosEsc`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEsc_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os CargosEsc');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEsc_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Cargos Esc com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEsc_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de CargosEsc');
        }
    }

            public async filter(filtro: FilterCargosEsc): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEsc_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar CargosEsc');
                }
            }


    public async validation(regCargosEsc: ICargosEsc): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regCargosEsc as CargosEsc, this.getHeaders());
    ...
    public async addAndUpdate(regCargosEsc: ICargosEsc): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Cargos Esc');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o CargosEsc. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'CargosEsc',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Cargos Esc com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterCargosEsc) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<CargosEsc[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de cargosesc:', error);
            this.notificationService.notify({
              entity: 'CargosEsc',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos cargosesc'
            });
          }
        });
    }
