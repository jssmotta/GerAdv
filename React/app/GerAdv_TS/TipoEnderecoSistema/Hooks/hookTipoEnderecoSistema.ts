...
export const useTipoEnderecoSistemaForm = (
  ...
  const loadTipoEnderecoSistema = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Endereco Sistema";
...
export const useTipoEnderecoSistemaList = (
  dataService: ITipoEnderecoSistemaService,
  currentFilter?: FilterTipoEnderecoSistema | undefined | null,
) => {
  const [data, setData] = useState<ITipoEnderecoSistema[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoEnderecoSistema | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterTipoEnderecoSistema | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipoenderecosistema";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoEnderecoSistema() {
  async function runValidation(
    ...
    const tipoenderecosistemaApi = new TipoEnderecoSistemaApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: ITipoEnderecoSistema): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.nome.length > 150) {
        return {
          isValid: false,
          message: "O campo Nome não pode ter mais de 150 caracteres.",
        };
      }
