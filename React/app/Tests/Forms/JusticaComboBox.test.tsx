// app/TestsForms/JusticaComboBox.test.tsx - Vers�o Corrigida
...
import { JusticaEmpty, JusticaTestEmpty } from '@/app/GerAdv_TS/Models/Justica';
...
jest.mock('@/app/GerAdv_TS/Justica/Apis/ApiJustica', () => ({
  JusticaApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Justica' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Justica' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Justica/Services/Justica.service', () => ({
  JusticaService: jest.fn().mockImplementation(() => ({
    fetchJusticaById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Justica' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Justica/Crud/Grids/JusticaWindow', () => {
  return function MockJusticaWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='justica-window'>
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: JusticaEmpty(), 
};
describe('JusticaComboBox', () => {
  ...
  const { useJusticaComboBox } = require('@/app/GerAdv_TS/Justica/Hooks/hookJustica');
  (useJusticaComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('shows loading state', () => {
    // Mock loading state
    const { useJusticaComboBox } = require('@/app/GerAdv_TS/Justica/Hooks/hookJustica');
    useJusticaComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens JusticaWindow on action button click (add)', async () => {
    render(<JusticaComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('justica-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('justica-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<JusticaComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles JusticaWindow onClose and onSuccess', () => {
    render(<JusticaComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<JusticaComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
