...
import { OperadorGrupoApiError } from '../GerAdv_TS/OperadorGrupo/Apis/ApiOperadorGrupo';
import { OperadorGrupoApi } from '@/app/GerAdv_TS/OperadorGrupo/Apis/ApiOperadorGrupo';
import { FilterOperadorGrupo } from '../GerAdv_TS/OperadorGrupo/Filters/OperadorGrupo';
...
describe('OperadorGrupoApi', () => {
  let operadorgrupoApi: OperadorGrupoApi;
  ...
  beforeEach(() => {
    ...
    operadorgrupoApi = new OperadorGrupoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(operadorgrupoApi).toBeInstanceOf(OperadorGrupoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OperadorGrupoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OperadorGrupoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all operadorgrupo items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorgrupoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(operadorgrupoApi.getAll()).rejects.toThrow(OperadorGrupoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await operadorgrupoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-OperadorGrupo_last_getAll_${200}`)));
    });
    ...
    it('should throw OperadorGrupoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOperadorGrupo = OperadorGrupoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockOperadorGrupo,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch operadorgrupo by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadorgrupoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOperadorGrupo));
      ...
      expect(result.data).toEqual(mockOperadorGrupo);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupo',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterOperadorGrupo = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter operadorgrupo items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await operadorgrupoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockOperadorGrupo: IOperadorGrupo = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new operadorgrupo successfully', async () => {
      const responseOperadorGrupo = { ...mockOperadorGrupo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOperadorGrupo,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorgrupoApi.addAndUpdate(mockOperadorGrupo);
    ...
    it('should update existing operadorgrupo successfully', async () => {
      const existingOperadorGrupo = { ...mockOperadorGrupo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOperadorGrupo,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadorgrupoApi.addAndUpdate(existingOperadorGrupo);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(operadorgrupoApi.addAndUpdate(mockOperadorGrupo)).rejects.toThrow(OperadorGrupoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupo',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete operadorgrupo successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await operadorgrupoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(operadorgrupoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorGrupo',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(operadorgrupoApi.delete(1)).rejects.toThrow(OperadorGrupoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterOperadorGrupo = { funcionario: 1 };
      
      const result = operadorgrupoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await operadorgrupoApi.delete(1);
  ...
  describe('OperadorGrupoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OperadorGrupoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OperadorGrupoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OperadorGrupoApiError('Test message', 500);
