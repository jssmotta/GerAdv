...
public static class ServicosEndpoints
{
    ...
    private static void MapServicosRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IServicosValidation validation, IServicosWriter writer, IServicosService service) =>
        {
            logger.LogInfo("Servicos", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("Servicos", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("Servicos_GetColumns").WithDisplayName("Get Servicos Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IServicosValidation validation, IServicosWriter writer, IServicosService service) =>
        {
            logger.LogInfo("Servicos", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("Servicos", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("Servicos_UpdateColumns").WithDisplayName("Update Servicos Columns");
