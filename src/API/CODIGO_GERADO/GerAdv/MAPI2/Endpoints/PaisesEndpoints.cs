...
public static class PaisesEndpoints
{
    ...
    private static void MapPaisesRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IPaisesValidation validation, IPaisesWriter writer, IPaisesService service) =>
        {
            logger.LogInfo("Paises", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("Paises", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("Paises_GetColumns").WithDisplayName("Get Paises Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IPaisesValidation validation, IPaisesWriter writer, IPaisesService service) =>
        {
            logger.LogInfo("Paises", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("Paises", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("Paises_UpdateColumns").WithDisplayName("Update Paises Columns");
