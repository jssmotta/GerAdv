...
// useEnderecosGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de Enderecos
"use client";
...
interface UseEnderecosGridProps {
  data: IEnderecos[];
...
export const useEnderecosGrid = ({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa("enderecos-columns-order"));
      if (savedOrder) {
  ...
  const filterLogic = useCallback(
    (data: IEnderecos, filters: Record<string, any>) => {
      const descricaoMatches = applyFilter(
        data,
        "descricao",
        filters.descricao,
      );
