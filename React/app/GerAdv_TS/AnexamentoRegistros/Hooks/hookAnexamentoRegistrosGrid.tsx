...
// useAnexamentoRegistrosGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de AnexamentoRegistros
'use client';
...
interface UseAnexamentoRegistrosGridProps {
...
export const useAnexamentoRegistrosGrid = ({
  data, 
  onRowClick, 
  onDeleteClick, 
  setSelectedId, 
  initialTake = 10, 
  useCustomSort = false
}: UseAnexamentoRegistrosGridProps) => {
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('anexamentoregistros-columns-order'), JSON.stringify(reorderableFields));
