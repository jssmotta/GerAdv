...
public static class FilterAtividadesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Atividades em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterAtividades:

- O filter irá filtrar os Atividades conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterAtividades:(Atividades)
- descricao: _nome ou descrição para Atividades;
...
- Se ouvir "todos os Atividades" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterAtividades? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Atividades length: {result.Length}");
#endif
