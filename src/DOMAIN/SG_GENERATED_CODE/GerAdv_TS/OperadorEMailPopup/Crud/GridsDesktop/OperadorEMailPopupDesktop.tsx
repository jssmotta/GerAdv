...
'rnomeoperador']);
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('operadoremailpopup-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('operadoremailpopup-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
