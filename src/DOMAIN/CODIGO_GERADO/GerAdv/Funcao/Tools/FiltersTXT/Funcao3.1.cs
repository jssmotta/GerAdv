...
public static class FilterFuncaoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterFuncao? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Funcao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterFuncao:

- O filter irá filtrar os Funcao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Funcao" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Funcao length: {result.Length}");
#endif
