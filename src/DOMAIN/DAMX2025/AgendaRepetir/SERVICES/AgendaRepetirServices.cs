...
{
    [XmlIgnore]
    public DateTime FDataFinalWithHora
    {
        set
        {
            pFldFDataFinal = true;
    ...
    [XmlIgnore]
    public string MDataFinalDataX_DataHora => $"{m_FDataFinal:dd/MM/yyyy HH:mm:ss}";

    [XmlIgnore]
    public string MDataFinalX_Hora => $"{m_FDataFinal:HH:mm:ss}";
...
#if (forWeb)
[XmlIgnore]
public string TRClick => DevourerOne.TableTrClick(ID, "AGENDAREPETIRINC");
#endif
}