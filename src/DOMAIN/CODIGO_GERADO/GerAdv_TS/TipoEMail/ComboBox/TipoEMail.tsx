...
const TipoEMailComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoEMail | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoEMail | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoEMail);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoEMailWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoEMail={editar ?? addValue ?? TipoEMailEmpty()} /> 
            )}
