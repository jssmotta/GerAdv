...
import { useTipoContatoCRMGrid } from "../../Hooks/hookTipoContatoCRMGrid";
...
export const TipoContatoCRMGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("tipocontatocrm-columns-order"),
        );
        if (savedOrder) {
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("tipocontatocrm-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-tipocontatocrm",
      columns: exportColumns,
      sheetName: "TipoContatoCRM",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-TipoContatoCRM-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-tipocontatocrm",
      columns: exportColumnsPdf,
      title: "TipoContatoCRM",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-TipoContatoCRM-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`tipocontatocrm-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-tipocontatocrm"
