...
import { StatusHTrabApiError } from '../GerAdv_TS/StatusHTrab/Apis/ApiStatusHTrab';
import { StatusHTrabApi } from '@/app/GerAdv_TS/StatusHTrab/Apis/ApiStatusHTrab';
import { FilterStatusHTrab } from '../GerAdv_TS/StatusHTrab/Filters/StatusHTrab';
...
describe('StatusHTrabApi', () => {
  let statushtrabApi: StatusHTrabApi;
  ...
  beforeEach(() => {
    ...
    statushtrabApi = new StatusHTrabApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(statushtrabApi).toBeInstanceOf(StatusHTrabApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new StatusHTrabApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(StatusHTrabApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all statushtrab items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statushtrabApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(statushtrabApi.getAll()).rejects.toThrow(StatusHTrabApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await statushtrabApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-StatusHTrab_last_getAll_${200}`)));
    });
    ...
    it('should throw StatusHTrabApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockStatusHTrab = StatusHTrabTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockStatusHTrab,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch statushtrab by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statushtrabApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockStatusHTrab));
      ...
      expect(result.data).toEqual(mockStatusHTrab);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusHTrab',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterStatusHTrab = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter statushtrab items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await statushtrabApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockStatusHTrab: IStatusHTrab = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new statushtrab successfully', async () => {
      const responseStatusHTrab = { ...mockStatusHTrab, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseStatusHTrab,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statushtrabApi.addAndUpdate(mockStatusHTrab);
    ...
    it('should update existing statushtrab successfully', async () => {
      const existingStatusHTrab = { ...mockStatusHTrab, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingStatusHTrab,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statushtrabApi.addAndUpdate(existingStatusHTrab);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(statushtrabApi.addAndUpdate(mockStatusHTrab)).rejects.toThrow(StatusHTrabApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusHTrab',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete statushtrab successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await statushtrabApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(statushtrabApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusHTrab',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(statushtrabApi.delete(1)).rejects.toThrow(StatusHTrabApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterStatusHTrab = { funcionario: 1 };
      
      const result = statushtrabApi.useFilter(mockFilter);
  ...
  describe('StatusHTrabApiError', () => {
    it('should create error with correct properties', () => {
      const error = new StatusHTrabApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('StatusHTrabApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new StatusHTrabApiError('Test message', 500);
