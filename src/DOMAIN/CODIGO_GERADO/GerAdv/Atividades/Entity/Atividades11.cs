// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Tabela:Atividades
// Fonte:FactoryApplications
namespace MenphisSI.GerAdv.Factory;
public partial class FAtividadesFactory : IFAtividadesFactory, IDisposable
{
    private bool _disposed = false;
    public FAtividades Create()
    {
        ThrowIfDisposed();
        return new FAtividades();
    }

    public FAtividades CreateFromReader(SqlDataReader? dbRec)
    {
        ThrowIfDisposed();
        return FAtividades.CreateFromReader(dbRec);
    }

    public FAtividades CreateFromDataRow(DataRow? dbRec)
    {
        ThrowIfDisposed();
        return FAtividades.CreateFromDataRow(dbRec);
    }

    public async Task<FAtividades> CreateAsync()
    {
        ThrowIfDisposed();
        await Task.Delay(1);
        return new FAtividades();
    }

    public FAtividades CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        ThrowIfDisposed();
        return FAtividades.CreateFromParameters(parameters, oCnn, cNome, fullSql, sqlWhere, join);
    }

    public Task DeleteAsync(int operadorId, int id, MsiSqlConnection? oCnn)
    {
        ThrowIfDisposed();
        return FAtividades.DeleteRecordAsync(operadorId, id, oCnn);
    }

    public async Task<FAtividades> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        ThrowIfDisposed();
        return await FAtividades.CreateFromIdAsync(id, oCnn);
    }

    private void ThrowIfDisposed()
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
    }

    public virtual void Dispose()
    {
        _disposed = true;
        GC.SuppressFinalize(this);
    }
}