...
import StatusInstanciaWindow from '../Crud/Grids/StatusInstanciaWindow';
...
import { useStatusInstanciaComboBox } from '../Hooks/hookStatusInstancia';
const StatusInstanciaComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'statusinstanciaInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useStatusInstanciaComboBox(statusinstanciaService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IStatusInstancia | null>(null);
const [addRecord, setAddRecord] = useState<IStatusInstancia | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await statusinstanciaService.fetchStatusInstanciaById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = StatusInstanciaEmpty();
...
const handleSuccess = (newStatusInstancia?: IStatusInstancia) => {
  if (newStatusInstancia) {
    setValue({ id: newStatusInstancia.id, nome: newStatusInstancia.nome });
    loadRecordForEdit(newStatusInstancia.id);
    handleValueChange(newStatusInstancia);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .statusinstancia-dropdown-popup-msi {
  ...
  .statusinstancia-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .statusinstancia-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .statusinstancia-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedStatusInstancia={editRecord || addRecord || StatusInstanciaEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'statusinstancia-dropdown-popup-msi'
          }}
          />
...
export default StatusInstanciaComboBox;