...
export const useColaboradoresList = (
  dataService: IColaboradoresService,
  currentFilter?: FilterColaboradores | undefined | null,
) => {
  const [data, setData] = useState<IColaboradores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterColaboradores | undefined | null>(
    currentFilter,
  );
...
export function useValidationsColaboradores() {
  ...
  function validate(data: IColaboradores): {
    isValid: boolean;
    message: string;
  } {
