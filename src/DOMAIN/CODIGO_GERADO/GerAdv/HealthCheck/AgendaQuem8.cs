...
public class AgendaQuemHealthCheck(IOptions<AppSettings> appSettings, AgendaQuemService agendaquemService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly AgendaQuemService _agendaquemService = agendaquemService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT AgendaQuem successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT AgendaQuem failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _agendaquemService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-AgendaQuem";
                    ...
                    static async Task<IEnumerable<Models.AgendaQuem>> GetData(string uri, CancellationToken token)
                    {
                        await Task.Delay(1, token);
                        return[new Models.AgendaQuem
                        {
                        }
            ...
            return isHealthy ? HealthCheckResult.Healthy("AgendaQuem is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
