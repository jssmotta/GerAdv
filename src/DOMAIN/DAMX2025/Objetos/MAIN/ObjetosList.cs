...
#region ListarDados_Objetos
    public static async IAsyncEnumerable<DBObjetos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBObjetos(dbRec);
        }
    }

    public static IEnumerable<DBObjetos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
