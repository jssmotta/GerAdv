...
import { useProDepositosGrid } from '../../Hooks/hookProDepositosGrid';
...
'nropastaprocessos',
'descricaofase',
'nometipoprodesposito']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('prodepositos-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('prodepositos-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const basicColumnMap: Record<string, React.ReactElement> = useMemo(() => ({
  ...
  'nometipoprodesposito': (
  <GridColumn
  key='nometipoprodesposito'

  field='nometipoprodesposito'
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('prodepositos-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-prodepositos',
  columns: exportColumns, 
  sheetName: 'ProDepositos'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-ProDepositos-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-prodepositos',
  columns: exportColumnsPdf, 
  title: 'ProDepositos'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-ProDepositos-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`prodepositos-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-prodepositos'
