...
export const useAuditor4KList = (dataService: IAuditor4KService, currentFilter?: FilterAuditor4K | undefined | null) => {
  const [data, setData] = useState<IAuditor4K[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterAuditor4K | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterAuditor4K | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterAuditor4K);
      setData(result);
    } catch (err) {
