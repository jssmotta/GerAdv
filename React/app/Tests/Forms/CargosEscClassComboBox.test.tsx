// app/TestsForms/CargosEscClassComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import CargosEscClassComboBox from '@/app/GerAdv_TS/CargosEscClass/ComboBox/CargosEscClass';
...
import { CargosEscClassEmpty, CargosEscClassTestEmpty } from '@/app/GerAdv_TS/Models/CargosEscClass';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/CargosEscClass/Hooks/hookCargosEscClass', () => ({
  useCargosEscClassComboBox: jest.fn(() => ({
    options: [
    { ...CargosEscClassEmpty(), id: 1, nome: 'Cargos Esc Class 1' },
    { ...CargosEscClassEmpty(), id: 2, nome: 'Cargos Esc Class 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/CargosEscClass/Apis/ApiCargosEscClass', () => ({
  CargosEscClassApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock CargosEscClass' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock CargosEscClass' }),
...
jest.mock('@/app/GerAdv_TS/CargosEscClass/Services/CargosEscClass.service', () => ({
  CargosEscClassService: jest.fn().mockImplementation(() => ({
    fetchCargosEscClassById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock CargosEscClass' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/CargosEscClass/Crud/Grids/CargosEscClassWindow', () => {
  return function MockCargosEscClassWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='cargosescclass-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Cargos Esc Class' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...CargosEscClassTestEmpty(), id: 1, nome: 'Cargos Esc Class 1' },
  { ...CargosEscClassTestEmpty(), id: 2, nome: 'Cargos Esc Class 2' },
];
// Props padr�o para o componente
const defaultProps = {
  label: 'Cargos Esc Class',
  name: 'cargosescclass',
  value: 0, 
  setValue: jest.fn(), 
  dataForm: CargosEscClassEmpty(), 
};
describe('CargosEscClassComboBox', () => {
  ...
  const { useCargosEscClassComboBox } = require('@/app/GerAdv_TS/CargosEscClass/Hooks/hookCargosEscClass');
  (useCargosEscClassComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<CargosEscClassComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Cargos Esc Class')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Cargos Esc Class')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...CargosEscClassTestEmpty(), id: 1, nome: 'Cargos Esc Class 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useCargosEscClassComboBox } = require('@/app/GerAdv_TS/CargosEscClass/Hooks/hookCargosEscClass');
    useCargosEscClassComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens CargosEscClassWindow on action button click (add)', async () => {
    render(<CargosEscClassComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('cargosescclass-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('cargosescclass-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<CargosEscClassComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles CargosEscClassWindow onClose and onSuccess', () => {
    render(<CargosEscClassComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<CargosEscClassComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
