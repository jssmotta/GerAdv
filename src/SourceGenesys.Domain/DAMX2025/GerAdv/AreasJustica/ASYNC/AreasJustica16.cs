...
public partial class DBAreasJustica
{
    ...
    /// </summary>
    public static async Task<DBAreasJustica> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAreasJustica();
...
#if (!NOTSTORED_AreasJustica)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFArea || pFldFJustica;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFArea)
            updateTool.Fields(DBAreasJusticaDicInfo.Area, m_FArea, ETiposCampos.FNumber);
        if (pFldFJustica)
            updateTool.Fields(DBAreasJusticaDicInfo.Justica, m_FJustica, ETiposCampos.FNumber);
    }
