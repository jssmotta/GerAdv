...
const EnderecosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IEnderecos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IEnderecos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IEnderecos);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <EnderecosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedEnderecos={editar ?? addValue ?? EnderecosEmpty()} /> 
            )}
