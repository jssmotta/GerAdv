// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Entity:Funcionarios
// Source:FactoryApplications
namespace MenphisSI.GerAdv.Factory;
public partial class FFuncionariosFactory : IFFuncionariosFactory, IDisposable
{
    private bool _disposed = false;
    public FFuncionarios Create()
    {
        ThrowIfDisposed();
        return new FFuncionarios();
    }

    public FFuncionarios CreateFromReader(SqlDataReader? dbRec)
    {
        ThrowIfDisposed();
        return FFuncionarios.CreateFromReader(dbRec);
    }

    public FFuncionarios CreateFromDataRow(DataRow? dbRec)
    {
        ThrowIfDisposed();
        return FFuncionarios.CreateFromDataRow(dbRec);
    }

    public async Task<FFuncionarios> CreateAsync()
    {
        ThrowIfDisposed();
        await Task.Delay(1);
        return new FFuncionarios();
    }

    public FFuncionarios CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        ThrowIfDisposed();
        return FFuncionarios.CreateFromParameters(parameters, oCnn, cNome, fullSql, sqlWhere, join);
    }

    public Task DeleteAsync(int operadorId, int id, MsiSqlConnection? oCnn)
    {
        ThrowIfDisposed();
        return FFuncionarios.DeleteRecordAsync(operadorId, id, oCnn);
    }

    public async Task<FFuncionarios> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        ThrowIfDisposed();
        return await FFuncionarios.CreateFromIdAsync(id, oCnn);
    }

    private void ThrowIfDisposed()
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
    }

    public virtual void Dispose()
    {
        _disposed = true;
        GC.SuppressFinalize(this);
    }
}