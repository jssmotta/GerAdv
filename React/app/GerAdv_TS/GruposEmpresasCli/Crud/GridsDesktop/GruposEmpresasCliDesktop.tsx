...
import { useGruposEmpresasCliGrid } from '../../Hooks/hookGruposEmpresasCliGrid';
...
const [columnsOrder, setColumnsOrder] = useState<string[]>(['descricaogruposempresas',
'nomeclientes']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('gruposempresascli-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('gruposempresascli-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const basicColumnMap: Record<string, React.ReactElement> = useMemo(() => ({
  'descricaogruposempresas': (
  <GridColumn
  key='descricaogruposempresas'

  field='descricaogruposempresas'
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('gruposempresascli-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-gruposempresascli',
  columns: exportColumns, 
  sheetName: 'GruposEmpresasCli'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-GruposEmpresasCli-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-gruposempresascli',
  columns: exportColumnsPdf, 
  title: 'GruposEmpresasCli'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-GruposEmpresasCli-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`gruposempresascli-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-gruposempresascli'
