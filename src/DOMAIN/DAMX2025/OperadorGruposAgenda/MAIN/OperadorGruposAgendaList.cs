...
#region ListarDados_OperadorGruposAgenda
    public static async IAsyncEnumerable<DBOperadorGruposAgenda> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperadorGruposAgenda(dbRec);
        }
    }

    public static IEnumerable<DBOperadorGruposAgenda> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
