...
export const useOutrasPartesClienteForm = (
  ...
  const loadOutrasPartesCliente = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialOutrasPartesCliente);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Outras Partes Cliente';
      setError(errorMessage);
      console.log('Erro ao carregar Outras Partes Cliente');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialOutrasPartesCliente]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadOutrasPartesCliente, resetForm]);
  return returnValue;
};


export const useOutrasPartesClienteNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('OutrasPartesCliente', (entity) => {
      try {
...
export const useOutrasPartesClienteList = (dataService: IOutrasPartesClienteService) => {
  const [data, setData] = useState<IOutrasPartesCliente[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar outraspartescliente';
      setError(errorMessage);
      console.log('Erro ao carregar outraspartescliente');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useOutrasPartesClienteNotifications(
...
if (data.nomefantasia.length > 255) { 
                                             return { isValid: false, message: 'O campo NomeFantasia não pode ter mais de 255 caracteres.' };
                                         } 
