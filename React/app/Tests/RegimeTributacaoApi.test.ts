...
import { RegimeTributacaoApiError } from '../GerAdv_TS/RegimeTributacao/Apis/ApiRegimeTributacao';
import { RegimeTributacaoApi } from '@/app/GerAdv_TS/RegimeTributacao/Apis/ApiRegimeTributacao';
import { FilterRegimeTributacao } from '../GerAdv_TS/RegimeTributacao/Filters/RegimeTributacao';
...
describe('RegimeTributacaoApi', () => {
  let regimetributacaoApi: RegimeTributacaoApi;
  ...
  beforeEach(() => {
    ...
    regimetributacaoApi = new RegimeTributacaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(regimetributacaoApi).toBeInstanceOf(RegimeTributacaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new RegimeTributacaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(RegimeTributacaoApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [RegimeTributacaoTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all regimetributacao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await regimetributacaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(regimetributacaoApi.getAll()).rejects.toThrow(RegimeTributacaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await regimetributacaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-RegimeTributacao_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'RegimeTributacao',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw RegimeTributacaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockRegimeTributacao = RegimeTributacaoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockRegimeTributacao,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch regimetributacao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await regimetributacaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockRegimeTributacao));
      ...
      expect(result.data).toEqual(mockRegimeTributacao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'RegimeTributacao',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterRegimeTributacao = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter regimetributacao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await regimetributacaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockRegimeTributacao: IRegimeTributacao = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new regimetributacao successfully', async () => {
      const responseRegimeTributacao = { ...mockRegimeTributacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseRegimeTributacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await regimetributacaoApi.addAndUpdate(mockRegimeTributacao);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockRegimeTributacao,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing regimetributacao successfully', async () => {
      const existingRegimeTributacao = { ...mockRegimeTributacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingRegimeTributacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await regimetributacaoApi.addAndUpdate(existingRegimeTributacao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(regimetributacaoApi.addAndUpdate(mockRegimeTributacao)).rejects.toThrow(RegimeTributacaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'RegimeTributacao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete regimetributacao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await regimetributacaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(regimetributacaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'RegimeTributacao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(regimetributacaoApi.delete(1)).rejects.toThrow(RegimeTributacaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterRegimeTributacao = { funcionario: 1 };
      
      const result = regimetributacaoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await regimetributacaoApi.delete(1);
  ...
  describe('RegimeTributacaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new RegimeTributacaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('RegimeTributacaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new RegimeTributacaoApiError('Test message', 500);
