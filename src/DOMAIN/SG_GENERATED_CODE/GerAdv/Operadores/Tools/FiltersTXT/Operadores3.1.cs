...
public static class FilterOperadoresTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Operadores em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterOperadores:

- O filter irá filtrar os Operadores conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterOperadores:(Operadores)
- enviado: Enviado;
- casa: Casa;
- casaid: número ou IDs;
- casacodigo: número ou IDs;
- isnovo: IsNovo;
- cliente: número ou IDs;
- grupo: número ou IDs;
- nome: _nome ou descrição para Operadores;
- email: EMail;
- senha: Senha;
- ativado: Ativado;
...
- Se ouvir "todos os Operadores" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterOperadores? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Operadores length: {result.Length}");
#endif
