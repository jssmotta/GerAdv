...
export const useViaRecebimentoForm = (
  ...
  const loadViaRecebimento = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialViaRecebimento);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Via Recebimento';
      setError(errorMessage);
      console.log('Erro ao carregar Via Recebimento');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialViaRecebimento]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadViaRecebimento, resetForm]);
  return returnValue;
};


export const useViaRecebimentoNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('ViaRecebimento', (entity) => {
      try {
...
export const useViaRecebimentoList = (dataService: IViaRecebimentoService) => {
  const [data, setData] = useState<IViaRecebimento[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar viarecebimento';
      setError(errorMessage);
      console.log('Erro ao carregar viarecebimento');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useViaRecebimentoNotifications(
