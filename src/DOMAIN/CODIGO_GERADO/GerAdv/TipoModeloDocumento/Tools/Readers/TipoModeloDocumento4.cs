...
public partial interface ITipoModeloDocumentoReader
{
    ...
    TipoModeloDocumentoResponse? Read(Entity.DBTipoModeloDocumento dbRec, MsiSqlConnection oCnn);
    ...
    TipoModeloDocumentoResponseAll? ReadAll(DBTipoModeloDocumento dbRec, SqlDataReader dr);
    IEnumerable<DBNomeID> ListarN(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
    IEnumerable<TipoModeloDocumentoResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order);
}
...
public partial class TipoModeloDocumento : ITipoModeloDocumentoReader
{
    ...
    public IEnumerable<TipoModeloDocumentoResponseAll> Listar(int max, string uri, string cWhere, List<SqlParameter> parameters, string order) => ListarTabela(BuildSqlQuery("*", cWhere, order, max), parameters, uri, caching: DevourerOne.PCachingDefault, max: max);
    private IEnumerable<TipoModeloDocumentoResponseAll> ListarTabela(string sql, List<SqlParameter> parameters, string uri, bool caching = DevourerOne.PCachingDefault, int max = 200)
    {
        using var oCnn = new MsiSqlConnection(uri);
        return ReadLocalAsync().Result;
        async Task<List<TipoModeloDocumentoResponseAll>> ReadLocalAsync()
        {
            ...
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                if (await reader.IsDBNullAsync(1).ConfigureAwait(false))
                    continue;
                result.Add(ReadAll(new Entity.DBTipoModeloDocumento(reader), reader)!);
            }
    ...
    static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var query = new StringBuilder($"SELECT TOP ({max}) {campos}  FROM {"TipoModeloDocumento".dbo(oCnn)} (NOLOCK) ");
    ...
    public TipoModeloDocumentoResponse? Read(Entity.DBTipoModeloDocumento dbRec, MsiSqlConnection oCnn)
    {
        return Read(dbRec);
    }
    ...
    public TipoModeloDocumentoResponseAll? ReadAll(DBTipoModeloDocumento dbRec, SqlDataReader dr)
    {
        if (dbRec == null)
        {
            return null;
