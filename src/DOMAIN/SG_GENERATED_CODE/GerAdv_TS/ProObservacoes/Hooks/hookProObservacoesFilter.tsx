...
interface UseProObservacoesFilterProps {
    handleFetchWithFilter: (filtro?: FilterProObservacoes | undefined | null) => Promise<void>;
}

interface UseProObservacoesFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterProObservacoes;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterProObservacoes>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterProObservacoes) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterProObservacoes>) => React.ReactNode;
...
export const useProObservacoesFilter = ({ handleFetchWithFilter }: UseProObservacoesFilterProps): UseProObservacoesFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterProObservacoes>({} as FilterProObservacoes);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterProObservacoes;
    ...
    const handleConfirmSearch = async (filter: FilterProObservacoes) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterProObservacoes);
        sessionStorage.removeItem(btoa('ProObservacoesFilter'));
        handleFetchWithFilter({} as FilterProObservacoes);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterProObservacoes>) => (        
        <>
        <InputInput
                type='text'
                id='processo'
