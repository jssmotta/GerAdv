...
public class FGruposEmpresasFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FGruposEmpresasFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FGruposEmpresasFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FGruposEmpresasFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "grpCodigo")
        var dt = new DataTable();
        dt.Columns.Add("grpCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["grpCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FGruposEmpresas>();
    }
    ...
    {
        // Arrange
        var factory = new FGruposEmpresasFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("grpCodigo", typeof(int));
        mockDataTable.Columns.Add("grpDescricao", typeof(string));
        var row = mockDataTable.NewRow();
        row["grpCodigo"] = testId;
        row["grpDescricao"] = "Test GruposEmpresas";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["grpCodigo"] = 123;
        row["grpDescricao"] = "Test GruposEmpresas Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FGruposEmpresas properties
        }
    }
