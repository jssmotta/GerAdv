...
public partial class DBAgendaStatus
{
    ...
    /// </summary>
    public static async Task<DBAgendaStatus> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAgendaStatus();
...
#if (!NOTSTORED_AgendaStatus)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFAgenda || pFldFCompleted;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFAgenda)
            updateTool.Fields(DBAgendaStatusDicInfo.Agenda, m_FAgenda, ETiposCampos.FNumber);
        if (pFldFCompleted)
            updateTool.Fields(DBAgendaStatusDicInfo.Completed, m_FCompleted, ETiposCampos.FNumber);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBAgendaStatusDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBAgendaStatusDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBAgendaStatusDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBAgendaStatusDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBAgendaStatusDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
