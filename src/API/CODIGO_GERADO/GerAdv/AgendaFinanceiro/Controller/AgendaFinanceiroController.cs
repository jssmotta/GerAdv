...
public partial class AgendaFinanceiroController(IAgendaFinanceiroService agendafinanceiroService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.AgendaFinanceiro regAgendaFinanceiro, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaFinanceiro: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "AgendaFinanceiro: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.AgendaFinanceiro regAgendaFinanceiro, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _agendafinanceiroService.Validation(regAgendaFinanceiro, uri);
            if (result == null)
            {
                _logger.Warn("AgendaFinanceiro: Validation failed to add or update AgendaFinanceiro, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "AgendaFinanceiro: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}