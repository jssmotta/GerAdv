...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('poderjudiciarioassociado-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('poderjudiciarioassociado-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('poderjudiciarioassociado-columns-order'));
}, []);
...
{initialized && (
  ...
  className='grid-desktop-crud grid-desktop-poderjudiciarioassociado'
