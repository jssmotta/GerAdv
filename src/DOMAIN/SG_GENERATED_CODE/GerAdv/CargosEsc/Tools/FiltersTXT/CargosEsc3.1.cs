...
public static class FilterCargosEscTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar CargosEsc em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterCargosEsc:

- O filter irá filtrar os CargosEsc conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CargosEsc é Cargos do Escritório
...
CAMPOS filterCargosEsc:(Cargos Esc)
- percentual: valores monetários ou com decimais;
- nome: _nome ou descrição para Cargos Esc;
- classificacao: número ou IDs;
...
- Se ouvir "todos os CargosEsc" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterCargosEsc? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt CargosEsc length: {result.Length}");
#endif
