...
public partial class ContaCorrente
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(ctoDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.ContaCorrente WITH (NOLOCK, INDEX = idx_ContaCorrente_AuditorDtAtu)
WHERE ctoCodigo = @id
...
FROM {oCnn.UseDbo}.ContaCorrente WITH (NOLOCK)
