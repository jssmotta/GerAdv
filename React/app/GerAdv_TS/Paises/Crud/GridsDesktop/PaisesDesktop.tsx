...
export const PaisesGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(btoa("paises-columns-order"));
        if (savedOrder) {
    ...
    const openSearchCellUF = (id: number) => {
      router.push(`/pages/uf/?paises=${id}`);
    };

    const SearchFromCellUF = (props: any) => {
      return (
        <td>
          <div onClick={() => openSearchCellUF(props.dataItem.id)}>
            <span title="Pesquisar relacionados em UF">
    ...
    const SearchFromCellUFMobile = (props: any) => {
      return (
        <div onClick={() => openSearchCellUF(props.dataItem.id)}>
          <span title="Pesquisar relacionados em UF">
            <SvgIcon icon={searchIcon} />
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Paises-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Paises-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        {initialized && (
          <Grid
            key={`paises-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-paises"
            ...
            onRowDoubleClick={(e) => {
              if (selectItem) {
                selectItem(e.dataItem as IPaises);
          ...
          >
            <GridToolbar>
              ...
              <PageTitle
                title="Paises"
                helpButton={<PaisesHelpCrudButton isMobile={false} />}
              />
            </GridToolbar>
            {finalGridColumns}
          </Grid>
