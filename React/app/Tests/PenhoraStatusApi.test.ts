...
import { PenhoraStatusApiError } from '../GerAdv_TS/PenhoraStatus/Apis/ApiPenhoraStatus';
import { PenhoraStatusApi } from '@/app/GerAdv_TS/PenhoraStatus/Apis/ApiPenhoraStatus';
import { FilterPenhoraStatus } from '../GerAdv_TS/PenhoraStatus/Filters/PenhoraStatus';
...
describe('PenhoraStatusApi', () => {
  let penhorastatusApi: PenhoraStatusApi;
  ...
  beforeEach(() => {
    ...
    penhorastatusApi = new PenhoraStatusApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(penhorastatusApi).toBeInstanceOf(PenhoraStatusApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PenhoraStatusApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PenhoraStatusApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all penhorastatus items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await penhorastatusApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(penhorastatusApi.getAll()).rejects.toThrow(PenhoraStatusApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await penhorastatusApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-PenhoraStatus_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PenhoraStatus',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw PenhoraStatusApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPenhoraStatus = PenhoraStatusTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockPenhoraStatus,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch penhorastatus by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await penhorastatusApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPenhoraStatus));
      ...
      expect(result.data).toEqual(mockPenhoraStatus);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PenhoraStatus',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterPenhoraStatus = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter penhorastatus items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await penhorastatusApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockPenhoraStatus: IPenhoraStatus = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new penhorastatus successfully', async () => {
      const responsePenhoraStatus = { ...mockPenhoraStatus, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePenhoraStatus,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await penhorastatusApi.addAndUpdate(mockPenhoraStatus);
    ...
    it('should update existing penhorastatus successfully', async () => {
      const existingPenhoraStatus = { ...mockPenhoraStatus, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPenhoraStatus,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await penhorastatusApi.addAndUpdate(existingPenhoraStatus);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(penhorastatusApi.addAndUpdate(mockPenhoraStatus)).rejects.toThrow(PenhoraStatusApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PenhoraStatus',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete penhorastatus successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await penhorastatusApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(penhorastatusApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PenhoraStatus',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(penhorastatusApi.delete(1)).rejects.toThrow(PenhoraStatusApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterPenhoraStatus = { funcionario: 1 };
      
      const result = penhorastatusApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await penhorastatusApi.delete(1);
  ...
  describe('PenhoraStatusApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PenhoraStatusApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PenhoraStatusApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PenhoraStatusApiError('Test message', 500);
