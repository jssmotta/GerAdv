...
public partial class FTiposAcao : MenphisSI.SG.GerAdv.DBTiposAcao, IDBTiposAcao
{
    // DI-compatible default constructor
    public FTiposAcao() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FTiposAcao CreateFromReader(SqlDataReader? dbRec)
    {
        return new FTiposAcao(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FTiposAcao CreateFromDataRow(DataRow? dbRec)
    {
        return new FTiposAcao(dbRec);
    }

    private FTiposAcao(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FTiposAcao(DataRow? dbRec) : base(dbRec)
    {
    }

    private FTiposAcao(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FTiposAcao CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FTiposAcao(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FTiposAcao> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fTiposAcao = new FTiposAcao();
        await fTiposAcao.CarregarAsync(id, oCnn);
        return fTiposAcao;
    }
