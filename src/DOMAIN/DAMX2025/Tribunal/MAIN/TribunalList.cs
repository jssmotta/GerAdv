...
#region ListarDados_Tribunal
    public static async IAsyncEnumerable<DBTribunal> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTribunal(dbRec);
        }
    }

    public static IEnumerable<DBTribunal> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
