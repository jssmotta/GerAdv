...
export const useDivisaoTribunalList = (dataService: IDivisaoTribunalService, currentFilter?: FilterDivisaoTribunal | undefined | null) => {
  const [data, setData] = useState<IDivisaoTribunal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterDivisaoTribunal | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterDivisaoTribunal | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterDivisaoTribunal);
      setData(result);
    } catch (err) {
