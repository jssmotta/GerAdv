...
#region Test Data Setup Helpers
    ...
    private void SetupMockFGUTAtividades(string? Nome = "João", string? Observacao = "Observação teste", int? GUTGrupo = 1, int? GUTPeriodicidade = 1, int? Operador = 1, bool? Concluido = false, string? DataConcluido = "24/04/1975", int? DiasParaIniciar = 1, int? MinutosParaRealizar = 1)
    {
        ...
        if (string.IsNullOrEmpty(DataConcluido))
        {
            _mockFGUTAtividades.Setup(f => f.FDataConcluido).Returns((DateOnly? )null);
        }
        else
        {
            DateTime parsedDateTime;
            if (!DateTime.TryParse(DataConcluido, new CultureInfo("pt-BR"), DateTimeStyles.None, out parsedDateTime))
            {
                parsedDateTime = DateTime.Parse(DataConcluido, CultureInfo.InvariantCulture);
            }

            var dateOnly = DateOnly.FromDateTime(parsedDateTime);
            _mockFGUTAtividades.Setup(f => f.FDataConcluido).Returns(dateOnly);
        }
...
#region DateTime Tests
    ...
    public void Read_WithValidDate_DataConcluido_Formats_ShouldParseCorrectly(string dateString)
    {
        // Arrange
        var where = "Id = @Id";
        var parameters = CreateTestParameters();
        ...
        var result = _gutatividadesWhere.Read(where, parameters, _mockConnection.Object);
        // Assert
        result.DataConcluido.Should().Be(expectedDate.ToString("dd/MM/yyyy"));
    }
#endregion
#endregion
}