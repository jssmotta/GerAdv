...
#region ListarDados_Fornecedores
    public static async IAsyncEnumerable<DBFornecedores> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBFornecedores(dbRec);
        }
    }

    public static IEnumerable<DBFornecedores> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
