...
public partial class DBDocsRecebidosItens
{
    ...
    /// </summary>
    public static async Task<DBDocsRecebidosItens> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBDocsRecebidosItens();
...
#if (!NOTSTORED_DocsRecebidosItens)
    /// <summary>
...
#if (!NOTSTORED_DocsRecebidosItens)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFContatoCRM || pFldFNome || pFldFDevolvido || pFldFSeraDevolvido || pFldFObservacoes || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFContatoCRM)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.ContatoCRM, m_FContatoCRM, ETiposCampos.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFDevolvido)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.Devolvido, m_FDevolvido, ETiposCampos.FBoolean);
        if (pFldFSeraDevolvido)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.SeraDevolvido, m_FSeraDevolvido, ETiposCampos.FBoolean);
        if (pFldFObservacoes)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.Observacoes, m_FObservacoes, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBDocsRecebidosItensDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBDocsRecebidosItensDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
