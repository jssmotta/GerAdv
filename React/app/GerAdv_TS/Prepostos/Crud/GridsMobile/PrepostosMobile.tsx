  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("prepostos-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-prepostos .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("prepostos-columns-order-mobile"));
    }, []);
    ...
    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-Prepostos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-Prepostos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
        ...
        {initialized && (
          <>
            ...
            <Grid
              ...
              key={`prepostos-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
