...
public partial class DBContatoCRMView : VNoAuditor, ICrud
{
#region TableDefinition_ContatoCRMView
...
#endregion
    public DBContatoCRMView(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBContatoCRMView(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
        if (oCnn == null)
            return;
...
#region GravarDados_ContatoCRMView
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (pFldFCGUID)
            clsW.Fields(DBContatoCRMViewDicInfo.CGUID, FCGUID, EGenericTypeFields.FString);
        if (pFldFData)
            clsW.Fields(DBContatoCRMViewDicInfo.Data, FData, EGenericTypeFields.FDate);
        if (pFldFIP)
            clsW.Fields(DBContatoCRMViewDicInfo.IP, FIP, EGenericTypeFields.FString);
        if (insertId != 0)
            return GravaNewId();
