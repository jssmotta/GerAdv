  ...
  const loadCargos = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Cargo";
...
export const useCargosList = (
  dataService: ICargosService,
  currentFilter?: FilterCargos | undefined | null,
) => {
  const [data, setData] = useState<ICargos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterCargos | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterCargos | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterCargos));
        setData(result);
      } catch (err) {
        const errorMessage =
