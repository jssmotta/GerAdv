...
public class FProProcuradoresFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProProcuradoresFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProProcuradoresFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProProcuradoresFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "papCodigo")
        var dt = new DataTable();
        dt.Columns.Add("papCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["papCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProProcuradores>();
    }
    ...
    {
        // Arrange
        var factory = new FProProcuradoresFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("papCodigo", typeof(int));
        mockDataTable.Columns.Add("papNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["papCodigo"] = testId;
        row["papNome"] = "Test ProProcuradores";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["papCodigo"] = 123;
        row["papNome"] = "Test ProProcuradores Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProProcuradores properties
        }
    }
