...
public static class FilterSetorTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterSetor? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Setor em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterSetor:

- O filter irá filtrar os Setor conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Setor" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Setor length: {result.Length}");
#endif
