...
export const useClientesSociosList = (dataService: IClientesSociosService, currentFilter?: FilterClientesSocios | undefined | null) => {
  const [data, setData] = useState<IClientesSocios[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterClientesSocios | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterClientesSocios | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterClientesSocios);
      setData(result);
    } catch (err) {
