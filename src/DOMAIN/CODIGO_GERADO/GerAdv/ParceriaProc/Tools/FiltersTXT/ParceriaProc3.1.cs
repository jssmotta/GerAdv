...
public static class FilterParceriaProcTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterParceriaProc? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar ParceriaProc em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterParceriaProc:

- O filter irá filtrar os ParceriaProc conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterParceriaProc:(Parceria Proc)
...
- Se ouvir "todos os ParceriaProc" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ParceriaProc length: {result.Length}");
#endif
