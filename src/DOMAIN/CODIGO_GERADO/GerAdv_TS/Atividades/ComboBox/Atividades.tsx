...
const AtividadesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IAtividades | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IAtividades | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IAtividades);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <AtividadesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedAtividades={editar ?? addValue ?? AtividadesEmpty()} /> 
            )}
