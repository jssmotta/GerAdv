...
export const useRegimeTributacaoForm = (
  ...
  const loadRegimeTributacao = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Regime Tributacao";
...
export const useRegimeTributacaoList = (
  dataService: IRegimeTributacaoService,
  currentFilter?: FilterRegimeTributacao | undefined | null,
) => {
  const [data, setData] = useState<IRegimeTributacao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterRegimeTributacao | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterRegimeTributacao | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar regimetributacao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsRegimeTributacao() {
  ...
  function validate(data: IRegimeTributacao): {
    isValid: boolean;
    message: string;
  } {
