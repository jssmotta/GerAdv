...
export const TipoOrigemSucumbenciaGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('tipoorigemsucumbencia-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('tipoorigemsucumbencia-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-tipoorigemsucumbencia .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
...
const isColumnVisible = useCallback((field: string) => {
  ...
  const resetColumnsOrder = useCallback(() => {
    ...
    localStorage.removeItem(btoa('tipoorigemsucumbencia-columns-order-mobile'));
  }, []);
