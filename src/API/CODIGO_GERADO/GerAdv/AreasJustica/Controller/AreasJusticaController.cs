...
public partial class AreasJusticaController(IAreasJusticaService areasjusticaService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterAreasJustica filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("AreasJustica: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _areasjusticaService.Filter(max, filtro, uri);
        return Ok(result);
    }
