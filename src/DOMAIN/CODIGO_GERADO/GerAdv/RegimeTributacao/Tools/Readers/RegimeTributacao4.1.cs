...
public partial class RegimeTributacao
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(rdtDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.RegimeTributacao WITH (NOLOCK, INDEX = idx_RegimeTributacao_AuditorDtAtu)
WHERE rdtCodigo = @id
...
FROM {oCnn.UseDbo}.RegimeTributacao WITH (NOLOCK)
