...
export const useFornecedoresList = (
  dataService: IFornecedoresService,
  currentFilter?: FilterFornecedores | undefined | null,
) => {
  const [data, setData] = useState<IFornecedores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterFornecedores | undefined | null>(
    currentFilter,
  );
...
export function useValidationsFornecedores() {
  ...
  function validate(data: IFornecedores): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.produtos.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Produtos não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.contatos.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Contatos não pode ter mais de 2147483647 caracteres.",
        };
      }
