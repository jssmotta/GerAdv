...
import { ProcessOutPutIDs } from '../../Models/ProcessOutPutIDs';
...
export class ProcessOutPutIDsApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ProcessOutPutIDsApiError';
    this.status = status;
    this.code = code;
...
export class ProcessOutPutIDsApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ProcessOutPutIDs`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutPutIDs_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ProcessOutPutIDs');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutPutIDs_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Process Out Put I Ds com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterProcessOutPutIDs): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutPutIDs_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ProcessOutPutIDs');
        }
    }

            public async filter(filtro: FilterProcessOutPutIDs): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessOutPutIDs_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ProcessOutPutIDs');
                }
            }


    public async validation(regProcessOutPutIDs: IProcessOutPutIDs): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regProcessOutPutIDs as ProcessOutPutIDs, this.getHeaders());
    ...
    public async addAndUpdate(regProcessOutPutIDs: IProcessOutPutIDs): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regProcessOutPutIDs as ProcessOutPutIDs, this.getHeaders());
            var register = result.data as IProcessOutPutIDs;        
            const action = regProcessOutPutIDs.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Process Out Put I Ds');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ProcessOutPutIDs. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ProcessOutPutIDs',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Process Out Put I Ds com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterProcessOutPutIDs) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<ProcessOutPutIDs[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de processoutputids:', error);
            this.notificationService.notify({
              entity: 'ProcessOutPutIDs',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos processoutputids'
            });
          }
        });
    }
