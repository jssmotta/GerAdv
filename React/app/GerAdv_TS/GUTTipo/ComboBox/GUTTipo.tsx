...
import GUTTipoWindow from '../Crud/Grids/GUTTipoWindow';
...
import { useGUTTipoComboBox } from '../Hooks/hookGUTTipo';
const GUTTipoComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useGUTTipoComboBox(guttipoService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IGUTTipo | null>(null);
const [addRecord, setAddRecord] = useState<IGUTTipo | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await guttipoService.fetchGUTTipoById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nome });
  } catch (error) {
  console.log('Erro ao carregar G U T Tipo:');
}
};
...
const handleSuccess = (newGUTTipo?: IGUTTipo) => {
  if (newGUTTipo) {
    setValue({ id: newGUTTipo.id, nome: newGUTTipo.nome });
    loadRecordForEdit(newGUTTipo.id);
    handleValueChange(newGUTTipo);
  }
  handleClose();
};
return (
...
{(editRecord || addRecord) && isOpen && (
  ...
  onError={handleClose}
  selectedGUTTipo={editRecord || addRecord || GUTTipoEmpty()}
  />
  )}
...
export default GUTTipoComboBox;