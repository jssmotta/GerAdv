...
#region ListarDados_Prepostos
    public static async IAsyncEnumerable<DBPrepostos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPrepostos(dbRec);
        }
    }

    public static IEnumerable<DBPrepostos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
