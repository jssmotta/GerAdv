...
public class FClientesSociosFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FClientesSociosFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FClientesSociosFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FClientesSociosFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "cscCodigo")
        var dt = new DataTable();
        dt.Columns.Add("cscCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["cscCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FClientesSocios>();
    }
    ...
    {
        // Arrange
        var factory = new FClientesSociosFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("cscCodigo", typeof(int));
        mockDataTable.Columns.Add("cscNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["cscCodigo"] = testId;
        row["cscNome"] = "Test ClientesSocios";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["cscCodigo"] = 123;
        row["cscNome"] = "Test ClientesSocios Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FClientesSocios properties
        }
    }
