...
import { TipoProDespositoApiError } from '../GerAdv_TS/TipoProDesposito/Apis/ApiTipoProDesposito';
import { TipoProDespositoApi } from '@/app/GerAdv_TS/TipoProDesposito/Apis/ApiTipoProDesposito';
import { FilterTipoProDesposito } from '../GerAdv_TS/TipoProDesposito/Filters/TipoProDesposito';
...
describe('TipoProDespositoApi', () => {
  let tipoprodespositoApi: TipoProDespositoApi;
  ...
  beforeEach(() => {
    ...
    tipoprodespositoApi = new TipoProDespositoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipoprodespositoApi).toBeInstanceOf(TipoProDespositoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoProDespositoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoProDespositoApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [TipoProDespositoTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all tipoprodesposito items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoprodespositoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipoprodespositoApi.getAll()).rejects.toThrow(TipoProDespositoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipoprodespositoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoProDesposito_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoProDespositoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoProDesposito = TipoProDespositoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoProDesposito,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipoprodesposito by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoprodespositoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoProDesposito));
      ...
      expect(result.data).toEqual(mockTipoProDesposito);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoProDesposito',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoProDesposito = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipoprodesposito items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipoprodespositoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoProDesposito: ITipoProDesposito = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipoprodesposito successfully', async () => {
      const responseTipoProDesposito = { ...mockTipoProDesposito, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoProDesposito,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoprodespositoApi.addAndUpdate(mockTipoProDesposito);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockTipoProDesposito,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing tipoprodesposito successfully', async () => {
      const existingTipoProDesposito = { ...mockTipoProDesposito, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoProDesposito,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoprodespositoApi.addAndUpdate(existingTipoProDesposito);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipoprodespositoApi.addAndUpdate(mockTipoProDesposito)).rejects.toThrow(TipoProDespositoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoProDesposito',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipoprodesposito successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipoprodespositoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipoprodespositoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoProDesposito',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipoprodespositoApi.delete(1)).rejects.toThrow(TipoProDespositoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoProDesposito = { funcionario: 1 };
      
      const result = tipoprodespositoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipoprodespositoApi.delete(1);
  ...
  describe('TipoProDespositoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoProDespositoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoProDespositoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoProDespositoApiError('Test message', 500);
