...
export class AgendaRelatorioApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AgendaRelatorioApiError';
    this.status = status;
    this.code = code;
...
export class AgendaRelatorioApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        throw new AgendaRelatorioApiError(
            ...
            public async filter(filtro: FilterAgendaRelatorio): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AgendaRelatorio_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AgendaRelatorio');
                }
            }


    public async validation(regAgendaRelatorio: IAgendaRelatorio): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAgendaRelatorio as AgendaRelatorio, this.getHeaders());
    ...
    public async addAndUpdate(regAgendaRelatorio: IAgendaRelatorio): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Agenda Relatorio');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Agenda Relatorio com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAgendaRelatorio) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AgendaRelatorio[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de agendarelatorio:', error);
            this.notificationService.notify({
              entity: 'AgendaRelatorio',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos agendarelatorio'
            });
          }
        });
    }
