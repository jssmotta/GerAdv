...
#pragma warning restore CA1822 // Mark members as static
    ...
    public static ImmutableArray<DBInfoSystem> List => [DBEnquadramentoEmpresaDicInfo.EqeNome, DBEnquadramentoEmpresaDicInfo.EqeGUID, DBEnquadramentoEmpresaDicInfo.EqeQuemCad, DBEnquadramentoEmpresaDicInfo.EqeDtCad, DBEnquadramentoEmpresaDicInfo.EqeQuemAtu, DBEnquadramentoEmpresaDicInfo.EqeDtAtu, DBEnquadramentoEmpresaDicInfo.EqeVisto];
    public static ImmutableArray<DBInfoSystem> ListWithoutAuditor => [DBEnquadramentoEmpresaDicInfo.EqeNome, DBEnquadramentoEmpresaDicInfo.EqeGUID];

    public static ImmutableArray<DBInfoSystem> ListPk()
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["eqeCodigo"]);
    ...
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["eqeCodigo", "eqeNome"]);
        var result = campos.Where(campo => !campo.Equals(DBEnquadramentoEmpresaDicInfo.CampoCodigo)).Select(campo => List.FirstOrDefault(t => t.FNome == campo)).Where(item => item != null).Cast<DBInfoSystem>().Distinct().ToList();
        return result.Count > 0 ? [..result] : ImmutableArray<DBInfoSystem>.Empty;
    }
}
