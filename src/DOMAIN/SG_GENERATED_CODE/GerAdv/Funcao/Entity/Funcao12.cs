...
public partial class FFuncao : MenphisSI.SG.GerAdv.DBFuncao, IDBFuncao
{
    ...
    public static FFuncao CreateFromReader(SqlDataReader? dbRec)
    {
        return new FFuncao(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FFuncao CreateFromDataRow(DataRow? dbRec)
    {
        return new FFuncao(dbRec);
    }

    private FFuncao(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FFuncao(DataRow? dbRec) : base(dbRec)
    {
    }

    private FFuncao(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FFuncao CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FFuncao(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FFuncao> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fFuncao = new FFuncao();
        await fFuncao.CarregarAsync(id, oCnn);
        return fFuncao;
    }
