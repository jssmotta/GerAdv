...
public class FAgendaSemanaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FAgendaSemanaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FAgendaSemanaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FAgendaSemanaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "xxxCodigo")
        var dt = new DataTable();
        dt.Columns.Add("xxxCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["xxxCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FAgendaSemana>();
    }
    ...
    {
        // Arrange
        var factory = new FAgendaSemanaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
   ...
   [Fact]
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("xxxCodigo", typeof(int));
        mockDataTable.Columns.Add("xxxParaNome", typeof(string)); 
        
        var row = mockDataTable.NewRow();
        row["xxxCodigo"] = testId;
        row["xxxParaNome"] = "Test AgendaSemana";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["xxxCodigo"] = 123;
        row["xxxParaNome"] = "Test AgendaSemana Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FAgendaSemana properties
        }
    }
