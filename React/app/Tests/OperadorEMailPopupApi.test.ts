...
describe('OperadorEMailPopupApi', () => {
  let operadoremailpopupApi: OperadorEMailPopupApi;
  ...
  beforeEach(() => {
    ...
    operadoremailpopupApi = new OperadorEMailPopupApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(operadoremailpopupApi).toBeInstanceOf(OperadorEMailPopupApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OperadorEMailPopupApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OperadorEMailPopupApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [OperadorEMailPopupTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all operadoremailpopup items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadoremailpopupApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(operadoremailpopupApi.getAll()).rejects.toThrow(OperadorEMailPopupApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await operadoremailpopupApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-OperadorEMailPopup_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorEMailPopup',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw OperadorEMailPopupApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOperadorEMailPopup = OperadorEMailPopupTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockOperadorEMailPopup,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch operadoremailpopup by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadoremailpopupApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOperadorEMailPopup));
      ...
      expect(result.data).toEqual(mockOperadorEMailPopup);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorEMailPopup',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter operadoremailpopup items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await operadoremailpopupApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new operadoremailpopup successfully', async () => {
      const responseOperadorEMailPopup = { ...mockOperadorEMailPopup, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOperadorEMailPopup,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadoremailpopupApi.addAndUpdate(mockOperadorEMailPopup);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockOperadorEMailPopup,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing operadoremailpopup successfully', async () => {
      const existingOperadorEMailPopup = { ...mockOperadorEMailPopup, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOperadorEMailPopup,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadoremailpopupApi.addAndUpdate(existingOperadorEMailPopup);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(operadoremailpopupApi.addAndUpdate(mockOperadorEMailPopup)).rejects.toThrow(OperadorEMailPopupApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorEMailPopup',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete operadoremailpopup successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await operadoremailpopupApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(operadoremailpopupApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'OperadorEMailPopup',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(operadoremailpopupApi.delete(1)).rejects.toThrow(OperadorEMailPopupApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await operadoremailpopupApi.delete(1);
  ...
  describe('OperadorEMailPopupApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OperadorEMailPopupApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OperadorEMailPopupApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OperadorEMailPopupApiError('Test message', 500);
