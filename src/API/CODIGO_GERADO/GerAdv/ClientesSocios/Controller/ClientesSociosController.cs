...
public partial class ClientesSociosController(IClientesSociosService clientessociosService) : ControllerBase
{
    ...
    {
        _logger.Info("ClientesSocios: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _clientessociosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ClientesSocios: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _clientessociosService.UpdateColumns(parameters, uri);
        if (!result)
        {
