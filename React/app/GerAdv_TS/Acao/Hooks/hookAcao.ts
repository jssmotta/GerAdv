...
export const useAcaoForm = (initialAcao: IAcao, dataService: IAcaoService) => {
  ...
  const loadAcao = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Acao";
...
export const useAcaoList = (
  dataService: IAcaoService,
  currentFilter?: FilterAcao | undefined | null,
) => {
  ...
  function validate(data: IAcao): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.descricao.length > 255) {
        return {
          isValid: false,
          message: "O campo Descricao não pode ter mais de 255 caracteres.",
        };
      }
