...
const DocsRecebidosItensComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IDocsRecebidosItens | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IDocsRecebidosItens | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IDocsRecebidosItens);
    ...
    const handleAddClick = () => {
        ...
        const add = DocsRecebidosItensEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <DocsRecebidosItensWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedDocsRecebidosItens={editar ?? addValue ?? DocsRecebidosItensEmpty()} /> 
            )}
