...
import PrepostosWindow from '../Crud/Grids/PrepostosWindow';
...
import { usePrepostosComboBox } from '../Hooks/hookPrepostos';
const PrepostosComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = usePrepostosComboBox(prepostosService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IPrepostos | null>(null);
const [addRecord, setAddRecord] = useState<IPrepostos | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await prepostosService.fetchPrepostosById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = PrepostosEmpty();
...
const handleSuccess = (newPrepostos?: IPrepostos) => {
  if (newPrepostos) {
    setValue({ id: newPrepostos.id, nome: newPrepostos.nome });
    loadRecordForEdit(newPrepostos.id);
    handleValueChange(newPrepostos);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .prepostos-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .prepostos-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .prepostos-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedPrepostos={editRecord || addRecord || PrepostosEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'prepostos-dropdown-popup-msi'
          }}
          />
...
export default PrepostosComboBox;