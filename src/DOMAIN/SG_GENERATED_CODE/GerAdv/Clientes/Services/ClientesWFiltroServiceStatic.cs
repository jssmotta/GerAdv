...
public partial class ClientesService
{
    ...
    public async Task<ResultApi<IEnumerable<NomeID>>> GetListN(int max, Filters.FilterClientes? filtro, string uri, CancellationToken token = default)
    {
        // Tracking: 20250606-0
        ThrowIfDisposed();
        ...
        var cacheKey = $"{uri}-Clientes-{max}-{filterHash}-GetListN-{keyCache}";
    ...
    public async Task<ResultApi<IEnumerable<ClientesResponseAll>>> GetAll(int max, string uri, CancellationToken token = default)
    {
        ThrowIfDisposed();
        ...
        {
            _logger.Error(ex, "Clientes: GetAll failed for uri = {0}", uri);
