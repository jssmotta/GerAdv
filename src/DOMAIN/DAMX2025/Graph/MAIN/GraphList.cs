...
#region ListarDados_Graph
    public static async IAsyncEnumerable<DBGraph> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGraph(dbRec);
        }
    }

    public static IEnumerable<DBGraph> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
