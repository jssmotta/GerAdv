...
import { FilterStatusInstancia } from '@/app/GerAdv_TS/StatusInstancia/Filters/StatusInstancia';
...
interface UseStatusInstanciaFilterProps {
    handleFetchWithFilter: (filtro?: FilterStatusInstancia | undefined | null) => Promise<void>;
}

interface UseStatusInstanciaFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterStatusInstancia;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterStatusInstancia>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterStatusInstancia) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterStatusInstancia>) => React.ReactNode;
...
export const useStatusInstanciaFilter = ({ handleFetchWithFilter }: UseStatusInstanciaFilterProps): UseStatusInstanciaFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterStatusInstancia>({} as FilterStatusInstancia);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterStatusInstancia;
    ...
    const handleConfirmSearch = async (filter: FilterStatusInstancia) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterStatusInstancia);
        sessionStorage.removeItem(btoa('StatusInstanciaFilter'));
        handleFetchWithFilter({} as FilterStatusInstancia);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterStatusInstancia>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
