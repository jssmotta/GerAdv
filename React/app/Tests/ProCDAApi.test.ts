...
describe('ProCDAApi', () => {
  ...
  beforeEach(() => {
    ...
    procdaApi = new ProCDAApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(procdaApi).toBeInstanceOf(ProCDAApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProCDAApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProCDAApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all procda items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await procdaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(procdaApi.getAll()).rejects.toThrow(ProCDAApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await procdaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProCDA_last_getAll_${200}`)));
    });
    ...
    it('should throw ProCDAApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProCDA = ProCDATestEmpty();
    ...
    it('should fetch procda by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await procdaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProCDA));
      ...
      expect(result.data).toEqual(mockProCDA);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProCDA',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter procda items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await procdaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new procda successfully', async () => {
      const responseProCDA = { ...mockProCDA, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProCDA,
        status: 200,
        statusText: 'OK',
      ...
      const result = await procdaApi.addAndUpdate(mockProCDA);
    ...
    it('should update existing procda successfully', async () => {
      const existingProCDA = { ...mockProCDA, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProCDA,
        status: 200,
        statusText: 'OK',
      ...
      const result = await procdaApi.addAndUpdate(existingProCDA);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(procdaApi.addAndUpdate(mockProCDA)).rejects.toThrow(ProCDAApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProCDA',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete procda successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await procdaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(procdaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProCDA',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(procdaApi.delete(1)).rejects.toThrow(ProCDAApiError);
    });
  });
  ...
  describe('ProCDAApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProCDAApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProCDAApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProCDAApiError('Test message', 500);
