...
public partial class DBAgendaQuem
{
    ...
    /// </summary>
    public static async Task<DBAgendaQuem> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAgendaQuem();
...
#if (!NOTSTORED_AgendaQuem)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFIDAgenda || pFldFAdvogado || pFldFFuncionario || pFldFPreposto;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFIDAgenda)
            updateTool.Fields(DBAgendaQuemDicInfo.IDAgenda, m_FIDAgenda, ETiposCampos.FNumber);
        if (pFldFAdvogado)
            updateTool.Fields(DBAgendaQuemDicInfo.Advogado, m_FAdvogado, ETiposCampos.FNumber);
        if (pFldFFuncionario)
            updateTool.Fields(DBAgendaQuemDicInfo.Funcionario, m_FFuncionario, ETiposCampos.FNumber);
        if (pFldFPreposto)
            updateTool.Fields(DBAgendaQuemDicInfo.Preposto, m_FPreposto, ETiposCampos.FNumber);
    }
