...
#region ListarDados_Ligacoes
    public static async IAsyncEnumerable<DBLigacoes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBLigacoes(dbRec);
        }
    }

    public static IEnumerable<DBLigacoes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
