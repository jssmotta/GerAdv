...
public class FContaCorrenteFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FContaCorrenteFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FContaCorrenteFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FContaCorrenteFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "ctoCodigo")
        var dt = new DataTable();
        dt.Columns.Add("ctoCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["ctoCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FContaCorrente>();
    }
    ...
    {
        // Arrange
        var factory = new FContaCorrenteFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("ctoCodigo", typeof(int));
        mockDataTable.Columns.Add("ctoGuid", typeof(string));
        var row = mockDataTable.NewRow();
        row["ctoCodigo"] = testId;
        row["ctoGuid"] = "Test ContaCorrente";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["ctoCodigo"] = 123;
        row["ctoGuid"] = "Test ContaCorrente Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FContaCorrente properties
        }
    }
