...
public partial class LigacoesController(ILigacoesService ligacoesService) : ControllerBase
{
    ...
    {
        _logger.Info("Ligacoes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _ligacoesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Ligacoes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _ligacoesService.UpdateColumns(parameters, uri);
        if (!result)
        {
