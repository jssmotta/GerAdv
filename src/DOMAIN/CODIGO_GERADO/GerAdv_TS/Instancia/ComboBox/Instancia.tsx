...
const InstanciaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IInstancia | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IInstancia | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setDisplayValue({ id: response.data.id, nome: response.data.nroprocesso });
                setEditar(response.data as IInstancia);
    ...
    const handleAddClick = () => {
        ...
        const add = InstanciaEmpty();
        add.nroprocesso = inputValue.toUpperCase();       
        setEditar(null);
        setAddValue(add);
        setIsOpen(true);
    }
        ...
        <>
          {(editar || addValue) && isOpen && (
            <InstanciaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedInstancia={editar ?? addValue ?? InstanciaEmpty()} /> 
            )}
