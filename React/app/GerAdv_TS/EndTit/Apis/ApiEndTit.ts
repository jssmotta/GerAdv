...
import { EndTit } from '../../Models/EndTit';
...
export class EndTitApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class EndTitApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/EndTit`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EndTit_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os EndTit');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EndTit_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar End Tit com ID ${id}`);
        }
    }

            public async filter(filtro: FilterEndTit): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EndTit_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar EndTit');
                }
            }


    public async validation(regEndTit: IEndTit): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEndTit as EndTit, this.getHeaders());
    ...
    public async addAndUpdate(regEndTit: IEndTit): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regEndTit as EndTit, this.getHeaders());
            var register = result.data as IEndTit;        
            const action = regEndTit.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar End Tit');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o EndTit. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'EndTit',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir End Tit com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEndTit) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<EndTit[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de endtit:', error);
            this.notificationService.notify({
              entity: 'EndTit',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos endtit'
            });
          }
        });
    }
