...
public partial class CidadeService(IOptions<AppSettings> appSettings, ICidadeReader reader, ICidadeValidation validation, ICidadeWriter writer, IUFReader ufReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : ICidadeService, IDisposable
{
    ...
    public async Task<IEnumerable<CidadeResponse>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-Cidade-GetAll-{max}";
    ...
    public async Task<CidadeResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-Cidade-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("Cidade: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBCidade(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBCidadeDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("Cidade: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBCidade(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBCidadeDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
