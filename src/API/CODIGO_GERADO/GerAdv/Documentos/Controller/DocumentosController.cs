...
public partial class DocumentosController(IDocumentosService documentosService) : ControllerBase
{
    ...
    {
        _logger.Info("Documentos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _documentosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Documentos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _documentosService.UpdateColumns(parameters, uri);
        if (!result)
        {
