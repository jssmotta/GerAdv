...
#region Testes de Propriedades Computadas
    ...
    {
        _instance.FDataCorrecao = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FDataCorrecao?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }
    ...
    {
        var instance = new DBProDespesas();
        Assert.Equal(null !, instance.FDataCorrecao);
    }
