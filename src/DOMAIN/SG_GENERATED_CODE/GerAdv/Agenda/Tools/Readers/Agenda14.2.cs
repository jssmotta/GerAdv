...
#pragma warning restore CA1822 // Mark members as static

    {
        if (oCnn is null)
            return;
        Console.WriteLine($"WarmUp Agenda: {tenantKey}");
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_Agenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[Agenda]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Agenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Agenda]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_Agenda_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_Agenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[Agenda]'))
                BEGIN                    
