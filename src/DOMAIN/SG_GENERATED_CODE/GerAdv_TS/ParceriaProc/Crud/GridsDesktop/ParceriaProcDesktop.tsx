...
export const ParceriaProcGridDesktopComponent = React.memo(({
  ...
  const [columnsOrder, setColumnsOrder] = useState<string[]>(['nomeadvogados', 
'nropastaprocessos']);

  // Carregar ordem salva do localStorage na inicialização
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('parceriaproc-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('parceriaproc-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
