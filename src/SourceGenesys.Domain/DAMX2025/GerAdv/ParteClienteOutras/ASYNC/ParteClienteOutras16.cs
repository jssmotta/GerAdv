...
public partial class DBParteClienteOutras
{
    ...
    /// </summary>
    public static async Task<DBParteClienteOutras> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBParteClienteOutras();
...
#if (!NOTSTORED_ParteClienteOutras)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFCliente || pFldFProcesso || pFldFPrimeiraReclamada;
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBParteClienteOutrasDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBParteClienteOutrasDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBParteClienteOutrasDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBParteClienteOutrasDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBParteClienteOutrasDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
