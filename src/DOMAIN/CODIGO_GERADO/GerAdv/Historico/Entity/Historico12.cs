...
public partial class FHistorico : MenphisSI.SG.GerAdv.DBHistorico, IDBHistorico
{
    // DI-compatible default constructor
    public FHistorico() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FHistorico CreateFromReader(SqlDataReader? dbRec)
    {
        return new FHistorico(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FHistorico CreateFromDataRow(DataRow? dbRec)
    {
        return new FHistorico(dbRec);
    }

    private FHistorico(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FHistorico(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FHistorico> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fHistorico = new FHistorico();
        await fHistorico.CarregarAsync(id, oCnn);
        return fHistorico;
    }

    private FHistorico(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FHistorico CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FHistorico(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
