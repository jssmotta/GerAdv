...
public partial class DBDadosProcuracao : VAuditor, ICadastros
{
...
#region GravarDados_DadosProcuracao
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (!isInsert)
            if (!(pFldFCliente || pFldFGUID || pFldFEstadoCivil || pFldFNacionalidade || pFldFProfissao || pFldFCTPS || pFldFPisPasep || pFldFRemuneracao || pFldFObjeto))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
