...
public static class FilterObjetosTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Objetos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterObjetos:

- O filter irá filtrar os Objetos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterObjetos:(Objetos)
- justica: número ou IDs;
- area: número ou IDs;
- nome: _nome ou descrição para Objetos;
- bold: Bold;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
...
- Se ouvir "todos os Objetos" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterObjetos? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Objetos length: {result.Length}");
#endif
