...
export const useDocsRecebidosItensList = (dataService: IDocsRecebidosItensService, currentFilter?: FilterDocsRecebidosItens | undefined | null) => {
  const [data, setData] = useState<IDocsRecebidosItens[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterDocsRecebidosItens | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterDocsRecebidosItens | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterDocsRecebidosItens);
      setData(result);
    } catch (err) {
