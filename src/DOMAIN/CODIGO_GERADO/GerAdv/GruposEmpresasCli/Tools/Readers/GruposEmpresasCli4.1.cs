...
public partial class GruposEmpresasCli
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(gecDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.GruposEmpresasCli WITH (NOLOCK, INDEX = idx_GruposEmpresasCli_AuditorDtAtu)
WHERE gecCodigo = @id
...
FROM {oCnn.UseDbo}.GruposEmpresasCli WITH (NOLOCK)
