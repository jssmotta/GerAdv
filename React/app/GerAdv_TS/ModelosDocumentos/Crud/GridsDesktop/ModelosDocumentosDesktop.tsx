...
import { useModelosDocumentosGrid } from "../../Hooks/hookModelosDocumentosGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("modelosdocumentos-columns-order"),
        );
        if (savedOrder) {
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        nometipomodelodocumento: (
          <GridColumn
            key="nometipomodelodocumento"
            field="nometipomodelodocumento"
            title="Tipo Modelo Documento"
            sortable={false}
            filterable={false}
          />
        ) /* Track G.04 */,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("modelosdocumentos-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-modelosdocumentos",
      columns: exportColumns,
      sheetName: "ModelosDocumentos",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-ModelosDocumentos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-modelosdocumentos",
      columns: exportColumnsPdf,
      title: "ModelosDocumentos",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-ModelosDocumentos-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`modelosdocumentos-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-modelosdocumentos"
