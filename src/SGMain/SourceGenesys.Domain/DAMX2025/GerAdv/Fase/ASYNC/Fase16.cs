...
public partial class DBFase
{
    ...
    /// </summary>
    public static async Task<DBFase> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBFase();
...
#if (!NOTSTORED_Fase)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFDescricao || pFldFJustica || pFldFArea || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFDescricao)
            updateTool.Fields(DBFaseDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFJustica)
            updateTool.Fields(DBFaseDicInfo.Justica, FJustica, EGenericTypeFields.FNumber);
        if (pFldFArea)
            updateTool.Fields(DBFaseDicInfo.Area, FArea, EGenericTypeFields.FNumber);
        if (pFldFGuid)
            updateTool.Fields(DBFaseDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBFaseDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBFaseDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBFaseDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBFaseDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBFaseDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBFaseDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
