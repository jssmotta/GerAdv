// 100% auto-generated by Source Genesys WebApi Front & Back Creator
// copyright © 2000-2026 Menphis - Sistemas Inteligentes
// This file is part of the Source Genesys project
// Tracking 21-08-25: Forms.tsx.txt
"use client";
import { IContaCorrente } from "@/app/GerAdv_TS/ContaCorrente/Interfaces/interface.ContaCorrente";
import { useRouter } from "next/navigation";
import React, { useEffect, useState, useRef } from "react";
import { useSystemContext } from "@/app/context/SystemContext";
import { getParamFromUrl } from "@/app/tools/helpers";
import FooterIncDocker from "@/app/components/Cruds/FooterIncDocker";
import "@/app/styles/CrudFormsBase.css";
import "@/app/styles/CrudFormsMobile.css";
import "@/app/styles/CrudForms.css"; // [ INDEX_SIZE ]

import ButtonSalvarCrud from "@/app/components/Cruds/ButtonSalvarCrud";
import { useIsMobile } from "@/app/context/MobileContext";
import DeleteButton from "@/app/components/Cruds/DeleteButton";
import AuditorButton from "@/app/components/Cruds/AuditorButton";
import PrintButton from "@/app/components/Cruds/PrintButton";
import CopyButton from "@/app/components/Cruds/CopyButton";
import { ContaCorrenteApi } from "../../Apis/ApiContaCorrente";
import { useValidationsContaCorrente } from "../../Hooks/hookContaCorrente";

import ClientesComboBox from "@/app/GerAdv_TS/Clientes/ComboBox/Clientes";
import { ClientesApi } from "@/app/GerAdv_TS/Clientes/Apis/ApiClientes";
import InputName from "@/app/components/Inputs/InputName";
import InputInput from "@/app/components/Inputs/InputInput";
import InputCheckbox from "@/app/components/Inputs/InputCheckbox";

interface ContaCorrenteFormProps {
  contacorrenteData: IContaCorrente;
  onChange: (e: any) => void;
  onSubmit: (e: React.FormEvent) => void;
  onClose: () => void;
  onError?: () => void;
  onReload?: () => void;
  onSuccess?: (registro?: any) => void;
}

export const ContaCorrenteForm: React.FC<ContaCorrenteFormProps> = ({
  contacorrenteData,
  onChange,
  onSubmit,
  onClose,
  onError,
  onReload,
  onSuccess,
}) => {
  const router = useRouter();
  const { systemContext } = useSystemContext();
  const isMobile = useIsMobile();
  const dadoApi = new ContaCorrenteApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  const [isSubmitting, setIsSubmitting] = useState(false);
  const initialized = useRef(false);
  const validationForm = useValidationsContaCorrente();

  const [_nomeClientes, setNomeClientes] = useState("");
  const clientesApi = new ClientesApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );

  if (getParamFromUrl("clientes") > 0) {
    if (contacorrenteData.id === 0 && contacorrenteData.cliente == 0) {
      clientesApi
        .getById(getParamFromUrl("clientes"))
        .then((response) => {
          setNomeClientes(response.data.nome);
        })
        .catch((error) => {
          if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
            console.log("Error unexpected");
        });

      contacorrenteData.cliente = getParamFromUrl("clientes");
    }
  }
  const addValorCliente = (e: any) => {
    if (e?.id > 0) onChange({ target: { name: "cliente", value: e.id } });
  };

  const onConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    if (e.stopPropagation) e.stopPropagation();

    if (!isSubmitting) {
      setIsSubmitting(true);

      try {
        onSubmit(e);
      } catch (error) {
        console.log("Erro ao submeter formulário de ContaCorrente:");
        setIsSubmitting(false);
        if (onError) onError();
      }
    }
  };

  const handleCancel = () => {
    if (onReload) {
      onReload(); // Recarrega os dados originais
    } else {
      onClose(); // Comportamento padrão se não há callback de recarga
    }
  };

  const handleDirectSave = () => {
    if (!isSubmitting) {
      setIsSubmitting(true);

      try {
        const syntheticEvent = {
          preventDefault: () => {},
          target: document.getElementById(
            `ContaCorrenteForm-${contacorrenteData.id}`,
          ),
        } as unknown as React.FormEvent;

        onSubmit(syntheticEvent);
      } catch (error) {
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
          console.log("Erro ao salvar ContaCorrente diretamente");
        setIsSubmitting(false);
        if (onError) onError();
      }
    }
  };

  useEffect(() => {
    const el = document.querySelector(".nameFormMobile");
    if (el) {
      el.textContent =
        contacorrenteData?.id == 0
          ? "Editar ContaCorrente"
          : "Adicionar Conta Corrente";
    }
  }, [contacorrenteData.id]);

  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-ContaCorrente"
            : "form-container form-container-ContaCorrente"
        }
      >
        <form
          className="formInputCadInc"
          id={`ContaCorrenteForm-${contacorrenteData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
              isMobile={false}
              validationForm={validationForm}
              entity="ContaCorrente"
              data={contacorrenteData}
              isSubmitting={isSubmitting}
              setIsSubmitting={setIsSubmitting}
              onClose={onClose}
              formId={`ContaCorrenteForm-${contacorrenteData.id}`}
              preventPropagation={true}
              onSave={handleDirectSave}
              onCancel={handleCancel}
            />
          )}
          <div className="grid-container">
            <InputName
              type="text"
              id="data"
              label="Data"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="data"
              value={contacorrenteData.data}
              placeholder={`Informe Data`}
              onChange={onChange}
              required
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="ciacordo"
              label="CIAcordo"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="ciacordo"
              value={contacorrenteData.ciacordo}
              onChange={onChange}
            />

            <InputCheckbox
              dataForm={contacorrenteData}
              label="Quitado"
              name="quitado"
              checked={contacorrenteData.quitado}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="idcontrato"
              label="IDContrato"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="idcontrato"
              value={contacorrenteData.idcontrato}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="quitadoid"
              label="QuitadoID"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="quitadoid"
              value={contacorrenteData.quitadoid}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="debitoid"
              label="DebitoID"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="debitoid"
              value={contacorrenteData.debitoid}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="livrocaixaid"
              label="LivroCaixaID"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="livrocaixaid"
              value={contacorrenteData.livrocaixaid}
              onChange={onChange}
            />

            <InputCheckbox
              dataForm={contacorrenteData}
              label="Sucumbencia"
              name="sucumbencia"
              checked={contacorrenteData.sucumbencia}
              onChange={onChange}
            />
            <InputCheckbox
              dataForm={contacorrenteData}
              label="DistRegra"
              name="distregra"
              checked={contacorrenteData.distregra}
              onChange={onChange}
            />
          </div>
          <div className="grid-container">
            <InputInput
              type="text"
              maxLength={2048}
              id="dtoriginal"
              label="DtOriginal"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="dtoriginal"
              value={contacorrenteData.dtoriginal}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="processo"
              label="Processo"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="processo"
              value={contacorrenteData.processo}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="parcelax"
              label="ParcelaX"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="parcelax"
              value={contacorrenteData.parcelax}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="valor"
              label="Valor"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="valor"
              value={contacorrenteData.valor}
              onChange={onChange}
            />

            <ClientesComboBox
              name={"cliente"}
              dataForm={contacorrenteData}
              value={contacorrenteData.cliente}
              setValue={addValorCliente}
              label={"Clientes"}
            />

            <InputInput
              type="text"
              maxLength={2147483647}
              id="historico"
              label="Historico"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="historico"
              value={contacorrenteData.historico}
              onChange={onChange}
            />

            <InputCheckbox
              dataForm={contacorrenteData}
              label="Contrato"
              name="contrato"
              checked={contacorrenteData.contrato}
              onChange={onChange}
            />
            <InputCheckbox
              dataForm={contacorrenteData}
              label="Pago"
              name="pago"
              checked={contacorrenteData.pago}
              onChange={onChange}
            />
            <InputCheckbox
              dataForm={contacorrenteData}
              label="Distribuir"
              name="distribuir"
              checked={contacorrenteData.distribuir}
              onChange={onChange}
            />
          </div>
          <div className="grid-container">
            <InputCheckbox
              dataForm={contacorrenteData}
              label="LC"
              name="lc"
              checked={contacorrenteData.lc}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="idhtrab"
              label="IDHTrab"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="idhtrab"
              value={contacorrenteData.idhtrab}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="nroparcelas"
              label="NroParcelas"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="nroparcelas"
              value={contacorrenteData.nroparcelas}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="valorprincipal"
              label="ValorPrincipal"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="valorprincipal"
              value={contacorrenteData.valorprincipal}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="parcelaprincipalid"
              label="ParcelaPrincipalID"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="parcelaprincipalid"
              value={contacorrenteData.parcelaprincipalid}
              onChange={onChange}
            />

            <InputCheckbox
              dataForm={contacorrenteData}
              label="Hide"
              name="hide"
              checked={contacorrenteData.hide}
              onChange={onChange}
            />

            <InputInput
              type="text"
              maxLength={2048}
              id="datapgto"
              label="DataPgto"
              dataForm={contacorrenteData}
              className="input-default-main"
              name="datapgto"
              value={contacorrenteData.datapgto}
              onChange={onChange}
            />
          </div>
        </form>

        {isMobile && (
          <ButtonSalvarCrud
            isMobile={true}
            validationForm={validationForm}
            entity="ContaCorrente"
            data={contacorrenteData}
            isSubmitting={isSubmitting}
            setIsSubmitting={setIsSubmitting}
            onClose={onClose}
            formId={`ContaCorrenteForm-${contacorrenteData.id}`}
            preventPropagation={true}
            onSave={handleDirectSave}
            onCancel={handleCancel}
          />
        )}

        <FooterIncDocker>
          <CopyButton formId={`ContaCorrenteForm-${contacorrenteData.id}`} />

          <PrintButton formId={`ContaCorrenteForm-${contacorrenteData.id}`} />

          <AuditorButton
            tableName="ContaCorrente"
            coolName="Conta Corrente"
            id={contacorrenteData.id}
          />

          <DeleteButton
            page={"/pages/contacorrente"}
            id={contacorrenteData.id}
            dadoApi={dadoApi}
          />
        </FooterIncDocker>
      </div>
      <div className="form-spacer"></div>
    </>
  );
};
