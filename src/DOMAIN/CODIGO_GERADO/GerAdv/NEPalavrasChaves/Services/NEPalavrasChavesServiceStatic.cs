...
public partial class NEPalavrasChavesService
{
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterNEPalavrasChaves filtro)
    {
        ...
        cWhere.Append(filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBNEPalavrasChavesDicInfo.PTabelaNome}].[{DBNEPalavrasChavesDicInfo.Nome}]  {DevourerConsts.MsiCollate} like @{nameof(DBNEPalavrasChavesDicInfo.Nome)}");
        if (!filtro.Codigo_filtro.IsEmpty() && filtro.Codigo_filtro_end.IsEmpty())
        {
            cWhere.Append(filtro.Codigo_filtro <= 0 ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBNEPalavrasChavesDicInfo.PTabelaNome}].[{DBNEPalavrasChavesDicInfo.CampoCodigo}] >= @{nameof(DBNEPalavrasChavesDicInfo.CampoCodigo)}");
        }
        else
        {
            cWhere.Append((filtro.Codigo_filtro <= 0 && filtro.Codigo_filtro_end <= 0) ? string.Empty : (!(filtro.Codigo_filtro <= 0) && !(filtro.Codigo_filtro_end <= 0)) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBNEPalavrasChavesDicInfo.CampoCodigo} BETWEEN @{nameof(DBNEPalavrasChavesDicInfo.CampoCodigo)} AND @{nameof(DBNEPalavrasChavesDicInfo.CampoCodigo)}_end" : !(filtro.Codigo_filtro <= 0) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBNEPalavrasChavesDicInfo.CampoCodigo} = @{nameof(DBNEPalavrasChavesDicInfo.CampoCodigo)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBNEPalavrasChavesDicInfo.CampoCodigo} <= @{nameof(DBNEPalavrasChavesDicInfo.CampoCodigo)}_end");
        }
