  ...
  const loadPaises = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Paises";
...
export const usePaisesList = (
  dataService: IPaisesService,
  currentFilter?: FilterPaises | undefined | null,
) => {
  const [data, setData] = useState<IPaises[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPaises | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterPaises | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterPaises));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar paises";
