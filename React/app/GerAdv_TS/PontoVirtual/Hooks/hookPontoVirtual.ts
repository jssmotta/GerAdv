...
export const usePontoVirtualList = (dataService: IPontoVirtualService, currentFilter?: FilterPontoVirtual | undefined | null) => {
  const [data, setData] = useState<IPontoVirtual[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterPontoVirtual | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterPontoVirtual | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterPontoVirtual);
      setData(result);
    } catch (err) {
