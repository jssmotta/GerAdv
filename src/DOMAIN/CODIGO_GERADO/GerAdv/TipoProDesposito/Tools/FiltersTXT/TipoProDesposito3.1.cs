...
public static class FilterTipoProDespositoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoProDesposito em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoProDesposito:

- O filter irá filtrar os TipoProDesposito conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterTipoProDesposito:(Tipo Pro Desposito)
- nome: _nome ou descrição para Tipo Pro Desposito;
...
- Se ouvir "todos os TipoProDesposito" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoProDesposito? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoProDesposito length: {result.Length}");
#endif
