...
public partial class EnquadramentoEmpresaController(IEnquadramentoEmpresaService enquadramentoempresaService) : ControllerBase
{
    ...
    {
        _logger.Info("EnquadramentoEmpresa: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _enquadramentoempresaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("EnquadramentoEmpresa: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _enquadramentoempresaService.UpdateColumns(parameters, uri);
        if (!result)
        {
