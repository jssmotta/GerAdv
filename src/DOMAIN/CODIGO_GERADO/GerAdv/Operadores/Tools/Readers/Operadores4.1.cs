...
public partial class OperadoresReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBOperadoresDicInfo.CampoNome}";
...
[{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.Nome}]
                   FROM {DBOperadores.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBOperadoresDicInfo.PTabelaNome}].[{DBOperadoresDicInfo.Cliente}]
...
SELECT TOP (1) 
	FORMAT(operDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(operDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    operVisto as Visto
FROM {oCnn.UseDbo}.Operadores dbq WITH (INDEX = idx_Operadores_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.operQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.operQuemAtu
WHERE [operCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBOperadores.PTabelaNome,
...
FROM {oCnn.UseDbo}.Operadores WITH (INDEX = idx_{DBOperadoresDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [operCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBOperadoresDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(operDtAtu, operDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBOperadoresDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(operDtAtu, operDtCad) DESC;", oCnn.InnerConnection);
