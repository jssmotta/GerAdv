...
export const useContratosList = (
  dataService: IContratosService,
  currentFilter?: FilterContratos | undefined | null,
) => {
  const [data, setData] = useState<IContratos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterContratos | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IContratos): { isValid: boolean; message: string } {
    ...
    try {
      if (data.protestar.length > 50) {
        return {
          isValid: false,
          message: "O campo Protestar não pode ter mais de 50 caracteres.",
        };
      }
      if (data.juros.length > 5) {
        return {
          isValid: false,
          message: "O campo Juros não pode ter mais de 5 caracteres.",
        };
      }
      if (data.documento.length > 15) {
        return {
          isValid: false,
          message: "O campo DOCUMENTO não pode ter mais de 15 caracteres.",
        };
      }
      if (data.email1.length > 300) {
        return {
          isValid: false,
          message: "O campo EMail1 não pode ter mais de 300 caracteres.",
        };
      }
      if (data.email2.length > 300) {
        return {
          isValid: false,
          message: "O campo EMail2 não pode ter mais de 300 caracteres.",
        };
      }
      if (data.email3.length > 300) {
        return {
          isValid: false,
          message: "O campo EMail3 não pode ter mais de 300 caracteres.",
        };
      }
      if (data.pessoa1.length > 100) {
        return {
          isValid: false,
          message: "O campo Pessoa1 não pode ter mais de 100 caracteres.",
        };
      }
      if (data.pessoa2.length > 100) {
        return {
          isValid: false,
          message: "O campo Pessoa2 não pode ter mais de 100 caracteres.",
        };
      }
      if (data.pessoa3.length > 100) {
        return {
          isValid: false,
          message: "O campo Pessoa3 não pode ter mais de 100 caracteres.",
        };
      }
      ...
      if (data.chavecontrato.length > 50) {
        return {
          isValid: false,
          message: "O campo ChaveContrato não pode ter mais de 50 caracteres.",
        };
      }
      if (data.multa.length > 10) {
        return {
          isValid: false,
          message: "O campo Multa não pode ter mais de 10 caracteres.",
        };
      }
