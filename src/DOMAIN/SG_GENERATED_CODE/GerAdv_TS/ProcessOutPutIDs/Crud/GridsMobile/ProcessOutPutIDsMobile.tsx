...
export const ProcessOutPutIDsGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('processoutputids-columns-order-mobile'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('processoutputids-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
useEffect(() => {
    ...
    scrollSelector: '.grid-mobile-crud.grid-mobile-processoutputids .k-grid-content',
    waitForDOM: true,
    retryDelay: 100,
