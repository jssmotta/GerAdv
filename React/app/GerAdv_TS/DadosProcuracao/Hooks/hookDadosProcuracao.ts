...
export const useDadosProcuracaoList = (dataService: IDadosProcuracaoService, currentFilter?: FilterDadosProcuracao | undefined | null) => {
  const [data, setData] = useState<IDadosProcuracao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterDadosProcuracao | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterDadosProcuracao | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterDadosProcuracao);
      setData(result);
    } catch (err) {
