...
export const useGUTTipoForm = (
  ...
  const loadGUTTipo = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialGUTTipo);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar G U T Tipo';
      setError(errorMessage);
      console.log('Erro ao carregar G U T Tipo');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialGUTTipo]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadGUTTipo, resetForm]);
  return returnValue;
};


export const useGUTTipoNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('GUTTipo', (entity) => {
      try {
...
export const useGUTTipoList = (dataService: IGUTTipoService) => {
  const [data, setData] = useState<IGUTTipo[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar guttipo';
      setError(errorMessage);
      console.log('Erro ao carregar guttipo');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
