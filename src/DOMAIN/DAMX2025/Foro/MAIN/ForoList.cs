...
#region ListarDados_Foro
    public static async IAsyncEnumerable<DBForo> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBForo(dbRec);
        }
    }

    public static IEnumerable<DBForo> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
