...
import { useAgendaStatusFilter } from '../../Hooks/hookAgendaStatusFilter';
...
const AgendaStatusGrid: React.FC = () => {
  ...
  const [agendastatusData, setAgendaStatusData] = useState<IAgendaStatus[]>([]);
...
const {
  ...
  fetchData, 
  refreshData
} = useAgendaStatusList(agendastatusService, currFilter);

const handleFetchWithFilter = useCallback(async (filtro?: FilterAgendaStatus | undefined | null) => {
  setCurrFilter(filtro);
  if (filtro && Object.keys(filtro).length > 0) {
    sessionStorage.setItem(btoa('AgendaStatusFilter'), btoa(JSON.stringify(filtro)));
  }
  ...
  const offlineData = await agendastatusService.getAll(
  filtro ?? {} as FilterAgendaStatus, 
  onlineData => {
    setAgendaStatusData(onlineData);
    setLoading(false);
  }
);
setAgendaStatusData(offlineData);
setLoading(false);

}, []);
const loadFilter = useCallback(() => {
  if (isInitialized) return; // Evita múltiplas chamadas
    const filtro = {} as FilterAgendaStatus;

    const filtroMem = sessionStorage.getItem(btoa('AgendaStatusFilter')) ? JSON.parse(atob(sessionStorage.getItem(btoa('AgendaStatusFilter')) ?? '') || '{}') : {} as FilterAgendaStatus;
...
const {
  ...
  clearFilter, 
  hasActiveFilter
} = useAgendaStatusFilter({ handleFetchWithFilter });
  useEffect(() => {
