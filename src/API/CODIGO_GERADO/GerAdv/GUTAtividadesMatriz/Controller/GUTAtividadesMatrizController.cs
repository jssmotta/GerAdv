...
public partial class GUTAtividadesMatrizController(IGUTAtividadesMatrizService gutatividadesmatrizService) : ControllerBase
{
    ...
    {
        _logger.Info("GUTAtividadesMatriz: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _gutatividadesmatrizService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("GUTAtividadesMatriz: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _gutatividadesmatrizService.UpdateColumns(parameters, uri);
        if (!result)
        {
