...
export const usePontoVirtualAcessosList = (dataService: IPontoVirtualAcessosService, currentFilter?: FilterPontoVirtualAcessos | undefined | null) => {
  const [data, setData] = useState<IPontoVirtualAcessos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterPontoVirtualAcessos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterPontoVirtualAcessos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterPontoVirtualAcessos);
      setData(result);
    } catch (err) {
