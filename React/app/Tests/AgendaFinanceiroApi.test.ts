...
import { AgendaFinanceiroApiError } from '../GerAdv_TS/AgendaFinanceiro/Apis/ApiAgendaFinanceiro';
import { AgendaFinanceiroApi } from '@/app/GerAdv_TS/AgendaFinanceiro/Apis/ApiAgendaFinanceiro';
import { FilterAgendaFinanceiro } from '../GerAdv_TS/AgendaFinanceiro/Filters/AgendaFinanceiro';
...
describe('AgendaFinanceiroApi', () => {
  let agendafinanceiroApi: AgendaFinanceiroApi;
  ...
  beforeEach(() => {
    ...
    agendafinanceiroApi = new AgendaFinanceiroApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendafinanceiroApi).toBeInstanceOf(AgendaFinanceiroApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaFinanceiroApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaFinanceiroApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [AgendaFinanceiroTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all agendafinanceiro items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendafinanceiroApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendafinanceiroApi.getAll()).rejects.toThrow(AgendaFinanceiroApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendafinanceiroApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaFinanceiro_last_getAll_${200}`)));
    });
    ...
    it('should throw AgendaFinanceiroApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaFinanceiro = AgendaFinanceiroTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAgendaFinanceiro,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch agendafinanceiro by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendafinanceiroApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaFinanceiro));
      ...
      expect(result.data).toEqual(mockAgendaFinanceiro);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaFinanceiro',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaFinanceiro = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendafinanceiro items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendafinanceiroApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaFinanceiro: IAgendaFinanceiro = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendafinanceiro successfully', async () => {
      const responseAgendaFinanceiro = { ...mockAgendaFinanceiro, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaFinanceiro,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendafinanceiroApi.addAndUpdate(mockAgendaFinanceiro);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockAgendaFinanceiro,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing agendafinanceiro successfully', async () => {
      const existingAgendaFinanceiro = { ...mockAgendaFinanceiro, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaFinanceiro,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendafinanceiroApi.addAndUpdate(existingAgendaFinanceiro);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendafinanceiroApi.addAndUpdate(mockAgendaFinanceiro)).rejects.toThrow(AgendaFinanceiroApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaFinanceiro',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendafinanceiro successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendafinanceiroApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendafinanceiroApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaFinanceiro',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendafinanceiroApi.delete(1)).rejects.toThrow(AgendaFinanceiroApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaFinanceiro = { funcionario: 1 };
      
      const result = agendafinanceiroApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await agendafinanceiroApi.delete(1);
  ...
  describe('AgendaFinanceiroApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaFinanceiroApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaFinanceiroApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaFinanceiroApiError('Test message', 500);
