...
#region ListarDados_Auditor4K
    public static async IAsyncEnumerable<DBAuditor4K> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAuditor4K(dbRec);
        }
    }

    public static IEnumerable<DBAuditor4K> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
