...
export class FuncionariosApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Funcionarios_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Funcionarios');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Funcionarios_last_getById_${id}`);
        try {
        ...
        public async getListN(max?: number, filtro?: FilterFuncionarios): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Funcionarios_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Funcionarios');
        }
    }

            public async filter(filtro: FilterFuncionarios): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Funcionarios_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Funcionarios');
                }
            }


    public async validation(regFuncionarios: IFuncionarios): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regFuncionarios as Funcionarios, this.getHeaders());
    ...
    public async addAndUpdate(regFuncionarios: IFuncionarios): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regFuncionarios as Funcionarios, this.getHeaders());
            var register = result.data as IFuncionarios;        
            const action = regFuncionarios.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Funcionarios. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Funcionarios',
                        id: id,
                        action: NotifySystemActions.ERROR,
    ...
    public useFilter(filtro: FilterFuncionarios) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Funcionarios[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de funcionarios:', error);
            this.notificationService.notify({
              entity: 'Funcionarios',
              id: 0,
              action: NotifySystemActions.ERROR,
