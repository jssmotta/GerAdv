...
#region ListarDados_CargosEscClass
    public static async IAsyncEnumerable<DBCargosEscClass> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBCargosEscClass(dbRec);
        }
    }

    public static IEnumerable<DBCargosEscClass> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
