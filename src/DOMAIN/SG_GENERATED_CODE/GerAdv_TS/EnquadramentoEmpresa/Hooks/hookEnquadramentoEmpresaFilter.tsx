...
import { FilterEnquadramentoEmpresa } from '@/app/GerAdv_TS/EnquadramentoEmpresa/Filters/EnquadramentoEmpresa';
...
interface UseEnquadramentoEmpresaFilterProps {
    handleFetchWithFilter: (filtro?: FilterEnquadramentoEmpresa | undefined | null) => Promise<void>;
}

interface UseEnquadramentoEmpresaFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterEnquadramentoEmpresa;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterEnquadramentoEmpresa>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterEnquadramentoEmpresa) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterEnquadramentoEmpresa>) => React.ReactNode;
...
export const useEnquadramentoEmpresaFilter = ({ handleFetchWithFilter }: UseEnquadramentoEmpresaFilterProps): UseEnquadramentoEmpresaFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterEnquadramentoEmpresa>({} as FilterEnquadramentoEmpresa);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterEnquadramentoEmpresa;
    ...
    const handleConfirmSearch = async (filter: FilterEnquadramentoEmpresa) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterEnquadramentoEmpresa);
        sessionStorage.removeItem(btoa('EnquadramentoEmpresaFilter'));
        handleFetchWithFilter({} as FilterEnquadramentoEmpresa);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterEnquadramentoEmpresa>) => (        
        <>
        <InputInput
                        type='text'
                        id='nome'
