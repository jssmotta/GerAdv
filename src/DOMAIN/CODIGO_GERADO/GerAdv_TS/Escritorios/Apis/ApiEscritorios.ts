...
export class EscritoriosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class EscritoriosApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Escritorios');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Escritorios com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Escritorios');
        }
    }

            public async filter(filtro: FilterEscritorios): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Escritorios_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Escritorios');
                }
            }


    public async validation(regEscritorios: IEscritorios): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEscritorios as Escritorios, this.getHeaders());
    ...
    public async addAndUpdate(regEscritorios: IEscritorios): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Escritorios');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Escritorios com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEscritorios) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Escritorios[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de escritorios:', error);
            this.notificationService.notify({
              entity: 'Escritorios',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos escritorios'
            });
          }
        });
    }
