...
public partial class AreasJusticaService(IOptions<AppSettings> appSettings, IAreasJusticaReader reader, IAreasJusticaValidation validation, IAreasJusticaWriter writer, IAreaReader areaReader, IJusticaReader justicaReader, HybridCache cache) : IAreasJusticaService, IDisposable
{
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AreasJustica: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAreasJustica(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBAreasJusticaDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AreasJustica: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAreasJustica(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBAreasJusticaDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
