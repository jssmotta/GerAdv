...
public partial class PoderJudiciarioAssociadoController(IPoderJudiciarioAssociadoService poderjudiciarioassociadoService) : ControllerBase
{
    ...
    {
        _logger.Info("PoderJudiciarioAssociado: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _poderjudiciarioassociadoService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("PoderJudiciarioAssociado: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _poderjudiciarioassociadoService.UpdateColumns(parameters, uri);
        if (!result)
        {
