...
const AlertasComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IAlertas | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IAlertas | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IAlertas);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <AlertasWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedAlertas={editar ?? addValue ?? AlertasEmpty()} /> 
            )}
