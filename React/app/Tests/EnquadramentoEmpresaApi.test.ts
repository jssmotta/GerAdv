...
import { EnquadramentoEmpresaApiError } from '../GerAdv_TS/EnquadramentoEmpresa/Apis/ApiEnquadramentoEmpresa';
import { EnquadramentoEmpresaApi } from '@/app/GerAdv_TS/EnquadramentoEmpresa/Apis/ApiEnquadramentoEmpresa';
import { FilterEnquadramentoEmpresa } from '../GerAdv_TS/EnquadramentoEmpresa/Filters/EnquadramentoEmpresa';
...
describe('EnquadramentoEmpresaApi', () => {
  let enquadramentoempresaApi: EnquadramentoEmpresaApi;
  ...
  beforeEach(() => {
    ...
    enquadramentoempresaApi = new EnquadramentoEmpresaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(enquadramentoempresaApi).toBeInstanceOf(EnquadramentoEmpresaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EnquadramentoEmpresaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EnquadramentoEmpresaApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [EnquadramentoEmpresaTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all enquadramentoempresa items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enquadramentoempresaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(enquadramentoempresaApi.getAll()).rejects.toThrow(EnquadramentoEmpresaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await enquadramentoempresaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-EnquadramentoEmpresa_last_getAll_${200}`)));
    });
    ...
    it('should throw EnquadramentoEmpresaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEnquadramentoEmpresa = EnquadramentoEmpresaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockEnquadramentoEmpresa,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch enquadramentoempresa by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await enquadramentoempresaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEnquadramentoEmpresa));
      ...
      expect(result.data).toEqual(mockEnquadramentoEmpresa);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnquadramentoEmpresa',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterEnquadramentoEmpresa = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter enquadramentoempresa items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await enquadramentoempresaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockEnquadramentoEmpresa: IEnquadramentoEmpresa = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new enquadramentoempresa successfully', async () => {
      const responseEnquadramentoEmpresa = { ...mockEnquadramentoEmpresa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEnquadramentoEmpresa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enquadramentoempresaApi.addAndUpdate(mockEnquadramentoEmpresa);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockEnquadramentoEmpresa,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing enquadramentoempresa successfully', async () => {
      const existingEnquadramentoEmpresa = { ...mockEnquadramentoEmpresa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEnquadramentoEmpresa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await enquadramentoempresaApi.addAndUpdate(existingEnquadramentoEmpresa);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(enquadramentoempresaApi.addAndUpdate(mockEnquadramentoEmpresa)).rejects.toThrow(EnquadramentoEmpresaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnquadramentoEmpresa',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete enquadramentoempresa successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await enquadramentoempresaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(enquadramentoempresaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EnquadramentoEmpresa',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(enquadramentoempresaApi.delete(1)).rejects.toThrow(EnquadramentoEmpresaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterEnquadramentoEmpresa = { funcionario: 1 };
      
      const result = enquadramentoempresaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await enquadramentoempresaApi.delete(1);
  ...
  describe('EnquadramentoEmpresaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EnquadramentoEmpresaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EnquadramentoEmpresaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EnquadramentoEmpresaApiError('Test message', 500);
