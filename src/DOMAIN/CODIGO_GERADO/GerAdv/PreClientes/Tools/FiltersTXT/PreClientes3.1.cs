...
public static class FilterPreClientesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar PreClientes em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterPreClientes:

- O filter irá filtrar os PreClientes conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterPreClientes:(Pre Clientes)
- inativo: Inativo;
- quemindicou: QuemIndicou;
- nome: _nome ou descrição para Pre Clientes;
- adv: número ou IDs;
- idrep: número ou IDs;
- juridica: Juridica;
- nomefantasia: NomeFantasia;
...
- inscest: InscEst;
- qualificacao: Qualificacao;
- sexo: sexo do PreClientes: 1=Homem; 0=Mulher; -2147483648=ambos;
- idade: número ou IDs;
- cnpj: somente os 14 caracteres sem mascara;
- cpf: somente os 11 caracteres sem mascara;
- rg: RG;
...
- fax: Fax;
- fone: Fone;
- data: não é a data de cadastramento é a data do PreClientes;
- homepage: HomePage;
- email: EMail;
- assistido: Assistido;
- assrg: AssRG;
...
- Se ouvir "todos os PreClientes" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterPreClientes? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt PreClientes length: {result.Length}");
#endif
