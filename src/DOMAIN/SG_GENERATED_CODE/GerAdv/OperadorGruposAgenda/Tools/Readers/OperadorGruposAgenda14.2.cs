...
public partial class OperadorGruposAgendaWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_OperadorGruposAgenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[OperadorGruposAgenda]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_OperadorGruposAgenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[OperadorGruposAgenda]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_OperadorGruposAgenda_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_OperadorGruposAgenda_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[OperadorGruposAgenda]'))
                BEGIN                    
