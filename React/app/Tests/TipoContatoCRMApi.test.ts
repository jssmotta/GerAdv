...
import { TipoContatoCRMApiError } from '../GerAdv_TS/TipoContatoCRM/Apis/ApiTipoContatoCRM';
import { TipoContatoCRMApi } from '@/app/GerAdv_TS/TipoContatoCRM/Apis/ApiTipoContatoCRM';
import { FilterTipoContatoCRM } from '../GerAdv_TS/TipoContatoCRM/Filters/TipoContatoCRM';
...
describe('TipoContatoCRMApi', () => {
  let tipocontatocrmApi: TipoContatoCRMApi;
  ...
  beforeEach(() => {
    ...
    tipocontatocrmApi = new TipoContatoCRMApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipocontatocrmApi).toBeInstanceOf(TipoContatoCRMApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoContatoCRMApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoContatoCRMApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tipocontatocrm items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipocontatocrmApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipocontatocrmApi.getAll()).rejects.toThrow(TipoContatoCRMApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipocontatocrmApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoContatoCRM_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoContatoCRM',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw TipoContatoCRMApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoContatoCRM = TipoContatoCRMTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoContatoCRM,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipocontatocrm by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipocontatocrmApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoContatoCRM));
      ...
      expect(result.data).toEqual(mockTipoContatoCRM);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoContatoCRM',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoContatoCRM = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipocontatocrm items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipocontatocrmApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoContatoCRM: ITipoContatoCRM = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipocontatocrm successfully', async () => {
      const responseTipoContatoCRM = { ...mockTipoContatoCRM, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoContatoCRM,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipocontatocrmApi.addAndUpdate(mockTipoContatoCRM);
    ...
    it('should update existing tipocontatocrm successfully', async () => {
      const existingTipoContatoCRM = { ...mockTipoContatoCRM, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoContatoCRM,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipocontatocrmApi.addAndUpdate(existingTipoContatoCRM);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipocontatocrmApi.addAndUpdate(mockTipoContatoCRM)).rejects.toThrow(TipoContatoCRMApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoContatoCRM',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipocontatocrm successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipocontatocrmApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipocontatocrmApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoContatoCRM',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipocontatocrmApi.delete(1)).rejects.toThrow(TipoContatoCRMApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoContatoCRM = { funcionario: 1 };
      
      const result = tipocontatocrmApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipocontatocrmApi.delete(1);
  ...
  describe('TipoContatoCRMApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoContatoCRMApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoContatoCRMApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoContatoCRMApiError('Test message', 500);
