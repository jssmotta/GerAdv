...
export const useSMSAliceForm = (
  ...
  const loadSMSAlice = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar S M S Alice";
...
export const useSMSAliceList = (
  dataService: ISMSAliceService,
  currentFilter?: FilterSMSAlice | undefined | null,
) => {
  const [data, setData] = useState<ISMSAlice[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterSMSAlice | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterSMSAlice | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar smsalice";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
