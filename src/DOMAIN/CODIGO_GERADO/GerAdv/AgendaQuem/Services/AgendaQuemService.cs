...
public partial class AgendaQuemService(IOptions<AppSettings> appSettings, IAgendaQuemReader reader, IAgendaQuemValidation validation, IAgendaQuemWriter writer, IAdvogadosReader advogadosReader, IFuncionariosReader funcionariosReader, IPrepostosReader prepostosReader, HybridCache cache) : IAgendaQuemService, IDisposable
{
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AgendaQuem: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAgendaQuem(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBAgendaQuemDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("AgendaQuem: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBAgendaQuem(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBAgendaQuemDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
