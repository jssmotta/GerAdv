...
[InlineData(int.MaxValue)]
[InlineData(int.MinValue)]
public void LiberaAgenda_ShouldAcceptIntegerValues(int value)
{
    _instance.FLiberaAgenda = value;
    Assert.Equal(value, _instance.FLiberaAgenda);
}
...
[Fact]
public void LiberaAgenda_DefaultValue_ShouldBeZero()
{
    var instance = new DBPrepostos();
    Assert.Equal(0, instance.LiberaAgenda);
}
*/
    /*
TEMPLATE_DATETIME_FIELD:
...
[InlineData("31/12/2023")]
[InlineData("15/08/2024")]
public void LiberaAgenda_ShouldFormatDateCorrectly(string dateString)
{
    _instance.FLiberaAgenda = dateString;
    Assert.Equal(dateString, _instance.FLiberaAgenda);
}

[Fact]
public void LiberaAgenda_EmptyDate_ShouldReturnEmptyString()
{
    var instance = new DBPrepostos();
    Assert.Equal(string.Empty, instance.LiberaAgenda);
}
*/
    /*
TEMPLATE_CPF_FIELD:
...
[InlineData("384.004.020-57", "38400402057")]
[InlineData("", "")]
public void LiberaAgenda_ShouldValidateCPF(string input, string expected)
{
    if (input == "" || IsValidCPF(input))
    {
        _instance.FLiberaAgenda = input;
        Assert.Equal(expected, _instance.FLiberaAgenda);
    }
    else
    {
        Assert.Throws<ArgumentException>(() => _instance.FLiberaAgenda = input);
    }
}
*/
    /*
...
[InlineData("test@example.com")]
[InlineData("user.name@domain.co.uk")]
public void LiberaAgenda_ValidEmail_ShouldAccept(string email)
{
    _instance.FLiberaAgenda = email;
    Assert.Equal(email, _instance.FLiberaAgenda);
}

[Theory]
[InlineData("invalid-email")]
[InlineData("@domain.com")]
public void LiberaAgenda_InvalidEmail_ShouldThrow(string email)
{
    Assert.Throws<ArgumentException>(() => _instance.FLiberaAgenda = email);
}
*/
    [Theory]
    [InlineData("", "")]
