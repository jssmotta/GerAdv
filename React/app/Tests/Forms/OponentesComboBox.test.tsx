// app/TestsForms/OponentesComboBox.test.tsx - Vers�o Corrigida
...
import { OponentesEmpty, OponentesTestEmpty } from '@/app/GerAdv_TS/Models/Oponentes';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Oponentes/Hooks/hookOponentes', () => ({
  useOponentesComboBox: jest.fn(() => ({
    options: [
    { ...OponentesEmpty(), id: 1, nome: 'Oponentes 1' },
    { ...OponentesEmpty(), id: 2, nome: 'Oponentes 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Oponentes/Apis/ApiOponentes', () => ({
  OponentesApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Oponentes' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Oponentes' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Oponentes/Services/Oponentes.service', () => ({
  OponentesService: jest.fn().mockImplementation(() => ({
    fetchOponentesById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Oponentes' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Oponentes/Crud/Grids/OponentesWindow', () => {
  return function MockOponentesWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='oponentes-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Oponentes' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...OponentesTestEmpty(), id: 1, nome: 'Oponentes 1' },
  { ...OponentesTestEmpty(), id: 2, nome: 'Oponentes 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: OponentesEmpty(), 
};
describe('OponentesComboBox', () => {
  ...
  const { useOponentesComboBox } = require('@/app/GerAdv_TS/Oponentes/Hooks/hookOponentes');
  (useOponentesComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<OponentesComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Oponentes')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...OponentesTestEmpty(), id: 1, nome: 'Oponentes 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useOponentesComboBox } = require('@/app/GerAdv_TS/Oponentes/Hooks/hookOponentes');
    useOponentesComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens OponentesWindow on action button click (add)', async () => {
    render(<OponentesComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('oponentes-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('oponentes-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<OponentesComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles OponentesWindow onClose and onSuccess', () => {
    render(<OponentesComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<OponentesComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
