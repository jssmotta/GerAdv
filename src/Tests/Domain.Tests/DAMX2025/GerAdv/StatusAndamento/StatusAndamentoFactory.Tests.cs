...
public class FStatusAndamentoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FStatusAndamentoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FStatusAndamentoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FStatusAndamentoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "sanCodigo")
        var dt = new DataTable();
        dt.Columns.Add("sanCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["sanCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FStatusAndamento>();
    }
    ...
    {
        // Arrange
        var factory = new FStatusAndamentoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("sanCodigo", typeof(int));
        mockDataTable.Columns.Add("sanNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["sanCodigo"] = testId;
        row["sanNome"] = "Test StatusAndamento";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["sanCodigo"] = 123;
        row["sanNome"] = "Test StatusAndamento Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FStatusAndamento properties
        }
    }
