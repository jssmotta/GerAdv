...
import StatusBiuWindow from '../Crud/Grids/StatusBiuWindow';
...
import { useStatusBiuComboBox } from '../Hooks/hookStatusBiu';
const StatusBiuComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useStatusBiuComboBox(statusbiuService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IStatusBiu | null>(null);
const [addRecord, setAddRecord] = useState<IStatusBiu | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await statusbiuService.fetchStatusBiuById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = StatusBiuEmpty();
...
const handleSuccess = (newStatusBiu?: IStatusBiu) => {
  if (newStatusBiu) {
    setValue({ id: newStatusBiu.id, nome: newStatusBiu.nome });
    loadRecordForEdit(newStatusBiu.id);
    handleValueChange(newStatusBiu);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .statusbiu-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .statusbiu-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .statusbiu-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedStatusBiu={editRecord || addRecord || StatusBiuEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'statusbiu-dropdown-popup-msi'
          }}
          />
...
export default StatusBiuComboBox;