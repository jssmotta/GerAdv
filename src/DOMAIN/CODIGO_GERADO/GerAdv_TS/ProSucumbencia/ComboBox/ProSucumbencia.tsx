...
const ProSucumbenciaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProSucumbencia | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProSucumbencia | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProSucumbencia);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProSucumbenciaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProSucumbencia={editar ?? addValue ?? ProSucumbenciaEmpty()} /> 
            )}
