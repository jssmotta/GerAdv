...
public partial class LivroCaixaClientesService(IOptions<AppSettings> appSettings, IFLivroCaixaClientesFactory livrocaixaclientesFactory, ILivroCaixaClientesReader reader, ILivroCaixaClientesValidation validation, ILivroCaixaClientesWriter writer, ILivroCaixaReader livrocaixaReader, IClientesReader clientesReader, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : ILivroCaixaClientesService, IDisposable
{
    ...
    private readonly IFLivroCaixaClientesFactory livrocaixaclientesFactory = livrocaixaclientesFactory;
    private readonly ILivroCaixaClientesReader reader = reader;
    private readonly ILivroCaixaClientesValidation validation = validation;
    private readonly ILivroCaixaClientesWriter writer = writer;
    private readonly ILivroCaixaReader livrocaixaReader = livrocaixaReader;
    ...
    public async Task<IEnumerable<LivroCaixaClientesResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-LivroCaixaClientes-Filter-{keyCache}";
    ...
    private async Task<LivroCaixaClientesResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<LivroCaixaClientesResponse?> AddAndUpdate([FromBody] Models.LivroCaixaClientes regLivroCaixaClientes, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regLivroCaixaClientes, operadorId, oCnn);
    ...
    public async Task<LivroCaixaClientesResponse?> Validation([FromBody] Models.LivroCaixaClientes regLivroCaixaClientes, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regLivroCaixaClientes.Id, oCnn);
    }
    ...
    public async Task<LivroCaixaClientesResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var livrocaixaclientes = await reader.Read(id, oCnn);
        try
        {
            if (livrocaixaclientes != null)
            {
                await writer.Delete(livrocaixaclientes, nOperador, oCnn);
