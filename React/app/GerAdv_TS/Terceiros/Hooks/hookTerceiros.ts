...
export const useTerceirosForm = (
  ...
  const loadTerceiros = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Terceiros";
...
export const useTerceirosList = (
  dataService: ITerceirosService,
  currentFilter?: FilterTerceiros | undefined | null,
) => {
  const [data, setData] = useState<ITerceiros[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTerceiros | undefined | null>(
    currentFilter,
  );
    ...
    try {
      ...
      if (data.varaforocomarca.length > 255) {
        return {
          isValid: false,
          message:
            "O campo VaraForoComarca não pode ter mais de 255 caracteres.",
        };
      }
