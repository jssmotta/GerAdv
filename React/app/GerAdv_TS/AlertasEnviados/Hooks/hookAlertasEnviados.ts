...
export const useAlertasEnviadosList = (dataService: IAlertasEnviadosService, currentFilter?: FilterAlertasEnviados | undefined | null) => {
  const [data, setData] = useState<IAlertasEnviados[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterAlertasEnviados | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterAlertasEnviados | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterAlertasEnviados);
      setData(result);
    } catch (err) {
