...
export class CidadeApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class CidadeApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Cidade_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Cidade');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Cidade_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Cidade com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterCidade): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Cidade_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Cidade');
        }
    }

            public async filter(filtro: FilterCidade): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Cidade_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Cidade');
                }
            }


    public async validation(regCidade: ICidade): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regCidade as Cidade, this.getHeaders());
    ...
    public async addAndUpdate(regCidade: ICidade): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regCidade as Cidade, this.getHeaders());
            var register = result.data as ICidade;        
            const action = regCidade.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Cidade');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Cidade. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Cidade',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Cidade com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterCidade) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Cidade[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de cidade:', error);
            this.notificationService.notify({
              entity: 'Cidade',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos cidade'
            });
          }
        });
    }
