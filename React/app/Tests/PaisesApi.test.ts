...
import { PaisesApiError } from '../GerAdv_TS/Paises/Apis/ApiPaises';
import { PaisesApi } from '@/app/GerAdv_TS/Paises/Apis/ApiPaises';
import { FilterPaises } from '../GerAdv_TS/Paises/Filters/Paises';
...
describe('PaisesApi', () => {
  ...
  beforeEach(() => {
    ...
    paisesApi = new PaisesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(paisesApi).toBeInstanceOf(PaisesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PaisesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PaisesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all paises items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await paisesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(paisesApi.getAll()).rejects.toThrow(PaisesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await paisesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Paises_last_getAll_${200}`)));
    });
    ...
    it('should throw PaisesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPaises = PaisesTestEmpty();
    ...
    it('should fetch paises by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await paisesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPaises));
  ...
  describe('filter', () => {
    ...
    it('should filter paises items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await paisesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new paises successfully', async () => {
      const responsePaises = { ...mockPaises, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePaises,
        status: 200,
        statusText: 'OK',
      ...
      const result = await paisesApi.addAndUpdate(mockPaises);
    ...
    it('should update existing paises successfully', async () => {
      const existingPaises = { ...mockPaises, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPaises,
        status: 200,
        statusText: 'OK',
      ...
      const result = await paisesApi.addAndUpdate(existingPaises);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(paisesApi.addAndUpdate(mockPaises)).rejects.toThrow(PaisesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Paises',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete paises successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await paisesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(paisesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Paises',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(paisesApi.delete(1)).rejects.toThrow(PaisesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterPaises = { funcionario: 1 };
      
      const result = paisesApi.useFilter(mockFilter);
  ...
  describe('PaisesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PaisesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PaisesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PaisesApiError('Test message', 500);
