...
import OponentesRepLegalWindow from '../Crud/Grids/OponentesRepLegalWindow';
...
import { useOponentesRepLegalComboBox } from '../Hooks/hookOponentesRepLegal';
const OponentesRepLegalComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useOponentesRepLegalComboBox(oponentesreplegalService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IOponentesRepLegal | null>(null);
const [addRecord, setAddRecord] = useState<IOponentesRepLegal | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await oponentesreplegalService.fetchOponentesRepLegalById(id);
...
const handleActionClick = () => {
  ...
  const newRecord = OponentesRepLegalEmpty();
...
const handleSuccess = (newOponentesRepLegal?: IOponentesRepLegal) => {
  if (newOponentesRepLegal) {
    setValue({ id: newOponentesRepLegal.id, nome: newOponentesRepLegal.nome });
    loadRecordForEdit(newOponentesRepLegal.id);
    handleValueChange(newOponentesRepLegal);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .oponentesreplegal-dropdown-popup-msi {
  ...
  .oponentesreplegal-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .oponentesreplegal-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .oponentesreplegal-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedOponentesRepLegal={editRecord || addRecord || OponentesRepLegalEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'oponentesreplegal-dropdown-popup-msi'
          }}
          />
