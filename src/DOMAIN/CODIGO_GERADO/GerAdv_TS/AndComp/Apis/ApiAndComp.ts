...
export class AndCompApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class AndCompApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/AndComp`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndComp_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AndComp');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndComp_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar And Comp com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAndComp): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AndComp_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AndComp');
                }
            }


    public async validation(regAndComp: IAndComp): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAndComp as AndComp, this.getHeaders());
    ...
    public async addAndUpdate(regAndComp: IAndComp): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar And Comp');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o AndComp. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'AndComp',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir And Comp com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAndComp) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AndComp[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de andcomp:', error);
            this.notificationService.notify({
              entity: 'AndComp',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos andcomp'
            });
          }
        });
    }
