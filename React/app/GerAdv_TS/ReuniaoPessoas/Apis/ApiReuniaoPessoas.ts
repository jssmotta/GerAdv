...
export class ReuniaoPessoasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ReuniaoPessoasApiError';
    this.status = status;
    this.code = code;
...
export class ReuniaoPessoasApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/ReuniaoPessoas`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ReuniaoPessoas_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ReuniaoPessoas');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ReuniaoPessoas_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Reuniao Pessoas com ID ${id}`);
        }
    }

            public async filter(filtro: FilterReuniaoPessoas): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ReuniaoPessoas_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ReuniaoPessoas');
                }
            }


    public async validation(regReuniaoPessoas: IReuniaoPessoas): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regReuniaoPessoas as ReuniaoPessoas, this.getHeaders());
    ...
    public async addAndUpdate(regReuniaoPessoas: IReuniaoPessoas): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Reuniao Pessoas');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o ReuniaoPessoas. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'ReuniaoPessoas',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Reuniao Pessoas com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterReuniaoPessoas) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ReuniaoPessoas[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de reuniaopessoas:', error);
            this.notificationService.notify({
              entity: 'ReuniaoPessoas',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos reuniaopessoas'
            });
          }
        });
    }
