...
public partial class ProSucumbenciaController(IProSucumbenciaService prosucumbenciaService) : ControllerBase
{
    ...
    {
        _logger.Info("ProSucumbencia: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _prosucumbenciaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProSucumbencia: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _prosucumbenciaService.UpdateColumns(parameters, uri);
        if (!result)
        {
