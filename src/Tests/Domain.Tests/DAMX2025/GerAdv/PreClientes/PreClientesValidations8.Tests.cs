...
#region Class Structure Tests
    ...
    private static GerAdv.Models.PreClientes CreateValidPreClientes()
    {
        return new GerAdv.Models.PreClientes
        {
            Id = 1,
            Inativo = false,
...
#region CPF Validation Tests
    ...
    public async Task ValidateReg_WithDuplicateCPF_ShouldThrowSGValidationException()
    {
        ...
        _mockPreClientesService.Setup(x => x.Filter(It.IsAny<int>(), It.IsAny<FilterPreClientes>(), It.IsAny<string>())).ReturnsAsync([existingPreClientes]);
        // Setup other mocks but don't override the PreClientess service mock
        _ = _mockClientesReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<ClientesResponse?>(new ClientesResponse { Id = id }));
        _ = _mockCidadeReader.Setup(x => x.ReadAsync(It.IsAny<int>(), It.IsAny<MsiSqlConnection>())).Returns<int, MsiSqlConnection>(valueFunction: static async (id, conn) => await Task.FromResult<CidadeResponse?>(new CidadeResponse { Id = id }));
        // Act & Assert
