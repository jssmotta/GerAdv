...
public partial class SituacaoReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var query = $@"SELECT TOP ({max})
                   {campos}
                   FROM {DBSituacao.PTabelaNome.dbo(oCnn)}
...
SELECT TOP (1) 
	FORMAT(sitDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(sitDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    sitVisto as Visto
FROM {oCnn.UseDbo}.Situacao dbq WITH (INDEX = idx_Situacao_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.sitQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.sitQuemAtu
WHERE [sitCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBSituacao.PTabelaNome,
...
FROM {oCnn.UseDbo}.Situacao WITH (INDEX = idx_{DBSituacaoDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [sitCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBSituacaoDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(sitDtAtu, sitDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBSituacaoDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(sitDtAtu, sitDtCad) DESC;", oCnn.InnerConnection);
