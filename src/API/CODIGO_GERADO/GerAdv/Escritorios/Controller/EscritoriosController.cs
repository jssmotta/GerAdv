...
public partial class EscritoriosController(IEscritoriosService escritoriosService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterEscritorios filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Escritorios: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _escritoriosService.Filter(max, filtro, uri);
        return Ok(result);
    }
