...
public partial class HonorariosDadosContratoWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_HonorariosDadosContrato_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[HonorariosDadosContrato]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_HonorariosDadosContrato_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[HonorariosDadosContrato]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_HonorariosDadosContrato_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_HonorariosDadosContrato_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[HonorariosDadosContrato]'))
                BEGIN                    
