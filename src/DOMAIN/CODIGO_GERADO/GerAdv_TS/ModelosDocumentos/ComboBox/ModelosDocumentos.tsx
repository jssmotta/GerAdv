...
const ModelosDocumentosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IModelosDocumentos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IModelosDocumentos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IModelosDocumentos);
    ...
    const handleAddClick = () => {
        ...
        const add = ModelosDocumentosEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ModelosDocumentosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedModelosDocumentos={editar ?? addValue ?? ModelosDocumentosEmpty()} /> 
            )}
