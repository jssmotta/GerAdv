...
export const useAdvogadosForm = (
  ...
  const loadAdvogados = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
...
export const useAdvogadosList = (
  dataService: IAdvogadosService,
  currentFilter?: FilterAdvogados | undefined | null,
) => {
  const [data, setData] = useState<IAdvogados[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterAdvogados | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IAdvogados): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.emailpro.length > 255) {
        return {
          isValid: false,
          message: "O campo EMailPro não pode ter mais de 255 caracteres.",
        };
      }
      ...
      if (data.oab.length > 12) {
        return {
          isValid: false,
          message: "O campo OAB não pode ter mais de 12 caracteres.",
        };
      }
      if (data.nomecompleto.length > 50) {
        return {
          isValid: false,
          message: "O campo NomeCompleto não pode ter mais de 50 caracteres.",
        };
      }
      ...
      if (data.ctps.length > 15) {
        return {
          isValid: false,
          message: "O campo CTPS não pode ter mais de 15 caracteres.",
        };
      }
      ...
      if (data.secretaria.length > 20) {
        return {
          isValid: false,
          message: "O campo Secretaria não pode ter mais de 20 caracteres.",
        };
      }
      if (data.textoprocuracao.length > 200) {
        return {
          isValid: false,
          message:
            "O campo TextoProcuracao não pode ter mais de 200 caracteres.",
        };
      }
      ...
      if (data.especializacao.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo Especializacao não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.pasta.length > 200) {
        return {
          isValid: false,
          message: "O campo Pasta não pode ter mais de 200 caracteres.",
        };
      }
      ...
      if (data.contabancaria.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo ContaBancaria não pode ter mais de 2147483647 caracteres.",
        };
      }
