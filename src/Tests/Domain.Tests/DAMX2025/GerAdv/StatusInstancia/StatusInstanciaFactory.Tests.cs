...
public class FStatusInstanciaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FStatusInstanciaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FStatusInstanciaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FStatusInstanciaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "istCodigo")
        var dt = new DataTable();
        dt.Columns.Add("istCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["istCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FStatusInstancia>();
    }
    ...
    {
        // Arrange
        var factory = new FStatusInstanciaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("istCodigo", typeof(int));
        mockDataTable.Columns.Add("istNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["istCodigo"] = testId;
        row["istNome"] = "Test StatusInstancia";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["istCodigo"] = 123;
        row["istNome"] = "Test StatusInstancia Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FStatusInstancia properties
        }
    }
