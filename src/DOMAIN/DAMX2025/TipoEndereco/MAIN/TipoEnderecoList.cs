...
#region ListarDados_TipoEndereco
    public static async IAsyncEnumerable<DBTipoEndereco> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoEndereco(dbRec);
        }
    }

    public static IEnumerable<DBTipoEndereco> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
