...
public partial class DBAlertas
{
    ...
    /// </summary>
    public static async Task<DBAlertas> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAlertas();
...
#if (!NOTSTORED_Alertas)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFNome || pFldFData || pFldFOperador || pFldFDataAte;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        ...
        if (pFldFDataAte)
            updateTool.Fields(DBAlertasDicInfo.DataAte, m_FDataAte, ETiposCampos.FDate);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBAlertasDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBAlertasDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBAlertasDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBAlertasDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBAlertasDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
