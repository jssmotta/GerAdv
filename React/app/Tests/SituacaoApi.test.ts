...
import { SituacaoApiError } from '../GerAdv_TS/Situacao/Apis/ApiSituacao';
import { SituacaoApi } from '@/app/GerAdv_TS/Situacao/Apis/ApiSituacao';
import { FilterSituacao } from '../GerAdv_TS/Situacao/Filters/Situacao';
...
describe('SituacaoApi', () => {
  let situacaoApi: SituacaoApi;
  ...
  beforeEach(() => {
    ...
    situacaoApi = new SituacaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(situacaoApi).toBeInstanceOf(SituacaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new SituacaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(SituacaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all situacao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await situacaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(situacaoApi.getAll()).rejects.toThrow(SituacaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await situacaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Situacao_last_getAll_${200}`)));
    });
    ...
    it('should throw SituacaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockSituacao = SituacaoTestEmpty();
    ...
    it('should fetch situacao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await situacaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockSituacao));
      ...
      expect(result.data).toEqual(mockSituacao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Situacao',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterSituacao = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter situacao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await situacaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new situacao successfully', async () => {
      const responseSituacao = { ...mockSituacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseSituacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await situacaoApi.addAndUpdate(mockSituacao);
    ...
    it('should update existing situacao successfully', async () => {
      const existingSituacao = { ...mockSituacao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingSituacao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await situacaoApi.addAndUpdate(existingSituacao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(situacaoApi.addAndUpdate(mockSituacao)).rejects.toThrow(SituacaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Situacao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete situacao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await situacaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(situacaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Situacao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(situacaoApi.delete(1)).rejects.toThrow(SituacaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterSituacao = { funcionario: 1 };
      
      const result = situacaoApi.useFilter(mockFilter);
  ...
  describe('SituacaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new SituacaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('SituacaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new SituacaoApiError('Test message', 500);
