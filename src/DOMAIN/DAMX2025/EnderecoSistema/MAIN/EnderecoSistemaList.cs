...
#region ListarDados_EnderecoSistema
    public static async IAsyncEnumerable<DBEnderecoSistema> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBEnderecoSistema(dbRec);
        }
    }

    public static IEnumerable<DBEnderecoSistema> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
