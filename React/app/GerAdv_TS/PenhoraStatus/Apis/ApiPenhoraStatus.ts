...
export class PenhoraStatusApiError extends Error {
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'PenhoraStatusApiError';
    this.status = status;
    this.code = code;
...
export class PenhoraStatusApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/PenhoraStatus`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os PenhoraStatus');
        }
    }
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Penhora Status com ID ${id}`);
        }
    }
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterPenhoraStatus): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-PenhoraStatus_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public async validation(regPenhoraStatus: IPenhoraStatus): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regPenhoraStatus as PenhoraStatus, this.getHeaders());
    ...
    public async addAndUpdate(regPenhoraStatus: IPenhoraStatus): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regPenhoraStatus as PenhoraStatus, this.getHeaders());
            var register = result.data as IPenhoraStatus;        
            const action = regPenhoraStatus.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Penhora Status');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o PenhoraStatus. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'PenhoraStatus',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Penhora Status com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterPenhoraStatus) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<PenhoraStatus[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de penhorastatus:', error);
            this.notificationService.notify({
              entity: 'PenhoraStatus',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos penhorastatus'
            });
          }
        });
    }
