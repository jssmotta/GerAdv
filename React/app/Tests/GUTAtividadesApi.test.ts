...
describe('GUTAtividadesApi', () => {
  let gutatividadesApi: GUTAtividadesApi;
  ...
  beforeEach(() => {
    ...
    gutatividadesApi = new GUTAtividadesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gutatividadesApi).toBeInstanceOf(GUTAtividadesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GUTAtividadesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GUTAtividadesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all gutatividades items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutatividadesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gutatividadesApi.getAll()).rejects.toThrow(GUTAtividadesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gutatividadesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GUTAtividades_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividades',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw GUTAtividadesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGUTAtividades = GUTAtividadesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockGUTAtividades,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch gutatividades by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutatividadesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGUTAtividades));
      ...
      expect(result.data).toEqual(mockGUTAtividades);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividades',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter gutatividades items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gutatividadesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new gutatividades successfully', async () => {
      const responseGUTAtividades = { ...mockGUTAtividades, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGUTAtividades,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutatividadesApi.addAndUpdate(mockGUTAtividades);
    ...
    it('should update existing gutatividades successfully', async () => {
      const existingGUTAtividades = { ...mockGUTAtividades, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGUTAtividades,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutatividadesApi.addAndUpdate(existingGUTAtividades);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gutatividadesApi.addAndUpdate(mockGUTAtividades)).rejects.toThrow(GUTAtividadesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividades',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gutatividades successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gutatividadesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gutatividadesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividades',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gutatividadesApi.delete(1)).rejects.toThrow(GUTAtividadesApiError);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await gutatividadesApi.delete(1);
  ...
  describe('GUTAtividadesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GUTAtividadesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GUTAtividadesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GUTAtividadesApiError('Test message', 500);
