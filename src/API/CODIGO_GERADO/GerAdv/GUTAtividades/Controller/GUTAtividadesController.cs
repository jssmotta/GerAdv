...
public partial class GUTAtividadesController(IGUTAtividadesService gutatividadesService) : ControllerBase
{
    ...
    {
        _logger.Info("GUTAtividades: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _gutatividadesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("GUTAtividades: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _gutatividadesService.UpdateColumns(parameters, uri);
        if (!result)
        {
