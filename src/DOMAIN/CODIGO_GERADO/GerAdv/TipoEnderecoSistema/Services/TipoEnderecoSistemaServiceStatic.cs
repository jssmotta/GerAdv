...
public partial class TipoEnderecoSistemaService
{
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterTipoEnderecoSistema filtro)
    {
        ...
        cWhere.Append(filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoEnderecoSistemaDicInfo.PTabelaNome}].[{DBTipoEnderecoSistemaDicInfo.Nome}]  {DevourerConsts.MsiCollate} like @{nameof(DBTipoEnderecoSistemaDicInfo.Nome)}");
        cWhere.Append(filtro.GUID.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoEnderecoSistemaDicInfo.PTabelaNome}].[{DBTipoEnderecoSistemaDicInfo.GUID}]  {DevourerConsts.MsiCollate} like @{nameof(DBTipoEnderecoSistemaDicInfo.GUID)}");
        if (!filtro.Codigo_filtro.IsEmpty() && filtro.Codigo_filtro_end.IsEmpty())
        {
            cWhere.Append(filtro.Codigo_filtro <= 0 ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"[{DBTipoEnderecoSistemaDicInfo.PTabelaNome}].[{DBTipoEnderecoSistemaDicInfo.CampoCodigo}] >= @{nameof(DBTipoEnderecoSistemaDicInfo.CampoCodigo)}");
        }
        else
        {
            cWhere.Append((filtro.Codigo_filtro <= 0 && filtro.Codigo_filtro_end <= 0) ? string.Empty : (!(filtro.Codigo_filtro <= 0) && !(filtro.Codigo_filtro_end <= 0)) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoEnderecoSistemaDicInfo.CampoCodigo} BETWEEN @{nameof(DBTipoEnderecoSistemaDicInfo.CampoCodigo)} AND @{nameof(DBTipoEnderecoSistemaDicInfo.CampoCodigo)}_end" : !(filtro.Codigo_filtro <= 0) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoEnderecoSistemaDicInfo.CampoCodigo} = @{nameof(DBTipoEnderecoSistemaDicInfo.CampoCodigo)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBTipoEnderecoSistemaDicInfo.CampoCodigo} <= @{nameof(DBTipoEnderecoSistemaDicInfo.CampoCodigo)}_end");
        }
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter> parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarN(max, uri, where, parameters, DBTipoEnderecoSistemaDicInfo.CampoNome);
