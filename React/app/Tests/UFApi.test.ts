...
describe('UFApi', () => {
  ...
  beforeEach(() => {
    ...
    ufApi = new UFApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(ufApi).toBeInstanceOf(UFApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new UFApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(UFApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all uf items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ufApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(ufApi.getAll()).rejects.toThrow(UFApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-UF_last_getAll_${200}`)));
    });
    ...
    it('should throw UFApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    ...
    it('should fetch uf by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await ufApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockUF));
      ...
      expect(result.data).toEqual(mockUF);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UF',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter uf items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await ufApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new uf successfully', async () => {
      ...
      const result = await ufApi.addAndUpdate(mockUF);
    ...
    it('should update existing uf successfully', async () => {
      const existingUF = { ...mockUF, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingUF,
        status: 200,
        statusText: 'OK',
      ...
      const result = await ufApi.addAndUpdate(existingUF);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(ufApi.addAndUpdate(mockUF)).rejects.toThrow(UFApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UF',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete uf successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await ufApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(ufApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'UF',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(ufApi.delete(1)).rejects.toThrow(UFApiError);
    });
  });
  ...
  describe('UFApiError', () => {
    it('should create error with correct properties', () => {
      const error = new UFApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('UFApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new UFApiError('Test message', 500);
