...
export const StatusInstanciaGridDesktopComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('statusinstancia-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('statusinstancia-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
