...
import { GUTAtividades } from '../../Models/GUTAtividades';
...
export class GUTAtividadesApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'GUTAtividadesApiError';
    this.status = status;
    this.code = code;
...
export class GUTAtividadesApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/GUTAtividades`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividades_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTAtividades');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividades_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar G U T Atividades com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterGUTAtividades): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividades_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de GUTAtividades');
        }
    }

            public async filter(filtro: FilterGUTAtividades): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTAtividades_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTAtividades');
                }
            }


    public async validation(regGUTAtividades: IGUTAtividades): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTAtividades as GUTAtividades, this.getHeaders());
    ...
    public async addAndUpdate(regGUTAtividades: IGUTAtividades): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regGUTAtividades as GUTAtividades, this.getHeaders());
            var register = result.data as IGUTAtividades;        
            const action = regGUTAtividades.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Atividades');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o GUTAtividades. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'GUTAtividades',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir G U T Atividades com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTAtividades) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<GUTAtividades[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de gutatividades:', error);
            this.notificationService.notify({
              entity: 'GUTAtividades',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gutatividades'
            });
          }
        });
    }
