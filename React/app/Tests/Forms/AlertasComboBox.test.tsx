// app/TestsForms/AlertasComboBox.test.tsx - Vers�o Corrigida
...
import { AlertasEmpty, AlertasTestEmpty } from '@/app/GerAdv_TS/Models/Alertas';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Alertas/Hooks/hookAlertas', () => ({
  useAlertasComboBox: jest.fn(() => ({
    options: [
    { ...AlertasEmpty(), id: 1, nome: 'Alertas 1' },
    { ...AlertasEmpty(), id: 2, nome: 'Alertas 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Alertas/Apis/ApiAlertas', () => ({
  AlertasApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Alertas' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Alertas' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Alertas/Services/Alertas.service', () => ({
  AlertasService: jest.fn().mockImplementation(() => ({
    fetchAlertasById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Alertas' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Alertas/Crud/Grids/AlertasWindow', () => {
  return function MockAlertasWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='alertas-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Alertas' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...AlertasTestEmpty(), id: 1, nome: 'Alertas 1' },
  { ...AlertasTestEmpty(), id: 2, nome: 'Alertas 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: AlertasEmpty(), 
};
describe('AlertasComboBox', () => {
  ...
  const { useAlertasComboBox } = require('@/app/GerAdv_TS/Alertas/Hooks/hookAlertas');
  (useAlertasComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<AlertasComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Alertas')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...AlertasTestEmpty(), id: 1, nome: 'Alertas 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useAlertasComboBox } = require('@/app/GerAdv_TS/Alertas/Hooks/hookAlertas');
    useAlertasComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens AlertasWindow on action button click (add)', async () => {
    render(<AlertasComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alertas-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('alertas-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<AlertasComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles AlertasWindow onClose and onSuccess', () => {
    render(<AlertasComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<AlertasComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
