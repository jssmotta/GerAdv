...
LEFT JOIN {"TipoEMail".dbo(oCnn)} (NOLOCK) ON tmlCodigo=smaTipoEMail
        ...
        if (ds != null)
            ...
            {
                var dbRec = new DBSMSAlice(item);
                if (dbRec.ID.IsEmptyIDNumber())
                {
                    continue;
                }

                var smsalice = reader.ReadAll(dbRec, item);
                if (smsalice != null)
                {
                    lista.Add(smsalice);
                }
    ...
    public async Task<IEnumerable<SMSAliceResponseAll>> Filter(Filters.FilterSMSAlice filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        using var oCnn = Configuracoes.GetConnectionByUri(uri);
        if (oCnn == null)
        ...
        var cacheKey = $"{uri}-SMSAlice-Filter-{where.GetHashCode()}{parameters.GetHashCode()}{keyCache}";
        var entryOptions = new HybridCacheEntryOptions
        {
    ...
    private async Task<SMSAliceResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await Task.Run(() => reader.Read(id, oCnn));
    ...
    public async Task<SMSAliceResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        return await Task.Run(async () =>
        {
            ...
            try
            {
                if (smsalice != null)
                {
                    writer.Delete(smsalice, UserTools.GetAuthenticatedUserId(_httpContextAccessor), oCnn);
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string where, List<SqlParameter> parameters, CancellationToken token)
    {
        return await Task.Run(() =>
        {
            ...
            foreach (var item in reader.ListarN(max, uri, where, parameters, DBSMSAliceDicInfo.CampoNome))
            {
    ...
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterSMSAlice filtro)
    {
        ...
        {
            parameters.Add(new($"@{nameof(DBSMSAliceDicInfo.Nome)}", filtro.Nome));
        }
        ...
        {
            parameters.Add(new($"@{nameof(DBSMSAliceDicInfo.GUID)}", filtro.GUID));
        }

        var cWhere = filtro.Operador == int.MinValue ? string.Empty : $"{DBSMSAliceDicInfo.Operador} = @{nameof(DBSMSAliceDicInfo.Operador)}";
        cWhere += filtro.Nome.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.Operator) + $"{DBSMSAliceDicInfo.Nome} = @{nameof(DBSMSAliceDicInfo.Nome)}";
        cWhere += filtro.TipoEMail == int.MinValue ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.Operator) + $"{DBSMSAliceDicInfo.TipoEMail} = @{nameof(DBSMSAliceDicInfo.TipoEMail)}";
        cWhere += filtro.GUID.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.Operator) + $"{DBSMSAliceDicInfo.GUID} = @{nameof(DBSMSAliceDicInfo.GUID)}";
