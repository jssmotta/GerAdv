...
public static class TipoOrigemSucumbenciaEndpoints
{
    ...
    private static void MapTipoOrigemSucumbenciaRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, ITipoOrigemSucumbenciaValidation validation, ITipoOrigemSucumbenciaWriter writer, ITipoOrigemSucumbenciaService service) =>
        {
            logger.LogInfo("TipoOrigemSucumbencia", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("TipoOrigemSucumbencia", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("TipoOrigemSucumbencia_GetColumns").WithDisplayName("Get TipoOrigemSucumbencia Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, ITipoOrigemSucumbenciaValidation validation, ITipoOrigemSucumbenciaWriter writer, ITipoOrigemSucumbenciaService service) =>
        {
            logger.LogInfo("TipoOrigemSucumbencia", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("TipoOrigemSucumbencia", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("TipoOrigemSucumbencia_UpdateColumns").WithDisplayName("Update TipoOrigemSucumbencia Columns");
