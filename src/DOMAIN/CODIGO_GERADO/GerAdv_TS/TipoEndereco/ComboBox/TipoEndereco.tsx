...
const TipoEnderecoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ITipoEndereco | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ITipoEndereco | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ITipoEndereco);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <TipoEnderecoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedTipoEndereco={editar ?? addValue ?? TipoEnderecoEmpty()} /> 
            )}
