...
public partial class EventoPrazoAgenda
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(epaDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.EventoPrazoAgenda WITH (NOLOCK, INDEX = idx_EventoPrazoAgenda_AuditorDtAtu)
WHERE epaCodigo = @id
...
FROM {oCnn.UseDbo}.EventoPrazoAgenda WITH (NOLOCK)
