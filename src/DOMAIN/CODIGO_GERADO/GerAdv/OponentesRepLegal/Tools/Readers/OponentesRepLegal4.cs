...
public partial class OponentesRepLegalReader(IFOponentesRepLegalFactory oponentesreplegalFactory, IConnectionService connection) : IOponentesRepLegalReader
{
    ...
    public async Task<IEnumerable<OponentesRepLegalResponseAll>> ListarAsync(int max, string uri, string cWhere, List<SqlParameter>? parameters, string order, CancellationToken cancellationToken)
    {
        return await ListarTabelaAsync(BuildSqlQuery(DBOponentesRepLegal.CamposSqlX, cWhere, order, max), parameters, uri, caching: false, max: max, cancellationToken: cancellationToken);
    }

    private async Task<IEnumerable<OponentesRepLegalResponseAll>> ListarTabelaAsync(string sql, List<SqlParameter>? parameters, string uri, bool caching = false, int max = 200, CancellationToken cancellationToken = default)
    {
