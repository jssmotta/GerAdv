...
export const useViaRecebimentoForm = (
  ...
  const loadViaRecebimento = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Via Recebimento";
...
export const useViaRecebimentoList = (
  dataService: IViaRecebimentoService,
  currentFilter?: FilterViaRecebimento | undefined | null,
) => {
  const [data, setData] = useState<IViaRecebimento[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterViaRecebimento | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterViaRecebimento | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar viarecebimento";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsViaRecebimento() {
  ...
  function validate(data: IViaRecebimento): {
    isValid: boolean;
    message: string;
  } {
