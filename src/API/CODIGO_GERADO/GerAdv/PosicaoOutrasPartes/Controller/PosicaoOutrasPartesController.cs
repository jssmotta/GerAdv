...
public partial class PosicaoOutrasPartesController(IPosicaoOutrasPartesService posicaooutraspartesService) : ControllerBase
{
    ...
    {
        _logger.Info("PosicaoOutrasPartes: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _posicaooutraspartesService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("PosicaoOutrasPartes: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _posicaooutraspartesService.UpdateColumns(parameters, uri);
        if (!result)
        {
