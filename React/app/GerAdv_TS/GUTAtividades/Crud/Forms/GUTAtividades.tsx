...
export const GUTAtividadesForm: React.FC<GUTAtividadesFormProps> = ({
  ...
  const dadoApi = new GUTAtividadesApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const [_nomeGUTPeriodicidade, setNomeGUTPeriodicidade] = useState("");
  const gutperiodicidadeApi = new GUTPeriodicidadeApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
      ...
      try {
        const syntheticEvent = {
          ...
          target: document.getElementById(
            `GUTAtividadesForm-${gutatividadesData.id}`,
          ),
  ...
  useEffect(() => {
    ...
    if (el) {
      el.textContent =
        gutatividadesData?.id == 0
          ? "Editar GUTAtividades"
          : "Adicionar G U T Atividades";
    }
  ...
  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-GUTAtividades"
            : "form-container5 form-container-GUTAtividades"
        }
      >
        <form
          className="formInputCadInc"
          id={`GUTAtividadesForm-${gutatividadesData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
              ...
              onClose={onClose}
              formId={`GUTAtividadesForm-${gutatividadesData.id}`}
          ...
          <div className="grid-container">
            ...
            <InputCheckbox
              ...
              label="Concluido"
              name="concluido"
              checked={gutatividadesData.concluido}
              onChange={onChange}
            />
        ...
        <FooterIncDocker>
          <CopyButton formId={`GUTAtividadesForm-${gutatividadesData.id}`} />

          <PrintButton formId={`GUTAtividadesForm-${gutatividadesData.id}`} />

          <AuditorButton
            tableName="GUTAtividades"
            coolName="G U T Atividades"
            id={gutatividadesData.id}
          />

          <DeleteButton
            page={"/pages/gutatividades"}
            id={gutatividadesData.id}
            dadoApi={dadoApi}
          />
        </FooterIncDocker>
