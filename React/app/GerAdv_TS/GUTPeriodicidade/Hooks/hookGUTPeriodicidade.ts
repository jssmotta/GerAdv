...
export const useGUTPeriodicidadeForm = (
  ...
  const loadGUTPeriodicidade = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar G U T Periodicidade";
...
export const useGUTPeriodicidadeList = (
  dataService: IGUTPeriodicidadeService,
  currentFilter?: FilterGUTPeriodicidade | undefined | null,
) => {
  const [data, setData] = useState<IGUTPeriodicidade[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGUTPeriodicidade | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterGUTPeriodicidade | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar gutperiodicidade";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsGUTPeriodicidade() {
  ...
  function validate(data: IGUTPeriodicidade): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.nome.length > 20) {
        return {
          isValid: false,
          message: "O campo Nome não pode ter mais de 20 caracteres.",
        };
      }
