...
public partial class PontoVirtualController(IPontoVirtualService pontovirtualService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.PontoVirtual regPontoVirtual, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "PontoVirtual: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "PontoVirtual: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.PontoVirtual regPontoVirtual, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _pontovirtualService.Validation(regPontoVirtual, uri);
            if (result == null)
            {
                _logger.Warn("PontoVirtual: Validation failed to add or update PontoVirtual, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "PontoVirtual: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}