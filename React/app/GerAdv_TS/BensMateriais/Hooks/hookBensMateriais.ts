...
export const useBensMateriaisForm = (
  ...
  const loadBensMateriais = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialBensMateriais);
      return;
    }
  ...
  }, [dataService, initialBensMateriais]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadBensMateriais, resetForm]);
  return returnValue;
};


export const useBensMateriaisNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('BensMateriais', (entity) => {
      try {
...
export const useBensMateriaisList = (dataService: IBensMateriaisService) => {
  const [data, setData] = useState<IBensMateriais[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
...
if (data.nfnro.length > 255) { 
                                             return { isValid: false, message: 'O campo NFNRO não pode ter mais de 255 caracteres.' };
                                         } 
if (data.nroserieproduto.length > 100) { 
                                             return { isValid: false, message: 'O campo NroSerieProduto não pode ter mais de 100 caracteres.' };
                                         } 
if (data.comprador.length > 100) { 
                                             return { isValid: false, message: 'O campo Comprador não pode ter mais de 100 caracteres.' };
                                         } 
...
if (data.nomevendedor.length > 255) { 
                                             return { isValid: false, message: 'O campo NomeVendedor não pode ter mais de 255 caracteres.' };
                                         } 
