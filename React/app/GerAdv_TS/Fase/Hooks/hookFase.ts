...
export const useFaseForm = (initialFase: IFase, dataService: IFaseService) => {
  ...
  const loadFase = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Fase";
...
export const useFaseList = (
  dataService: IFaseService,
  currentFilter?: FilterFase | undefined | null,
) => {
  const [data, setData] = useState<IFase[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterFase | undefined | null>(currentFilter);
  ...
  const fetchData = useCallback(
    async (filtro?: FilterFase | undefined | null) => {
      ...
      try {
        const result = await dataService.getAll(filtro ?? ({} as FilterFase));
        setData(result);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar fase";
