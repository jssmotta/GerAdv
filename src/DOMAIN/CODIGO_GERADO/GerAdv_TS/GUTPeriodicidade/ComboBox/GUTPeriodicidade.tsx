...
const GUTPeriodicidadeComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IGUTPeriodicidade | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IGUTPeriodicidade | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IGUTPeriodicidade);
    ...
    const handleAddClick = () => {
        ...
        const add = GUTPeriodicidadeEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <GUTPeriodicidadeWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedGUTPeriodicidade={editar ?? addValue ?? GUTPeriodicidadeEmpty()} /> 
            )}
