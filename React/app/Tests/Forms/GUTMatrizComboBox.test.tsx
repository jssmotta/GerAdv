// app/TestsForms/GUTMatrizComboBox.test.tsx - Vers�o Corrigida
...
import { GUTMatrizEmpty, GUTMatrizTestEmpty } from '@/app/GerAdv_TS/Models/GUTMatriz';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/GUTMatriz/Hooks/hookGUTMatriz', () => ({
  useGUTMatrizComboBox: jest.fn(() => ({
    options: [
    { ...GUTMatrizEmpty(), id: 1, descricao: 'G U T Matriz 1' },
    { ...GUTMatrizEmpty(), id: 2, descricao: 'G U T Matriz 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/GUTMatriz/Apis/ApiGUTMatriz', () => ({
  GUTMatrizApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GUTMatriz' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GUTMatriz' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/GUTMatriz/Services/GUTMatriz.service', () => ({
  GUTMatrizService: jest.fn().mockImplementation(() => ({
    fetchGUTMatrizById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock GUTMatriz' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/GUTMatriz/Crud/Grids/GUTMatrizWindow', () => {
  return function MockGUTMatrizWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='gutmatriz-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'G U T Matriz' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...GUTMatrizTestEmpty(), id: 1, descricao: 'G U T Matriz 1' },
  { ...GUTMatrizTestEmpty(), id: 2, descricao: 'G U T Matriz 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: GUTMatrizEmpty(), 
};
describe('GUTMatrizComboBox', () => {
  ...
  const { useGUTMatrizComboBox } = require('@/app/GerAdv_TS/GUTMatriz/Hooks/hookGUTMatriz');
  (useGUTMatrizComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<GUTMatrizComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('G U T Matriz')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select G U T Matriz')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...GUTMatrizTestEmpty(), id: 1, descricao: 'G U T Matriz 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useGUTMatrizComboBox } = require('@/app/GerAdv_TS/GUTMatriz/Hooks/hookGUTMatriz');
    useGUTMatrizComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens GUTMatrizWindow on action button click (add)', async () => {
    render(<GUTMatrizComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('gutmatriz-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('gutmatriz-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<GUTMatrizComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles GUTMatrizWindow onClose and onSuccess', () => {
    render(<GUTMatrizComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<GUTMatrizComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
