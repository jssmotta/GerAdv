...
#region ListarDados_StatusTarefas
    public static async IAsyncEnumerable<DBStatusTarefas> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBStatusTarefas(dbRec);
        }
    }

    public static IEnumerable<DBStatusTarefas> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
