...
#region ListarDados_ProObservacoes
    public static async IAsyncEnumerable<DBProObservacoes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProObservacoes(dbRec);
        }
    }

    public static IEnumerable<DBProObservacoes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
