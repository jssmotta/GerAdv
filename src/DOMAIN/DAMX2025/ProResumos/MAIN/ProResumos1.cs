...
public partial class DBProResumos : VAuditor, ICadastros, IAuditor
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_ProResumos)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBProResumos();
...
#region TableDefinition_ProResumos
    [XmlIgnore]
    public string TabelaNome => "ProResumos";
...
#endregion
    public DBProResumos(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    // REF. 250325
    public DBProResumos(MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
...
#region GravarDados_ProResumos
...
#endif
    {
        ...
        if (!isInsert)
            if (!(pFldFProcesso || pFldFData || pFldFResumo || pFldFTipoResumo || pFldFBold))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
