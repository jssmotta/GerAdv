...
public static class FilterViaRecebimentoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ViaRecebimento em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterViaRecebimento:

- O filter irá filtrar os ViaRecebimento conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterViaRecebimento:(Via Recebimento)
- nome: _nome ou descrição para Via Recebimento;
...
- Se ouvir "todos os ViaRecebimento" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterViaRecebimento? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ViaRecebimento length: {result.Length}");
#endif
