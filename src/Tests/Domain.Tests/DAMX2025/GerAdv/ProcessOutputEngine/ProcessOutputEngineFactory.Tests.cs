...
public class FProcessOutputEngineFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProcessOutputEngineFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProcessOutputEngineFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProcessOutputEngineFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "poeCodigo")
        var dt = new DataTable();
        dt.Columns.Add("poeCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["poeCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProcessOutputEngine>();
    }
    ...
    {
        // Arrange
        var factory = new FProcessOutputEngineFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("poeCodigo", typeof(int));
        mockDataTable.Columns.Add("poeNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["poeCodigo"] = testId;
        row["poeNome"] = "Test ProcessOutputEngine";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["poeCodigo"] = 123;
        row["poeNome"] = "Test ProcessOutputEngine Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProcessOutputEngine properties
        }
    }
