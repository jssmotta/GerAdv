...
public partial class DBTipoEMail : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_TipoEMail)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBTipoEMail();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_TipoEMail
    [XmlIgnore]
    public string TabelaNome => "TipoEMail";
...
#endregion
    public DBTipoEMail(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBTipoEMail(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBTipoEMail(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_TipoEMail
...
#endif
    {
        ...
        {
            throw new Exception("Campo 'tmlNome' está vazio!");
        }
        ...
        if (pFldFNome)
            clsW.Fields(DBTipoEMailDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (insertId != 0)
            return GravaNewId();
