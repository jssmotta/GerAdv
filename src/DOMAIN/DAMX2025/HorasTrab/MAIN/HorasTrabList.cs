...
#region ListarDados_HorasTrab
    public static async IAsyncEnumerable<DBHorasTrab> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBHorasTrab(dbRec);
        }
    }

    public static IEnumerable<DBHorasTrab> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
