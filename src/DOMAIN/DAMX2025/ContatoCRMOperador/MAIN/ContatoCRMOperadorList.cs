...
#region ListarDados_ContatoCRMOperador
    public static async IAsyncEnumerable<DBContatoCRMOperador> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBContatoCRMOperador(dbRec);
        }
    }

    public static IEnumerable<DBContatoCRMOperador> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
