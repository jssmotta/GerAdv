...
import { ReuniaoPessoasApiError } from '../GerAdv_TS/ReuniaoPessoas/Apis/ApiReuniaoPessoas';
import { ReuniaoPessoasApi } from '@/app/GerAdv_TS/ReuniaoPessoas/Apis/ApiReuniaoPessoas';
import { FilterReuniaoPessoas } from '../GerAdv_TS/ReuniaoPessoas/Filters/ReuniaoPessoas';
...
describe('ReuniaoPessoasApi', () => {
  let reuniaopessoasApi: ReuniaoPessoasApi;
  ...
  beforeEach(() => {
    ...
    reuniaopessoasApi = new ReuniaoPessoasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(reuniaopessoasApi).toBeInstanceOf(ReuniaoPessoasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ReuniaoPessoasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ReuniaoPessoasApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all reuniaopessoas items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await reuniaopessoasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(reuniaopessoasApi.getAll()).rejects.toThrow(ReuniaoPessoasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await reuniaopessoasApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ReuniaoPessoas_last_getAll_${200}`)));
    });
    ...
    it('should throw ReuniaoPessoasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockReuniaoPessoas = ReuniaoPessoasTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockReuniaoPessoas,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch reuniaopessoas by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await reuniaopessoasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockReuniaoPessoas));
      ...
      expect(result.data).toEqual(mockReuniaoPessoas);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ReuniaoPessoas',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterReuniaoPessoas = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter reuniaopessoas items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await reuniaopessoasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockReuniaoPessoas: IReuniaoPessoas = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new reuniaopessoas successfully', async () => {
      const responseReuniaoPessoas = { ...mockReuniaoPessoas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseReuniaoPessoas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await reuniaopessoasApi.addAndUpdate(mockReuniaoPessoas);
    ...
    it('should update existing reuniaopessoas successfully', async () => {
      const existingReuniaoPessoas = { ...mockReuniaoPessoas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingReuniaoPessoas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await reuniaopessoasApi.addAndUpdate(existingReuniaoPessoas);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(reuniaopessoasApi.addAndUpdate(mockReuniaoPessoas)).rejects.toThrow(ReuniaoPessoasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ReuniaoPessoas',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete reuniaopessoas successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await reuniaopessoasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(reuniaopessoasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ReuniaoPessoas',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(reuniaopessoasApi.delete(1)).rejects.toThrow(ReuniaoPessoasApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterReuniaoPessoas = { funcionario: 1 };
      
      const result = reuniaopessoasApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await reuniaopessoasApi.delete(1);
  ...
  describe('ReuniaoPessoasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ReuniaoPessoasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ReuniaoPessoasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ReuniaoPessoasApiError('Test message', 500);
