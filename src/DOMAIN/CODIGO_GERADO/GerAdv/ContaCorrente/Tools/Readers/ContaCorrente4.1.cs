...
public partial class ContaCorrenteReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBContaCorrenteDicInfo.CampoNome}";
...
[{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBContaCorrenteDicInfo.PTabelaNome}].[{DBContaCorrenteDicInfo.Cliente}]
...
SELECT TOP (1) 
	FORMAT(ctoDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(ctoDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    ctoVisto as Visto
FROM {oCnn.UseDbo}.ContaCorrente dbq WITH (INDEX = idx_ContaCorrente_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.ctoQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.ctoQuemAtu
WHERE [ctoCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBContaCorrente.PTabelaNome,
...
FROM {oCnn.UseDbo}.ContaCorrente WITH (INDEX = idx_{DBContaCorrenteDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [ctoCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBContaCorrenteDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(ctoDtAtu, ctoDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBContaCorrenteDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(ctoDtAtu, ctoDtCad) DESC;", oCnn.InnerConnection);
