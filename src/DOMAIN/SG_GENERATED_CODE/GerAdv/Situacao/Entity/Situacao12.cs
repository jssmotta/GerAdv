...
public partial class FSituacao : MenphisSI.SG.GerAdv.DBSituacao, IDBSituacao
{
    // DI-compatible default constructor
    public FSituacao() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FSituacao CreateFromReader(SqlDataReader? dbRec)
    {
        return new FSituacao(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FSituacao CreateFromDataRow(DataRow? dbRec)
    {
        return new FSituacao(dbRec);
    }

    private FSituacao(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FSituacao(DataRow? dbRec) : base(dbRec)
    {
    }

    private FSituacao(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FSituacao CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FSituacao(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FSituacao> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fSituacao = new FSituacao();
        await fSituacao.CarregarAsync(id, oCnn);
        return fSituacao;
    }
