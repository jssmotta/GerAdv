...
export const useEnquadramentoEmpresaForm = (
  ...
  const loadEnquadramentoEmpresa = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Enquadramento Empresa";
...
export const useEnquadramentoEmpresaList = (
  dataService: IEnquadramentoEmpresaService,
  currentFilter?: FilterEnquadramentoEmpresa | undefined | null,
) => {
  const [data, setData] = useState<IEnquadramentoEmpresa[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    async (filtro?: FilterEnquadramentoEmpresa | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar enquadramentoempresa";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsEnquadramentoEmpresa() {
  ...
  function validate(data: IEnquadramentoEmpresa): {
    isValid: boolean;
    message: string;
  } {
