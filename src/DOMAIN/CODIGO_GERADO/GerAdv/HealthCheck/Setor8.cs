...
public class SetorHealthCheck(IOptions<AppSettings> appSettings, SetorService setorService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly SetorService _setorService = setorService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        if (DBSetorDicInfo.CampoCodigo.NotIsEmpty())
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) MAX(setCodigo) FROM {"Setor".dbo(connection)} (NOLOCK);";
                        ...
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) setDescricao,setGUID FROM {"Setor".dbo(connection)} (NOLOCK);";
                            tableCheck.CommandTimeout = 5;
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT Setor successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT Setor failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _setorService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-Setor";
                    ...
                    static async Task<IEnumerable<Models.Setor>> GetData(string uri, CancellationToken token)
                    {
            ...
            return isHealthy ? HealthCheckResult.Healthy("Setor is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
