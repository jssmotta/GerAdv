...
public partial class DBAlertasEnviados
{
    ...
    /// </summary>
    public static async Task<DBAlertasEnviados> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAlertasEnviados();
...
#if (!NOTSTORED_AlertasEnviados)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFOperador || pFldFAlerta || pFldFDataAlertado || pFldFVisualizado;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFOperador)
            updateTool.Fields(DBAlertasEnviadosDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFAlerta)
