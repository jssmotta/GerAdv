...
export class ParteClienteOutrasApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'ParteClienteOutrasApiError';
    this.status = status;
    this.code = code;
...
export class ParteClienteOutrasApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParteClienteOutras_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os ParteClienteOutras');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParteClienteOutras_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Parte Cliente Outras com ID ${id}`);
        }
    }

            public async filter(filtro: FilterParteClienteOutras): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ParteClienteOutras_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar ParteClienteOutras');
                }
            }


    public async validation(regParteClienteOutras: IParteClienteOutras): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regParteClienteOutras as ParteClienteOutras, this.getHeaders());
    ...
    public async addAndUpdate(regParteClienteOutras: IParteClienteOutras): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Parte Cliente Outras');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Parte Cliente Outras com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterParteClienteOutras) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ParteClienteOutras[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de parteclienteoutras:', error);
            this.notificationService.notify({
              entity: 'ParteClienteOutras',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos parteclienteoutras'
            });
          }
        });
    }
