...
import ServicosWindow from '../Crud/Grids/ServicosWindow';
...
import { useServicosComboBox } from '../Hooks/hookServicos';
const ServicosComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useServicosComboBox(servicosService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IServicos | null>(null);
const [addRecord, setAddRecord] = useState<IServicos | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await servicosService.fetchServicosById(id);
...
const handleSuccess = (newServicos?: IServicos) => {
  if (newServicos) {
    setValue({ id: newServicos.id, nome: newServicos.descricao });
    loadRecordForEdit(newServicos.id);
    handleValueChange(newServicos);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .servicos-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .servicos-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .servicos-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedServicos={editRecord || addRecord || ServicosEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'servicos-dropdown-popup-msi'
          }}
          />
...
export default ServicosComboBox;