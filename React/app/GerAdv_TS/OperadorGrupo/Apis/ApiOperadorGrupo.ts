...
export class OperadorGrupoApi {
 ...
 public async filterPreload(max: number, filtro: FilterOperadorGrupo): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupo-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterOperadorGrupo): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupo-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterOperadorGrupo) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<OperadorGrupo[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de OperadorGrupo');
            this.notificationService.notify({
              entity: 'OperadorGrupo',
              id: 0,
              action: NotifySystemActions.ERROR,
