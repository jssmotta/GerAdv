...
import { TipoStatusBiuApiError } from '../GerAdv_TS/TipoStatusBiu/Apis/ApiTipoStatusBiu';
import { TipoStatusBiuApi } from '@/app/GerAdv_TS/TipoStatusBiu/Apis/ApiTipoStatusBiu';
import { FilterTipoStatusBiu } from '../GerAdv_TS/TipoStatusBiu/Filters/TipoStatusBiu';
...
describe('TipoStatusBiuApi', () => {
  let tipostatusbiuApi: TipoStatusBiuApi;
  ...
  beforeEach(() => {
    ...
    tipostatusbiuApi = new TipoStatusBiuApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipostatusbiuApi).toBeInstanceOf(TipoStatusBiuApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoStatusBiuApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoStatusBiuApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tipostatusbiu items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipostatusbiuApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipostatusbiuApi.getAll()).rejects.toThrow(TipoStatusBiuApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipostatusbiuApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoStatusBiu_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoStatusBiuApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoStatusBiu = TipoStatusBiuTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoStatusBiu,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipostatusbiu by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipostatusbiuApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoStatusBiu));
      ...
      expect(result.data).toEqual(mockTipoStatusBiu);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoStatusBiu',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoStatusBiu = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipostatusbiu items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipostatusbiuApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoStatusBiu: ITipoStatusBiu = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipostatusbiu successfully', async () => {
      const responseTipoStatusBiu = { ...mockTipoStatusBiu, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoStatusBiu,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipostatusbiuApi.addAndUpdate(mockTipoStatusBiu);
    ...
    it('should update existing tipostatusbiu successfully', async () => {
      const existingTipoStatusBiu = { ...mockTipoStatusBiu, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoStatusBiu,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipostatusbiuApi.addAndUpdate(existingTipoStatusBiu);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipostatusbiuApi.addAndUpdate(mockTipoStatusBiu)).rejects.toThrow(TipoStatusBiuApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoStatusBiu',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipostatusbiu successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipostatusbiuApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipostatusbiuApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoStatusBiu',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipostatusbiuApi.delete(1)).rejects.toThrow(TipoStatusBiuApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoStatusBiu = { funcionario: 1 };
      
      const result = tipostatusbiuApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipostatusbiuApi.delete(1);
  ...
  describe('TipoStatusBiuApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoStatusBiuApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoStatusBiuApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoStatusBiuApiError('Test message', 500);
