...
public partial class DBRito
{
    ...
    /// </summary>
    public static async Task<DBRito> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBRito();
...
#if (!NOTSTORED_Rito)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFDescricao || pFldFTop || pFldFBold || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFDescricao)
            updateTool.Fields(DBRitoDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFTop || updateTool.Insert)
            updateTool.Fields(DBRitoDicInfo.Top, FTop, EGenericTypeFields.FBoolean);
        if (pFldFBold || updateTool.Insert)
            updateTool.Fields(DBRitoDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            updateTool.Fields(DBRitoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBRitoDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBRitoDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBRitoDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBRitoDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBRitoDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBRitoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
