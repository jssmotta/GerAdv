...
import { ParteClienteApiError } from '../GerAdv_TS/ParteCliente/Apis/ApiParteCliente';
import { ParteClienteApi } from '@/app/GerAdv_TS/ParteCliente/Apis/ApiParteCliente';
import { FilterParteCliente } from '../GerAdv_TS/ParteCliente/Filters/ParteCliente';
...
describe('ParteClienteApi', () => {
  let parteclienteApi: ParteClienteApi;
  ...
  beforeEach(() => {
    ...
    parteclienteApi = new ParteClienteApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(parteclienteApi).toBeInstanceOf(ParteClienteApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ParteClienteApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ParteClienteApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all partecliente items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteclienteApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(parteclienteApi.getAll()).rejects.toThrow(ParteClienteApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await parteclienteApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ParteCliente_last_getAll_${200}`)));
    });
    ...
    it('should throw ParteClienteApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockParteCliente = ParteClienteTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockParteCliente,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch partecliente by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await parteclienteApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockParteCliente));
      ...
      expect(result.data).toEqual(mockParteCliente);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteCliente',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterParteCliente = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter partecliente items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await parteclienteApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockParteCliente: IParteCliente = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new partecliente successfully', async () => {
      const responseParteCliente = { ...mockParteCliente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseParteCliente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteclienteApi.addAndUpdate(mockParteCliente);
    ...
    it('should update existing partecliente successfully', async () => {
      const existingParteCliente = { ...mockParteCliente, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingParteCliente,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await parteclienteApi.addAndUpdate(existingParteCliente);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(parteclienteApi.addAndUpdate(mockParteCliente)).rejects.toThrow(ParteClienteApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteCliente',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete partecliente successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await parteclienteApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(parteclienteApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ParteCliente',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(parteclienteApi.delete(1)).rejects.toThrow(ParteClienteApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterParteCliente = { funcionario: 1 };
      
      const result = parteclienteApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await parteclienteApi.delete(1);
  ...
  describe('ParteClienteApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ParteClienteApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ParteClienteApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ParteClienteApiError('Test message', 500);
