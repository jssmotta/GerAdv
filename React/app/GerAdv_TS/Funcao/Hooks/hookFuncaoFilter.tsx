...
import { FilterFuncao } from "@/app/GerAdv_TS/Funcao/Filters/Funcao";
import FilterDialogButton from "@/app/components/Cruds/FilterDialogButton";

interface UseFuncaoFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterFuncao | undefined | null,
  ) => Promise<void>;
}

interface UseFuncaoFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterFuncao;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterFuncao>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterFuncao) => Promise<void>;

  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterFuncao>,
  ) => React.ReactNode;
...
export const useFuncaoFilter = ({
  handleFetchWithFilter,
}: UseFuncaoFilterProps): UseFuncaoFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterFuncao>(
    {} as FilterFuncao,
  );
  ...
  const handleConfirmSearch = async (filter: FilterFuncao) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterFuncao);
    sessionStorage.removeItem(btoa("FuncaoFilter"));
    handleFetchWithFilter({} as FilterFuncao);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterFuncao>) => (
    <>
      <InputInput
        tabIndex={0}
        type="text"
