...
export class ContatoCRMApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        public async getListN(max?: number, filtro?: FilterContatoCRM): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ContatoCRM_last_listN_data`);
        ...
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar lista de ContatoCRM');
        }
    }
 public async filterPreload(filtro: FilterContatoCRM): Promise<AxiosResponse> {
    ...
    public useFilter(filtro: FilterContatoCRM) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ContatoCRM[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de ContatoCRM');
            this.notificationService.notify({
              entity: 'ContatoCRM',
              id: 0,
              action: NotifySystemActions.ERROR,
