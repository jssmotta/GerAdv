...
public static class FilterPaisesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Paises em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterPaises:

- O filter irá filtrar os Paises conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Paises" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterPaises? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Paises length: {result.Length}");
#endif
