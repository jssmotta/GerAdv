...
export const useEscritoriosForm = (
  ...
  const loadEscritorios = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Escritorios";
...
export const useEscritoriosList = (
  dataService: IEscritoriosService,
  currentFilter?: FilterEscritorios | undefined | null,
) => {
  const [data, setData] = useState<IEscritorios[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterEscritorios | undefined | null>(
    currentFilter,
  );
  ...
  function validate(data: IEscritorios): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.oab.length > 15) {
        return {
          isValid: false,
          message: "O campo OAB não pode ter mais de 15 caracteres.",
        };
      }
      if (data.endereco.length > 50) {
        return {
          isValid: false,
          message: "O campo Endereco não pode ter mais de 50 caracteres.",
        };
      }
      if (data.bairro.length > 30) {
        return {
          isValid: false,
          message: "O campo Bairro não pode ter mais de 30 caracteres.",
        };
      }
      ...
      if (data.site.length > 200) {
        return {
          isValid: false,
          message: "O campo Site não pode ter mais de 200 caracteres.",
        };
      }
      ...
      if (data.advresponsavel.length > 80) {
        return {
          isValid: false,
          message: "O campo AdvResponsavel não pode ter mais de 80 caracteres.",
        };
      }
      if (data.secretaria.length > 80) {
        return {
          isValid: false,
          message: "O campo Secretaria não pode ter mais de 80 caracteres.",
        };
      }
