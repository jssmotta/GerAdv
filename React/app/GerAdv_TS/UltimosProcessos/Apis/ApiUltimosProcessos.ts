...
export class UltimosProcessosApi {
 ...
 public async filterPreload(max: number, filtro: FilterUltimosProcessos): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UltimosProcessos-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterUltimosProcessos): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-UltimosProcessos-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterUltimosProcessos) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<UltimosProcessos[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de UltimosProcessos');
            this.notificationService.notify({
              entity: 'UltimosProcessos',
              id: 0,
              action: NotifySystemActions.ERROR,
