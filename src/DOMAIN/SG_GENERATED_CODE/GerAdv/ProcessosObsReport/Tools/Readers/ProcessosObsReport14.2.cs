...
public partial class ProcessosObsReportWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_ProcessosObsReport_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[ProcessosObsReport]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_ProcessosObsReport_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[ProcessosObsReport]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_ProcessosObsReport_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_ProcessosObsReport_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[ProcessosObsReport]'))
                BEGIN                    
