...
public static class FilterReuniaoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterReuniao? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Reuniao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterReuniao:

- O filter irá filtrar os Reuniao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterReuniao:(Reunião)
- cliente: número ou IDs;
- idagenda: número ou IDs;
- data: não é a data de cadastramento é a data do Reuniao;
- pauta: Pauta;
- ata: ATA;
...
- Se ouvir "todos os Reuniao" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Reuniao length: {result.Length}");
#endif
