...
export const usePaisesList = (dataService: IPaisesService, currentFilter?: FilterPaises | undefined | null) => {
  const [data, setData] = useState<IPaises[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterPaises | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterPaises | undefined | null) => {
