...
describe('ProResumosApi', () => {
  let proresumosApi: ProResumosApi;
  ...
  beforeEach(() => {
    ...
    proresumosApi = new ProResumosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(proresumosApi).toBeInstanceOf(ProResumosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProResumosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProResumosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all proresumos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proresumosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(proresumosApi.getAll()).rejects.toThrow(ProResumosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await proresumosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProResumos_last_getAll_${200}`)));
    });
    ...
    it('should throw ProResumosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProResumos = ProResumosTestEmpty();
    ...
    it('should fetch proresumos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await proresumosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProResumos));
      ...
      expect(result.data).toEqual(mockProResumos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProResumos',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter proresumos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await proresumosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new proresumos successfully', async () => {
      const responseProResumos = { ...mockProResumos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProResumos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proresumosApi.addAndUpdate(mockProResumos);
    ...
    it('should update existing proresumos successfully', async () => {
      const existingProResumos = { ...mockProResumos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProResumos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await proresumosApi.addAndUpdate(existingProResumos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(proresumosApi.addAndUpdate(mockProResumos)).rejects.toThrow(ProResumosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProResumos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete proresumos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await proresumosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(proresumosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProResumos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(proresumosApi.delete(1)).rejects.toThrow(ProResumosApiError);
    });
  });
  ...
  describe('ProResumosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProResumosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProResumosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProResumosApiError('Test message', 500);
