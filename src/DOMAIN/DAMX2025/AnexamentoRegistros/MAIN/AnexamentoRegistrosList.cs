...
#region ListarDados_AnexamentoRegistros
    public static async IAsyncEnumerable<DBAnexamentoRegistros> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAnexamentoRegistros(dbRec);
        }
    }

    public static IEnumerable<DBAnexamentoRegistros> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
