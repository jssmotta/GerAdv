// app/TestsForms/RamalComboBox.test.tsx - Vers�o Corrigida
...
import { RamalEmpty, RamalTestEmpty } from '@/app/GerAdv_TS/Models/Ramal';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Ramal/Hooks/hookRamal', () => ({
  useRamalComboBox: jest.fn(() => ({
    options: [
    { ...RamalEmpty(), id: 1, nome: 'Ramal 1' },
    { ...RamalEmpty(), id: 2, nome: 'Ramal 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Ramal/Apis/ApiRamal', () => ({
  RamalApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ramal' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ramal' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Ramal/Services/Ramal.service', () => ({
  RamalService: jest.fn().mockImplementation(() => ({
    fetchRamalById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Ramal' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Ramal/Crud/Grids/RamalWindow', () => {
  return function MockRamalWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='ramal-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Ramal' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...RamalTestEmpty(), id: 1, nome: 'Ramal 1' },
  { ...RamalTestEmpty(), id: 2, nome: 'Ramal 2' },
];
...
describe('RamalComboBox', () => {
  ...
  const { useRamalComboBox } = require('@/app/GerAdv_TS/Ramal/Hooks/hookRamal');
  (useRamalComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<RamalComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Ramal')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Ramal')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...RamalTestEmpty(), id: 1, nome: 'Ramal 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useRamalComboBox } = require('@/app/GerAdv_TS/Ramal/Hooks/hookRamal');
    useRamalComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens RamalWindow on action button click (add)', async () => {
    render(<RamalComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ramal-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('ramal-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<RamalComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles RamalWindow onClose and onSuccess', () => {
    render(<RamalComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<RamalComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
