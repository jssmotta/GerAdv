...
public partial class ProTipoBaixaController(IProTipoBaixaService protipobaixaService) : ControllerBase
{
    ...
    {
        _logger.Info("ProTipoBaixa: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _protipobaixaService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProTipoBaixa: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _protipobaixaService.UpdateColumns(parameters, uri);
        if (!result)
        {
