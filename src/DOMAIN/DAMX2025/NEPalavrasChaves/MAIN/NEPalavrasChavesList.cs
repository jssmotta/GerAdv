...
#region ListarDados_NEPalavrasChaves
    public static async IAsyncEnumerable<DBNEPalavrasChaves> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBNEPalavrasChaves(dbRec);
        }
    }

    public static IEnumerable<DBNEPalavrasChaves> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
