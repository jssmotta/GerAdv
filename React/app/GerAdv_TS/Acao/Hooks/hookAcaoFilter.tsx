...
interface UseAcaoFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterAcao | undefined | null,
  ) => Promise<void>;
}

interface UseAcaoFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterAcao;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterAcao>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterAcao) => Promise<void>;

  // Render function
  renderInputFilters: (handlers: FilterHandlers<FilterAcao>) => React.ReactNode;
...
export const useAcaoFilter = ({
  handleFetchWithFilter,
}: UseAcaoFilterProps): UseAcaoFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterAcao>(
    {} as FilterAcao,
  );
  ...
  const handleConfirmSearch = async (filter: FilterAcao) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterAcao);
    sessionStorage.removeItem(btoa("AcaoFilter"));
    handleFetchWithFilter({} as FilterAcao);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterAcao>) => (
    <>
      <JusticaComboBox
        name="justica"
        dataForm={null}
