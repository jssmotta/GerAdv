...
public partial class ParteClienteOutrasController(IParteClienteOutrasService parteclienteoutrasService) : ControllerBase
{
    ...
    {
        _logger.Info("ParteClienteOutras: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _parteclienteoutrasService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ParteClienteOutras: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _parteclienteoutrasService.UpdateColumns(parameters, uri);
        if (!result)
        {
