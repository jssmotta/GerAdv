...
#region ListarDados_TipoCompromisso
    public static async IAsyncEnumerable<DBTipoCompromisso> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBTipoCompromisso(dbRec);
        }
    }

    public static IEnumerable<DBTipoCompromisso> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
