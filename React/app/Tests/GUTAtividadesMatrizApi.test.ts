...
import { GUTAtividadesMatrizApiError } from '../GerAdv_TS/GUTAtividadesMatriz/Apis/ApiGUTAtividadesMatriz';
import { GUTAtividadesMatrizApi } from '@/app/GerAdv_TS/GUTAtividadesMatriz/Apis/ApiGUTAtividadesMatriz';
import { FilterGUTAtividadesMatriz } from '../GerAdv_TS/GUTAtividadesMatriz/Filters/GUTAtividadesMatriz';
...
describe('GUTAtividadesMatrizApi', () => {
  let gutatividadesmatrizApi: GUTAtividadesMatrizApi;
  ...
  beforeEach(() => {
    ...
    gutatividadesmatrizApi = new GUTAtividadesMatrizApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gutatividadesmatrizApi).toBeInstanceOf(GUTAtividadesMatrizApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GUTAtividadesMatrizApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GUTAtividadesMatrizApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [GUTAtividadesMatrizTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all gutatividadesmatriz items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutatividadesmatrizApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gutatividadesmatrizApi.getAll()).rejects.toThrow(GUTAtividadesMatrizApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gutatividadesmatrizApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GUTAtividadesMatriz_last_getAll_${200}`)));
    });
    ...
    it('should throw GUTAtividadesMatrizApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGUTAtividadesMatriz = GUTAtividadesMatrizTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockGUTAtividadesMatriz,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch gutatividadesmatriz by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutatividadesmatrizApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGUTAtividadesMatriz));
      ...
      expect(result.data).toEqual(mockGUTAtividadesMatriz);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividadesMatriz',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGUTAtividadesMatriz = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter gutatividadesmatriz items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gutatividadesmatrizApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockGUTAtividadesMatriz: IGUTAtividadesMatriz = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new gutatividadesmatriz successfully', async () => {
      const responseGUTAtividadesMatriz = { ...mockGUTAtividadesMatriz, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGUTAtividadesMatriz,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutatividadesmatrizApi.addAndUpdate(mockGUTAtividadesMatriz);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockGUTAtividadesMatriz,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing gutatividadesmatriz successfully', async () => {
      const existingGUTAtividadesMatriz = { ...mockGUTAtividadesMatriz, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGUTAtividadesMatriz,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutatividadesmatrizApi.addAndUpdate(existingGUTAtividadesMatriz);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gutatividadesmatrizApi.addAndUpdate(mockGUTAtividadesMatriz)).rejects.toThrow(GUTAtividadesMatrizApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividadesMatriz',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gutatividadesmatriz successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gutatividadesmatrizApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gutatividadesmatrizApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTAtividadesMatriz',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gutatividadesmatrizApi.delete(1)).rejects.toThrow(GUTAtividadesMatrizApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGUTAtividadesMatriz = { funcionario: 1 };
      
      const result = gutatividadesmatrizApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await gutatividadesmatrizApi.delete(1);
  ...
  describe('GUTAtividadesMatrizApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GUTAtividadesMatrizApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GUTAtividadesMatrizApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GUTAtividadesMatrizApiError('Test message', 500);
