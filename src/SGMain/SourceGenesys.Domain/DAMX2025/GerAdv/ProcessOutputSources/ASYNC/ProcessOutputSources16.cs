...
public partial class DBProcessOutputSources
{
    ...
    /// </summary>
    public static async Task<DBProcessOutputSources> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBProcessOutputSources();
...
#if (!NOTSTORED_ProcessOutputSources)
    /// <summary>
    ...
    {
        if (pFldFNome)
            updateTool.Fields(DBProcessOutputSourcesDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFGuid)
            updateTool.Fields(DBProcessOutputSourcesDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
