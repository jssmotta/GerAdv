...
{ 
    public static string substring(this string? text, in int startIndex, in int length = 0) =>
        text == null || text.Length ==0 ? "" : (length == 0 || length > $"{text}".Length ?
        $"{text}"[startIndex..] :
    ...
    {
        if (text is null || oldValue is null) return text ?? string.Empty;
        if (string.IsNullOrWhiteSpace(oldValue))
            return text ?? string.Empty; // or throw, or handle as you wish
        if (string.IsNullOrWhiteSpace(text)) return text ?? "";
            return text.Replace(oldValue, newValue ?? "");
    }
      

    public static int Length(this string? text) => string.IsNullOrWhiteSpace(text) ? 0 : text.trim().Length;

    public static bool IsEquals(this string? text, string? value)
        =>
            $"{text}".Equals($"{value}");

    public static string trim(this string? text) => string.IsNullOrWhiteSpace(text) ? "" : text?.Trim() ?? string.Empty;
    ...
    public static bool IsEmptyIDNumber(this int? valueID) => valueID == null || valueID <= 0;
    ...
    {
        if (text.IsEmpty()) return "#000"; // Default color
    ...
    public static bool IsEmpty(this string? text) =>
        string.IsNullOrWhiteSpace(text) || text.trim().Equals(string.Empty); // 22-12-2016
    ...
    public static bool NotIsEmpty(this string? text) =>
        !(string.IsNullOrWhiteSpace(text) || text.trim().Equals(string.Empty)); // 05-05-2017 15:00
     

    public static bool ContemUpper(this string? text, string palavra) => !string.IsNullOrWhiteSpace(text) &&
                                                                        !string.IsNullOrWhiteSpace(palavra) &&
                                                                        text.ToUpper().IndexOf(palavra.ToUpper(),
                                                                            StringComparison.Ordinal) != -1;

    public static bool NãoContemUpper(this string? text, string palavra) => string.IsNullOrWhiteSpace(text) ||
                                                                            text.toUpper().IndexOf(palavra.ToUpper(),
                                                                                StringComparison.Ordinal) == -1;

    public static bool NotEquals(this string? text, string? value) => text != value; 

 
    public static bool Contem(this string? text, string palavra) => !string.IsNullOrWhiteSpace(text) &&
    ...
    public static string LastName(this string? text)
    {
        ...
        return split != null ? split[^1] : string.Empty;
    }
 
    public static string FixAbc(this string? text) =>
        string.IsNullOrWhiteSpace(text) ? string.Empty :
        ((text.trim().Length <= 1 || !"abcdefghijklmnopqrstuvwxyz\u00FA\u00ED\u00F3\u00E1\u00E0\u00E9".Contains(text.trim()[..1])) ?
           text :
           (!$"{text}".Contains('@') ? text.trim()[..1].ToUpper() + text.trim()[1..]
               : text))!;
    ...
    public static string FixAbc(this string? text, int maxWidth)
    {
        ...
        var ret = text.Trim();
        ret =
           (ret.Length <= 1 || !"abcdefghijklmnopqrstuvwxyz\u00FA\u00ED\u00F3\u00E1\u00E0\u00E9".Contains(ret[..1])) ?
           ret.ToUpper() :
           (!ret.Contains('@') ? ret[..1].ToUpper() + ret[1..]
            : text);
