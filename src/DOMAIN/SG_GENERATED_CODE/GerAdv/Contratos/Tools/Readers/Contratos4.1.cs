...
[{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBContratosDicInfo.PTabelaNome}].[{DBContratosDicInfo.Cliente}]
...
LEFT JOIN {DBAdvogadosDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.CampoCodigo}]=[{DBContratosDicInfo.PTabelaNome}].[{DBContratosDicInfo.Advogado}]
...
SELECT TOP (1) 
	FORMAT(cttDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(cttDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    cttVisto as Visto
FROM {oCnn.UseDbo}.Contratos dbq WITH (INDEX = idx_Contratos_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.cttQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.cttQuemAtu
WHERE [cttCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBContratos.PTabelaNome,
...
FROM {oCnn.UseDbo}.Contratos WITH (INDEX = idx_{DBContratosDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [cttCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBContratosDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(cttDtAtu, cttDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBContratosDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(cttDtAtu, cttDtCad) DESC;", oCnn.InnerConnection);
