...
const GruposEmpresasComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label }) => {
    ...
    const handleAddDado = async (descricao: string) => {
        ...
        const nova = { id: 0, descricao: (UpperCaseDefault ? descricao.toUpperCase() : descricao) } as GruposEmpresas;
        const novoDado = await dadoApi.addAndUpdate(nova as IGruposEmpresas);
