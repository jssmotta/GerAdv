...
export const useNEPalavrasChavesForm = (
  ...
  const loadNEPalavrasChaves = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar N E Palavras Chaves";
...
export const useNEPalavrasChavesList = (
  dataService: INEPalavrasChavesService,
  currentFilter?: FilterNEPalavrasChaves | undefined | null,
) => {
  const [data, setData] = useState<INEPalavrasChaves[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterNEPalavrasChaves | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterNEPalavrasChaves | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar nepalavraschaves";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsNEPalavrasChaves() {
  ...
  function validate(data: INEPalavrasChaves): {
    isValid: boolean;
    message: string;
  } {
