...
export const ModelosDocumentosForm: React.FC<ModelosDocumentosFormProps> = ({
  ...
  const dadoApi = new ModelosDocumentosApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const tipomodelodocumentoApi = new TipoModeloDocumentoApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  if (getParamFromUrl("tipomodelodocumento") > 0) {
    if (
      modelosdocumentosData.id === 0 &&
      modelosdocumentosData.tipomodelodocumento == 0
    ) {
      ...
      modelosdocumentosData.tipomodelodocumento = getParamFromUrl(
        "tipomodelodocumento",
      );
    }
  }
      ...
      try {
        const syntheticEvent = {
          ...
          target: document.getElementById(
            `ModelosDocumentosForm-${modelosdocumentosData.id}`,
          ),
  ...
  useEffect(() => {
    ...
    if (el) {
      el.textContent =
        modelosdocumentosData?.id == 0
          ? "Editar ModelosDocumentos"
          : "Adicionar Modelos Documentos";
    }
  ...
  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-ModelosDocumentos"
            : "form-container form-container-ModelosDocumentos"
        }
      >
        <form
          className="formInputCadInc"
          id={`ModelosDocumentosForm-${modelosdocumentosData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
