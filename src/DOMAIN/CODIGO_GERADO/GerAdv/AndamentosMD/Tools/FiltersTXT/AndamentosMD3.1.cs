...
public static class FilterAndamentosMDTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar AndamentosMD em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterAndamentosMD:

- O filter irá filtrar os AndamentosMD conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterAndamentosMD:(Andamentos M D)
- nome: _nome ou descrição para Andamentos M D;
- processo: número ou IDs;
- andamento: número ou IDs;
- pathfull: PathFull;
- unc: UNC;
...
- Se ouvir "todos os AndamentosMD" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterAndamentosMD? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt AndamentosMD length: {result.Length}");
#endif
