...
import { GUTTipo } from '../../Models/GUTTipo';
...
export class GUTTipoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class GUTTipoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/GUTTipo`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTTipo_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTTipo');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTTipo_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar G U T Tipo com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterGUTTipo): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTTipo_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de GUTTipo');
        }
    }

            public async filter(filtro: FilterGUTTipo): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTTipo_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTTipo');
                }
            }


    public async validation(regGUTTipo: IGUTTipo): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTTipo as GUTTipo, this.getHeaders());
    ...
    public async addAndUpdate(regGUTTipo: IGUTTipo): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regGUTTipo as GUTTipo, this.getHeaders());
            var register = result.data as IGUTTipo;        
            const action = regGUTTipo.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Tipo');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o GUTTipo. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'GUTTipo',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir G U T Tipo com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTTipo) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<GUTTipo[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de guttipo:', error);
            this.notificationService.notify({
              entity: 'GUTTipo',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos guttipo'
            });
          }
        });
    }
