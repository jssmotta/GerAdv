...
import { EnderecoSistema } from '../../Models/EnderecoSistema';
...
export class EnderecoSistemaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'EnderecoSistemaApiError';
    this.status = status;
    this.code = code;
...
export class EnderecoSistemaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/EnderecoSistema`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnderecoSistema_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os EnderecoSistema');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnderecoSistema_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Endereco Sistema com ID ${id}`);
        }
    }

            public async filter(filtro: FilterEnderecoSistema): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EnderecoSistema_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar EnderecoSistema');
                }
            }


    public async validation(regEnderecoSistema: IEnderecoSistema): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEnderecoSistema as EnderecoSistema, this.getHeaders());
    ...
    public async addAndUpdate(regEnderecoSistema: IEnderecoSistema): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regEnderecoSistema as EnderecoSistema, this.getHeaders());
            var register = result.data as IEnderecoSistema;        
            const action = regEnderecoSistema.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Endereco Sistema');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o EnderecoSistema. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'EnderecoSistema',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Endereco Sistema com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEnderecoSistema) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<EnderecoSistema[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de enderecosistema:', error);
            this.notificationService.notify({
              entity: 'EnderecoSistema',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos enderecosistema'
            });
          }
        });
    }
