...
public static class FaseEndpoints
{
    ...
    private static void MapFaseRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/GetColumns", async (GetColumns parameters, string uri, IFaseValidation validation, IFaseWriter writer, IJusticaReader justicaReader, IAreaReader areaReader, IFaseService service) =>
        {
            logger.LogInfo("Fase", "GetColumns", $"id = {parameters.Id}", $"columns = {parameters.Columns}", uri);
            var result = await service.GetColumns(parameters, uri);
            if (result == null)
            {
                logger.LogWarn("Fase", "GetColumns", $"No columns found for id = {parameters.Id}", uri);
                return Results.NotFound();
            }

            return Results.Ok(result);
        }).WithName("Fase_GetColumns").WithDisplayName("Get Fase Columns");
        group.MapPost("/UpdateColumns", async (UpdateColumnsRequest parameters, string uri, IFaseValidation validation, IFaseWriter writer, IJusticaReader justicaReader, IAreaReader areaReader, IFaseService service) =>
        {
            logger.LogInfo("Fase", "UpdateColumns", $"id = {parameters.Id}", $"parameters = {parameters}", uri);
            var result = await service.UpdateColumns(parameters, uri);
            if (!result)
            {
                logger.LogWarn("Fase", "UpdateColumns", $"Failed to update columns for id = {parameters.Id}", uri);
                return Results.BadRequest();
            }

            return Results.Ok();
        }).WithName("Fase_UpdateColumns").WithDisplayName("Update Fase Columns");
