...
public class FTipoModeloDocumentoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FTipoModeloDocumentoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FTipoModeloDocumentoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FTipoModeloDocumentoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "tpdCodigo")
        var dt = new DataTable();
        dt.Columns.Add("tpdCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["tpdCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FTipoModeloDocumento>();
    }
    ...
    {
        // Arrange
        var factory = new FTipoModeloDocumentoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("tpdCodigo", typeof(int));
        mockDataTable.Columns.Add("tpdNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["tpdCodigo"] = testId;
        row["tpdNome"] = "Test TipoModeloDocumento";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["tpdCodigo"] = 123;
        row["tpdNome"] = "Test TipoModeloDocumento Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FTipoModeloDocumento properties
        }
    }
