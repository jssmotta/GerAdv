...
import { DadosProcuracaoApiError } from '../GerAdv_TS/DadosProcuracao/Apis/ApiDadosProcuracao';
import { DadosProcuracaoApi } from '@/app/GerAdv_TS/DadosProcuracao/Apis/ApiDadosProcuracao';
import { FilterDadosProcuracao } from '../GerAdv_TS/DadosProcuracao/Filters/DadosProcuracao';
...
describe('DadosProcuracaoApi', () => {
  let dadosprocuracaoApi: DadosProcuracaoApi;
  ...
  beforeEach(() => {
    ...
    dadosprocuracaoApi = new DadosProcuracaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(dadosprocuracaoApi).toBeInstanceOf(DadosProcuracaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new DadosProcuracaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(DadosProcuracaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all dadosprocuracao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await dadosprocuracaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(dadosprocuracaoApi.getAll()).rejects.toThrow(DadosProcuracaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await dadosprocuracaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-DadosProcuracao_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DadosProcuracao',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw DadosProcuracaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockDadosProcuracao = DadosProcuracaoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockDadosProcuracao,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch dadosprocuracao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await dadosprocuracaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockDadosProcuracao));
      ...
      expect(result.data).toEqual(mockDadosProcuracao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DadosProcuracao',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterDadosProcuracao = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter dadosprocuracao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await dadosprocuracaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockDadosProcuracao: IDadosProcuracao = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new dadosprocuracao successfully', async () => {
      const responseDadosProcuracao = { ...mockDadosProcuracao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseDadosProcuracao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await dadosprocuracaoApi.addAndUpdate(mockDadosProcuracao);
    ...
    it('should update existing dadosprocuracao successfully', async () => {
      const existingDadosProcuracao = { ...mockDadosProcuracao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingDadosProcuracao,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await dadosprocuracaoApi.addAndUpdate(existingDadosProcuracao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(dadosprocuracaoApi.addAndUpdate(mockDadosProcuracao)).rejects.toThrow(DadosProcuracaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DadosProcuracao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete dadosprocuracao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await dadosprocuracaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(dadosprocuracaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'DadosProcuracao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(dadosprocuracaoApi.delete(1)).rejects.toThrow(DadosProcuracaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterDadosProcuracao = { funcionario: 1 };
      
      const result = dadosprocuracaoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await dadosprocuracaoApi.delete(1);
  ...
  describe('DadosProcuracaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new DadosProcuracaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('DadosProcuracaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new DadosProcuracaoApiError('Test message', 500);
