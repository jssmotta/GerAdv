...
#region ListarDados_AgendaQuem
    public static async IAsyncEnumerable<DBAgendaQuem> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBAgendaQuem(dbRec);
        }
    }

    public static IEnumerable<DBAgendaQuem> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
