...
export const UFGridMobileComponent = React.memo(({
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('uf-columns-order-mobile'));
      if (savedOrder) {
...
const SearchFromCellCidade = (props: any) => {
    return (
      <>
        <td>
            <div onClick={() => openSearchCellCidade(props.dataItem.id)}><span title='Pesquisar relacionados em Cidade'><SvgIcon icon={searchIcon} /></span></div>
        </td>
      </>
    );
};
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('uf-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
