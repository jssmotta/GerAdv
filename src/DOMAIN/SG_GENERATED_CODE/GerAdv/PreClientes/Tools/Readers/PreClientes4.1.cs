...
public partial class PreClientesReader
{
    private static string BuildSqlQuery(string campos, string whereClause, string orderClause, int max, MsiSqlConnection? oCnn = null)
    {
        ...
        var orderQuery = $"{TSql.OrderBy} {DBPreClientesDicInfo.CampoNome}";
...
[{DBCidadeDicInfo.PTabelaNome}].[{DBCidadeDicInfo.Nome}]
                   FROM {DBPreClientes.PTabelaNome.dbo(oCnn)}
                   LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBPreClientesDicInfo.PTabelaNome}].[{DBPreClientesDicInfo.IDRep}]
...
SELECT TOP (1) 
	FORMAT(cliDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(cliDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    cliVisto as Visto
FROM {oCnn.UseDbo}.PreClientes dbq WITH (INDEX = idx_PreClientes_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.cliQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.cliQuemAtu
WHERE [cliCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBPreClientes.PTabelaNome,
...
FROM {oCnn.UseDbo}.PreClientes WITH (INDEX = idx_{DBPreClientesDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [cliCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBPreClientesDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(cliDtAtu, cliDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBPreClientesDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(cliDtAtu, cliDtCad) DESC;", oCnn.InnerConnection);
