  ...
  const loadObjetos = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Objetos";
...
export const useObjetosList = (
  dataService: IObjetosService,
  currentFilter?: FilterObjetos | undefined | null,
) => {
  const [data, setData] = useState<IObjetos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterObjetos | undefined | null>(
    currentFilter,
  );
