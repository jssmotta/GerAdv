...
public static class FilterProProcuradoresTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar ProProcuradores em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterProProcuradores:

- O filter irá filtrar os ProProcuradores conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterProProcuradores:(Pro Procuradores)
- advogado: número ou IDs;
- nome: _nome ou descrição para Pro Procuradores;
- processo: número ou IDs;
- data: não é a data de cadastramento é a data do ProProcuradores;
...
- Se ouvir "todos os ProProcuradores" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterProProcuradores? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt ProProcuradores length: {result.Length}");
#endif
