...
public partial class PenhoraController(IPenhoraService penhoraService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterPenhora filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Penhora: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _penhoraService.Filter(max, filtro, uri);
        return Ok(result);
    }
