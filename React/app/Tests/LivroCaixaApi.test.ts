...
import { LivroCaixaApiError } from '../GerAdv_TS/LivroCaixa/Apis/ApiLivroCaixa';
import { LivroCaixaApi } from '@/app/GerAdv_TS/LivroCaixa/Apis/ApiLivroCaixa';
import { FilterLivroCaixa } from '../GerAdv_TS/LivroCaixa/Filters/LivroCaixa';
...
describe('LivroCaixaApi', () => {
  let livrocaixaApi: LivroCaixaApi;
  ...
  beforeEach(() => {
    ...
    livrocaixaApi = new LivroCaixaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(livrocaixaApi).toBeInstanceOf(LivroCaixaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new LivroCaixaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(LivroCaixaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all livrocaixa items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await livrocaixaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(livrocaixaApi.getAll()).rejects.toThrow(LivroCaixaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await livrocaixaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-LivroCaixa_last_getAll_${200}`)));
    });
    ...
    it('should throw LivroCaixaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockLivroCaixa = LivroCaixaTestEmpty();
    ...
    it('should fetch livrocaixa by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await livrocaixaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockLivroCaixa));
      ...
      expect(result.data).toEqual(mockLivroCaixa);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixa',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterLivroCaixa = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter livrocaixa items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await livrocaixaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockLivroCaixa: ILivroCaixa = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new livrocaixa successfully', async () => {
      const responseLivroCaixa = { ...mockLivroCaixa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseLivroCaixa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await livrocaixaApi.addAndUpdate(mockLivroCaixa);
    ...
    it('should update existing livrocaixa successfully', async () => {
      const existingLivroCaixa = { ...mockLivroCaixa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingLivroCaixa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await livrocaixaApi.addAndUpdate(existingLivroCaixa);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(livrocaixaApi.addAndUpdate(mockLivroCaixa)).rejects.toThrow(LivroCaixaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixa',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete livrocaixa successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await livrocaixaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(livrocaixaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'LivroCaixa',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(livrocaixaApi.delete(1)).rejects.toThrow(LivroCaixaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterLivroCaixa = { funcionario: 1 };
      
      const result = livrocaixaApi.useFilter(mockFilter);
  ...
  describe('LivroCaixaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new LivroCaixaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('LivroCaixaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new LivroCaixaApiError('Test message', 500);
