...
describe('OperadoresApi', () => {
  let operadoresApi: OperadoresApi;
  ...
  beforeEach(() => {
    ...
    operadoresApi = new OperadoresApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(operadoresApi).toBeInstanceOf(OperadoresApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new OperadoresApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(OperadoresApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all operadores items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadoresApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(operadoresApi.getAll()).rejects.toThrow(OperadoresApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await operadoresApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Operadores_last_getAll_${200}`)));
    });
    ...
    it('should throw OperadoresApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockOperadores = OperadoresTestEmpty();
    ...
    it('should fetch operadores by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await operadoresApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockOperadores));
      ...
      expect(result.data).toEqual(mockOperadores);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operadores',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter operadores items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await operadoresApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new operadores successfully', async () => {
      const responseOperadores = { ...mockOperadores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseOperadores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadoresApi.addAndUpdate(mockOperadores);
    ...
    it('should update existing operadores successfully', async () => {
      const existingOperadores = { ...mockOperadores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingOperadores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await operadoresApi.addAndUpdate(existingOperadores);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(operadoresApi.addAndUpdate(mockOperadores)).rejects.toThrow(OperadoresApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operadores',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete operadores successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await operadoresApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(operadoresApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Operadores',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(operadoresApi.delete(1)).rejects.toThrow(OperadoresApiError);
    });
  });
  ...
  describe('OperadoresApiError', () => {
    it('should create error with correct properties', () => {
      const error = new OperadoresApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('OperadoresApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new OperadoresApiError('Test message', 500);
