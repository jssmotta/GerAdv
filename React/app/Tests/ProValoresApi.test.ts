...
import { ProValoresApiError } from '../GerAdv_TS/ProValores/Apis/ApiProValores';
import { ProValoresApi } from '@/app/GerAdv_TS/ProValores/Apis/ApiProValores';
import { FilterProValores } from '../GerAdv_TS/ProValores/Filters/ProValores';
...
describe('ProValoresApi', () => {
  let provaloresApi: ProValoresApi;
  ...
  beforeEach(() => {
    ...
    provaloresApi = new ProValoresApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(provaloresApi).toBeInstanceOf(ProValoresApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProValoresApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProValoresApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all provalores items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await provaloresApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(provaloresApi.getAll()).rejects.toThrow(ProValoresApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await provaloresApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProValores_last_getAll_${200}`)));
    });
    ...
    it('should throw ProValoresApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProValores = ProValoresTestEmpty();
    ...
    it('should fetch provalores by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await provaloresApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProValores));
      ...
      expect(result.data).toEqual(mockProValores);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProValores',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProValores = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter provalores items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await provaloresApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProValores: IProValores = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new provalores successfully', async () => {
      const responseProValores = { ...mockProValores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProValores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await provaloresApi.addAndUpdate(mockProValores);
    ...
    it('should update existing provalores successfully', async () => {
      const existingProValores = { ...mockProValores, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProValores,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await provaloresApi.addAndUpdate(existingProValores);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(provaloresApi.addAndUpdate(mockProValores)).rejects.toThrow(ProValoresApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProValores',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete provalores successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await provaloresApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(provaloresApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProValores',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(provaloresApi.delete(1)).rejects.toThrow(ProValoresApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProValores = { funcionario: 1 };
      
      const result = provaloresApi.useFilter(mockFilter);
  ...
  describe('ProValoresApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProValoresApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProValoresApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProValoresApiError('Test message', 500);
