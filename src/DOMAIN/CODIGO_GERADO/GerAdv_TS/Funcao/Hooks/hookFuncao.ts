...
export const useFuncaoForm = (
  ...
  const loadFuncao = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialFuncao);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Função';
      setError(errorMessage);
      console.log('Erro ao carregar Função');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialFuncao]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadFuncao, resetForm]);
  return returnValue;
};


export const useFuncaoNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('Funcao', (entity) => {
      try {
...
export const useFuncaoList = (dataService: IFuncaoService) => {
  const [data, setData] = useState<IFuncao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar funcao';
      setError(errorMessage);
      console.log('Erro ao carregar funcao');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
