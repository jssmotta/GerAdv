...
public partial class ReuniaoController(IReuniaoService reuniaoService) : ControllerBase
{
    ...
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] MenphisSI.GerAdv.Filters.FilterReuniao filter, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Reuniao: Filter called max {0} with filtro = {1}, {2}", max, filter, uri);
        var result = await _reuniaoService.Filter(max, filter, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> FilterVoice([FromBody] MenphisSI.GerAdv.Filters.FilterReuniaoWithVoiceRequest request, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Reuniao: Filter called with {0} filtro = {1}", request, uri);
        var result = await _reuniaoService.FilterVoice(request.Filter, request.VoiceCommand, uri);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetAuditor(int id, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        try
        {
            //_logger.Info("Reuniao: GetAuditor called with id = {0}, {1}", id, uri);
            var result = await _reuniaoService.GetAuditor(id, uri, token);
            if (result == null)
            {
