...
describe('PreClientesApi', () => {
  let preclientesApi: PreClientesApi;
  ...
  beforeEach(() => {
    ...
    preclientesApi = new PreClientesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(preclientesApi).toBeInstanceOf(PreClientesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PreClientesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PreClientesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all preclientes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await preclientesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(preclientesApi.getAll()).rejects.toThrow(PreClientesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await preclientesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-PreClientes_last_getAll_${200}`)));
    });
    ...
    it('should throw PreClientesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPreClientes = PreClientesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockPreClientes,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch preclientes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await preclientesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPreClientes));
      ...
      expect(result.data).toEqual(mockPreClientes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PreClientes',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter preclientes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await preclientesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new preclientes successfully', async () => {
      const responsePreClientes = { ...mockPreClientes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePreClientes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await preclientesApi.addAndUpdate(mockPreClientes);
    ...
    it('should update existing preclientes successfully', async () => {
      const existingPreClientes = { ...mockPreClientes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPreClientes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await preclientesApi.addAndUpdate(existingPreClientes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(preclientesApi.addAndUpdate(mockPreClientes)).rejects.toThrow(PreClientesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PreClientes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete preclientes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await preclientesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(preclientesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PreClientes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(preclientesApi.delete(1)).rejects.toThrow(PreClientesApiError);
    });
  });
  ...
  describe('PreClientesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PreClientesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PreClientesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PreClientesApiError('Test message', 500);
