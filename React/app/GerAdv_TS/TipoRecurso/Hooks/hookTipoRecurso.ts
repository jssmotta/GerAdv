...
export const useTipoRecursoForm = (
  ...
  const loadTipoRecurso = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Tipo Recurso";
...
export const useTipoRecursoList = (
  dataService: ITipoRecursoService,
  currentFilter?: FilterTipoRecurso | undefined | null,
) => {
  const [data, setData] = useState<ITipoRecurso[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoRecurso | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterTipoRecurso | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar tiporecurso";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
