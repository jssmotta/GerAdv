// app/TestsForms/GUTTipoComboBox.test.tsx - Vers�o Corrigida
...
import { GUTTipoEmpty, GUTTipoTestEmpty } from '@/app/GerAdv_TS/Models/GUTTipo';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/GUTTipo/Hooks/hookGUTTipo', () => ({
  useGUTTipoComboBox: jest.fn(() => ({
    options: [
    { ...GUTTipoEmpty(), id: 1, nome: 'G U T Tipo 1' },
    { ...GUTTipoEmpty(), id: 2, nome: 'G U T Tipo 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/GUTTipo/Apis/ApiGUTTipo', () => ({
  GUTTipoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTTipo' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTTipo' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/GUTTipo/Services/GUTTipo.service', () => ({
  GUTTipoService: jest.fn().mockImplementation(() => ({
    fetchGUTTipoById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock GUTTipo' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/GUTTipo/Crud/Grids/GUTTipoWindow', () => {
  return function MockGUTTipoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='guttipo-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'G U T Tipo' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...GUTTipoTestEmpty(), id: 1, nome: 'G U T Tipo 1' },
  { ...GUTTipoTestEmpty(), id: 2, nome: 'G U T Tipo 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: GUTTipoEmpty(), 
};
describe('GUTTipoComboBox', () => {
  ...
  const { useGUTTipoComboBox } = require('@/app/GerAdv_TS/GUTTipo/Hooks/hookGUTTipo');
  (useGUTTipoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<GUTTipoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('G U T Tipo')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select G U T Tipo')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...GUTTipoTestEmpty(), id: 1, nome: 'G U T Tipo 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useGUTTipoComboBox } = require('@/app/GerAdv_TS/GUTTipo/Hooks/hookGUTTipo');
    useGUTTipoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens GUTTipoWindow on action button click (add)', async () => {
    render(<GUTTipoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('guttipo-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('guttipo-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<GUTTipoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles GUTTipoWindow onClose and onSuccess', () => {
    render(<GUTTipoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<GUTTipoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
