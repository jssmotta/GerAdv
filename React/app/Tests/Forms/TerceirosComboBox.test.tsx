// app/TestsForms/TerceirosComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import TerceirosComboBox from '@/app/GerAdv_TS/Terceiros/ComboBox/Terceiros';
...
import { TerceirosEmpty, TerceirosTestEmpty } from '@/app/GerAdv_TS/Models/Terceiros';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Terceiros/Hooks/hookTerceiros', () => ({
  useTerceirosComboBox: jest.fn(() => ({
    options: [
    { ...TerceirosEmpty(), id: 1, nome: 'Terceiros 1' },
    { ...TerceirosEmpty(), id: 2, nome: 'Terceiros 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Terceiros/Apis/ApiTerceiros', () => ({
  TerceirosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Terceiros' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Terceiros' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Terceiros/Services/Terceiros.service', () => ({
  TerceirosService: jest.fn().mockImplementation(() => ({
    fetchTerceirosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Terceiros' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Terceiros/Crud/Grids/TerceirosWindow', () => {
  return function MockTerceirosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='terceiros-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Terceiros' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TerceirosTestEmpty(), id: 1, nome: 'Terceiros 1' },
  { ...TerceirosTestEmpty(), id: 2, nome: 'Terceiros 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TerceirosEmpty(), 
};
describe('TerceirosComboBox', () => {
  ...
  const { useTerceirosComboBox } = require('@/app/GerAdv_TS/Terceiros/Hooks/hookTerceiros');
  (useTerceirosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TerceirosComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Terceiros')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TerceirosTestEmpty(), id: 1, nome: 'Terceiros 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTerceirosComboBox } = require('@/app/GerAdv_TS/Terceiros/Hooks/hookTerceiros');
    useTerceirosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TerceirosWindow on action button click (add)', async () => {
    render(<TerceirosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('terceiros-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('terceiros-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TerceirosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TerceirosWindow onClose and onSuccess', () => {
    render(<TerceirosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TerceirosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
