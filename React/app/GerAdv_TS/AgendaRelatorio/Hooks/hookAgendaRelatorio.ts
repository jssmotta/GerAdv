...
export const useAgendaRelatorioForm = (
  ...
  const loadAgendaRelatorio = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Agenda Relatorio";
...
export const useAgendaRelatorioList = (
  dataService: IAgendaRelatorioService,
  currentFilter?: FilterAgendaRelatorio | undefined | null,
) => {
  const [data, setData] = useState<IAgendaRelatorio[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterAgendaRelatorio | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterAgendaRelatorio | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar agendarelatorio";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsAgendaRelatorio() {
  ...
  function validate(data: IAgendaRelatorio): {
    isValid: boolean;
    message: string;
  } {
