...
export const useAgendaStatusList = (dataService: IAgendaStatusService, currentFilter?: FilterAgendaStatus | undefined | null) => {
  const [data, setData] = useState<IAgendaStatus[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterAgendaStatus | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterAgendaStatus | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterAgendaStatus);
      setData(result);
    } catch (err) {
