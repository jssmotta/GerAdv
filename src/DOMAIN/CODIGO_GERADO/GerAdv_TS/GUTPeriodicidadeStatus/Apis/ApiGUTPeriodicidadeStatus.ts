...
export class GUTPeriodicidadeStatusApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'GUTPeriodicidadeStatusApiError';
    this.status = status;
    this.code = code;
...
export class GUTPeriodicidadeStatusApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/GUTPeriodicidadeStatus`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidadeStatus_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTPeriodicidadeStatus');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTPeriodicidadeStatus_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTPeriodicidadeStatus');
                }
            }


    public async validation(regGUTPeriodicidadeStatus: IGUTPeriodicidadeStatus): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTPeriodicidadeStatus as GUTPeriodicidadeStatus, this.getHeaders());
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
                if (error.response && error.response.data) {
                    ...
                    this.notificationService.notify({
                        entity: 'GUTPeriodicidadeStatus',
                        id: regGUTPeriodicidadeStatus.id,
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Periodicidade Status');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o GUTPeriodicidadeStatus. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'GUTPeriodicidadeStatus',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir G U T Periodicidade Status com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTPeriodicidadeStatus) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<GUTPeriodicidadeStatus[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de gutperiodicidadestatus:', error);
            this.notificationService.notify({
              entity: 'GUTPeriodicidadeStatus',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gutperiodicidadestatus'
            });
          }
        });
    }
