// app/TestsForms/TipoStatusBiuComboBox.test.tsx - Vers�o Corrigida
...
import { TipoStatusBiuEmpty, TipoStatusBiuTestEmpty } from '@/app/GerAdv_TS/Models/TipoStatusBiu';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TipoStatusBiu/Hooks/hookTipoStatusBiu', () => ({
  useTipoStatusBiuComboBox: jest.fn(() => ({
    options: [
    { ...TipoStatusBiuEmpty(), id: 1, nome: 'Staus  Usuários 1' },
    { ...TipoStatusBiuEmpty(), id: 2, nome: 'Staus  Usuários 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TipoStatusBiu/Apis/ApiTipoStatusBiu', () => ({
  TipoStatusBiuApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoStatusBiu' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoStatusBiu' }),
...
jest.mock('@/app/GerAdv_TS/TipoStatusBiu/Services/TipoStatusBiu.service', () => ({
  TipoStatusBiuService: jest.fn().mockImplementation(() => ({
    fetchTipoStatusBiuById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoStatusBiu' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TipoStatusBiu/Crud/Grids/TipoStatusBiuWindow', () => {
  return function MockTipoStatusBiuWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tipostatusbiu-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Staus  Usuários' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TipoStatusBiuTestEmpty(), id: 1, nome: 'Staus  Usuários 1' },
  { ...TipoStatusBiuTestEmpty(), id: 2, nome: 'Staus  Usuários 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TipoStatusBiuEmpty(), 
};
describe('TipoStatusBiuComboBox', () => {
  ...
  const { useTipoStatusBiuComboBox } = require('@/app/GerAdv_TS/TipoStatusBiu/Hooks/hookTipoStatusBiu');
  (useTipoStatusBiuComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TipoStatusBiuComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Staus  Usuários')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Staus  Usuários')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TipoStatusBiuTestEmpty(), id: 1, nome: 'Staus  Usuários 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTipoStatusBiuComboBox } = require('@/app/GerAdv_TS/TipoStatusBiu/Hooks/hookTipoStatusBiu');
    useTipoStatusBiuComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TipoStatusBiuWindow on action button click (add)', async () => {
    render(<TipoStatusBiuComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tipostatusbiu-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tipostatusbiu-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TipoStatusBiuComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TipoStatusBiuWindow onClose and onSuccess', () => {
    render(<TipoStatusBiuComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TipoStatusBiuComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
