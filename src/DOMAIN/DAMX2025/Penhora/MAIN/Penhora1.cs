...
public partial class DBPenhora : VAuditor, ICadastros, IAuditor
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_Penhora)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBPenhora();
...
#region TableDefinition_Penhora
    [XmlIgnore]
    public string TabelaNome => "Penhora";
...
#endregion
    public DBPenhora(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBPenhora(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBPenhora(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_Penhora
...
#endif
    {
        ...
        if (!isInsert)
            if (!(pFldFProcesso || pFldFNome || pFldFDescricao || pFldFDataPenhora || pFldFPenhoraStatus || pFldFMaster))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
        ...
        {
            throw new Exception("Campo 'phrNome' está vazio!");
        }
