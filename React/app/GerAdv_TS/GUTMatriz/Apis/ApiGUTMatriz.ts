...
export class GUTMatrizApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class GUTMatrizApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os GUTMatriz');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar G U T Matriz com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de GUTMatriz');
        }
    }

            public async filter(filtro: FilterGUTMatriz): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GUTMatriz_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar GUTMatriz');
                }
            }


    public async validation(regGUTMatriz: IGUTMatriz): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regGUTMatriz as GUTMatriz, this.getHeaders());
    ...
    public async addAndUpdate(regGUTMatriz: IGUTMatriz): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar G U T Matriz');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir G U T Matriz com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterGUTMatriz) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<GUTMatriz[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de gutmatriz:', error);
            this.notificationService.notify({
              entity: 'GUTMatriz',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos gutmatriz'
            });
          }
        });
    }
