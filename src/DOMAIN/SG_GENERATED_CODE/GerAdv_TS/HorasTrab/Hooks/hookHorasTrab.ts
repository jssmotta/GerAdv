...
export const useHorasTrabList = (dataService: IHorasTrabService, currentFilter?: FilterHorasTrab | undefined | null) => {
  const [data, setData] = useState<IHorasTrab[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterHorasTrab | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterHorasTrab | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterHorasTrab);
      setData(result);
    } catch (err) {
