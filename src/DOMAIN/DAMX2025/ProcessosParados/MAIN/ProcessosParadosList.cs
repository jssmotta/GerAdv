...
#region ListarDados_ProcessosParados
    public static async IAsyncEnumerable<DBProcessosParados> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProcessosParados(dbRec);
        }
    }

    public static IEnumerable<DBProcessosParados> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
