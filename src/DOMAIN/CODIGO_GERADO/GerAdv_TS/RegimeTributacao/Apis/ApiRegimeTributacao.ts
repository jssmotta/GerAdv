...
import { RegimeTributacao } from '../../Models/RegimeTributacao';
...
export class RegimeTributacaoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'RegimeTributacaoApiError';
    this.status = status;
    this.code = code;
...
export class RegimeTributacaoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/RegimeTributacao`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-RegimeTributacao_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os RegimeTributacao');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-RegimeTributacao_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Regime Tributacao com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterRegimeTributacao): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-RegimeTributacao_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de RegimeTributacao');
        }
    }

            public async filter(filtro: FilterRegimeTributacao): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-RegimeTributacao_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar RegimeTributacao');
                }
            }


    public async validation(regRegimeTributacao: IRegimeTributacao): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regRegimeTributacao as RegimeTributacao, this.getHeaders());
    ...
    public async addAndUpdate(regRegimeTributacao: IRegimeTributacao): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regRegimeTributacao as RegimeTributacao, this.getHeaders());
            var register = result.data as IRegimeTributacao;        
            const action = regRegimeTributacao.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Regime Tributacao');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o RegimeTributacao. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'RegimeTributacao',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Regime Tributacao com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterRegimeTributacao) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<RegimeTributacao[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de regimetributacao:', error);
            this.notificationService.notify({
              entity: 'RegimeTributacao',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos regimetributacao'
            });
          }
        });
    }
