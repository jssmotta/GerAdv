...
export const useTipoStatusBiuForm = (
  ...
  const loadTipoStatusBiu = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Staus  Usuários";
...
export const useTipoStatusBiuList = (
  dataService: ITipoStatusBiuService,
  currentFilter?: FilterTipoStatusBiu | undefined | null,
) => {
  const [data, setData] = useState<ITipoStatusBiu[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoStatusBiu | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterTipoStatusBiu | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar tipostatusbiu";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoStatusBiu() {
  ...
  function validate(data: ITipoStatusBiu): {
    isValid: boolean;
    message: string;
  } {
