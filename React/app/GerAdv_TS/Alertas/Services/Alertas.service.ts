...
export class AlertasService implements IAlertasService {
  ...
  async saveAlertas(alertas: IAlertas): Promise<IAlertas> {    
    ...
    try {
      ...
      console.log('Error fetching Alertas list');
      return [];
    }
  }
   ...
   async getAll(
    filtro?: FilterAlertas,
    onOnlineData?: (data: IAlertas[]) => void
  ): Promise<IAlertas[]> {
    try {
      ...
      if (onOnlineData) {
        ...
        this.api.filter(0, filtro ?? {})
          ...
          .catch(error => {
            console.log('Error fetching online Alertas');
          });
      ...
      console.log('Error fetching all Alertas:');
      return [];
    }
  }
