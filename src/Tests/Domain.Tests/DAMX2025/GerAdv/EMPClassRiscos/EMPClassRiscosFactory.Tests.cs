...
public class FEMPClassRiscosFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FEMPClassRiscosFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FEMPClassRiscosFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FEMPClassRiscosFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "ecrCodigo")
        var dt = new DataTable();
        dt.Columns.Add("ecrCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["ecrCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FEMPClassRiscos>();
    }
    ...
    {
        // Arrange
        var factory = new FEMPClassRiscosFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("ecrCodigo", typeof(int));
        mockDataTable.Columns.Add("ecrNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["ecrCodigo"] = testId;
        row["ecrNome"] = "Test EMPClassRiscos";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["ecrCodigo"] = 123;
        row["ecrNome"] = "Test EMPClassRiscos Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FEMPClassRiscos properties
        }
    }
