...
#region ListarDados_Funcionarios
    public static async IAsyncEnumerable<DBFuncionarios> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBFuncionarios(dbRec);
        }
    }

    public static IEnumerable<DBFuncionarios> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
