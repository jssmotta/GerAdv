...
public class FProcessosObsReportFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProcessosObsReportFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProcessosObsReportFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProcessosObsReportFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "prrCodigo")
        var dt = new DataTable();
        dt.Columns.Add("prrCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["prrCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProcessosObsReport>();
    }
    ...
    {
        // Arrange
        var factory = new FProcessosObsReportFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("prrCodigo", typeof(int));
        mockDataTable.Columns.Add("prrObservacao", typeof(string));
        var row = mockDataTable.NewRow();
        row["prrCodigo"] = testId;
        row["prrObservacao"] = "Test ProcessosObsReport";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["prrCodigo"] = 123;
        row["prrObservacao"] = "Test ProcessosObsReport Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProcessosObsReport properties
        }
    }
