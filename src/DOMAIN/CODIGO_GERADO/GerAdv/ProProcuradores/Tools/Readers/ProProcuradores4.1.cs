...
[{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBAdvogadosDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBAdvogadosDicInfo.PTabelaNome}].[{DBAdvogadosDicInfo.CampoCodigo}]=[{DBProProcuradoresDicInfo.PTabelaNome}].[{DBProProcuradoresDicInfo.Advogado}]
...
LEFT JOIN {DBCidadeDicInfo.PTabelaNome.dbo(oCnn)} ON [{DBCidadeDicInfo.PTabelaNome}].[{DBCidadeDicInfo.CampoCodigo}]=[{DBEscritoriosDicInfo.PTabelaNome}].[{DBEscritoriosDicInfo.Cidade}]
...
FROM {oCnn.UseDbo}.ProProcuradores WITH (INDEX = idx_ProProcuradores_AuditorDtAtu)
WHERE [papCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
