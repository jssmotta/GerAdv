...
import { FilterPaises } from "@/app/GerAdv_TS/Paises/Filters/Paises";
...
interface UsePaisesFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterPaises | undefined | null,
  ) => Promise<void>;
}

interface UsePaisesFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterPaises;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterPaises>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterPaises) => Promise<void>;

  // Render function
  renderInputFilters: (
    handlers: FilterHandlers<FilterPaises>,
  ) => React.ReactNode;
...
export const usePaisesFilter = ({
  handleFetchWithFilter,
}: UsePaisesFilterProps): UsePaisesFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterPaises>(
    {} as FilterPaises,
  );
  ...
  const handleConfirmSearch = async (filter: FilterPaises) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterPaises);
    sessionStorage.removeItem(btoa("PaisesFilter"));
    handleFetchWithFilter({} as FilterPaises);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterPaises>) => (
    <>
      <InputInput
        tabIndex={0}
        type="text"
