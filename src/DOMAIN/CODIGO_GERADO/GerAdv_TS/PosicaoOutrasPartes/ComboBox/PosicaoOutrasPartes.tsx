...
const PosicaoOutrasPartesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPosicaoOutrasPartes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPosicaoOutrasPartes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPosicaoOutrasPartes);
    ...
    const handleAddClick = () => {
        ...
        const add = PosicaoOutrasPartesEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PosicaoOutrasPartesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPosicaoOutrasPartes={editar ?? addValue ?? PosicaoOutrasPartesEmpty()} /> 
            )}
