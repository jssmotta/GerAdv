...
const ProcessOutputSourcesComboBox: React.FC<DadosSelectProps> = ({
  ...
  const [editRecord, setEditRecord] = useState<IProcessOutputSources | null>(
    null,
  );
  const [addRecord, setAddRecord] = useState<IProcessOutputSources | null>(
    null,
  );
  ...
  const loadRecordForEdit = async (id: number) => {
    try {
      const record =
        await processoutputsourcesService.fetchProcessOutputSourcesById(id);
  ...
  const handleSuccess = (newProcessOutputSources?: IProcessOutputSources) => {
    if (newProcessOutputSources) {
      setValue({
        id: newProcessOutputSources.id,
        nome: newProcessOutputSources.nome,
      });
      ...
      {(editRecord || addRecord) && isOpen && (
        <ProcessOutputSourcesWindow
          ...
          selectedProcessOutputSources={
            editRecord || addRecord || ProcessOutputSourcesEmpty()
          }
        />
      )}
