...
public partial class HistoricoService(IOptions<AppSettings> appSettings, IFHistoricoFactory historicoFactory, IHistoricoReader reader, IHistoricoValidation validation, IHistoricoWriter writer, IProcessosReader processosReader, IPrecatoriaReader precatoriaReader, IApensoReader apensoReader, IFaseReader faseReader, IStatusAndamentoReader statusandamentoReader, IProcessosObsReportService processosobsreportService, IRecadosService recadosService, IHttpContextAccessor httpContextAccessor, HybridCache cache, IMemoryCache memory) : IHistoricoService, IDisposable
{
    ...
    private readonly IFHistoricoFactory historicoFactory = historicoFactory;
    private readonly IHistoricoReader reader = reader;
    private readonly IHistoricoValidation validation = validation;
    private readonly IHistoricoWriter writer = writer;
    ...
    private readonly IStatusAndamentoReader statusandamentoReader = statusandamentoReader;
    ...
    public async Task<IEnumerable<HistoricoResponseAll>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
        ...
        var cacheKey = $"{uri}-Historico-Filter-{keyCache}";
    ...
    private async Task<HistoricoResponse?> GetDataByIdAsync(int id, MsiSqlConnection oCnn, CancellationToken token) => await reader.Read(id, oCnn);
    public async Task<HistoricoResponse?> AddAndUpdate([FromBody] Models.Historico regHistorico, [FromRoute, Required] string uri)
    {
        ...
        using var saved = await writer.WriteAsync(regHistorico, operadorId, oCnn);
    ...
    public async Task<HistoricoResponse?> Validation([FromBody] Models.Historico regHistorico, [FromRoute, Required] string uri)
    {
        ...
        return await reader.Read(regHistorico.Id, oCnn);
    }
    ...
    public async Task<HistoricoResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        var historico = await reader.Read(id, oCnn);
        try
        {
            if (historico != null)
            {
                await writer.Delete(historico, nOperador, oCnn);
