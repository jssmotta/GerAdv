...
[{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.Nome}]
                   ...
                   LEFT JOIN {DBOponentesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBOponentesDicInfo.PTabelaNome}].[{DBOponentesDicInfo.CampoCodigo}]=[{DBGruposEmpresasDicInfo.PTabelaNome}].[{DBGruposEmpresasDicInfo.Oponente}]
...
LEFT JOIN {DBClientesDicInfo.PTabelaNome.dbo(oCnn)} (NOLOCK) ON [{DBClientesDicInfo.PTabelaNome}].[{DBClientesDicInfo.CampoCodigo}]=[{DBGruposEmpresasDicInfo.PTabelaNome}].[{DBGruposEmpresasDicInfo.Cliente}]
...
SELECT TOP (1) 
	FORMAT(grpDtCad, 'dd/MM/yyyy HH:mm:ss') as DtCad,
    FORMAT(grpDtAtu, 'dd/MM/yyyy HH:mm:ss') as DtAtu,
	c.operNome as QuemCad,
	u.operNome as QuemAtu,
    grpVisto as Visto
FROM {oCnn.UseDbo}.GruposEmpresas dbq WITH (INDEX = idx_GruposEmpresas_AuditorDtAtu)
LEFT JOIN Operador c on c.operCodigo = dbq.grpQuemCad
LEFT JOIN Operador u on u.operCodigo = dbq.grpQuemAtu
WHERE [grpCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        ...
        if (await dr.ReadAsync())
        {
            ...
            {
                TableName = DBGruposEmpresas.PTabelaNome,
...
FROM {oCnn.UseDbo}.GruposEmpresas WITH (INDEX = idx_{DBGruposEmpresasDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [grpCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBGruposEmpresasDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(grpDtAtu, grpDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBGruposEmpresasDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(grpDtAtu, grpDtCad) DESC;", oCnn.InnerConnection);
