...
export const useProProcuradoresList = (dataService: IProProcuradoresService, currentFilter?: FilterProProcuradores | undefined | null) => {
  const [data, setData] = useState<IProProcuradores[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterProProcuradores | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterProProcuradores | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterProProcuradores);
      setData(result);
    } catch (err) {
