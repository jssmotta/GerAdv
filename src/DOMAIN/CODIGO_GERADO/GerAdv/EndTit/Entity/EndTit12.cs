...
public partial class FEndTit : MenphisSI.SG.GerAdv.DBEndTit, IDBEndTit
{
    ...
    public static FEndTit CreateFromReader(SqlDataReader? dbRec)
    {
        return new FEndTit(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FEndTit CreateFromDataRow(DataRow? dbRec)
    {
        return new FEndTit(dbRec);
    }

    private FEndTit(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FEndTit(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FEndTit> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fEndTit = new FEndTit();
        await fEndTit.CarregarAsync(id, oCnn);
        return fEndTit;
    }

    private FEndTit(List<SqlParameter> parameters, MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FEndTit CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FEndTit(parameters, oCnn, fullSql, sqlWhere, join);
    }
