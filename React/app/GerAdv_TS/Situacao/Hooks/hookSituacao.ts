...
export const useSituacaoForm = (
  ...
  const loadSituacao = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Situação";
...
export const useSituacaoList = (
  dataService: ISituacaoService,
  currentFilter?: FilterSituacao | undefined | null,
) => {
  const [data, setData] = useState<ISituacao[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterSituacao | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterSituacao | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar situacao";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsSituacao() {
  ...
  function validate(data: ISituacao): { isValid: boolean; message: string } {
    ...
    try {
      if (data.parte_int.includes("%")) {
        return { isValid: false, message: `Remova o caracter '%' inválido!` };
      }
      if (data.parte_int.length <= 0) {
        return {
          isValid: false,
          message: "O campo Parte_Int não pode ficar vazio.",
        };
      }
      if (data.parte_int.length > 30) {
        return {
          isValid: false,
          message: "O campo Parte_Int não pode ter mais de 30 caracteres.",
        };
      }
      if (data.parte_opo.length > 30) {
        return {
          isValid: false,
          message: "O campo Parte_Opo não pode ter mais de 30 caracteres.",
        };
      }
