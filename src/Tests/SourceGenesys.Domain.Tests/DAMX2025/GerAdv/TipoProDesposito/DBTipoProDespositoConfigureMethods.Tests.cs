...
/// <summary>
/// Comprehensive unit tests for ConfigureUpdateFields and ConfigureAuditorFields methods in DBTipoProDesposito class
...
/// </summary>
public class DBTipoProDespositoConfigureMethodsTests : IDisposable
{
    private readonly DBTipoProDesposito _tipoprodesposito;
    private readonly Mock<MsiSqlConnection> _mockConnection;
    public DBTipoProDespositoConfigureMethodsTests()
    {
        _tipoprodesposito = new DBTipoProDesposito();
    ...
    public void ConfigureUpdateFields_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        // Arrange
        ResetAllFieldFlags();
        _tipoprodesposito.ID = 1; // Update scenario
        // Act
        var result = _tipoprodesposito.Update(_mockConnection.Object);
    ...
    public async Task ConfigureUpdateFieldsAsync_WhenNoFieldsChanged_ShouldNotUpdateDatabase()
    {
        ...
        try
        {
            var result = await _tipoprodesposito.UpdateAsync(_mockConnection.Object);
...
#region Helper Methods
    ...
    {
        var field = typeof(DBTipoProDesposito).GetField(fieldName, BindingFlags.NonPublic | BindingFlags.Instance);
        field?.SetValue(_tipoprodesposito, value);
    }
    ...
    private void SetFieldValue(string fieldName, object? value)
    {
        ...
        {
            field?.SetValue(_tipoprodesposito, Convert.ToDateTime(value));
        }
    }
...
#region Fields_All
    ...
    public void ConfigureUpdateFields_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _tipoprodesposito.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
    ...
    public async Task ConfigureUpdateFieldsAsync_WithNomeFieldChanged_ShouldProcessUpdate(string value)
    {
        ...
        _tipoprodesposito.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _tipoprodesposito.UpdateAsync(_mockConnection.Object));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithCancellationToken_ShouldRespectCancellation()
    {
        ...
        var exception = await Record.ExceptionAsync(async () => await _tipoprodesposito.UpdateAsync(_mockConnection.Object, cancellationToken: cts.Token));
        exception.Should().NotBeNull();
    }
    ...
    public async Task ConfigureUpdateFieldsAsyncNome_WithTimeout_ShouldRespectTimeout()
    {
        ...
        _tipoprodesposito.ID = 0; // Insert scenario
        // Act & Assert - Should throw exception due to mocked connection
        var exception = await Record.ExceptionAsync(async () => await _tipoprodesposito.UpdateAsync(_mockConnection.Object, timeoutSeconds: 1));
        exception.Should().NotBeNull();
    }
    ...
    {
        // Arrange
        _tipoprodesposito.ID = 123; // Update scenario
        _tipoprodesposito.AuditorQuem = 7;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection but audit fields should be set
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        _tipoprodesposito.AuditorQuem.Should().Be(7);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemIsZero_ShouldSetToDefaultValue()
    {
        // Arrange
        _tipoprodesposito.ID = 0; // Insert scenario
        _tipoprodesposito.AuditorQuem = 0; // Should be set to 1
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        ...
        _tipoprodesposito.AuditorQuem.Should().Be(1);
    }
    ...
    public void ConfigureAuditorFieldsNome_WhenAuditorQuemHasValue_ShouldPreserveValue(int auditorValue)
    {
        // Arrange
        _tipoprodesposito.ID = 0; // Insert scenario
        _tipoprodesposito.AuditorQuem = auditorValue;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        ...
        _tipoprodesposito.AuditorQuem.Should().Be(auditorValue);
    }
    ...
    {
        // Arrange
        _tipoprodesposito.ID = 0;
        _tipoprodesposito.AuditorQuem = 10;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        ...
        _tipoprodesposito.AuditorQuem.Should().Be(10);
    }
    ...
    {
        // Arrange
        _tipoprodesposito.ID = id;
        _tipoprodesposito.AuditorQuem = 15;
        SetFieldFlag("pFldFNome", true); // At least one field must change
        SetFieldValue("m_FNome", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        ...
        _tipoprodesposito.AuditorQuem.Should().Be(15);
    }
    ...
    public void ConfigureMethodsNome_Integration_BothMethodsWorkTogether_ShouldNotInterfere()
    {
        ...
        SetAllFieldValues();
        _tipoprodesposito.ID = 123;
        _tipoprodesposito.AuditorQuem = 99;
        // Act & Assert - Should throw exception due to mocked connection
        var exception = Record.Exception(() => _tipoprodesposito.Update(_mockConnection.Object));
        exception.Should().NotBeNull();
        // Assert - Both update and audit configuration should work
        _tipoprodesposito.FNome.Should().Be("");
        _tipoprodesposito.AuditorQuem.Should().Be(99);
    }
    ...
    public void ConfigureFieldsNome_Integration_FieldChangeTracking_ShouldWorkCorrectly()
    {
        ...
        var result1 = _tipoprodesposito.Update(_mockConnection.Object);
