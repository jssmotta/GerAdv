...
export const useProcessOutputSourcesForm = (
  ...
  const loadProcessOutputSources = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Process Output Sources";
...
export const useProcessOutputSourcesList = (
  dataService: IProcessOutputSourcesService,
  currentFilter?: FilterProcessOutputSources | undefined | null,
) => {
  const [data, setData] = useState<IProcessOutputSources[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    async (filtro?: FilterProcessOutputSources | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar processoutputsources";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProcessOutputSources() {
  async function runValidation(
    ...
    const processoutputsourcesApi = new ProcessOutputSourcesApi(
      uri ?? "",
      token ?? "",
    );
  ...
  function validate(data: IProcessOutputSources): {
    isValid: boolean;
    message: string;
  } {
