...
import { usePoderJudiciarioAssociadoGrid } from '../../Hooks/hookPoderJudiciarioAssociadoGrid';
...
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('poderjudiciarioassociado-columns-order-mobile'));
    if (savedOrder) {
...
const {
  ...
  RowNumberCell, 
  DeleteRow, 
} = usePoderJudiciarioAssociadoGrid({
data, 
onRowClick, 
onDeleteClick, 
setSelectedId, 
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('poderjudiciarioassociado-columns-order-mobile'), JSON.stringify(newOrder));
        } catch (error) {
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('poderjudiciarioassociado-columns-order-mobile'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-poderjudiciarioassociado',
  columns: exportColumns, 
  sheetName: 'PoderJudiciarioAssociado'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-PoderJudiciarioAssociado-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-poderjudiciarioassociado',
  columns: exportColumnsPdf, 
  title: 'PoderJudiciarioAssociado'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-PoderJudiciarioAssociado-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  className='grid-mobile-crud grid-mobile-poderjudiciarioassociado'
  key={`poderjudiciarioassociado-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
