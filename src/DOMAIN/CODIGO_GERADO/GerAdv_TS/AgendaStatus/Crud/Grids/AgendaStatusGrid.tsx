...
import { useAgendaStatusList } from '../../Hooks/hookAgendaStatus';
...
const {
  ...
  fetchData, 
  refreshData
} = useAgendaStatusList(agendastatusService);
...
const handleFetchWithFilter = async (filtro?: FilterAgendaStatus | undefined | null) => {
  setCurrFilter(filtro);
  await fetchData(filtro ?? {} as FilterAgendaStatus);
};
...
const confirmDelete = async () => {
  if (deleteId !== null) {
    try {
      await agendastatusService.deleteAgendaStatus(deleteId);
      // O hook já escuta as notificações e recarrega automaticamente
    } catch (error) {
...
{loading && (
  ...
  {isMobile ? (
    <AgendaStatusGridMobileComponent
    data={agendastatus}
    ...
    <AgendaStatusGridDesktopComponent
    data={agendastatus}
    onRowClick={handleRowClick}
    onDeleteClick={onDeleteClick}
    setSelectedId={setSelectedId}
