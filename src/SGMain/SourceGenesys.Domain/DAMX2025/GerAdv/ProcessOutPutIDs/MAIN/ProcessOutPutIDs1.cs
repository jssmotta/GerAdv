...
public partial class DBProcessOutPutIDs : VNoAuditor, ICrud
{
...
#region GravarDados_ProcessOutPutIDs
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (pFldFNome)
            clsW.Fields(DBProcessOutPutIDsDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFGuid)
            clsW.Fields(DBProcessOutPutIDsDicInfo.Guid, FGuid, EGenericTypeFields.FString);
        if (insertId != 0)
            return GravaNewId();
