...
public static class FilterStatusAndamentoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar StatusAndamento em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterStatusAndamento:

- O filter irá filtrar os StatusAndamento conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterStatusAndamento:(Status Andamento)
- nome: _nome ou descrição para Status Andamento;
- icone: número ou IDs;
- bold: Bold;
...
- Se ouvir "todos os StatusAndamento" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterStatusAndamento? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt StatusAndamento length: {result.Length}");
#endif
