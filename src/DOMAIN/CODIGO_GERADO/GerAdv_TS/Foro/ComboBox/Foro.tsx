...
const ForoComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IForo | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IForo | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IForo);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ForoWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedForo={editar ?? addValue ?? ForoEmpty()} /> 
            )}
