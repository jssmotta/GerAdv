...
public class FTipoCompromissoFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FTipoCompromissoFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FTipoCompromissoFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FTipoCompromissoFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "tipCodigo")
        var dt = new DataTable();
        dt.Columns.Add("tipCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["tipCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FTipoCompromisso>();
    }
    ...
    {
        // Arrange
        var factory = new FTipoCompromissoFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("tipCodigo", typeof(int));
        mockDataTable.Columns.Add("tipDescricao", typeof(string));
        var row = mockDataTable.NewRow();
        row["tipCodigo"] = testId;
        row["tipDescricao"] = "Test TipoCompromisso";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["tipCodigo"] = 123;
        row["tipDescricao"] = "Test TipoCompromisso Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FTipoCompromisso properties
        }
    }
