...
#region ListarDados_ProcessOutPutIDs
    public static async IAsyncEnumerable<DBProcessOutPutIDs> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProcessOutPutIDs(dbRec);
        }
    }

    public static IEnumerable<DBProcessOutPutIDs> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
