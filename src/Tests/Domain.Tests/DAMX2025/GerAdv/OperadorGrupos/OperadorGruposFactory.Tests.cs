...
public class FOperadorGruposFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FOperadorGruposFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FOperadorGruposFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FOperadorGruposFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "opgCodigo")
        var dt = new DataTable();
        dt.Columns.Add("opgCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["opgCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FOperadorGrupos>();
    }
    ...
    {
        // Arrange
        var factory = new FOperadorGruposFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("opgCodigo", typeof(int));
        mockDataTable.Columns.Add("opgNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["opgCodigo"] = testId;
        row["opgNome"] = "Test OperadorGrupos";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["opgCodigo"] = 123;
        row["opgNome"] = "Test OperadorGrupos Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FOperadorGrupos properties
        }
    }
