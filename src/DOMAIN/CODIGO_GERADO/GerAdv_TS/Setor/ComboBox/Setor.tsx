...
const SetorComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ISetor | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ISetor | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ISetor);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <SetorWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedSetor={editar ?? addValue ?? SetorEmpty()} /> 
            )}
