// app/TestsForms/TipoEMailComboBox.test.tsx - Vers�o Corrigida
...
import { TipoEMailEmpty, TipoEMailTestEmpty } from '@/app/GerAdv_TS/Models/TipoEMail';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TipoEMail/Hooks/hookTipoEMail', () => ({
  useTipoEMailComboBox: jest.fn(() => ({
    options: [
    { ...TipoEMailEmpty(), id: 1, nome: 'Tipo E Mail 1' },
    { ...TipoEMailEmpty(), id: 2, nome: 'Tipo E Mail 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TipoEMail/Apis/ApiTipoEMail', () => ({
  TipoEMailApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEMail' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEMail' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/TipoEMail/Services/TipoEMail.service', () => ({
  TipoEMailService: jest.fn().mockImplementation(() => ({
    fetchTipoEMailById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEMail' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TipoEMail/Crud/Grids/TipoEMailWindow', () => {
  return function MockTipoEMailWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tipoemail-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Tipo E Mail' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TipoEMailTestEmpty(), id: 1, nome: 'Tipo E Mail 1' },
  { ...TipoEMailTestEmpty(), id: 2, nome: 'Tipo E Mail 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TipoEMailEmpty(), 
};
describe('TipoEMailComboBox', () => {
  ...
  const { useTipoEMailComboBox } = require('@/app/GerAdv_TS/TipoEMail/Hooks/hookTipoEMail');
  (useTipoEMailComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TipoEMailComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Tipo E Mail')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Tipo E Mail')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TipoEMailTestEmpty(), id: 1, nome: 'Tipo E Mail 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTipoEMailComboBox } = require('@/app/GerAdv_TS/TipoEMail/Hooks/hookTipoEMail');
    useTipoEMailComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TipoEMailWindow on action button click (add)', async () => {
    render(<TipoEMailComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tipoemail-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tipoemail-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TipoEMailComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TipoEMailWindow onClose and onSuccess', () => {
    render(<TipoEMailComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TipoEMailComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
