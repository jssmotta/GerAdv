...
const PrepostosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IPrepostos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IPrepostos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IPrepostos);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <PrepostosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedPrepostos={editar ?? addValue ?? PrepostosEmpty()} /> 
            )}
