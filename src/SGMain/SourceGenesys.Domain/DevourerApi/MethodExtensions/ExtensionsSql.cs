...
public static class ExtensionMethodStringsW3
{
    ...
    public static bool IsValidEmail(this string? email)
    {
        ...
        {
            if (email.Contains(" "))
                return false; // Invalid email if it contains spaces
            if (email.Contains("@@"))
                return false; // Invalid email if it contains double '@'
            if (email.Contains(".."))
                return false; // Invalid email if it contains double '..'
    ...
    public static string SqlCmdTextLike(this string campo, string? text) => $" [{campo}] {DevourerConsts.MsiCollate} like '{text?.Trim().Replace("?", "%").PreparaParaSql()}%' ";

    public static string SqlCmdTextLikeInit(this string campo, string? text) => $" [{campo}] {DevourerConsts.MsiCollate} like '%{text.replace("?", "%").trim().PreparaParaSql()}%' ";
    public static string SqlCmdTextLikeSpaces(this string campo, string? text) => $" [{campo}] {DevourerConsts.MsiCollate} like '%{text.PreparaParaSql().Replace("?", "%").Replace(" ", "%")}%' "; // less .ToUpper - 09-03-2017
