...
export const useAndamentosMDForm = (
  ...
  const loadAndamentosMD = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Andamentos M D";
...
export const useAndamentosMDList = (
  dataService: IAndamentosMDService,
  currentFilter?: FilterAndamentosMD | undefined | null,
) => {
  const [data, setData] = useState<IAndamentosMD[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterAndamentosMD | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterAndamentosMD | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar andamentosmd";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsAndamentosMD() {
  ...
  function validate(data: IAndamentosMD): {
    isValid: boolean;
    message: string;
  } {
    ...
    try {
      ...
      if (data.pathfull.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo PathFull não pode ter mais de 2147483647 caracteres.",
        };
      }
      if (data.unc.length > 2147483647) {
        return {
          isValid: false,
          message: "O campo UNC não pode ter mais de 2147483647 caracteres.",
        };
      }
