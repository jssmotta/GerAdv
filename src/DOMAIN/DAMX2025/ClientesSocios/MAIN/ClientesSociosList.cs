...
#region ListarDados_ClientesSocios
    public static async IAsyncEnumerable<DBClientesSocios> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBClientesSocios(dbRec);
        }
    }

    public static IEnumerable<DBClientesSocios> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
