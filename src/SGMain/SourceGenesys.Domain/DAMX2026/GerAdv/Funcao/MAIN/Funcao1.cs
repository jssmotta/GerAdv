...
public partial class DBFuncao : VAuditor, ICrud
{
...
#region GravarDados_Funcao
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (pFldFDescricao)
            clsW.Fields(DBFuncaoDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (pFldFQuemCad)
            clsW.Fields(DBFuncaoDicInfo.QuemCad, FQuemCad, EGenericTypeFields.FNumberNull);
        if (pFldFDtCad)
            clsW.Fields(DBFuncaoDicInfo.DtCad, FDtCad, EGenericTypeFields.FDate);
        if (pFldFQuemAtu)
            clsW.Fields(DBFuncaoDicInfo.QuemAtu, FQuemAtu, EGenericTypeFields.FNumberNull);
        if (pFldFDtAtu)
            clsW.Fields(DBFuncaoDicInfo.DtAtu, FDtAtu, EGenericTypeFields.FDate);
        if (pFldFVisto || ID.IsEmptyIDNumber())
            clsW.Fields(DBFuncaoDicInfo.Visto, FVisto, EGenericTypeFields.FBoolean);
        if (insertId != 0)
            return GravaNewId();
