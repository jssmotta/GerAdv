...
public partial class ProcessOutputRequest
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(porDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.ProcessOutputRequest WITH (NOLOCK, INDEX = idx_ProcessOutputRequest_AuditorDtAtu)
WHERE porCodigo = @id
...
FROM {oCnn.UseDbo}.ProcessOutputRequest WITH (NOLOCK)
