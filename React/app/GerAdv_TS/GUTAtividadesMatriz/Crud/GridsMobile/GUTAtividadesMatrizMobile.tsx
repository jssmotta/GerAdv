...
import { useGUTAtividadesMatrizGrid } from "../../Hooks/hookGUTAtividadesMatrizGrid";
  ...
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("gutatividadesmatriz-columns-order-mobile"),
        );
        if (savedOrder) {
    ...
    useEffect(() => {
      const cleanup = useIOSScrollFallback({
        scrollSelector:
          ".grid-mobile-crud.grid-mobile-gutatividadesmatriz .k-grid-content",
        waitForDOM: true,
        retryDelay: 100,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("gutatividadesmatriz-columns-order-mobile"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-gutatividadesmatriz",
      columns: exportColumns,
      sheetName: "GUTAtividadesMatriz",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-GUTAtividadesMatriz-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-gutatividadesmatriz",
      columns: exportColumnsPdf,
      title: "GUTAtividadesMatriz",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-GUTAtividadesMatriz-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
      ...
      <>
        {initialized && (
          <>
            ...
            <Grid
              className="grid-mobile-crud grid-mobile-gutatividadesmatriz"
              key={`gutatividadesmatriz-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
