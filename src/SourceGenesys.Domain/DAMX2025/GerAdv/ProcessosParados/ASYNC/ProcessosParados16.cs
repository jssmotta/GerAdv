...
public partial class DBProcessosParados
{
    ...
    /// </summary>
    public static async Task<DBProcessosParados> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBProcessosParados();
...
#if (!NOTSTORED_ProcessosParados)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFSemana || pFldFAno || pFldFDataHora || pFldFOperador || pFldFDataHistorico || pFldFDataNENotas;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBProcessosParadosDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFSemana)
            updateTool.Fields(DBProcessosParadosDicInfo.Semana, m_FSemana, ETiposCampos.FNumber);
        if (pFldFAno)
            updateTool.Fields(DBProcessosParadosDicInfo.Ano, m_FAno, ETiposCampos.FNumber);
        if (pFldFDataHora)
            updateTool.Fields(DBProcessosParadosDicInfo.DataHora, m_FDataHora, ETiposCampos.FDate);
        if (pFldFOperador)
            updateTool.Fields(DBProcessosParadosDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFDataHistorico)
            updateTool.Fields(DBProcessosParadosDicInfo.DataHistorico, m_FDataHistorico, ETiposCampos.FDate);
        if (pFldFDataNENotas)
            updateTool.Fields(DBProcessosParadosDicInfo.DataNENotas, m_FDataNENotas, ETiposCampos.FDate);
    }
