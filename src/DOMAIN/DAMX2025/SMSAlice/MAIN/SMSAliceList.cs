...
#region ListarDados_SMSAlice
    public static async IAsyncEnumerable<DBSMSAlice> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBSMSAlice(dbRec);
        }
    }

    public static IEnumerable<DBSMSAlice> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
