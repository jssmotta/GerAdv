...
public partial class DBAgendaRepetirDias
{
    ...
    /// </summary>
    public static async Task<DBAgendaRepetirDias> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBAgendaRepetirDias();
...
#if (!NOTSTORED_AgendaRepetirDias)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFHoraFinal || pFldFMaster || pFldFDia || pFldFHora;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFHoraFinal)
            updateTool.Fields(DBAgendaRepetirDiasDicInfo.HoraFinal, m_FHoraFinal, ETiposCampos.FDate);
        if (pFldFMaster)
            updateTool.Fields(DBAgendaRepetirDiasDicInfo.Master, m_FMaster, ETiposCampos.FNumber);
        if (pFldFDia)
            updateTool.Fields(DBAgendaRepetirDiasDicInfo.Dia, m_FDia, ETiposCampos.FNumber);
        if (pFldFHora)
            updateTool.Fields(DBAgendaRepetirDiasDicInfo.Hora, m_FHora, ETiposCampos.FDate);
    }
