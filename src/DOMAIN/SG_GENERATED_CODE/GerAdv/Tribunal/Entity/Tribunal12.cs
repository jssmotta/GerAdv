...
public partial class FTribunal : MenphisSI.SG.GerAdv.DBTribunal, IDBTribunal
{
    // DI-compatible default constructor
    public FTribunal() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FTribunal CreateFromReader(SqlDataReader? dbRec)
    {
        return new FTribunal(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FTribunal CreateFromDataRow(DataRow? dbRec)
    {
        return new FTribunal(dbRec);
    }

    private FTribunal(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FTribunal(DataRow? dbRec) : base(dbRec)
    {
    }

    private FTribunal(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FTribunal CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FTribunal(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FTribunal> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fTribunal = new FTribunal();
        await fTribunal.CarregarAsync(id, oCnn);
        return fTribunal;
    }
