...
public partial class ParteClienteOutras
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(pcoDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.ParteClienteOutras WITH (NOLOCK, INDEX = idx_ParteClienteOutras_AuditorDtAtu)
WHERE pcoCodigo = @id
...
FROM {oCnn.UseDbo}.ParteClienteOutras WITH (NOLOCK)
