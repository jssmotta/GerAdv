...
public partial class FPrepostos : MenphisSI.SG.GerAdv.DBPrepostos, IDBPrepostos
{
    // DI-compatible default constructor
    public FPrepostos() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FPrepostos CreateFromReader(SqlDataReader? dbRec)
    {
        return new FPrepostos(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FPrepostos CreateFromDataRow(DataRow? dbRec)
    {
        return new FPrepostos(dbRec);
    }

    private FPrepostos(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FPrepostos(DataRow? dbRec) : base(dbRec)
    {
    }

    private FPrepostos(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FPrepostos CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FPrepostos(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FPrepostos> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fPrepostos = new FPrepostos();
        await fPrepostos.CarregarAsync(id, oCnn);
        return fPrepostos;
    }
