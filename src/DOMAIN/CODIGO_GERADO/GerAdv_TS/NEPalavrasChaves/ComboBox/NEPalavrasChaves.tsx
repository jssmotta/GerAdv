...
const NEPalavrasChavesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<INEPalavrasChaves | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<INEPalavrasChaves | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as INEPalavrasChaves);
    ...
    const handleAddClick = () => {
        ...
        const add = NEPalavrasChavesEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <NEPalavrasChavesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedNEPalavrasChaves={editar ?? addValue ?? NEPalavrasChavesEmpty()} /> 
            )}
