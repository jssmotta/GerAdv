...
export class GruposEmpresasCliApi {
 ...
 public async filterPreload(max: number, filtro: FilterGruposEmpresasCli): Promise<AxiosResponse> {
                        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GruposEmpresasCli-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                        const offlineData = localStorage.getItem(storageKey);        
            ...
            public async filter(max: number, filtro: FilterGruposEmpresasCli): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-GruposEmpresasCli-${max}_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterGruposEmpresasCli) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<GruposEmpresasCli[]>(key, fetcher, {
          ...
          onError: (error) => {            
            console.log('Erro no SWR para filtro de GruposEmpresasCli');
            this.notificationService.notify({
              entity: 'GruposEmpresasCli',
              id: 0,
              action: NotifySystemActions.ERROR,
