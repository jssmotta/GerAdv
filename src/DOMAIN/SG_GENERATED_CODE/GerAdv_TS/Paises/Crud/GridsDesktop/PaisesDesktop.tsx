...
const SearchFromCellUF = (props: any) => {
    return (
      <>
        <td>
            <div onClick={() => openSearchCellUF(props.dataItem.id)}><span title='Pesquisar relacionados em UF'><SvgIcon icon={searchIcon} /></span></div>
        </td>
      </>
    );
};
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('paises-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
