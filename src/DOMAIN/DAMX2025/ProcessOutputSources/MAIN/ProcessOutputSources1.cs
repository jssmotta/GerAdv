...
public partial class DBProcessOutputSources : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_ProcessOutputSources)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBProcessOutputSources();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_ProcessOutputSources
    [XmlIgnore]
    public string TabelaNome => "ProcessOutputSources";

    public DBProcessOutputSources()
    {
    }

#endregion
    public DBProcessOutputSources(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBProcessOutputSources(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBProcessOutputSources(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#endif
#region GravarDados_ProcessOutputSources
#if (forWeb)
...
#endif
    {
        ...
        {
            throw new Exception("Campo 'posNome' está vazio!");
        }
        ...
        if (pFldFNome)
            clsW.Fields(DBProcessOutputSourcesDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFGUID)
            clsW.Fields(DBProcessOutputSourcesDicInfo.GUID, m_FGUID, ETiposCampos.FString);
        if (insertId != 0)
            return GravaNewId();
