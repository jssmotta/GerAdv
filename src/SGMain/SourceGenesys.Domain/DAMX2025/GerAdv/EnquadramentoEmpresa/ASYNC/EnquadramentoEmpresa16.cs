...
public partial class DBEnquadramentoEmpresa
{
    ...
    /// </summary>
    public static async Task<DBEnquadramentoEmpresa> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBEnquadramentoEmpresa();
...
#if (!NOTSTORED_EnquadramentoEmpresa)
    /// <summary>
    ...
    {
        if (pFldFNome)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFGuid)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBEnquadramentoEmpresaDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBEnquadramentoEmpresaDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
