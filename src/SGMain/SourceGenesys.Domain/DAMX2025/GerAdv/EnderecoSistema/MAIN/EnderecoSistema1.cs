...
public partial class DBEnderecoSistema : VAuditor, ICadastros
{
...
#region GravarDados_EnderecoSistema
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (!isInsert)
            if (!(pFldFCadastro || pFldFCadastroExCod || pFldFTipoEnderecoSistema || pFldFGUID || pFldFProcesso || pFldFMotivo || pFldFContatoNoLocal || pFldFCidade || pFldFEndereco || pFldFBairro || pFldFCEP || pFldFFone || pFldFFax || pFldFObservacao))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
