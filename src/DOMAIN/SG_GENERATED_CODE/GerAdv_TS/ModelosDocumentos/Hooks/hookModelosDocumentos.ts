...
export const useModelosDocumentosList = (dataService: IModelosDocumentosService, currentFilter?: FilterModelosDocumentos | undefined | null) => {
  const [data, setData] = useState<IModelosDocumentos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterModelosDocumentos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterModelosDocumentos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterModelosDocumentos);
      setData(result);
    } catch (err) {
