...
#region ListarDados_StatusInstancia
    public static async IAsyncEnumerable<DBStatusInstancia> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBStatusInstancia(dbRec);
        }
    }

    public static IEnumerable<DBStatusInstancia> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
