...
#region ListarDados_PreClientes
    public static async IAsyncEnumerable<DBPreClientes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPreClientes(dbRec);
        }
    }

    public static IEnumerable<DBPreClientes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
