...
#region ListarDados_ProProcuradores
    public static async IAsyncEnumerable<DBProProcuradores> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProProcuradores(dbRec);
        }
    }

    public static IEnumerable<DBProProcuradores> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
