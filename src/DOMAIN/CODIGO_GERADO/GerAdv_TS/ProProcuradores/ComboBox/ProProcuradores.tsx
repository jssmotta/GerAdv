...
const ProProcuradoresComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProProcuradores | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProProcuradores | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProProcuradores);
    ...
    const handleAddClick = () => {
        ...
        const add = ProProcuradoresEmpty();
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProProcuradoresWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProProcuradores={editar ?? addValue ?? ProProcuradoresEmpty()} /> 
            )}
