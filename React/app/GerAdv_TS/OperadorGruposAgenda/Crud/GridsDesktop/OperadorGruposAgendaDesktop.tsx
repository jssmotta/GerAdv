...
import { useOperadorGruposAgendaGrid } from "../../Hooks/hookOperadorGruposAgendaGrid";
...
export const OperadorGruposAgendaGridDesktopComponent = React.memo(
  ({
    ...
    useEffect(() => {
      try {
        const savedOrder = localStorage.getItem(
          btoa("operadorgruposagenda-columns-order"),
        );
        if (savedOrder) {
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("operadorgruposagenda-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-operadorgruposagenda",
      columns: exportColumns,
      sheetName: "OperadorGruposAgenda",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-OperadorGruposAgenda-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const { exportToPdf } = useExportToPdf({
      filename: "pdf-operadorgruposagenda",
      columns: exportColumnsPdf,
      title: "OperadorGruposAgenda",
    });

    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-OperadorGruposAgenda-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`operadorgruposagenda-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
