...
export class StatusAndamentoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'StatusAndamentoApiError';
    this.status = status;
    this.code = code;
...
export class StatusAndamentoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/StatusAndamento`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusAndamento_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os StatusAndamento');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusAndamento_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Status Andamento com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusAndamento_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de StatusAndamento');
        }
    }

            public async filter(filtro: FilterStatusAndamento): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-StatusAndamento_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar StatusAndamento');
                }
            }


    public async validation(regStatusAndamento: IStatusAndamento): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regStatusAndamento as StatusAndamento, this.getHeaders());
    ...
    public async addAndUpdate(regStatusAndamento: IStatusAndamento): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Status Andamento');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o StatusAndamento. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'StatusAndamento',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Status Andamento com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterStatusAndamento) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<StatusAndamento[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de statusandamento:', error);
            this.notificationService.notify({
              entity: 'StatusAndamento',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos statusandamento'
            });
          }
        });
    }
