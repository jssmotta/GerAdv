...
import InstanciaWindow from '../Crud/Grids/InstanciaWindow';
...
import { useInstanciaComboBox } from '../Hooks/hookInstancia';
const InstanciaComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useInstanciaComboBox(instanciaService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IInstancia | null>(null);
const [addRecord, setAddRecord] = useState<IInstancia | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await instanciaService.fetchInstanciaById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.nroprocesso });
  } catch (error) {
...
const handleActionClick = () => {
  ...
  const newRecord = InstanciaEmpty();
  newRecord.nroprocesso = inputValue.toUpperCase();
...
const handleSuccess = (newInstancia?: IInstancia) => {
  if (newInstancia) {
    setValue({ id: newInstancia.id, nome: newInstancia.nroprocesso });
    loadRecordForEdit(newInstancia.id);
    handleValueChange(newInstancia);
  }
  handleClose();
};
return (
...
{(editRecord || addRecord) && isOpen && (
  ...
  onError={handleClose}
  selectedInstancia={editRecord || addRecord || InstanciaEmpty()}
  />
  )}
...
export default InstanciaComboBox;