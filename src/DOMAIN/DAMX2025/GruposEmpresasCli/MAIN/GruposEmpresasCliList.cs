...
#region ListarDados_GruposEmpresasCli
    public static async IAsyncEnumerable<DBGruposEmpresasCli> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGruposEmpresasCli(dbRec);
        }
    }

    public static IEnumerable<DBGruposEmpresasCli> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
