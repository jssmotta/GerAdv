...
export class AtividadesApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterAtividades): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Atividades_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterAtividades) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Atividades[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de atividades:', error);
            this.notificationService.notify({
              entity: 'Atividades',
              id: 0,
              action: NotifySystemActions.ERROR,
