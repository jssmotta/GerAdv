...
export class DivisaoTribunalApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DivisaoTribunal_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os DivisaoTribunal');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DivisaoTribunal_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Divisao Tribunal com ID ${id}`);
        }
    }

            public async filter(filtro: FilterDivisaoTribunal): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-DivisaoTribunal_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar DivisaoTribunal');
                }
            }


    public async validation(regDivisaoTribunal: IDivisaoTribunal): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regDivisaoTribunal as DivisaoTribunal, this.getHeaders());
    ...
    public async addAndUpdate(regDivisaoTribunal: IDivisaoTribunal): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regDivisaoTribunal as DivisaoTribunal, this.getHeaders());
            var register = result.data as IDivisaoTribunal;        
            const action = regDivisaoTribunal.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o DivisaoTribunal. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'DivisaoTribunal',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Divisao Tribunal com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterDivisaoTribunal) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<DivisaoTribunal[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de divisaotribunal:', error);
            this.notificationService.notify({
              entity: 'DivisaoTribunal',
              id: 0,
              action: NotifySystemActions.ERROR,
