  ...
  const loadNENotas = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar N E Notas";
...
export const useNENotasList = (
  dataService: INENotasService,
  currentFilter?: FilterNENotas | undefined | null,
) => {
  const [data, setData] = useState<INENotas[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterNENotas | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterNENotas | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar nenotas";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
    ...
    try {
      ...
      if (data.notapublicada.length > 2147483647) {
        return {
          isValid: false,
          message:
            "O campo NotaPublicada não pode ter mais de 2147483647 caracteres.",
        };
      }
