...
import { FilterArea } from "@/app/GerAdv_TS/Area/Filters/Area";
...
interface UseAreaFilterProps {
  handleFetchWithFilter: (
    filtro?: FilterArea | undefined | null,
  ) => Promise<void>;
}

interface UseAreaFilterReturn {
  // Estados
  showSearch: boolean;
  windowFilter: FilterArea;
  setWindowFilter: React.Dispatch<React.SetStateAction<FilterArea>>;

  // Handlers do Dialog
  handleSearch: () => void;
  handleCloseSearch: () => void;
  handleConfirmSearch: (filter: FilterArea) => Promise<void>;

  // Render function
  renderInputFilters: (handlers: FilterHandlers<FilterArea>) => React.ReactNode;
...
export const useAreaFilter = ({
  handleFetchWithFilter,
}: UseAreaFilterProps): UseAreaFilterReturn => {
  const [showSearch, setShowSearch] = useState(false);
  const [windowFilter, setWindowFilter] = useState<FilterArea>(
    {} as FilterArea,
  );
  ...
  const handleConfirmSearch = async (filter: FilterArea) => {
    await handleFetchWithFilter(filter);
  };

  // Função para limpar filtros
  const clearFilter = () => {
    setWindowFilter({} as FilterArea);
    sessionStorage.removeItem(btoa("AreaFilter"));
    handleFetchWithFilter({} as FilterArea);
  };
  ...
  const renderInputFilters = (handlers: FilterHandlers<FilterArea>) => (
    <>
      <InputInput
        tabIndex={0}
        type="text"
