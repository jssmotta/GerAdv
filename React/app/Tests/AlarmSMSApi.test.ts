...
import { AlarmSMSApiError } from '../GerAdv_TS/AlarmSMS/Apis/ApiAlarmSMS';
import { AlarmSMSApi } from '@/app/GerAdv_TS/AlarmSMS/Apis/ApiAlarmSMS';
import { FilterAlarmSMS } from '../GerAdv_TS/AlarmSMS/Filters/AlarmSMS';
...
describe('AlarmSMSApi', () => {
  let alarmsmsApi: AlarmSMSApi;
  ...
  beforeEach(() => {
    ...
    alarmsmsApi = new AlarmSMSApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(alarmsmsApi).toBeInstanceOf(AlarmSMSApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AlarmSMSApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AlarmSMSApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all alarmsms items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alarmsmsApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(alarmsmsApi.getAll()).rejects.toThrow(AlarmSMSApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await alarmsmsApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AlarmSMS_last_getAll_${200}`)));
    });
    ...
    it('should throw AlarmSMSApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAlarmSMS = AlarmSMSTestEmpty();
    ...
    it('should fetch alarmsms by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alarmsmsApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAlarmSMS));
      ...
      expect(result.data).toEqual(mockAlarmSMS);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlarmSMS',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAlarmSMS = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter alarmsms items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await alarmsmsApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new alarmsms successfully', async () => {
      const responseAlarmSMS = { ...mockAlarmSMS, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAlarmSMS,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alarmsmsApi.addAndUpdate(mockAlarmSMS);
    ...
    it('should update existing alarmsms successfully', async () => {
      const existingAlarmSMS = { ...mockAlarmSMS, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAlarmSMS,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alarmsmsApi.addAndUpdate(existingAlarmSMS);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(alarmsmsApi.addAndUpdate(mockAlarmSMS)).rejects.toThrow(AlarmSMSApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlarmSMS',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete alarmsms successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await alarmsmsApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(alarmsmsApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlarmSMS',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(alarmsmsApi.delete(1)).rejects.toThrow(AlarmSMSApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAlarmSMS = { funcionario: 1 };
      
      const result = alarmsmsApi.useFilter(mockFilter);
  ...
  describe('AlarmSMSApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AlarmSMSApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AlarmSMSApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AlarmSMSApiError('Test message', 500);
