...
public static class FilterRegimeTributacaoTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterRegimeTributacao? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar RegimeTributacao em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterRegimeTributacao:

- O filter irá filtrar os RegimeTributacao conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- nome: nome ou descrição para Regime Tributacao;
...
- Se ouvir "todos os RegimeTributacao" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt RegimeTributacao length: {result.Length}");
#endif
