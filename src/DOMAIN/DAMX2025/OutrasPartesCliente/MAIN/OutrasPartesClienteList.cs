...
#region ListarDados_OutrasPartesCliente
    public static async IAsyncEnumerable<DBOutrasPartesCliente> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOutrasPartesCliente(dbRec);
        }
    }

    public static IEnumerable<DBOutrasPartesCliente> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
