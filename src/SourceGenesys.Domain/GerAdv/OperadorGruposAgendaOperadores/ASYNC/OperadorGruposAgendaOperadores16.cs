...
public partial class DBOperadorGruposAgendaOperadores
{
    ...
    /// </summary>
    public static async Task<DBOperadorGruposAgendaOperadores> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBOperadorGruposAgendaOperadores();
...
#if (!NOTSTORED_OperadorGruposAgendaOperadores)
    /// <summary>
...
#if (!NOTSTORED_OperadorGruposAgendaOperadores)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFOperadorGruposAgenda || pFldFOperador || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFOperadorGruposAgenda)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.OperadorGruposAgenda, m_FOperadorGruposAgenda, ETiposCampos.FNumber);
        if (pFldFOperador)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBOperadorGruposAgendaOperadoresDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
