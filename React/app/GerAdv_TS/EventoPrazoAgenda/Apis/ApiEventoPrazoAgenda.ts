...
export class EventoPrazoAgendaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'EventoPrazoAgendaApiError';
    this.status = status;
    this.code = code;
...
export class EventoPrazoAgendaApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EventoPrazoAgenda_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os EventoPrazoAgenda');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EventoPrazoAgenda_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Evento Prazo Agenda com ID ${id}`);
        }
    }
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EventoPrazoAgenda_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de EventoPrazoAgenda');
        }
    }

            public async filter(filtro: FilterEventoPrazoAgenda): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-EventoPrazoAgenda_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar EventoPrazoAgenda');
                }
            }


    public async validation(regEventoPrazoAgenda: IEventoPrazoAgenda): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regEventoPrazoAgenda as EventoPrazoAgenda, this.getHeaders());
    ...
    public async addAndUpdate(regEventoPrazoAgenda: IEventoPrazoAgenda): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Evento Prazo Agenda');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Evento Prazo Agenda com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterEventoPrazoAgenda) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<EventoPrazoAgenda[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de eventoprazoagenda:', error);
            this.notificationService.notify({
              entity: 'EventoPrazoAgenda',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos eventoprazoagenda'
            });
          }
        });
    }
