...
#region ListarDados_ContatoCRMView
    public static async IAsyncEnumerable<DBContatoCRMView> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBContatoCRMView(dbRec);
        }
    }

    public static IEnumerable<DBContatoCRMView> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
