...
import ProObservacoesWindow from '../Crud/Grids/ProObservacoesWindow';
...
const ProObservacoesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IProObservacoes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IProObservacoes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IProObservacoes);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ProObservacoesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedProObservacoes={editar ?? addValue ?? ProObservacoesEmpty()} /> 
            )}
