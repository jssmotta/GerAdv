...
public class FStatusTarefasFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FStatusTarefasFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FStatusTarefasFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FStatusTarefasFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "sttCodigo")
        var dt = new DataTable();
        dt.Columns.Add("sttCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["sttCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FStatusTarefas>();
    }
    ...
    {
        // Arrange
        var factory = new FStatusTarefasFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("sttCodigo", typeof(int));
        mockDataTable.Columns.Add("sttNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["sttCodigo"] = testId;
        row["sttNome"] = "Test StatusTarefas";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["sttCodigo"] = 123;
        row["sttNome"] = "Test StatusTarefas Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FStatusTarefas properties
        }
    }
