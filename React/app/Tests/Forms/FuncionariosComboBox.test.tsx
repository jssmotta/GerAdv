// app/TestsForms/FuncionariosComboBox.test.tsx - Vers�o Corrigida
...
import { FuncionariosEmpty, FuncionariosTestEmpty } from '@/app/GerAdv_TS/Models/Funcionarios';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Funcionarios/Hooks/hookFuncionarios', () => ({
  useFuncionariosComboBox: jest.fn(() => ({
    options: [
    { ...FuncionariosEmpty(), id: 1, nome: 'Colaborador 1' },
    { ...FuncionariosEmpty(), id: 2, nome: 'Colaborador 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Funcionarios/Apis/ApiFuncionarios', () => ({
  FuncionariosApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Funcionarios' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Funcionarios' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Funcionarios/Services/Funcionarios.service', () => ({
  FuncionariosService: jest.fn().mockImplementation(() => ({
    fetchFuncionariosById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Funcionarios' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Funcionarios/Crud/Grids/FuncionariosWindow', () => {
  return function MockFuncionariosWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='funcionarios-window'>
  ...
  const mockOptions = [
  { ...FuncionariosTestEmpty(), id: 1, nome: 'Colaborador 1' },
  { ...FuncionariosTestEmpty(), id: 2, nome: 'Colaborador 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: FuncionariosEmpty(), 
};
describe('FuncionariosComboBox', () => {
  ...
  const { useFuncionariosComboBox } = require('@/app/GerAdv_TS/Funcionarios/Hooks/hookFuncionarios');
  (useFuncionariosComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<FuncionariosComboBox {...defaultProps} />);
  ...
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...FuncionariosTestEmpty(), id: 1, nome: 'Colaborador 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useFuncionariosComboBox } = require('@/app/GerAdv_TS/Funcionarios/Hooks/hookFuncionarios');
    useFuncionariosComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens FuncionariosWindow on action button click (add)', async () => {
    render(<FuncionariosComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('funcionarios-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('funcionarios-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<FuncionariosComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles FuncionariosWindow onClose and onSuccess', () => {
    render(<FuncionariosComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<FuncionariosComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
