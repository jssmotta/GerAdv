...
public partial class StatusBiuService(IOptions<AppSettings> appSettings, IStatusBiuReader reader, IStatusBiuValidation validation, IStatusBiuWriter writer, ITipoStatusBiuReader tipostatusbiuReader, IOperadorReader operadorReader, HybridCache cache) : IStatusBiuService, IDisposable
{
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("StatusBiu: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBStatusBiu(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBStatusBiuDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("StatusBiu: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBStatusBiu(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBStatusBiuDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
