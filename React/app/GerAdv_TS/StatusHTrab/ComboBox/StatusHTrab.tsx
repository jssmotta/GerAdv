...
import StatusHTrabWindow from '../Crud/Grids/StatusHTrabWindow';
...
import { useStatusHTrabComboBox } from '../Hooks/hookStatusHTrab';
const StatusHTrabComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const cssDado = 'statushtrabInput';
...
const {
  ...
  handleValueChange, 
  clearValue
} = useStatusHTrabComboBox(statushtrabService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IStatusHTrab | null>(null);
const [addRecord, setAddRecord] = useState<IStatusHTrab | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await statushtrabService.fetchStatusHTrabById(id);
    setEditRecord(record);
    setAction(ActionEditar);
    handleValueChange({ id: record.id, nome: record.descricao });
  } catch (error) {
  console.log('Erro ao carregar Status H Trab:');
}
};
...
const handleActionClick = () => {
  ...
  const newRecord = StatusHTrabEmpty();
...
const handleSuccess = (newStatusHTrab?: IStatusHTrab) => {
  if (newStatusHTrab) {
    setValue({ id: newStatusHTrab.id, nome: newStatusHTrab.descricao });
    loadRecordForEdit(newStatusHTrab.id);
    handleValueChange(newStatusHTrab);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  .statushtrab-dropdown-popup-msi {
  ...
  .statushtrab-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .statushtrab-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .statushtrab-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedStatusHTrab={editRecord || addRecord || StatusHTrabEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'statushtrab-dropdown-popup-msi'
          }}
          />
...
export default StatusHTrabComboBox;