...
public partial class SetorController(ISetorService setorService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterSetor filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Setor: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _setorService.Filter(max, filtro, uri);
        return Ok(result);
    }
