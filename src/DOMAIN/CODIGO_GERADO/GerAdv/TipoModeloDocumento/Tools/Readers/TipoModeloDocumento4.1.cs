...
public partial class TipoModeloDocumento
{
    public async Task<string> ReadStringAuditor(int id, string uri, MsiSqlConnection oCnn)
    {
        string query = $@"
SELECT TOP (1) 
    FORMAT(tpdDtAtu, 'yyyy-MM-dd-HH-mm-ss')
FROM {oCnn.UseDbo}.TipoModeloDocumento WITH (NOLOCK, INDEX = idx_TipoModeloDocumento_AuditorDtAtu)
WHERE tpdCodigo = @id
...
FROM {oCnn.UseDbo}.TipoModeloDocumento WITH (NOLOCK)
