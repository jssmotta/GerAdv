...
interface StatusBiuGridProps {
  data: IStatusBiu[];
  onRowClick: (statusbiu: IStatusBiu) => void;
  onDeleteClick: (e: any) => void;
  setSelectedId: (id: number | null) => void;
}
export const StatusBiuGridDesktopComponent = React.memo(
({
  data, 
  onRowClick, 
  onDeleteClick, 
...
const filterLogic = useCallback((data: IStatusBiu, filters: Record<string, any>) => {
...
const openConsultaOperador = (id: number) => {
  router.push(`/pages/operador/?statusbiu=${id}`);
};
const EditarCellOperador = (props: any) => {
  return (
  <>
  <td>
    <div onClick={() => openConsultaOperador(props.dataItem.id)}><span title='Editar Operador'><SvgIcon icon={pencilIcon} /></span></div>
  </td>
</>
);
};
...
const gridColumns = useMemo(() => [
  ...
  width={'65px'}
  title='Operador'
  cells={{ data: EditarCellOperador }}
  />, 
  <GridColumn field='nometipostatusbiu' title='Staus Usuários' sortable={false} filterable={false} />, /* Track G.01 */
...
{initialized && (
  ...
  className='grid-desktop-statusbiu'
