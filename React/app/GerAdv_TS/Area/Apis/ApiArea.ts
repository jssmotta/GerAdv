...
export class AreaApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class AreaApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Area`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Area_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Area');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Area_last_getById_${id}`);
        try {
        ...
        public async getListN(max?: number, filtro?: FilterArea): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Area_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de Area');
        }
    }

            public async filter(filtro: FilterArea): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Area_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Area');
                }
            }


    public async validation(regArea: IArea): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regArea as Area, this.getHeaders());
    ...
    public async addAndUpdate(regArea: IArea): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regArea as Area, this.getHeaders());
            var register = result.data as IArea;        
            const action = regArea.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Area. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Area',
                        id: id,
                        action: NotifySystemActions.ERROR,
    ...
    public useFilter(filtro: FilterArea) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Area[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de area:', error);
            this.notificationService.notify({
              entity: 'Area',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos area'
            });
          }
        });
    }
