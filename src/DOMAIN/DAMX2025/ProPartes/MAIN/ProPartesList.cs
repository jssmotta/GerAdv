...
#region ListarDados_ProPartes
    public static async IAsyncEnumerable<DBProPartes> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBProPartes(dbRec);
        }
    }

    public static IEnumerable<DBProPartes> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
