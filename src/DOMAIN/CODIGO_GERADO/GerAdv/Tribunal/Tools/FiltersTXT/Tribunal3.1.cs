...
public static class FilterTribunalTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterTribunal? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Tribunal em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTribunal:

- O filter irá filtrar os Tribunal conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Tribunal" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Tribunal length: {result.Length}");
#endif
