...
#region ListarDados_GUTPeriodicidade
    public static async IAsyncEnumerable<DBGUTPeriodicidade> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTPeriodicidade(dbRec);
        }
    }

    public static IEnumerable<DBGUTPeriodicidade> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
