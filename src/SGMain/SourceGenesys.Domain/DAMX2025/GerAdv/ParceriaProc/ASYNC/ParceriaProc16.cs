...
public partial class DBParceriaProc
{
    ...
    /// </summary>
    public static async Task<DBParceriaProc> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBParceriaProc();
...
#if (!NOTSTORED_ParceriaProc)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFAdvogado || pFldFProcesso || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFAdvogado)
            updateTool.Fields(DBParceriaProcDicInfo.Advogado, FAdvogado, EGenericTypeFields.FNumber);
        if (pFldFProcesso)
            updateTool.Fields(DBParceriaProcDicInfo.Processo, FProcesso, EGenericTypeFields.FNumber);
        if (pFldFGuid)
            updateTool.Fields(DBParceriaProcDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBParceriaProcDicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBParceriaProcDicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
