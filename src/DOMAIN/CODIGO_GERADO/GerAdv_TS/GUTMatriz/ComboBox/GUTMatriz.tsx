...
const GUTMatrizComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IGUTMatriz | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IGUTMatriz | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IGUTMatriz);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <GUTMatrizWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedGUTMatriz={editar ?? addValue ?? GUTMatrizEmpty()} /> 
            )}
