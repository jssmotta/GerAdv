...
public class FTipoStatusBiuFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FTipoStatusBiuFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FTipoStatusBiuFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FTipoStatusBiuFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "tsbCodigo")
        var dt = new DataTable();
        dt.Columns.Add("tsbCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["tsbCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FTipoStatusBiu>();
    }
    ...
    {
        // Arrange
        var factory = new FTipoStatusBiuFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("tsbCodigo", typeof(int));
        mockDataTable.Columns.Add("tsbNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["tsbCodigo"] = testId;
        row["tsbNome"] = "Test TipoStatusBiu";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["tsbCodigo"] = 123;
        row["tsbNome"] = "Test TipoStatusBiu Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FTipoStatusBiu properties
        }
    }
