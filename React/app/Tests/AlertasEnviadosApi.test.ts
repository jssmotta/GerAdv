...
import { AlertasEnviadosApiError } from '../GerAdv_TS/AlertasEnviados/Apis/ApiAlertasEnviados';
import { AlertasEnviadosApi } from '@/app/GerAdv_TS/AlertasEnviados/Apis/ApiAlertasEnviados';
import { FilterAlertasEnviados } from '../GerAdv_TS/AlertasEnviados/Filters/AlertasEnviados';
...
describe('AlertasEnviadosApi', () => {
  let alertasenviadosApi: AlertasEnviadosApi;
  ...
  beforeEach(() => {
    ...
    alertasenviadosApi = new AlertasEnviadosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(alertasenviadosApi).toBeInstanceOf(AlertasEnviadosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AlertasEnviadosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AlertasEnviadosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all alertasenviados items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alertasenviadosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(alertasenviadosApi.getAll()).rejects.toThrow(AlertasEnviadosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await alertasenviadosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AlertasEnviados_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlertasEnviados',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw AlertasEnviadosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAlertasEnviados = AlertasEnviadosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAlertasEnviados,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch alertasenviados by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alertasenviadosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAlertasEnviados));
      ...
      expect(result.data).toEqual(mockAlertasEnviados);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlertasEnviados',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAlertasEnviados = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter alertasenviados items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await alertasenviadosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAlertasEnviados: IAlertasEnviados = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new alertasenviados successfully', async () => {
      const responseAlertasEnviados = { ...mockAlertasEnviados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAlertasEnviados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alertasenviadosApi.addAndUpdate(mockAlertasEnviados);
    ...
    it('should update existing alertasenviados successfully', async () => {
      const existingAlertasEnviados = { ...mockAlertasEnviados, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAlertasEnviados,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alertasenviadosApi.addAndUpdate(existingAlertasEnviados);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(alertasenviadosApi.addAndUpdate(mockAlertasEnviados)).rejects.toThrow(AlertasEnviadosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlertasEnviados',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete alertasenviados successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await alertasenviadosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(alertasenviadosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AlertasEnviados',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(alertasenviadosApi.delete(1)).rejects.toThrow(AlertasEnviadosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAlertasEnviados = { funcionario: 1 };
      
      const result = alertasenviadosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await alertasenviadosApi.delete(1);
  ...
  describe('AlertasEnviadosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AlertasEnviadosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AlertasEnviadosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AlertasEnviadosApiError('Test message', 500);
