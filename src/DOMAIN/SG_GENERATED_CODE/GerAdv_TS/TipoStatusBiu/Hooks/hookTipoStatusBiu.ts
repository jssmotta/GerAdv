...
export const useTipoStatusBiuList = (dataService: ITipoStatusBiuService, currentFilter?: FilterTipoStatusBiu | undefined | null) => {
  const [data, setData] = useState<ITipoStatusBiu[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTipoStatusBiu | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTipoStatusBiu | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTipoStatusBiu);
      setData(result);
    } catch (err) {
