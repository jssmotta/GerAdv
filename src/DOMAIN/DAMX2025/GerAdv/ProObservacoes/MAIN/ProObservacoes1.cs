...
#region TableDefinition_ProObservacoes
...
#endregion
    public DBProObservacoes(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBProObservacoes(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBProObservacoes(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_ProObservacoes)
        if (clsW.HasUpdates)
        {
            HasChanges = true;
        }
