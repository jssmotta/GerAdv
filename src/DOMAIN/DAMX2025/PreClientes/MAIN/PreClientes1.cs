...
public partial class DBPreClientes : VSexo, ICadastrosAuditor, IAuditor, IListagemCidade
{
#if (DEBUG)
...
#region TableDefinition_PreClientes
...
#endregion
    public DBPreClientes(in int nCodigo, SqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBPreClientes(in string? cNome = "", SqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        if (oCnn is null)
            return;
    ...
    public DBPreClientes(in string? sqlWhere, SqlConnection? oCnn)
    {
