...
export const useTipoProDespositoForm = (
  ...
  const loadTipoProDesposito = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Pro Desposito";
...
export const useTipoProDespositoList = (
  dataService: ITipoProDespositoService,
  currentFilter?: FilterTipoProDesposito | undefined | null,
) => {
  const [data, setData] = useState<ITipoProDesposito[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoProDesposito | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterTipoProDesposito | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipoprodesposito";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoProDesposito() {
  ...
  function validate(data: ITipoProDesposito): {
    isValid: boolean;
    message: string;
  } {
