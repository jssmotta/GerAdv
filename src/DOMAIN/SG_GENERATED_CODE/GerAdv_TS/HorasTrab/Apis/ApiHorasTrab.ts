...
import { FilterHorasTrab, FilterHorasTrabDefaults } from '../Filters/HorasTrab'
import { IFilterHorasTrabWithVoiceRequest } from '../Interfaces/interface.CommandSpeaker';
...
/** Circuit Breaker configuration for HorasTrab API */
const HORASTRAB_CIRCUIT_BREAKER_CONFIG: Partial<CircuitBreakerConfig> = {
...
export class HorasTrabApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.circuitBreaker = circuitBreakerRegistry.getBreaker(
          `HorasTrab-${uri}`,
          {
            ...HORASTRAB_CIRCUIT_BREAKER_CONFIG,
        ...
        switch (state) {
          case CircuitBreakerState.OPEN:
            this.notificationService.notify({
              ...
              message: `Serviço temporariamente indisponível. Tentando reconectar em ${HORASTRAB_CIRCUIT_BREAKER_CONFIG.resetTimeout! / 1000}s...`,
            });
            break;
    ...
    public async getAll(max: number = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HorasTrab-${uniqueKeyDay()}_lst_getAll_${max}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar todos os HorasTrab'
        );
    }
    
    public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HorasTrab-${uniqueKeyDay()}_lst_getById_${id}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          `Erro ao buscar Horas Trab com ID ${id}`,
          id
        );
    }
    
    public async getListN(max?: number, filtro?: FilterHorasTrab): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HorasTrab-${uniqueKeyDay()}_lst_listN_data`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao buscar lista de HorasTrab'
        );
    }
 public async filterPreload(max: number, filtro?: FilterHorasTrab): Promise<AxiosResponse> {

        const _filtro = filtro || new FilterHorasTrabDefaults();
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HorasTrab-${max}-${uniqueKeyDay()}_lst_filter_data_${JSON.stringify(_filtro)}`);
        const offlineData = localStorage.getItem(storageKey);        
    ...
    public async filter(max: number, filtro?: FilterHorasTrab): Promise<AxiosResponse> {
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao filtrar HorasTrab'
        );
    }


    public async filterVoice(filtro?: FilterHorasTrab, voiceCommand?: ICommandSpeakerRequest): Promise<AxiosResponse> {
        const _filtro = filtro || new FilterHorasTrabDefaults();

        // Create request object according to new backend structure
        const request: IFilterHorasTrabWithVoiceRequest = {filter: _filtro, voiceCommand: voiceCommand };

        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-HorasTrab-${uniqueKeyDay()}-fltVoice-last_filter_data_${JSON.stringify(request)}`);
        ...
        return this.executeWithCircuitBreaker(
          ...
          storageKey,
          'Erro ao gerar voice filter HorasTrab'
        );
    }
    ...
    public async validation(regHorasTrab: IHorasTrab): Promise<{ isValid: boolean; message: string } | null> {
        ...
        try {
            await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/validation`, regHorasTrab as HorasTrab, this.getHeaders());
            });
    ...
    public async addAndUpdate(regHorasTrab: IHorasTrab): Promise<AxiosResponse> {
        ...
        try {
            const result = await this.circuitBreaker.execute(async () => {
              return await axios.post(`${this.baseUrl}/AddAndUpdate`, regHorasTrab as HorasTrab, this.getHeaders());
            });
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new HorasTrabApiError(
                'Erro ao salvar Horas Trab: Serviço temporariamente indisponível',
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        ...
        try {
            ...
            if (error instanceof CircuitBreakerOpenError) {
              ...
              throw new HorasTrabApiError(
                `Erro ao excluir Horas Trab com ID ${id}: Serviço temporariamente indisponível`,
                503,
                'CIRCUIT_BREAKER_OPEN',
                error,
                true,
    ...
    public useFilter(filtro: FilterHorasTrab, voiceCommand?: ICommandSpeakerRequest) {
    const request: IFilterHorasTrabWithVoiceRequest = {filter: filtro || new FilterHorasTrabDefaults(),
    ...
    const key = `${url}::${this.authorization}::${JSON.stringify(request)}`;
        ...
        return useSWR<HorasTrab[]>(key, fetcher, {
          ...
          onError: (error) => {            
            if (process.env.NEXT_PUBLIC_SHOW_LOG === '1')
                console.log('Erro no SWR para filtro de HorasTrab');
            this.notificationService.notify({
              entity: 'HorasTrab',
              id: 0,
              action: NotifySystemActions.ERROR,
