...
public partial class DBTipoValorProcesso
{
    ...
    /// </summary>
    public static async Task<DBTipoValorProcesso> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBTipoValorProcesso();
...
#if (!NOTSTORED_TipoValorProcesso)
    /// <summary>
    ...
    {
        if (pFldFDescricao)
            updateTool.Fields(DBTipoValorProcessoDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBTipoValorProcessoDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
