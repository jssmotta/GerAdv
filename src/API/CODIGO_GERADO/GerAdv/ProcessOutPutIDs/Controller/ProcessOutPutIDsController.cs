...
public partial class ProcessOutPutIDsController(IProcessOutPutIDsService processoutputidsService) : ControllerBase
{
    ...
    {
        _logger.Info("ProcessOutPutIDs: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _processoutputidsService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("ProcessOutPutIDs: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _processoutputidsService.UpdateColumns(parameters, uri);
        if (!result)
        {
