...
import { AtividadesApiError } from '../GerAdv_TS/Atividades/Apis/ApiAtividades';
import { AtividadesApi } from '@/app/GerAdv_TS/Atividades/Apis/ApiAtividades';
import { FilterAtividades } from '../GerAdv_TS/Atividades/Filters/Atividades';
...
describe('AtividadesApi', () => {
  let atividadesApi: AtividadesApi;
  ...
  beforeEach(() => {
    ...
    atividadesApi = new AtividadesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(atividadesApi).toBeInstanceOf(AtividadesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AtividadesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AtividadesApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all atividades items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await atividadesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(atividadesApi.getAll()).rejects.toThrow(AtividadesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await atividadesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Atividades_last_getAll_${200}`)));
    });
    ...
    it('should throw AtividadesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAtividades = AtividadesTestEmpty();
    ...
    it('should fetch atividades by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await atividadesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAtividades));
      ...
      expect(result.data).toEqual(mockAtividades);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Atividades',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAtividades = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter atividades items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await atividadesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAtividades: IAtividades = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new atividades successfully', async () => {
      const responseAtividades = { ...mockAtividades, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAtividades,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await atividadesApi.addAndUpdate(mockAtividades);
    ...
    it('should update existing atividades successfully', async () => {
      const existingAtividades = { ...mockAtividades, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAtividades,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await atividadesApi.addAndUpdate(existingAtividades);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(atividadesApi.addAndUpdate(mockAtividades)).rejects.toThrow(AtividadesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Atividades',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete atividades successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await atividadesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(atividadesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Atividades',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(atividadesApi.delete(1)).rejects.toThrow(AtividadesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAtividades = { funcionario: 1 };
      
      const result = atividadesApi.useFilter(mockFilter);
  ...
  describe('AtividadesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AtividadesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AtividadesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AtividadesApiError('Test message', 500);
