...
export class RecadosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class RecadosApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Recados`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Recados_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Recados');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Recados_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Recados com ID ${id}`);
        }
    }

            public async filter(filtro: FilterRecados): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Recados_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Recados');
                }
            }


    public async validation(regRecados: IRecados): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regRecados as Recados, this.getHeaders());
    ...
    public async addAndUpdate(regRecados: IRecados): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regRecados as Recados, this.getHeaders());
            var register = result.data as IRecados;        
            const action = regRecados.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Recados');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Recados. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Recados',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Recados com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterRecados) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<Recados[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de recados:', error);
            this.notificationService.notify({
              entity: 'Recados',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos recados'
            });
          }
        });
    }
