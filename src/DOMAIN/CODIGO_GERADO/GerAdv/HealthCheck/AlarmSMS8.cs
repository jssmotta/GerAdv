...
public class AlarmSMSHealthCheck(IOptions<AppSettings> appSettings, AlarmSMSService alarmsmsService, HybridCache cache) : IHealthCheck, IDisposable
{
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private static readonly Logger _logger = AppSettingsHealthCheck.ConfigureNLog();
    private readonly AlarmSMSService _alarmsmsService = alarmsmsService;
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
                ...
                try
                {
                    ...
                    try
                    {
                        if (DBAlarmSMSDicInfo.CampoCodigo.NotIsEmpty())
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) MAX(alrCodigo) FROM {"AlarmSMS".dbo(connection)} (NOLOCK);";
                        ...
                        {
                            await using var tableCheck = connection.CreateCommand();
                            tableCheck.CommandText = $"SELECT TOP (1) alrDescricao,alrHora,alrMinuto,alrD1,alrD2,alrD3,alrD4,alrD5,alrD6,alrD7,alrEMail,alrDesativar,alrToday,alrExcetoDiasFelizes,alrDesktop,alrAlertarDataHora,alrOperador,alrGuidExo,alrAgenda,alrRecado,alrEmocao,alrGUID FROM {"AlarmSMS".dbo(connection)} (NOLOCK);";
                            tableCheck.CommandTimeout = 5;
                        ...
                        {
                            status = "Healthy",
                            message = "SELECT AlarmSMS successful",
                            timestamp = DateTime.UtcNow
                        };
                    }
                    catch (Exception ex)
                    {
                        isHealthy = false;
                        exceptions.Add(ex);
                        _logger.Warn(ex, "SELECT AlarmSMS failed for {0}. Error: {1}", uri, ex.Message);
                        healthData[$"domain_{uri}"] = new
                        {
                            status = "Unhealthy",
                            message = ex.Message,
                ...
                try
                {
                    if (maxId > 0)
                    {
                        ...
                        _ = _alarmsmsService.GetById(maxId, uri, token);
                    }
                ...
                {
                    var token = new CancellationToken();
                    var cacheKey = $"HealthCheck-{uri}-AlarmSMS";
                    ...
                    static async Task<IEnumerable<Models.AlarmSMS>> GetData(string uri, CancellationToken token)
                    {
                        await Task.Delay(1, token);
                        return[new Models.AlarmSMS
                        {
                        }
            ...
            return isHealthy ? HealthCheckResult.Healthy("AlarmSMS is operational", healthData) : exceptions.Count > 0 ? HealthCheckResult.Unhealthy("One or more systems are not operational", exceptions.First(), healthData) : HealthCheckResult.Unhealthy("One or more systems are not operational", null, healthData);
        }
        catch (Exception ex)
        {
