...
public partial class ApensoController(IApensoService apensoService) : ControllerBase
{
    ...
    public async Task<IActionResult> AddAndUpdate([FromBody] Models.Apenso regApenso, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Apenso: AddAndUpdate failed with exception for uri = {0}", uri);
    ...
    public async Task<IActionResult> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        {
            _logger.Error(ex, "Apenso: Delete failed with exception for id = {0}, {1}", id, uri);
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Validation([FromBody] Models.Apenso regApenso, [FromRoute, Required] string uri)
    {
        try
        {
            var result = await _apensoService.Validation(regApenso, uri);
            if (result == null)
            {
                _logger.Warn("Apenso: Validation failed to add or update Apenso, {0}", uri);
                return BadRequest();
            }
        ...
        {
            _logger.Error(ex, "Apenso: Validation failed with exception for uri = {0}", uri);
            return StatusCode(500, new { success = false, data = "", message = ex.Message });
        }
    }
}