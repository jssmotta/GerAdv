...
import { CargosEscClass } from '../../Models/CargosEscClass';
...
export class CargosEscClassApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'CargosEscClassApiError';
    this.status = status;
    this.code = code;
...
export class CargosEscClassApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '2')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/CargosEscClass`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEscClass_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os CargosEscClass');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEscClass_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Cargos Esc Class com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterCargosEscClass): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEscClass_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de CargosEscClass');
        }
    }

            public async filter(filtro: FilterCargosEscClass): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-CargosEscClass_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar CargosEscClass');
                }
            }


    public async validation(regCargosEscClass: ICargosEscClass): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regCargosEscClass as CargosEscClass, this.getHeaders());
    ...
    public async addAndUpdate(regCargosEscClass: ICargosEscClass): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regCargosEscClass as CargosEscClass, this.getHeaders());
            var register = result.data as ICargosEscClass;        
            const action = regCargosEscClass.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Cargos Esc Class');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o CargosEscClass. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'CargosEscClass',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Cargos Esc Class com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterCargosEscClass) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<CargosEscClass[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de cargosescclass:', error);
            this.notificationService.notify({
              entity: 'CargosEscClass',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos cargosescclass'
            });
          }
        });
    }
