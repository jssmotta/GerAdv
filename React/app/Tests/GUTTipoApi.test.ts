...
import { GUTTipoApiError } from '../GerAdv_TS/GUTTipo/Apis/ApiGUTTipo';
import { GUTTipoApi } from '@/app/GerAdv_TS/GUTTipo/Apis/ApiGUTTipo';
import { FilterGUTTipo } from '../GerAdv_TS/GUTTipo/Filters/GUTTipo';
...
describe('GUTTipoApi', () => {
  ...
  beforeEach(() => {
    ...
    guttipoApi = new GUTTipoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(guttipoApi).toBeInstanceOf(GUTTipoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GUTTipoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GUTTipoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all guttipo items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await guttipoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(guttipoApi.getAll()).rejects.toThrow(GUTTipoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await guttipoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GUTTipo_last_getAll_${200}`)));
    });
    ...
    it('should throw GUTTipoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGUTTipo = GUTTipoTestEmpty();
    ...
    it('should fetch guttipo by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await guttipoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGUTTipo));
      ...
      expect(result.data).toEqual(mockGUTTipo);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTTipo',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGUTTipo = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter guttipo items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await guttipoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new guttipo successfully', async () => {
      const responseGUTTipo = { ...mockGUTTipo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGUTTipo,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await guttipoApi.addAndUpdate(mockGUTTipo);
    ...
    it('should update existing guttipo successfully', async () => {
      const existingGUTTipo = { ...mockGUTTipo, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGUTTipo,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await guttipoApi.addAndUpdate(existingGUTTipo);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(guttipoApi.addAndUpdate(mockGUTTipo)).rejects.toThrow(GUTTipoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTTipo',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete guttipo successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await guttipoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(guttipoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTTipo',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(guttipoApi.delete(1)).rejects.toThrow(GUTTipoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGUTTipo = { funcionario: 1 };
      
      const result = guttipoApi.useFilter(mockFilter);
  ...
  describe('GUTTipoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GUTTipoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GUTTipoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GUTTipoApiError('Test message', 500);
