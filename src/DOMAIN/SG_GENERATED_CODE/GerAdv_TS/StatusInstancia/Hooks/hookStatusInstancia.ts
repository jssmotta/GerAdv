...
export const useStatusInstanciaList = (dataService: IStatusInstanciaService, currentFilter?: FilterStatusInstancia | undefined | null) => {
  const [data, setData] = useState<IStatusInstancia[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterStatusInstancia | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterStatusInstancia | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterStatusInstancia);
      setData(result);
    } catch (err) {
