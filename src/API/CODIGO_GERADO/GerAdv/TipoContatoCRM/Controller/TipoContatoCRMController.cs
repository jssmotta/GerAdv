...
public partial class TipoContatoCRMController(ITipoContatoCRMService tipocontatocrmService) : ControllerBase
{
    ...
    {
        _logger.Info("TipoContatoCRM: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _tipocontatocrmService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("TipoContatoCRM: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _tipocontatocrmService.UpdateColumns(parameters, uri);
        if (!result)
        {
