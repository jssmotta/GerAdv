...
export const useOperadorGruposAgendaList = (dataService: IOperadorGruposAgendaService, currentFilter?: FilterOperadorGruposAgenda | undefined | null) => {
  const [data, setData] = useState<IOperadorGruposAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterOperadorGruposAgenda | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterOperadorGruposAgenda | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterOperadorGruposAgenda);
      setData(result);
    } catch (err) {
