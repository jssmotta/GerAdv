...
const LigacoesComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ILigacoes | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ILigacoes | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ILigacoes);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <LigacoesWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedLigacoes={editar ?? addValue ?? LigacoesEmpty()} /> 
            )}
