...
import { OperadorGrupos } from '../../Models/OperadorGrupos';
...
export class OperadorGruposApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'OperadorGruposApiError';
    this.status = status;
    this.code = code;
...
export class OperadorGruposApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/OperadorGrupos`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupos_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os OperadorGrupos');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupos_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Operador Grupos com ID ${id}`);
        }
    }
    
        public async getListN(max?: number, filtro?: FilterOperadorGrupos): Promise<AxiosResponse> {
        if (max === undefined) max = CRUD_CONSTANTS.DEFAULT_MAX_RECORDS;
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupos_last_listN_data`);
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, 'Erro ao buscar lista de OperadorGrupos');
        }
    }

            public async filter(filtro: FilterOperadorGrupos): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-OperadorGrupos_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar OperadorGrupos');
                }
            }


    public async validation(regOperadorGrupos: IOperadorGrupos): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regOperadorGrupos as OperadorGrupos, this.getHeaders());
    ...
    public async addAndUpdate(regOperadorGrupos: IOperadorGrupos): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regOperadorGrupos as OperadorGrupos, this.getHeaders());
            var register = result.data as IOperadorGrupos;        
            const action = regOperadorGrupos.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
            ...
            this.handleApiError(error, 'Erro ao salvar Operador Grupos');
        }
    }
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o OperadorGrupos. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'OperadorGrupos',
                        id: id,
                        action: NotifySystemActions.ERROR,
             ...
             this.handleApiError(error, `Erro ao excluir Operador Grupos com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterOperadorGrupos) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<OperadorGrupos[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de operadorgrupos:', error);
            this.notificationService.notify({
              entity: 'OperadorGrupos',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos operadorgrupos'
            });
          }
        });
    }
