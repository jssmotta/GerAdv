...
import { TribEnderecosApiError } from '../GerAdv_TS/TribEnderecos/Apis/ApiTribEnderecos';
import { TribEnderecosApi } from '@/app/GerAdv_TS/TribEnderecos/Apis/ApiTribEnderecos';
import { FilterTribEnderecos } from '../GerAdv_TS/TribEnderecos/Filters/TribEnderecos';
...
describe('TribEnderecosApi', () => {
  let tribenderecosApi: TribEnderecosApi;
  ...
  beforeEach(() => {
    ...
    tribenderecosApi = new TribEnderecosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tribenderecosApi).toBeInstanceOf(TribEnderecosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TribEnderecosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TribEnderecosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tribenderecos items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tribenderecosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tribenderecosApi.getAll()).rejects.toThrow(TribEnderecosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tribenderecosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TribEnderecos_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TribEnderecos',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw TribEnderecosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTribEnderecos = TribEnderecosTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTribEnderecos,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tribenderecos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tribenderecosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTribEnderecos));
      ...
      expect(result.data).toEqual(mockTribEnderecos);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TribEnderecos',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTribEnderecos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tribenderecos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tribenderecosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTribEnderecos: ITribEnderecos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tribenderecos successfully', async () => {
      const responseTribEnderecos = { ...mockTribEnderecos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTribEnderecos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tribenderecosApi.addAndUpdate(mockTribEnderecos);
    ...
    it('should update existing tribenderecos successfully', async () => {
      const existingTribEnderecos = { ...mockTribEnderecos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTribEnderecos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tribenderecosApi.addAndUpdate(existingTribEnderecos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tribenderecosApi.addAndUpdate(mockTribEnderecos)).rejects.toThrow(TribEnderecosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TribEnderecos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tribenderecos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tribenderecosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tribenderecosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TribEnderecos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tribenderecosApi.delete(1)).rejects.toThrow(TribEnderecosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTribEnderecos = { funcionario: 1 };
      
      const result = tribenderecosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tribenderecosApi.delete(1);
  ...
  describe('TribEnderecosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TribEnderecosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TribEnderecosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TribEnderecosApiError('Test message', 500);
