...
import { useParteClienteOutrasGrid } from '../../Hooks/hookParteClienteOutrasGrid';
...
const [columnsOrder, setColumnsOrder] = useState<string[]>(['nomeoutraspartescliente',
'nropastaprocessos']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('parteclienteoutras-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('parteclienteoutras-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const basicColumnMap: Record<string, React.ReactElement> = useMemo(() => ({
  'nomeoutraspartescliente': (
  <GridColumn
  key='nomeoutraspartescliente'

  field='nomeoutraspartescliente'
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('parteclienteoutras-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-parteclienteoutras',
  columns: exportColumns, 
  sheetName: 'ParteClienteOutras'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-ParteClienteOutras-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-parteclienteoutras',
  columns: exportColumnsPdf, 
  title: 'ParteClienteOutras'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-ParteClienteOutras-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`parteclienteoutras-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-parteclienteoutras'
