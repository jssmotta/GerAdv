...
public static class FilterGUTPeriodicidadeStatusTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar GUTPeriodicidadeStatus em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGUTPeriodicidadeStatus:

- O filter irá filtrar os GUTPeriodicidadeStatus conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterGUTPeriodicidadeStatus:(G U T Periodicidade Status)
...
- Se ouvir "todos os GUTPeriodicidadeStatus" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterGUTPeriodicidadeStatus? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt GUTPeriodicidadeStatus length: {result.Length}");
#endif
