...
#region ListarDados_GUTPeriodicidadeStatus
    public static async IAsyncEnumerable<DBGUTPeriodicidadeStatus> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTPeriodicidadeStatus(dbRec);
        }
    }

    public static IEnumerable<DBGUTPeriodicidadeStatus> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
