// app/TestsForms/TipoEnderecoComboBox.test.tsx - Vers�o Corrigida
import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import TipoEnderecoComboBox from '@/app/GerAdv_TS/TipoEndereco/ComboBox/TipoEndereco';
...
import { TipoEnderecoEmpty, TipoEnderecoTestEmpty } from '@/app/GerAdv_TS/Models/TipoEndereco';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TipoEndereco/Hooks/hookTipoEndereco', () => ({
  useTipoEnderecoComboBox: jest.fn(() => ({
    options: [
    { ...TipoEnderecoEmpty(), id: 1, descricao: 'Tipo Endereco 1' },
    { ...TipoEnderecoEmpty(), id: 2, descricao: 'Tipo Endereco 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TipoEndereco/Apis/ApiTipoEndereco', () => ({
  TipoEnderecoApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock TipoEndereco' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock TipoEndereco' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/TipoEndereco/Services/TipoEndereco.service', () => ({
  TipoEnderecoService: jest.fn().mockImplementation(() => ({
    fetchTipoEnderecoById: jest.fn().mockResolvedValue({ id: 1, descricao: 'Mock TipoEndereco' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TipoEndereco/Crud/Grids/TipoEnderecoWindow', () => {
  return function MockTipoEnderecoWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tipoendereco-window'>
  ...
  >
  <option value=''>Select {props.textField === 'descricao' ? 'Tipo Endereco' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TipoEnderecoTestEmpty(), id: 1, descricao: 'Tipo Endereco 1' },
  { ...TipoEnderecoTestEmpty(), id: 2, descricao: 'Tipo Endereco 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TipoEnderecoEmpty(), 
};
describe('TipoEnderecoComboBox', () => {
  ...
  const { useTipoEnderecoComboBox } = require('@/app/GerAdv_TS/TipoEndereco/Hooks/hookTipoEndereco');
  (useTipoEnderecoComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TipoEnderecoComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Tipo Endereco')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Tipo Endereco')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TipoEnderecoTestEmpty(), id: 1, descricao: 'Tipo Endereco 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTipoEnderecoComboBox } = require('@/app/GerAdv_TS/TipoEndereco/Hooks/hookTipoEndereco');
    useTipoEnderecoComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TipoEnderecoWindow on action button click (add)', async () => {
    render(<TipoEnderecoComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tipoendereco-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tipoendereco-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TipoEnderecoComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TipoEnderecoWindow onClose and onSuccess', () => {
    render(<TipoEnderecoComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TipoEnderecoComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
