...
public partial class FOperador : MenphisSI.SG.GerAdv.DBOperador, IDBOperador
{
    // DI-compatible default constructor
    public FOperador() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FOperador CreateFromReader(SqlDataReader? dbRec)
    {
        return new FOperador(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FOperador CreateFromDataRow(DataRow? dbRec)
    {
        return new FOperador(dbRec);
    }

    private FOperador(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FOperador(DataRow? dbRec) : base(dbRec)
    {
    }

    private FOperador(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FOperador CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FOperador(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FOperador> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fOperador = new FOperador();
        await fOperador.CarregarAsync(id, oCnn);
        return fOperador;
    }
