...
import { CargosEscApiError } from '../GerAdv_TS/CargosEsc/Apis/ApiCargosEsc';
import { CargosEscApi } from '@/app/GerAdv_TS/CargosEsc/Apis/ApiCargosEsc';
import { FilterCargosEsc } from '../GerAdv_TS/CargosEsc/Filters/CargosEsc';
...
describe('CargosEscApi', () => {
  let cargosescApi: CargosEscApi;
  ...
  beforeEach(() => {
    ...
    cargosescApi = new CargosEscApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(cargosescApi).toBeInstanceOf(CargosEscApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new CargosEscApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(CargosEscApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all cargosesc items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosescApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(cargosescApi.getAll()).rejects.toThrow(CargosEscApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await cargosescApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-CargosEsc_last_getAll_${200}`)));
    });
    ...
    it('should throw CargosEscApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockCargosEsc = CargosEscTestEmpty();
    ...
    it('should fetch cargosesc by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await cargosescApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockCargosEsc));
      ...
      expect(result.data).toEqual(mockCargosEsc);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEsc',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterCargosEsc = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter cargosesc items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await cargosescApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockCargosEsc: ICargosEsc = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new cargosesc successfully', async () => {
      const responseCargosEsc = { ...mockCargosEsc, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseCargosEsc,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await cargosescApi.addAndUpdate(mockCargosEsc);
    ...
    it('should update existing cargosesc successfully', async () => {
      const existingCargosEsc = { ...mockCargosEsc, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingCargosEsc,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await cargosescApi.addAndUpdate(existingCargosEsc);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(cargosescApi.addAndUpdate(mockCargosEsc)).rejects.toThrow(CargosEscApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEsc',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete cargosesc successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await cargosescApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(cargosescApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'CargosEsc',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(cargosescApi.delete(1)).rejects.toThrow(CargosEscApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterCargosEsc = { funcionario: 1 };
      
      const result = cargosescApi.useFilter(mockFilter);
  ...
  describe('CargosEscApiError', () => {
    it('should create error with correct properties', () => {
      const error = new CargosEscApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('CargosEscApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new CargosEscApiError('Test message', 500);
