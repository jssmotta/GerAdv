...
import { JusticaApiError } from '../GerAdv_TS/Justica/Apis/ApiJustica';
import { JusticaApi } from '@/app/GerAdv_TS/Justica/Apis/ApiJustica';
import { FilterJustica } from '../GerAdv_TS/Justica/Filters/Justica';
...
describe('JusticaApi', () => {
  ...
  beforeEach(() => {
    ...
    justicaApi = new JusticaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(justicaApi).toBeInstanceOf(JusticaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new JusticaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(JusticaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all justica items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await justicaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(justicaApi.getAll()).rejects.toThrow(JusticaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await justicaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Justica_last_getAll_${200}`)));
    });
    ...
    it('should throw JusticaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockJustica = JusticaTestEmpty();
    ...
    it('should fetch justica by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await justicaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockJustica));
      ...
      expect(result.data).toEqual(mockJustica);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Justica',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterJustica = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter justica items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await justicaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new justica successfully', async () => {
      const responseJustica = { ...mockJustica, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseJustica,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await justicaApi.addAndUpdate(mockJustica);
    ...
    it('should update existing justica successfully', async () => {
      const existingJustica = { ...mockJustica, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingJustica,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await justicaApi.addAndUpdate(existingJustica);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(justicaApi.addAndUpdate(mockJustica)).rejects.toThrow(JusticaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Justica',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete justica successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await justicaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(justicaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Justica',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(justicaApi.delete(1)).rejects.toThrow(JusticaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterJustica = { funcionario: 1 };
      
      const result = justicaApi.useFilter(mockFilter);
  ...
  describe('JusticaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new JusticaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('JusticaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new JusticaApiError('Test message', 500);
