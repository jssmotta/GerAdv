...
public class FBensMateriaisFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FBensMateriaisFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FBensMateriaisFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FBensMateriaisFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "bmtCodigo")
        var dt = new DataTable();
        dt.Columns.Add("bmtCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["bmtCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FBensMateriais>();
    }
    ...
    {
        // Arrange
        var factory = new FBensMateriaisFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("bmtCodigo", typeof(int));
        mockDataTable.Columns.Add("bmtNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["bmtCodigo"] = testId;
        row["bmtNome"] = "Test BensMateriais";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["bmtCodigo"] = 123;
        row["bmtNome"] = "Test BensMateriais Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FBensMateriais properties
        }
    }
