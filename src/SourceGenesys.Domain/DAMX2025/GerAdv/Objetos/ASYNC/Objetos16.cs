...
public partial class DBObjetos
{
...
#if (!NOTSTORED_Objetos)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFJustica || pFldFArea || pFldFNome || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFJustica)
            updateTool.Fields(DBObjetosDicInfo.Justica, m_FJustica, ETiposCampos.FNumber);
        if (pFldFArea)
            updateTool.Fields(DBObjetosDicInfo.Area, m_FArea, ETiposCampos.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBObjetosDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFGUID)
            updateTool.Fields(DBObjetosDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBObjetosDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBObjetosDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBObjetosDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBObjetosDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBObjetosDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
