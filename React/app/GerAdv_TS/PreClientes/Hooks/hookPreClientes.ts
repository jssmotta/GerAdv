...
export const usePreClientesForm = (
  ...
  const loadPreClientes = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Pre Clientes";
...
export const usePreClientesList = (
  dataService: IPreClientesService,
  currentFilter?: FilterPreClientes | undefined | null,
) => {
  const [data, setData] = useState<IPreClientes[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterPreClientes | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterPreClientes | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar preclientes";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IPreClientes): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.quemindicou.length > 80) {
        return {
          isValid: false,
          message: "O campo QuemIndicou não pode ter mais de 80 caracteres.",
        };
      }
      ...
      if (data.nomefantasia.length > 80) {
        return {
          isValid: false,
          message: "O campo NomeFantasia não pode ter mais de 80 caracteres.",
        };
      }
      ...
      if (data.homepage.length > 60) {
        return {
          isValid: false,
          message: "O campo HomePage não pode ter mais de 60 caracteres.",
        };
      }
      ...
      if (data.assistido.length > 50) {
        return {
          isValid: false,
          message: "O campo Assistido não pode ter mais de 50 caracteres.",
        };
      }
      if (data.assrg.length > 30) {
        return {
          isValid: false,
          message: "O campo AssRG não pode ter mais de 30 caracteres.",
        };
      }
      if (data.asscpf.length > 11) {
        return {
          isValid: false,
          message: "O campo AssCPF não pode ter mais de 11 caracteres.",
        };
      }
      if (data.assendereco.length > 70) {
        return {
          isValid: false,
          message: "O campo AssEndereco não pode ter mais de 70 caracteres.",
        };
      }
