...
#region ListarDados_Colaboradores
    public static async IAsyncEnumerable<DBColaboradores> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBColaboradores(dbRec);
        }
    }

    public static IEnumerable<DBColaboradores> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
