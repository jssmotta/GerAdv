...
describe('PenhoraApi', () => {
  ...
  beforeEach(() => {
    ...
    penhoraApi = new PenhoraApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(penhoraApi).toBeInstanceOf(PenhoraApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PenhoraApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PenhoraApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all penhora items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await penhoraApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(penhoraApi.getAll()).rejects.toThrow(PenhoraApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await penhoraApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Penhora_last_getAll_${200}`)));
    });
    ...
    it('should throw PenhoraApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPenhora = PenhoraTestEmpty();
    ...
    it('should fetch penhora by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await penhoraApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPenhora));
      ...
      expect(result.data).toEqual(mockPenhora);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Penhora',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter penhora items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await penhoraApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new penhora successfully', async () => {
      const responsePenhora = { ...mockPenhora, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePenhora,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await penhoraApi.addAndUpdate(mockPenhora);
    ...
    it('should update existing penhora successfully', async () => {
      const existingPenhora = { ...mockPenhora, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPenhora,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await penhoraApi.addAndUpdate(existingPenhora);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(penhoraApi.addAndUpdate(mockPenhora)).rejects.toThrow(PenhoraApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Penhora',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete penhora successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await penhoraApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(penhoraApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Penhora',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(penhoraApi.delete(1)).rejects.toThrow(PenhoraApiError);
    });
  });
  ...
  describe('PenhoraApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PenhoraApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PenhoraApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PenhoraApiError('Test message', 500);
