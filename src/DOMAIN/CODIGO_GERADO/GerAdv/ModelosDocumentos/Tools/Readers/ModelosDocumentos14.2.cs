...
public partial class ModelosDocumentosWarmUp
{
...
#pragma warning restore CA1822 // Mark members as static

    {
        ...
        var testSql = $"SELECT SUM(1) AS TOTAL FROM sys.indexes WHERE name = 'idx_ModelosDocumentos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnn?.UseDbo}].[ModelosDocumentos]')";
        ...
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_ModelosDocumentos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[ModelosDocumentos]'))
                BEGIN
                    DROP INDEX [{oCnnRw.UseDbo}].idx_ModelosDocumentos_AuditorDtAtu;
                END
                ", oCnnRw);
        ConfiguracoesDBT.ExecuteSqlCreate($@"IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_ModelosDocumentos_AuditorDtAtu' AND object_id = OBJECT_ID('[{oCnnRw.UseDbo}].[ModelosDocumentos]'))
                BEGIN                    
