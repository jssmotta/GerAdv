...
#region ListarDados_UF
    public static async IAsyncEnumerable<DBUF> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBUF(dbRec);
        }
    }

    public static IEnumerable<DBUF> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
