...
public class FModelosDocumentosFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FModelosDocumentosFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FModelosDocumentosFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FModelosDocumentosFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "mdcCodigo")
        var dt = new DataTable();
        dt.Columns.Add("mdcCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["mdcCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FModelosDocumentos>();
    }
    ...
    {
        // Arrange
        var factory = new FModelosDocumentosFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("mdcCodigo", typeof(int));
        mockDataTable.Columns.Add("mdcNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["mdcCodigo"] = testId;
        row["mdcNome"] = "Test ModelosDocumentos";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["mdcCodigo"] = 123;
        row["mdcNome"] = "Test ModelosDocumentos Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FModelosDocumentos properties
        }
    }
