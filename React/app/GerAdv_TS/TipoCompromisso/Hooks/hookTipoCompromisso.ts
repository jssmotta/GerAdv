...
export const useTipoCompromissoForm = (
  ...
  const loadTipoCompromisso = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Tipo Compromisso";
...
export const useTipoCompromissoList = (
  dataService: ITipoCompromissoService,
  currentFilter?: FilterTipoCompromisso | undefined | null,
) => {
  const [data, setData] = useState<ITipoCompromisso[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterTipoCompromisso | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterTipoCompromisso | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar tipocompromisso";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsTipoCompromisso() {
  ...
  function validate(data: ITipoCompromisso): {
    isValid: boolean;
    message: string;
  } {
