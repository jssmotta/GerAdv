...
export class AgendaFinanceiroApi {
    ...
    public async validation(regAgendaFinanceiro: IAgendaFinanceiro): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAgendaFinanceiro as AgendaFinanceiro, this.getHeaders());
