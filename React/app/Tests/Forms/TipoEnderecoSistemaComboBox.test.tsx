// app/TestsForms/TipoEnderecoSistemaComboBox.test.tsx - Vers�o Corrigida
...
import { TipoEnderecoSistemaEmpty, TipoEnderecoSistemaTestEmpty } from '@/app/GerAdv_TS/Models/TipoEnderecoSistema';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/TipoEnderecoSistema/Hooks/hookTipoEnderecoSistema', () => ({
  useTipoEnderecoSistemaComboBox: jest.fn(() => ({
    options: [
    { ...TipoEnderecoSistemaEmpty(), id: 1, nome: 'Tipo Endereco Sistema 1' },
    { ...TipoEnderecoSistemaEmpty(), id: 2, nome: 'Tipo Endereco Sistema 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/TipoEnderecoSistema/Apis/ApiTipoEnderecoSistema', () => ({
  TipoEnderecoSistemaApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEnderecoSistema' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEnderecoSistema' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/TipoEnderecoSistema/Services/TipoEnderecoSistema.service', () => ({
  TipoEnderecoSistemaService: jest.fn().mockImplementation(() => ({
    fetchTipoEnderecoSistemaById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock TipoEnderecoSistema' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/TipoEnderecoSistema/Crud/Grids/TipoEnderecoSistemaWindow', () => {
  return function MockTipoEnderecoSistemaWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tipoenderecosistema-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Tipo Endereco Sistema' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TipoEnderecoSistemaTestEmpty(), id: 1, nome: 'Tipo Endereco Sistema 1' },
  { ...TipoEnderecoSistemaTestEmpty(), id: 2, nome: 'Tipo Endereco Sistema 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TipoEnderecoSistemaEmpty(), 
};
describe('TipoEnderecoSistemaComboBox', () => {
  ...
  const { useTipoEnderecoSistemaComboBox } = require('@/app/GerAdv_TS/TipoEnderecoSistema/Hooks/hookTipoEnderecoSistema');
  (useTipoEnderecoSistemaComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TipoEnderecoSistemaComboBox {...defaultProps} />);
    // Verificar se o label � renderizado
    expect(screen.getByText('Tipo Endereco Sistema')).toBeInTheDocument();
    // Verificar se o combobox � renderizado
    const combobox = screen.getByRole('combobox');
    expect(combobox).toBeInTheDocument();
    // Verificar se a op��o padr�o est� presente
    expect(screen.getByText('Select Tipo Endereco Sistema')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TipoEnderecoSistemaTestEmpty(), id: 1, nome: 'Tipo Endereco Sistema 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTipoEnderecoSistemaComboBox } = require('@/app/GerAdv_TS/TipoEnderecoSistema/Hooks/hookTipoEnderecoSistema');
    useTipoEnderecoSistemaComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TipoEnderecoSistemaWindow on action button click (add)', async () => {
    render(<TipoEnderecoSistemaComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tipoenderecosistema-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tipoenderecosistema-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TipoEnderecoSistemaComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TipoEnderecoSistemaWindow onClose and onSuccess', () => {
    render(<TipoEnderecoSistemaComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TipoEnderecoSistemaComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
