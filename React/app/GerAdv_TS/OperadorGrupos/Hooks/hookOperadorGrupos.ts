...
export const useOperadorGruposForm = (
  ...
  const loadOperadorGrupos = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Operador Grupos";
...
export const useOperadorGruposList = (
  dataService: IOperadorGruposService,
  currentFilter?: FilterOperadorGrupos | undefined | null,
) => {
  const [data, setData] = useState<IOperadorGrupos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterOperadorGrupos | undefined | null>(
    currentFilter,
  );
    ...
    async (filtro?: FilterOperadorGrupos | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar operadorgrupos";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsOperadorGrupos() {
  ...
  function validate(data: IOperadorGrupos): {
    isValid: boolean;
    message: string;
  } {
