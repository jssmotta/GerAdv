...
#region Testes de Propriedades Computadas
    ...
    {
        _instance.FDataPenhora = DateOnly.Parse(DateTime.Parse(dateString).ToString("dd/MM/yyyy"));
        DateTime.Parse(_instance?.FDataPenhora?.ToString() ?? "").ToString("dd/MM/yyyy").Equals(dateString.Split(' ')[0]).Should().BeTrue();
    }
    ...
    {
        var instance = new DBPenhora();
        Assert.Equal(null !, instance.FDataPenhora);
    }
