...
import { EndTitApiError } from '../GerAdv_TS/EndTit/Apis/ApiEndTit';
import { EndTitApi } from '@/app/GerAdv_TS/EndTit/Apis/ApiEndTit';
import { FilterEndTit } from '../GerAdv_TS/EndTit/Filters/EndTit';
...
describe('EndTitApi', () => {
  ...
  beforeEach(() => {
    ...
    endtitApi = new EndTitApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(endtitApi).toBeInstanceOf(EndTitApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EndTitApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EndTitApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all endtit items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await endtitApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(endtitApi.getAll()).rejects.toThrow(EndTitApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await endtitApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-EndTit_last_getAll_${200}`)));
    });
    ...
    it('should throw EndTitApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEndTit = EndTitTestEmpty();
    ...
    it('should fetch endtit by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await endtitApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEndTit));
      ...
      expect(result.data).toEqual(mockEndTit);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EndTit',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter endtit items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await endtitApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new endtit successfully', async () => {
      const responseEndTit = { ...mockEndTit, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEndTit,
        status: 200,
        statusText: 'OK',
      ...
      const result = await endtitApi.addAndUpdate(mockEndTit);
    ...
    it('should update existing endtit successfully', async () => {
      const existingEndTit = { ...mockEndTit, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEndTit,
        status: 200,
        statusText: 'OK',
      ...
      const result = await endtitApi.addAndUpdate(existingEndTit);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(endtitApi.addAndUpdate(mockEndTit)).rejects.toThrow(EndTitApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EndTit',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete endtit successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await endtitApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(endtitApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EndTit',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(endtitApi.delete(1)).rejects.toThrow(EndTitApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterEndTit = { funcionario: 1 };
      
      const result = endtitApi.useFilter(mockFilter);
  ...
  describe('EndTitApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EndTitApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EndTitApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EndTitApiError('Test message', 500);
