...
public partial class FLivroCaixa : MenphisSI.SG.GerAdv.DBLivroCaixa, IDBLivroCaixa
{
    // DI-compatible default constructor
    public FLivroCaixa() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FLivroCaixa CreateFromReader(SqlDataReader? dbRec)
    {
        return new FLivroCaixa(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FLivroCaixa CreateFromDataRow(DataRow? dbRec)
    {
        return new FLivroCaixa(dbRec);
    }

    private FLivroCaixa(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FLivroCaixa(DataRow? dbRec) : base(dbRec)
    {
    }

    private FLivroCaixa(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FLivroCaixa CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FLivroCaixa(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FLivroCaixa> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fLivroCaixa = new FLivroCaixa();
        await fLivroCaixa.CarregarAsync(id, oCnn);
        return fLivroCaixa;
    }
