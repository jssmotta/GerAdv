...
#region ListarDados_OperadorGrupos
    public static async IAsyncEnumerable<DBOperadorGrupos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBOperadorGrupos(dbRec);
        }
    }

    public static IEnumerable<DBOperadorGrupos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
