...
public partial class FAgendaFinanceiro : MenphisSI.SG.GerAdv.DBAgendaFinanceiro, IDBAgendaFinanceiro
{
    // DI-compatible default constructor
    public FAgendaFinanceiro() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FAgendaFinanceiro CreateFromReader(SqlDataReader? dbRec)
    {
        return new FAgendaFinanceiro(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FAgendaFinanceiro CreateFromDataRow(DataRow? dbRec)
    {
        return new FAgendaFinanceiro(dbRec);
    }

    private FAgendaFinanceiro(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FAgendaFinanceiro(DataRow? dbRec) : base(dbRec)
    {
    }

    public async Task<FAgendaFinanceiro> CreateFromIdAsync(int id, MsiSqlConnection oCnn)
    {
        var fAgendaFinanceiro = new FAgendaFinanceiro();
        await fAgendaFinanceiro.CarregarAsync(id, oCnn);
        return fAgendaFinanceiro;
    }

    private FAgendaFinanceiro(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FAgendaFinanceiro CreateFromParameters(List<SqlParameter> parameters, MsiSqlConnection oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FAgendaFinanceiro(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }
