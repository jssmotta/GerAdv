...
#region TableDefinition_ViaRecebimento
...
#endregion
    public DBViaRecebimento(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBViaRecebimento(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBViaRecebimento(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#region GravarDados_ViaRecebimento
    public int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        {
#if (!shadowsDisabled && !shadows_MenphisSI_SG_GerAdv && !shadows_MenphisSI_SG_GerAdv_ViaRecebimento)
#endif
