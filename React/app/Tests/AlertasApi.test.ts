...
import { AlertasApiError } from '../GerAdv_TS/Alertas/Apis/ApiAlertas';
import { AlertasApi } from '@/app/GerAdv_TS/Alertas/Apis/ApiAlertas';
import { FilterAlertas } from '../GerAdv_TS/Alertas/Filters/Alertas';
...
describe('AlertasApi', () => {
  ...
  beforeEach(() => {
    ...
    alertasApi = new AlertasApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(alertasApi).toBeInstanceOf(AlertasApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AlertasApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AlertasApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all alertas items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alertasApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(alertasApi.getAll()).rejects.toThrow(AlertasApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Alertas_last_getAll_${200}`)));
    });
    ...
    it('should throw AlertasApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAlertas = AlertasTestEmpty();
    ...
    it('should fetch alertas by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await alertasApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAlertas));
      ...
      expect(result.data).toEqual(mockAlertas);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Alertas',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAlertas = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter alertas items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await alertasApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new alertas successfully', async () => {
      const responseAlertas = { ...mockAlertas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAlertas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alertasApi.addAndUpdate(mockAlertas);
    ...
    it('should update existing alertas successfully', async () => {
      const existingAlertas = { ...mockAlertas, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAlertas,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await alertasApi.addAndUpdate(existingAlertas);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(alertasApi.addAndUpdate(mockAlertas)).rejects.toThrow(AlertasApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Alertas',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete alertas successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await alertasApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(alertasApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Alertas',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(alertasApi.delete(1)).rejects.toThrow(AlertasApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAlertas = { funcionario: 1 };
      
      const result = alertasApi.useFilter(mockFilter);
  ...
  describe('AlertasApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AlertasApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AlertasApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AlertasApiError('Test message', 500);
