// app/TestsForms/TribunalComboBox.test.tsx - Vers�o Corrigida
...
import { TribunalEmpty, TribunalTestEmpty } from '@/app/GerAdv_TS/Models/Tribunal';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Tribunal/Hooks/hookTribunal', () => ({
  useTribunalComboBox: jest.fn(() => ({
    options: [
    { ...TribunalEmpty(), id: 1, nome: 'Tribunal 1' },
    { ...TribunalEmpty(), id: 2, nome: 'Tribunal 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Tribunal/Apis/ApiTribunal', () => ({
  TribunalApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Tribunal' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Tribunal' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Tribunal/Services/Tribunal.service', () => ({
  TribunalService: jest.fn().mockImplementation(() => ({
    fetchTribunalById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Tribunal' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Tribunal/Crud/Grids/TribunalWindow', () => {
  return function MockTribunalWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='tribunal-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Tribunal' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...TribunalTestEmpty(), id: 1, nome: 'Tribunal 1' },
  { ...TribunalTestEmpty(), id: 2, nome: 'Tribunal 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: TribunalEmpty(), 
};
describe('TribunalComboBox', () => {
  ...
  const { useTribunalComboBox } = require('@/app/GerAdv_TS/Tribunal/Hooks/hookTribunal');
  (useTribunalComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<TribunalComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Tribunal')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...TribunalTestEmpty(), id: 1, nome: 'Tribunal 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useTribunalComboBox } = require('@/app/GerAdv_TS/Tribunal/Hooks/hookTribunal');
    useTribunalComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens TribunalWindow on action button click (add)', async () => {
    render(<TribunalComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('tribunal-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('tribunal-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<TribunalComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles TribunalWindow onClose and onSuccess', () => {
    render(<TribunalComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<TribunalComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
