...
export const useEnderecosList = (dataService: IEnderecosService, currentFilter?: FilterEnderecos | undefined | null) => {
  const [data, setData] = useState<IEnderecos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterEnderecos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterEnderecos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterEnderecos);
      setData(result);
    } catch (err) {
