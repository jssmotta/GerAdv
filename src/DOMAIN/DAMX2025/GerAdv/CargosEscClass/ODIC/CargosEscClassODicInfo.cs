...
#pragma warning restore CA1822 // Mark members as static
    ...
    public static ImmutableArray<DBInfoSystem> List => [DBCargosEscClassDicInfo.CecNome, DBCargosEscClassDicInfo.CecGUID, DBCargosEscClassDicInfo.CecQuemCad, DBCargosEscClassDicInfo.CecDtCad, DBCargosEscClassDicInfo.CecQuemAtu, DBCargosEscClassDicInfo.CecDtAtu, DBCargosEscClassDicInfo.CecVisto];
    public static ImmutableArray<DBInfoSystem> ListWithoutAuditor => [DBCargosEscClassDicInfo.CecNome, DBCargosEscClassDicInfo.CecGUID];

    public static ImmutableArray<DBInfoSystem> ListPk()
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["cecCodigo"]);
    ...
    {
        ImmutableArray<string> campos = ImmutableArray.CreateRange(["cecCodigo", "cecNome"]);
        var result = campos.Where(campo => !campo.Equals(DBCargosEscClassDicInfo.CampoCodigo)).Select(campo => List.FirstOrDefault(t => t.FNome == campo)).Where(item => item != null).Cast<DBInfoSystem>().Distinct().ToList();
        return result.Count > 0 ? [..result] : ImmutableArray<DBInfoSystem>.Empty;
    }
}
