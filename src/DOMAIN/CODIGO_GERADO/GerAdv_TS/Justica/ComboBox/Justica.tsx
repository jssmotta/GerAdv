...
const JusticaComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IJustica | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IJustica | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IJustica);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <JusticaWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedJustica={editar ?? addValue ?? JusticaEmpty()} /> 
            )}
