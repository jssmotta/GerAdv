...
public class FParceriaProcFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FParceriaProcFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FParceriaProcFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FParceriaProcFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "parCodigo")
        var dt = new DataTable();
        dt.Columns.Add("parCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["parCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FParceriaProc>();
    }
    ...
    {
        // Arrange
        var factory = new FParceriaProcFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("parCodigo", typeof(int));
        mockDataTable.Columns.Add("parGuid", typeof(string));
        var row = mockDataTable.NewRow();
        row["parCodigo"] = testId;
        row["parGuid"] = "Test ParceriaProc";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["parCodigo"] = 123;
        row["parGuid"] = "Test ParceriaProc Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FParceriaProc properties
        }
    }
