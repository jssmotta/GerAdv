...
public partial class DBStatusHTrab : VNoAuditor, ICrud
{
...
#region GravarDados_StatusHTrab
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (pFldFDescricao)
            clsW.Fields(DBStatusHTrabDicInfo.Descricao, FDescricao, EGenericTypeFields.FString);
        if (pFldFResID)
            clsW.Fields(DBStatusHTrabDicInfo.ResID, FResID, EGenericTypeFields.FNumberNull);
        if (insertId != 0)
            return GravaNewId();
