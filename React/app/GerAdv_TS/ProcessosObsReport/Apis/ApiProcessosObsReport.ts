...
export class ProcessosObsReportApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterProcessosObsReport): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-ProcessosObsReport_last_filter_data_${JSON.stringify(filtro)}`);
                try {
    ...
    public useFilter(filtro: FilterProcessosObsReport) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<ProcessosObsReport[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de processosobsreport:', error);
            this.notificationService.notify({
              entity: 'ProcessosObsReport',
              id: 0,
              action: NotifySystemActions.ERROR,
