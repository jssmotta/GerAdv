...
import { PosicaoOutrasPartesApiError } from '../GerAdv_TS/PosicaoOutrasPartes/Apis/ApiPosicaoOutrasPartes';
import { PosicaoOutrasPartesApi } from '@/app/GerAdv_TS/PosicaoOutrasPartes/Apis/ApiPosicaoOutrasPartes';
import { FilterPosicaoOutrasPartes } from '../GerAdv_TS/PosicaoOutrasPartes/Filters/PosicaoOutrasPartes';
...
describe('PosicaoOutrasPartesApi', () => {
  let posicaooutraspartesApi: PosicaoOutrasPartesApi;
  ...
  beforeEach(() => {
    ...
    posicaooutraspartesApi = new PosicaoOutrasPartesApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(posicaooutraspartesApi).toBeInstanceOf(PosicaoOutrasPartesApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new PosicaoOutrasPartesApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(PosicaoOutrasPartesApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [PosicaoOutrasPartesTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all posicaooutraspartes items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await posicaooutraspartesApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(posicaooutraspartesApi.getAll()).rejects.toThrow(PosicaoOutrasPartesApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await posicaooutraspartesApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-PosicaoOutrasPartes_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PosicaoOutrasPartes',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw PosicaoOutrasPartesApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockPosicaoOutrasPartes = PosicaoOutrasPartesTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockPosicaoOutrasPartes,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch posicaooutraspartes by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await posicaooutraspartesApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockPosicaoOutrasPartes));
      ...
      expect(result.data).toEqual(mockPosicaoOutrasPartes);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PosicaoOutrasPartes',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterPosicaoOutrasPartes = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter posicaooutraspartes items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await posicaooutraspartesApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockPosicaoOutrasPartes: IPosicaoOutrasPartes = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new posicaooutraspartes successfully', async () => {
      const responsePosicaoOutrasPartes = { ...mockPosicaoOutrasPartes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responsePosicaoOutrasPartes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await posicaooutraspartesApi.addAndUpdate(mockPosicaoOutrasPartes);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockPosicaoOutrasPartes,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing posicaooutraspartes successfully', async () => {
      const existingPosicaoOutrasPartes = { ...mockPosicaoOutrasPartes, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingPosicaoOutrasPartes,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await posicaooutraspartesApi.addAndUpdate(existingPosicaoOutrasPartes);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(posicaooutraspartesApi.addAndUpdate(mockPosicaoOutrasPartes)).rejects.toThrow(PosicaoOutrasPartesApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PosicaoOutrasPartes',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete posicaooutraspartes successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await posicaooutraspartesApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(posicaooutraspartesApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'PosicaoOutrasPartes',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(posicaooutraspartesApi.delete(1)).rejects.toThrow(PosicaoOutrasPartesApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterPosicaoOutrasPartes = { funcionario: 1 };
      
      const result = posicaooutraspartesApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await posicaooutraspartesApi.delete(1);
  ...
  describe('PosicaoOutrasPartesApiError', () => {
    it('should create error with correct properties', () => {
      const error = new PosicaoOutrasPartesApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('PosicaoOutrasPartesApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new PosicaoOutrasPartesApiError('Test message', 500);
