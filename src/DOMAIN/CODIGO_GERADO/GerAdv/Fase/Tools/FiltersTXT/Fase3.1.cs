...
public static class FilterFaseTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar Fase em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterFase:

- O filter irá filtrar os Fase conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterFase:(Fase)
- descricao: _nome ou descrição para Fase;
...
- Se ouvir "todos os Fase" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterFase? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Fase length: {result.Length}");
#endif
