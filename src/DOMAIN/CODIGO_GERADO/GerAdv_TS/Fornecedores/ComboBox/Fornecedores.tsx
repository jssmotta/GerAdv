...
const FornecedoresComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IFornecedores | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IFornecedores | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IFornecedores);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <FornecedoresWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedFornecedores={editar ?? addValue ?? FornecedoresEmpty()} /> 
            )}
