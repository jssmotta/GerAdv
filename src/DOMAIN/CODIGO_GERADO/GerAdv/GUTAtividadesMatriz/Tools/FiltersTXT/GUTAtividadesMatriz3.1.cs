...
public static class FilterGUTAtividadesMatrizTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar GUTAtividadesMatriz em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGUTAtividadesMatriz:

- O filter irá filtrar os GUTAtividadesMatriz conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterGUTMatriz: filtro específico para G U T Matriz
- filterGUTAtividades: filtro específico para G U T Atividades
...
CAMPOS filterGUTMatriz (G U T Matriz):
- descricao: _nome ou descrição para G U T Matriz;
- guttipo: número ou IDs;
- valor: número ou IDs;

CAMPOS filterGUTAtividades (G U T Atividades):
- nome: _nome ou descrição para G U T Atividades;
- observacao: Observacao;
- gutgrupo: número ou IDs;
- gutperiodicidade: número ou IDs;
- operador: número ou IDs;
- concluido: Concluido;
- dataconcluido: DataConcluido;
- diasparainiciar: número ou IDs;
- minutospararealizar: número ou IDs;
...
CAMPOS filterGUTAtividadesMatriz:(G U T Atividades Matriz)
- gutmatriz: número ou IDs;
- gutatividade: número ou IDs;
Campos de Auditoria (detalhes no PromptAuditor):
- dtcad, dtcad_end: quando o Tipo foi CADASTRADO no sistema
- dtatu, dtatu_end: quando o Tipo foi ALTERADO no sistema
- quemcad, quemcad_end: ID do operador que cadastrou
...
- Se ouvir "todos os GUTAtividadesMatriz" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterGUTAtividadesMatriz? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt GUTAtividadesMatriz length: {result.Length}");
#endif
