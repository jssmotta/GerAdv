...
public partial class ObjetosController(IObjetosService objetosService) : ControllerBase
{
    ...
    {
        _logger.Info("Objetos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _objetosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Objetos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _objetosService.UpdateColumns(parameters, uri);
        if (!result)
        {
