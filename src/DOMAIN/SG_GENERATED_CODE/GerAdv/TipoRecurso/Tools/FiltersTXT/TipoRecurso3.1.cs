...
public static class FilterTipoRecursoTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar TipoRecurso em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterTipoRecurso:

- O filter irá filtrar os TipoRecurso conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os TipoRecurso" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterTipoRecurso? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt TipoRecurso length: {result.Length}");
#endif
