...
public partial class DBProcessOutputRequest
{
    ...
    /// </summary>
    public static async Task<DBProcessOutputRequest> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBProcessOutputRequest();
...
#if (!NOTSTORED_ProcessOutputRequest)
    /// <summary>
...
#if (!NOTSTORED_ProcessOutputRequest)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcessOutputEngine || pFldFOperador || pFldFProcesso || pFldFUltimoIdTabelaExo || pFldFGUID;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcessOutputEngine)
            updateTool.Fields(DBProcessOutputRequestDicInfo.ProcessOutputEngine, m_FProcessOutputEngine, ETiposCampos.FNumber);
        if (pFldFOperador)
            updateTool.Fields(DBProcessOutputRequestDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFProcesso)
            updateTool.Fields(DBProcessOutputRequestDicInfo.Processo, m_FProcesso, ETiposCampos.FNumber);
        if (pFldFUltimoIdTabelaExo)
            updateTool.Fields(DBProcessOutputRequestDicInfo.UltimoIdTabelaExo, m_FUltimoIdTabelaExo, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBProcessOutputRequestDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBProcessOutputRequestDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBProcessOutputRequestDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBProcessOutputRequestDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBProcessOutputRequestDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBProcessOutputRequestDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
