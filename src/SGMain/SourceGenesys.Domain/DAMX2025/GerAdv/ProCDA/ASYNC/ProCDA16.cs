...
public partial class DBProCDA
{
    ...
    /// </summary>
    public static async Task<DBProCDA> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBProCDA();
...
#if (!NOTSTORED_ProCDA)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFProcesso || pFldFNome || pFldFNroInterno || pFldFBold || pFldFGuid;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFProcesso)
            updateTool.Fields(DBProCDADicInfo.Processo, FProcesso, EGenericTypeFields.FNumber);
        if (pFldFNome)
            updateTool.Fields(DBProCDADicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFNroInterno)
            updateTool.Fields(DBProCDADicInfo.NroInterno, FNroInterno, EGenericTypeFields.FString);
        if (pFldFBold || updateTool.Insert)
            updateTool.Fields(DBProCDADicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            updateTool.Fields(DBProCDADicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBProCDADicInfo.QuemCad, AuditorQuem, EGenericTypeFields.FNumber);
        if (isInsert)
            updateTool.Fields(DBProCDADicInfo.DtCad, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        if (!isInsert)
            updateTool.Fields(DBProCDADicInfo.QuemAtu, AuditorQuem, EGenericTypeFields.FNumber);
        if (!isInsert)
            updateTool.Fields(DBProCDADicInfo.DtAtu, DevourerOne.DateTimeUtc, EGenericTypeFields.FDate);
        updateTool.Fields(DBProCDADicInfo.Visto, false, EGenericTypeFields.FBoolean);
        CreateGuid();
        if (isInsert)
            updateTool.Fields(DBProCDADicInfo.Guid, FGuid, EGenericTypeFields.FString);
    }
