...
import RamalWindow from '../Crud/Grids/RamalWindow';
...
import { useRamalComboBox } from '../Hooks/hookRamal';
const RamalComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useRamalComboBox(ramalService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<IRamal | null>(null);
const [addRecord, setAddRecord] = useState<IRamal | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await ramalService.fetchRamalById(id);
...
const handleSuccess = (newRamal?: IRamal) => {
  if (newRamal) {
    setValue({ id: newRamal.id, nome: newRamal.nome });
    loadRecordForEdit(newRamal.id);
    handleValueChange(newRamal);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .ramal-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .ramal-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .ramal-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedRamal={editRecord || addRecord || RamalEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'ramal-dropdown-popup-msi'
          }}
          />
