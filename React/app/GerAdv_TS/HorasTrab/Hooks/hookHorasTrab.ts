...
export const useHorasTrabForm = (
  ...
  const loadHorasTrab = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar Horas Trab";
...
export const useHorasTrabList = (
  dataService: IHorasTrabService,
  currentFilter?: FilterHorasTrab | undefined | null,
) => {
  const [data, setData] = useState<IHorasTrab[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterHorasTrab | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterHorasTrab | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar horastrab";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
  ...
  function validate(data: IHorasTrab): { isValid: boolean; message: string } {
    ...
    try {
      ...
      if (data.hrini.length > 5) {
        return {
          isValid: false,
          message: "O campo HrIni não pode ter mais de 5 caracteres.",
        };
      }
      if (data.hrfim.length > 5) {
        return {
          isValid: false,
          message: "O campo HrFim não pode ter mais de 5 caracteres.",
        };
      }
      ...
      if (data.anexo.length > 255) {
        return {
          isValid: false,
          message: "O campo Anexo não pode ter mais de 255 caracteres.",
        };
      }
      if (data.anexocomp.length > 50) {
        return {
          isValid: false,
          message: "O campo AnexoComp não pode ter mais de 50 caracteres.",
        };
      }
      if (data.anexounc.length > 255) {
        return {
          isValid: false,
          message: "O campo AnexoUNC não pode ter mais de 255 caracteres.",
        };
      }
