...
import { TipoEnderecoApiError } from '../GerAdv_TS/TipoEndereco/Apis/ApiTipoEndereco';
import { TipoEnderecoApi } from '@/app/GerAdv_TS/TipoEndereco/Apis/ApiTipoEndereco';
import { FilterTipoEndereco } from '../GerAdv_TS/TipoEndereco/Filters/TipoEndereco';
...
describe('TipoEnderecoApi', () => {
  let tipoenderecoApi: TipoEnderecoApi;
  ...
  beforeEach(() => {
    ...
    tipoenderecoApi = new TipoEnderecoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(tipoenderecoApi).toBeInstanceOf(TipoEnderecoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new TipoEnderecoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(TipoEnderecoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all tipoendereco items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoenderecoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(tipoenderecoApi.getAll()).rejects.toThrow(TipoEnderecoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await tipoenderecoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-TipoEndereco_last_getAll_${200}`)));
    });
    ...
    it('should throw TipoEnderecoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockTipoEndereco = TipoEnderecoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockTipoEndereco,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch tipoendereco by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await tipoenderecoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockTipoEndereco));
      ...
      expect(result.data).toEqual(mockTipoEndereco);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEndereco',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterTipoEndereco = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter tipoendereco items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await tipoenderecoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockTipoEndereco: ITipoEndereco = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new tipoendereco successfully', async () => {
      const responseTipoEndereco = { ...mockTipoEndereco, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseTipoEndereco,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoenderecoApi.addAndUpdate(mockTipoEndereco);
    ...
    it('should update existing tipoendereco successfully', async () => {
      const existingTipoEndereco = { ...mockTipoEndereco, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingTipoEndereco,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await tipoenderecoApi.addAndUpdate(existingTipoEndereco);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(tipoenderecoApi.addAndUpdate(mockTipoEndereco)).rejects.toThrow(TipoEnderecoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEndereco',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete tipoendereco successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await tipoenderecoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(tipoenderecoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'TipoEndereco',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(tipoenderecoApi.delete(1)).rejects.toThrow(TipoEnderecoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterTipoEndereco = { funcionario: 1 };
      
      const result = tipoenderecoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await tipoenderecoApi.delete(1);
  ...
  describe('TipoEnderecoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new TipoEnderecoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('TipoEnderecoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new TipoEnderecoApiError('Test message', 500);
