...
#region ListarDados_PoderJudiciarioAssociado
    public static async IAsyncEnumerable<DBPoderJudiciarioAssociado> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBPoderJudiciarioAssociado(dbRec);
        }
    }

    public static IEnumerable<DBPoderJudiciarioAssociado> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
