...
import { ProTipoBaixaApiError } from '../GerAdv_TS/ProTipoBaixa/Apis/ApiProTipoBaixa';
import { ProTipoBaixaApi } from '@/app/GerAdv_TS/ProTipoBaixa/Apis/ApiProTipoBaixa';
import { FilterProTipoBaixa } from '../GerAdv_TS/ProTipoBaixa/Filters/ProTipoBaixa';
...
describe('ProTipoBaixaApi', () => {
  let protipobaixaApi: ProTipoBaixaApi;
  ...
  beforeEach(() => {
    ...
    protipobaixaApi = new ProTipoBaixaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(protipobaixaApi).toBeInstanceOf(ProTipoBaixaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new ProTipoBaixaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(ProTipoBaixaApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all protipobaixa items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await protipobaixaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(protipobaixaApi.getAll()).rejects.toThrow(ProTipoBaixaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await protipobaixaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-ProTipoBaixa_last_getAll_${200}`)));
    });
    ...
    it('should throw ProTipoBaixaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockProTipoBaixa = ProTipoBaixaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockProTipoBaixa,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch protipobaixa by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await protipobaixaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockProTipoBaixa));
      ...
      expect(result.data).toEqual(mockProTipoBaixa);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProTipoBaixa',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterProTipoBaixa = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter protipobaixa items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await protipobaixaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockProTipoBaixa: IProTipoBaixa = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new protipobaixa successfully', async () => {
      const responseProTipoBaixa = { ...mockProTipoBaixa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseProTipoBaixa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await protipobaixaApi.addAndUpdate(mockProTipoBaixa);
    ...
    it('should update existing protipobaixa successfully', async () => {
      const existingProTipoBaixa = { ...mockProTipoBaixa, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingProTipoBaixa,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await protipobaixaApi.addAndUpdate(existingProTipoBaixa);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(protipobaixaApi.addAndUpdate(mockProTipoBaixa)).rejects.toThrow(ProTipoBaixaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProTipoBaixa',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete protipobaixa successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await protipobaixaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(protipobaixaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'ProTipoBaixa',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(protipobaixaApi.delete(1)).rejects.toThrow(ProTipoBaixaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterProTipoBaixa = { funcionario: 1 };
      
      const result = protipobaixaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await protipobaixaApi.delete(1);
  ...
  describe('ProTipoBaixaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new ProTipoBaixaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('ProTipoBaixaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new ProTipoBaixaApiError('Test message', 500);
