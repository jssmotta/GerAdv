...
#region ListarDados_Contratos
    public static async IAsyncEnumerable<DBContratos> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBContratos(dbRec);
        }
    }

    public static IEnumerable<DBContratos> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
