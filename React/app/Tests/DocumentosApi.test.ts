...
import { DocumentosApiError } from '../GerAdv_TS/Documentos/Apis/ApiDocumentos';
import { DocumentosApi } from '@/app/GerAdv_TS/Documentos/Apis/ApiDocumentos';
import { FilterDocumentos } from '../GerAdv_TS/Documentos/Filters/Documentos';
...
describe('DocumentosApi', () => {
  let documentosApi: DocumentosApi;
  ...
  beforeEach(() => {
    ...
    documentosApi = new DocumentosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(documentosApi).toBeInstanceOf(DocumentosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new DocumentosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(DocumentosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all documentos items successfully', async () => {
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(documentosApi.getAll()).rejects.toThrow(DocumentosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await documentosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Documentos_last_getAll_${200}`)));
    });
    ...
    it('should throw DocumentosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockDocumentos = DocumentosTestEmpty();
    ...
    it('should fetch documentos by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await documentosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockDocumentos));
  ...
  describe('filter', () => {
    const mockFilter: FilterDocumentos = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter documentos items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await documentosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockDocumentos: IDocumentos = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new documentos successfully', async () => {
      const responseDocumentos = { ...mockDocumentos, id: 1 };
      const mockResponse: AxiosResponse = {
      ...
      const result = await documentosApi.addAndUpdate(mockDocumentos);
    ...
    it('should update existing documentos successfully', async () => {
      const existingDocumentos = { ...mockDocumentos, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingDocumentos,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await documentosApi.addAndUpdate(existingDocumentos);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(documentosApi.addAndUpdate(mockDocumentos)).rejects.toThrow(DocumentosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Documentos',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete documentos successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await documentosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(documentosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Documentos',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(documentosApi.delete(1)).rejects.toThrow(DocumentosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterDocumentos = { funcionario: 1 };
      
      const result = documentosApi.useFilter(mockFilter);
  ...
  describe('DocumentosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new DocumentosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('DocumentosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new DocumentosApiError('Test message', 500);
