...
public partial class DBStatusBiu
{
    ...
    /// </summary>
    public static async Task<DBStatusBiu> CreateAsync(int nCodigo, MsiSqlConnection? oCnn, CancellationToken cancellationToken = default)
    {
        var registro = new DBStatusBiu();
...
#if (!NOTSTORED_StatusBiu)
    // Helper methods
    private bool HasAnyFieldChanged() => pFldFNome || pFldFTipoStatusBiu || pFldFOperador || pFldFIcone;
    private void ConfigureUpdateFields(DBToolWTable32Async updateTool)
    {
        if (pFldFNome)
            updateTool.Fields(DBStatusBiuDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (pFldFTipoStatusBiu)
            updateTool.Fields(DBStatusBiuDicInfo.TipoStatusBiu, m_FTipoStatusBiu, ETiposCampos.FNumber);
        if (pFldFOperador)
            updateTool.Fields(DBStatusBiuDicInfo.Operador, m_FOperador, ETiposCampos.FNumber);
        if (pFldFIcone)
            updateTool.Fields(DBStatusBiuDicInfo.Icone, m_FIcone, ETiposCampos.FNumber);
    }
