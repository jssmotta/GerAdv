...
const ObjetosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IObjetos | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IObjetos | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IObjetos);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <ObjetosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedObjetos={editar ?? addValue ?? ObjetosEmpty()} /> 
            )}
