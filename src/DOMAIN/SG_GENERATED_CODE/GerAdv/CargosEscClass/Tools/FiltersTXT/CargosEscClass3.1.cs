...
public static class FilterCargosEscClassTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar CargosEscClass em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterCargosEscClass:

- O filter irá filtrar os CargosEscClass conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterCargosEscClass:(Cargos Esc Class)
- nome: _nome ou descrição para Cargos Esc Class;
...
- Se ouvir "todos os CargosEscClass" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterCargosEscClass? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt CargosEscClass length: {result.Length}");
#endif
