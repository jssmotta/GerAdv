...
public partial class TribunalController(ITribunalService tribunalService) : ControllerBase
{
    ...
    {
        _logger.Info("Tribunal: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _tribunalService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Tribunal: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _tribunalService.UpdateColumns(parameters, uri);
        if (!result)
        {
