...
public partial class DBFase
{
    ...
    {
        if (pFldFDescricao)
            updateTool.Fields(DBFaseDicInfo.Descricao, m_FDescricao, ETiposCampos.FString);
        if (pFldFJustica)
            updateTool.Fields(DBFaseDicInfo.Justica, m_FJustica, ETiposCampos.FNumber);
        if (pFldFArea)
            updateTool.Fields(DBFaseDicInfo.Area, m_FArea, ETiposCampos.FNumber);
        if (pFldFGUID)
            updateTool.Fields(DBFaseDicInfo.GUID, m_FGUID, ETiposCampos.FString);
    }
    ...
    private void ConfigureAuditorFields(DBToolWTable32Async updateTool)
    {
        ...
        if (m_AuditorQuem == 0)
            AuditorQuem = 1;
        if (isInsert)
            updateTool.Fields(DBFaseDicInfo.QuemCad, AuditorQuem, ETiposCampos.FNumber);
        if (isInsert)
            updateTool.Fields(DBFaseDicInfo.DtCad, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        if (!isInsert)
            updateTool.Fields(DBFaseDicInfo.QuemAtu, AuditorQuem, ETiposCampos.FNumber);
        if (!isInsert)
            updateTool.Fields(DBFaseDicInfo.DtAtu, DevourerOne.DateTimeUtc, ETiposCampos.FDate);
        updateTool.Fields(DBFaseDicInfo.Visto, false, ETiposCampos.FBoolean);
    }
