...
public partial class TribEnderecosService(IOptions<AppSettings> appSettings, ITribEnderecosReader reader, ITribEnderecosValidation validation, ITribEnderecosWriter writer, ITribunalReader tribunalReader, HybridCache cache) : ITribEnderecosService, IDisposable
{
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("TribEnderecos: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBTribEnderecos(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBTribEnderecosDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("TribEnderecos: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBTribEnderecos(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBTribEnderecosDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
