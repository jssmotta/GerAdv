...
public class FFornecedoresFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FFornecedoresFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FFornecedoresFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FFornecedoresFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "forCodigo")
        var dt = new DataTable();
        dt.Columns.Add("forCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["forCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FFornecedores>();
    }
    ...
    {
        // Arrange
        var factory = new FFornecedoresFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("forCodigo", typeof(int));
        mockDataTable.Columns.Add("forNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["forCodigo"] = testId;
        row["forNome"] = "Test Fornecedores";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["forCodigo"] = 123;
        row["forNome"] = "Test Fornecedores Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FFornecedores properties
        }
    }
