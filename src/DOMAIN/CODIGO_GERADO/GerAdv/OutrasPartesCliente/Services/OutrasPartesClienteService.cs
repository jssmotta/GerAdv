...
public partial class OutrasPartesClienteService(IOptions<AppSettings> appSettings, IOutrasPartesClienteReader reader, IOutrasPartesClienteValidation validation, IOutrasPartesClienteWriter writer, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IOutrasPartesClienteService, IDisposable
{
    ...
    public async Task<OutrasPartesClienteResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        try
        {
            var result = await _cache.GetOrCreateAsync($"{uri}-OutrasPartesCliente-GetById-{id}", async cancel => await GetDataByIdAsync(id, uri, cancel), entryOptions, cancellationToken: token);
            return result;
        }
        catch (Exception ex)
        {
    ...
    {
        ThrowIfDisposed();
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("OutrasPartesCliente: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBOutrasPartesCliente(parameters?.Id ?? throw new Exception("Id == null"), oCnn);
            var campos = new List<ColumnValueItem>();
            foreach (var column in parameters?.Columns!)
                if (column != null && column.Length > 0)
                {
                    var value = dbRec.GetValueByNameField($"{DBOutrasPartesClienteDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}");
                    if (value != null)
    ...
    public async Task<bool> UpdateColumns([FromBody] UpdateColumnsRequest parameters, [FromRoute, Required] string uri)
    {
        ...
        return !Uris.ValidaUri(uri, _uris) ? throw new Exception("OutrasPartesCliente: URI inválida") : await Task.Run(() =>
        {
            ...
            using var dbRec = new DBOutrasPartesCliente(parameters?.Id ?? throw new Exception("Id is null"), oCnn);
            foreach (var(column, value)in parameters?.Columns!)
                dbRec.SetValueByNameField($"{DBOutrasPartesClienteDicInfo.TablePrefix}{char.ToUpper(column[0])}{column[1..]}", value);
