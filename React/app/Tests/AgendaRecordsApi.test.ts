...
import { AgendaRecordsApiError } from '../GerAdv_TS/AgendaRecords/Apis/ApiAgendaRecords';
import { AgendaRecordsApi } from '@/app/GerAdv_TS/AgendaRecords/Apis/ApiAgendaRecords';
import { FilterAgendaRecords } from '../GerAdv_TS/AgendaRecords/Filters/AgendaRecords';
...
describe('AgendaRecordsApi', () => {
  let agendarecordsApi: AgendaRecordsApi;
  ...
  beforeEach(() => {
    ...
    agendarecordsApi = new AgendaRecordsApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(agendarecordsApi).toBeInstanceOf(AgendaRecordsApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AgendaRecordsApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AgendaRecordsApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all agendarecords items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarecordsApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(agendarecordsApi.getAll()).rejects.toThrow(AgendaRecordsApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await agendarecordsApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-AgendaRecords_last_getAll_${200}`)));
    });
    ...
    it('should decode and return offline data if present in localStorage', async () => {
      ...
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRecords',
          action: NotifySystemActions.INFO,
        })
      );
    });

    it('should throw AgendaRecordsApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockAgendaRecords = AgendaRecordsTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockAgendaRecords,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch agendarecords by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await agendarecordsApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAgendaRecords));
      ...
      expect(result.data).toEqual(mockAgendaRecords);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRecords',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterAgendaRecords = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter agendarecords items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await agendarecordsApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockAgendaRecords: IAgendaRecords = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new agendarecords successfully', async () => {
      const responseAgendaRecords = { ...mockAgendaRecords, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAgendaRecords,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarecordsApi.addAndUpdate(mockAgendaRecords);
    ...
    it('should update existing agendarecords successfully', async () => {
      const existingAgendaRecords = { ...mockAgendaRecords, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAgendaRecords,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await agendarecordsApi.addAndUpdate(existingAgendaRecords);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(agendarecordsApi.addAndUpdate(mockAgendaRecords)).rejects.toThrow(AgendaRecordsApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRecords',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete agendarecords successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await agendarecordsApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(agendarecordsApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'AgendaRecords',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(agendarecordsApi.delete(1)).rejects.toThrow(AgendaRecordsApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAgendaRecords = { funcionario: 1 };
      
      const result = agendarecordsApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await agendarecordsApi.delete(1);
  ...
  describe('AgendaRecordsApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AgendaRecordsApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AgendaRecordsApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AgendaRecordsApiError('Test message', 500);
