...
import { FuncionariosApiError } from '../GerAdv_TS/Funcionarios/Apis/ApiFuncionarios';
import { FuncionariosApi } from '@/app/GerAdv_TS/Funcionarios/Apis/ApiFuncionarios';
import { FilterFuncionarios } from '../GerAdv_TS/Funcionarios/Filters/Funcionarios';
...
describe('FuncionariosApi', () => {
  let funcionariosApi: FuncionariosApi;
  ...
  beforeEach(() => {
    ...
    funcionariosApi = new FuncionariosApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(funcionariosApi).toBeInstanceOf(FuncionariosApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new FuncionariosApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(FuncionariosApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all funcionarios items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await funcionariosApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(funcionariosApi.getAll()).rejects.toThrow(FuncionariosApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await funcionariosApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Funcionarios_last_getAll_${200}`)));
    });
    ...
    it('should throw FuncionariosApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockFuncionarios = FuncionariosTestEmpty();
    ...
    it('should fetch funcionarios by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await funcionariosApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockFuncionarios));
      ...
      expect(result.data).toEqual(mockFuncionarios);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcionarios',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterFuncionarios = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter funcionarios items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await funcionariosApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockFuncionarios: IFuncionarios = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new funcionarios successfully', async () => {
      const responseFuncionarios = { ...mockFuncionarios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseFuncionarios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await funcionariosApi.addAndUpdate(mockFuncionarios);
    ...
    it('should update existing funcionarios successfully', async () => {
      const existingFuncionarios = { ...mockFuncionarios, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingFuncionarios,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await funcionariosApi.addAndUpdate(existingFuncionarios);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(funcionariosApi.addAndUpdate(mockFuncionarios)).rejects.toThrow(FuncionariosApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcionarios',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete funcionarios successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await funcionariosApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(funcionariosApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Funcionarios',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(funcionariosApi.delete(1)).rejects.toThrow(FuncionariosApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterFuncionarios = { funcionario: 1 };
      
      const result = funcionariosApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await funcionariosApi.delete(1);
  ...
  describe('FuncionariosApiError', () => {
    it('should create error with correct properties', () => {
      const error = new FuncionariosApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('FuncionariosApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new FuncionariosApiError('Test message', 500);
