...
import { GUTMatrizApiError } from '../GerAdv_TS/GUTMatriz/Apis/ApiGUTMatriz';
import { GUTMatrizApi } from '@/app/GerAdv_TS/GUTMatriz/Apis/ApiGUTMatriz';
import { FilterGUTMatriz } from '../GerAdv_TS/GUTMatriz/Filters/GUTMatriz';
...
describe('GUTMatrizApi', () => {
  let gutmatrizApi: GUTMatrizApi;
  ...
  beforeEach(() => {
    ...
    gutmatrizApi = new GUTMatrizApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(gutmatrizApi).toBeInstanceOf(GUTMatrizApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new GUTMatrizApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(GUTMatrizApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all gutmatriz items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutmatrizApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(gutmatrizApi.getAll()).rejects.toThrow(GUTMatrizApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await gutmatrizApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-GUTMatriz_last_getAll_${200}`)));
    });
    ...
    it('should throw GUTMatrizApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockGUTMatriz = GUTMatrizTestEmpty();
    ...
    it('should fetch gutmatriz by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await gutmatrizApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockGUTMatriz));
      ...
      expect(result.data).toEqual(mockGUTMatriz);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTMatriz',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterGUTMatriz = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter gutmatriz items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await gutmatrizApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockGUTMatriz: IGUTMatriz = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new gutmatriz successfully', async () => {
      const responseGUTMatriz = { ...mockGUTMatriz, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseGUTMatriz,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutmatrizApi.addAndUpdate(mockGUTMatriz);
    ...
    it('should update existing gutmatriz successfully', async () => {
      const existingGUTMatriz = { ...mockGUTMatriz, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingGUTMatriz,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await gutmatrizApi.addAndUpdate(existingGUTMatriz);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(gutmatrizApi.addAndUpdate(mockGUTMatriz)).rejects.toThrow(GUTMatrizApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTMatriz',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete gutmatriz successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await gutmatrizApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(gutmatrizApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'GUTMatriz',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(gutmatrizApi.delete(1)).rejects.toThrow(GUTMatrizApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterGUTMatriz = { funcionario: 1 };
      
      const result = gutmatrizApi.useFilter(mockFilter);
  ...
  describe('GUTMatrizApiError', () => {
    it('should create error with correct properties', () => {
      const error = new GUTMatrizApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('GUTMatrizApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new GUTMatrizApiError('Test message', 500);
