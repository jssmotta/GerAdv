...
public static class FilterGUTAtividadesTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar GUTAtividades em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterGUTAtividades:

- O filter irá filtrar os GUTAtividades conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterGUTPeriodicidade: filtro específico para G U T Periodicidade
...
CAMPOS filterGUTPeriodicidade (G U T Periodicidade):
- nome: _nome ou descrição para G U T Periodicidade;
- intervalodias: número ou IDs;
...
- Se ouvir "todos os GUTAtividades" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterGUTAtividades? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt GUTAtividades length: {result.Length}");
#endif
