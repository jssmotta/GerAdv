...
export const useProTipoBaixaGrid = ({
  ...
  const handleColumnsReorder = useCallback((event: any) => {
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('protipobaixa-columns-order'), JSON.stringify(reorderableFields));
  ...
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('protipobaixa-columns-order'));
      if (savedOrder) {
