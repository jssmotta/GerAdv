...
import { FaseApiError } from '../GerAdv_TS/Fase/Apis/ApiFase';
import { FaseApi } from '@/app/GerAdv_TS/Fase/Apis/ApiFase';
import { FilterFase } from '../GerAdv_TS/Fase/Filters/Fase';
...
describe('FaseApi', () => {
  ...
  beforeEach(() => {
    ...
    faseApi = new FaseApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(faseApi).toBeInstanceOf(FaseApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new FaseApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(FaseApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all fase items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await faseApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(faseApi.getAll()).rejects.toThrow(FaseApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await faseApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Fase_last_getAll_${200}`)));
    });
    ...
    it('should throw FaseApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    ...
    it('should fetch fase by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await faseApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockFase));
      ...
      expect(result.data).toEqual(mockFase);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fase',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter fase items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await faseApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new fase successfully', async () => {
      const responseFase = { ...mockFase, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseFase,
        status: 200,
        statusText: 'OK',
      ...
      const result = await faseApi.addAndUpdate(mockFase);
    ...
    it('should update existing fase successfully', async () => {
      const existingFase = { ...mockFase, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingFase,
        status: 200,
        statusText: 'OK',
      ...
      const result = await faseApi.addAndUpdate(existingFase);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(faseApi.addAndUpdate(mockFase)).rejects.toThrow(FaseApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fase',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete fase successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await faseApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(faseApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Fase',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(faseApi.delete(1)).rejects.toThrow(FaseApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterFase = { funcionario: 1 };
      
      const result = faseApi.useFilter(mockFilter);
  ...
  describe('FaseApiError', () => {
    it('should create error with correct properties', () => {
      const error = new FaseApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('FaseApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new FaseApiError('Test message', 500);
