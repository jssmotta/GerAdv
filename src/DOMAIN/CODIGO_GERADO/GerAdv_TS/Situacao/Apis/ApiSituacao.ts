...
export class SituacaoApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
...
export class SituacaoApi {
    ...
    constructor(uri: string, authorization: string, version: number = parseInt(process.env.NEXT_PUBLIC_URL_VERSION_API ?? '1')) {
        ...
        this.baseUrl = `${process.env.NEXT_PUBLIC_URL_API_BASE}${version}/${uri}/Situacao`;
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Situacao_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os Situacao');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Situacao_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            public async filter(filtro: FilterSituacao): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-Situacao_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar Situacao');
                }
            }


    public async validation(regSituacao: ISituacao): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regSituacao as Situacao, this.getHeaders());
    ...
    public async addAndUpdate(regSituacao: ISituacao): Promise<AxiosResponse> {
        try {
            var result = await axios.post(`${this.baseUrl}/AddAndUpdate`, regSituacao as Situacao, this.getHeaders());
            var register = result.data as ISituacao;        
            const action = regSituacao.id == 0 ? NotifySystemActions.ADD : NotifySystemActions.UPDATE;
        ...
        try {
            ...
            if (error.response && error.response.status === 409) {
              if (error.response && error.response.data) {
                    ...
                    const errorMessage = message|| 'Erro ao excluir o Situacao. Verifique se ele não está vinculado a outros registros.';
                    this.notificationService.notify({
                        entity: 'Situacao',
                        id: id,
                        action: NotifySystemActions.ERROR,
    ...
    public useFilter(filtro: FilterSituacao) {
       const url = `${this.baseUrl}/Filter`;
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
    
        return useSWR<Situacao[]>(key, fetcher, {
          revalidateOnFocus: false,
          revalidateOnReconnect: false,
          onError: (error) => {
            console.error('Erro no SWR para filtro de situacao:', error);
            this.notificationService.notify({
              entity: 'Situacao',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos situacao'
            });
          }
        });
    }
