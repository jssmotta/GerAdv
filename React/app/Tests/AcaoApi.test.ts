...
import { AcaoApiError } from '../GerAdv_TS/Acao/Apis/ApiAcao';
import { AcaoApi } from '@/app/GerAdv_TS/Acao/Apis/ApiAcao';
import { FilterAcao } from '../GerAdv_TS/Acao/Filters/Acao';
...
describe('AcaoApi', () => {
  ...
  beforeEach(() => {
    ...
    acaoApi = new AcaoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(acaoApi).toBeInstanceOf(AcaoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new AcaoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(AcaoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all acao items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await acaoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(acaoApi.getAll()).rejects.toThrow(AcaoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await acaoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-Acao_last_getAll_${200}`)));
    });
    ...
    it('should throw AcaoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    ...
    it('should fetch acao by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await acaoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockAcao));
      ...
      expect(result.data).toEqual(mockAcao);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Acao',
          id: 1,
  ...
  describe('filter', () => {
    ...
    it('should filter acao items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await acaoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    ...
    it('should add new acao successfully', async () => {
      const responseAcao = { ...mockAcao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseAcao,
        status: 200,
        statusText: 'OK',
      ...
      const result = await acaoApi.addAndUpdate(mockAcao);
    ...
    it('should update existing acao successfully', async () => {
      const existingAcao = { ...mockAcao, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingAcao,
        status: 200,
        statusText: 'OK',
      ...
      const result = await acaoApi.addAndUpdate(existingAcao);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(acaoApi.addAndUpdate(mockAcao)).rejects.toThrow(AcaoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Acao',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete acao successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await acaoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(acaoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'Acao',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(acaoApi.delete(1)).rejects.toThrow(AcaoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterAcao = { funcionario: 1 };
      
      const result = acaoApi.useFilter(mockFilter);
  ...
  describe('AcaoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new AcaoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('AcaoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new AcaoApiError('Test message', 500);
