...
public partial class DBTiposAcao : VAuditor, ICrud
{
...
#region GravarDados_TiposAcao
    public virtual int Update(MsiSqlConnection? oCnn, int insertId = 0)
    {
        ...
        if (!isInsert)
            if (!(pFldFNome || pFldFInativo || pFldFBold || pFldFGuid))
                return 0;
        if (oCnn is null)
#if (DEBUG)
			        {
        ...
        if (pFldFNome)
            clsW.Fields(DBTiposAcaoDicInfo.Nome, FNome, EGenericTypeFields.FString);
        if (pFldFInativo || ID.IsEmptyIDNumber())
            clsW.Fields(DBTiposAcaoDicInfo.Inativo, FInativo, EGenericTypeFields.FBoolean);
        if (pFldFBold || ID.IsEmptyIDNumber())
            clsW.Fields(DBTiposAcaoDicInfo.Bold, FBold, EGenericTypeFields.FBoolean);
        if (pFldFGuid)
            clsW.Fields(DBTiposAcaoDicInfo.Guid, FGuid, EGenericTypeFields.FString);
        ...
        if (AuditorQuem == 0)
            AuditorQuem = 1;
        if (pFldFQuemCad)
            clsW.Fields(DBTiposAcaoDicInfo.QuemCad, FQuemCad, EGenericTypeFields.FNumberNull);
        if (pFldFDtCad)
            clsW.Fields(DBTiposAcaoDicInfo.DtCad, FDtCad, EGenericTypeFields.FDate);
        if (pFldFQuemAtu)
            clsW.Fields(DBTiposAcaoDicInfo.QuemAtu, FQuemAtu, EGenericTypeFields.FNumberNull);
        if (pFldFDtAtu)
            clsW.Fields(DBTiposAcaoDicInfo.DtAtu, FDtAtu, EGenericTypeFields.FDate);
        if (pFldFVisto || ID.IsEmptyIDNumber())
            clsW.Fields(DBTiposAcaoDicInfo.Visto, FVisto, EGenericTypeFields.FBoolean);
        if (insertId != 0)
            return GravaNewId();
