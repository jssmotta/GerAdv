...
#region ListarDados_ParteClienteOutras
    public static async IAsyncEnumerable<DBParteClienteOutras> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBParteClienteOutras(dbRec);
        }
    }

    public static IEnumerable<DBParteClienteOutras> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
