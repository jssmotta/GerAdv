...
public static class FilterDivisaoTribunalTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterDivisaoTribunal? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar DivisaoTribunal em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterDivisaoTribunal:

- O filter irá filtrar os DivisaoTribunal conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- filterTribunal: filtro específico para Tribunal
...
CAMPOS filterTribunal (Tribunal):
- nome: nome ou descrição para Tribunal;
- area: número ou IDs;
- justica: número ou IDs;
- descricao: triDescricao;
...
CAMPOS filterDivisaoTribunal:(Divisao Tribunal)
- numcodigo: número ou IDs;
- justica: número ou IDs;
- nomeespecial: NomeEspecial;
- area: número ou IDs;
- cidade: número ou IDs;
- foro: número ou IDs;
- tribunal: número ou IDs;
- codigodiv: CodigoDiv;
- endereco: Endereco;
- fone: Fone;
- fax: Fax;
...
- Se ouvir "todos os DivisaoTribunal" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt DivisaoTribunal length: {result.Length}");
#endif
