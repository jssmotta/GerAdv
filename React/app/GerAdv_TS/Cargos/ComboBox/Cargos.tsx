...
import { useCargosComboBox } from '../Hooks/hookCargos';
const CargosComboBox: React.FC<DadosSelectProps> = ({
  name, 
  value, 
  setValue, 
  label, 
...
const {
  ...
  handleValueChange, 
  clearValue
} = useCargosComboBox(cargosService);

const [isOpen, setIsOpen] = useState(false);
const [editRecord, setEditRecord] = useState<ICargos | null>(null);
const [addRecord, setAddRecord] = useState<ICargos | null>(null);
...
const loadRecordForEdit = async (id: number) => {
  try {
    const record = await cargosService.fetchCargosById(id);
...
const handleSuccess = (newCargos?: ICargos) => {
  if (newCargos) {
    setValue({ id: newCargos.id, nome: newCargos.nome });
    loadRecordForEdit(newCargos.id);
    handleValueChange(newCargos);
  }
  handleClose();
};
return (
...
<style jsx global>{`
  ...
  .cargos-dropdown-popup-msi .k-list {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
    background: white !important;
  ...
  .cargos-dropdown-popup-msi .k-list-item {
    display: block !important;
    visibility: visible !important;
    opacity: 1 !important;
  ...
  .cargos-dropdown-popup-msi .k-list-item: hover {
  ...
  {(editRecord || addRecord) && isOpen && (
    ...
    onError={handleClose}
    selectedCargos={editRecord || addRecord || CargosEmpty()}
    />
    )}
    ...
    <div className={`${cssDado} inputCombobox input-container`}>
      <div className='comboboxLabel'>
        ...
        <div className='comboboxBox'>
          ...
          suggest={true}
          popupSettings={{
            className: 'cargos-dropdown-popup-msi'
          }}
          />
