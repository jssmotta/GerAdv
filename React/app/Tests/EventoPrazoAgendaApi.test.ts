...
import { EventoPrazoAgendaApiError } from '../GerAdv_TS/EventoPrazoAgenda/Apis/ApiEventoPrazoAgenda';
import { EventoPrazoAgendaApi } from '@/app/GerAdv_TS/EventoPrazoAgenda/Apis/ApiEventoPrazoAgenda';
import { FilterEventoPrazoAgenda } from '../GerAdv_TS/EventoPrazoAgenda/Filters/EventoPrazoAgenda';
...
describe('EventoPrazoAgendaApi', () => {
  let eventoprazoagendaApi: EventoPrazoAgendaApi;
  ...
  beforeEach(() => {
    ...
    eventoprazoagendaApi = new EventoPrazoAgendaApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(eventoprazoagendaApi).toBeInstanceOf(EventoPrazoAgendaApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new EventoPrazoAgendaApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(EventoPrazoAgendaApi);
    });
  });
  ...
  describe('getAll', () => {
    const mockResponse: AxiosResponse = {
      data: [EventoPrazoAgendaTestEmpty()],
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch all eventoprazoagenda items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await eventoprazoagendaApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(eventoprazoagendaApi.getAll()).rejects.toThrow(EventoPrazoAgendaApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await eventoprazoagendaApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-EventoPrazoAgenda_last_getAll_${200}`)));
    });
    ...
    it('should throw EventoPrazoAgendaApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockEventoPrazoAgenda = EventoPrazoAgendaTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockEventoPrazoAgenda,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch eventoprazoagenda by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await eventoprazoagendaApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockEventoPrazoAgenda));
      ...
      expect(result.data).toEqual(mockEventoPrazoAgenda);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EventoPrazoAgenda',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterEventoPrazoAgenda = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter eventoprazoagenda items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await eventoprazoagendaApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockEventoPrazoAgenda: IEventoPrazoAgenda = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new eventoprazoagenda successfully', async () => {
      const responseEventoPrazoAgenda = { ...mockEventoPrazoAgenda, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseEventoPrazoAgenda,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await eventoprazoagendaApi.addAndUpdate(mockEventoPrazoAgenda);

      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining('/AddAndUpdate'),
        mockEventoPrazoAgenda,
        expect.any(Object)
      );
      expect(result).toEqual(mockResponse);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
    ...
    it('should update existing eventoprazoagenda successfully', async () => {
      const existingEventoPrazoAgenda = { ...mockEventoPrazoAgenda, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingEventoPrazoAgenda,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await eventoprazoagendaApi.addAndUpdate(existingEventoPrazoAgenda);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(eventoprazoagendaApi.addAndUpdate(mockEventoPrazoAgenda)).rejects.toThrow(EventoPrazoAgendaApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EventoPrazoAgenda',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete eventoprazoagenda successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await eventoprazoagendaApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(eventoprazoagendaApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'EventoPrazoAgenda',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(eventoprazoagendaApi.delete(1)).rejects.toThrow(EventoPrazoAgendaApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterEventoPrazoAgenda = { funcionario: 1 };
      
      const result = eventoprazoagendaApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await eventoprazoagendaApi.delete(1);
  ...
  describe('EventoPrazoAgendaApiError', () => {
    it('should create error with correct properties', () => {
      const error = new EventoPrazoAgendaApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('EventoPrazoAgendaApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new EventoPrazoAgendaApiError('Test message', 500);
