...
public partial class PenhoraStatusController(IPenhoraStatusService penhorastatusService) : ControllerBase
{
    ...
    {
        _logger.Info("PenhoraStatus: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _penhorastatusService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("PenhoraStatus: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _penhorastatusService.UpdateColumns(parameters, uri);
        if (!result)
        {
