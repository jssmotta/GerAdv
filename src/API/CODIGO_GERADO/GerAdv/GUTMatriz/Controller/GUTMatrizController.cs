...
public partial class GUTMatrizController(IGUTMatrizService gutmatrizService) : ControllerBase
{
    ...
    {
        _logger.Info("GUTMatriz: GetByName called with name = {0}, {1}", name, uri);
        var result = await _gutmatrizService.GetByName(name, uri);
        if (result == null)
        {
            _logger.Warn("GetByName: No GUTMatriz found with name = {0}, {1}", name, uri);
            return NotFound();
        }
    ...
    {
        _logger.Info("GUTMatriz: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _gutmatrizService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("GUTMatriz: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _gutmatrizService.UpdateColumns(parameters, uri);
        if (!result)
        {
