...
#region ListarDados_BensClassificacao
    public static async IAsyncEnumerable<DBBensClassificacao> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBBensClassificacao(dbRec);
        }
    }

    public static IEnumerable<DBBensClassificacao> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
        {
