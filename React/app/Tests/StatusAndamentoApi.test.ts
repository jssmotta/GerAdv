...
import { StatusAndamentoApiError } from '../GerAdv_TS/StatusAndamento/Apis/ApiStatusAndamento';
import { StatusAndamentoApi } from '@/app/GerAdv_TS/StatusAndamento/Apis/ApiStatusAndamento';
import { FilterStatusAndamento } from '../GerAdv_TS/StatusAndamento/Filters/StatusAndamento';
...
describe('StatusAndamentoApi', () => {
  let statusandamentoApi: StatusAndamentoApi;
  ...
  beforeEach(() => {
    ...
    statusandamentoApi = new StatusAndamentoApi(mockUri, mockToken);
  });

  describe('Constructor', () => {
    it('should initialize with correct baseUrl and authorization', () => {
      expect(statusandamentoApi).toBeInstanceOf(StatusAndamentoApi);
    ...
    it('should use custom version when provided', () => {
      ...
      const customApi = new StatusAndamentoApi(mockUri, mockToken, customVersion);
      expect(customApi).toBeInstanceOf(StatusAndamentoApi);
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('should fetch all statusandamento items successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusandamentoApi.getAll();
    ...
    it('should return null when localStorage has no offline data', async () => {
      ...
      await expect(statusandamentoApi.getAll()).rejects.toThrow(StatusAndamentoApiError);
    ...
    it('should call localStorage.getItem with correct key on failure', async () => {
      ...
      await statusandamentoApi.getAll();

      expect(global.localStorage.getItem).toHaveBeenCalledWith(expect.stringContaining(btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${mockUri}-StatusAndamento_last_getAll_${200}`)));
    });
    ...
    it('should throw StatusAndamentoApiError when request fails and no offline data', async () => {
  ...
  describe('getById', () => {
    const mockStatusAndamento = StatusAndamentoTestEmpty();

    const mockResponse: AxiosResponse = {
      data: mockStatusAndamento,
      status: 200,
      statusText: 'OK',
      headers: {},
      config: {},
    ...
    it('should fetch statusandamento by id successfully', async () => {
      mockedAxios.get.mockResolvedValue(mockResponse);

      const result = await statusandamentoApi.getById(1);
    ...
    it('should use offline data when request fails', async () => {
      const encodedData = btoa(JSON.stringify(mockStatusAndamento));
      ...
      expect(result.data).toEqual(mockStatusAndamento);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusAndamento',
          id: 1,
  ...
  describe('filter', () => {
    const mockFilter: FilterStatusAndamento = {
      funcionario: 1,
      data: '2024-01-01',
    };
    ...
    it('should filter statusandamento items successfully', async () => {
      mockedAxios.post.mockResolvedValue(mockResponse);

      const result = await statusandamentoApi.filter(mockFilter);
  ...
  describe('addAndUpdate', () => {
    const mockStatusAndamento: IStatusAndamento = {
      id: 0,
      funcionario: 1,
      tipocompromisso: 1,
      fornecedor: 1,
    ...
    it('should add new statusandamento successfully', async () => {
      const responseStatusAndamento = { ...mockStatusAndamento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: responseStatusAndamento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusandamentoApi.addAndUpdate(mockStatusAndamento);
    ...
    it('should update existing statusandamento successfully', async () => {
      const existingStatusAndamento = { ...mockStatusAndamento, id: 1 };
      const mockResponse: AxiosResponse = {
        data: existingStatusAndamento,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {},
      ...
      const result = await statusandamentoApi.addAndUpdate(existingStatusAndamento);
    ...
    it('should handle conflict error (409)', async () => {
      ...
      await expect(statusandamentoApi.addAndUpdate(mockStatusAndamento)).rejects.toThrow(StatusAndamentoApiError);
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusAndamento',
          action: NotifySystemActions.ERROR,
  ...
  describe('delete', () => {
    it('should delete statusandamento successfully', async () => {
      const mockResponse: AxiosResponse = {
        data: { success: true },
        status: 200,
        statusText: 'OK',
      ...
      const result = await statusandamentoApi.delete(1);
    ...
    it('should handle conflict error on delete (409)', async () => {
      ...
      await expect(statusandamentoApi.delete(1)).rejects.toThrow();
      expect(mockNotificationService.notify).toHaveBeenCalledWith(
        expect.objectContaining({
          entity: 'StatusAndamento',
          id: 1,
    ...
    it('should handle general delete error', async () => {
      ...
      await expect(statusandamentoApi.delete(1)).rejects.toThrow(StatusAndamentoApiError);
    });
  });

  describe('useFilter', () => {
    it('should create SWR hook with correct parameters', () => {
      const mockFilter: FilterStatusAndamento = { funcionario: 1 };
      
      const result = statusandamentoApi.useFilter(mockFilter);
  ...
  describe('Error Handling', () => {
    it('should create proper notification entity for delete', async () => {
      ...
      await statusandamentoApi.delete(1);
  ...
  describe('StatusAndamentoApiError', () => {
    it('should create error with correct properties', () => {
      const error = new StatusAndamentoApiError('Test message', 400, 'TEST_CODE', 'original');

      expect(error.message).toBe('Test message');
      expect(error.status).toBe(400);
      expect(error.code).toBe('TEST_CODE');
      expect(error.originalError).toBe('original');
      expect(error.name).toBe('StatusAndamentoApiError');
    });

    it('should create error without optional parameters', () => {
      const error = new StatusAndamentoApiError('Test message', 500);
