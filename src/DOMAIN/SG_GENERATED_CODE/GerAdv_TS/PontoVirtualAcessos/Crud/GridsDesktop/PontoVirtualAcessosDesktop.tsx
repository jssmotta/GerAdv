...
export const PontoVirtualAcessosGridDesktopComponent = React.memo(({
  ...
  const [columnsOrder, setColumnsOrder] = useState<string[]>(['rnomeoperador']);

  // Carregar ordem salva do localStorage na inicialização
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem(btoa('pontovirtualacessos-columns-order'));
      if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('pontovirtualacessos-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
