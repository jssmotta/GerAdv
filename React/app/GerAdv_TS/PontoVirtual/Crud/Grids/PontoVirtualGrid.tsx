...
import { PontoVirtualGridMobileComponent } from '../GridsMobile/PontoVirtualMobile';
import { PontoVirtualGridDesktopComponent } from '../GridsDesktop/PontoVirtualDesktop';
...
import { usePontoVirtualFilter } from '../../Hooks/hookPontoVirtualFilter';
...
const PontoVirtualGrid: React.FC = () => {
  ...
  const [pontovirtualData, setPontoVirtualData] = useState<IPontoVirtual[]>([]);
...
const {
  ...
  fetchData, 
  refreshData
} = usePontoVirtualList(pontovirtualService, currFilter);

const handleFetchWithFilter = useCallback(async (filtro?: FilterPontoVirtual | undefined | null) => {
  setCurrFilter(filtro);
  if (filtro && Object.keys(filtro).length > 0) {
    sessionStorage.setItem(btoa('PontoVirtualFilter'), btoa(JSON.stringify(filtro)));
  }
  ...
  const offlineData = await pontovirtualService.getAll(
  filtro ?? {} as FilterPontoVirtual, 
  onlineData => {
    setPontoVirtualData(onlineData);
    setLoading(false);
  }
);
setPontoVirtualData(offlineData);
setLoading(false);

}, []);
const loadFilter = useCallback(() => {
  if (isInitialized) return; // Evita múltiplas chamadas
    const filtro = {} as FilterPontoVirtual;

    const filtroMem = sessionStorage.getItem(btoa('PontoVirtualFilter')) ? JSON.parse(atob(sessionStorage.getItem(btoa('PontoVirtualFilter')) ?? '') || '{}') : {} as FilterPontoVirtual;
...
const {
  ...
  clearFilter, 
  hasActiveFilter
} = usePontoVirtualFilter({ handleFetchWithFilter });
  useEffect(() => {
