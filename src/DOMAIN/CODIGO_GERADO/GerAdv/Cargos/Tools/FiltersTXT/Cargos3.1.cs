...
public static class FilterCargosTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterCargos? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Cargos em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterCargos:

- O filter irá filtrar os Cargos conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Cargos" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Cargos length: {result.Length}");
#endif
