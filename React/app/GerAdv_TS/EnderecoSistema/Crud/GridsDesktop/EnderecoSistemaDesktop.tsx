...
import { useEnderecoSistemaGrid } from '../../Hooks/hookEnderecoSistemaGrid';
...
const [columnsOrder, setColumnsOrder] = useState<string[]>(['nometipoenderecosistema',
'nropastaprocessos',
'nomecidade']);
// Carregar ordem salva do localStorage na inicialização
useEffect(() => {
  try {
    const savedOrder = localStorage.getItem(btoa('enderecosistema-columns-order'));
    if (savedOrder) {
  ...
  if (event.columns) {
    ...
    setColumnsOrder(prevOrder => {
      if (newOrder.length > 0 && JSON.stringify(newOrder) !== JSON.stringify(prevOrder)) {
        ...
        try {
          localStorage.setItem(btoa('enderecosistema-columns-order'), JSON.stringify(newOrder));
        } catch (error) {
...
const basicColumnMap: Record<string, React.ReactElement> = useMemo(() => ({
  ...
  <GridColumn
  key='nometipoenderecosistema'
  field='nometipoenderecosistema'
  title='Tipo Endereco Sistema'
...
const resetColumnsOrder = useCallback(() => {
  ...
  localStorage.removeItem(btoa('enderecosistema-columns-order'));
}, []);
...
const { exportToExcel } = useExportToExcel({
  filename: 'planilha-enderecosistema',
  columns: exportColumns, 
  sheetName: 'EnderecoSistema'
});
const handleExportFiltered = useCallback(() => {
  exportToExcel(filteredData, {
    customFilename: `excel-EnderecoSistema-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToExcel, filteredData]);
const exportColumnsPdf = getExportColumnsPdf(finalGridColumns, columnsState, ['id', 'index']);
const { exportToPdf } = useExportToPdf({
  filename: 'pdf-enderecosistema',
  columns: exportColumnsPdf, 
  title: 'EnderecoSistema'
});
const handleExportFilteredPdf = useCallback(() => {
  exportToPdf(filteredData, {
    customFilename: `pdf-EnderecoSistema-${new Date().toISOString().replace('T', '_').substring(0, 16).replace(':', '-')}`,
  });
}, [exportToPdf, filteredData]);
// ===== DADOS PAGINADOS =====
...
{initialized && (
  <Grid
  key={`enderecosistema-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
  columnMenu={columnMenuComponent}
  className='grid-desktop-crud grid-desktop-enderecosistema'
