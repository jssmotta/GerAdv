...
export const useProTipoBaixaForm = (
  ...
  const loadProTipoBaixa = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar Pro Tipo Baixa";
...
export const useProTipoBaixaList = (
  dataService: IProTipoBaixaService,
  currentFilter?: FilterProTipoBaixa | undefined | null,
) => {
  const [data, setData] = useState<IProTipoBaixa[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterProTipoBaixa | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterProTipoBaixa | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar protipobaixa";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsProTipoBaixa() {
  ...
  function validate(data: IProTipoBaixa): {
    isValid: boolean;
    message: string;
  } {
