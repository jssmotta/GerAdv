...
import { FilterAtividades } from '@/app/GerAdv_TS/Atividades/Filters/Atividades';
...
interface UseAtividadesFilterProps {
    handleFetchWithFilter: (filtro?: FilterAtividades | undefined | null) => Promise<void>;
}

interface UseAtividadesFilterReturn {
    // Estados
    showSearch: boolean;
    windowFilter: FilterAtividades;
    setWindowFilter: React.Dispatch<React.SetStateAction<FilterAtividades>>;

    // Handlers do Dialog
    handleSearch: () => void;
    handleCloseSearch: () => void;
    handleConfirmSearch: (filter: FilterAtividades) => Promise<void>;

    // Render function
    renderInputFilters: (handlers: FilterHandlers<FilterAtividades>) => React.ReactNode;
...
export const useAtividadesFilter = ({ handleFetchWithFilter }: UseAtividadesFilterProps): UseAtividadesFilterReturn => {
    const [showSearch, setShowSearch] = useState(false);
    const [windowFilter, setWindowFilter] = useState<FilterAtividades>({} as FilterAtividades);

    // Handlers do Dialog
    const handleSearch = () => {
        setShowSearch(true);
        const filterWildechar = {...windowFilter, wildcardChar: '%' } as FilterAtividades;
    ...
    const handleConfirmSearch = async (filter: FilterAtividades) => {
        await handleFetchWithFilter(filter);
    };

    // Função para limpar filtros
    const clearFilter = () => {
        setWindowFilter({} as FilterAtividades);
        sessionStorage.removeItem(btoa('AtividadesFilter'));
        handleFetchWithFilter({} as FilterAtividades);
    };
    ...
    const renderInputFilters = (handlers: FilterHandlers<FilterAtividades>) => (        
        <>
        <InputInput
                        type='text'
                        id='descricao'
