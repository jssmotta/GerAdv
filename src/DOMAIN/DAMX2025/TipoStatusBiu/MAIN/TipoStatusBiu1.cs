...
public partial class DBTipoStatusBiu : XCodeIdBase, ICadastros
{
#if (DEBUG)
#if (!shadowsDisabled && !shadows_MenphisSI_GerAdv && !shadows_MenphisSI_GerAdv_TipoStatusBiu)
#pragma warning disable CA1822 // Mark members as static

//public bool CertSignature() => DicionarioDeDadosManagedDatabaseCode.CodeSigntature_DBTipoStatusBiu();
#pragma warning restore CA1822 // Mark members as static

#endif
#endif
#region TableDefinition_TipoStatusBiu
    [XmlIgnore]
    public string TabelaNome => "TipoStatusBiu";

    public DBTipoStatusBiu()
    {
    }

#endregion
    public DBTipoStatusBiu(in int nCodigo, MsiSqlConnection? oCnn) => Carregar(id: nCodigo, oCnn: oCnn);
    public DBTipoStatusBiu(List<SqlParameter> parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        // Tracking: 250605-0
        if (oCnn is null)
            return;
    ...
    public DBTipoStatusBiu(in string? sqlWhere, MsiSqlConnection? oCnn)
    {
...
#endif
#region GravarDados_TipoStatusBiu
#if (forWeb)
...
#endif
    {
        ...
        {
            throw new Exception("Campo 'tsbNome' está vazio!");
        }
        ...
        if (pFldFNome)
            clsW.Fields(DBTipoStatusBiuDicInfo.Nome, m_FNome, ETiposCampos.FString);
        if (insertId != 0)
            return GravaNewId();
