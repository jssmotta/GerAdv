...
export const useGUTAtividadesForm = (
  ...
  const loadGUTAtividades = useCallback(
    async (id: number) => {
      if (!id || id === 0) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar G U T Atividades";
...
export const useGUTAtividadesList = (
  dataService: IGUTAtividadesService,
  currentFilter?: FilterGUTAtividades | undefined | null,
) => {
  const [data, setData] = useState<IGUTAtividades[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const currentFilterRef = useRef<FilterGUTAtividades | undefined | null>(
    currentFilter,
  );
  ...
  const fetchData = useCallback(
    async (filtro?: FilterGUTAtividades | undefined | null) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar gutatividades";
        setError(errorMessage);
        if (process.env.NEXT_PUBLIC_SHOW_LOG === "1")
...
export function useValidationsGUTAtividades() {
  ...
  function validate(data: IGUTAtividades): {
    isValid: boolean;
    message: string;
  } {
