...
public partial class HorasTrabController(IHorasTrabService horastrabService) : ControllerBase
{
    ...
    {
        _logger.Info("HorasTrab: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _horastrabService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("HorasTrab: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _horastrabService.UpdateColumns(parameters, uri);
        if (!result)
        {
