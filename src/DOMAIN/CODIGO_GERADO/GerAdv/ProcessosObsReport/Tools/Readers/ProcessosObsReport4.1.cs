...
FROM {oCnn.UseDbo}.ProcessosObsReport WITH (INDEX = idx_{DBProcessosObsReportDicInfo.PTabelaNome}_AuditorDtAtu)
WHERE [prrCodigo] = @id
OPTION (OPTIMIZE FOR (@id UNKNOWN), FAST 1);";
        using var cmd = new SqlCommand(query, oCnn.InnerConnection);
        cmd.Parameters.AddWithValue("@id", id);
    ...
    public async Task<string> ReadStringAuditorAsync(string uri, MsiSqlConnection? oCnn, IHybridCache cache)
    {
        if (oCnn is null)
            return string.Empty;
        var cacheKey = $"{uri}-{DBProcessosObsReportDicInfo.PTabelaNome}-RRSAA";
...
SELECT TOP(1)
    COALESCE(prrDtAtu, prrDtCad) AS DataReferencia
FROM [{oCnn.UseDbo}].[{DBProcessosObsReportDicInfo.PTabelaNome}] (NOLOCK)
ORDER BY COALESCE(prrDtAtu, prrDtCad) DESC;", oCnn.InnerConnection);
