...
public partial class AlertasController(IAlertasService alertasService) : ControllerBase
{
    ...
    {
        _logger.Info("Alertas: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _alertasService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Alertas: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _alertasService.UpdateColumns(parameters, uri);
        if (!result)
        {
