...
#region ListarDados_GUTAtividadesMatriz
    public static async IAsyncEnumerable<DBGUTAtividadesMatriz> ListarAsync(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        ...
        {
            yield return new DBGUTAtividadesMatriz(dbRec);
        }
    }

    public static IEnumerable<DBGUTAtividadesMatriz> Listar(string? cSqlMain, string? cWhere, string? cOrder, string? cCnn)
    {
        using var oTCnn = ConfiguracoesDBT.GetConnection(cCnn);
        var cSql = new StringBuilder(); // checkpoint 0x3
        if (!cSqlMain.IsEmpty())
