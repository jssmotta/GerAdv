...
const EscritoriosComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IEscritorios | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IEscritorios | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IEscritorios);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <EscritoriosWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedEscritorios={editar ?? addValue ?? EscritoriosEmpty()} /> 
            )}
