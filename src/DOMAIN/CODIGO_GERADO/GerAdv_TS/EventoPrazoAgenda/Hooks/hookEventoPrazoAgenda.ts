...
export const useEventoPrazoAgendaForm = (
  ...
  const loadEventoPrazoAgenda = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialEventoPrazoAgenda);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar Evento Prazo Agenda';
      setError(errorMessage);
      console.log('Erro ao carregar Evento Prazo Agenda');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialEventoPrazoAgenda]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadEventoPrazoAgenda, resetForm]);
  return returnValue;
};


export const useEventoPrazoAgendaNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('EventoPrazoAgenda', (entity) => {
      try {
...
export const useEventoPrazoAgendaList = (dataService: IEventoPrazoAgendaService) => {
  const [data, setData] = useState<IEventoPrazoAgenda[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar eventoprazoagenda';
      setError(errorMessage);
      console.log('Erro ao carregar eventoprazoagenda');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
  ...
  useEventoPrazoAgendaNotifications(
