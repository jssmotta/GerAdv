...
export class AnexamentoRegistrosApiError extends Error {
  status: number;
  code?: string;
  originalError?: any;
  ...
  constructor(message: string, status: number, code?: string, originalError?: any) {
    super(message);
    this.name = 'AnexamentoRegistrosApiError';
    this.status = status;
    this.code = code;
...
export class AnexamentoRegistrosApi {
    ...
    private handleApiError(error: any, context: string): never {
        ...
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AnexamentoRegistros_last_getAll_${max}`);
        try {
            ...
            this.handleApiError(error, 'Erro ao buscar todos os AnexamentoRegistros');
        }
    }
    
        public async getById(id: number): Promise<AxiosResponse> {
        const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AnexamentoRegistros_last_getById_${id}`);
        try {
        ...
        localStorage.setItem(storageKey, encoded);
            ...
            this.handleApiError(error, `Erro ao buscar Anexamento Registros com ID ${id}`);
        }
    }

            public async filter(filtro: FilterAnexamentoRegistros): Promise<AxiosResponse> {
                const storageKey = btoa(`${process.env.NEXT_PUBLIC_APP_ID}-${this.uri}-AnexamentoRegistros_last_filter_data_${JSON.stringify(filtro)}`);
                try {
        ...
        localStorage.setItem(storageKey, encoded);
                    ...
                    this.handleApiError(error, 'Erro ao filtrar AnexamentoRegistros');
                }
            }


    public async validation(regAnexamentoRegistros: IAnexamentoRegistros): Promise<{ isValid: boolean; message: string } | null> {
        try {
            const result = await axios.post(`${this.baseUrl}/validation`, regAnexamentoRegistros as AnexamentoRegistros, this.getHeaders());
    ...
    public async addAndUpdate(regAnexamentoRegistros: IAnexamentoRegistros): Promise<AxiosResponse> {
        try {
            ...
            this.handleApiError(error, 'Erro ao salvar Anexamento Registros');
        }
    }
    ...
    public async delete(id: number): Promise<AxiosResponse | void> {
        try {
            ...
            if (error.response && error.response.status === 409) {
             ...
             this.handleApiError(error, `Erro ao excluir Anexamento Registros com ID ${id}`);                
        }
    }
    ...
    public useFilter(filtro: FilterAnexamentoRegistros) {
       ...
       const key = `${url}::${this.authorization}::${JSON.stringify(filtro)}`;
        ...
        return useSWR<AnexamentoRegistros[]>(key, fetcher, {
          ...
          onError: (error) => {
            console.error('Erro no SWR para filtro de anexamentoregistros:', error);
            this.notificationService.notify({
              entity: 'AnexamentoRegistros',
              id: 0,
              action: NotifySystemActions.ERROR,
              message: 'Erro ao carregar dados dos anexamentoregistros'
            });
          }
        });
    }
