...
public partial class FTerceiros : MenphisSI.SG.GerAdv.DBTerceiros, IDBTerceiros
{
    // DI-compatible default constructor
    public FTerceiros() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FTerceiros CreateFromReader(SqlDataReader? dbRec)
    {
        return new FTerceiros(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FTerceiros CreateFromDataRow(DataRow? dbRec)
    {
        return new FTerceiros(dbRec);
    }

    private FTerceiros(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FTerceiros(DataRow? dbRec) : base(dbRec)
    {
    }

    private FTerceiros(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FTerceiros CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FTerceiros(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FTerceiros> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fTerceiros = new FTerceiros();
        await fTerceiros.CarregarAsync(id, oCnn);
        return fTerceiros;
    }
