...
interface RitoFormProps {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
    ...
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao submeter formulário de Rito:", error);
  ...
  const handleDirectSave = () => {
    if (!isSubmitting) {
      ...
      try {
        ...
        console.error("Erro ao salvar Rito diretamente:", error);
        setIsSubmitting(false);
        if (onError) onError();
      }
    }
  ...
  <>
        ...
        <div className={isMobile ? 'form-container-Rito' : 'form-container5 form-container-Rito'}>
       
            <form className='formInputCadInc' id={`RitoForm-${ritoData.id}`} onSubmit={onConfirm}>

                <ButtonsCrud data={ritoData} isSubmitting={isSubmitting} onClose={onClose} formId={`RitoForm-${ritoData.id}`} preventPropagation={true} onSave={handleDirectSave} />
