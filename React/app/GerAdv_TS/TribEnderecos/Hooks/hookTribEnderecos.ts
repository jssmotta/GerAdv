...
export const useTribEnderecosList = (dataService: ITribEnderecosService, currentFilter?: FilterTribEnderecos | undefined | null) => {
  const [data, setData] = useState<ITribEnderecos[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterTribEnderecos | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterTribEnderecos | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterTribEnderecos);
      setData(result);
    } catch (err) {
