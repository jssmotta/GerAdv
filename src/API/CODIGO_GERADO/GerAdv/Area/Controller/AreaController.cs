...
public partial class AreaController(IAreaService areaService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Filter([FromQuery] int max, [FromBody] Filters.FilterArea filtro, [FromRoute, Required] string uri)
    {
        ...
        //_logger.Info("Area: Filter called max {0} with filtro = {1}, {2}", max, filtro, uri);
        var result = await _areaService.Filter(max, filtro, uri);
        return Ok(result);
    }
