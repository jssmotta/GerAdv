...
import { useProValoresGrid } from "../../Hooks/hookProValoresGrid";
  ...
  ({
    ...
    const basicColumnMap: Record<string, React.ReactElement> = useMemo(
      () => ({
        ...
        descricaotipovalorprocesso: (
          <GridColumn
            key="descricaotipovalorprocesso"
            field="descricaotipovalorprocesso"
            title="TipoValorProcesso"
            sortable={false}
            filterable={false}
          />
        ) /* Track G.04 */,
    ...
    const resetColumnsOrder = useCallback(() => {
      ...
      localStorage.removeItem(btoa("provalores-columns-order"));
    }, []);
    ...
    const { exportToExcel } = useExportToExcel({
      filename: "planilha-provalores",
      columns: exportColumns,
      sheetName: "ProValores",
    });

    const handleExportFiltered = useCallback(() => {
      exportToExcel(filteredData, {
        customFilename: `excel-ProValores-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToExcel, filteredData]);
    ...
    const handleExportFilteredPdf = useCallback(() => {
      exportToPdf(filteredData, {
        customFilename: `pdf-ProValores-${new Date().toISOString().replace("T", "_").substring(0, 16).replace(":", "-")}`,
      });
    }, [exportToPdf, filteredData]);
    ...
    return (
      <>
        ...
        {initialized && (
          <Grid
            key={`provalores-grid-${JSON.stringify(columnsOrder)}-${JSON.stringify(columnsState)}`}
            columnMenu={columnMenuComponent}
            className="grid-desktop-crud grid-desktop-provalores"
