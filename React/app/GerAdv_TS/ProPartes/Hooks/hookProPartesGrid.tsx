...
// useProPartesGrid.ts
// Hook centralizado para lógicas compartilhadas dos grids de ProPartes
'use client';
...
interface UseProPartesGridProps {
  data: IProPartes[];
  ...
  const handleColumnsReorder = useCallback((event: any) => {
    ...
    .filter((col: any) =>
      ...
      if (reorderableFields.length > 0 && JSON.stringify(reorderableFields) !== JSON.stringify(columnsOrder)) {
        ...
        try {
          localStorage.setItem(btoa('propartes-columns-order'), JSON.stringify(reorderableFields));
