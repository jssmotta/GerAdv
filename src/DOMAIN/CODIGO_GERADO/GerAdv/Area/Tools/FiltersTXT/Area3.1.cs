...
public static class FilterAreaTXT
{ 
    ...
    public static (string, string) GetPrompt(FilterArea? existingFilter, JsonSerializerOptions options)
...
Você é um assistente que ajuda a preencher filtros para buscar Area em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterArea:

- O filter irá filtrar os Area conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
- Se ouvir "todos os Area" ou similar, deixe todos os campos com valores padrão;
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt Area length: {result.Length}");
#endif
