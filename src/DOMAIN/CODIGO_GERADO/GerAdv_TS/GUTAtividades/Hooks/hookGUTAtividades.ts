...
export const useGUTAtividadesForm = (
  ...
  const loadGUTAtividades = useCallback(async (id: number) => {
    if (!id || id === 0) {
      setData(initialGUTAtividades);
      return;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar G U T Atividades';
      setError(errorMessage);
      console.log('Erro ao carregar G U T Atividades');
    } finally {
      setLoading(false);
    }
  }, [dataService, initialGUTAtividades]);
  ...
  const returnValue = useMemo(() => ({
    ...
    resetForm,
    setData
  }), [data, loading, error, handleChange, loadGUTAtividades, resetForm]);
  return returnValue;
};


export const useGUTAtividadesNotifications = (
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNotifications('GUTAtividades', (entity) => {
      try {
...
export const useGUTAtividadesList = (dataService: IGUTAtividadesService) => {
  const [data, setData] = useState<IGUTAtividades[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async (filtro?: any) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar gutatividades';
      setError(errorMessage);
      console.log('Erro ao carregar gutatividades');
    } finally {
      setLoading(false);
    }
  }, [dataService]);
