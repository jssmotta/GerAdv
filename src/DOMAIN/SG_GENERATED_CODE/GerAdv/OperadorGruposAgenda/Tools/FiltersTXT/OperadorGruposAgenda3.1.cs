...
public static class FilterOperadorGruposAgendaTXT
{ 
...
Você é um assistente que ajuda a preencher filtros para buscar OperadorGruposAgenda em um sistema de banco de dados relacionados.

Instruções para preenchimento do FilterOperadorGruposAgenda:

- O filter irá filtrar os OperadorGruposAgenda conforme os campos preenchidos, o usuário pode pedir de várias formas para você buscar os dados.
...
CAMPOS filterOperadorGruposAgenda:(Operador Grupos Agenda)
- sqlwhere: SQLWhere;
- nome: _nome ou descrição para Operador Grupos Agenda;
...
- Se ouvir "todos os OperadorGruposAgenda" ou similar, deixe todos os campos com valores padrão;
...
"""; 
    public static (string, string) GetPrompt(FilterOperadorGruposAgenda? existingFilter, JsonSerializerOptions options)
...
#if DEBUG   
        Console.WriteLine($"DEBUG: Prompt OperadorGruposAgenda length: {result.Length}");
#endif
