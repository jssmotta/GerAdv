...
public partial class AgendaSemanaController : ControllerBase
{
    ...
    [HttpPost("{data}")]
    [Authorize]
    public async Task<IActionResult> Filter30(string data, int paciente, int isMobile, string tenantKey)
    {
        if (DateTime.TryParse(data, out var dia))
        {
            _logger.Info("AgendaSemana: Filter30 called with filtro = {0}", dia);
#pragma warning disable CS8602 // Dereference of a possibly null reference.
            var result = await _agendasemanaService.Filter30(Convert.ToDateTime(dia), paciente, isMobile, tenantKey) ?? [];
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    [Authorize]
    public async Task<IActionResult> Monta(string dataInicial, bool isMobile, string tenantKey)
    {
...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
        var result = await _agendasemanaService.Monta(data, isMobile, tenantKey);
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        return Ok(result);
    }
