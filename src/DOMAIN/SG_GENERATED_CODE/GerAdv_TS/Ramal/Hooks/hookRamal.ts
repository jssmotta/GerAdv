...
export const useRamalList = (dataService: IRamalService, currentFilter?: FilterRamal | undefined | null) => {
  const [data, setData] = useState<IRamal[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterRamal | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterRamal | undefined | null) => {
