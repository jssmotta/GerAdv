...
const RamalComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<IRamal | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<IRamal | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as IRamal);
    ...
    React.useEffect(() => {
        const unsubscribe = subscribeToNotifications("Ramal", (entity) => {
      try {
                fetchDados();                
        ...
        <>
          {(editar || addValue) && isOpen && (
            <RamalWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedRamal={editar ?? addValue ?? RamalEmpty()} /> 
            )}
