...
public partial class ProcessosController(IProcessosService processosService) : ControllerBase
{
    ...
    {
        _logger.Info("Processos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _processosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Processos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _processosService.UpdateColumns(parameters, uri);
        if (!result)
        {
