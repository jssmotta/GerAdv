...
const CidadeComboBox: React.FC<DadosSelectProps> = ({ name, value, setValue, label, dataForm }) => {
    ...
    const [addValue, setAddValue] = useState<ICidade | null>(null);
    const [action, setAction] = useState(ActionAdicionar);
    const [editar, setEditar] = useState<ICidade | null>(null);    
    ...
    useEffect(() => {       
        if (typeof value === 'number' && !isNaN(value) && value > 0) {
            const fetchData = async () => {
                ...
                setEditar(response.data as ICidade);
        ...
        <>
          {(editar || addValue) && isOpen && (
            <CidadeWindow
                ...
                onSuccess={onClose}
                onError={onClose}
                selectedCidade={editar ?? addValue ?? CidadeEmpty()} /> 
            )}
