...
public partial class FOponentes : MenphisSI.SG.GerAdv.DBOponentes, IDBOponentes
{
    // DI-compatible default constructor
    public FOponentes() : base()
    {
    }

    // Factory method for creating instances with SqlDataReader
    public static FOponentes CreateFromReader(SqlDataReader? dbRec)
    {
        return new FOponentes(dbRec);
    }

    // Factory method for creating instances with DataRow
    public static FOponentes CreateFromDataRow(DataRow? dbRec)
    {
        return new FOponentes(dbRec);
    }

    private FOponentes(SqlDataReader? dbRec) : base(dbRec)
    {
    }

    private FOponentes(DataRow? dbRec) : base(dbRec)
    {
    }

    private FOponentes(List<SqlParameter>? parameters, in string? cNome = "", MsiSqlConnection? oCnn = null, string? fullSql = "", string sqlWhere = "", in string join = "") : base(parameters, cNome, oCnn, fullSql, sqlWhere, join)
    {
    }

    // Factory method for creating instances with parameters
    public static FOponentes CreateFromParameters(List<SqlParameter>? parameters, MsiSqlConnection? oCnn, in string? cNome = "", string? fullSql = "", string sqlWhere = "", in string join = "")
    {
        return new FOponentes(parameters, cNome, oCnn, fullSql, sqlWhere, join);
    }

    public static async Task<FOponentes> CreateFromIdAsync(int id, MsiSqlConnection? oCnn)
    {
        var fOponentes = new FOponentes();
        await fOponentes.CarregarAsync(id, oCnn);
        return fOponentes;
    }
