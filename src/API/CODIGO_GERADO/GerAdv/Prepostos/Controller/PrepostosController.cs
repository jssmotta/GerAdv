...
public partial class PrepostosController(IPrepostosService prepostosService) : ControllerBase
{
    ...
    {
        _logger.Info("Prepostos: GetColumns called with id = {0} and columns = {1}, {2}", parameters.Id, parameters.Columns, uri);
        var result = await _prepostosService.GetColumns(parameters, uri);
        if (result == null)
        {
    ...
    {
        _logger.Info("Prepostos: UpdateColumns called with id = {0} and campos = {1}, {2}", parameters.Id, parameters, uri);
        var result = await _prepostosService.UpdateColumns(parameters, uri);
        if (!result)
        {
