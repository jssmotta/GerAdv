    ...
    private static (string where, List<SqlParameter> parametros)? WFiltro(Filters.FilterHorasTrab filtro)
    {
        ...
        if (!filtro.Data.IsEmpty())
        {
            if (DateTime.TryParse(filtro.Data, out var dataParam))
                parameters.Add(new($"@{nameof(DBHorasTrabDicInfo.Data)}", dataParam));
        }

        if (!filtro.Data_end.IsEmpty())
        {
            if (DateTime.TryParse(filtro.Data_end, out var dataParam))
                parameters.Add(new($"@{nameof(DBHorasTrabDicInfo.Data)}_end", dataParam));
        }
        ...
        {
            cWhere.Append(filtro.Data.IsEmpty() ? string.Empty : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"CONVERT(DATE,[{DBHorasTrabDicInfo.PTabelaNome}].[{DBHorasTrabDicInfo.Data}], 103) >= CONVERT(DATE, @{nameof(DBHorasTrabDicInfo.Data)}, 103)");
        }
        else
        {
            cWhere.Append((filtro.Data.IsEmpty() && filtro.Data_end.IsEmpty()) ? string.Empty : (!(filtro.Data.IsEmpty()) && !(filtro.Data_end.IsEmpty())) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBHorasTrabDicInfo.Data} BETWEEN @{nameof(DBHorasTrabDicInfo.Data)} AND @{nameof(DBHorasTrabDicInfo.Data)}_end" : !(filtro.Data.IsEmpty()) ? (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBHorasTrabDicInfo.Data} = @{nameof(DBHorasTrabDicInfo.Data)}" : (cWhere.Length == 0 ? string.Empty : filtro.LogicalOperator) + $"{DBHorasTrabDicInfo.Data} <= @{nameof(DBHorasTrabDicInfo.Data)}_end");
        }
