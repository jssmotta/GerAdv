// app/TestsForms/CidadeComboBox.test.tsx - Vers�o Corrigida
...
import { CidadeEmpty, CidadeTestEmpty } from '@/app/GerAdv_TS/Models/Cidade';
// Mock do hook antes dos imports
jest.mock('@/app/GerAdv_TS/Cidade/Hooks/hookCidade', () => ({
  useCidadeComboBox: jest.fn(() => ({
    options: [
    { ...CidadeEmpty(), id: 1, nome: 'Cidade 1' },
    { ...CidadeEmpty(), id: 2, nome: 'Cidade 2' },
    ], 
    loading: false, 
    selectedValue: null, 
...
jest.mock('@/app/GerAdv_TS/Cidade/Apis/ApiCidade', () => ({
  CidadeApi: jest.fn().mockImplementation(() => ({
    getById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cidade' }),
    getAll: jest.fn().mockResolvedValue([]), 
    filter: jest.fn().mockResolvedValue([]), 
    addAndUpdate: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cidade' }),
    delete: jest.fn().mockResolvedValue(true), 
  })), 
}));
jest.mock('@/app/GerAdv_TS/Cidade/Services/Cidade.service', () => ({
  CidadeService: jest.fn().mockImplementation(() => ({
    fetchCidadeById: jest.fn().mockResolvedValue({ id: 1, nome: 'Mock Cidade' }),
    getList: jest.fn().mockResolvedValue([]), 
  })), 
}));
// Mock do Window
jest.mock('@/app/GerAdv_TS/Cidade/Crud/Grids/CidadeWindow', () => {
  return function MockCidadeWindow(props: any) {
    return props.isOpen ? (
    <div data-testid='cidade-window'>
  ...
  >
  <option value=''>Select {props.textField === 'nome' ? 'Cidade' : 'Item'}</option>
  ...
  const mockOptions = [
  { ...CidadeTestEmpty(), id: 1, nome: 'Cidade 1' },
  { ...CidadeTestEmpty(), id: 2, nome: 'Cidade 2' },
];
...
const defaultProps = {
  ...
  value: 0, 
  setValue: jest.fn(), 
  dataForm: CidadeEmpty(), 
};
describe('CidadeComboBox', () => {
  ...
  const { useCidadeComboBox } = require('@/app/GerAdv_TS/Cidade/Hooks/hookCidade');
  (useCidadeComboBox as jest.Mock).mockReturnValue({
    options: mockOptions, 
    loading: false, 
    selectedValue: null, 
    handleFilter: jest.fn(), 
  ...
  it('renders label and ComboBox', () => {
    render(<CidadeComboBox {...defaultProps} />);
    ...
    expect(screen.getByText('Select Cidade')).toBeInTheDocument();
  });
  it('calls setValue on ComboBox change', () => {
    ...
    expect(defaultProps.setValue).toHaveBeenCalledWith({ ...CidadeTestEmpty(), id: 1, nome: 'Cidade 1' });
  });
  it('shows loading state', () => {
    // Mock loading state
    const { useCidadeComboBox } = require('@/app/GerAdv_TS/Cidade/Hooks/hookCidade');
    useCidadeComboBox.mockReturnValue({
      options: [], 
      loading: true, 
      selectedValue: null, 
      handleFilter: jest.fn(), 
  ...
  it('opens CidadeWindow on action button click (add)', async () => {
    render(<CidadeComboBox {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('cidade-window')).toBeInTheDocument();
    });
  });
  ...
  it('handles window close', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('cidade-window')).not.toBeInTheDocument();
    });
  });
  ...
  it('handles invalid value gracefully', () => {
    ...
    render(<CidadeComboBox {...propsWithInvalidValue} />);
    const combobox = screen.getByRole('combobox');
    expect(combobox).toHaveValue('');
  });
  it('handles CidadeWindow onClose and onSuccess', () => {
    render(<CidadeComboBox {...defaultProps} />);
    // Abrir a janela
    const addButton = screen.getByTestId('svg-icon-plus-icon');
    fireEvent.click(addButton.closest('label')!);
  ...
  it('handles edit mode when value is provided', () => {
    ...
    render(<CidadeComboBox {...propsWithValue} />);
    const combobox = screen.getByRole('combobox');
