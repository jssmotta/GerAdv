...
export const BensMateriaisForm: React.FC<BensMateriaisFormProps> = ({
  ...
  const dadoApi = new BensMateriaisApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  ...
  const [_nomeBensClassificacao, setNomeBensClassificacao] = useState("");
  const bensclassificacaoApi = new BensClassificacaoApi(
    systemContext?.Uri ?? "",
    systemContext?.Token ?? "",
  );
  const [_nomeFornecedores, setNomeFornecedores] = useState("");
  ...
  if (getParamFromUrl("bensclassificacao") > 0) {
    if (
      bensmateriaisData.id === 0 &&
      bensmateriaisData.bensclassificacao == 0
    ) {
      ...
      bensmateriaisData.bensclassificacao =
        getParamFromUrl("bensclassificacao");
    }
  }
  ...
  const addValorFornecedor = (e: any) => {
    if (e?.id > 0) onChange({ target: { name: "fornecedor", value: e.id } });
  };
      ...
      try {
        const syntheticEvent = {
          ...
          target: document.getElementById(
            `BensMateriaisForm-${bensmateriaisData.id}`,
          ),
  ...
  useEffect(() => {
    ...
    if (el) {
      el.textContent =
        bensmateriaisData?.id == 0
          ? "Editar BensMateriais"
          : "Adicionar Bens Materiais";
    }
  ...
  return (
    <>
      <div
        className={
          isMobile
            ? "form-container form-container-BensMateriais"
            : "form-container form-container-BensMateriais"
        }
      >
        <form
          className="formInputCadInc"
          id={`BensMateriaisForm-${bensmateriaisData.id}`}
          onSubmit={onConfirm}
        >
          {!isMobile && (
            <ButtonSalvarCrud
              ...
              onClose={onClose}
              formId={`BensMateriaisForm-${bensmateriaisData.id}`}
          ...
          <div className="grid-container">
            ...
            <InputCheckbox
              ...
              label="GarantiaLoja"
              name="garantialoja"
              checked={bensmateriaisData.garantialoja}
              onChange={onChange}
            />
        ...
        <FooterIncDocker>
          <CopyButton formId={`BensMateriaisForm-${bensmateriaisData.id}`} />

          <PrintButton formId={`BensMateriaisForm-${bensmateriaisData.id}`} />

          <AuditorButton
            tableName="BensMateriais"
            coolName="Bens Materiais"
            id={bensmateriaisData.id}
          />

          <DeleteButton
            page={"/pages/bensmateriais"}
            id={bensmateriaisData.id}
            dadoApi={dadoApi}
          />
        </FooterIncDocker>
