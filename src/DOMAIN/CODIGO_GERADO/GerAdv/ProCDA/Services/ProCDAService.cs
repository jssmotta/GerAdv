...
public partial class ProCDAService(IOptions<AppSettings> appSettings, IProCDAReader reader, IProCDAValidation validation, IProCDAWriter writer, IProcessosReader processosReader, IHttpContextAccessor httpContextAccessor, HybridCache cache) : IProCDAService, IDisposable
{
    private readonly IHttpContextAccessor _httpContextAccessor = httpContextAccessor;
    private readonly string _uris = appSettings.Value.ValidUris;
    private readonly HybridCache _cache = cache;
    private bool _disposed;
    public async Task<IEnumerable<ProCDAResponse>> GetAll(int max, [FromRoute, Required] string uri, CancellationToken token = default)
    {
    ...
    private async Task<IEnumerable<ProCDAResponse>> GetDataAllAsync(int max, string uri, CancellationToken token)
    {
        var query = $@"SELECT DISTINCT TOP {max} 
                   {DBProCDA.SensivelCamposSqlX} 
                   FROM {DBProCDA.PTabelaNome} (NOLOCK)
                   ORDER BY {DBProCDADicInfo.CampoNome}
                   OPTION (OPTIMIZE FOR UNKNOWN)";
        var connection = Configuracoes.ConnectionByUri(uri);
        var lista = new List<DBProCDA>(max);
        await foreach (var item in DBProCDA.ListarAsync(query, string.Empty, string.Empty, connection).WithCancellation(token).ConfigureAwait(false))
        {
            if (item != null)
            {
                lista.Add(item);
    ...
    public async Task<IEnumerable<ProCDAResponse>> Filter(Filters.FilterProCDA filtro, [FromRoute, Required] string uri)
    {
        ThrowIfDisposed();
        if (!Uris.ValidaUri(uri, _uris))
        {
        ...
        return await Task.Run(() =>
        {
            ...
            var result = new List<ProCDAResponse>();
            var cWhere = filtro == null ? string.Empty : WFiltro(filtro!);
            var list = DBProCDA.Listar("", cWhere, "", Configuracoes.ConnectionByUri(uri));
            if (list != null)
            {
    ...
    public async Task<ProCDAResponse?> GetById([FromQuery] int id, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        {
            throw new Exception($"ProCDA - {uri}-: GetById - {ex.Message}");
        }
    }

    private async Task<ProCDAResponse?> GetDataByIdAsync(int id, string uri, CancellationToken token)
    {
        return await Task.Run(() =>
        {
    ...
    public async Task<ProCDAResponse?> Delete([FromQuery] int id, [FromRoute, Required] string uri)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            if (procda != null)
            {
                new DBProCDA().DeletarItem(procda.Id, oCnn, null);
            }
    ...
    public async Task<ProCDAResponse?> GetByName(string name, [FromRoute, Required] string uri)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            var cWhere = $"{DBProCDADicInfo.CampoNome} like '{name.PreparaParaSql()}'";
    ...
    public async Task<IEnumerable<NomeID>> GetListN([FromQuery] int max, [FromBody] Filters.FilterProCDA? filtro, [FromRoute, Required] string uri, CancellationToken token)
    {
        ...
        var cWhere = filtro == null ? string.Empty : WFiltro(filtro!);
        var cacheKey = $"{uri}-ProCDA-{max}-{cWhere.GetHashCode()}-GetListN";
        var entryOptions = new HybridCacheEntryOptions
        {
            Expiration = TimeSpan.FromSeconds(BaseConsts.PMaxSecondsCacheId),
            LocalCacheExpiration = TimeSpan.FromSeconds(BaseConsts.PMaxSecondsCacheId)
    ...
    private static async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string uri, string cWhere, CancellationToken token)
    {
        return await Task.Run(() =>
        {
            ...
            foreach (var item in DBProCDA.ListarN(cWhere, DBProCDADicInfo.CampoNome, Configuracoes.ConnectionByUri(uri), max: max))
            {
    ...
    private static string WFiltro(Filters.FilterProCDA filtro)
    {
