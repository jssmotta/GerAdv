...
export const useProcessOutputSourcesList = (dataService: IProcessOutputSourcesService, currentFilter?: FilterProcessOutputSources | undefined | null) => {
  const [data, setData] = useState<IProcessOutputSources[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); 
  const currentFilterRef = useRef<FilterProcessOutputSources | undefined | null>(currentFilter);

  useEffect(() => {
    currentFilterRef.current = currentFilter;
  }, [currentFilter]);

  const fetchData = useCallback(async (filtro?: FilterProcessOutputSources | undefined | null) => {
   
    setLoading(true);
    setError(null);
    
    try {
      const result = await dataService.getAll(filtro ?? {} as FilterProcessOutputSources);
      setData(result);
    } catch (err) {
