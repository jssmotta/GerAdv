...
public class FProTipoBaixaFactoryTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private FProTipoBaixaFactory _factory;
    private Mock<MsiSqlConnection> _mockConnection;
    private Mock<SqlDataReader> _mockReader;
    private DataRow _validDataRow;
    public FProTipoBaixaFactoryTests(ITestOutputHelper output)
    {
        _output = output;
        _factory = new FProTipoBaixaFactory();
        _mockConnection = new Mock<MsiSqlConnection>();
        _mockReader = new Mock<SqlDataReader>();
        // Prepare a real DataRow with the expected schema (at minimum CampoCodigo: "ptxCodigo")
        var dt = new DataTable();
        dt.Columns.Add("ptxCodigo", typeof(int));
        var row = dt.NewRow();
        // Set as DBNull to allow constructor to return early without requiring all columns
        row["ptxCodigo"] = DBNull.Value;
        dt.Rows.Add(row);
    ...
    public void Create_ShouldReturnNewInstance()
    {
        ...
        result.Should().BeOfType<FProTipoBaixa>();
    }
    ...
    {
        // Arrange
        var factory = new FProTipoBaixaFactory();
        // Act
        factory.Dispose();
...
#region Helper Methods    
    ...
    public async Task CreateFromIdAsync_WithValidId_ShouldReturnInstanceWithData()
    {
        ...
        mockDataTable.Columns.Add("ptxCodigo", typeof(int));
        mockDataTable.Columns.Add("ptxNome", typeof(string));
        var row = mockDataTable.NewRow();
        row["ptxCodigo"] = testId;
        row["ptxNome"] = "Test ProTipoBaixa";
    ...
    public void CreateFromDataRow_WithCompleteDataRow_ShouldPopulateAllProperties()
    {
        ...
        var row = dt.NewRow();
        row["ptxCodigo"] = 123;
        row["ptxNome"] = "Test ProTipoBaixa Description";
        dt.Rows.Add(row);
        // Act
        ...
        // Add more property assertions based on FProTipoBaixa properties
        }
    }
