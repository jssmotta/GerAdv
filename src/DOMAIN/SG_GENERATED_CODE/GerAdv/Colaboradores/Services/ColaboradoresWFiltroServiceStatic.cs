...
{
    private async Task<IEnumerable<ColaboradoresResponseAll>> GetDataAllAsync(MsiSqlConnection oCnn, int max, string where, List<SqlParameter>? parameters, string tenantKey, CancellationToken cancellationToken)
    {
    ...
    public async Task<ResultApi<IEnumerable<NomeID>>> GetListN(int max, Filters.FilterColaboradores? filtro, string tenantKey, CancellationToken token = default)
    {
        // Tracking: 20250606-0
        ThrowIfDisposed();
        ...
        var cacheKey = $"{tenantKey}-Colaboradores-{max}-{filterHash}-GetListN-{keyCache}";
        ...
        {
            _logger.Error(ex, "Colaboradores: GetListN failed");
    ...
    private async Task<IEnumerable<NomeID>> GetDataListNAsync(int max, string tenantKey, string where, List<SqlParameter>? parameters, CancellationToken token)
    {
        ...
        var lista = await reader.ListarNAsync(max, tenantKey, where, parameters, DBColaboradoresDicInfo.CampoNome);
    ...
    public async Task<ResultApi<IEnumerable<ColaboradoresResponseAll>>> GetAll(int max, string tenantKey, CancellationToken token = default)
    {
        ThrowIfDisposed();
        ...
        using var oCnn = scope.Connection;
        var keyCache = await reader.ReadStringAuditorAsync(tenantKey, oCnn, _cache);
        var cacheKey = $"{tenantKey}-Colaboradores-Filter-{max}-{keyCache}";
        try
        {
        ...
        {
            _logger.Error(ex, "Colaboradores: GetAll failed for tenantKey = {0}", tenantKey);
            return ResultApi<IEnumerable<ColaboradoresResponseAll>>.Fail(ex.Message, 500);
        }
    }

    private async Task<IEnumerable<ColaboradoresResponseAll>> GetDataAllAsync2(MsiSqlConnection oCnn, int max, string where, List<SqlParameter>? parameters, string tenantKey, CancellationToken token)
    {
        if (oCnn == null || oCnn?.InnerConnection?.State == ConnectionState.Closed)
        {
